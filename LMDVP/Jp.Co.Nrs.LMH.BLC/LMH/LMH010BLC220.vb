...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC220 = New LMH010DAC220()
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    Private Function SetSemiInkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_AGCW").NewRow()
        ...
        '荷主固有データ
        drEdiRcvDtl("ARR_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_1").ToString().Trim(), 8)              '納品日
        drEdiRcvDtl("CUST_GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_2").ToString().Trim(), 20)        '商品コード
        drEdiRcvDtl("GOODS_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 60)             '商品名
        drEdiRcvDtl("LOT_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 40)               'ロットNo.
        drEdiRcvDtl("IRIME") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 10)                '入目
        drEdiRcvDtl("QT") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 10)                   '本数

        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "1"                                                          '実績処理フラグ

        'データセットに設定
        ds.Tables("LMH010_INKAEDI_DTL_AGCW").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI入荷(中))"
    ...
    Private Function SetSemiInkaEdiM(ByVal setDs As DataSet _
        ...
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH010_INKAEDI_DTL_AGCW").Rows(0)
        ...
        drInkaEdiM("NB") = drRcvEdiDtl("QT")
        drInkaEdiM("HASU") = drRcvEdiDtl("QT")
        drInkaEdiM("IRIME") = drRcvEdiDtl("IRIME")
...
#Region "セミEDI時　データセット設定(EDI入荷(大))"
    ...
    Private Function SetSemiinkaediL(ByVal setDs As DataSet) As DataSet
        ...
        drInkaEdiL("INKA_DATE") = drRcvEdiDtl("ARR_DATE")
        ...
        drInkaEdiL("HOKAN_STR_DATE") = drRcvEdiDtl("ARR_DATE")
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_AGCW")
    ...
    Private Function SemiEdiTorikomiChk(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To hedmax
            ...
            Else
                ...
                'ADD START 2019/9/20 依頼番号:007533
                '納品日(COLUMN_1)の取得値が怪しいので統一する
...
#Region "カラム項目の値の先頭" '"を取る"
    ...
    Public Sub ApostropheCutEdit(ByVal dr As DataRow)
        ...
        dr.Item("COLUMN_1") = Me._Blc.ApostropheCut(dr.Item("COLUMN_1").ToString())
...
#Region "商品明細マスタ"
    ...
    Public Function TorikomiMstChk(ByVal dr As DataRow, ByVal ds As DataSet) As Boolean
        ...
        sGoodsCd = dr.Item("COLUMN_2").ToString()       '商品コード
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_AGCW")         'EDI受信Dtl
        ...
        '対象データのみソート
        Dim strSort As String = "COLUMN_1"  '納品日
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            ds.Tables("LMH010_INKAEDI_DTL_AGCW").Clear() '受信DTLをクリア

            ds = Me.SetSemiInkaEdiRcv(ds, i)

            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_AGCW").Rows(0)
            ...
            If Convert.ToDecimal(drEdiRcvDtl.Item("QT").ToString()) = 0.0 Then
                Continue For
            End If

            '納品日が同じ場合、同じEDI_CTL_NOにする
            ...
            '要望管理009031：AGC若狭：最初期の仕様として実装したが要望により削除
            ''---------------------------------------------------------------------------
            '' EDI受信データを検索して、既に同一納品日が存在する場合はスルーする（キーが変わったタイミングで調査）
            ''---------------------------------------------------------------------------
            'If bSameKeyFlg = False OrElse bBeforeDup = True Then
            '    If Me._Dac.GetInkaEdiRcvDtlCount(ds) > 0 Then
            '        MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E285", New String() {String.Concat("納品日:", drEdiRcvDtl.Item("ARR_DATE").ToString())}, drEdiRcvDtl.Item("REC_NO").ToString().Trim(), LMH030BLC.EXCEL_COLTITLE_SEMIEDI, drEdiRcvDtl.Item("FILE_NAME").ToString().Trim())
            '        bBeforeDup = True
            '        Continue For
            '    Else
            '        bBeforeDup = False
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH010_INKAEDI_DTL_AGCW")
            setDtlDt.Clear()
