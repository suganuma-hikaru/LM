...
'  EDI荷主ID　　　　:  163　　　        : ENEOS(2023/01/11(REV.1910)のセミEDI標準仕様000からコピー)
'  作  成  者       :  
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC163 = New LMH030DAC163()
...
#If True Then 'フィルメニッヒ セミEDI対応  20160926 added inoue
    ...
    ''' </summary>
    ''' <remarks>
    ''' 届先コードの前方一致検索、ワーニング画面の入目付検索/届先コード付検索を追加
    ''' </remarks>
    Private _IsFirmeType As Boolean = False
...
#Region "CONST"
    ...
    '2018/04/11 000291 【LMS】納品書_横浜古川エージェンシ-注文番号(M単位)表示(横浜福永) Annen add start
    Private Const CUST_CD_FURUKAWA As String = "00448"
    '2018/04/11 000291 【LMS】納品書_横浜古川エージェンシ-注文番号(M単位)表示(横浜福永) Annen add end
...
#If True Then 'フィルメニッヒ セミEDI対応  20160926 added inoue
        ' ToDo: フィルメニッヒのセミEDIを共通から別クラスへ分離後にフィルメ固有処理を削除する
        _IsFirmeType = Me.IsFirmeType(ds)
#End If
...
#Else
            If (_IsFirmeType) Then

                Dim choiceKb As String = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.STD_WID_L010, 0)

                If WARNING_CHOICE_KB.SELECT_YES.Equals(choiceKb) = False Then
                    ' 前方一致検索
                    ds = MyBase.CallDAC(Me._Dac, "SelectLikeDataMdest", ds)
                Else
                    ' 完全一致検索
                    ds = MyBase.CallDAC(Me._DacCom, "SelectDataMdest", ds)
                End If
...
#If True Then   'ADD 2019/12/11 009299【LMS】フィルメEDI出荷登録_数量10倍誤出荷理由調査
...
#End If
        ...
        '商品コード
        ediMDr("CUST_GOODS_CD") = mGoodsDr("GOODS_CD_CUST")

        '商品名(マスタの値で入替は行わない。必ずEDIで送られてくる値を使用)
        'ediMDr("GOODS_NM") = ediMDr("FREE_C04")    '<<商品が見つからないなら素直にFREE_C04を見ること
        ediMDr("GOODS_NM") = mGoodsDr("GOODS_NM_1") '<<商品マスタの値で登録(LMSに合わせる)
        ...
        Select Case alctdKb
            ...
            Case "01"
                ...
                'SHINODA 2014/11/13 ナフコセミEDI対応 E N D
            Case "02"
...
#Else
            If (_IsFirmeType) Then
                ...
                ds = Me._Blc.SetComWarningL("W208" _
                                           , LMH030BLC.STD_WID_L010 _
                                           , ds _
                                           , msgArray _
                                           , workDestCd _
                                           , String.Empty _
...
#Region "EDI出荷(中)のDAC側でのチェック + 初期値設定"
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                Else
                    ...
                    'セミEDI取込時に商品複数ありだった場合、かつ、EDIワーニング・エラーチェック画面表示前の場合
                    If dtM.Rows(i)("GOODSM_FUKAKUTEI_FLG").ToString().Equals("1") AndAlso String.IsNullOrEmpty(choiceKb) = True Then

                        Dim fieldNm As String = "商品コード"
                        msgArray(1) = "品目と荷姿"
                        ds = Me.SetComWarningM163("W296" _
                                                , LMH030BLC.NKS_WID_M001 _
                                                , ds _
                                                , setDs _
                                                , msgArray _
                                                , fieldNm _
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetComWarningM163(ByVal msgId As String _
...
#If True Then 'フィルメニッヒ セミEDI対応  20160926 added inoue
                If (_IsFirmeType) Then
                    ...
                    Select Case mstFlg
                        Case LMH030BLC.WARNING_ID_MST_FLG.M_DEST
                            'ワーニング処理設定の値を反映
...
#Region "データセット設定(出荷管理番号L)"
    ...
    Private Function GetOutkaNoL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        ElseIf unsoFlg.Equals("1") = True Then
...
#If True Then 'フィルメニッヒ セミEDI対応  20160926 added inoue
            ...
            If (_IsFirmeType) Then
                ...
                Const BUYER_ORD_NO_COLUMN_NAME As String = "BUYER_ORD_NO"
                Const REMARK_COLUMN_NAME As String = "REMARK"

                Dim buyerOrdNo As String = ""
                Dim remark As String = ""

                If (Me.IsSetBuyerOrdNoToRemarkWithConcatOrdNo(matomesakiDt _
                                                            , ediDr(BUYER_ORD_NO_COLUMN_NAME).ToString() _
                                                            , buyerOrdNo _
                                                            , remark)) Then
                    ' 注文番号を連結した備考を設定
                    outkaDr(REMARK_COLUMN_NAME) = remark
                Else
                    ' 元の備考を再設定
                    outkaDr(REMARK_COLUMN_NAME) = TryCast(matomesakiDt.Rows(0)(REMARK_COLUMN_NAME), String)
                End If

                outkaDr(BUYER_ORD_NO_COLUMN_NAME) = buyerOrdNo
...
#End If
    ...
    ''' <summary>
    ''' まとめ対象の注文番号の連結と備考への設定要否を判定する
    ''' </summary>
    ''' <param name="matomesakiDt">まとめ先のデータ</param>
    ''' <param name="nextBuyerOrdrNo">まとめ先に追加される注文番号</param>
    ''' <param name="outBuyerOrdNo">追加分の注文番号を連結した注文番号群</param>
    ''' <param name="outRemark">注文番号を連結した備考</param>
    ''' <returns>備考への注文番号の設定要否</returns>
    ''' <remarks>
    ''' まとめ先の注文番号が3件以上の場合、リマークに注文番号を設定する
    ''' </remarks>
    Private Function IsSetBuyerOrdNoToRemarkWithConcatOrdNo(ByVal matomesakiDt As DataTable _
                                                          , ByVal nextBuyerOrdrNo As String _
                                                          , ByRef outBuyerOrdNo As String _
                                                          , ByRef outRemark As String) As Boolean

        Const BUYER_ORD_NO_CONNECTOR As String = ","
        Const BUYER_ORD_NO_LENGTH As Integer = 30
        Const BUYER_ORD_NO_COLUMN_NAME As String = "BUYER_ORD_NO"
        Const REMARK_LENGTH As Integer = 100
        Const REMARK_COLUMN_NAME As String = "REMARK"
        Const TH_SET_REMARK As Integer = 2

        Dim buyerOrdNumbers As String() = {}

        Dim isSetRemark As Boolean = False

        If (matomesakiDt.Rows.Count > 0) Then

            outBuyerOrdNo = TryCast(matomesakiDt.Rows(0)(BUYER_ORD_NO_COLUMN_NAME), String)
            outRemark = TryCast(matomesakiDt.Rows(0)(REMARK_COLUMN_NAME), String)

            If (String.IsNullOrEmpty(outBuyerOrdNo) = False) Then
                ' まとめ注文番号分割
                buyerOrdNumbers = Split(outBuyerOrdNo, BUYER_ORD_NO_CONNECTOR)
            End If

        End If

        ' 連結された注文番号が3件以上なら、注文番号をリマークへ記載する
        isSetRemark = buyerOrdNumbers.Length >= TH_SET_REMARK

        If (isSetRemark) Then
            ' 連結元の設定をリマークから再取得
            buyerOrdNumbers = Split(outRemark, BUYER_ORD_NO_CONNECTOR)
        End If

        ' 結合注文番号生成
        If (buyerOrdNumbers.Count > 0) Then

            If (buyerOrdNumbers.Contains(nextBuyerOrdrNo) = False) Then

                If (isSetRemark) Then

                    ' REMARKに処理中の注文番号を単純連結
                    outRemark = _
                        String.Format("{0}{1}{2}" _
                                    , outRemark _
                                    , BUYER_ORD_NO_CONNECTOR _
                                    , nextBuyerOrdrNo)

                    If (buyerOrdNumbers.Length = TH_SET_REMARK) Then

                        ' BUYER_ORD_NOにカンマだけを連結
                        outBuyerOrdNo = _
                            String.Format("{0}{1}" _
                                        , outBuyerOrdNo _
                                        , BUYER_ORD_NO_CONNECTOR)
                    End If

                Else

                    ' BUYER_ORD_NOに処理中の注文番号を単純連結
                    outBuyerOrdNo = _
                        String.Format("{0}{1}{2}" _
                                    , outBuyerOrdNo _
                                    , BUYER_ORD_NO_CONNECTOR _
        ...
        ElseIf String.IsNullOrEmpty(nextBuyerOrdrNo) = False Then

            'まとめ一件目
            outBuyerOrdNo = nextBuyerOrdrNo
        End If


        ' 注文番号
        outBuyerOrdNo = Me._Blc.LeftB(outBuyerOrdNo, BUYER_ORD_NO_LENGTH)

        ' リマーク
        If (isSetRemark) Then
            outRemark = Left(outRemark, REMARK_LENGTH)
        Else
            outRemark = TryCast(matomesakiDt.Rows(0)(REMARK_COLUMN_NAME), String)
        End If
    ...
    Private Function SetDatasetOutkaM(ByVal ds As DataSet, ByVal matomeflg As Boolean) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            If ediDr("BUYER_ORD_NO_DTL").ToString = sBuyerOrdNo Then
                '2018/04/11 000291 【LMS】納品書_横浜古川エージェンシ-注文番号(M単位)表示(横浜福永) Annen upd start
                If ediLDr("CUST_CD_L").ToString.Equals(CUST_CD_FURUKAWA) Then
                    outkaDr("BUYER_ORD_NO_DTL") = ediDr("BUYER_ORD_NO_DTL")
                Else
                    outkaDr("BUYER_ORD_NO_DTL") = vbNullString
                End If
                'outkaDr("BUYER_ORD_NO_DTL") = vbNullString
                '2018/04/11 000291 【LMS】納品書_横浜古川エージェンシ-注文番号(M単位)表示(横浜福永) Annen upd end
            Else
                outkaDr("BUYER_ORD_NO_DTL") = ediDr("BUYER_ORD_NO_DTL")
            End If
            '要望番号:1712  umano 2013.01.11 END
...
#Region "データセット設定(EDI受信DTL)[使用していません]"
    ''' <summary>
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
                                    ...
                                    , ByVal drEdiRcvDtl As DataRow _
                                    , ByVal drSetDtl As DataRow _
                                    , ByRef sEdiCtlNo As String _
                                    , ByRef iEdiCtlNoChu As Integer _
                                    , ByRef sUnsoFlg As String
                                    ) As DataSet
        ...
        '削除区分
        drOutkaEdiL("DEL_KB") = LMConst.FLG.OFF
        ...
        '出荷日
        Dim outkaPlanDate As String = Date.Parse(drEdiRcvDtl("INOUTKA_PLAN_DATE").ToString().Trim()).ToString("yyyyMMdd")
        drOutkaEdiL("OUTKA_PLAN_DATE") = outkaPlanDate
        '出庫日
        drOutkaEdiL("OUTKO_DATE") = outkaPlanDate

        '納入予定日
        Dim arrPlanDate As String = Date.Parse(drEdiRcvDtl("DUE_DATE").ToString().Trim()).ToString("yyyyMMdd")
        ...
        '荷主L
        drOutkaEdiL("CUST_CD_L") = drEdiRcvDtl("CUST_CD_L").ToString()
        '荷主M
        drOutkaEdiL("CUST_CD_M") = drEdiRcvDtl("CUST_CD_M").ToString()
        ...
        drOutkaEdiL("EDI_DEST_CD") = destCd
        '届先
        drOutkaEdiL("DEST_CD") = destCd

        '届先名称
        drOutkaEdiL("DEST_NM") = drEdiRcvDtl("DEST_NM").ToString()
        '届先郵便番号
        drOutkaEdiL("DEST_ZIP") = drEdiRcvDtl("DEST_ZIP").ToString()

        '届先住所1～3
        Dim sDestAd As String = drEdiRcvDtl("DEST_ADDRESS").ToString()
        Dim sAd1 As String = String.Empty
        Dim sAd2 As String = String.Empty
        Dim sAd3 As String = String.Empty
        sAd1 = Me._Blc.LeftB(sDestAd, 40)
        If sDestAd.Length > sAd1.Length Then
            sDestAd = sDestAd.Substring(sAd1.Length)
            sAd2 = Me._Blc.LeftB(sDestAd, 40)
            If sDestAd.Length > sAd2.Length Then
                sDestAd = sDestAd.Substring(sAd2.Length)
                sAd3 = Me._Blc.LeftB(sDestAd, 60)
            End If
        End If
        drOutkaEdiL("DEST_AD_1") = sAd1
        drOutkaEdiL("DEST_AD_2") = sAd2
        drOutkaEdiL("DEST_AD_3") = sAd3
        ...
        '荷主オーダー番号
        drOutkaEdiL("CUST_ORD_NO") = drEdiRcvDtl("DENP_NO").ToString()
        '買主オーダー番号
        drOutkaEdiL("BUYER_ORD_NO") = drEdiRcvDtl("SENPO_ORD_NO").ToString()
        ...
        '文字列1
        drOutkaEdiL("FREE_C01") = drEdiRcvDtl("DENP_NO").ToString()             ' 文字列01     ←EDI出荷受信テーブル.伝票番号
        drOutkaEdiL("FREE_C02") = drEdiRcvDtl("INOUTKA_PLAN_DATE").ToString()   ' 文字列02     ←EDI出荷受信テーブル.入出荷予定日
        drOutkaEdiL("FREE_C03") = drEdiRcvDtl("DUE_DATE").ToString()            ' 文字列03     ←EDI出荷受信テーブル.納期
        drOutkaEdiL("FREE_C04") = drEdiRcvDtl("DEST_CD").ToString()             ' 文字列04     ←EDI出荷受信テーブル.出荷先
        drOutkaEdiL("FREE_C05") = drEdiRcvDtl("DEST_NM").ToString()             ' 文字列05     ←EDI出荷受信テーブル.出荷先名称
        drOutkaEdiL("FREE_C06") = drEdiRcvDtl("DEST_ZIP").ToString()            ' 文字列06     ←EDI出荷受信テーブル.出荷先郵便番号
        drOutkaEdiL("FREE_C07") = Me._Blc.LeftB(drEdiRcvDtl("DEST_ADDRESS").ToString(), 100)    ' 文字列07     ←EDI出荷受信テーブル.出荷先住所
        ...
        'ファイル名
        drOutkaEdiL("FREE_C27") = drSetDtl("FILE_NAME_RCV").ToString().Trim()

        drOutkaEdiL("FREE_C28") = String.Empty
        drOutkaEdiL("FREE_C29") = String.Empty

        If sUnsoFlg.Equals("1") = True Then
            '運送データ作成の場合
            drOutkaEdiL("FREE_C30") = String.Concat("01-", DEF_CTL_NO)
        Else
            '出荷データ作成の場合
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
        ...
        If dtMGoods.Rows.Count > 1 Then
            '商品複数ありフラグセット
            drOutkaEdiM("GOODSM_FUKAKUTEI_FLG") = "1"
        Else
            drOutkaEdiM("GOODSM_FUKAKUTEI_FLG") = "0"
        End If

        '商品が複数存在した場合は入目と使用状態：可の条件で絞り込む
        If dtMGoods.Rows.Count > 1 Then
            Dim dIrime As Double = Me.CalcIrime(drEdiRcvDtl)
            Dim drs As DataRow()
            drs = dtMGoods.Select(String.Concat("CONVERT(STD_IRIME_NB, System.Double)=", dIrime.ToString(), " AND AVAL_YN='01'"))
            If drs.Count > 0 Then
                drMgoods = drs(0)
            End If
        End If
        If drMgoods Is Nothing Then
            drMgoods = dtMGoods.Rows(0)
        End If

        '削除区分
        drOutkaEdiM("DEL_KB") = LMConst.FLG.OFF
        drOutkaEdiM("SYS_DEL_FLG") = LMConst.FLG.OFF
        '営業所コード
        drOutkaEdiM("NRS_BR_CD") = drSemiEdiInfo("NRS_BR_CD").ToString()
        'EDI管理番号L
        drOutkaEdiM("EDI_CTL_NO") = sEdiCtlNo
        'EDI管理番号M
        drOutkaEdiM("EDI_CTL_NO_CHU") = iEdiCtlNoChu.ToString("000")
        '出荷管理番号L
        ...
        '商品コード
        drOutkaEdiM("CUST_GOODS_CD") = drMgoods("GOODS_CD_CUST").ToString()
        '商品キー
        drOutkaEdiM("NRS_GOODS_CD") = String.Empty
        '商品名
        drOutkaEdiM("GOODS_NM") = drMgoods("GOODS_NM_1").ToString()
        '予約番号
        drOutkaEdiM("RSV_NO") = String.Empty
        'ロット№
        drOutkaEdiM("LOT_NO") = drEdiRcvDtl("LOT_NO").ToString().Trim()
        'シリアル№
        ...
        '出荷包装個数
        If dtMGoods.Rows.Count > 1 Then
            '複数商品が存在する場合は梱包を0で設定する
            drOutkaEdiM("OUTKA_PKG_NB") = 0
        Else
            drOutkaEdiM("OUTKA_PKG_NB") = Convert.ToInt32(drEdiRcvDtl("NB").ToString().Trim()) \ Convert.ToInt32(drMgoods("PKG_NB"))
        End If

        '出荷端数
        drOutkaEdiM("OUTKA_HASU") = Convert.ToInt32(drEdiRcvDtl("NB").ToString().Trim()) Mod Convert.ToInt32(drMgoods("PKG_NB"))

        '出荷数量
        drOutkaEdiM("OUTKA_QT") = drEdiRcvDtl("WT").ToString().Trim()

        '出荷総個数
        drOutkaEdiM("OUTKA_TTL_NB") = drEdiRcvDtl("NB").ToString().Trim()

        '出荷総数量
        drOutkaEdiM("OUTKA_TTL_QT") = drEdiRcvDtl("WT").ToString().Trim()
        ...
        '包装個数
        drOutkaEdiM("PKG_NB") = String.Empty
        '包装単位
        ...
        '入目
        drOutkaEdiM("IRIME") = drMgoods("STD_IRIME_NB").ToString()
        ...
        drOutkaEdiM("FREE_C01") = drEdiRcvDtl("GOODS_CD_CUST").ToString()   ' 文字列01     ←EDI出荷受信テーブル.品目
        drOutkaEdiM("FREE_C02") = Me._Blc.LeftB(drEdiRcvDtl("GOODS_NM").ToString(), 100)    ' 文字列02     ←EDI出荷受信テーブル.品目テキスト
        drOutkaEdiM("FREE_C03") = drEdiRcvDtl("PKG_UT").ToString()          ' 文字列03     ←EDI出荷受信テーブル.荷姿
        drOutkaEdiM("FREE_C04") = drEdiRcvDtl("NB").ToString()              ' 文字列04     ←EDI出荷受信テーブル.個数
        drOutkaEdiM("FREE_C05") = drEdiRcvDtl("WT_TTL").ToString()          ' 文字列05     ←EDI出荷受信テーブル.重量計
        drOutkaEdiM("FREE_C06") = drEdiRcvDtl("WT").ToString()              ' 文字列06     ←EDI出荷受信テーブル.重量
        ...
        '行番号
        drOutkaEdiM("FREE_C27") = drEdiRcvDtl("REC_NO").ToString()
        ...
        If sUnsoFlg.Equals("1") = True Then
            '運送データ作成の場合
            drOutkaEdiM("FREE_C30") = String.Concat("01-", DEF_CTL_NO, "000")
        Else
            '出荷データ作成の場合
            drOutkaEdiM("FREE_C30") = String.Empty
        End If
...
#Region "セミEDI時  データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        ' 登録用EDI管理番号
        drEdiRcvDtl.Item("EDI_CTL_NO") = sEdiCtlNo
        drEdiRcvDtl.Item("EDI_CTL_NO_CHU") = iEdiCtlNoChu.ToString("000")
...
#Region "セミEDI時  データセット設定(商品マスタ)"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub SetInDataSelectGoodsMst(ByRef setDs As DataSet)
        ...
        Dim drEdiDtl As DataRow = setDs.Tables("LMH030_H_OUTKAEDI_DTL_ENE").Rows(0)
        ...
        'カラム指定INデータ
        '荷主商品コード
        setDr.Item("GOODS_CD_CUST") = drEdiDtl.Item("GOODS_CD_CUST").ToString().Trim()
        '荷姿
        setDr.Item("PKG_UT") = drEdiDtl.Item("PKG_UT").ToString().Trim()
...
#Region "セミEDI時  データセット設定(届先マスタ)"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub SetInDataSelectDestMst(ByRef setDs As DataSet)
        ...
        'カラム指定INデータ
        '出荷先
        setDr.Item("DEST_CD") = drEdiDtl.Item("DEST_CD").ToString().Trim()
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow) As Boolean
        ...
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        Else
            'バイト数チェック
            validLen = System.Text.Encoding.GetEncoding(932).GetByteCount(targetStr)
            If validLen > 20 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
        End If
        ...
        Else
            ' 年月日チェック
            If IsDate(targetStr) = True Then
            Else
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
        End If
        ...
        If String.IsNullOrEmpty(targetStr) Then
            ...
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr).Equals(False) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat(sMsg, targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        Else
            If IsConvertDbl(targetStr).Equals(False) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E005", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            Else
                dNum = Convert.ToDouble(targetStr)
                dNum = System.Math.Abs(dNum)
                If Convert.ToDouble(targetStr) < 0 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E185", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                ElseIf dNum > 999999999999 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                Else
                    If targetStr.IndexOf(".") >= 0 AndAlso Convert.ToDecimal(targetStr.Substring(targetStr.IndexOf(".") + 1)) > 0 Then
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E025", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                        bRet = False
                    End If
                End If
            End If
        ...
        ' 伝票数量
        targetStr = dr.Item("COLUMN_21").ToString().Trim()
        sMsg = "伝票数量(カラム21番目)["
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
        ...
        ' 個数計
        targetStr = dr.Item("COLUMN_23").ToString().Trim()
        sMsg = "個数計(カラム23番目)["
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
        ...
        ' 個数
        targetStr = dr.Item("COLUMN_24").ToString().Trim()
        sMsg = "個数(カラム24番目)["
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
        ...
        ' 重量計
        targetStr = dr.Item("COLUMN_26").ToString().Trim()
        sMsg = "重量計(カラム26番目)["
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
        ...
        ' 重量
        targetStr = dr.Item("COLUMN_27").ToString().Trim()
        sMsg = "重量(カラム27番目)["
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
        ...
        ' 容量計
        targetStr = dr.Item("COLUMN_29").ToString().Trim()
        sMsg = "容量計(カラム29番目)["
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
        ...
        Else
            ' 半角文字列長チェック
            If IsHalfSize(targetStr).Equals(False) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat(sMsg, targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
        ...
        '出荷先郵便番号
        targetStr = dr.Item("COLUMN_42").ToString.Trim()
        sMsg = "出荷先郵便番号(カラム42番目)["
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        ...
        '出荷先住所
        targetStr = dr.Item("COLUMN_43").ToString.Trim()
        sMsg = "出荷先住所(カラム43番目)["
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
        ...
        ' 先方注文番号
        targetStr = dr.Item("COLUMN_64").ToString.Trim()
        sMsg = "先方注文番号(カラム64番目)["
        If String.IsNullOrEmpty(targetStr) Then
        Else
            'バイト数チェック
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim sUnsoFlg As String = String.Empty                                '運送フラグ
        Dim dsUnso As DataSet = New Jp.Co.Nrs.LM.DSL.LMH030DS()              '運送フラグ取得用DataSet
        Dim procDenNoSet As HashSet(Of String) = New HashSet(Of String)
        ...
        '運送フラグ取得
        dsUnso.Tables("LMH030INOUT").Clear()
        dsUnso.Tables("LMH030_SEMIEDI_INFO").Clear()
        dsUnso.Tables("LMH030_SEMIEDI_INFO").ImportRow(ds.Tables("LMH030_SEMIEDI_INFO").Rows(0))
        dsUnso = MyBase.CallDAC(Me._Dac, "GetUnsoFlg", dsUnso)
        If MyBase.GetResultCount > 0 Then
            sUnsoFlg = dsUnso.Tables("LMH030INOUT").Rows(0).Item("EDI_CUST_UNSOFLG").ToString()
        End If
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_ENE").NewRow()
            gyo += 1
            ds.Tables("LMH030_H_OUTKAEDI_DTL_ENE").Clear() ' 受信DTLのクリア
            ...
            If drEdiRcvDtl.Item("DENP_NO").ToString().Trim().Equals("") = False Then
                '---------------------------------------------------------------------------
                ' EDI出荷データ件数および出荷データL 出荷管理番号L 等 取得処理
                '---------------------------------------------------------------------------
                setDs.Tables("LMH030_H_OUTKAEDI_DTL_ENE").ImportRow(drEdiRcvDtl)

                Dim errMsgId As String = String.Empty
                If sUnsoFlg.Equals("1") = False Then
                    '出荷データ作成の場合
                    setDs = MyBase.CallDAC(Me._Dac, "GetOutkaCntAndNoL", setDs)
                    errMsgId = "E847"
                Else
                    '運送データ作成の場合
                    setDs = MyBase.CallDAC(Me._Dac, "GetOutkaCntAndNoLUnso", setDs)
                    errMsgId = "E02S"
                End If

                ediRecCnt = MyBase.GetResultCount()
                Dim outkaNoL As String = setDs.Tables("LMH030_H_OUTKAEDI_DTL_ENE").Rows(0).Item("OUTKA_CTL_NO").ToString()
                If ediRecCnt > 0 AndAlso (outkaNoL.Equals("") = False) Then
                    ' 出荷EDI存在かつ出荷存在（「出荷管理番号L IS NULL → ""」でない）の場合
                    ' 当該行はエラー
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, errMsgId, New String() {outkaNoL},
            ...
            ' EDI管理番号(大,中)の採番
            '---------------------------------------------------------------------------
            ds = Me.GetEdiCtlNo(ds, bSameKeyFlg, sEdiCtlNo, iEdiCtlNoChu, drEdiRcvDtl)
            ...
            '伝票番号設定
            Dim sDenNo As String = drEdiRcvDtl.Item("DENP_NO").ToString().Trim()
            ...
            If bSameKeyFlg = False Then
                ...
                '届先マスタ読込用INデータ作成
                Me.SetInDataSelectDestMst(setDs)
                ...
                If MyBase.GetResultCount = 0 Then
                    If sFlg17.Equals("0") Then
                        'エラー返却
                        Dim sDestCd As String = drEdiRcvDtl.Item("DEST_CD").ToString().Trim()

                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E493",
                                               New String() {String.Concat("届先コード:", sDestCd), "届先マスタ", String.Concat(" 伝票管理番号:", sDenNo)},
                                               dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(),
                                               LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                        bNoErr = False
                        Continue For
            ...
            '---------------------------------------------------------------------------
            '商品マスタ読込用INデータ作成
            SetInDataSelectGoodsMst(setDs)

            '商品マスタ読込処理(商品コードがマスタに存在しない場合はエラー)
            setDs = MyBase.CallDAC(Me._Dac, "SelectMstGoods", setDs)

            Dim sGoodsCd As String = drEdiRcvDtl.Item("GOODS_CD_CUST").ToString().Trim()
            Dim sPkgUt As String = drEdiRcvDtl.Item("PKG_UT").ToString().Trim()
            Dim dIrime As Double = Me.CalcIrime(drEdiRcvDtl)

            If MyBase.GetResultCount = 0 Then
                '商品マスタに荷主商品コードが存在しない場合はエラー
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E493",
                                       New String() {String.Concat("品目:", sGoodsCd, " 荷姿:", sPkgUt),
                                                     "商品マスタ", String.Concat("伝票番号:", sDenNo)},
                                       dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(),
                                       LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                bNoErr = False
                Continue For
            ...
            Else
                '商品マスタを入目の条件で絞り込み
                Dim iGoodsCount As Integer = setDs.Tables("LMH030_M_GOODS").Select(String.Concat("CONVERT(STD_IRIME_NB, System.Double)=", dIrime.ToString())).Count
                If iGoodsCount = 0 Then
                    '商品マスタに入目の条件で絞り込んだ商品が存在しない場合はエラー
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E493",
                                       New String() {String.Concat("品目:", sGoodsCd, " 荷姿:", sPkgUt, " 入目:", dIrime.ToString()),
                                                     "商品マスタ", String.Concat("伝票番号:", sDenNo)},
                                       dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(),
                                       LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                    bNoErr = False
                ...
                Else
                    '商品マスタに入目の条件で絞り込んだ商品が存在した場合、使用状態の条件でさらに絞り込み
                    iGoodsCount = setDs.Tables("LMH030_M_GOODS").Select(String.Concat("CONVERT(STD_IRIME_NB, System.Double)=", dIrime.ToString(), " AND AVAL_YN='01'")).Count
                    If iGoodsCount = 0 Then
                        '商品マスタに使用状態：可の商品が存在しない場合はエラー
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E454",
                                           New String() {String.Concat("品目:", sGoodsCd, " 荷姿:", sPkgUt, " 入目:", dIrime.ToString(), " の商品は使用状態不可"),
                                                         "取り込み", String.Concat("伝票番号:", sDenNo)},
                                           dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(),
                                           LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                        bNoErr = False
                        Continue For

                    ElseIf iGoodsCount > 1 Then
                        '商品マスタに入目と使用状態：可の条件で絞り込んだ商品が複数存在した場合はエラー
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E494",
                                           New String() {String.Concat("品目:", sGoodsCd, " 荷姿:", sPkgUt, " 入目:", dIrime.ToString()),
                                                         "商品マスタ", String.Concat("伝票番号:", sDenNo)},
                                           dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(),
                                           LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
            ...
            '---------------------------------------------------------------------------
            If ediRecCnt > 0 AndAlso (bSameKeyFlg = False) AndAlso (procDenNoSet.Contains(sDenNo) = False) Then
            ...
            ' EDI受信データの新規追加
            '---------------------------------------------------------------------------
            setDs.Tables("LMH030_H_OUTKAEDI_DTL_ENE").Clear()
            setDs.Tables("LMH030_H_OUTKAEDI_DTL_ENE").ImportRow(drEdiRcvDtl)
            ...
            If iSkipFlg = 0 Then
                ...
                setDs = Me.SetSemiOutkaEdiM(setDs, drEdiRcvDtl, sEdiCtlNo, iEdiCtlNoChu, sUnsoFlg)
                ...
                If bSameKeyFlg = False Then
                    ...
                    setDs = Me.SetSemiOutkaEdiL(setDs, _WhCd, drEdiRcvDtl, drSetDtl, sEdiCtlNo, iEdiCtlNoChu, sUnsoFlg)
            ...
            '処理済みの伝票番号セット
            If procDenNoSet.Contains(sDenNo) = False Then
                procDenNoSet.Add(sDenNo)
            End If
...
#Region "データセット設定 EDI出荷受信テーブル(明細)"
    ...
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer, ByVal gyo As Integer, ByVal drEdiRcvDtl As DataRow) As DataSet
        ...
        drEdiRcvDtl("OUT_TGT") = Me._Blc.LeftB(drSetDtl("COLUMN_1").ToString().Trim(), 8)                   ' 出力対象
        drEdiRcvDtl("PLANT_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_2").ToString().Trim(), 10)                 ' プラント
        drEdiRcvDtl("PLANT_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 100)                ' プラント名称
        drEdiRcvDtl("INOUTKA_KBN") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 10)              ' 入出荷区分
        drEdiRcvDtl("DENP_TYPE") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 20)                ' 伝票タイプ
        drEdiRcvDtl("DENP_TYPE_TXT") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 50)            ' 伝票タイプテキスト
        drEdiRcvDtl("DTL_CTG") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 20)                  ' 明細カテゴリ
        drEdiRcvDtl("DTL_CTG_TXT") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 50)              ' 明細カテゴリテキスト
        drEdiRcvDtl("DENP_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 20)                  ' 伝票番号
        drEdiRcvDtl("DTL_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 20)                  ' 明細番号
        drEdiRcvDtl("INOUTKA_PLAN_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 10)       ' 入出荷予定日
        drEdiRcvDtl("DUE_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 10)                ' 納期
        drEdiRcvDtl("DUE_TIME") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 8)                 ' 時間
        drEdiRcvDtl("GOODS_CD_CUST") = Me._Blc.LeftB(drSetDtl("COLUMN_14").ToString().Trim(), 20)           ' 品目
        drEdiRcvDtl("GOODS_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_15").ToString().Trim(), 100)               ' 品目テキスト
        drEdiRcvDtl("PKG_UT") = Me._Blc.LeftB(drSetDtl("COLUMN_16").ToString().Trim(), 10)                  ' 荷姿
        drEdiRcvDtl("EVA_CLASS") = Me._Blc.LeftB(drSetDtl("COLUMN_17").ToString().Trim(), 20)               ' 評価クラス
        drEdiRcvDtl("LOT_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_18").ToString().Trim(), 40)                  ' ロット番号
        drEdiRcvDtl("GASOLINE_TAX_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_19").ToString().Trim(), 40)         ' 揮発油税区分
        drEdiRcvDtl("DENPYO_QT_TTL") = Me._Blc.LeftB(drSetDtl("COLUMN_20").ToString().Trim(), 12)           ' 伝票数量計
        drEdiRcvDtl("DENP_QT") = Me._Blc.LeftB(drSetDtl("COLUMN_21").ToString().Trim(), 8)                  ' 伝票数量
        drEdiRcvDtl("DENP_QT_UT") = Me._Blc.LeftB(drSetDtl("COLUMN_22").ToString().Trim(), 8)               ' 伝票数量単位
        drEdiRcvDtl("NB_TTL") = Me._Blc.LeftB(drSetDtl("COLUMN_23").ToString().Trim(), 12)                  ' 個数計
        drEdiRcvDtl("NB") = Me._Blc.LeftB(drSetDtl("COLUMN_24").ToString().Trim(), 8)                       ' 個数
        drEdiRcvDtl("NB_UT") = Me._Blc.LeftB(drSetDtl("COLUMN_25").ToString().Trim(), 8)                    ' 個数単位
        drEdiRcvDtl("WT_TTL") = Me._Blc.LeftB(drSetDtl("COLUMN_26").ToString().Trim(), 13)                  ' 重量計
        drEdiRcvDtl("WT") = Me._Blc.LeftB(drSetDtl("COLUMN_27").ToString().Trim(), 13)                      ' 重量
        drEdiRcvDtl("WT_UT") = Me._Blc.LeftB(drSetDtl("COLUMN_28").ToString().Trim(), 8)                    ' 重量単位
        drEdiRcvDtl("IRIME_TTL") = Me._Blc.LeftB(drSetDtl("COLUMN_29").ToString().Trim(), 13)               ' 容量計
        drEdiRcvDtl("STD_IRIME_NB") = Me._Blc.LeftB(drSetDtl("COLUMN_30").ToString().Trim(), 13)            ' 容量
        drEdiRcvDtl("STD_IRIME_UT") = Me._Blc.LeftB(drSetDtl("COLUMN_31").ToString().Trim(), 8)             ' 容量単位
        drEdiRcvDtl("JYUYOKA_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_32").ToString().Trim(), 20)              ' 需要家
        drEdiRcvDtl("JYUYOKA_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_33").ToString().Trim(), 100)             ' 需要家名称
        drEdiRcvDtl("ORD_RCV_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_34").ToString().Trim(), 20)              ' 受注先
        drEdiRcvDtl("ORD_RCV_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_35").ToString().Trim(), 100)             ' 受注先名称
        drEdiRcvDtl("PURCHASE_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_36").ToString().Trim(), 20)             ' 仕入先
        drEdiRcvDtl("PURCHASE_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_37").ToString().Trim(), 100)            ' 仕入先名称
        drEdiRcvDtl("SUPPLY_PLANT_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_38").ToString().Trim(), 20)         ' 供給プラント
        drEdiRcvDtl("SUPPLY_PLANT_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_39").ToString().Trim(), 100)        ' 供給プラント名称
        drEdiRcvDtl("DEST_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_40").ToString().Trim(), 20)                 ' 出荷先
        drEdiRcvDtl("DEST_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_41").ToString().Trim(), 100)                ' 出荷先名称
        drEdiRcvDtl("DEST_ZIP") = Me._Blc.LeftB(drSetDtl("COLUMN_42").ToString().Trim(), 10)                ' 出荷先郵便番号
        drEdiRcvDtl("DEST_ADDRESS") = Me._Blc.LeftB(drSetDtl("COLUMN_43").ToString().Trim(), 240)           ' 出荷先住所
        drEdiRcvDtl("SHIP_FROM_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_44").ToString().Trim(), 20)            ' 出荷元
        drEdiRcvDtl("SHIP_FROM_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_45").ToString().Trim(), 100)           ' 出荷元名称
        drEdiRcvDtl("SHIP_FROM_ZIP") = Me._Blc.LeftB(drSetDtl("COLUMN_46").ToString().Trim(), 10)           ' 出荷元郵便番号
        drEdiRcvDtl("SHIP_FROM_ADDRESS") = Me._Blc.LeftB(drSetDtl("COLUMN_47").ToString().Trim(), 240)      ' 出荷元住所
        drEdiRcvDtl("UNSO_MODE_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_48").ToString().Trim(), 8)             ' 輸送手段
        drEdiRcvDtl("UNSO_MODE_TXT") = Me._Blc.LeftB(drSetDtl("COLUMN_49").ToString().Trim(), 50)           ' 輸送手段テキスト
        drEdiRcvDtl("UNSO_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_50").ToString().Trim(), 20)                 ' 輸送業者
        drEdiRcvDtl("UNSO_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_51").ToString().Trim(), 100)                ' 輸送業者名称
        drEdiRcvDtl("UNSO_TYPE_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_52").ToString().Trim(), 20)            ' 輸送機関タイプ
        drEdiRcvDtl("UNSO_TYPE_TXT") = Me._Blc.LeftB(drSetDtl("COLUMN_53").ToString().Trim(), 100)          ' 輸送機関タイプテキスト
        drEdiRcvDtl("TRANS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_54").ToString().Trim(), 20)                ' 輸送機関
        drEdiRcvDtl("TRANS_TXT") = Me._Blc.LeftB(drSetDtl("COLUMN_55").ToString().Trim(), 100)              ' 輸送機関テキスト
        drEdiRcvDtl("DESCRIPTION") = Me._Blc.LeftB(drSetDtl("COLUMN_56").ToString().Trim(), 100)            ' 内容説明
        drEdiRcvDtl("TRIP_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_57").ToString().Trim(), 8)                  ' トリップ番号
        drEdiRcvDtl("EQT") = Me._Blc.LeftB(drSetDtl("COLUMN_58").ToString().Trim(), 10)                     ' EQT番号
        drEdiRcvDtl("KENPOU") = Me._Blc.LeftB(drSetDtl("COLUMN_59").ToString().Trim(), 8)                   ' 検報添
        drEdiRcvDtl("KEEP_MK") = Me._Blc.LeftB(drSetDtl("COLUMN_60").ToString().Trim(), 10)                 ' キープマーク
        drEdiRcvDtl("UPD_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_61").ToString().Trim(), 10)                  ' 更新区分
        drEdiRcvDtl("KOKUYU_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_62").ToString().Trim(), 10)               ' 国輸区分
        drEdiRcvDtl("INOUTKA_DONE_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_63").ToString().Trim(), 8)          ' 入出荷済区分
        drEdiRcvDtl("SENPO_ORD_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_64").ToString().Trim(), 20)            ' 先方注文番号
        drEdiRcvDtl("DO_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_65").ToString().Trim(), 20)                   ' DOナンバー
        drEdiRcvDtl("ORDER_COM") = Me._Blc.LeftB(drSetDtl("COLUMN_66").ToString().Trim(), 100)              ' 受注コメント
        drEdiRcvDtl("SENPO_COM") = Me._Blc.LeftB(drSetDtl("COLUMN_67").ToString().Trim(), 100)              ' 先方コメント
        drEdiRcvDtl("SASHIZU_COM") = Me._Blc.LeftB(drSetDtl("COLUMN_68").ToString().Trim(), 100)            ' 指図コメント

        'データセットに設定
        ds.Tables("LMH030_H_OUTKAEDI_DTL_ENE").Rows.Add(drEdiRcvDtl)
...
#End Region
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Function CalcIrime(ByVal detailRow As DataRow) As Double

        Dim irime As Double = 0.0
        Dim ttlNb As Double = 0
        Dim ttlQt As Double = 0

        If Double.TryParse(TryCast(detailRow.Item("NB"), String), ttlNb) Then

            If Double.TryParse(TryCast(detailRow.Item("WT_TTL"), String), ttlQt) Then
