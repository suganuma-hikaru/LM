...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC156 = New LMH030DAC156()
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow, ByVal ediCustIndex As String) As Boolean
        ...
        If LenB(targetStr) > 30 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("受注番号(カラム1番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        If IsDate(sDate) = True Then
        Else
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat("希望納期(カラム3番目)[", sDate, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("商品名(カラム4番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 文字列長チェック
        ElseIf LenB(targetStr) > 60 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("商品名(カラム4番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("数量(カラム5番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        Else
            If IsConvertDbl(targetStr).Equals(False) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E005", New String() {String.Concat("数量(カラム5番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            Else
                ...
                ElseIf Convert.ToDouble(targetStr) < 0 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E185", New String() {String.Concat("数量(カラム5番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                ElseIf dNum > 9999999999 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("数量(カラム5番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                Else
                    If targetStr.IndexOf(".") >= 0 AndAlso Convert.ToDecimal(targetStr.Substring(targetStr.IndexOf(".") + 1)) > 0 Then
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E025", New String() {String.Concat("数量(カラム5番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                        bRet = False
                    End If
                End If
            End If
        ...
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("単位(カラム6番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("納品先名称(カラム7番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 文字列長チェック
        ElseIf LenB(targetStr) > 80 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("納品先名称(カラム7番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        '必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("納品先郵便番号(カラム8番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 10, False).Equals(False) Or targetStr.Replace("-", "").Length < 7 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("納品先郵便番号(カラム8番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        ' 納品先住所1, 2
        targetStr = String.Concat(dr.Item("COLUMN_9").ToString.Trim(), dr.Item("COLUMN_10").ToString.Trim())
        '必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("納品先住所(カラム9-10番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 文字列長チェック
        Else
            Dim ad() As String = editAddr(targetStr)
            If ad(0).Equals(targetStr) = False Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("納品先住所(カラム9-10番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
        End If
        ...
        '必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("電話番号(カラム11番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 20, False).Equals(False) Or targetStr.Replace("-", "").Length < 8 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("電話番号(カラム11番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        '文字列長チェック
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("出荷コメント(カラム12番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        ' 倉庫名
        ' （参考情報としてフリー項目に文字列保存するのみ）
        targetStr = dr.Item("COLUMN_13").ToString.Trim()
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("倉庫名(カラム13番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("運送業者名(カラム14番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("会社識別コード(カラム15番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("商品コード(カラム16番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 20, False).Equals(False) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("商品コード(カラム16番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' （参考情報としてフリー項目に文字列保存するのみ）
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("単位コード(カラム17番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' （参考情報としてフリー項目に文字列保存するのみ）
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("倉庫コード(カラム18番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' （参考情報としてフリー項目に文字列保存するのみ）
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("運送業者コード(カラム19番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' （参考情報としてフリー項目に文字列保存するのみ）
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("納品先コード(カラム20番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        ' 取込Dtlの並べ替え（先頭列: 受注番号昇順）
        Dim tmpDt As DataTable = dtSetDtl.Clone()
        Dim tmpDr() As DataRow = Nothing
        tmpDr = dtSetDtl.Select(String.Empty, "COLUMN_1 ASC")
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_AGRM").NewRow()
            ds.Tables("LMH030_H_OUTKAEDI_DTL_AGRM").Clear() ' 受信DTLのクリア
            ds = Me.SetSemiOutkaEdiRcv(ds, i, drEdiRcvDtl)
            ...
            If drEdiRcvDtl("CUST_ORD_NO").ToString().Trim().Equals("") = False Then
                '---------------------------------------------------------------------------
                ' EDI出荷データ件数および出荷データL 出荷管理番号L 等 取得処理
                '---------------------------------------------------------------------------
                setDs.Tables("LMH030_H_OUTKAEDI_DTL_AGRM").ImportRow(drEdiRcvDtl)

                setDs = MyBase.CallDAC(Me._Dac, "GetOutkaCntAndNoL", setDs)

                ediRecCnt = MyBase.GetResultCount()
                Dim outkaNoL As String = setDs.Tables("LMH030_H_OUTKAEDI_DTL_AGRM").Rows(0).Item("OUTKA_CTL_NO").ToString()
                If ediRecCnt > 0 AndAlso (outkaNoL.Equals("") = False) Then
                    ' 出荷EDI存在かつ出荷存在（出荷管理番号L IS NULL → ""）の場合
                    ' 当該行はエラー
            ...
            If iSkipFlg = 0 Then
                ...
                If MyBase.GetResultCount = 0 Then
                    ' 商品コード末尾が "-ZZ" の場合、"-ZZ" を除いた商品コードで再度検索する。
                    If Right(custGoodsCd, 3).Equals("-ZZ") Then
                        custGoodsCd = custGoodsCd.Substring(0, custGoodsCd.Length() - "-ZZ".Length)
                        setDs.Tables("LMH030_H_OUTKAEDI_DTL_AGRM").Rows(0).Item("CUST_GOODS_CD") = custGoodsCd
                        setDs = MyBase.CallDAC(Me._Dac, "SelectMstGoods", setDs)
                    End If
                End If
                If MyBase.GetResultCount = 0 Then
                    ...
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E493",
                                           New String() {String.Concat("商品コード:", custGoodsCd), "商品マスタ", ""},
                                           dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(),
                                           LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                    bNoErr = False
                    Continue For
                ElseIf MyBase.GetResultCount > 1 Then
                    ...
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E975",
                                           New String() {String.Concat("商品コード:", custGoodsCd)},
                                           dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(),
                                           LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                    bNoErr = False
                    Continue For
...
#Region "データセット設定 EDI出荷受信テーブル(明細)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer, ByRef drEdiRcvDtl As DataRow) As DataSet
        ...
        drEdiRcvDtl("CUST_ORD_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_1").ToString().Trim(), 60)          ' 荷主注文番号（全体）←受信データ(CSV) 受注番号
        drEdiRcvDtl("OUTKA_PLAN_DATE") =
            Date.Parse(drSetDtl("COLUMN_2").ToString().Trim()).ToString("yyyyMMdd")                     ' 出荷日　　　　　　　←受信データ(CSV) 出荷日
        drEdiRcvDtl("ARR_PLAN_DATE") =
            Date.Parse(drSetDtl("COLUMN_3").ToString().Trim()).ToString("yyyyMMdd")                     ' 納入予定日　　　　　←受信データ(CSV) 希望納期
        drEdiRcvDtl("GOODS_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 120)            ' 商品名　　　　　　　←受信データ(CSV) 商品名
        drEdiRcvDtl("OUTKA_TTL_NB") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 10)         ' 出荷総個数　　　　　←受信データ(CSV) 数量
        drEdiRcvDtl("UT") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 200)                  ' 単位　　　　　　　　←受信データ(CSV) 単位
        drEdiRcvDtl("DEST_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 160)             ' 届先名　　　　　　　←受信データ(CSV) 納品先名称
        drEdiRcvDtl("DEST_ZIP") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 10)             ' 届先郵便番号　　　　←受信データ(CSV) 納品先郵便番号
        drEdiRcvDtl("DEST_AD_1") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 240)           ' 届先住所1 　　　　　←受信データ(CSV) 納品先住所1
        drEdiRcvDtl("DEST_AD_2") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 240)          ' 届先住所2 　　　　　←受信データ(CSV) 納品先住所2
        drEdiRcvDtl("DEST_TEL") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 20)            ' 届先電話番号　　　　←受信データ(CSV) 電話番号
        drEdiRcvDtl("REMARK") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 200)             ' 備考　　　　　　　　←受信データ(CSV) 出荷コメント
        drEdiRcvDtl("WH_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 200)              ' 倉庫名称　　　　　　←受信データ(CSV) 倉庫名
        drEdiRcvDtl("UNSOCO_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_14").ToString().Trim(), 200)          ' 運送会社名称　　　　←受信データ(CSV) 運送業者名
        drEdiRcvDtl("CO_SKBT_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_15").ToString().Trim(), 200)         ' 会社識別コード　　　←受信データ(CSV) 会社識別コード
        drEdiRcvDtl("CUST_GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_16").ToString().Trim(), 20)       ' 商品コード　　　　　←受信データ(CSV) 商品コード
        drEdiRcvDtl("UT_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_17").ToString().Trim(), 200)              ' 単位コード　　　　　←受信データ(CSV) 単位コード
        drEdiRcvDtl("WH_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_18").ToString().Trim(), 200)              ' 倉庫コード　　　　　←受信データ(CSV) 倉庫コード
        drEdiRcvDtl("UNSOCO_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_19").ToString().Trim(), 200)          ' 運送会社コード　　　←受信データ(CSV) 運送業者コード
        drEdiRcvDtl("NOHNSK_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_20").ToString().Trim(), 200)          ' 納品先コード　　　　←受信データ(CSV) 納品先コード

        'データセットに設定
        ds.Tables("LMH030_H_OUTKAEDI_DTL_AGRM").Rows.Add(drEdiRcvDtl)
...
#Region "データセット設定 EDI出荷データ 用 EDI管理番号(大・中)"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim sNrsBrCd As String = ds.Tables("LMH030_H_OUTKAEDI_DTL_AGRM").Rows(0).Item("NRS_BR_CD").ToString()
...
#Region "データセット設定(EDI出荷(中)"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet, ByVal drEdiRcvDtl As DataRow) As DataSet
        ...
        drOutkaEdiM("CUST_ORD_NO_DTL") = drEdiRcvDtl("CUST_ORD_NO")     ' 荷主注文番号（明細単位）←EDI出荷受信テーブル（明細）.荷主注文番号（全体）←受信データ(CSV) 受注番号
        drOutkaEdiM("BUYER_ORD_NO_DTL") = String.Empty

        ' 商品コード ←
        ' 　EDI出荷受信テーブル（明細）.商品コード(←受信データ(CSV) 商品コード) で検索した商品マスタ.商品コード
        ' 　ただし、上記で商品マスタが存在せず、EDI出荷受信テーブル（明細）.商品コード末尾が "-ZZ" の場合、"-ZZ" を除いた商品コード
        ' 　（この段階に至っている場合、上記いずれかで、商品コードが商品マスタに存在する）
        drOutkaEdiM("CUST_GOODS_CD") = drGoods("GOODS_CD_CUST")
        drOutkaEdiM("NRS_GOODS_CD") = String.Empty
        drOutkaEdiM("GOODS_NM") = drEdiRcvDtl("GOODS_NM")               ' 商品名　　　　　　　　　←EDI出荷受信テーブル（明細）.商品名　　　　　　　←受信データ(CSV) 商品名
        ...
        drOutkaEdiM("OUTKA_HASU") = drEdiRcvDtl("OUTKA_TTL_NB")         ' 出荷端数　　　　　　　　←EDI出荷受信テーブル（明細）.出荷総個数　　　　　←受信データ(CSV) 数量
        drOutkaEdiM("OUTKA_QT") = 0
        drOutkaEdiM("OUTKA_TTL_NB") = drEdiRcvDtl("OUTKA_TTL_NB")       ' 出荷総個数　　　　　　　←EDI出荷受信テーブル（明細）.出荷総個数　　　　　←受信データ(CSV) 数量
        ...
        drOutkaEdiM("FREE_C01") = drEdiRcvDtl("UT_CD")                  ' 文字列01　　　　　　　　←EDI出荷受信テーブル（明細）.単位コード　　　　　←受信データ(CSV) 単位コード
        drOutkaEdiM("FREE_C02") = drEdiRcvDtl("UT")                     ' 文字列02　　　　　　　　←EDI出荷受信テーブル（明細）.単位　　　　　　　　←受信データ(CSV) 単位
...
#Region "データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        drOutkaEdiL("OUTKA_PLAN_DATE") = drEdiRcvDtl("OUTKA_PLAN_DATE") ' 出荷日　　　　　　　←EDI出荷受信テーブル（明細）.出荷日　　　　　　　　　　　←受信データ(CSV) 出荷日
        drOutkaEdiL("OUTKO_DATE") = drEdiRcvDtl("OUTKA_PLAN_DATE")      ' 出荷日　　　　　　　←EDI出荷受信テーブル（明細）.出荷日　　　　　　　　　　　←受信データ(CSV) 出荷日
        drOutkaEdiL("ARR_PLAN_DATE") = drEdiRcvDtl("ARR_PLAN_DATE")     ' 出荷日　　　　　　　←EDI出荷受信テーブル（明細）.納入予定日　　　　　　　　　←受信データ(CSV) 希望納期
        ...
        End If
        ' 届先コード ←
        ' 　EDI出荷受信テーブル（明細）.届先郵便番号←受信データ(CSV) 納品先郵便番号 (ハイフン抜き上7桁) + 
        ' 　EDI出荷受信テーブル（明細）.届先電話番号←受信データ(CSV) 電話番号 (ハイフン抜き下8桁)
        ...
        drOutkaEdiL("DEST_NM") = drEdiRcvDtl("DEST_NM")                 ' 届先名　　　　　　　←EDI出荷受信テーブル（明細）.届先名　　　　　　　　　　　←受信データ(CSV) 納品先名称
        drOutkaEdiL("DEST_ZIP") = destZip.Replace("-", "")              ' 届先郵便番号　　　　←EDI出荷受信テーブル（明細）.届先郵便番号 (ハイフン抜き) ←受信データ(CSV) 納品先郵便番号

        If LenB(drEdiRcvDtl("DEST_AD_1").ToString()) <= 40 AndAlso
            LenB(drEdiRcvDtl("DEST_AD_2").ToString()) <= 40 Then
            ' 受信データ(CSV) 納品先住所1, 2 をそのままの桁数で設定できる場合
            drOutkaEdiL("DEST_AD_1") = drEdiRcvDtl("DEST_AD_1")         ' 届先住所1 　　　　　←EDI出荷受信テーブル（明細）.届先住所1 　　　　　　　　　←受信データ(CSV) 納品先住所1
            drOutkaEdiL("DEST_AD_2") = drEdiRcvDtl("DEST_AD_2")         ' 届先住所2 　　　　　←EDI出荷受信テーブル（明細）.届先住所2 　　　　　　　　　←受信データ(CSV) 納品先住所2
        ...
        Else
            ' 住所編集（住所を格納）
            ' 届先住所1～5 ←
            ' 　EDI出荷受信テーブル（明細）.届先住所1←受信データ(CSV) 納品先住所1 +
            ' 　EDI出荷受信テーブル（明細）.届先住所2←受信データ(CSV) 納品先住所2 の分割文字列
            Dim mergeAddr As String = String.Concat(drEdiRcvDtl("DEST_AD_1").ToString(),
                                                drEdiRcvDtl("DEST_AD_2").ToString())
        ...
        drOutkaEdiL("DEST_TEL") = destTel                               ' 届先電話番号　　　　←EDI出荷受信テーブル（明細）.届先電話番号　　　　　　　　←受信データ(CSV) 電話番号
        ...
        drOutkaEdiL("CUST_ORD_NO") = drEdiRcvDtl("CUST_ORD_NO")         ' 荷主注文番号（全体）←EDI出荷受信テーブル（明細）.荷主注文番号（全体）　　　　←受信データ(CSV) 受注番号
        ...
        drOutkaEdiL("REMARK") = drEdiRcvDtl("REMARK")                   ' 注意事項　　　　　　←EDI出荷受信テーブル（明細）.備考　　　　　　　　　　　　←受信データ(CSV) 出荷コメント
        ...
        drOutkaEdiL("FREE_C01") = drEdiRcvDtl("WH_CD")                  ' 文字列01　　　　　　←EDI出荷受信テーブル（明細）.倉庫コード　　　　　　　　　←受信データ(CSV) 倉庫コード
        drOutkaEdiL("FREE_C02") = drEdiRcvDtl("WH_NM")                  ' 文字列02　　　　　　←EDI出荷受信テーブル（明細）.倉庫名称　　　　　　　　　　←受信データ(CSV) 倉庫名
        drOutkaEdiL("FREE_C03") = drEdiRcvDtl("UNSOCO_CD")              ' 文字列03　　　　　　←EDI出荷受信テーブル（明細）.運送会社コード　　　　　　　←受信データ(CSV) 運送業者コード
        drOutkaEdiL("FREE_C04") = drEdiRcvDtl("UNSOCO_NM")              ' 文字列04　　　　　　←EDI出荷受信テーブル（明細）.運送会社名称　　　　　　　　←受信データ(CSV) 運送業者名
        drOutkaEdiL("FREE_C05") = drEdiRcvDtl("CO_SKBT_CD")             ' 文字列05　　　　　　←EDI出荷受信テーブル（明細）.会社識別コード　　　　　　　←受信データ(CSV) 会社識別コード
        drOutkaEdiL("FREE_C06") = drEdiRcvDtl("NOHNSK_CD")              ' 文字列06　　　　　　←EDI出荷受信テーブル（明細）.納品先コード　　　　　　　　←受信データ(CSV) 納品先コード
