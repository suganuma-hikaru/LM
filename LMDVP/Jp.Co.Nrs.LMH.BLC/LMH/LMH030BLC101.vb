...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC101 = New LMH030DAC101()
...
#Region "実績作成処理"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        '日医工EDI実績の値設定(EDI受信TBLより)
        ds = MyBase.CallDAC(Me._Dac, "SelectNikEdiSend", ds)
        ...
        '日医工EDI実績データの作成
        ds = MyBase.CallDAC(Me._Dac, "InsertNikEdiSendData", ds)
...
#Region "EDI出荷(中)の初期値設定"
    ...
    Private Function EdiMDefaultSet(ByVal ds As DataSet, ByVal setDs As DataSet, _
        ...
        Else
            ...
            choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.NIK_WID_M002, count)

            If String.IsNullOrEmpty(choiceKb) = True Then
                msgArray(1) = "商品名"
                msgArray(2) = "商品マスタ"
                msgArray(3) = "商品名"
                msgArray(4) = "注意)日医工製品マスタに差分データが送られてきている可能性があります。必ず確認して下さい。"
                msgArray(5) = String.Empty

                ds = Me._Blc.SetComWarningM("W195", LMH030BLC.NIK_WID_M002, ds, setDs, msgArray, _
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                '日医工専用チェック
                '日医工製品マスタ検索（荷主商品コード）
                setDs = (MyBase.CallDAC(Me._Dac, "SelectDataSeihinNik", setDs))

                'If MyBase.GetResultCount() > 0 Then
                '    '未反映データが１件以上ある場合エラー
                '    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E160", New String() {"日医工製品マスタ", "未反映データ"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                '    'このレコードのワーニングをクリア
                '    ds.Tables("WARNING_DTL").Rows.Clear()
                '    Return False
                'End If
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    Else
                        ...
                        ds = Me._Blc.SetComWarningM("W162", LMH030BLC.NIK_WID_M001, ds, setDs, msgArray, custGoodsCd, String.Empty)
...
#Region "データセット設定(出荷L)"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            '日医工専用設定START
            '出荷梱包個数は1固定(詰め合わせ作業の際に荷物を１箱にまとめる)
            '要望管理2233 2014/09/22 SHINODA START
            'outkaDr("OUTKA_PKG_NB") = 1
            If outkaDr("NRS_BR_CD").ToString() = "20" Then
                '大阪日医工は通常通り
            ...
            Else
                ...
                outkaDr("OUTKA_PKG_NB") = 1
            End If
            '要望管理2233 2014/09/22 SHINODA E N D
            '日医工専用設定END
