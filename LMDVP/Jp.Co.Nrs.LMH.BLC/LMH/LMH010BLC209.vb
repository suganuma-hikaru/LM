...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC209 = New LMH010DAC209()
...
#Region "Const"
    ...
    Public Const NISUGATA_KO As String = "EA"

    Public Const RomeYok00003_00 As Integer = 49
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    Private Function SetSemiInkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_ROM").NewRow()
        ...
        drEdiRcvDtl("REC_NO") = drSetDtl("REC_NO")
        drEdiRcvDtl("GYO") = "000"
        ...
        '荷主固有データ
        drEdiRcvDtl("DELIVERY") = Me._Blc.LeftB(drSetDtl("COLUMN_1").ToString().Trim(), 9)                     '注文番号
        drEdiRcvDtl("GOODS_ISSUE_DATE") = Format(Convert.ToDateTime(drSetDtl("COLUMN_2")), "yyyyMMdd")         '発送予定日
        drEdiRcvDtl("OVERALL_PICK_STATUS") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 2)          'ピック進捗状態
        drEdiRcvDtl("MATERIAL") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 11).Substring(3)       '商品コード
        drEdiRcvDtl("DESCRIPTION") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 80)                 '商品名
        drEdiRcvDtl("DERIVERY_QUANTITY") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 12)           '入荷個数量
        drEdiRcvDtl("BATCH") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 10)                       'ロット№
        drEdiRcvDtl("SOLDTO_PARTY") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 10)                '売上先コード
        drEdiRcvDtl("NAME_OF_SOLDTO_PARTY") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 40)        '売上先名
        drEdiRcvDtl("DERIVERY_DATE") = Format(Convert.ToDateTime(drSetDtl("COLUMN_10")), "yyyyMMdd")           '入荷日
        drEdiRcvDtl("SHIPPNG_POINT") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 4)               '輸送地点
        drEdiRcvDtl("ROUTE") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 6)                       'ルート
        drEdiRcvDtl("PURCHASING_DOCUMENT") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 10)        '購買文書
        drEdiRcvDtl("TOTAL_WEIGHT") = Me._Blc.LeftB(drSetDtl("COLUMN_14").ToString().Trim(), 12)               '入荷重量
        drEdiRcvDtl("PLANT") = Me._Blc.LeftB(drSetDtl("COLUMN_15").ToString().Trim(), 4)                       'プラント
        drEdiRcvDtl("TOTAL_GDS_MVT_STAT") = Me._Blc.LeftB(drSetDtl("COLUMN_18").ToString().Trim(), 2)          'ステータス
        drEdiRcvDtl("SALES_UNIT") = Me._Blc.LeftB(drSetDtl("COLUMN_19").ToString().Trim(), 5)                  '荷姿
        drEdiRcvDtl("DELIVERY_TYPE") = Me._Blc.LeftB(drSetDtl("COLUMN_20").ToString().Trim(), 5)               '入荷タイプ
        drEdiRcvDtl("LODING_DATE") = Format(Convert.ToDateTime(drSetDtl("COLUMN_21")), "yyyyMMdd")             '更新日
        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "1"                                                          '実績処理フラグ

        'データセットに設定
        ds.Tables("LMH010_INKAEDI_DTL_ROM").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI入荷(中))"
    ...
    Private Function SetSemiInkaEdiM(ByVal setDs As DataSet _
                                    ...
                                    , ByVal ediIndex As Integer _
                                    ) As DataSet

        Dim drInkaEdiM As DataRow = setDs.Tables("LMH010_INKAEDI_M").NewRow()
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH010_INKAEDI_DTL_ROM").Rows(0)
        ...
        If drRcvEdiDtl.Item("SALES_UNIT").ToString().Equals(LMH010BLC209.NISUGATA_KO) = True Then
        ...
        Else
            ...
            Try
                Convert.ToInt32(drRcvEdiDtl("DERIVERY_QUANTITY").ToString())
            ...
            If errFlg = True Then
                ...
                '2015.09.24 ローム横浜追加START
                If RomeYok00003_00 = ediIndex Then
                    drInkaEdiM("IRIME") = drRcvEdiDtl("DERIVERY_QUANTITY").ToString()
                End If
                '2015.09.24 ローム横浜追加END

            Else

                '2015.06.01 ローム追加修正START
                If setDs.Tables("LMH010_M_GOODS").Rows.Count = 0 Then

                    drInkaEdiM("NB") = 0
                    drInkaEdiM("HASU") = 0
                    drInkaEdiM("INKA_PKG_NB") = 0
                    '2015.09.24 ローム横浜追加修正START
                    If RomeYok00003_00 = ediIndex Then
                        drInkaEdiM("IRIME") = drRcvEdiDtl("DERIVERY_QUANTITY").ToString()
                    End If
                    '2015.09.24 ローム横浜追加修正END
                Else
                    drInkaEdiM("NB") = Convert.ToInt32(drRcvEdiDtl("DERIVERY_QUANTITY").ToString()) / Convert.ToDecimal(setDs.Tables("LMH010_M_GOODS").Rows(0).Item("STD_IRIME_NB").ToString())
                    drInkaEdiM("HASU") = Convert.ToInt32(drRcvEdiDtl("DERIVERY_QUANTITY").ToString()) Mod Convert.ToDecimal(setDs.Tables("LMH010_M_GOODS").Rows(0).Item("STD_IRIME_NB").ToString())
                    drInkaEdiM("INKA_PKG_NB") = Convert.ToInt32(drRcvEdiDtl("DERIVERY_QUANTITY").ToString()) / Convert.ToDecimal(setDs.Tables("LMH010_M_GOODS").Rows(0).Item("STD_IRIME_NB").ToString())
                End If
                '2015.06.01 ローム追加修正END
            End If
        ...
        drInkaEdiM("REMARK") = drRcvEdiDtl("SALES_UNIT")
        ...
        drInkaEdiM("FREE_C01") = String.Empty
        drInkaEdiM("FREE_C02") = String.Empty
        drInkaEdiM("FREE_C03") = String.Empty
        drInkaEdiM("FREE_C04") = String.Empty
...
#Region "セミEDI時　データセット設定(EDI入荷(大))"
    ...
    Private Function SetSemiinkaediL(ByVal setDs As DataSet) As DataSet
        ...
        '税区分コード（ローム横浜用）
        Dim drZeiKbnRome As DataRow = Nothing
        If setDs.Tables("LMH010_ZEI_KBN_ROME").Rows.Count = 1 Then
            drZeiKbnRome = setDs.Tables("LMH010_ZEI_KBN_ROME").Rows(0)
        End If
        ...
        drInkaEdiL("INKA_DATE") = drRcvEdiDtl("DERIVERY_DATE")
        ...
        drInkaEdiL("OUTKA_FROM_ORD_NO") = drRcvEdiDtl("DELIVERY")
        drInkaEdiL("SEIQTO_CD") = drRcvEdiDtl("SOLDTO_PARTY")

        drInkaEdiL("TOUKI_HOKAN_YN") = "1"
        drInkaEdiL("HOKAN_YN") = "1"
        drInkaEdiL("HOKAN_FREE_KIKAN") = 0

        drInkaEdiL("HOKAN_STR_DATE") = drRcvEdiDtl("DERIVERY_DATE")
        drInkaEdiL("NIYAKU_YN") = "1"
        '2018/12/11 ADD START 要望管理003739
        '2019/01/29 MOD START 要望管理004404 条件に営業所コードを追加、設定値を区分マスタからの取得値に変更
        If drInkaEdiL("NRS_BR_CD").ToString = "40" AndAlso drInkaEdiL("CUST_CD_L").ToString = "00003" AndAlso drInkaEdiL("CUST_CD_M").ToString = "00" Then
            drInkaEdiL("TAX_KB") = drZeiKbnRome("ZEI_KBN_CD2")
        ElseIf drInkaEdiL("NRS_BR_CD").ToString = "40" AndAlso drInkaEdiL("CUST_CD_L").ToString = "00061" AndAlso drInkaEdiL("CUST_CD_M").ToString = "00" Then
            drInkaEdiL("TAX_KB") = drZeiKbnRome("ZEI_KBN_CD1")
            '2019/01/29 MOD END   要望管理004404
        Else
            '2018/12/11 ADD END   要望管理003739
            drInkaEdiL("TAX_KB") = "01"  '01:課税
        End If '2018/12/11 ADD 要望管理003739
        drInkaEdiL("REMARK") = drRcvEdiDtl("PURCHASING_DOCUMENT")
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_ROM")
        ...
        '    Dim dtRcvHedDel As DataTable = ds.Tables("LMH010_DTL_ROM_CANCELOUT")
        '    Dim drRcvHedDel As DataRow = ds.Tables("LMH010_DTL_ROM_CANCELOUT").Rows(0)

        '    Dim dtRcvDtlDel As DataTable = ds.Tables("LMH010_DTL_ROM_CANCELOUT")
        '    Dim drRcvDtlDel As DataRow = ds.Tables("LMH010_DTL_ROM_CANCELOUT").Rows(0)
    ...
    Private Function SemiEdiTorikomiChk(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To hedmax
            ...
            Else
                ...
                'EXCELデータ抽出条件を取得する
                ds = MyBase.CallDAC(Me._Dac, "SelectDataExtraction", ds)
                ...
                For j As Integer = iRowCnt To max
                    ...
                    If (dr.Item("FILE_NAME_RCV").ToString().Trim()).Equals(dtSemiHed.Rows(i).Item("FILE_NAME_RCV").ToString().Trim()) = True Then
                        ...
                        '荷姿情報を取得する
                        setDs = MyBase.CallDAC(Me._Dac, "CheckGoodsDetails", setDs)
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow, ByVal ediCustIndex As String) As Boolean
        ...
        sMsg = "商品名(カラム5番目)["
        sStr = dr.Item("COLUMN_5").ToString().Trim()
        ...
        sMsg = "ロット№(カラム7番目)["
        sStr = dr.Item("COLUMN_7").ToString().Trim()
        ...
        sMsg = "入荷日(カラム10番目)["
        sDate = dr.Item("COLUMN_10").ToString()
...
#Region "商品明細マスタ"
    ...
    Public Function TorikomiMstChk(ByVal dr As DataRow, ByVal ds As DataSet) As Boolean
        ...
        If ds.Tables("LMH010_M_GOODS_DETAILS").Rows.Count = 0 Then
            'MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E493", New String() {"荷姿情報", "商品マスタまたは商品明細マスタ", String.Concat("荷主商品コード：", dr.Item("COLUMN_4").ToString().Trim().Substring(3), " 商品名：", sGoodsNm)}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E493", New String() {"荷主商品コード", "商品マスタ", String.Concat("荷主商品コード：", dr.Item("COLUMN_4").ToString().Trim().Substring(3), " 商品名：", sGoodsNm)}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            Return bRet
        ElseIf ds.Tables("LMH010_M_GOODS_DETAILS").Rows.Count = 1 Then
            'mNisugata = ds.Tables("LMH010_M_GOODS_DETAILS").Rows(0).Item("SET_NAIYO").ToString()

        ElseIf ds.Tables("LMH010_M_GOODS_DETAILS").Rows.Count > 1 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E494", New String() {"荷主商品コード", "商品マスタ", String.Concat("荷主商品コード：", dr.Item("COLUMN_4").ToString().Trim().Substring(3))}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            Return bRet
        End If

        ''荷姿情報の整合性チェック
        'If sNisugata.Equals(mNisugata) = False Then
        '    MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E332", New String() {sMsg, "商品マスタ", String.Concat("荷主商品コード：", dr.Item("COLUMN_4").ToString().Trim().Substring(3), "商品名：", sGoodsNm, " 荷姿：", sNisugata)}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
        '    bRet = False
        'End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_ROM")         'EDI受信Dtl
        'Dim dtRcvDtlCancel As DataTable = ds.Tables("LMH010_DTL_ROM_CANCELOUT")  'EDI受信Hed
        ...
        '2015.09.24 ローム横浜追加START
        Dim ediindex As Integer
        '2015.09.24 ローム横浜追加END

        '2019/01/29 ADD START 要望管理004404
        '税区分コードの取得（ローム横浜用）
        Dim drSemiEdiInfo As DataRow = ds.Tables("LMH010_SEMIEDI_INFO").Rows(0)
        If drSemiEdiInfo("NRS_BR_CD").ToString = "40" _
        AndAlso (drSemiEdiInfo("CUST_CD_L").ToString = "00003" Or drSemiEdiInfo("CUST_CD_L").ToString = "00061") Then
            ds = MyBase.CallDAC(Me._Dac, "SelectZeiKbnCdForRome", ds)
        End If
        '2019/01/29 ADD END   要望管理004404
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            ds.Tables("LMH010_INKAEDI_DTL_ROM").Clear() '受信DTLをクリア
            ds = Me.SetSemiInkaEdiRcv(ds, i)
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_ROM").Rows(0)
            ...
            If Convert.ToDecimal(drEdiRcvDtl.Item("DERIVERY_QUANTITY").ToString()) = 0.0 Then
                Continue For
            End If

            ''2015.05.28 ローム セミEDI時にDELIVERYでまとめない対応START
            ''---------------------------------------------------------------------------
            '' キー項目設定
            ''---------------------------------------------------------------------------
            'sNewKey = drEdiRcvDtl.Item("DELIVERY").ToString
            ''2015.05.28 ローム セミEDI時にDELIVERYでまとめない対応END
            ...
            ''    '削除EDI管理番号に設定する
            ''    Dim drRcvDtlCancel As DataRow = ds.Tables("LMH010_DTL_ROM_CANCELOUT").Rows(0)
            ...
            '2015.09.24 ローム横浜追加START
            ediindex = Convert.ToInt32(ds.Tables("LMH010_SEMIEDI_INFO").Rows(0)("EDI_CUST_INDEX"))
            '2015.09.24 ローム横浜追加END
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH010_INKAEDI_DTL_ROM")
            setDtlDt.Clear()
            ...
            If iSkipFlg = 0 Then
                ...
                '2015.09.24 ローム横浜追加START
                setDs = Me.SetSemiInkaEdiM(setDs, sWhcd, sCustCdL, sCustCdM, sNrsGoodsCd, sNrsGoodsNm, sIrime, ediindex)
                '2015.09.24 ローム横浜追加END
