...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC506 = New LMH030DAC506()
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _DsetInsFlg As Boolean
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        'EDI出荷(大)の初期値設定
        'ゴードー溶剤の場合、届先のエラーチェック有
        If Me.SetEdiLShoki(ds, rowNo, ediCtlNo) = False Then
            Return ds
        End If
        ...
        '注)基本的にゴードー溶剤はまとめ処理を行わないが、共通まとめ処理のロジックを組み込む
        ...
        If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
            ...
            ElseIf MyBase.GetResultCount > 1 Then
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.GODO_WID_L006, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningL("W199", LMH030BLC.GODO_WID_L006, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("0") = True Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.GODO_WID_L007, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = String.Empty
                    matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                    ds = Me._Blc.SetComWarningL("W168", LMH030BLC.GODO_WID_L007, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("1") = True Then
                ...
                If matomeStatus.Equals("10") = False Then
                    ...
                    choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.GODO_WID_L008, 0)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(4) = String.Empty
                        matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                        ds = Me._Blc.SetComWarningL("W198", LMH030BLC.GODO_WID_L008, ds, msgArray, matomeNo, String.Empty)
                        Return ds
        ...
        '        choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.GODO_WID_L009, 0)
        ...
        '            msgArray(4) = String.Empty
        '            matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
        '            ds = Me._Blc.SetComWarningL("W168", LMH030BLC.GODO_WID_L009, ds, msgArray, matomeNo, String.Empty)
        '            Return ds
...
#Region "EDI_Lの初期値設定(出荷登録処理)"
    ...
    Private Function SetEdiLShoki(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        If ds.Tables("LMH030_M_DEST").Rows.Count > 0 Then
            ...
            mDestFlgYN = True
        ElseIf ds.Tables("LMH030_M_DEST").Rows.Count = 0 Then

            '届先コードの類似コードで届先Mより取得
            ds = MyBase.CallDAC(Me._Dac, "SelectDataRuijiMdest", ds)
            '届先マスタの自動追加
            ...
            ElseIf ds.Tables("LMH030_M_DEST").Rows.Count > 1 Then
                'ゴードー溶剤の場合、類似コード検索で複数件の場合はエラー
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E330", New String() {"届先コード", "届先マスタ"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                Return False
            End If

        End If
        'ゴードー溶剤専用(届先マスタ取得SQL)修正END
    ...
    Private Function EdiMDefaultSet(ByVal ds As DataSet, ByVal setDs As DataSet, _
        ...
        ElseIf String.IsNullOrEmpty(mGoodsNm) = False AndAlso String.IsNullOrEmpty(mDestGoodsNm) = False AndAlso _
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                ds = Me._Blc.SetComWarningM("W194", LMH030BLC.GODO_WID_M001, ds, setDs, msgArray, _
        ...
        'ゴードー溶剤専用 EDI出荷(大)の再設定項目
        ''-------------------------------------------------------------------------------------
        ''●荷主固有設定(ゴードー溶剤専用)①荷送人コード,荷送人名
        ''-------------------------------------------------------------------------------------

        If String.IsNullOrEmpty(mGoodsDr("SHIP_CD_L").ToString()) = False Then
            ediLDr("SHIP_CD_L") = mGoodsDr("SHIP_CD_L")

            '荷送人名の設定
            ds = MyBase.CallDAC(Me._DacCom, "SelectDataMdestShip", ds)
            If ds.Tables("LMH030_M_DEST_SHIP_CD_L").Rows.Count <> 0 Then
                ediLDr("SHIP_NM_L") = ds.Tables("LMH030_M_DEST_SHIP_CD_L").Rows(0).Item("DEST_NM").ToString().Trim()
            End If

        End If

        ''-------------------------------------------------------------------------------------
        ''●荷主固有設定(ゴードー溶剤専用)②買主注文番号(全体)
        ''-------------------------------------------------------------------------------------
        If String.IsNullOrEmpty(ediLDr("BUYER_ORD_NO").ToString()) = True Then
            '買主注文番号(明細)の先頭行をセット
            ediLDr("BUYER_ORD_NO") = ediMDr("BUYER_ORD_NO_DTL")
        End If

        '商品Remark
        ediMDr("FREE_C28") = mGoodsDr("OUTKA_ATT")
...
#Region "EDI出荷(大)のDAC側でのチェック"
    ...
    Private Function EdiLDbExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        ''オーダー番号重複チェック
        'ゴードー溶剤特殊チェック↓
        'EDI出荷(大)の届先CDが、"107702136", "015000067", "015000070"の場合はチェックを行わない。
        If (drEdiL.Item("DEST_CD").ToString()).Equals("107702136") = False AndAlso _
           (drEdiL.Item("DEST_CD").ToString()).Equals("015000067") = False AndAlso _
           (drEdiL.Item("DEST_CD").ToString()).Equals("015000070") = False Then
        ...
        '届先Mが１件かつ届先自動追加フラグがFALSE
        If mDestCount = 1 AndAlso Me._DsetInsFlg = False Then
            ...
            '届先Mが１件(類似コードより取得)かつ届先自動追加フラグがTRUE
        ElseIf mDestCount = 1 AndAlso Me._DsetInsFlg = True Then
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                ''重要)ゴードー溶剤の場合、商品マスタの１行目(GOODS_CD_NRSでORDER BYをした値を使用するのでワーニングにしない)
                'choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.GODO_WID_M002, 0)
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    Else
                        ...
                        'ds = Me._Blc.SetComWarningM("W162", LMH030BLC.GODO_WID_M002, ds, setDs, msgArray, custGoodsCd, String.Empty)
                ...
                If Me.EdiMDefaultSet(ds, setDs, i, unsoData, rowNo, ediCtlNo) = False Then
                    ...
                    'ゴードー溶剤はEDI出荷(中)でワーニングチェックがある為、エラーorワーニングの判断を行う
                    Dim chkRowNo As Integer = Convert.ToInt32(rowNo)
                    If MyBase.IsMessageStoreExist(chkRowNo) = True Then
                        '整合性チェックでエラーがあった場合は処理終了
                        Return False
...
#Region "届先マスタチェック(ゴードー溶剤専用)"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        Dim compareWarningFlg As Boolean = False
        Dim shoguchiStr As String = "諸口"
        Dim allNine As String = "999"
        Dim idoData As String = "950000"
        ...
        mDestNm = Me._Blc.SpaceCutChk(mDestNm)
        ediDestNm = Me._Blc.SpaceCutChk(ediDestNm)

        '移動データおよび届先名称未登録対応
        If ediDestCd.Equals(idoData) = True OrElse String.IsNullOrEmpty(ediDestNm) = True Then

            dtEdi.Rows(0)("DEST_NM") = dtMdest.Rows(0).Item("DEST_NM").ToString()
            dtEdi.Rows(0)("DEST_ZIP") = String.Concat(dtMdest.Rows(0).Item("ZIP").ToString().Substring(0, 3), _
                                                      "-", _
                                                      dtMdest.Rows(0).Item("ZIP").ToString().Substring(3, 4))
            ...
            dtEdi.Rows(0)("FREE_C21") = String.Concat(dtMdest.Rows(0).Item("AD_1").ToString(), _
                                                      dtMdest.Rows(0).Item("AD_2").ToString(), _
                                                      dtMdest.Rows(0).Item("AD_3").ToString())
            dtEdi.Rows(0)("DEST_TEL") = dtMdest.Rows(0).Item("TEL").ToString()

        End If

        '届先諸口チェック
        If mDestNm.Equals(shoguchiStr) = True AndAlso (Right(ediDestCd, 3)).Equals(allNine) = True Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E320", New String() {String.Concat("届先マスタの内容が", shoguchiStr), "出荷登録", shoguchiStr}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            Return False
        End If

        '届先名称(届先マスタの届先名にEDI出荷(大)の届先名称が一致しなければワーニング)
        If mDestNm.Equals(ediDestNm) = True Then
            'チェックなし
            'ElseIf InStr(mDestNm.ToString().Trim(), ediDestNm.ToString().Trim()) = 0 Then
        ElseIf (mDestNm.Trim()).Equals(ediDestNm.ToString().Trim()) = False Then
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.GODO_WID_L001, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                ds = Me._Blc.SetComWarningL("W166", LMH030BLC.GODO_WID_L001, ds, msgArray, _
        ...
        'FREE_C21:届先住所(届先マスタの住所1+住所2+住所3にEDI出荷(大)のFREE_C21が一致しなければワーニング)
        If String.IsNullOrEmpty(ediFreeC21) = True Then
            'チェックなし
        Else

            mAdAll = Me._Blc.SpaceCutChk(mAdAll)
            ediFreeC21 = Me._Blc.SpaceCutChk(ediFreeC21)

            If (mAdAll.ToString().Trim()).Equals(ediFreeC21.ToString().Trim()) = False Then

                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.GODO_WID_L002, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.GODO_WID_L002, ds, msgArray, ediFreeC21, mAdAll)
        ...
        Else
            If mTel.Equals(ediTel) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.GODO_WID_L003, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.GODO_WID_L003, ds, msgArray, ediTel, mTel)
        ...
        Else
            If mZip.Equals(Replace(ediZip, "-", String.Empty)) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.GODO_WID_L004, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.GODO_WID_L004, ds, msgArray, ediZip, mZip)
...
#Region "届先マスタ自動追加(ゴードー溶剤専用)"
    ...
    Private Function SetInsMDestFromDest(ByVal ds As DataSet, ByVal workDestCd As String, ByVal workDestString As String) As Boolean
        ...
        Dim msgArray(5) As String
        Dim choiceKb As String = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.GODO_WID_L005, 0)
        Dim drEdiL As DataRow = ds.Tables("LMH030_OUTKAEDI_L").Rows(0)
        Dim compareWarningFlg As Boolean = False
        ...
        If String.IsNullOrEmpty(choiceKb) = True Then
            ...
            ds = Me._Blc.SetComWarningL("W182", LMH030BLC.GODO_WID_L005, ds, msgArray, workDestCd, String.Empty)

            compareWarningFlg = True

        ElseIf choiceKb.Equals("01") = True Then
            'ワーニングで"はい"を選択時
            '類似コードより取得してきた届先情報をそのまま使用
            '入れ替える値はEDI出荷(大)の届先コード
            Dim drMD As DataRow = dtMD.Rows(0)
...
#Region "データセット設定(出荷M)"
    ...
    Private Function SetDatasetOutkaM(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            'Start 要望管理2055 商品マスタからの取得
            'outkaDr("REMARK") = ediDr("REMARK")
            Dim goodsRemark As String = ediDr("FREE_C28").ToString()
            If ediDr("REMARK").ToString().Contains(goodsRemark) = False Then
                outkaDr("REMARK") = String.Concat(ediDr("REMARK").ToString(), " ", ediDr("FREE_C28").ToString()).Trim
            Else
                outkaDr("REMARK") = ediDr("REMARK").ToString()
            End If
            'End 要望管理2055 

            outkaDr("SIZE_KB") = String.Empty
            'ゴードー溶剤(久喜のみ)専用設定項目START
            'outkaDr("ZAIKO_KB") = ediDr("FREE_C06")
            outkaDr("ZAIKO_KB") = String.Empty
            'ゴードー溶剤(久喜のみ)専用設定項目END
    ...
    Private Function SetdatasetUnsoJyuryo(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = True Then
            ...
            If MyBase.GetResultCount = 0 Then
                ...
                '2012.06.15 ゴードー特殊処理 修正START
                'unsoLDr("NB_UT") = ediMDr("KB_UT")
                unsoLDr("NB_UT") = ediMDr("PKG_UT")
                '2012.06.15 ゴードー特殊処理 修正END
                Return ds
