...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC131 = New LMH030DAC131()
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet, ByVal mGoodsCnt As Integer) As DataSet
        ...
        Dim pkgNb As Double = Convert.ToDouble(mGoodsDt.Rows(0).Item("PKG_NB"))
        Dim outkaTtlNb As Double = Convert.ToDouble(rcvDtlDr("LOAD_NO"))
        Dim outkaPkgNb As Double = Math.Floor(outkaTtlNb / pkgNb)
        Dim outkaHasu As Double = outkaTtlNb Mod pkgNb

        outkaediMDr("OUTKA_PKG_NB") = outkaPkgNb
        outkaediMDr("OUTKA_HASU") = outkaHasu
        outkaediMDr("OUTKA_QT") = rcvDtlDr("TOTAL_NO")
        outkaediMDr("OUTKA_TTL_NB") = outkaTtlNb
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet) As DataSet
        ...
        outkaediLDr("OUTKA_PLAN_DATE") = CDate(rcvDtlDr("OUTKA_PLAN_DATE").ToString).ToString("yyyyMMdd")
        outkaediLDr("OUTKO_DATE") = CDate(rcvDtlDr("OUTKA_PLAN_DATE").ToString).ToString("yyyyMMdd")
        ...
        For i As Integer = 0 To max
            ...
            If mGoodsCnt > 1 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, _
                                       "E975", _
                                       New String() {String.Concat("商品コード[", setDs.Tables("LMH030_H_OUTKAEDI_DTL_GKE").Rows(0).Item("CUST_GOODS_CD").ToString, "]")}, _
