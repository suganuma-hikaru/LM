...
'  システム名       :  LM
'  サブシステム名   :  LMH              : EDI
'  プログラムID     :  LMH010HBLC122    : EDI入荷検索:フィルメニッヒ(404)から複写
'  EDI荷主ID　　　　:  45741　　　      : ミマキエンジニアリング社
'  作  成  者       :  安念
' ==========================================================================
Imports Jp.Co.Nrs.LM.Const
Imports Jp.Co.Nrs.LM.DAC
Imports Jp.Co.Nrs.LM.DAC.LMH010DAC122
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC122 = New LMH010DAC122()
...
#End Region
    ...
    '    Public Enum IsDateType
    '        NoDate = 0             '日付以外
    '        DateType1 = 1          'yyyyMMdd形式の日付
    '        DateType2 = 2          'yyyy/MM/dd形式の日付
    '        DateType3 = 3          'yyyy/M/d形式の日付
    '        DateTypeEtc = 4        '上記形式以外の日付
    '    End Enum
...
#Region "入荷管理番号(中)取得"
    ...
    Private Function GetInkaNoM(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        Else
            Dim goodsCodeNrs As String = String.Empty
            Dim inkaCtlNoM As Integer = 0
            For i As Integer = 0 To max
                If Not goodsCodeNrs.Equals(dt.Rows(i)("NRS_GOODS_CD").ToString) Then
                    inkaCtlNoM += 1
                    goodsCodeNrs = dt.Rows(i)("NRS_GOODS_CD").ToString
                End If
                dt.Rows(i)("INKA_CTL_NO_M") = inkaCtlNoM.ToString("000")
...
#Region "データセット設定(入荷L)"
    ...
    Private Function SetDatasetInkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            inkaDr("OUTKA_FROM_ORD_NO_L") = String.Empty
...
#Region "データセット設定(入荷M)"
    ...
    Private Function SetDatasetInkaM(ByVal ds As DataSet) As DataSet
        ...
        Dim beforInkaNoM As String = String.Empty

        For i As Integer = 0 To max

            ediDr = ds.Tables("LMH010_INKAEDI_M").Rows(i)

            '入荷管理番号Ｍが前レコードと同じなら設定を飛ばす
            If beforInkaNoM.Equals(ediDr("INKA_CTL_NO_M").ToString) Then
                Continue For
            Else
                beforInkaNoM = ediDr("INKA_CTL_NO_M").ToString
            End If
...
#Region "データセット設定(入荷S)"
    ...
    Private Function SetDatasetInkaS(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            '入荷管理番号（中）が切り替わるタイミングで入荷管理番号（小）はクリア
            '切り替わるタイミングでなければ入荷管理番号（小）はインクリメントする
            If inkaCtlNoM.Equals(ediDr("INKA_CTL_NO_M").ToString) Then
                inkaCtlNoS += 1
            Else
                inkaCtlNoS = 1
                inkaCtlNoM = ediDr("INKA_CTL_NO_M").ToString
            End If

            inkaDr("NRS_BR_CD") = ediDr("NRS_BR_CD")
            inkaDr("INKA_NO_L") = ediDr("INKA_CTL_NO_L")
            inkaDr("INKA_NO_M") = ediDr("INKA_CTL_NO_M")
            inkaDr("INKA_NO_S") = inkaCtlNoS.ToString("000")
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    Private Function SetSemiInkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_MMK").NewRow()
        ...
        ' 荷主固有データ
        drEdiRcvDtl("GYO_NO") = (i + 1).ToString
        drEdiRcvDtl("SYUKKO_KIBOU_DATE") = Format(Convert.ToDateTime(drSetDtl(GetColumnName(MimakiArrivalColumns.SYUKKO_KIBOU_DATE))), "yyyy/MM/dd")
        drEdiRcvDtl("PALETTE_NO") = Left(drSetDtl(GetColumnName(MimakiArrivalColumns.PALETTE_NO)).ToString().Trim(), 40)
        drEdiRcvDtl("BOX_NO") = Left(drSetDtl(GetColumnName(MimakiArrivalColumns.BOX_NO)).ToString().Trim(), 40)
        drEdiRcvDtl("KAKUNIN") = Left(drSetDtl(GetColumnName(MimakiArrivalColumns.KAKUNIN)).ToString().Trim(), 10)
        drEdiRcvDtl("HINBAN") = Left(drSetDtl(GetColumnName(MimakiArrivalColumns.HINBAN)).ToString().Trim(), 20)
        drEdiRcvDtl("HINMEI") = Left(drSetDtl(GetColumnName(MimakiArrivalColumns.HINMEI)).ToString().Trim(), 60)
        drEdiRcvDtl("LOT_NO") = Left(drSetDtl(GetColumnName(MimakiArrivalColumns.LOT_NO)).ToString().Trim(), 10)
        drEdiRcvDtl("YUUKOU_KIGEN") = Left(drSetDtl(GetColumnName(MimakiArrivalColumns.YUUKOU_KIGEN)).ToString().Trim(), 10)
        drEdiRcvDtl("ZAIKO_SUU") = Left(drSetDtl(GetColumnName(MimakiArrivalColumns.ZAIKO_SUU)).ToString().Trim(), 9)
        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "1"                                                          '実績処理フラグ

        'データセットに設定
        ds.Tables("LMH010_INKAEDI_DTL_MMK").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI入荷(中))"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetSemiInkaEdiM(ByVal setDs As DataSet) As DataSet

        Dim drInkaEdiM As DataRow = setDs.Tables("LMH010_INKAEDI_M").NewRow()
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH010_INKAEDI_DTL_MMK").Rows(0)
        Dim drSemiEdiInfo As DataRow = setDs.Tables("LMH010_SEMIEDI_INFO").Rows(0)
        Dim drGoods As DataRow = setDs.Tables("LMH010_M_GOODS").Rows(0)
        ...
        drInkaEdiM("NRS_GOODS_CD") = drGoods("GOODS_CD_NRS")
        drInkaEdiM("CUST_GOODS_CD") = drRcvEdiDtl("HINBAN")
        drInkaEdiM("GOODS_NM") = drRcvEdiDtl("HINMEI")
        drInkaEdiM("NB") = "1"
        drInkaEdiM("NB_UT") = drGoods.Item("NB_UT")
        drInkaEdiM("PKG_NB") = drGoods.Item("PKG_NB")
        drInkaEdiM("PKG_UT") = drGoods.Item("PKG_UT")
        drInkaEdiM("INKA_PKG_NB") = "1"
        drInkaEdiM("HASU") = 0
        drInkaEdiM("STD_IRIME") = drGoods.Item("STD_IRIME_NB")
        drInkaEdiM("STD_IRIME_UT") = drGoods.Item("STD_IRIME_UT")
        drInkaEdiM("BETU_WT") = drGoods.Item("STD_WT_KGS")
        drInkaEdiM("CBM") = drGoods.Item("STD_CBM")
        drInkaEdiM("ONDO_KB") = drGoods.Item("ONDO_KB")
        drInkaEdiM("OUTKA_FROM_ORD_NO") = String.Empty
        drInkaEdiM("BUYER_ORD_NO") = String.Empty
        drInkaEdiM("REMARK") = String.Empty
        drInkaEdiM("LOT_NO") = drRcvEdiDtl("PALETTE_NO")
        drInkaEdiM("SERIAL_NO") = drRcvEdiDtl("BOX_NO")
        drInkaEdiM("IRIME") = drGoods.Item("STD_IRIME_NB")
        drInkaEdiM("IRIME_UT") = drGoods.Item("STD_IRIME_UT")
        drInkaEdiM("OUT_KB") = "0"
        ...
        drInkaEdiM("FREE_C01") = drRcvEdiDtl("GYO_NO")
        drInkaEdiM("FREE_C02") = drRcvEdiDtl("BOX_NO")
        drInkaEdiM("FREE_C03") = drRcvEdiDtl("LOT_NO")
        drInkaEdiM("FREE_C04") = "20" & drRcvEdiDtl("YUUKOU_KIGEN").ToString
        drInkaEdiM("FREE_C05") = String.Empty
        drInkaEdiM("FREE_C06") = String.Empty
        drInkaEdiM("FREE_C07") = String.Empty
...
#Region "セミEDI時　データセット設定(EDI入荷(大))"
    ...
    Private Function SetSemiinkaediL(ByVal setDs As DataSet) As DataSet
        ...
        drInkaEdiL("INKA_TP") = "10"                                            ' 入荷データ種別:通常
        drInkaEdiL("INKA_KB") = "10"                                            ' 入荷データ区分:通常
        drInkaEdiL("INKA_STATE_KB") = "10"                                      ' 入荷作業進捗:予定入力済
        drInkaEdiL("INKA_DATE") = Jp.Co.Nrs.Com.Utility.DateFormatUtility.DeleteSlash(drRcvEdiDtl("SYUKKO_KIBOU_DATE").ToString)
        drInkaEdiL("INKA_TIME") = String.Empty
        ...
        drInkaEdiL("NAIGAI_KB") = String.Empty
        drInkaEdiL("BUYER_ORD_NO") = String.Empty
        drInkaEdiL("OUTKA_FROM_ORD_NO") = String.Empty

        drInkaEdiL("SEIQTO_CD") = String.Empty

        drInkaEdiL("TOUKI_HOKAN_YN") = "1"                                      ' 当期保管料負担有無:有
        drInkaEdiL("HOKAN_YN") = "1"                                            ' 保管料有無:有
        drInkaEdiL("HOKAN_FREE_KIKAN") = 0

        drInkaEdiL("HOKAN_STR_DATE") = String.Empty
        drInkaEdiL("NIYAKU_YN") = "1"                                           ' 荷役料有無:有
        drInkaEdiL("TAX_KB") = "01"                                             ' 税区分:課税
        drInkaEdiL("REMARK") = String.Empty
        drInkaEdiL("NYUBAN_L") = String.Empty
        drInkaEdiL("UNCHIN_TP") = "90"                                          ' 運賃タイプ:未定
        ...
        drInkaEdiL("OUT_FLAG") = "0"                                            ' 入荷ﾃﾞｰﾀ書込フラグ:未書込
        drInkaEdiL("AKAKURO_KB") = "0"                                          ' 赤黒区分:黒
        drInkaEdiL("JISSEKI_FLAG") = "0"                                        ' 実績書込フラグ:未出力
        ...
        drInkaEdiL("FREE_C20") = String.Empty
        ...
        drInkaEdiL("FREE_C29") = String.Empty   ' タリフ設定処理で使用
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_MMK")
        ...
        If iSkipFlg = 0 Then
            ...
            If bSameKeyFlg = False Then
                ...
                Dim num As New NumberMasterUtility
                '2018/02/20 000731 セミEDI_新規開発ディストリビューション･ミマキエンジニアリング-入庫･出庫 upd start
                '入荷の管理番号を出荷から取得していたバグ対応
                ...
                '2018/02/20 000731 セミEDI_新規開発ディストリビューション･ミマキエンジニアリング-入庫･出庫 upd end
            End If
...
#Region "カラム項目の値・日付チェック"
    ...
    Private Function TorikomiValChk(ByVal dr As DataRow) As Boolean
        ...
        '未設定、日付でなければエラー
        bIsErr = False
        sMsg = "出荷希望日(カラム" & CType(MimakiArrivalColumns.SYUKKO_KIBOU_DATE, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(MimakiArrivalColumns.SYUKKO_KIBOU_DATE)).ToString().Trim()
        bIsErr = sStr.Equals(String.Empty)
        bIsErr = bIsErr OrElse Not IsDate(sStr)
        'エラーメッセージ出力
        If bIsErr Then
        ...
        '未設定、40桁より大きければエラー
        bIsErr = False
        sMsg = "パレット番号(カラム" & CType(MimakiArrivalColumns.PALETTE_NO, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(MimakiArrivalColumns.PALETTE_NO)).ToString().Trim()
        bIsErr = sStr.Equals(String.Empty)
        bIsErr = bIsErr OrElse sStr.Length > 40
        'エラーメッセージ出力
        If bIsErr Then
        ...
        '未設定、40桁より大きければエラー
        bIsErr = False
        sMsg = "箱番号(カラム" & CType(MimakiArrivalColumns.BOX_NO, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(MimakiArrivalColumns.BOX_NO)).ToString().Trim()
        bIsErr = sStr.Equals(String.Empty)
        bIsErr = bIsErr OrElse sStr.Length > 40
        'エラーメッセージ出力
        If bIsErr Then
        ...
        '未設定、20桁より大きければエラー
        bIsErr = False
        sMsg = "品番(カラム" & CType(MimakiArrivalColumns.HINBAN, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(MimakiArrivalColumns.HINBAN)).ToString().Trim()
        bIsErr = sStr.Equals(String.Empty)
        bIsErr = bIsErr OrElse sStr.Length > 20
        'エラーメッセージ出力
        If bIsErr Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E433", New String() {String.Concat(sMsg, sStr, "]")}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        ...
        '60桁より大きければエラー
        sMsg = "品名(カラム" & CType(MimakiArrivalColumns.HINMEI, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(MimakiArrivalColumns.HINMEI)).ToString().Trim()
        'エラーメッセージ出力
        ...
        '有効期限（カラム９番目）
        '頭に"20"を付けて日付として扱えなければエラー（未設定場合は20を日付として判定するのでエラーとなる）
        sMsg = "有効期間(カラム" & CType(MimakiArrivalColumns.YUUKOU_KIGEN, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(MimakiArrivalColumns.YUUKOU_KIGEN)).ToString().Trim()
        If Not IsTargetDateFormat("20" & sStr, "yyyyMMdd") Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E445", New String() {String.Concat(sMsg, sStr, "]")}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
    ...
    ' ''' <returns>求めた日付形式（IsDateType列挙体参照）</returns>
    ' ''' <remarks>
    ' ''' 対象日付文字列にスラッシュが入っていた場合、スラッシュを削除する
    ' ''' 対象日付文字列が10文字でなかったり、スラッシュが入っていない場合、
    ' ''' 入力されたままの文字列を参照渡しで受け渡す。
    ' ''' 最終的に加工された文字列がどのようなフォーマットであるかを返却する
    ' ''' </remarks>
    'Private Function GetSlashDelDate(ByRef value As String) As IsDateType
    '    If value.Length.Equals(10) Then
    ...
    ' ''' <summary>
    ' ''' 日付をyyyy/MM/ddからyyyyMMdd形式にする
    ' ''' </summary>
    ' ''' <param name="value">対象日付文字列</param>
    ' ''' <returns>求めた日付形式（IsDateType列挙体参照）</returns>
    ' ''' <remarks>
    ' '''対象日付文字列が8文字でなかった場合、入力されたままの文字列を参照渡しで受け渡す
    ' ''' 最終的に加工された文字列がどのようなフォーマットであるかを返却する
    ' ''' </remarks>
    'Private Function GetSlashEditDate(ByRef value As String) As IsDateType
    '    If value.Length.Equals(8) Then
    '        value = String.Concat(value.Substring(0, 4), "/", value.Substring(4, 2), "/", value.Substring(6, 2))
    '    End If
    '    Return ChekDate(value)
    'End Function

    ' ''' <summary>
    ' ''' 対象日付文字列がどのようなフォーマットであるかを返却する
    ' ''' </summary>
    ' ''' <param name="value">対象日付文字列</param>
    ' ''' <returns>求めた日付形式（IsDateType列挙体参照）</returns>
    ' ''' <remarks>
    ' ''' 引数に設定された文字列のフォーマット状態をIsDateType列挙体の要素にて返却する
    ' ''' 日付形式でない場合、IsDateType.NoDateを返却する
    ' ''' yyyyMMdd形式の場合、IsDateType.DateType1を返却する
    ' ''' yyyy/MM/dd形式の場合、IsDateType.DateType2を返却する
    ' ''' yyyy/M/d形式の場合、IsDateType.DateType3を返却する
    ' ''' 日付形式で上記以外のフォーマットである場合、IsDateType.DateTypeEtcを返却する
    ' ''' </remarks>
    'Private Function ChekDate(ByVal value As String) As IsDateType
    '    If Not IsDate(value) Then Return IsDateType.NoDate
    '    If IsTargetDateFormat(value, "yyyyMMdd") Then Return IsDateType.DateType1
    '    If IsTargetDateFormat(value, "yyyy/MM/dd") Then Return IsDateType.DateType2
    '    If IsTargetDateFormat(value, "yyyy/MM/dd") Then Return IsDateType.DateType3
    '    Return IsDateType.DateTypeEtc
    'End Function
    ...
    ''' <param name="formatString">日付フォーマット文字列</param>
    ''' <returns>True = 該当のフォーマット False = 該当のフォーマット外</returns>
    ''' <remarks></remarks>
    Public Shared Function IsTargetDateFormat(ByVal value As String, ByVal formatString As String) As Boolean
        Dim result As DateTime
        Return DateTime.TryParseExact(value, formatString, Nothing, Globalization.DateTimeStyles.None, result)
    End Function
...
#Region "商品明細マスタ"
    ...
    Public Function TorikomiMstChk(ByVal dr As DataRow, ByVal ds As DataSet) As Boolean
        ...
        Dim sGoodsCd As String = dr.Item(GetColumnName(MimakiArrivalColumns.HINBAN)).ToString
        Dim sGoodsNm As String = dr.Item(GetColumnName(MimakiArrivalColumns.HINMEI)).ToString
        Dim iGoodsCount As Integer = MyBase.GetResultCount()
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_MMK")         'EDI受信Dtl
        ...
        Dim beforBoxNo As String = String.Empty     'ループ一回前の箱番号
        Dim beforSyukkoKiboubi As String = String.Empty

        'ソート順変更(商品コード、シリアルNO)
        Dim dtSortDtl As DataTable
        Dim dv As DataView
        dtSortDtl = dtSetDtl.Clone()
        dv = New DataView(ds.Tables("LMH010_EDI_TORIKOMI_DTL"))
        dv.Sort() = "COLUMN_5 ASC , COLUMN_3 ASC"
        For Each drv As DataRowView In dv
            dtSortDtl.ImportRow(drv.Row)
        Next
        ds.Tables("LMH010_EDI_TORIKOMI_DTL").Clear()
        ds.Tables("LMH010_EDI_TORIKOMI_DTL").Merge(dtSortDtl)
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            ds.Tables("LMH010_INKAEDI_DTL_MMK").Clear() '受信DTLをクリア

            'EDI受信データセット設定
            ds = Me.SetSemiInkaEdiRcv(ds, i)
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_MMK").Rows(0)

            '箱番号が一回前のレコードと同じなら次の行へ
            If beforBoxNo.Equals(drEdiRcvDtl("BOX_NO").ToString) Then
                Continue For
            End If

            beforBoxNo = drEdiRcvDtl("BOX_NO").ToString
            ...
            If mGoodsCnt > 1 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, _
                                       "E975", _
                                       New String() {String.Concat("商品コード[", drEdiRcvDtl.Item("HINBAN").ToString, "]")}, _
                                       drEdiRcvDtl.Item("REC_NO").ToString(), _
            ...
            'If Convert.ToDecimal(drEdiRcvDtl.Item("OPEN_QUANTITY").ToString()) = 0.0 Then
            '    Continue For
            'End If

            '一回前のレコードと今回のレコードを比較して
            '同じであればbSameKeyFlgにTrueを、そうでなければFalseを設定する
            bSameKeyFlg = beforSyukkoKiboubi.Equals(drEdiRcvDtl("SYUKKO_KIBOU_DATE").ToString)
            beforSyukkoKiboubi = drEdiRcvDtl("SYUKKO_KIBOU_DATE").ToString
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH010_INKAEDI_DTL_MMK")
            setDtlDt.Clear()
            ...
            If iSkipFlg = 0 Then
                ...
                setDs = Me.SetSemiInkaEdiM(setDs)
