...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC701 = New LMH030DAC701()
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            If ("COLUMN_MST").Equals(sTempColNo) Then
                ...
                End If
            Else
                validVal = drRcvEdiDtl.Item(sTempColNo).ToString().Trim()
                'バイト数チェック
                validLen = System.Text.Encoding.GetEncoding(932).GetByteCount(validVal)
                drOutkaEdiL("DEST_AD_1") = _Blc.LeftB(drRcvEdiDtl(sTempColNo).ToString().Trim(), 40)
            End If
            ...
            If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
                ...
                Else
                    If validLen > 40 Then
                        drOutkaEdiL("DEST_AD_2") = _Blc.LeftB(String.Concat(validVal.Replace(drOutkaEdiL("DEST_AD_1").ToString(), String.Empty), drRcvEdiDtl(sTempColNo).ToString().Trim()), 40)
                    Else
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
        ...
        sKey = "M_IRIME_NO"
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            If Convert.ToInt32(drRcvEdiDtl.Item(sTempColNo)) > 0 Then
