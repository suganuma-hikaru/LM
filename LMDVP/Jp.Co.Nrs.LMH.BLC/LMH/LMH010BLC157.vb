...
'  システム名       :  LM
'  サブシステム名   :  LMH              : EDI
'  プログラムID     :  LMH010HBLC157    : EDI入荷検索
'  EDI荷主ID　　　　:  157              : シグマアルドリッチジャパン
'  作  成  者       :  
' ==========================================================================
Imports Jp.Co.Nrs.LM.Const
Imports Jp.Co.Nrs.LM.DAC
Imports Jp.Co.Nrs.LM.DAC.LMH010DAC157
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC157 = New LMH010DAC157()
...
#Region "入荷登録"
    ...
    Private Function InkaToroku(ByVal ds As DataSet) As DataSet
        ...
        '↓流用元 LMH010BLC151 の処理 (実質は「受信明細の更新」)
        '受信ヘッダの更新
        ...
        '↓流用元 LMH010BLC151 の処理 (UpdateRcvDtl を 2度呼んでいるのみで繰り返す意味なし)
        ''受信明細の更新
        'ds = MyBase.CallDAC(Me._Dac, "UpdateRcvDtl", ds)
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    Private Function SetSemiInkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_SGM").NewRow()
        ...
        ' 荷主固有データ
        '----------------
        drEdiRcvDtl("OUTKA_FROM_ORD_NO_L") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.OUTKA_FROM_ORD_NO_L)).ToString().Trim(), 30)
        drEdiRcvDtl("BILL_OF_LADING") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.BILL_OF_LADING)).ToString().Trim(), 100)
        drEdiRcvDtl("CUST_GOODS_CD") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.CUST_GOODS_CD)).ToString().Trim(), 20)
        drEdiRcvDtl("NB") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.NB)).ToString().Trim(), 10)
        drEdiRcvDtl("LOT_NO") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.LOT_NO)).ToString().Trim(), 40)
        drEdiRcvDtl("UT") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.UT)).ToString().Trim(), 100)
        drEdiRcvDtl("COMPLIANCE") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.COMPLIANCE)).ToString().Trim(), 100)
        drEdiRcvDtl("JFC") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.JFC)).ToString().Trim(), 100)
        drEdiRcvDtl("PDC") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.PDC)).ToString().Trim(), 100)
        drEdiRcvDtl("ROAN_DISP") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.ROAN_DISP)).ToString().Trim(), 100)
        drEdiRcvDtl("F_F") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.F_F)).ToString().Trim(), 100)
        drEdiRcvDtl("ONDO") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.ONDO)).ToString().Trim(), 100)
        drEdiRcvDtl("OUTKA_POINT") = Left(drSetDtl(GetColumnName(SGMArrivalColumns.OUTKA_POINT)).ToString().Trim(), 100)

        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "1"                                                          '実績処理フラグ

        'データセットに設定
        ds.Tables("LMH010_INKAEDI_DTL_SGM").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI入荷(中))"
    ...
    Private Function SetSemiInkaEdiM(ByVal setDs As DataSet _
        ...
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH010_INKAEDI_DTL_SGM").Rows(0)
        ...
        drInkaEdiM("CUST_GOODS_CD") = drRcvEdiDtl("CUST_GOODS_CD")
        ...
        drInkaEdiM("NB") = Convert.ToInt32(drRcvEdiDtl("NB").ToString())
        drInkaEdiM("INKA_PKG_NB") = 1
        drInkaEdiM("HASU") = 0
        drInkaEdiM("OUTKA_FROM_ORD_NO") = drRcvEdiDtl("OUTKA_FROM_ORD_NO_L").ToString()
        drInkaEdiM("BUYER_ORD_NO") = String.Empty
        drInkaEdiM("REMARK") = String.Empty
        drInkaEdiM("LOT_NO") = drRcvEdiDtl("LOT_NO")
        ...
        drInkaEdiM("FREE_C01") = drRcvEdiDtl("BILL_OF_LADING").ToString()
        drInkaEdiM("FREE_C02") = drRcvEdiDtl("UT").ToString()
        drInkaEdiM("FREE_C03") = drRcvEdiDtl("COMPLIANCE").ToString()
        drInkaEdiM("FREE_C04") = drRcvEdiDtl("JFC").ToString()
        drInkaEdiM("FREE_C05") = drRcvEdiDtl("PDC").ToString()
        drInkaEdiM("FREE_C06") = drRcvEdiDtl("ROAN_DISP").ToString()
        drInkaEdiM("FREE_C07") = drRcvEdiDtl("F_F").ToString()
        drInkaEdiM("FREE_C08") = drRcvEdiDtl("ONDO").ToString()
        drInkaEdiM("FREE_C09") = drRcvEdiDtl("OUTKA_POINT").ToString()
...
#Region "セミEDI時　データセット設定(EDI入荷(大))"
    ...
    Private Function SetSemiinkaediL(ByVal setDs As DataSet) As DataSet
        ...
        Dim fileNmWithoutPath As String = System.IO.Path.GetFileName(drRcvEdiDtl("FILE_NAME").ToString())
        If fileNmWithoutPath.Length > 8 Then
            inkaDate = fileNmWithoutPath.Substring(0, 8)
        End If
        ...
        drInkaEdiL("OUTKA_FROM_ORD_NO") = drRcvEdiDtl("OUTKA_FROM_ORD_NO_L").ToString()
        ...
        drInkaEdiL("HOKAN_STR_DATE") = inkaDate
        ...
        drInkaEdiL("FREE_C01") = drRcvEdiDtl("OUTKA_FROM_ORD_NO_L").ToString()
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_SGM")
            ...
            Else
                ...
                For j As Integer = iRowCnt To max
                    ...
                    If (dr.Item("FILE_NAME_RCV").ToString().Trim()).Equals(dtSemiHed.Rows(i).Item("FILE_NAME_RCV").ToString().Trim()) = True Then
                        'ヘッダと明細のファイル名称が等しい場合
                        '入力チェック(数値,日付チェック)
                        If Me.TorikomiValChk(dr, cnt) = False Then
                            '異常の場合
                            '詳細のエラーフラグに"1"をセットする
                            dr.Item("ERR_FLG") = "1"
...
#Region "カラム項目の値・日付チェック"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function TorikomiValChk(ByVal dr As DataRow, ByRef cnt As Integer) As Boolean
        ...
        If cnt = 1 Then
            ...
            sMsg = "入荷日(ファイル名先頭8桁)["
            Dim inkaDate As String = String.Empty
            Dim fileNmWithoutPath As String = System.IO.Path.GetFileName(sFileNm)
            If fileNmWithoutPath.Length > 8 Then
                inkaDate = fileNmWithoutPath.Substring(0, 8)
            End If
            sStr = inkaDate
            If IsDate(Jp.Co.Nrs.Win.Utility.DateFormatUtility.EditSlash(sStr)) = False Then
                MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E445", New String() {String.Concat(sMsg, sStr, "]")}, New String("0"c, sRecNo.Length), LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
        End If

        sMsg = "出荷伝票(カラム" & CType(SGMArrivalColumns.OUTKA_FROM_ORD_NO_L, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(SGMArrivalColumns.OUTKA_FROM_ORD_NO_L)).ToString()
        ...
        sMsg = "船荷証券(カラム" & CType(SGMArrivalColumns.BILL_OF_LADING, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(SGMArrivalColumns.BILL_OF_LADING)).ToString()
        ...
        sMsg = "品目コード(カラム" & CType(SGMArrivalColumns.CUST_GOODS_CD, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(SGMArrivalColumns.CUST_GOODS_CD)).ToString().Trim()
        If LenB(sStr) > 20 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, sStr, "]"), ""}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        sMsg = "出荷数(カラム" & CType(SGMArrivalColumns.NB, Int32) & "番目)["
        sNum = dr.Item(GetColumnName(SGMArrivalColumns.NB)).ToString().Trim()
        If String.IsNullOrEmpty(sNum) = True Then
            '↓流用元 LMH010BLC151 の処理
            ''空の場合はゼロをセット
            'dr.Item(GetColumnName(SGMArrivalColumns.NB)) = 0
            '↑流用元 LMH010BLC151 の処理
            ' 空の場合
        ...
        sMsg = "ﾛｯﾄ(カラム" & CType(SGMArrivalColumns.LOT_NO, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(SGMArrivalColumns.LOT_NO)).ToString().Trim()
        If LenB(sStr) > 40 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, sStr, "]"), ""}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        sMsg = "単位(カラム" & CType(SGMArrivalColumns.UT, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(SGMArrivalColumns.UT)).ToString().Trim()
        If LenB(sStr) > 100 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, sStr, "]"), ""}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        sMsg = "Compliance(カラム" & CType(SGMArrivalColumns.COMPLIANCE, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(SGMArrivalColumns.COMPLIANCE)).ToString().Trim()
        If LenB(sStr) > 100 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, sStr, "]"), ""}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        sMsg = "JFC(カラム" & CType(SGMArrivalColumns.JFC, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(SGMArrivalColumns.JFC)).ToString().Trim()
        If LenB(sStr) > 100 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, sStr, "]"), ""}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        sMsg = "PDC(カラム" & CType(SGMArrivalColumns.PDC, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(SGMArrivalColumns.PDC)).ToString().Trim()
        If LenB(sStr) > 100 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, sStr, "]"), ""}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        sMsg = "ROAN-DISP(カラム" & CType(SGMArrivalColumns.ROAN_DISP, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(SGMArrivalColumns.ROAN_DISP)).ToString().Trim()
        If LenB(sStr) > 100 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, sStr, "]"), ""}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        sMsg = "F&F(カラム" & CType(SGMArrivalColumns.F_F, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(SGMArrivalColumns.F_F)).ToString().Trim()
        If LenB(sStr) > 100 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, sStr, "]"), ""}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        sMsg = "温度(カラム" & CType(SGMArrivalColumns.ONDO, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(SGMArrivalColumns.ONDO)).ToString().Trim()
        If LenB(sStr) > 100 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, sStr, "]"), ""}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        sMsg = "出ポ(カラム" & CType(SGMArrivalColumns.OUTKA_POINT, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(SGMArrivalColumns.OUTKA_POINT)).ToString().Trim()
        If LenB(sStr) > 100 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, sStr, "]"), ""}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
...
#Region "商品明細マスタ"
    ...
    Public Function TorikomiMstChk(ByVal dr As DataRow, ByVal ds As DataSet) As Boolean
        ...
        sMsg = "品目コード(カラム" & CType(SGMArrivalColumns.CUST_GOODS_CD, Int32) & "番目)"
        sGoodsCd = dr.Item(GetColumnName(SGMArrivalColumns.CUST_GOODS_CD)).ToString()   '品目コード
        If MyBase.GetResultCount() = 0 Then

            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E493", New String() {"品目コード", "商品マスタ", String.Concat("荷主商品コード：", sGoodsCd)}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            Return bRet
        End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_SGM")         'EDI受信Dtl
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            ds.Tables("LMH010_INKAEDI_DTL_SGM").Clear() '受信DTLをクリア
            ds = Me.SetSemiInkaEdiRcv(ds, i)
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_SGM").Rows(0)
            ...
            '出荷数が0.0の場合は取り込まずに次のレコードへ
            '→TorikomiValChk() にてゼロ以下をエラーとしている場合は起こりえない。
            If Convert.ToDecimal(drEdiRcvDtl.Item("NB").ToString()) = 0.0 Then
                Continue For
            End If
            ...
            ' キー項目設定
            '---------------------------------------------------------------------------
            sNewKey = drEdiRcvDtl.Item("OUTKA_FROM_ORD_NO_L").ToString()
            ...
            If drEdiRcvDtl("OUTKA_FROM_ORD_NO_L").ToString().Trim().Equals("") = False Then
                '---------------------------------------------------------------------------
                ' EDI入荷データ件数および入荷データL 入荷管理番号L 等 取得処理
                '---------------------------------------------------------------------------
                setDs.Tables("LMH010_INKAEDI_DTL_SGM").ImportRow(drEdiRcvDtl)

                setDs = MyBase.CallDAC(Me._Dac, "GetInkaCntAndNoL", setDs)

                ediRecCnt = MyBase.GetResultCount()
                Dim inkaNoL As String = setDs.Tables("LMH010_INKAEDI_DTL_SGM").Rows(0).Item("INKA_CTL_NO_L").ToString()
                If ediRecCnt > 0 AndAlso (inkaNoL.Equals("") = False) Then
                    ...
                    MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E02I", New String() {inkaNoL},
                                       ...
                                       LMH010BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                    bNoErr = False
                    Continue For
                End If
            End If
            ...
            If ediRecCnt > 0 AndAlso (bSameKeyFlg = False) Then
                ...
                iInDtlCanCnt = iInDtlCanCnt + MyBase.GetResultCount()

                ' EDI入荷(大)の削除(論理削除)
                setDs = MyBase.CallDAC(Me._Dac, "UpdateDelInkaEdiL", setDs)
                iInHedCanCnt = iInHedCanCnt + 1
            ...
            '別インスタンス
            setDs = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH010_INKAEDI_DTL_SGM")
            setDtlDt.Clear()
            ...
            setDs = MyBase.CallDAC(Me._Dac, "GetInkaEdiCnt", setDs)
            If MyBase.GetResultCount() > 0 Then
                ' 同一キーレコード存在時の物理削除
