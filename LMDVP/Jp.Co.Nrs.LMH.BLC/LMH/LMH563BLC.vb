...
'  システム名       :  LM
'  サブシステム名   :  LMH       : EDI
'  プログラムID     :  LMH563    : 入出荷EDI受信帳票(日産物流)
'  作  成  者       :  渡部剛
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH563DAC = New LMH563DAC()
...
#Region "Const"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_IN As String = "LMH563IN"

    ''' <summary>
    ''' OUTテーブル名
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_OUT As String = "LMH563OUT"
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = LMH563BLC.TABLE_NM_IN
        Dim tableNmOut As String = LMH563BLC.TABLE_NM_OUT
        Dim tableNmRpt As String = LMH563BLC.TABLE_NM_M_RPT
        ...
        If ds.Tables(tableNmIn).Rows.Count = 0 Then
            ...
            MyBase.SetMessage("G052", New String() {"入出荷受信帳票"})                      '【Notes】№1007/1008対応：G039→G052に変更
            MyBase.SetMessageStore(String.Empty, "G052", New String() {"入出荷受信帳票"})   '【Notes】№1007/1008対応：G039→G052に変更
            Return ds
        End If
        ...
        workDtRpt = view.ToTable(True, LMH563BLC.NRS_BR_CD, LMH563BLC.PTN_ID, LMH563BLC.PTN_CD, LMH563BLC.RPT_ID)
        ...
        For Each dr As DataRow In ds.Tables(tableNmRpt).Rows
            ...
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables(tableNmOut), dr.Item(LMH563BLC.RPT_ID).ToString())

            '帳票ごとの編集があるなら行う。
            prtDs = Me.EditPrintDataSet(dr.Item(LMH563BLC.RPT_ID).ToString(), prtDs)

            '帳票CSV出力
            comPrt.StartPrint(Me, dr.Item(LMH563BLC.NRS_BR_CD).ToString(), _
                                dr.Item(LMH563BLC.PTN_ID).ToString(), _
                                dr.Item(LMH563BLC.PTN_CD).ToString(), _
                                dr.Item(LMH563BLC.RPT_ID).ToString(), _
    ...
    Private Function SelectMPrt(ByVal ds As DataSet, ByVal intbl As DataTable) As DataSet
        ...
        ds = MyBase.CallDAC(Me._Dac, LMH563BLC.ACTION_ID_SELECT_MPRT, ds)
        ...
        If count < 1 Then
            ...
            MyBase.SetMessage("G053", New String() {"　【入出荷受信帳票】　"})

            If prtFlg.Equals("1") = True Then
                '出力済の場合、エラーメッセージはExcel出力
                MyBase.SetMessageStore("00", "G053", New String() {"　【入出荷受信帳票】　"}, rowNo, "EDI管理番号", ediCtlNo)
            Else
                If outType.Equals(ALL_PRINT) = True Then
                    '一括印刷時、エラーメッセージはExcel出力
                    MyBase.SetMessageStore("00", "G053", New String() {"　【入出荷受信帳票】　"}, rowNo, "帳票名", "入出荷受信帳票")
                End If
            End If
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        Return MyBase.CallDAC(Me._Dac, LMH563BLC.ACTION_ID_SELECT_PRT_DATA, ds)

    End Function

    '要望番号1008 2012.10.31 追加START
    ''' <summary>
    '''  EDI印刷対象テーブルINデータ設定(LMH563)
    ''' </summary>
