...
'  システム名       :  LM
'  サブシステム名   :  LMH       : EDI
'  プログラムID     :  LMH010HBLC405    : EDI入荷検索:ローム(209)から複写
'  EDI荷主ID　　　　:  115　　　 : インターコンチネンタル商事株式会社(横浜)
'  作  成  者       :  daikoku
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC405 = New LMH010DAC405()
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    Private Function SetSemiInkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_ITC").NewRow()
        ...
        '荷主固有データ
        drEdiRcvDtl("INKABI") = Format(Convert.ToDateTime(drSetDtl("COLUMN_1")), "yyyyMMdd")        '入荷日
        drEdiRcvDtl("GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_2").ToString().Trim(), 20)         '商品コード
        drEdiRcvDtl("KOSU") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 12)             '入荷個数量
        drEdiRcvDtl("LOT_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 40)           'ロット
        drEdiRcvDtl("TOU_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 2)            '棟
        drEdiRcvDtl("SITU_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 2)           '室
        drEdiRcvDtl("ZONE_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 2)           'ゾーン
        drEdiRcvDtl("LOCA") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 10)             'ロケーション
        ...
        'データセットに設定
        ds.Tables("LMH010_INKAEDI_DTL_ITC").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI入荷(中))"
    ...
    Private Function SetSemiInkaEdiM(ByVal setDs As DataSet _
                                    ...
                                    , ByRef bNoErr As Boolean _
                                    ) As DataSet

        Dim drInkaEdiM As DataRow = setDs.Tables("LMH010_INKAEDI_M").NewRow()
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH010_INKAEDI_DTL_ITC").Rows(0)
        ...
        '2018/01/17 Annen 【LMS】セミEDI_YCCインターコンチネンタル-本数→ケース数へ変更対応 upd start
        'drInkaEdiM("NB") = drRcvEdiDtl("KOSU")
        Dim nb As Decimal = 0
        If Convert.ToDecimal(drRcvEdiDtl("KOSU")) Mod Convert.ToDecimal(setDs.Tables("LMH010_M_GOODS").Rows(0).Item("STD_IRIME_NB")) <> 0 Then
            '端数が発生したらエラーとする
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E976", , , LMH010BLC.EXCEL_COLTITLE, drRcvEdiDtl("EDI_CTL_NO").ToString)
            bNoErr = False
            Return setDs
        End If
        '2018/01/17 Annen 【LMS】セミEDI_YCCインターコンチネンタル-本数→ケース数へ変更対応 upd end
        ...
        If setDs.Tables("LMH010_M_GOODS").Rows.Count = 1 Then
            ...
            '2018/01/17 Annen 【LMS】セミEDI_YCCインターコンチネンタル-本数→ケース数へ変更対応 add start
            nb = Math.Floor(Convert.ToDecimal(drRcvEdiDtl("KOSU")) / Convert.ToDecimal(setDs.Tables("LMH010_M_GOODS").Rows(0).Item("STD_IRIME_NB")))
            '2018/01/17 Annen 【LMS】セミEDI_YCCインターコンチネンタル-本数→ケース数へ変更対応 add end
        ...
        drInkaEdiM("FREE_C01") = drRcvEdiDtl("TOU_NO")      '棟
        drInkaEdiM("FREE_C02") = drRcvEdiDtl("SITU_NO")     '室
        drInkaEdiM("FREE_C03") = drRcvEdiDtl("ZONE_CD")     'ZONE
        drInkaEdiM("FREE_C04") = drRcvEdiDtl("LOCA")        'ロケ
...
#Region "セミEDI時　データセット設定(EDI入荷(大))"
    ...
    Private Function SetSemiinkaediL(ByVal setDs As DataSet) As DataSet
        ...
        drInkaEdiL("INKA_DATE") = drRcvEdiDtl("INKABI")
        ...
        drInkaEdiL("HOKAN_STR_DATE") = drRcvEdiDtl("INKABI")
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_ITC")
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow) As Boolean
        ...
        sMsg = "商品コード(カラ2番目)["
        sStr = dr.Item("COLUMN_2").ToString().Trim()
        ...
        sMsg = "室(カラム6番目)["
        sStr = dr.Item("COLUMN_6").ToString().Trim()
        If sStr.Length > 2 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E433", New String() {String.Concat(sMsg, sStr, "]")}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        sMsg = "ロケーション(カラム8番目)["
        sStr = dr.Item("COLUMN_8").ToString().Trim()
...
#Region "商品明細マスタ"
    ...
    Public Function TorikomiMstChk(ByVal dr As DataRow, ByVal ds As DataSet) As Boolean
        ...
        If MyBase.GetResultCount() = 0 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E493", New String() {"荷主商品コード", "商品マスタ", String.Concat("荷主商品コード：", dr.Item("COLUMN_2").ToString().Trim())}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            Return bRet
        ElseIf MyBase.GetResultCount() = 1 Then

        ElseIf MyBase.GetResultCount() > 1 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E494", New String() {"荷主商品コード", "商品マスタ", String.Concat("荷主商品コード：", dr.Item("COLUMN_2").ToString().Trim())}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            Return bRet
        End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_ITC")         'EDI受信Dtl
        ...
        '対象データのみソート
        Dim strSort As String = "COLUMN_1"  '入荷日
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            ds.Tables("LMH010_INKAEDI_DTL_ITC").Clear() '受信DTLをクリア

            ds = Me.SetSemiInkaEdiRcv(ds, i)

            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_ITC").Rows(0)
            ...
            If Convert.ToDecimal(drEdiRcvDtl.Item("KOSU").ToString()) = 0.0 Then
                Continue For
            End If

            '入荷日
            sChkKey = dtSetDtl.Rows(i).Item("COLUMN_1").ToString()
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH010_INKAEDI_DTL_ITC")
            setDtlDt.Clear()
            ...
            If iSkipFlg = 0 Then
                ...
                setDs = Me.SetSemiInkaEdiM(setDs, sWhcd, sCustCdL, sCustCdM, sNrsGoodsCd, sNrsGoodsNm, sIrime, bNoErr)
                'setDs = Me.SetSemiInkaEdiM(setDs, sWhcd, sCustCdL, sCustCdM, sNrsGoodsCd, sNrsGoodsNm, sIrime)

                'エラーが発生したら登録しない（データセットに値を設定せずに登録処理を行ったら実行時エラーになるため）
                If bNoErr.Equals(True) Then
