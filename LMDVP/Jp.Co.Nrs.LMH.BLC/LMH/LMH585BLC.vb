...
'  システム名       :  LM
'  サブシステム名   :  LMH       : EDI
'  プログラムID     :  LMH585    : EDI納品書(大阪 日興産業 イエローハット)
'  作  成  者       :  大貫和正
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH585DAC = New LMH585DAC()
...
#Region "Const"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_IN As String = "LMH585IN"

    ''' <summary>
    ''' OUTテーブル名
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_OUT As String = "LMH585OUT"
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = LMH585BLC.TABLE_NM_IN
        Dim tableNmOut As String = LMH585BLC.TABLE_NM_OUT
        Dim tableNmRpt As String = LMH585BLC.TABLE_NM_M_RPT
        ...
        workDtRpt = view.ToTable(True, LMH585BLC.NRS_BR_CD, LMH585BLC.PTN_ID, LMH585BLC.PTN_CD, LMH585BLC.RPT_ID)
        ...
        For Each dr As DataRow In ds.Tables(tableNmRpt).Rows
            ...
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables(tableNmOut), dr.Item(LMH585BLC.RPT_ID).ToString())

            '帳票ごとの編集があるなら行う。
            prtDs = Me.EditPrintDataSet(dr.Item(LMH585BLC.RPT_ID).ToString(), prtDs)

            '帳票CSV出力
            comPrt.StartPrint(Me, _
                              dr.Item(LMH585BLC.NRS_BR_CD).ToString(), _
                              dr.Item(LMH585BLC.PTN_ID).ToString(), _
                              dr.Item(LMH585BLC.PTN_CD).ToString(), _
                              dr.Item(LMH585BLC.RPT_ID).ToString(), _
    ...
    Private Function SelectMPrt(ByVal ds As DataSet, ByVal intbl As DataTable) As DataSet
        ...
        ds = MyBase.CallDAC(Me._Dac, LMH585BLC.ACTION_ID_SELECT_MPRT, ds)
        ...
        If count < 1 Then
            ...
            Else
                If outType.Equals(ALL_PRINT) = True Then
                    ...
                    MyBase.SetMessageStore("00", "G053", New String() {"　【EDI納品書】　"}, rowNo, "帳票名", "EDI納品書 日興ｲｴﾛｰﾊｯﾄ")
                End If
            End If
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        Return MyBase.CallDAC(Me._Dac, LMH585BLC.ACTION_ID_SELECT_PRT_DATA, ds)
