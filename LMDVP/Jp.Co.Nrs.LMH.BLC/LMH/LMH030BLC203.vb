...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC203 = New LMH030DAC203()
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        '浮間合成対応 20120318 Start
        'オーダー重複チェック【浮間合成専用】
        If Me.CustOrderNoCheckUkima(ds, rowNo, ediCtlNo) = False Then
            Return ds
        End If
        '浮間合成対応 20120318 End
        ...
        '浮間合成修正箇所 2012.02.25 START
        '注)基本的に浮間合成はまとめ処理を行わないが、共通まとめ処理のロジックを組み込む
...
#Region "EDI出荷(中)の初期値設定"
    ...
    Private Function EdiMDefaultSet(ByVal ds As DataSet, ByVal setDs As DataSet, _
        ...
        If MyBase.GetResultCount > 0 Then
            Dim ruibetuCd As String = setDs.Tables("LMH030_M_GOODS_DETAILS").Rows(0)("SET_NAIYO").ToString()
            Dim ediFreeC07 As String = ediMDr.Item("FREE_C07").ToString()
            'FREE_C07と商品明細マスタ(サブ区分"07"(類別コード))の整合性チェック
            If (ediFreeC07).Equals(ruibetuCd) = False Then

                choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.UKIMA_WID_M001, count)

                If String.IsNullOrEmpty(choiceKb) = True Then
                    msgArray(1) = "類別コード(FREE_C07)"
                    msgArray(2) = "商品明細マスタ"
                    msgArray(3) = "類別コード"
                    msgArray(4) = "注意) 荷主に確認の上、商品明細マスタを修正して下さい。"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningM("W195", LMH030BLC.UKIMA_WID_M001, ds, setDs, msgArray, ediFreeC07, ruibetuCd)
        ...
        ElseIf String.IsNullOrEmpty(mGoodsNm) = False AndAlso String.IsNullOrEmpty(mDestGoodsNm) = False AndAlso _
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(5) = String.Empty
                'ds = Me._Blc.SetComWarningM("W194", LMH030BLC.UKIMA_WID_M002, ds, setDs, msgArray, ediGoodsNm, mGoodsNm)
        ...
        ElseIf mGoodsNm.Equals(ediGoodsNm) = False Then
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(5) = String.Empty
                '2012.03.08 修正START
                'ds = Me._Blc.SetComWarningM("W194", LMH030BLC.UKIMA_WID_M002, ds, ds, msgArray, ediGoodsNm, mGoodsNm)
                ds = Me._Blc.SetComWarningM("W194", LMH030BLC.UKIMA_WID_M002, ds, setDs, msgArray, _
        ...
        If (ediIrime).Equals(mIrime) = False Then
            ...
            choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.UKIMA_WID_M004, count)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(5) = String.Empty
                ds = Me._Blc.SetComWarningM("W194", LMH030BLC.UKIMA_WID_M004, ds, setDs, msgArray, ediIrime, mIrime)
        ...
        If String.IsNullOrEmpty(ediIrimeUt) = False Then
            ...
            If mIrimeUt.Equals(ediIrimeUt) = True Then
                'チェックなし
            Else
                choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.UKIMA_WID_M003, count)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningM("W159", LMH030BLC.UKIMA_WID_M003, ds, setDs, msgArray, ediIrimeUt, mIrimeUt)
        ...
        '2012.03.18 浮間対応　追加START
        ediMDr("OUTKA_ATT") = mGoodsDr("OUTKA_ATT")
        '2012.03.18 浮間対応　追加END
...
#Region "EDI出荷(大)のDAC側でのチェック"
    ...
    Private Function EdiLDbExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        '-------------------------------------------------------------------------------------
        ' ''オーダー番号重複チェック
        '浮間合成は後続で浮間専用のオーダー番号重複チェックを行う為、ここではチェックを行わない
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                '2012.05.28 浮間合成(岩槻)対応 修正START
                '①岩槻の場合は商品M先頭行を取得
                '②大阪の場合はワーニング画面に遷移
                If LMH030BLC.EdiCustIndex.UkimaOsk00856_00 = Convert.ToInt32(ds.Tables("LMH030INOUT").Rows(0).Item("EDI_CUST_INDEX").ToString()) Then

                    '2012.04.17 要望番号1000 修正START(コメントをはずす)
                    choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.UKIMA_WID_M005, 0)
                    '2012.04.17 要望番号1000 修正END(コメントをはずす)

                End If
                '2012.05.28 浮間合成(岩槻)対応 修正END
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    Else
                        ...
                        If LMH030BLC.EdiCustIndex.UkimaOsk00856_00 = Convert.ToInt32(ds.Tables("LMH030INOUT").Rows(0).Item("EDI_CUST_INDEX").ToString()) Then
                            ...
                            '2012.04.17 要望番号1000 修正START(コメントをはずす)
                            '浮間合成の場合、商品マスタの１行目(GOODS_CD_NRSでORDER BYをした値を使用するのでワーニングにしない)
                            '↑商品POP選択のワーニングが出力されるように修正(2012.04.17)
                            ...
                            ds = Me._Blc.SetComWarningM("W162", LMH030BLC.UKIMA_WID_M005, ds, setDs, msgArray, custGoodsCd, String.Empty)
                ...
                If Me.EdiMDefaultSet(ds, setDs, i, unsoData, rowNo, ediCtlNo) = False Then
                    ...
                    '浮間合成はEDI出荷(中)でワーニングチェックがある為、エラーorワーニングの判断を行う
                    Dim chkRowNo As Integer = Convert.ToInt32(rowNo)
                    If MyBase.IsMessageStoreExist(chkRowNo) = True Then
                        '整合性チェックでエラーがあった場合は処理終了
                        Return False
...
#End Region
    ...
    '浮間合成　対応 20120318 Start
#Region "オーダーNO重複チェック(浮間合成専用)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function CustOrderNoCheckUkima(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To edimCount
            ...
            If String.IsNullOrEmpty(drEdiL.Item("CUST_ORD_NO").ToString()) = False Then
                If drIn("ORDER_CHECK_FLG").Equals("1") = True Then
                    Call MyBase.CallDAC(Me._Dac, "SelectOrderCheckDataUkima", setDs)
                    If MyBase.GetResultCount > 0 Then
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E377", New String() {"出荷データ"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                        Return False
                    End If
...
#End Region
    ...
    '2012.02.13 浮間用修正START
#Region "届先マスタチェック(浮間合成専用)"
    ''' <summary>
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        mDestNm = Me.CompanyCutChk(mDestNm)
        ediDestNm = Me.CompanyCutChk(ediDestNm)

        '届先名称(前４文字が"暫定対応"の場合はエラー)
        '2012.06.05 テスト時不具合修正START
        If mDestNm.Length >= 4 Then

            If mDestNm.Substring(0, 4).Equals(zanteiStr) = True Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E320", New String() {String.Concat("届先マスタの内容が", zanteiStr), "出荷登録", zanteiStr}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                Return False
            End If
        End If
        '2012.06.05 テスト時不具合修正END
        ...
        ElseIf InStr(mDestNm.ToString().Trim(), ediDestNm.ToString().Trim()) = 0 Then
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.UKIMA_WID_L001, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                '2012.03.22 修正START
                ds = Me._Blc.SetComWarningL("W166", LMH030BLC.UKIMA_WID_L001, ds, msgArray, _
                                            dtEdi.Rows(0)("DEST_NM").ToString(), dtMdest.Rows(0).Item("DEST_NM").ToString())
                'ds = Me._Blc.SetComWarningL("W166", LMH030BLC.UKIMA_WID_L001, ds, msgArray, ediDestNm, mDestNm)
                '2012.03.22 修正END
        ...
        Else
            ...
            If InStr(mAdAll.ToString().Trim(), ediFreeC21.ToString().Trim()) = 0 Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.UKIMA_WID_L002, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.UKIMA_WID_L002, ds, msgArray, ediFreeC21, mAdAll)
        ...
        Else
            If mTel.Equals(Replace(Replace(ediTel, "(", String.Empty), ")", String.Empty)) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.UKIMA_WID_L003, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.UKIMA_WID_L003, ds, msgArray, ediTel, mTel)
        ...
        Else
            If mZip.Equals(Replace(ediZip, "-", String.Empty)) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.UKIMA_WID_L004, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.UKIMA_WID_L004, ds, msgArray, ediZip, mZip)
...
#Region "届先マスタ自動追加(浮間合成専用)"
    ...
    Private Function SetInsMDestFromDest(ByVal ds As DataSet, ByVal workDestCd As String, ByVal workDestString As String) As Boolean
        ...
        Dim msgArray(5) As String
        Dim choiceKb As String = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.UKIMA_WID_L005, 0)
        Dim drEdiL As DataRow = ds.Tables("LMH030_OUTKAEDI_L").Rows(0)
        Dim zanteiStr As String = "暫定登録"
        Dim compareWarningFlg As Boolean = False
        ...
        If String.IsNullOrEmpty(choiceKb) = True Then
            ...
            ds = Me._Blc.SetComWarningL("W182", LMH030BLC.UKIMA_WID_L005, ds, msgArray, workDestCd, String.Empty)
        ...
        ElseIf choiceKb.Equals("01") = True Then
            ...
            '浮間合成専用設定START
            drMD("DEST_NM") = String.Concat(zanteiStr, drEdiL("DEST_NM").ToString())
            drMD("ZIP") = Replace(drEdiL("DEST_ZIP").ToString().Trim(), "-", String.Empty)
            drMD("COA_YN") = "02"
            drMD("TEL") = Replace(Replace(drEdiL("DEST_TEL").ToString(), "(", String.Empty), ")", String.Empty)
            drMD("JIS") = String.Empty
            '浮間合成専用設定END
...
#Region "株式会社or（株）除去 + 文字変換"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function CompanyCutChk(ByVal chkFld As String) As String

        chkFld = Replace(Trim(chkFld), "株式会社", String.Empty)
        chkFld = Replace(chkFld, "（株）", String.Empty)
...
#Region "データセット設定(出荷L)"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            '浮間合成専用設定START
            outkaDr("DENP_NO") = ediDr("FREE_C10")
            '浮間合成専用設定START
...
#Region "データセット設定(出荷M)"
    ...
    Private Function SetDatasetOutkaM(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            '2012.03.18 浮間専用 修正START
            '注意事項
            If String.IsNullOrEmpty(ediDr("OUTKA_ATT").ToString) = False Then
                outkaDr("REMARK") = Me._Blc.LeftB(String.Concat(ediDr("OUTKA_ATT").ToString, Space(2), ediDr("REMARK").ToString), 100)
            Else
                outkaDr("REMARK") = ediDr("REMARK")
            End If
            'outkaDr("REMARK") = ediDr("REMARK")
            '2012.03.18 浮間専用 修正END
...
#Region "データセット設定(EDI受信DTL)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetDatasetEdiRcvDtl(ByVal ds As DataSet, ByVal judgeFlg As String) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            If judgeFlg.Equals("0") = True Then
                ...
                'DTL,EXT判断フラグ
                '浮間合成用に追加
                '最初DTLの更新から行うので、初期は"0"
                rcvDr("EXT_JUDGE_FLG") = judgeFlg

                'データセットに設定
                ds.Tables("LMH030_EDI_RCV_DTL").Rows.Add(rcvDr)

            ElseIf judgeFlg.Equals("1") = True Then

                'DTL,EXT判断フラグ
                '浮間合成用に追加
                'DTLの更新後はEXTTBLの更新を行うのでフラグを"1"にする
                ds.Tables("LMH030_EDI_RCV_DTL").Rows(i)("EXT_JUDGE_FLG") = judgeFlg
