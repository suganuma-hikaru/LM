...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC133 = New LMH030DAC133()
...
#Region "コーヴァンス・ジャパン用CONST"

    ''' <summary>
    ''' コーヴァンス・ジャパン
    ''' </summary>
    ''' <remarks></remarks>
    Public Const CUST_CD_L_CJC As String = "00787"          '荷主コード（大）
    Public Const CUST_CD_M_CJC As String = "00"             '荷主コード（中）
...
#Region "届先マスタチェック(日興産業物流専用)"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        Dim mAdAll As String = String.Concat(mAd1, mAd2)        'コーヴァンスは住所3を含めない
        Dim mZipJis As String = String.Empty
        ...
        Dim ediDestAdAll As String = String.Concat(ediDestAd1, ediDestAd2)      'コーヴァンスは住所3を含めない
        Dim ediDestJisCd As String = dtEdi.Rows(0)("DEST_JIS_CD").ToString()
        ...
        Else
            ...
            If mAdAll.Equals(ediDestAdAll) = False Then
                ...
                ElseIf choiceKb.Equals("01") = True Then
                    ...
                    'dtMdest.Rows(0).Item("AD_3") = dtEdi.Rows(0)("DEST_AD_3").ToString()       'コーヴァンスは住所3は届先マスタ登録値をそのまま
                    'マスタ更新対象フラグ
                    dtMdest.Rows(0).Item("MST_UPDATE_FLG") = "1"
...
#Region "データセット設定(作業)"
    ...
    Private Function SetDatasetSagyo(ByVal ds As DataSet) As DataSet
        ...
        Dim num As New NumberMasterUtility
        Dim outkaDr As DataRow = ds.Tables("LMH030_C_OUTKA_L").Rows(0)  'ADD 2019/12/19 009981
        'ADD S 2019/12/23 009981
        '作業項目マスタの取得(営業所コード、荷主コード(大)で絞り込んで取得)
        Dim mSagyoInDt As DataTable = ds.Tables("LMH030_M_SAGYO_IN")
        mSagyoInDt.Rows.Clear()
        Dim mSagyoInDr As DataRow = mSagyoInDt.NewRow
        mSagyoInDr("NRS_BR_CD") = ediDrL("NRS_BR_CD")
        mSagyoInDr("CUST_CD_L") = ediDrL("CUST_CD_L")
        mSagyoInDt.Rows.Add(mSagyoInDr)
        ds = MyBase.CallDAC(Me._Dac, "SelectMSagyo", ds)
        Dim mSagyoOutDt As DataTable = ds.Tables("LMH030_M_SAGYO")
        ...
        For i As Integer = 0 To max
            ...
            For j As Integer = 1 To 5
                ...
                If String.IsNullOrEmpty(sagyoCD) = False Then
                    ...
                    Dim mSagyoDrs As DataRow() = mSagyoOutDt.Select(String.Concat("SAGYO_CD = '", sagyoCD, "'"))
                    Dim mSagyoDr As DataRow = Nothing
                    If mSagyoDrs.Length > 0 Then mSagyoDr = mSagyoDrs(0)
                    'ADD E 2019/12/23 009981
                    ...
                    'MOD S 2019/12/23 009981
                    ''sagyoDr("SAGYO_NB") = 0
                    If mSagyoDr IsNot Nothing AndAlso mSagyoDr("INV_TANI").ToString = "01" Then
                        '請求単位=01:個 の場合
                        sagyoDr("SAGYO_NB") = outkaDr("OUTKA_PKG_NB")
                    Else
                        sagyoDr("SAGYO_NB") = 1
                    End If
                    ''sagyoDr("SAGYO_GK") = 0
                    If mSagyoDr IsNot Nothing Then
                        '作業数×単価
                        sagyoDr("SAGYO_GK") = CDec(sagyoDr("SAGYO_NB")) * CDec(mSagyoDr("SAGYO_UP"))
                    Else
                        sagyoDr("SAGYO_GK") = 0
                    End If
                    'MOD E 2019/12/23 009981
                    '2019/11/27 要望管理009444 rep
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC").NewRow()
        ...
        drEdiRcvDtl("CUST_CD_L") = CUST_CD_L_CJC                                                        '荷主コード（大）
        drEdiRcvDtl("CUST_CD_M") = CUST_CD_M_CJC                                                        '荷主コード（中）
        drEdiRcvDtl("PRTFLG") = "0"                                                                     'プリントフラグ
        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "1"                                                          '実績処理フラグ

        '荷主固有データ
#If False Then  'UPD 2019/10/09 007956   【LMS】セミEDI_千葉コーヴァンスジャパン_運用開始後要望3件
        drEdiRcvDtl("WKO_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_1").ToString().Trim(), 100)
        drEdiRcvDtl("CONTAINER_CODE") = Me._Blc.LeftB(drSetDtl("COLUMN_2").ToString().Trim(), 100)
        drEdiRcvDtl("PROJECT") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 100)
        drEdiRcvDtl("SITE_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 100)
        drEdiRcvDtl("ENTERED") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 100)
        drEdiRcvDtl("QTY") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 100)
        drEdiRcvDtl("SYSTEM_KRD") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 100)
        drEdiRcvDtl("MODIFIED_KRD") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 100)
        drEdiRcvDtl("SHUKASU") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 100)
        drEdiRcvDtl("NOUKI") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 100)
        drEdiRcvDtl("DUPLICATE_CHECK") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 100)
        drEdiRcvDtl("SAISOKAISU") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 100)
        drEdiRcvDtl("HACHUDATE") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 100)
        drEdiRcvDtl("HINBAN") = Me._Blc.LeftB(drSetDtl("COLUMN_14").ToString().Trim(), 100)
        drEdiRcvDtl("MEISHO") = Me._Blc.LeftB(drSetDtl("COLUMN_15").ToString().Trim(), 100)
        drEdiRcvDtl("KIKAKU") = Me._Blc.LeftB(drSetDtl("COLUMN_16").ToString().Trim(), 100)
        drEdiRcvDtl("LOT") = Me._Blc.LeftB(drSetDtl("COLUMN_17").ToString().Trim(), 100)
        drEdiRcvDtl("WKO_MODIFICATION_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_18").ToString().Trim(), 100)
        drEdiRcvDtl("POSTNO") = Me._Blc.LeftB(drSetDtl("COLUMN_19").ToString().Trim(), 100)
        drEdiRcvDtl("ADDRESS") = Me._Blc.LeftB(drSetDtl("COLUMN_20").ToString().Trim(), 100)
        drEdiRcvDtl("SITE") = Me._Blc.LeftB(drSetDtl("COLUMN_21").ToString().Trim(), 100)
        drEdiRcvDtl("SR") = Me._Blc.LeftB(drSetDtl("COLUMN_22").ToString().Trim(), 100)
        drEdiRcvDtl("TELNO") = Me._Blc.LeftB(drSetDtl("COLUMN_23").ToString().Trim(), 100)
        drEdiRcvDtl("COMMENTS") = Me._Blc.LeftB(drSetDtl("COLUMN_24").ToString().Trim(), 100)
        drEdiRcvDtl("MISSING_INFORMATION") = Me._Blc.LeftB(drSetDtl("COLUMN_25").ToString().Trim(), 100)
        drEdiRcvDtl("RECEIPT_OF_DELIVERY") = Me._Blc.LeftB(drSetDtl("COLUMN_26").ToString().Trim(), 100)

#Else
        '左端に追加された項目(新規等)は無視で構わない。
        drEdiRcvDtl("WKO_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_2").ToString().Trim(), 100)
        drEdiRcvDtl("CONTAINER_CODE") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 100)
        drEdiRcvDtl("PROJECT") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 100)
        drEdiRcvDtl("SITE_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 100)
        drEdiRcvDtl("ENTERED") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 100)
        drEdiRcvDtl("QTY") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 100)
        drEdiRcvDtl("SYSTEM_KRD") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 100)
        drEdiRcvDtl("MODIFIED_KRD") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 100)
        drEdiRcvDtl("SHUKASU") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 100)
        drEdiRcvDtl("NOUKI") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 100)
        drEdiRcvDtl("DUPLICATE_CHECK") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 100)
        drEdiRcvDtl("SAISOKAISU") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 100)
        drEdiRcvDtl("HACHUDATE") = Me._Blc.LeftB(drSetDtl("COLUMN_14").ToString().Trim(), 100)
        drEdiRcvDtl("HINBAN") = Me._Blc.LeftB(drSetDtl("COLUMN_15").ToString().Trim(), 100)
        drEdiRcvDtl("MEISHO") = Me._Blc.LeftB(drSetDtl("COLUMN_16").ToString().Trim(), 100)
        drEdiRcvDtl("KIKAKU") = Me._Blc.LeftB(drSetDtl("COLUMN_17").ToString().Trim(), 100)
        drEdiRcvDtl("LOT") = Me._Blc.LeftB(drSetDtl("COLUMN_18").ToString().Trim(), 100)
        drEdiRcvDtl("WKO_MODIFICATION_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_19").ToString().Trim(), 100)
        drEdiRcvDtl("POSTNO") = Me._Blc.LeftB(drSetDtl("COLUMN_20").ToString().Trim(), 100)
        drEdiRcvDtl("ADDRESS") = Me._Blc.LeftB(drSetDtl("COLUMN_21").ToString().Trim(), 100)
        drEdiRcvDtl("SITE") = Me._Blc.LeftB(drSetDtl("COLUMN_22").ToString().Trim(), 100)
        drEdiRcvDtl("SR") = Me._Blc.LeftB(drSetDtl("COLUMN_23").ToString().Trim(), 100)
        drEdiRcvDtl("TELNO") = Me._Blc.LeftB(drSetDtl("COLUMN_24").ToString().Trim(), 100)
        drEdiRcvDtl("COMMENTS") = Me._Blc.LeftB(drSetDtl("COLUMN_25").ToString().Trim(), 100)
        drEdiRcvDtl("MISSING_INFORMATION") = Me._Blc.LeftB(drSetDtl("COLUMN_26").ToString().Trim(), 100)
        drEdiRcvDtl("RECEIPT_OF_DELIVERY") = Me._Blc.LeftB(drSetDtl("COLUMN_27").ToString().Trim(), 100)

#End If

        'データセットに設定
        ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
        ...
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH030_H_OUTKAEDI_DTL_CJC").Rows(0)
        ...
        drOutkaEdiM("CUST_GOODS_CD") = drRcvEdiDtl("HINBAN").ToString
        drOutkaEdiM("GOODS_NM") = drRcvEdiDtl("MEISHO").ToString

        drOutkaEdiM("RSV_NO") = String.Empty
        drOutkaEdiM("LOT_NO") = drRcvEdiDtl("LOT").ToString
        drOutkaEdiM("SERIAL_NO") = String.Empty
        drOutkaEdiM("ALCTD_KB") = "01"                                  '個数引当

        drOutkaEdiM("OUTKA_PKG_NB") = 0                                 '出荷包装個数
        drOutkaEdiM("OUTKA_HASU") = drRcvEdiDtl("SHUKASU")              '出荷端数 
        drOutkaEdiM("OUTKA_TTL_NB") = drRcvEdiDtl("SHUKASU")            '出荷総個数

        If setDs.Tables("LMH030_M_GOODS").Rows.Count = 1 Then

            drOutkaEdiM("NRS_GOODS_CD") = drGoods("GOODS_CD_NRS")
            'drOutkaEdiM("GOODS_NM") = drGoods("GOODS_NM_1")
            drOutkaEdiM("OUTKA_QT") = Convert.ToDecimal(drRcvEdiDtl("SHUKASU")) * Convert.ToDecimal(drGoods("STD_IRIME_NB"))        '出荷数量
            drOutkaEdiM("OUTKA_TTL_QT") = Convert.ToDecimal(drRcvEdiDtl("SHUKASU")) * Convert.ToDecimal(drGoods("STD_IRIME_NB"))    '出荷総数量
...
#If False Then  'UPD 2019/10/15 007956
        drOutkaEdiM("FREE_C01") = String.Empty
        drOutkaEdiM("FREE_C02") = String.Empty

#Else
        drOutkaEdiM("FREE_C01") = drRcvEdiDtl("HENKANMAE_HINBAN").ToString          '変換前商品CD
        drOutkaEdiM("FREE_C02") = drRcvEdiDtl("HENKANMAE_MEISHO").ToString          '変換前商品名
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        '郵便番号マスタからJISコードを取得する
        Dim jisCd As String = String.Empty
        setDs.Tables("LMH030_ZIP_CJC").Clear()
        Dim drSetKbn As DataRow = setDs.Tables("LMH030_ZIP_CJC").NewRow()
        drSetKbn("NRS_BR_CD") = drSemiEdiInfo("NRS_BR_CD")
        drSetKbn("ZIP_NO") = drRcvEdiDtl("POSTNO").ToString().Replace("-", "")
        setDs.Tables("LMH030_ZIP_CJC").Rows.Add(drSetKbn)

        setDs = MyBase.CallDAC(Me._Dac, "SelectZip", setDs)

        If MyBase.GetResultCount > 0 Then
            jisCd = setDs.Tables("LMH030_ZIP_CJC").Rows(0).Item("JIS_CD").ToString()
        End If

        '電話番号を使用できる形に加工する
        Dim sTelno As String = drRcvEdiDtl("TELNO").ToString()
        Dim posNG As Integer = sTelno.Length()
        For k As Integer = 0 To sTelno.Length - 1
            If (Not IsNumeric(sTelno.Substring(k, 1))) AndAlso (Not sTelno.Substring(k, 1).Equals("-")) Then
                posNG = k
                Exit For
            End If
        Next
        sTelno = sTelno.Substring(0, posNG)
        ...
        '納入予定日
        Dim sDate As String = drRcvEdiDtl("NOUKI").ToString
        Dim idx As Integer = sDate.IndexOf("(")
        If idx >= 0 Then
            sDate = sDate.Substring(0, idx)
        End If
        Dim dDate As DateTime
        If DateTime.TryParse(sDate, dDate) Then
            drOutkaEdiL("ARR_PLAN_DATE") = dDate.ToString("yyyyMMdd")
        Else
            drOutkaEdiL("ARR_PLAN_DATE") = ""
        End If
        '出荷予定日
        If String.IsNullOrEmpty(drOutkaEdiL("ARR_PLAN_DATE").ToString()) Then
            drOutkaEdiL("OUTKA_PLAN_DATE") = String.Empty
        Else
            Dim preDate As String = Convert.ToDateTime(Date.Parse(Format(Convert.ToInt32(drOutkaEdiL("ARR_PLAN_DATE").ToString()), "0000/00/00"))).AddDays(-1).ToString("yyyyMMdd")
            drOutkaEdiL("OUTKA_PLAN_DATE") = preDate
        End If
        '配送時注意事項
        If idx < 0 Then
            drOutkaEdiL("UNSO_ATT") = String.Empty
        Else
            sDate = drRcvEdiDtl("NOUKI").ToString
            sDate = sDate.Substring(idx + 1)
            If Right(sDate, 1) = ")" Then
                sDate = sDate.Substring(0, sDate.Length - 1)
            End If
            drOutkaEdiL("UNSO_ATT") = sDate
        End If
        If Not String.IsNullOrEmpty(drRcvEdiDtl("COMMENTS").ToString()) Then
            If Not String.IsNullOrEmpty(drOutkaEdiL("UNSO_ATT").ToString()) Then
                drOutkaEdiL("UNSO_ATT") = String.Concat(drOutkaEdiL("UNSO_ATT").ToString(), "　")
            End If
            drOutkaEdiL("UNSO_ATT") = SubStringOfByte(String.Concat(drOutkaEdiL("UNSO_ATT").ToString(), drRcvEdiDtl("COMMENTS").ToString()), 1, 100)
        End If
#If False Then   'ADD 2019/10/09 007956   【LMS】セミEDI_千葉コーヴァンスジャパン_運用開始後要望3件
        drOutkaEdiL("ARR_PLAN_TIME") = String.Empty

#Else
        drOutkaEdiL("ARR_PLAN_TIME") = "10"    '常に指定
...
#End If
        ...
        drOutkaEdiL("EDI_DEST_CD") = sTelno
        drOutkaEdiL("DEST_CD") = String.Empty
        drOutkaEdiL("DEST_NM") = SubStringOfByte(drRcvEdiDtl("SITE").ToString(), 1, 80)
        drOutkaEdiL("DEST_ZIP") = drRcvEdiDtl("POSTNO").ToString().Replace("-", "")

        '住所編集（住所を格納）
        Dim mergeAddr As String = drRcvEdiDtl("ADDRESS").ToString()
        Dim cutAddr As String = String.Empty
        '1
        cutAddr = SubStringOfByte(mergeAddr, 1, 40)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        drOutkaEdiL("DEST_AD_1") = cutAddr
        '2
        cutAddr = SubStringOfByte(mergeAddr, 1, 40)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        drOutkaEdiL("DEST_AD_2") = cutAddr

        '住所編集（宛先を格納）
        mergeAddr = drRcvEdiDtl("SR").ToString()
        cutAddr = String.Empty
        '3
        cutAddr = SubStringOfByte(mergeAddr, 1, 80)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        drOutkaEdiL("DEST_AD_3") = cutAddr
        '4
        cutAddr = SubStringOfByte(mergeAddr, 1, 40)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        drOutkaEdiL("DEST_AD_4") = cutAddr
        '5
        cutAddr = SubStringOfByte(mergeAddr, 1, 40)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        drOutkaEdiL("DEST_AD_5") = cutAddr

        drOutkaEdiL("DEST_TEL") = sTelno
        drOutkaEdiL("DEST_FAX") = String.Empty
        drOutkaEdiL("DEST_MAIL") = String.Empty
        drOutkaEdiL("DEST_JIS_CD") = jisCd
        drOutkaEdiL("SP_NHS_KB") = String.Empty
        drOutkaEdiL("COA_YN") = String.Empty

        drOutkaEdiL("CUST_ORD_NO") = drRcvEdiDtl("WKO_NO")                        '荷主注文番号（全体）
    ...
    Private Function GetCustCd(ByVal ds As DataSet _
        ...
        Select Case iMstGoodsCnt
            ...
            Case 0      '商品マスタ取得０件
                ...
                sCustCdL = CUST_CD_L_CJC
                sCustCdM = CUST_CD_M_CJC
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC")
        Dim drRcvEdiDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC").Rows(0)
        Dim sNrsBrCd As String = ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC").Rows(0).Item("NRS_BR_CD").ToString()
        ...
        If iDeleteFlg = 1 Then
            Dim dtRcvHedDel As DataTable = ds.Tables("LMH030_DTL_CJC_CANCELOUT")
            Dim drRcvHedDel As DataRow = ds.Tables("LMH030_DTL_CJC_CANCELOUT").Rows(0)

            Dim dtRcvDtlDel As DataTable = ds.Tables("LMH030_DTL_CJC_CANCELOUT")
            Dim drRcvDtlDel As DataRow = ds.Tables("LMH030_DTL_CJC_CANCELOUT").Rows(0)
...
#Region "画面取込(セミEDI)チェック処理"
    ...
    Private Function SemiEdiTorikomiChk(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To hedmax
            ...
            Else
                If dtSemiDtl.Rows.Count = 0 Then
                    '抜き出したデータRowが０件の場合
...
#If False Then  'UPD 2019/10/09 007956   【LMS】セミEDI_千葉コーヴァンスジャパン_運用開始後要望3件
        sMsg = "出荷数(カラム9番目)["
        sNum = dr.Item("COLUMN_9").ToString()
...
#Else
        sMsg = "出荷数(カラム10番目)["
        sNum = dr.Item("COLUMN_10").ToString()
        If String.IsNullOrEmpty(sNum) Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_10") = 0
        Else
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC")          'EDI受信Dtl
        Dim dtRcvDtlCancel As DataTable = ds.Tables("LMH030_DTL_CJC_CANCELOUT")     'EDI受信Hed
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            Dim brErr As String = String.Empty
            Dim unsoErr As String = String.Empty
            ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC").Clear() '受信DTLをクリア
            ds = Me.SetSemiOutkaEdiRcv(ds, i)
#If True Then   'ADD 2019/10/15 007816

            ds = MyBase.CallDAC(Me._Dac, "CheckGoods", ds)
            If ds.Tables("LMH030_HENKAN_GOODS").Rows.Count > 0 Then
                ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC").Rows(0).Item("HENKANMAE_HINBAN") = ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC").Rows(0).Item("HINBAN").ToString.Trim
                ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC").Rows(0).Item("HINBAN") = ds.Tables("LMH030_HENKAN_GOODS").Rows(0).Item("HENKAN_GOODS_CD").ToString.Trim
            End If
#End If

            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC").Rows(0)

            '---------------------------------------------------------------------------
            ' 商品マスタ読込
            '---------------------------------------------------------------------------
            '商品マスタ情報を取得する
            ds.Tables("LMH030_M_GOODS").Clear() '商品マスタクリア
            ds = MyBase.CallDAC(Me._Dac, "SelectMstGoods", ds)

#If True Then   'ADD 2019/10/15 007816

            If ds.Tables("LMH030_HENKAN_GOODS").Rows.Count > 0 Then
                ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC").Rows(0).Item("HENKANMAE_MEISHO") = ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC").Rows(0).Item("MEISHO").ToString.Trim
                ds.Tables("LMH030_H_OUTKAEDI_DTL_CJC").Rows(0).Item("MEISHO") = ds.Tables("LMH030_M_GOODS").Rows(0).Item("GOODS_NM_1").ToString.Trim
            End If
#End If
            '---------------------------------------------------------------------------
            ' キー項目設定
            '---------------------------------------------------------------------------
            '取込データ1行=1出荷なのでキーによる纏め処理は不要
            bSameKeyFlg = False

            '---------------------------------------------------------------------------
            ' 納期に"キャンセル"が指定されている場合はEDI出荷登録しない
            ' あとで手動削除するための情報提供としてエラーEXCELは出力するが処理続行
            '---------------------------------------------------------------------------
            If drEdiRcvDtl.Item("NOUKI").ToString().Trim.Equals("キャンセル") Then
                MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E01I", New String() {String.Concat("WKO#:", drEdiRcvDtl.Item("WKO_NO").ToString())}, drEdiRcvDtl.Item("REC_NO").ToString().Trim(), LMH030BLC.EXCEL_COLTITLE_SEMIEDI, drEdiRcvDtl.Item("FILE_NAME").ToString().Trim())
                iSkipFlg = 1
            End If
            ...
            ' 商品マスタ読込
            '---------------------------------------------------------------------------
            sWhcd = ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("WH_CD").ToString()
            sCustCdL = CUST_CD_L_CJC
            sCustCdM = CUST_CD_M_CJC
            ...
            '---------------------------------------------------------------------------
            ' WKO#(オーダー番号)を元に取消データの確認処理を行う()
            '---------------------------------------------------------------------------
            '受信DTL取消データ取得処理
            ds.Tables("LMH030_DTL_CJC_CANCELOUT").Clear()    '取得用DSをクリア
            ds = MyBase.CallDAC(Me._Dac, "SelectOutkaEdiRcvCancel", ds)

            'データ取得できた場合
            If MyBase.GetResultCount > 0 Then
                '※直近のレコードで判断（SQL内でDESCされているので１件目のレコード）
                Dim drRcvDtlCancel As DataRow = ds.Tables("LMH030_DTL_CJC_CANCELOUT").Rows(0)

                '取得したデータの出荷管理番号が"00000000"の場合(出荷未登録の場合)　
                If Right((drRcvDtlCancel.Item("OUTKA_CTL_NO").ToString()), 8).Equals("00000000") Then
                ...
                Else
                    'エラーEXCELは出力するが処理続行
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E496", New String() {String.Concat("WKO#:", drEdiRcvDtl.Item("WKO_NO").ToString()), String.Concat("出荷管理番号:", drRcvDtlCancel.Item("OUTKA_CTL_NO").ToString(), LMH030BLC133.GUI_MSG_03)}, dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(), LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                End If
            End If
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH030_H_OUTKAEDI_DTL_CJC")
            setDtlDt.Clear()
            ...
            If iSkipFlg = 0 Then
                ...
                If MyBase.GetResultCount = 0 Then
                    ...
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E493", New String() {String.Concat("商品コード:", setDtlDt.Rows(0).Item("HINBAN").ToString()), "商品マスタ", String.Concat(" WKO#:", setDtlDt.Rows(0).Item("WKO_NO").ToString(), LMH030BLC133.GUI_MSG_01)}, dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(), LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                    'bNoErr = False
                    'Continue For
                ElseIf MyBase.GetResultCount > 1 Then
                    '商品マスタに荷主商品コードが複数存在する場合はエラー
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E493", New String() {String.Concat("商品コード:", setDtlDt.Rows(0).Item("HINBAN").ToString()), "商品マスタ", String.Concat(" WKO#:", setDtlDt.Rows(0).Item("WKO_NO").ToString(), LMH030BLC133.GUI_MSG_02)}, dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(), LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                    'bNoErr = False
                    'Continue For
                End If
...
#Region "Util"
    ...
    Public Shared Function SubStringOfByte(ByVal para As String, ByVal startIndex As Integer, Optional ByVal length As Integer = 0) As String
        ...
        Return SubStringOfByteNotTrim(para, startIndex, length)
