...
''' </summary>
''' <remarks></remarks>
Public Class LMH030BLC108
    Inherits Jp.Co.Nrs.LM.Base.BLC.LMBaseBLC

    '2012.02.28 千葉対応START
    '2012.02.28 旭化成ケミカルズ専用START
#Region "Const"

#Region "届先(引き取り)コード(旭化成ケミカルズ専用)パラメータ用CONST"

    ''' <summary>
    ''' 届先コード(00488080:引き取り)出荷(大)の届先名称にFREE_C11をセットするかで使用
    ''' </summary>
    ''' <remarks></remarks>
    Public Const EDI_DEST_CD_HIKITORI As String = "00488080"

#End Region

#End Region
    '2012.02.28 旭化成ケミカルズ専用END
    '2012.02.28 千葉対応END
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC108 = New LMH030DAC108()
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        '追加箇所 20110824 start
        '旭化成ケミカルズはまとめ対象外荷主(共通のまとめSQLを使用)
        ...
        If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
            ...
            ElseIf MyBase.GetResultCount > 1 Then
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.ASH_WID_L013, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningL("W199", LMH030BLC.ASH_WID_L013, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("0") = True Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.ASH_WID_L005, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = String.Empty
                    matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                    ds = Me._Blc.SetComWarningL("W168", LMH030BLC.ASH_WID_L005, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("1") = True Then
                ...
                If matomeStatus.Equals("10") = False Then
                    ...
                    choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.ASH_WID_L014, 0)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(4) = String.Empty
                        matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                        ds = Me._Blc.SetComWarningL("W198", LMH030BLC.ASH_WID_L014, ds, msgArray, matomeNo, String.Empty)
                        Return ds
...
#Region "実績作成処理"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        '旭化成ケミカルズEDI実績の値設定(EDI受信TBLより)
        ds = MyBase.CallDAC(Me._Dac, "SelectASHEdiSend", ds)
        ...
        '旭化成ケミカルズEDI実績データの更新
        ds = MyBase.CallDAC(Me._Dac, "InsertASHEdiSendData", ds)
...
#Region "EDI出荷(中)の初期値設定"
    ...
    Private Function EdiMDefaultSet(ByVal ds As DataSet, ByVal setDs As DataSet, _
        ...
        Else
            ...
            '2012.03.19 修正START
            choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.ASH_WID_M002, count)
            '2012.03.19 修正END
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                '2012.03.19 修正START
                'ds = Me._Blc.SetComWarningM("W159", LMH030BLC.ASH_WID_M002, ds, setDs, msgArray, ediGOODSNm, mGOODSNm)
                ds = Me._Blc.SetComWarningM("W159", LMH030BLC.ASH_WID_M002, ds, setDs, msgArray, _
        ...
        '入目単位
        '旭化成ケミカルズの場合、入目単位が異なる場合でもエラーまたはワーニングにしない(目欠対応)
        ...
        '        'チェックなし
        '    Else
        '        choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.ASH_WID_M003, count)
        ...
        '            msgArray(4) = "EDIデータ"
        '            msgArray(5) = String.Empty
        '            ds = Me._Blc.SetComWarningM("W159", LMH030BLC.ASH_WID_M003, ds, setDs, msgArray, ediIRIMENm, mIRIMENm)
...
#Region "EDI出荷(大)のDAC側でのチェック"
    ...
    Private Function EdiLDbExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        'DEST_CD、EDI_DEST_CDが両方とも空の場合はノーチェック
        If String.IsNullOrEmpty(workDestCd) = False Then
            ...
            Select mDestCount
                ...
                Case 1  '1件の場合
                    'マスタ値とEDI出荷(大)の整合性チェックを行い、
                    '旭化成ケミカルズの場合、不整合の場合はエラー
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.ASH_WID_M001, 0)
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    Else
                        ...
                        '2012.03.19 修正START
                        ds = Me._Blc.SetComWarningM("W162", LMH030BLC.ASH_WID_M001, ds, setDs, msgArray, custGoodsCd, String.Empty)
                        '2012.03.19 修正END
                ...
                If Me.EdiMDefaultSet(ds, setDs, i, unsoData, rowNo, ediCtlNo) = False Then
                    ...
                    '2012.03.01 千葉対応START
                    '旭化成ケミカルズは現段階ではワーニングが存在するが、エラーはなし。共通のロジックを組み込む為入れておく
                    Dim chkRowNo As Integer = Convert.ToInt32(rowNo)
                    If MyBase.IsMessageStoreExist(chkRowNo) = True Then
                        '整合性チェックでエラーがあった場合は処理終了
                        Return False
...
#Region "届先マスタチェック(旭化成ケミカルズ専用)"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        '2012.11.26 要望番号1626 修正START
        '旭化成はマスタの値を使用するので、受信したEDI出荷と整合性チェックは行わない

        ''届先名称(マスタ値が完全一致でない場合はエラー)
        'mDestNm = Me.SpaceCutChk(mDestNm)
        'ediDestNm = Me.SpaceCutChk(ediDestNm)
        'If Replace(mDestNm, "㈱", "（株）").Equals(ediDestNm) = False Then
        '    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E332", New String() {"届先名称", "届先マスタ", "届先名称"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
        '    Return False

        'End If

        ''届先マスタの住所1+2+3がEDI出荷(大)の届先住所(FREE_C21)に一致しない場合はエラー
        'mAdAll = SpaceCutChk(mAdAll)
        'ediFreeC21 = SpaceCutChk(ediFreeC21)
        'If ediFreeC21.Equals(mAdAll) = False Then
        '    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E332", New String() {"届先住所", "届先マスタ", "住所"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
        '    Return False
        'End If

        ''届先電話番号（マスタ値が完全一致でない場合はエラー）
        'If String.IsNullOrEmpty(ediTel) = False Then
        '    '入力されている場合のみチェック
        '    If mTel.Equals(ediTel) = False Then
        '        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E332", New String() {"届先電話番号", "届先マスタ", "電話番号"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
        '        Return False
        '    End If
        'End If
        '2012.11.26 要望番号1626 修正END
...
#End Region
    ...
    '        choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.ASH_WID_L012, 0)
    ...
    '            ds = Me._Blc.SetComWarningL("W186", LMH030BLC.ASH_WID_L012, ds, msgArray, workDestCd, String.Empty) '追加箇所 20110222
...
#Region "データセット設定(出荷M)"
    ...
    Private Function SetDatasetOutkaM(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            End If
            outkaDr("COA_YN") = FormatZero(ediDr("COA_YN").ToString(), 2)
            '要望番号:1741 terakawa 2013.01.08 Start
            'EDI出荷(大)のオーダ番号とEDI出荷(中)のオーダ番号が同一である場合、出荷(中)のオーダ番号を空で登録する。
            'outkaDr("CUST_ORD_NO_DTL") = ediDr("CUST_ORD_NO_DTL")
            If ediDr("CUST_ORD_NO_DTL").ToString = ediLDr("CUST_ORD_NO").ToString Then
            ...
            If ediDr("BUYER_ORD_NO_DTL").ToString = ediLDr("BUYER_ORD_NO").ToString Then
                outkaDr("BUYER_ORD_NO_DTL") = String.Empty
            Else
                outkaDr("BUYER_ORD_NO_DTL") = ediDr("BUYER_ORD_NO_DTL")
            End If
            '要望番号:1741 terakawa 2013.01.08 End
...
#Region "データセット設定(運送L)"
    ...
    Private Function SetDatasetUnsoL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            '2012.02.28 千葉対応START
            '旭化成ケミカルズ特殊処理START
            '通常はEDI出荷の配送時注意事項をセットするが、
            '旭化成ケミカルズの場合はEDI出荷(大)のFREE_C03(ＳＨ番号)をセットする
            'unsoDr("REMARK") = ediDr("UNSO_ATT")
            unsoDr("REMARK") = ediDr("FREE_C03")
            '旭化成ケミカルズ特殊処理END
            '2012.02.28 千葉対応END
