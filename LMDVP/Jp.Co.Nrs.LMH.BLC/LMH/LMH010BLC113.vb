...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC113 = New LMH010DAC113()
...
#Region "データセット設定(入荷S)"
    ...
    Private Function SetDatasetInkaS(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            '    inkaDr("GOODS_COND_KB_3") = ediDr("FREE_C01").ToString()
            'End If
            'If String.IsNullOrEmpty(ediDr("FREE_C02").ToString()) = False Then
            '    inkaDr("GOODS_CRT_DATE") = ediDr("FREE_C02").ToString()
            'End If
            inkaDr("GOODS_COND_KB_3") = String.Empty
            inkaDr("GOODS_CRT_DATE") = String.Empty
            ''使用期限
            'If String.IsNullOrEmpty(ediDr("FREE_C10").ToString()) = False Then
            '    inkaDr("LT_DATE") = ediDr("FREE_C10").ToString()
            'End If
            ...
            If String.IsNullOrEmpty(ediDr("FREE_C10").ToString()) = False Then
                inkaDr("LT_DATE") = ediDr("FREE_C10").ToString()
            End If
            'Select Case inkaDr("GOODS_COND_KB_3").ToString()
            '    Case "10", "15", "70", "80", "99"  '良品,良品（預託管理）,流通加工,クロスドック,ステージ
            '        inkaDr("SPD_KB") = "01"
            '        inkaDr("OFB_KB") = "01"

            '    Case "20", "30", "40", "60"   '不良品,返品,期限切れ,倉庫凍結
            '        inkaDr("SPD_KB") = "02"
            '        inkaDr("OFB_KB") = "01"
            ...
            'End Select
            inkaDr("SPD_KB") = "01"     '保留品区分
            inkaDr("OFB_KB") = "01"     '簿外品区分
...
#Region "実績作成処理(テルモ)"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        Dim rowNo As String = ds.Tables("LMH010INOUT").Rows(0).Item("ROW_NO").ToString()
        Dim ediCtlNo As String = ds.Tables("LMH010INOUT").Rows(0).Item("EDI_CTL_NO").ToString()
        Dim outkaFromOrdNo As String = ds.Tables("LMH010INOUT").Rows(0).Item("OUTKA_FROM_ORD_NO").ToString()

        '(テルモ)送信データ作成用情報取得
        ds = MyBase.CallDAC(Me._Dac, "SelectSendTermo", ds)
        ...
        '(テルモ)送信データの更新
        ds = MyBase.CallDAC(Me._Dac, "InsertSendTermo", ds)
        ...
        '入荷(大)の更新
        '※倉庫移動データは入荷登録を行わないので入荷データが存在しない
        If Not "倉庫移動".Equals(Left(outkaFromOrdNo, 4)) Then
