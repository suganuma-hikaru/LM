...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC206 = New LMH030DAC206()
...
#Region "プラントコード(大日本住友専用)パラメータ用CONST"

    ''' <summary>
    ''' プラントコード(医薬品用)(0251)
    ''' </summary>
    ''' <remarks></remarks>
    Public Const PLANT_CD_IYAKU As String = "0251"

    ''' <summary>
    ''' プラントコード(医薬品用)(9251)
    ''' </summary>
    ''' <remarks></remarks>
    Public Const PLANT_CD_IYAKU_2 As String = "9251"

    ''' <summary>
    ''' プラントコード(動物薬用)(8251)
    ''' </summary>
    ''' <remarks></remarks>
    Public Const PLANT_CD_DOUBUTU As String = "8251"

    ''' <summary>
    ''' 請求済数量MAX値(9999999999.999)
    ''' </summary>
    ''' <remarks></remarks>
    Public Const FKIMG_MAX As Double = 9999999999.999

    ''' <summary>
    ''' 梱包量MAX値(9999)
    ''' </summary>
    ''' <remarks></remarks>
    Public Const FXMRYOKNP_MAX As Integer = 9999

    ''' <summary>
    ''' バラ数量MAX値(99999)
    ''' </summary>
    ''' <remarks></remarks>
    Public Const FXMRYOBR_MAX As Integer = 99999
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        '大日本住友修正箇所 2012.02.25 START
        '第日本住友はまとめ対象荷主(共通のまとめSQLを使用)
        ...
        ds = Me.SetDatasetSagyo(ds, matomeFlg)
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.DSP_WID_M001, 0)
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    Else
                        ...
                        ds = Me._Blc.SetComWarningM("W162", LMH030BLC.DSP_WID_M001, ds, setDs, msgArray, custGoodsCd, String.Empty)
...
#Region "届先マスタチェック"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        Else
            '届先マスタの郵便番号からJISコードを取得する
            If (String.IsNullOrEmpty(mZip) = False) Then

                ds = MyBase.CallDAC(Me._DacCom, "SelectDataMjisFromDestZip", ds)
...
#Region "データセット設定(作業)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetDatasetSagyo(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet    'ADD 2023/08/09 037987 住友ファーマ 端数出荷時出荷オーダー単位の作業コード設定
        'Private Function SetDatasetSagyo(ByVal ds As DataSet) As DataSet   'DEL 2023/08/09 037987 住友ファーマ 端数出荷時出荷オーダー単位の作業コード設定
        ...
        Dim setHasuSagyo As Boolean = False
        Dim ediDrMSort As DataRow()

        '荷主明細マスタの値取得
        ds = MyBase.CallDAC(Me._Dac, "SelectListDataMcustDetails", ds)
        Dim custSagyoDt As DataTable = ds.Tables("LMH030_M_CUST_DETAILS")

        '出荷管理番号(中)の順にソート
        ediDrMSort = ds.Tables("LMH030_OUTKAEDI_M").Select(Nothing, "OUTKA_CTL_NO_CHU")
        'ADD END 2023/07/14 037987 住友ファーマ 端数出荷時出荷オーダー単位の作業コード設定

        'ADD START 2023/08/09 037987 住友ファーマ 端数出荷時出荷オーダー単位の作業コード設定
        'まとめ処理の場合
        If matomeFlg = True And custSagyoDt.Rows.Count > 0 Then
            'まとめ先データに端数で登録された商品が存在する場合、
            '既に端数商品への作業コード設定が行われているため、作業コード設定はスキップする
            Call MyBase.CallDAC(Me._Dac, "SelectSagyoCdSetCheckData", ds)
            If MyBase.GetResultCount > 0 Then
                setHasuSagyo = True
            End If
        End If
        ...
        For i As Integer = 0 To max
            ...
            'ediDrM = ds.Tables("LMH030_OUTKAEDI_M").Rows(i) 'DEL 2023/07/14 037987 住友ファーマ 端数出荷時出荷オーダー単位の作業コード設定
            ediDrM = ediDrMSort(i) 'ADD 2023/07/14 037987 住友ファーマ 端数出荷時出荷オーダー単位の作業コード設定

            'ADD START 2023/07/14 037987 住友ファーマ 端数出荷時出荷オーダー単位の作業コード設定
            If custSagyoDt.Rows.Count > 0 AndAlso Integer.TryParse(ediDrM.Item("OUTKA_HASU").ToString(), hasu) AndAlso hasu >= 1 Then

                '端数出荷の商品が登録された場合⇒端数で登録された商品のうち、出荷管理番号(中)が最も若い端数の商品に作業コードを設定
                If setHasuSagyo = False Then
                    ...
                    sagyoDr("IOZS_KB") = "21"
                    sagyoDr("SAGYO_CD") = custSagyoDt.Rows(0).Item("SET_NAIYO_2").ToString()
                    ...
                    'UPD START 2023/08/17 037987 住友ファーマ 端数出荷時出荷オーダー単位の作業コード設定
                    'sagyoDr("SAGYO_NB") = 0
                    sagyoDr("SAGYO_NB") = Me.GetSagyoNB(ds, sagyoDr("SAGYO_CD").ToString, ediDrM)
                    'UPD END 2023/08/17 037987 住友ファーマ 端数出荷時出荷オーダー単位の作業コード設定
                    sagyoDr("SAGYO_GK") = 0
            ...
            End If 'ADD 2023/07/14 037987 住友ファーマ 端数出荷時出荷オーダー単位の作業コード設定
        Next
...
#Region "セミEDI時　データセット設定(EDI受信DTL)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetSemiEdiRcvDtlDspCom(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        rcvDtlDr = ds.Tables("LMH030_OUTKAEDI_DTL_DSPCOM").NewRow()
        ...
        rcvDtlDr("ZIF_DENSYUBETU") = Me._Blc.LeftB(toriDtlDr("COLUMN_1").ToString().Trim(), 1)         '伝票種別
        rcvDtlDr("ZIF_NYUKABI") = Me._Blc.LeftB(toriDtlDr("COLUMN_2").ToString().Trim(), 8)            '入出荷日
        rcvDtlDr("ZIF_HARAI") = Me._Blc.LeftB(toriDtlDr("COLUMN_3").ToString().Trim(), 3)              '払場所
        rcvDtlDr("ZIF_WVFBSPDNR") = Me._Blc.LeftB(toriDtlDr("COLUMN_4").ToString().Trim(), 10)         '輸送業者
        rcvDtlDr("ZIF_NAME1") = Me._Blc.LeftB(toriDtlDr("COLUMN_5").ToString().Trim(), 35)             '輸送業者名
        rcvDtlDr("ZIF_SYUKASAKI") = Me._Blc.LeftB(toriDtlDr("COLUMN_6").ToString().Trim(), 7)          '出荷先コード
        rcvDtlDr("ZIF_TORIHIKI") = Me._Blc.LeftB(toriDtlDr("COLUMN_7").ToString().Trim(), 7)           '取引先コード
        rcvDtlDr("ZIF_HYOJYUNNO") = Me._Blc.LeftB(toriDtlDr("COLUMN_8").ToString().Trim(), 10)         '整理No（R/3標準伝票No.）
        rcvDtlDr("ZIF_SEIRI1") = Me._Blc.LeftB(toriDtlDr("COLUMN_9").ToString().Trim(), 8)             'SYSTEMNo（整理No.）
        rcvDtlDr("ZIF_CANCEL") = Me._Blc.LeftB(toriDtlDr("COLUMN_10").ToString().Trim(), 1)            '取消F
        rcvDtlDr("ZIF_SEIRI2") = Me._Blc.LeftB(toriDtlDr("COLUMN_11").ToString().Trim(), 1)            '明細No.
        rcvDtlDr("ZIF_DENTYPE") = Me._Blc.LeftB(toriDtlDr("COLUMN_12").ToString().Trim(), 4)           '伝票タイプ
        rcvDtlDr("ZIF_SYUBETU") = Me._Blc.LeftB(toriDtlDr("COLUMN_13").ToString().Trim(), 3)           '受払種別
        rcvDtlDr("ZIF_GYOUMU") = Me._Blc.LeftB(toriDtlDr("COLUMN_14").ToString().Trim(), 1)            '業務区分
        rcvDtlDr("ZIF_TAITLEA") = Me._Blc.LeftB(toriDtlDr("COLUMN_15").ToString().Trim(), 24)          'ﾀｲﾄﾙA
        rcvDtlDr("ZIF_TAITLEB") = Me._Blc.LeftB(toriDtlDr("COLUMN_16").ToString().Trim(), 8)           'ﾀｲﾄﾙB
        rcvDtlDr("ZIF_DENNO") = Me._Blc.LeftB(toriDtlDr("COLUMN_17").ToString().Trim(), 5)             '伝票№
        rcvDtlDr("ZIF_TORNAME1") = Me._Blc.LeftB(toriDtlDr("COLUMN_18").ToString().Trim(), 30)         '取引先名称上
        rcvDtlDr("ZIF_TORNAME2") = Me._Blc.LeftB(toriDtlDr("COLUMN_19").ToString().Trim(), 30)         '取引先名称下
        rcvDtlDr("FKM_K_SHN") = Me._Blc.LeftB(toriDtlDr("COLUMN_20").ToString().Trim(), 70)            '送品先名
        rcvDtlDr("ZIF_SOUHIJYU1") = Me._Blc.LeftB(toriDtlDr("COLUMN_21").ToString().Trim(), 60)        '送品先住所
        rcvDtlDr("ZID_OROSIDEP") = Me._Blc.LeftB(toriDtlDr("COLUMN_22").ToString().Trim(), 3)          'デポコード
        rcvDtlDr("ZIF_BIKOU1") = Me._Blc.LeftB(toriDtlDr("COLUMN_23").ToString().Trim(), 1)            '備区
        rcvDtlDr("ZIF_BIKOU2") = Me._Blc.LeftB(toriDtlDr("COLUMN_24").ToString().Trim(), 49)           '備考
        rcvDtlDr("ZID_BSARK") = Me._Blc.LeftB(toriDtlDr("COLUMN_25").ToString().Trim(), 4)             '発注T
        rcvDtlDr("HOUMEN") = Me._Blc.LeftB(toriDtlDr("COLUMN_26").ToString().Trim(), 3)                '方面
        rcvDtlDr("XHAISO") = Me._Blc.LeftB(toriDtlDr("COLUMN_27").ToString().Trim(), 4)                '配送仕分
        rcvDtlDr("ZIF_HAKOUSUMI") = Me._Blc.LeftB(toriDtlDr("COLUMN_28").ToString().Trim(), 1)         '発行済
        rcvDtlDr("FKM_NNY") = Me._Blc.LeftB(toriDtlDr("COLUMN_29").ToString().Trim(), 6)               '納入タ
        rcvDtlDr("ZIF_ONETIME") = Me._Blc.LeftB(toriDtlDr("COLUMN_30").ToString().Trim(), 1)           'ﾜﾝﾀｲﾑ
        rcvDtlDr("FKM_XHYO") = Me._Blc.LeftB(toriDtlDr("COLUMN_31").ToString().Trim(), 1)              '出力済
        rcvDtlDr("FXM_TRSD") = Me._Blc.LeftB(toriDtlDr("COLUMN_32").ToString().Trim(), 7)              '注文主CODE
        rcvDtlDr("FXM_LNTRRD") = Me._Blc.LeftB(toriDtlDr("COLUMN_33").ToString().Trim(), 50)           '注文主漢字
        rcvDtlDr("FXM_NWJKN") = Me._Blc.LeftB(toriDtlDr("COLUMN_34").ToString().Trim(), 2)             '荷渡条件
        rcvDtlDr("FXM_UKWB") = Me._Blc.LeftB(toriDtlDr("COLUMN_35").ToString().Trim(), 2)              '受渡場所
        rcvDtlDr("FXM_LNUKWB") = Me._Blc.LeftB(toriDtlDr("COLUMN_36").ToString().Trim(), 50)           '受渡場所漢字
        rcvDtlDr("FXM_KOKNM") = Me._Blc.LeftB(toriDtlDr("COLUMN_37").ToString().Trim(), 3)             '国名
        rcvDtlDr("FXM_GBHO") = Me._Blc.LeftB(toriDtlDr("COLUMN_38").ToString().Trim(), 4)              '保管区分
        rcvDtlDr("ZIF_CYOKU0") = Me._Blc.LeftB(toriDtlDr("COLUMN_39").ToString().Trim(), 7)            '直送先ｺｰﾄﾞ
        rcvDtlDr("ZIF_CYOKU1") = Me._Blc.LeftB(toriDtlDr("COLUMN_40").ToString().Trim(), 50)           '直送先名称
        rcvDtlDr("ZIF_CYOKU2") = Me._Blc.LeftB(toriDtlDr("COLUMN_41").ToString().Trim(), 50)           '直送先住所
        rcvDtlDr("ZIF_CKUB") = Me._Blc.LeftB(toriDtlDr("COLUMN_42").ToString().Trim(), 1)              'ｾﾝﾀｰ
        rcvDtlDr("ZIF_VSBED") = Me._Blc.LeftB(toriDtlDr("COLUMN_43").ToString().Trim(), 3)             '地域
        rcvDtlDr("ZIF_BTCSHIP") = Me._Blc.LeftB(toriDtlDr("COLUMN_44").ToString().Trim(), 2)           '出荷条件
        rcvDtlDr("ZIF_SUTETAS") = Me._Blc.LeftB(toriDtlDr("COLUMN_45").ToString().Trim(), 10)          '出荷バッチ
        rcvDtlDr("ZIF_JYUFUJU") = Me._Blc.LeftB(toriDtlDr("COLUMN_46").ToString().Trim(), 1)           '重複F
        rcvDtlDr("ZIF_SYORIDATE") = Me._Blc.LeftB(toriDtlDr("COLUMN_47").ToString().Trim(), 8)         '処理日
        rcvDtlDr("ZIF_SYORITIME") = Me._Blc.LeftB(toriDtlDr("COLUMN_48").ToString().Trim(), 6)         '処理時間
        rcvDtlDr("ZIF_HAKOUDATE") = Me._Blc.LeftB(toriDtlDr("COLUMN_49").ToString().Trim(), 8)         '発行日付
        rcvDtlDr("ZIF_HAKOUTIME") = Me._Blc.LeftB(toriDtlDr("COLUMN_50").ToString().Trim(), 6)         '発行時間
        rcvDtlDr("BWART") = Me._Blc.LeftB(toriDtlDr("COLUMN_51").ToString().Trim(), 3)                 'MvT
        rcvDtlDr("WERKS") = Me._Blc.LeftB(toriDtlDr("COLUMN_52").ToString().Trim(), 4)                 'Plnt(プラントコード)
        rcvDtlDr("LGORT") = Me._Blc.LeftB(toriDtlDr("COLUMN_53").ToString().Trim(), 4)                 'SLoc
        rcvDtlDr("ZIF_JULNO") = Me._Blc.LeftB(toriDtlDr("COLUMN_54").ToString().Trim(), 8)             'JUL№
        rcvDtlDr("MATNR") = Me._Blc.LeftB(toriDtlDr("COLUMN_55").ToString().Trim(), 18)                '品目
        rcvDtlDr("FXM_SP_CD") = Me._Blc.LeftB(toriDtlDr("COLUMN_56").ToString().Trim(), 3)             'ﾒｰｶｰｺｰﾄﾞ
        rcvDtlDr("CHO") = Me._Blc.LeftB(toriDtlDr("COLUMN_57").ToString().Trim(), 1)                   '貯
        rcvDtlDr("FXM_CD_THIN") = Me._Blc.LeftB(toriDtlDr("COLUMN_58").ToString().Trim(), 6)           '統一品名CD
        rcvDtlDr("FXM_GIMU") = Me._Blc.LeftB(toriDtlDr("COLUMN_59").ToString().Trim(), 1)              '生物
        rcvDtlDr("CHRG") = Me._Blc.LeftB(toriDtlDr("COLUMN_60").ToString().Trim(), 10)                 'ロット
        rcvDtlDr("VFDAT") = Me._Blc.LeftB(toriDtlDr("COLUMN_61").ToString().Trim(), 8)                 '有効期限
        rcvDtlDr("ZIF_FUGOU") = Me._Blc.LeftB(toriDtlDr("COLUMN_62").ToString().Trim(), 1)             '符号
        rcvDtlDr("FKIMG") = Me._Blc.LeftB(toriDtlDr("COLUMN_63").ToString().Trim(), 13)                '請求済数量 
        rcvDtlDr("FXM_RYO_KNP") = Me._Blc.LeftB(toriDtlDr("COLUMN_64").ToString().Trim(), 4)           '梱包量
        rcvDtlDr("FXM_RYO_BR") = Me._Blc.LeftB(toriDtlDr("COLUMN_65").ToString().Trim(), 5)            'バラ数量
        rcvDtlDr("ZIF_BARAFG") = Me._Blc.LeftB(toriDtlDr("COLUMN_66").ToString().Trim(), 1)            'バラF
        rcvDtlDr("ZIF_UKE") = Me._Blc.LeftB(toriDtlDr("COLUMN_67").ToString().Trim(), 3)               '受場所
        rcvDtlDr("ZIF_ZAIKOKUBUN") = Me._Blc.LeftB(toriDtlDr("COLUMN_68").ToString().Trim(), 1)        '在庫区分
        rcvDtlDr("ZIF_TEKIYOU1") = Me._Blc.LeftB(toriDtlDr("COLUMN_69").ToString().Trim(), 15)         '摘要前
        rcvDtlDr("ZIF_TEKIYOU2") = Me._Blc.LeftB(toriDtlDr("COLUMN_70").ToString().Trim(), 10)         '摘要後
        rcvDtlDr("ZIF_YOBIKUB1") = Me._Blc.LeftB(toriDtlDr("COLUMN_71").ToString().Trim(), 1)          '予備区分１
        rcvDtlDr("ZIF_YOBIKUB2") = Me._Blc.LeftB(toriDtlDr("COLUMN_72").ToString().Trim(), 1)          '予備区分２
        rcvDtlDr("ZIF_YUKOU") = Me._Blc.LeftB(toriDtlDr("COLUMN_73").ToString().Trim(), 8)             '有効日
        rcvDtlDr("MAKTX") = Me._Blc.LeftB(toriDtlDr("COLUMN_74").ToString().Trim(), 40)                '品目テキスト（品目名称）
        rcvDtlDr("HAISOUSAKI_CD") = Me._Blc.LeftB(toriDtlDr("COLUMN_75").ToString().Trim(), 7)         '配送先
        rcvDtlDr("HIN") = Me._Blc.LeftB(toriDtlDr("COLUMN_76").ToString().Trim(), 1)                   '品
        rcvDtlDr("T_KBN") = Me._Blc.LeftB(toriDtlDr("COLUMN_77").ToString().Trim(), 1)                 'T（テーブル区分）

        rcvDtlDr("JISSEKI_SHORI_FLG") = "1"                                                     '実績処理フラグ

        'データセットに設定
        ds.Tables("LMH030_OUTKAEDI_DTL_DSPCOM").Rows.Add(rcvDtlDr)
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet, ByVal mGoodsCnt As Integer) As DataSet

        Dim outkaediMDr As DataRow

        Dim mGoodsDt As DataTable = setDs.Tables("LMH030_M_GOODS")
        Dim rcvDtlDr As DataRow = setDs.Tables("LMH030_OUTKAEDI_DTL_DSPCOM").Rows(0)
        ...
        outkaediMDr("CUST_ORD_NO_DTL") = rcvDtlDr("FXM_TRSD")
        outkaediMDr("BUYER_ORD_NO_DTL") = String.Empty
        outkaediMDr("CUST_GOODS_CD") = rcvDtlDr("MATNR")

        If mGoodsCnt = 0 Then
            '商品マスタ取得０件
            outkaediMDr("NRS_GOODS_CD") = String.Empty
            outkaediMDr("GOODS_NM") = String.Empty
            outkaediMDr("IRIME") = 0
        ElseIf mGoodsCnt = 1 Then
            '商品マスタ取得１件
            outkaediMDr("NRS_GOODS_CD") = mGoodsDt.Rows(0).Item("GOODS_CD_NRS")
            outkaediMDr("GOODS_NM") = mGoodsDt.Rows(0).Item("GOODS_NM_1")
            outkaediMDr("IRIME") = mGoodsDt.Rows(0).Item("STD_IRIME_NB")
        ElseIf mGoodsCnt > 1 Then
            '商品マスタ取得２件以上
            outkaediMDr("NRS_GOODS_CD") = String.Empty
            outkaediMDr("GOODS_NM") = String.Empty
        ...
        outkaediMDr("LOT_NO") = rcvDtlDr("CHRG")
        outkaediMDr("SERIAL_NO") = String.Empty
        outkaediMDr("ALCTD_KB") = "01"
        outkaediMDr("OUTKA_PKG_NB") = 0
        outkaediMDr("OUTKA_HASU") = rcvDtlDr("FKIMG")
        outkaediMDr("OUTKA_QT") = 0
        outkaediMDr("OUTKA_TTL_NB") = rcvDtlDr("FKIMG")
        outkaediMDr("OUTKA_TTL_QT") = 0
        outkaediMDr("KB_UT") = String.Empty
        outkaediMDr("QT_UT") = String.Empty
        outkaediMDr("PKG_NB") = 0
        ...
        outkaediMDr("AKAKURO_KB") = rcvDtlDr("ZIF_CANCEL")
        ...
        outkaediMDr("FREE_N01") = rcvDtlDr("FKIMG")
        outkaediMDr("FREE_N02") = rcvDtlDr("FKIMG")
        ...
        outkaediMDr("FREE_C01") = Me._Blc.LeftB(rcvDtlDr("FILE_NAME").ToString(), 100)  'ファイル名が長い場合に備えて
        outkaediMDr("FREE_C02") = Convert.ToInt32(rcvDtlDr("REC_NO")).ToString("00000")
        outkaediMDr("FREE_C03") = String.Empty
        outkaediMDr("FREE_C04") = String.Empty
        outkaediMDr("FREE_C05") = rcvDtlDr("ZIF_HYOJYUNNO")
        outkaediMDr("FREE_C06") = rcvDtlDr("ZIF_SEIRI2")
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet) As DataSet
        ...
        '要望番号987対応 2012/05/14 honmyo Start
        'outkaediLDr("OUTKA_KB") = String.Empty
        outkaediLDr("OUTKA_KB") = "10"
        '要望番号987対応 2012/05/14 honmyo End
        ...
        Select Case ediCustIndex
            Case "82"   '医薬品の場合
                ...
                '出荷予定日
                '出庫日
                If Left(MyBase.GetSystemTime(), 4) < "1430" Then
                    '14時30分前の場合は当日
                    outkaediLDr("OUTKA_PLAN_DATE") = MyBase.GetSystemDate()
                    outkaediLDr("OUTKO_DATE") = MyBase.GetSystemDate()
                Else
                    'そうでない場合は翌稼働日
                    Dim sWorkDay As String = GetBussinessDay(MyBase.GetSystemDate(), +1, setDs).ToString("yyyyMMdd")
                    outkaediLDr("OUTKA_PLAN_DATE") = sWorkDay
                    outkaediLDr("OUTKO_DATE") = sWorkDay
                End If

                '納入予定日
                If Left(MyBase.GetSystemTime(), 4) < "1300" Then
                    '13時前の場合は翌稼働日
                    Dim sWorkDay As String = GetBussinessDay(MyBase.GetSystemDate(), +1, setDs).ToString("yyyyMMdd")
                    outkaediLDr("ARR_PLAN_DATE") = sWorkDay
                Else
                    'そうでない場合は翌々稼働日
                    Dim sWorkDay As String = GetBussinessDay(MyBase.GetSystemDate(), +2, setDs).ToString("yyyyMMdd")
                    outkaediLDr("ARR_PLAN_DATE") = sWorkDay
                End If
            ...
            Case "85"   '動物薬の場合
                ...
                '納入予定日
                '無条件に翌々稼働日
                sWorkDay = GetBussinessDay(MyBase.GetSystemDate(), +2, setDs).ToString("yyyyMMdd")
                outkaediLDr("ARR_PLAN_DATE") = sWorkDay
        ...
        outkaediLDr("EDI_DEST_CD") = rcvDtlDr("ZIF_CYOKU0")
        outkaediLDr("DEST_CD") = rcvDtlDr("ZIF_CYOKU0")
        outkaediLDr("DEST_NM") = rcvDtlDr("ZIF_CYOKU1")
        outkaediLDr("DEST_ZIP") = String.Empty
        outkaediLDr("DEST_AD_1") = Me._Blc.LeftB(rcvDtlDr("ZIF_SOUHIJYU1").ToString(), 40)
        outkaediLDr("DEST_AD_2") = Me._Blc.MidB(rcvDtlDr("ZIF_SOUHIJYU1").ToString(), 41)   '41バイト目以降すべて
        outkaediLDr("DEST_AD_3") = String.Empty
        outkaediLDr("DEST_AD_4") = String.Empty
        outkaediLDr("DEST_AD_5") = String.Empty
        outkaediLDr("DEST_TEL") = String.Empty
        outkaediLDr("DEST_FAX") = String.Empty
        ...
        outkaediLDr("CUST_ORD_NO") = rcvDtlDr("ZIF_DENNO")
        outkaediLDr("BUYER_ORD_NO") = rcvDtlDr("ZIF_TEKIYOU1")

        '運送元区分は出荷時に判断されるので必要なし2012/03/23
        ...
        ''注意事項
        Dim sBikou1 As String = Trim(rcvDtlDr("ZIF_BIKOU1").ToString)
        Dim sBikou2 As String = Trim(rcvDtlDr("ZIF_BIKOU2").ToString)
        Dim sRemark As String = String.Empty

        If sBikou1.Equals("Q") Then
            If String.IsNullOrEmpty(sBikou2) Then
                sRemark = "緊急出荷"
            Else
                sRemark = String.Concat("緊急出荷", ":", sBikou2)
            End If
        Else
            sRemark = sBikou2
        End If
        outkaediLDr("REMARK") = sRemark

        outkaediLDr("UNSO_ATT") = String.Empty
        outkaediLDr("DENP_YN") = "1"
        outkaediLDr("PC_KB") = String.Empty
        outkaediLDr("UNCHIN_YN") = "1"
        outkaediLDr("NIYAKU_YN") = String.Empty
        outkaediLDr("OUT_FLAG") = "0"
        outkaediLDr("AKAKURO_KB") = rcvDtlDr("ZIF_CANCEL")
        ...
        outkaediLDr("FREE_C01") = Me._Blc.LeftB(rcvDtlDr("FILE_NAME").ToString(), 100)  'ファイル名が長い場合に備えて
        outkaediLDr("FREE_C02") = Convert.ToInt32(rcvDtlDr("REC_NO")).ToString("00000")
        outkaediLDr("FREE_C03") = String.Empty
        outkaediLDr("FREE_C04") = rcvDtlDr("ZIF_HYOJYUNNO")
        outkaediLDr("FREE_C05") = rcvDtlDr("ZIF_SEIRI2")
        ...
        outkaediLDr("FREE_C20") = String.Concat(semiEdiDr("CUST_CD_L").ToString(), semiEdiDr("CUST_CD_M").ToString())
        outkaediLDr("FREE_C21") = rcvDtlDr("ZIF_SOUHIJYU1").ToString()
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function GetEdiCtlNo(ByVal ds As DataSet, ByVal iDeleteFlg As Integer, ByVal iCancelFlg As Integer, ByVal bSameKeyFlg As Boolean _
        ...
        Dim rcvDtldr As DataRow = ds.Tables("LMH030_OUTKAEDI_DTL_DSPCOM").Rows(0)
        Dim nrsBrCd As String = ds.Tables("LMH030_OUTKAEDI_DTL_DSPCOM").Rows(0).Item("NRS_BR_CD").ToString()



        If iCancelFlg = 1 Then
            'キャンセルフラグが１の場合はEDI管理番号は"T00000000"固定
            sEdiCtlNo = "T00000000"
            iEdiCtlNoChu = 0    '0固定
        Else
            'レコードが１行目または送られてきたtxtの整理№(カラム８列目)が前行と差異がある場合
        ...
        If iDeleteFlg = 1 Then
            Dim rcvDtlDeldt As DataTable = ds.Tables("LMH030_DTL_DSPCOM_CANCELOUT")
            Dim rcvDtlDeldr As DataRow = ds.Tables("LMH030_DTL_DSPCOM_CANCELOUT").Rows(0)
            rcvDtlDeldt.Rows(0).Item("DELETE_EDI_NO") = sEdiCtlNo
            rcvDtlDeldt.Rows(0).Item("DELETE_EDI_NO_CHU") = iEdiCtlNoChu.ToString("000")
        End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim rcvDtlDt As DataTable = ds.Tables("LMH030_OUTKAEDI_DTL_DSPCOM")
        Dim rcvCancelDt As DataTable = ds.Tables("LMH030_DTL_DSPCOM_CANCELOUT")
        ...
        For i As Integer = 0 To max
            ...
            ds.Tables("LMH030_OUTKAEDI_DTL_DSPCOM").Clear() '受信DTLをクリア
            ds = Me.SetSemiEdiRcvDtlDspCom(ds, i)
            ...
            If i = 0 Then
                ...
                bSameKeyFlg = False
                sOldKey = rcvDtlDr.Item("ZIF_SEIRI1").ToString()
            Else
                '1番目以降はキーを比較
                sNewKey = rcvDtlDr.Item("ZIF_SEIRI1").ToString()
            ...
            '送られてきたtxtの取消F(カラム10番目)が"1"(：取消し)の場合
            If (rcvDtlDr.Item("ZIF_CANCEL").ToString()).Equals("1") = True Then

                '受信DTL取消データ取得処理
                ds.Tables("LMH030_DTL_DSPCOM_CANCELOUT").Clear()    '取得用DSをクリア
                ds = MyBase.CallDAC(Me._Dac, "SelectRcvDtlDspComTorikesi", ds)
                ...
                If MyBase.GetResultCount > 0 Then
                    ...
                    '取消フラグを1にする
                    iDeleteFlg = 1
                    cancelCnt = MyBase.GetResultCount

                    '取得したデータの出荷管理番号が("T00000000")(出荷未登録の場合)　※直近のレコードで判断（SQL内でDESCされているので１件目のレコード）
                    If Right((rcvCancelDt.Rows(0).Item("OUTKA_CTL_NO").ToString()), 8).Equals("00000000") = True Then
            ...
            ds = Me.GetEdiCtlNo(ds, iDeleteFlg, iCancelFlg, bSameKeyFlg, sEdiCtlNo, iEdiCtlNoChu)

            '取消フラグが"1"の場合、取消処理を行う
            If iDeleteFlg = 1 Then

                'EDI受信(DTL)の削除(更新)
                ds = MyBase.CallDAC(Me._Dac, "UpdateEdiDelRcvDtlData", ds)
                iRcvDtlCanCnt = iRcvDtlCanCnt + 1
                'とりあえずコメント（件数０件の場合も有り得る？）
                'If MyBase.GetResultCount = 0 Then
                '    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E011", , toriDtlDt.Rows(i).Item("REC_NO").ToString(), LMH030BLC.EXCEL_COLTITLE, String.Empty)
                '    Return ds
                'End If
            End If
            ...
            If iCancelFlg = 1 Then
                ...
                'EDI出荷(大)の削除(更新)
                ds = MyBase.CallDAC(Me._Dac, "UpdateEdiDelOutkaEdiL", ds)
                ...
                'EDI出荷(中)の削除(更新)
                ds = MyBase.CallDAC(Me._Dac, "UpdateEdiDelOutkaEdiM", ds)
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDt As DataTable = setDs.Tables("LMH030_OUTKAEDI_DTL_DSPCOM")
            setDt.Clear()
            setDt.ImportRow(rcvDtlDt.Rows(0))

            'EDI受信(DTL)の新規追加
            '削除EDI番号をクリアする
            setDs = MyBase.CallDAC(Me._Dac, "InsertEdiRcvDtlData", setDs)
            iRcvDtlInsCnt = iRcvDtlInsCnt + 1

            'キャンセルフラグが"0"の場合、EDI出荷データの追加処理を行う
            If iCancelFlg = 0 Then

                '商品マスタ情報の取得
                setDs = MyBase.CallDAC(Me._Dac, "SelectDataMgoodsOutka", setDs)
                mGoodsCnt = MyBase.GetResultCount
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow, ByVal ediCustIndex As String) As Boolean
        ...
        'プラントコード(カラム52番目)
        Dim plantCd As String = dr.Item("COLUMN_52").ToString()

        Select Case ediCustIndex
            Case "82"   '医薬品の場合
                '20140604 要望管理2199対応 Start
                'If plantCd.Equals(LMH030BLC206.PLANT_CD_IYAKU) = False Then
                If plantCd.Equals(LMH030BLC206.PLANT_CD_IYAKU) = False AndAlso plantCd.Equals(LMH030BLC206.PLANT_CD_IYAKU_2) = False Then
                    '20140604 要望管理2199対応 E N D
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E434", , sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If

            Case "85"   '動物薬の場合
                If plantCd.Equals(LMH030BLC206.PLANT_CD_DOUBUTU) = False Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E444", , sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If
        End Select

        '請求済数量(カラム63番目)
        Dim sFkiMg As String = dr.Item("COLUMN_63").ToString().Trim()

        If String.IsNullOrEmpty(sFkiMg) = True Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_63") = 0
        Else
            If IsNumeric(sFkiMg) = True Then
                '数値の場合
                Dim dFkiMg As Double = Convert.ToDouble(sFkiMg)

                If dFkiMg > LMH030BLC206.FKIMG_MAX Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("請求済数量(カラム63番目)[", dFkiMg.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If
            Else
                '数値でない場合
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("請求済数量(カラム63番目)[", sFkiMg, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
        End If

        '梱包量(カラム64番目)
        Dim sFxmRyoKnp As String = dr.Item("COLUMN_64").ToString().Trim()
        If String.IsNullOrEmpty(sFxmRyoKnp) = True Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_64") = 0
        Else
            If IsNumeric(sFxmRyoKnp) Then
                '数値の場合
                Dim iFxmRyoKnp As Integer = Convert.ToInt32(sFxmRyoKnp)
                If iFxmRyoKnp > LMH030BLC206.FXMRYOKNP_MAX Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("梱包量(カラム64番目)[", iFxmRyoKnp.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If
            Else
                '数値でない場合
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("梱包量(カラム64番目)[", sFxmRyoKnp, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            End If
        End If

        'バラ数量(カラム65番目)
        Dim sFxmRyoBr As String = dr.Item("COLUMN_65").ToString().Trim()
        If String.IsNullOrEmpty(sFxmRyoBr) = True Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_65") = 0
        Else
            If IsNumeric(sFxmRyoBr) Then
                '数値の場合
                Dim iFxmRyoBr As Integer = Convert.ToInt32(sFxmRyoBr)
                If iFxmRyoBr > LMH030BLC206.FXMRYOBR_MAX Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("バラ数量(カラム65番目)[", iFxmRyoBr.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If
            Else
                '数値でない場合
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("バラ数量(カラム65番目)[", sFxmRyoBr.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            End If
        End If

        '出荷日(カラム2番目)
        Dim sOutkaDate As String = dr.Item("COLUMN_2").ToString()

        If IsDate(sOutkaDate) = True Then
            dr.Item("COLUMN_2") = CDate(sOutkaDate).ToString("yyyyMMdd")    'スラッシュを除いた日付を設定
        Else
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat("出荷日(カラム2番目)[", sOutkaDate, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        '有効期限(カラム61番目)
        Dim sVfDat As String = dr.Item("COLUMN_61").ToString()

        If IsDate(sVfDat) = True Then
            dr.Item("COLUMN_61") = CDate(sVfDat).ToString("yyyyMMdd")
        Else
            dr.Item("COLUMN_61") = String.Empty
            '有効期限はエラーとしない
            'MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {"有効期限(カラム61番目)"}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            'bRet = False
        End If


        '有効日(カラム73番目)
        Dim sZifYukou As String = dr.Item("COLUMN_73").ToString()

        If IsDate(sZifYukou) = True Then
            dr.Item("COLUMN_73") = CDate(sZifYukou).ToString("yyyyMMdd")
        Else
            '※有効日は0000/00/00のパターンが有り得るのでエラーとしない
            dr.Item("COLUMN_73") = String.Empty
            'MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {"有効日(カラム73番目)"}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            'bRet = False
        End If

        'エラーチェックは行わないが、日付、時間のフォーマット変換を行う
        dr.Item("COLUMN_47") = CDate(dr.Item("COLUMN_47").ToString()).ToString("yyyyMMdd")  '処理日
        dr.Item("COLUMN_48") = CDate(dr.Item("COLUMN_48").ToString()).ToString("HHmmss")    '処理時間
        dr.Item("COLUMN_49") = CDate(dr.Item("COLUMN_49").ToString()).ToString("yyyyMMdd")  '発行日付
        dr.Item("COLUMN_50") = CDate(dr.Item("COLUMN_50").ToString()).ToString("HHmmss")    '発行時間
...
#Region "作業個数の設定"
    ...
    ''' <param name="ediDrM"></param>
    ''' <returns></returns>
    Private Function GetSagyoNB(ByVal ds As DataSet, ByVal sagyoCD As String, ByVal ediDrM As DataRow) As Decimal

        Dim mSagyo As DataRow() = ds.Tables("LMH030_M_SAGYO").Select(String.Concat("SAGYO_CD ='", sagyoCD, "'"))
        Dim sagyoNB As Integer = 0
        Dim outkaM As DataRow() = ds.Tables("LMH030_C_OUTKA_M").Select(String.Concat("OUTKA_NO_L ='", ediDrM.Item("OUTKA_CTL_NO").ToString(), "' AND OUTKA_NO_M ='", ediDrM.Item("OUTKA_CTL_NO_CHU").ToString(), "'"))
        Dim pkgNb As Integer = 0

        If outkaM.Length > 0 Then
            If IsNumeric(outkaM(0).Item("OUTKA_M_PKG_NB").ToString()) = True Then
                pkgNb = Convert.ToInt32(outkaM(0).Item("OUTKA_M_PKG_NB").ToString())
            End If
        End If
        ...
        If mSagyo.Length > 0 Then
            ...
            ElseIf ("02").Equals(mSagyo(0).Item("KOSU_BAI").ToString) = True Then
                '出荷総個数を設定
                If IsNumeric(ediDrM.Item("OUTKA_TTL_NB").ToString()) = True Then
                    sagyoNB = Convert.ToInt32(ediDrM.Item("OUTKA_TTL_NB").ToString())
                End If

            ElseIf ("03").Equals(mSagyo(0).Item("KOSU_BAI").ToString) = True Then
                '出荷データ編集では、ロットナンバーが設定されている出荷（小）の件数を設定しているが、
                'この時点では出荷（小）を作成していないため設定なし
        ...
        If sagyoNB = 0 Then
            '０の場合、出荷梱包個数を設定（出荷データ編集の設定ロジックに準拠）
            sagyoNB = pkgNb
        End If
