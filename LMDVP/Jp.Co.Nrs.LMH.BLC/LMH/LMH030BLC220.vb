...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC220 = New LMH030DAC220()
...
#Region "日興産業用CONST"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Const WH_CD_AGCW As String = "200"               '倉庫コード
    'Public Const CUST_CD_L_AGCW As String = "00440"         '荷主コード（大）
    Public Const CUST_CD_L_AGCW As String = "30859"         '荷主コード（大）
    Public Const CUST_CD_M_AGCW As String = "00"            '荷主コード（中）
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer, ByRef brErr As String, ByRef usnoErr As String) As DataSet

        Dim drSemiEdiInfo As DataRow = ds.Tables("LMH030_SEMIEDI_INFO").Rows(0)

        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_AGCW").NewRow()
        Dim drSetDtl As DataRow = ds.Tables("LMH030_EDI_TORIKOMI_DTL").Rows(i)

        '配送会社コードを区分マスタを用いてNRSのコードに変換する
        ds.Tables("LMH030_Z_KBN_AGCW").Clear()
        Dim drSetKbn As DataRow = ds.Tables("LMH030_Z_KBN_AGCW").NewRow()
        drSetKbn("NRS_BR_CD") = drSemiEdiInfo("NRS_BR_CD")
        drSetKbn("KBN_GROUP_CD") = "H029"
        drSetKbn("KBN_NM1") = drSemiEdiInfo("NRS_BR_CD")
        drSetKbn("KBN_NM2") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 5)
        ds.Tables("LMH030_Z_KBN_AGCW").Rows.Add(drSetKbn)

        ds = MyBase.CallDAC(Me._Dac, "SelectMKbnH029", ds)

        Dim nrsUnsoCd As String = String.Empty
        Dim nrsUnsoBrCd As String = String.Empty
        If MyBase.GetResultCount = 0 Then
            '区分マスタに存在しない場合はエラー扱いとする（変換前のコードを返すことで呼び元がエラーとする）
            brErr = drSemiEdiInfo("NRS_BR_CD").ToString()
            usnoErr = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 5)
        Else
            nrsUnsoCd = ds.Tables("LMH030_Z_KBN_AGCW").Rows(0).Item("KBN_NM3").ToString()
            nrsUnsoBrCd = ds.Tables("LMH030_Z_KBN_AGCW").Rows(0).Item("KBN_NM4").ToString()
        End If
        ...
        drEdiRcvDtl("CUST_CD_L") = CUST_CD_L_AGCW                                                       '荷主コード（大）
        drEdiRcvDtl("CUST_CD_M") = CUST_CD_M_AGCW                                                       '荷主コード（中）
        drEdiRcvDtl("PRTFLG") = "0"                                                                     'プリントフラグ
        'drEdiRcvDtl("CANCEL_FLG") = "0"                                                                 'キャンセルフラグ
        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "1"                                                          '実績処理フラグ

        '荷主固有データ
        drEdiRcvDtl("OUTKO_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_1").ToString().Trim(), 8)            '出庫日
        drEdiRcvDtl("CUST_GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_2").ToString().Trim(), 20)        '商品コード
        drEdiRcvDtl("LOT_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 40)               'ロットNo.
        drEdiRcvDtl("IRIME") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 10)                '入数
        drEdiRcvDtl("QT") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 10)                   '本数
        drEdiRcvDtl("UNSO_CD") = nrsUnsoCd                                                              '配送会社コード（NRSのコードに変換したもの）
        drEdiRcvDtl("UNSO_BR_CD") = nrsUnsoBrCd                                                         '配送会社支店コード（NRSのコードに変換したもの）
        drEdiRcvDtl("CUST_UNSO_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 5)          '配送会社コード（取込ファイルそのまま）
        drEdiRcvDtl("UNSO_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 60)              '配送会社名
        drEdiRcvDtl("DEST_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 80)              '出荷先名
        drEdiRcvDtl("DEST_BUSYO_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 40)        '出荷先部署、担当
        drEdiRcvDtl("DEST_AD_1") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 80)           '出荷先住所１
        drEdiRcvDtl("DEST_AD_2") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 80)           '出荷先住所２
        drEdiRcvDtl("DEST_TEL") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 20)            '出荷先電話番号
        drEdiRcvDtl("REMARK") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 100)             '備考

        'データセットに設定
        ds.Tables("LMH030_H_OUTKAEDI_DTL_AGCW").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
        ...
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH030_H_OUTKAEDI_DTL_AGCW").Rows(0)
        ...
        drOutkaEdiM("OUTKA_HASU") = drRcvEdiDtl("QT")                    '出荷端数 
        'drOutkaEdiM("OUTKA_TTL_NB") = drRcvEdiDtl("DENPYO_TANI_TTLQT")  '出荷総個数
        drOutkaEdiM("OUTKA_TTL_NB") = drRcvEdiDtl("QT")                  '出荷総個数
        ...
        If setDs.Tables("LMH030_M_GOODS").Rows.Count = 1 Then
            ...
            drOutkaEdiM("OUTKA_QT") = Convert.ToDecimal(drRcvEdiDtl("QT")) * Convert.ToDecimal(drGoods("STD_IRIME_NB"))       '出荷数量
            drOutkaEdiM("OUTKA_TTL_QT") = Convert.ToDecimal(drRcvEdiDtl("QT")) * Convert.ToDecimal(drGoods("STD_IRIME_NB"))   '出荷総数量
        ...
        '注意事項 
        drOutkaEdiM("REMARK") = drRcvEdiDtl("REMARK").ToString
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        drOutkaEdiL("OUTKA_PLAN_DATE") = drRcvEdiDtl("OUTKO_DATE").ToString    '出荷予定日
        drOutkaEdiL("OUTKO_DATE") = drRcvEdiDtl("OUTKO_DATE").ToString         '出庫日
        'drOutkaEdiL("ARR_PLAN_DATE") = drRcvEdiDtl("ARR_PLAN_DATE").ToString           '納入予定日
        drOutkaEdiL("ARR_PLAN_DATE") = drRcvEdiDtl("OUTKO_DATE").ToString           '納入予定日(納入予定日は出荷予定日と同値をセット)
        ...
        drOutkaEdiL("DEST_NM") = drRcvEdiDtl("DEST_NM")

        drOutkaEdiL("DEST_ZIP") = String.Empty

        '住所編集
        Dim mergeAddr As String = String.Concat(drRcvEdiDtl("DEST_AD_1").ToString(), "　", drRcvEdiDtl("DEST_AD_2").ToString(), "　", drRcvEdiDtl("DEST_BUSYO_NM").ToString())
        Dim cutAddr As String = String.Empty
        '1
        cutAddr = SubStringOfByte(mergeAddr, 1, 40)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        drOutkaEdiL("DEST_AD_1") = cutAddr.Trim()
        '2
        cutAddr = SubStringOfByte(mergeAddr, 1, 40)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        drOutkaEdiL("DEST_AD_2") = cutAddr.Trim()
        '3
        cutAddr = SubStringOfByte(mergeAddr, 1, 80)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        drOutkaEdiL("DEST_AD_3") = cutAddr.Trim()
        '4
        cutAddr = SubStringOfByte(mergeAddr, 1, 40)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        drOutkaEdiL("DEST_AD_4") = cutAddr.Trim()
        '5
        cutAddr = SubStringOfByte(mergeAddr, 1, 40)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        drOutkaEdiL("DEST_AD_5") = cutAddr.Trim()
        ...
        drOutkaEdiL("CUST_ORD_NO") = String.Empty                        '荷主注文番号（全体）
        ...
        drOutkaEdiL("UNSO_CD") = drRcvEdiDtl("UNSO_CD")
        drOutkaEdiL("UNSO_BR_CD") = drRcvEdiDtl("UNSO_BR_CD")

        drOutkaEdiL("UNSO_NM") = drRcvEdiDtl("UNSO_NM")
        ...
        drOutkaEdiL("FREE_C01") = drRcvEdiDtl("DEST_BUSYO_NM").ToString
        drOutkaEdiL("FREE_C02") = drRcvEdiDtl("CUST_UNSO_CD").ToString
    ...
    Private Function GetCustCd(ByVal ds As DataSet _
        ...
        Select Case iMstGoodsCnt
            ...
            Case 0      '商品マスタ取得０件
                ...
                sCustCdL = CUST_CD_L_AGCW
                sCustCdM = CUST_CD_M_AGCW
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH030_H_OUTKAEDI_DTL_AGCW")
        Dim drRcvEdiDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_AGCW").Rows(0)
        Dim sNrsBrCd As String = ds.Tables("LMH030_H_OUTKAEDI_DTL_AGCW").Rows(0).Item("NRS_BR_CD").ToString()
        ...
        If iDeleteFlg = 1 Then
            Dim dtRcvHedDel As DataTable = ds.Tables("LMH030_DTL_AGCW_CANCELOUT")
            Dim drRcvHedDel As DataRow = ds.Tables("LMH030_DTL_AGCW_CANCELOUT").Rows(0)

            Dim dtRcvDtlDel As DataTable = ds.Tables("LMH030_DTL_AGCW_CANCELOUT")
            Dim drRcvDtlDel As DataRow = ds.Tables("LMH030_DTL_AGCW_CANCELOUT").Rows(0)
...
#Region "画面取込(セミEDI)チェック処理"
    ...
    Private Function SemiEdiTorikomiChk(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To hedmax
            ...
            Else
                '出庫日(COLUMN_1)の取得値が怪しいので統一する
                For k As Integer = 0 To dtSemiDtl.Rows.Count - 1
                    Dim sDate As String = dtSemiDtl.Rows(k).Item("COLUMN_1").ToString()
                    If sDate.IndexOf("/") > 0 Then
                        dtSemiDtl.Rows(k).Item("COLUMN_1") = Date.Parse(sDate).ToString("yyyyMMdd")
                    End If
                Next

                'ファイル内(dtSemiDtl)の出庫日(COLUMN_1),出荷先電話番号(COLUMN_12)でソートしセットする
                Dim drSelect As DataRow() = dtSemiDtl.Select(String.Empty, " COLUMN_1 ASC, COLUMN_12 ASC")   'ソート順
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow, ByVal ediCustIndex As String) As Boolean
        ...
        sMsg = "出庫日(カラム1番目)["
        sDate = dr.Item("COLUMN_1").ToString()
        ...
        sMsg = "本数(カラム5番目)["
        sNum = dr.Item("COLUMN_5").ToString()
        If String.IsNullOrEmpty(sNum) Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_5") = 0
        Else
        ...
        sMsg = "出荷先電話番号(カラム12番目)["
        sDate = dr.Item("COLUMN_12").ToString()
        '桁数チェック
        If sDate.Length = 0 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E433", New String() {String.Concat(sMsg, sDate, "]")}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH030_H_OUTKAEDI_DTL_AGCW")         'EDI受信Dtl
        Dim dtRcvDtlCancel As DataTable = ds.Tables("LMH030_DTL_AGCW_CANCELOUT")    'EDI受信Hed
        ...
        Dim bBeforeDup As Boolean = False           '直前行で重複エラーが発生True、否False
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            ds.Tables("LMH030_H_OUTKAEDI_DTL_AGCW").Clear() '受信DTLをクリア
            ds = Me.SetSemiOutkaEdiRcv(ds, i, brErr, unsoErr)
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_AGCW").Rows(0)
            If Not String.IsNullOrEmpty(unsoErr) Then
                '配送会社コードの変換ができなければスルーする
                MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E493", New String() {"配送会社変換情報", "区分マスタ", String.Concat("区分グループコード:H029 区分名称１:", brErr, " 区分名称２:", unsoErr)}, drEdiRcvDtl.Item("REC_NO").ToString().Trim(), LMH030BLC.EXCEL_COLTITLE_SEMIEDI, drEdiRcvDtl.Item("FILE_NAME").ToString().Trim())
                Continue For
            End If
            ...
            ' キー項目設定
            '---------------------------------------------------------------------------
            sNewKey = String.Concat(drEdiRcvDtl.Item("OUTKO_DATE").ToString, "_", _
                                    drEdiRcvDtl.Item("DEST_TEL").ToString)
            ...
            'AGC若狭：最初期の仕様として実装したが要望により削除
            ''---------------------------------------------------------------------------
            '' EDI受信データを検索して、既に同一出庫日,出荷先電話番号が存在する場合はスルーする
            ''---------------------------------------------------------------------------
            'If bSameKeyFlg = False OrElse bBeforeDup = True Then
            '    If Me._Dac.GetOutkaEdiRcvDtlCount(ds) > 0 Then
            '        MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E285", New String() {String.Concat("出庫日:", drEdiRcvDtl.Item("OUTKO_DATE").ToString(), " 出荷先電話番号:", drEdiRcvDtl.Item("DEST_TEL").ToString())}, drEdiRcvDtl.Item("REC_NO").ToString().Trim(), LMH030BLC.EXCEL_COLTITLE_SEMIEDI, drEdiRcvDtl.Item("FILE_NAME").ToString().Trim())
            '        bBeforeDup = True
            '        Continue For
            '    Else
            '        bBeforeDup = False
            ...
            ''---------------------------------------------------------------------------
            ''受信DTL取消データ取得処理
            'ds.Tables("LMH030_DTL_AGCW_CANCELOUT").Clear()    '取得用DSをクリア
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH030_H_OUTKAEDI_DTL_AGCW")
            setDtlDt.Clear()
            ...
            If iSkipFlg = 0 Then
                ...
                If MyBase.GetResultCount = 0 Then
                    ...
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E493", New String() {String.Concat("商品コード:", setDtlDt.Rows(0).Item("CUST_GOODS_CD").ToString()), "商品マスタ", String.Concat(" 出庫日:", setDtlDt.Rows(0).Item("OUTKO_DATE").ToString(), " 出荷先電話番号:", setDtlDt.Rows(0).Item("DEST_TEL").ToString(), LMH030BLC220.GUI_MSG_01)}, dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(), LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                    'bNoErr = False
                    'Continue For
                ElseIf MyBase.GetResultCount > 1 Then
                    '商品マスタに荷主商品コードが複数存在する場合はエラー
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E493", New String() {String.Concat("商品コード:", setDtlDt.Rows(0).Item("CUST_GOODS_CD").ToString()), "商品マスタ", String.Concat(" 出庫日:", setDtlDt.Rows(0).Item("OUTKO_DATE").ToString(), " 出荷先電話番号:", setDtlDt.Rows(0).Item("DEST_TEL").ToString(), LMH030BLC220.GUI_MSG_02)}, dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(), LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                    'bNoErr = False
                    'Continue For
                End If
