...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC504 = New LMH030DAC504()
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        '注)基本的に大日精化はまとめ処理を行わないが、共通まとめ処理のロジックを組み込む
        ...
        If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
            ...
            ElseIf MyBase.GetResultCount > 1 Then
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.DNS_WID_L006, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningL("W199", LMH030BLC.DNS_WID_L006, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("0") = True Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.DNS_WID_L004, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = String.Empty
                    matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                    ds = Me._Blc.SetComWarningL("W168", LMH030BLC.DNS_WID_L004, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("1") = True Then
                ...
                If matomeStatus.Equals("10") = False Then
                    ...
                    choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.DNS_WID_L005, 0)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(4) = String.Empty
                        matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                        ds = Me._Blc.SetComWarningL("W198", LMH030BLC.DNS_WID_L005, ds, msgArray, matomeNo, String.Empty)
                        Return ds
        ...
        '要望番号:1180(まとめ先の出荷データが2件以上ある場合、登録できない) 2012/06/20 本明 Start（以下Endまでコメント化）
        ''自動まとめフラグ = "0" or "1"の場合、まとめ処理
        'If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
        ...
        '        choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.DNS_WID_L007, 0)
        ...
        '            msgArray(4) = String.Empty
        '            matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
        '            ds = Me._Blc.SetComWarningL("W168", LMH030BLC.DNS_WID_L007, ds, msgArray, matomeNo, String.Empty)
        '            Return ds
        ...
        '要望番号:1180(まとめ先の出荷データが2件以上ある場合、登録できない) 2012/06/20 本明 End
...
#Region "EDI_Lの初期値設定(出荷登録処理)"
    ...
    Private Function SetEdiLShoki(ByVal ds As DataSet) As DataSet
        ...
        '0件対応 修正 terakawa 2012.08.21 Start
        If MyBase.GetResultCount > 0 Then
            ediDr("UNSO_NM") = ds.Tables("LMH030_M_UNSOCO").Rows(0).Item("UNSOCO_NM").ToString()
        End If
        '0件対応 修正 terakawa 2012.08.21 End
        ...
        If String.IsNullOrEmpty(ediDr("DENP_YN").ToString().Trim()) = True Then
            If String.IsNullOrEmpty(ediDr("UNSO_CD").ToString().Trim()) = False AndAlso _
                ...
                Else
                    ...
                    ''運送会社M取得
                    'ds = MyBase.CallDAC(Me._DacCom, "SelectDataMUnsocoList", ds)
                    'ediDr("FREE_C10") = ds.Tables("LMH030_M_UNSOCO").Rows(0).Item("NRS_SBETU_CD").ToString()
                End If
            Else
                ediDr("DENP_YN") = "0"
            End If
...
#Region "EDI出荷(中)の初期値設定"
    ...
    Private Function EdiMDefaultSet(ByVal ds As DataSet, ByVal setDs As DataSet, _
        ...
        ''-------------------------------------------------------------------------------------
        ''●荷主固有チェック(大日精化専用)①イエローカード
        ''-------------------------------------------------------------------------------------
        '商品明細マスタの取得
        setDs = MyBase.CallDAC(Me._Dac, "SelectListDataMgoodsDetails", setDs)

        If MyBase.GetResultCount > 0 Then
            Dim sYellowCardMst As String = setDs.Tables("LMH030_M_GOODS_DETAILS").Rows(0)("SET_NAIYO").ToString()
            Dim sYellowCardEdi As String = ediLDr.Item("FREE_C01").ToString()

            'FREE_C01(EDI_L)と商品明細マスタ(サブ区分"01"(イエローカード))の整合性チェック
            If (sYellowCardEdi).Equals(sYellowCardMst) = False Then

                choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.DNS_WID_M001, count)

                If String.IsNullOrEmpty(choiceKb) = True Then
                    msgArray(1) = "イエローカード(FREE_C01)"
                    msgArray(2) = "商品明細マスタ"
                    msgArray(3) = "イエローカード"
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningM("W159", LMH030BLC.DNS_WID_M001, ds, setDs, msgArray, sYellowCardEdi, sYellowCardMst)
                ...
                ElseIf choiceKb.Equals("01") = True Then
                    'ワーニングで"はい"を選択時
                    '商品明細マスタの内容で登録する
                    ediLDr.Item("FREE_C01") = sYellowCardMst
        ...
        'さらに促音変換を行う（大日専用特殊処理）
        mGoodsNm = _Blc.ReplaceSokuon(mGoodsNm)
        ediGoodsNm = _Blc.ReplaceSokuon(ediGoodsNm)
        ...
        If String.IsNullOrEmpty(ediMDr("DEST_GOODS_NM").ToString()) = False Then
            ...
            'さらに促音変換を行う（大日専用特殊処理）
            mDestGoodsNm = _Blc.ReplaceSokuon(mDestGoodsNm)
        End If
        ...
        ElseIf String.IsNullOrEmpty(mGoodsNm) = False AndAlso String.IsNullOrEmpty(mDestGoodsNm) = False AndAlso _
            ...
            choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.DNS_WID_M002, count)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(5) = String.Empty
                ds = Me._Blc.SetComWarningM("W194", LMH030BLC.DNS_WID_M002, ds, setDs, msgArray, _
        ...
        If (ediIrime).Equals(mIrime) = False Then
            ...
            choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.DNS_WID_M003, count)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(5) = String.Empty
                ds = Me._Blc.SetComWarningM("W194", LMH030BLC.DNS_WID_M003, ds, setDs, msgArray, ediIrime, mIrime)
        ...
        '' ''-------------------------------------------------------------------------------------
        '' ''●荷主固有チェック(大日精化専用)④入目単位
        '' ''-------------------------------------------------------------------------------------
        ' ''入目単位
        ''Dim mIrimeUt As String = mGoodsDr("STD_IRIME_UT").ToString()
        ''Dim ediIrimeUt As String = ediMDr("IRIME_UT").ToString()

        ''If String.IsNullOrEmpty(ediIrimeUt) = False Then

        ''    If mIrimeUt.Equals(ediIrimeUt) = True Then
        ''        'チェックなし
        ''    Else
        ''        choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.DNS_WID_M004, count)
        ...
        ''            msgArray(4) = "EDIデータ"
        ''            msgArray(5) = String.Empty
        ''            ds = Me._Blc.SetComWarningM("W159", LMH030BLC.DNS_WID_M004, ds, setDs, msgArray, ediIrimeUt, mIrimeUt)
...
#Region "EDI出荷(大)のDAC側でのチェック"
    ...
    Private Function EdiLDbExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        ElseIf mDestCount = 0 Then
            ...
            'If SetInsMDestFromDest(ds, workDestCd, workDestString) = False Then
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.DNS_WID_M004, 0)
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    Else
                        ...
                        ds = Me._Blc.SetComWarningM("W162", LMH030BLC.DNS_WID_M004, ds, setDs, msgArray, custGoodsCd, String.Empty)

                        flgWarning = True 'ワーニングフラグをたてて処理続行

                        Continue For
                        '2012.04.17 要望番号1000 修正END(コメントをはずす)
                    End If

                End If

                '商品が確定した場合、商品の在庫引当可能数を取得し、引当可能数-出荷数量<0の時エラーとする
                If flgWarning <> True Then

                    Dim sNRS_BR_CD As String = dtL.Rows(0).Item("NRS_BR_CD").ToString                                   '営業所コード
                    Dim sGOODS_CD_NRS As String = setDs.Tables("LMH030_M_GOODS").Rows(0).Item("GOODS_CD_NRS").ToString  '日陸商品コード
                    Dim sLOT_NO As String = dtM.Rows(i).Item("LOT_NO").ToString                                         'ロットナンバー
                    '要望番号:1155 2012/06/14 本明 Start
                    Dim dIRIME As Double = Convert.ToDouble(dtM.Rows(i).Item("IRIME"))                                  '入目
                    '要望番号:1155 2012/06/14 本明 End
                    Dim iOUTKA_TTL_NB As Integer = Convert.ToInt32(dtM.Rows(i).Item("OUTKA_TTL_NB"))                    '出荷数量

                    Dim sEDI_CTL_NO As String = dtM.Rows(i).Item("EDI_CTL_NO").ToString                                 'EDI_CTL_NO
                    Dim sEDI_CTL_NO_CHU As String = dtM.Rows(i).Item("EDI_CTL_NO_CHU").ToString                         'EDI_CTL_NO_CHU
                    Dim sGOODS_NM1 As String = setDs.Tables("LMH030_M_GOODS").Rows(0).Item("GOODS_NM_1").ToString       '商品名

                    '要望番号:1166 2012/06/18 本明 Start
                    Dim sCUST_GOODS_CD As String = dtM.Rows(i).Item("CUST_GOODS_CD").ToString                               '荷主商品コード
                    '要望番号:1166 2012/06/18 本明 End

                    '営業所コード、日陸商品コード、ロット№、入目から引当可能数を取得する
                    '要望番号:1155 2012/06/14 本明 Start
                    'Dim iALLOC_CAN_NB As Integer = _DacCom.GetALLOC_CAN_NB(sNRS_BR_CD, sGOODS_CD_NRS, sLOT_NO)
                    Dim iALLOC_CAN_NB As Integer = _DacCom.GetALLOC_CAN_NB(sNRS_BR_CD, sGOODS_CD_NRS, sLOT_NO, dIRIME)
                    '要望番号:1155 2012/06/14 本明 End

                    '引当可能数-出荷数量<0の場合はエラー
                    If iALLOC_CAN_NB - iOUTKA_TTL_NB < 0 Then
                        '要望番号:1155 2012/06/14 本明 Start
                        'Dim sMsgErr As String = String.Concat(" EDI管理番号 = ", sEDI_CTL_NO, "-", sEDI_CTL_NO_CHU _
                        '                                      , "、商品コード = ", sGOODS_CD_NRS _
                        '                                      , "、商品名 = ", sGOODS_NM1 _
                        '                                      , "、ロット番号 = ", sLOT_NO _
                        '                                      , "、出荷指示数 = ", iOUTKA_TTL_NB.ToString _
                        '                                      , "、引当可能数 = ", iALLOC_CAN_NB.ToString)
                        '要望番号:1166 2012/06/18 本明 Start
                        'Dim sMsgErr As String = String.Concat(" EDI管理番号 = ", sEDI_CTL_NO, "-", sEDI_CTL_NO_CHU _
                        '                                    , "、商品コード = ", sGOODS_CD_NRS _
                        '                                    , "、商品名 = ", sGOODS_NM1 _
                        ...
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E486", New String() {sMsgErr}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                        Return False
                    End If
                End If
                ...
                If Me.EdiMDefaultSet(ds, setDs, i, unsoData, rowNo, ediCtlNo) = False Then
                    ...
                    '大日精化はEDI出荷(中)でワーニングチェックがある為、エラーorワーニングの判断を行う
                    Dim chkRowNo As Integer = Convert.ToInt32(rowNo)
                    If MyBase.IsMessageStoreExist(chkRowNo) = True Then
                        '整合性チェックでエラーがあった場合は処理終了
                        Return False
...
#Region "届先マスタチェック(大日精化専用)"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        mDestNm = _Blc.ReplaceCompany(mDestNm) '大日精化特化処理　"㈱"、"（株）"、"（株式会社）"、"株式会社"、"㈲"、 "（有）"、 "（有限会社）"、 "有限会社"を除く

        ediDestNm = _Blc.SpaceCutChk(ediDestNm)
        ediDestNm = _Blc.ReplaceCompany(ediDestNm) '大日精化特化処理　"㈱"、"（株）"、"（株式会社）"、"株式会社"、"㈲"、 "（有）"、 "（有限会社）"、 "有限会社"を除く


        '届先名称(届先マスタの届先名にEDI出荷(大)の届先名称が含まれていなければワーニング)
        If mDestNm.Equals(ediDestNm) = True Then
            'チェックなし
        ElseIf InStr(mDestNm.ToString().Trim(), ediDestNm.ToString().Trim()) = 0 Then
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DNS_WID_L001, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                ds = Me._Blc.SetComWarningL("W166", LMH030BLC.DNS_WID_L001, ds, msgArray, _
        ...
        'FREE_C21:届先住所(届先マスタの住所1+住所2+住所3にEDI出荷(大)のFREE_C21が含まれていなければワーニング)
        If String.IsNullOrEmpty(ediFreeC21) = True Then
            'チェックなし
        Else

            mAdAll = _Blc.SpaceCutChk(mAdAll)
            mAdAll = _Blc.ReplaceHyphen(mAdAll)          '大日精化特化処理　"ー"、"―"を"－"に置換

            ediFreeC21 = _Blc.SpaceCutChk(ediFreeC21)
            ediFreeC21 = _Blc.ReplaceHyphen(ediFreeC21)  '大日精化特化処理　"ー"、"―"を"－"に置換

            If InStr(mAdAll.ToString().Trim(), ediFreeC21.ToString().Trim()) = 0 Then
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DNS_WID_L002, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.DNS_WID_L002, ds, msgArray, ediFreeC21, mAdAll)
        ...
        If String.IsNullOrEmpty(ediTel) = True Then
            'チェックなし
        Else
            If mTel.Equals(Replace(Replace(ediTel, "(", String.Empty), ")", String.Empty)) = False Then

                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DNS_WID_L003, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.DNS_WID_L003, ds, msgArray, ediTel, mTel)

                    compareWarningFlg = True

                ElseIf choiceKb.Equals("01") = True Then
                    'ワーニングで"はい"を選択時
                    dtMdest.Rows(0).Item("TEL") = Replace(Replace(dtEdi.Rows(0)("DEST_TEL").ToString(), "(", String.Empty), ")", String.Empty)
                    'マスタ更新対象フラグ
                    dtMdest.Rows(0).Item("MST_UPDATE_FLG") = "1"
        ...
        ''        choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DNS_WID_L004, 0)
        ...
        ''            msgArray(4) = "EDIデータ"
        ''            msgArray(5) = String.Empty
        ''            ds = Me._Blc.SetComWarningL("W166", LMH030BLC.DNS_WID_L004, ds, msgArray, ediZip, mZip)
        ''            compareWarningFlg = True
...
#Region "データセット設定(出荷L)"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            '大日特殊処理 出荷大のREMARKには登録の必要なし、ただし運送大のREMARKに登録する 2012/06/01
...
#Region "データセット設定(出荷M)"
    ...
    Private Function SetDatasetOutkaM(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            outkaDr("YELLOW_CARD") = ediLDr("Free_C01")
...
#Region "紐付け処理"
    ...
    Private Function Himoduke(ByVal ds As DataSet) As DataSet
        ...
        ' ''受信DTL(EXT)データセット
        ''ds = Me.SetDatasetEdiRcvDtl(ds, "1")
