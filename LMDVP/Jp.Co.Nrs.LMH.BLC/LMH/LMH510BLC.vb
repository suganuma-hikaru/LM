...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH510DAC = New LMH510DAC()

    Private _513_Dac As LMH513DAC = New LMH513DAC()
...
#Region "Const"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_IN As String = "LMH510IN"

    ''' <summary>
    ''' OUTテーブル名
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_OUT As String = "LMH510OUT"
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = LMH510BLC.TABLE_NM_IN
        Dim tableNmOut As String = LMH510BLC.TABLE_NM_OUT
        Dim tableNmRpt As String = LMH510BLC.TABLE_NM_M_RPT
        ...
        workDtRpt = view.ToTable(True, LMH510BLC.NRS_BR_CD, LMH510BLC.PTN_ID, LMH510BLC.PTN_CD, LMH510BLC.RPT_ID)
        ...
        For Each dr As DataRow In ds.Tables(tableNmRpt).Rows
            ...
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables(tableNmOut), dr.Item(LMH510BLC.RPT_ID).ToString())

            '帳票ごとの編集があるなら行う。
            prtDs = Me.EditPrintDataSet(dr.Item(LMH510BLC.RPT_ID).ToString(), prtDs)

            '帳票CSV出力
            comPrt.StartPrint(Me, _
                              dr.Item(LMH510BLC.NRS_BR_CD).ToString(), _
                              dr.Item(LMH510BLC.PTN_ID).ToString(), _
                              dr.Item(LMH510BLC.PTN_CD).ToString(), _
                              dr.Item(LMH510BLC.RPT_ID).ToString(), _
    ...
    Private Function SelectMPrt(ByVal ds As DataSet) As DataSet
        ...
        ds = MyBase.CallDAC(Me._Dac, LMH510BLC.ACTION_ID_SELECT_MPRT, ds)
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        Select Case ds.Tables("M_RPT").Rows(0).Item("RPT_ID").ToString
            Case "LMH513"
                Return MyBase.CallDAC(Me._513_Dac, LMH510BLC.ACTION_ID_SELECT_PRT_DATA, ds)
            Case Else

                Return MyBase.CallDAC(Me._Dac, LMH510BLC.ACTION_ID_SELECT_PRT_DATA, ds)
