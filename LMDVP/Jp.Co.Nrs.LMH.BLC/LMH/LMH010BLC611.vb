...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC611 = New LMH010DAC611()
...
#Region "入荷登録マスタ存在チェック(中)"
    Private Function SetGoodsMst(ByRef ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        drKbnIn.Item("KBN_GROUP_CD") = "L009"
        drKbnIn.Item("KBN_NM1") = dtL.Rows(0).Item("NRS_BR_CD")
        drKbnIn.Item("KBN_NM2") = dtL.Rows(0).Item("CUST_CD_L")
        drKbnIn.Item("KBN_NM3") = dtL.Rows(0).Item("CUST_CD_M")
        dtKbnIn.Rows.Add(drKbnIn)
        dsKbn = MyBase.CallDAC(Me._MstDac, "SelectZKbnHanyo", dsKbn)
        If dtKbnOut.Rows.Count >= 1 Then
            touNo = dtKbnOut.Rows(0).Item("KBN_NM4").ToString()
            situNo = dtKbnOut.Rows(0).Item("KBN_NM5").ToString()
            zoneCd = dtKbnOut.Rows(0).Item("KBN_NM6").ToString()
        End If
...
#Region "データセット設定(入荷S)"
    ...
    Private Function SetDatasetInkaS(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            inkaDr("GOODS_COND_KB_3") = ediDr("FREE_C03")
            inkaDr("SPD_KB") = "01"
            inkaDr("OFB_KB") = "01"
            If ediDr.Item("DENP_KBN_RAPI").ToString() = "004" Then
                inkaDr("REMARK") = "容器返却"
            End If
...
#Region "実績作成処理"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        With "通常の入荷実績作成処理"
            ...
            '送信データ作成用情報取得
            ds = MyBase.CallDAC(Me._Dac, "SelectSendRapidus", ds)
            ...
            '送信データの登録
            ds = MyBase.CallDAC(Me._Dac, "InsertSendRapidus", ds)
        ...
        With "出荷EDI作成処理"
            '※ Rapidusでは出荷のEDIが送信されてこないため、入荷EDIを元にして出荷EDIを自前で作成する必要がある

            'EDI出荷管理番号Lの付与
            Dim num As New NumberMasterUtility
            Dim savInkaEdiCtlNo As String = String.Empty
            Dim ediOutkaNoL As String = String.Empty
            Dim ediCtlNoChu As Integer = 0

            For Each dr As DataRow In ds.Tables("H_SENDINEDI_RAPI").Rows
                Dim inkaEdiCtlNo As String = dr.Item("EDI_CTL_NO").ToString()
                If savInkaEdiCtlNo.Equals(inkaEdiCtlNo) Then
                    ediCtlNoChu += 1
                Else
                    ediOutkaNoL = num.GetAutoCode(NumberMasterUtility.NumberKbn.EDI_OUTKA_NO_L, Me, ds.Tables("LMH010INOUT").Rows(0).Item("NRS_BR_CD").ToString)
                    ediCtlNoChu = 1
                    savInkaEdiCtlNo = inkaEdiCtlNo
                End If
                dr.Item("OUTKA_EDI_CTL_NO") = ediOutkaNoL
                dr.Item("OUTKA_EDI_CTL_NO_CHU") = ediCtlNoChu.ToString().PadLeft(3, CChar("0"))
            Next

            '出荷EDI受信データの登録
            ds = MyBase.CallDAC(Me._Dac, "InsertOutkaEdiRapidus", ds)

            'EDI出荷(大)の登録
            ds = MyBase.CallDAC(Me._Dac, "InsertEdiOutkaL", ds)

            'EDI出荷(中)の登録
            ds = MyBase.CallDAC(Me._Dac, "InsertEdiOutkaM", ds)
