...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC113 = New LMH030DAC113()
...
#Region "実績作成処理"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        'テルモEDI実績の値設定(EDI受信TBLより)
        ds = MyBase.CallDAC(Me._Dac, "SelectTerumoEdiSend", ds)
        ...
        Else
            ...
            max = ds.Tables("LMH030_H_SENDOUTEDI_TRM").Rows.Count - 1
            For i As Integer = 0 To max

                If Convert.ToDecimal(ds.Tables("LMH030_H_SENDOUTEDI_TRM").Rows(i).Item("NON_SHIPPABLE_QTY")) = 0 _
                AndAlso Convert.ToDecimal(ds.Tables("LMH030_H_SENDOUTEDI_TRM").Rows(i).Item("SHIPPING_QTY")) = 0 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E539", New String() {String.Concat("商品コード： ", _
                                           ds.Tables("LMH030_H_SENDOUTEDI_TRM").Rows(i).Item("ITEM_CODE").ToString())}, _
        ...
        'テルモEDI実績データの作成
        ds = MyBase.CallDAC(Me._Dac, "InsertTerumoEdiSendData", ds)
        ...
        '出荷(大)の更新
        If updFlg.Equals("1") = True AndAlso (outkaDelFlg.Equals("1")) = False Then
            ds = MyBase.CallDAC(Me._Dac, "UpdateOutkaLData", ds)
            If MyBase.GetResultCount = 0 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E011", , rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                Return ds
    ...
    Private Function EdiMDefaultSet(ByVal ds As DataSet, ByVal setDs As DataSet, _
        ...
        Else
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(2) = "商品マスタ"
                msgArray(3) = "商品名"
                msgArray(4) = "注意)テルモ製品マスタに差分データが送られてきている可能性があります。必ず確認して下さい。"
