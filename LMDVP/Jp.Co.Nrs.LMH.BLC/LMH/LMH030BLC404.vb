...
'  EDI荷主ID　　　　:  404　　　        : 丸和(横浜)　セミEDI標準仕様
'  作  成  者       :  daikoku　　　　　:(000　標準ベース生産) 
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC404 = New LMH030DAC404()
...
#Region "EDI出荷(大)のBLC側でのチェック"
    ...
    Private Function EdiLKanrenCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        If String.IsNullOrEmpty(outkaDate) = True Then
            'E019
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E019", New String() {"出荷予定日"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            Return False
        Else
        ...
        If String.IsNullOrEmpty(arrPlanDate) = True Then
            'E019
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E019", New String() {"納入予定日"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            Return False
        Else
...
#Region "届先マスタチェック"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        If String.IsNullOrEmpty(ediZip) = True Then
            'チェックなし
        Else
            If mZip.Replace("-", String.Empty).Equals(ediZip.Replace("-", String.Empty)) = False Then
...
#Region "データセット設定(出荷L)"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            '納品先担当者名をAD_3に付加
            If String.Empty.Equals(ediDr("FREE_C02").ToString.Trim) = False Then
                outkaDr("DEST_AD_3") = String.Concat(outkaDr("DEST_AD_3").ToString.Trim, " ", ediDr("FREE_C02").ToString.Trim)

                '60バイトまでセット
                outkaDr("DEST_AD_3") = SetString(outkaDr("DEST_AD_3").ToString.Trim, 60)

            End If

            If ("○").Equals(ediDr("FREE_C26").ToString.Trim) Then
                If IsNumeric(ediDr("FREE_N01")) = True Then
                    Dim lngAmt As Long = CLng(ediDr("FREE_N01").ToString)

                    outkaDr("NHS_REMARK") = String.Concat("代金引換額 \", Format(lngAmt, "###,###,##0"))
                Else
                End If
            End If

            'outkaDr("NHS_REMARK") = ediDr("FREE_C26")
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        '    drOutkaEdiL("DEST_AD_3") = String.Concat(drOutkaEdiL("DEST_AD_3").ToString.Trim, " ", drRcvEdiDtl(sTempColNo).ToString().Trim())

        '    'バイト数チェック
        '    Dim validLen As Integer = 0

        '    validLen = System.Text.Encoding.GetEncoding(932).GetByteCount(drOutkaEdiL("DEST_AD_3").ToString.Trim)

        '    If validLen > 60 Then
        '        drOutkaEdiL("DEST_AD_3") = CombineString(drOutkaEdiL("DEST_AD_3").ToString.Trim, 60)
        '    End If
        ...
        '荷主オーダー番号
        Dim sKey1 As String = String.Empty
        Dim sKey2 As String = String.Empty
        Dim sKey3 As String = String.Empty

        sKey = "DEVIDE_NO_1"
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            sKey1 = drRcvEdiDtl.Item(sTempColNo).ToString().Trim()
        End If

        sKey = "DEVIDE_NO_2"
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            sKey2 = drRcvEdiDtl.Item(sTempColNo).ToString().Trim()
        End If

        sKey = "DEVIDE_NO_3"
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            sKey3 = drRcvEdiDtl.Item(sTempColNo).ToString().Trim()
        End If

        drOutkaEdiL.Item("CUST_ORD_NO") = String.Concat(sKey1, sKey2, sKey3)
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
        ...
        Dim sTempColNo As String = String.Empty
        Dim sTempColNo2 As String = String.Empty
        Dim sTempColNo3 As String = String.Empty
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            ...
            sKey = "DEVIDE_NO_1"
            If isNullColKey(drSemiEdiInfo, sKey, sTempColNo2) <> False Then
                sKey1 = drRcvEdiDtl.Item(sTempColNo2).ToString().Trim()
            End If

            sKey = "DEVIDE_NO_2"
            If isNullColKey(drSemiEdiInfo, sKey, sTempColNo3) <> False Then
                sKey2 = drRcvEdiDtl.Item(sTempColNo3).ToString().Trim()
            End If

            drOutkaEdiM.Item("CUST_ORD_NO_DTL") = String.Concat(sKey1, sKey2, drRcvEdiDtl.Item(sTempColNo).ToString().Trim())
        End If
...
#Region "画面取込(セミEDI)チェック処理"
    ...
    Private Function SemiEdiTorikomiChk(ByVal ds As DataSet) As DataSet
        ...
        'ADD 2017/02/08 Start 
        ' オーダー番号検索用データセット
        Dim selectLoadedDS As DataSet = ds.Clone
        Dim nrsBrCd As String = drSemiInfo.Item("NRS_BR_CD").ToString
        Dim custOrderNo As String = String.Empty
            ...
            Else
                ...
                For j As Integer = iRowCnt To max
                    ...
                    If (dr.Item("FILE_NAME_RCV").ToString().Trim()).Equals(dtSemiHed.Rows(i).Item("FILE_NAME_RCV").ToString().Trim()) = True Then
                        ...
                        'ADD 2017/02/08 オーダー番号が登録済み場合はチェックしない
                        Dim sKey As String = String.Empty
                        Dim drRcvEdiDtl As DataRow = ds.Tables("LMH030_EDI_TORIKOMI_DTL").Rows(j)
                        sKey = "DEVIDE_NO_1"
                        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
                            sKey1 = drRcvEdiDtl.Item(sTempColNo).ToString().Trim()
                        End If
                        ...
                        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
                            sKey3 = drRcvEdiDtl.Item(sTempColNo).ToString().Trim()
                        End If
                        custOrderNo = String.Concat(sKey1, sKey2, sKey3)

                        If Me.ExistsLoadedCustOrdNo(selectLoadedDS, nrsBrCd, custOrderNo) = False Then
...
#Region "カラム項目の値・日付チェック"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function TorikomiValChk(ByVal dr As DataRow, ByVal columnDt As DataTable _
                                   , ByVal dtMcustD As DataTable) As Boolean
        ...
        Dim sysDate As String = MyBase.GetSystemDate()      'ADD 2017/01/30
        ...
        If isNullColKey(columnDr, sKey, sTempColNo) <> False Then
            ...
            If Not (sDate.Length <= 10 AndAlso sDate.IndexOf("/"c) > 0) Then
                ...
                If sDate.Length <> 8 Then
                    ...
                    'ADD 2017/01/30 過去日付をエラーにする
                ElseIf sDate.ToString < sysDate.ToString Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat(sMsg, sDate, "](過去日付)")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
...
#If True Then ' 20161018 added inoue

        ' 取込済オーダー番号リスト
        Dim loadedCustOrderNoList As New List(Of String)()
...
#If True Then ' 20161018 added inoue
            ' [丸和特殊仕様] 同一のオーダー番号が取込済(進捗区分:EDI以上)の場合、対象行から除外する(取消は使用しない)
            If (loadedCustOrderNoList.Contains(sNewKey)) Then

                ' 明細の親が除外された場合、子も除外する
                sOldKey = sNewKey
                Continue For

            ElseIf (bSameKeyFlg = False AndAlso
                    Me.ExistsLoadedCustOrdNo(selectLoadedDS, nrsBrCd, sNewKey)) Then

                ' 親のオーダー番号が存在する場合除外する。

                loadedCustOrderNoList.Add(sNewKey)
...
#End If
    ...
    ''' 取込済オーダー番号が存在するか判別する
    ''' </summary>
    ''' <param name="cloneDS"></param>
    ''' <param name="nrsBrCd">営業所コード</param>
    ''' <param name="custOrdNo">オーダー番号</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function ExistsLoadedCustOrdNo(ByVal cloneDS As DataSet _
                                        , ByVal nrsBrCd As String _
                                        , ByVal custOrdNo As String) As Boolean
        cloneDS.Clear()

        Dim loadedRow As DataRow = cloneDS.Tables("LMH030INOUT").NewRow()
        loadedRow.Item("NRS_BR_CD") = nrsBrCd
        loadedRow.Item("CUST_ORD_NO") = custOrdNo
        loadedRow.Item("CUST_CD_L") = _CustCdL
        loadedRow.Item("CUST_CD_M") = _CustCdM
        cloneDS.Tables("LMH030INOUT").Rows.Add(loadedRow)

        cloneDS = MyBase.CallDAC(Me._Dac, "SelectLoadedData", cloneDS)

        Return (MyBase.GetResultCount > 0)
...
#Region "ユーティリティ"
    ...
    ''' 文字列結合
    ''' </summary>
    ''' <param name="inStr1">文字列1</param>
    ''' <param name="intLimitByte">最大文字数</param>
    ''' <returns>String</returns>
    ''' <remarks>
    '''          1) 引き渡された文字列1
    '''          2) 返却文字列が 最大文字数 を超える場合、最大文字数までを返却する。
    ''' </remarks>
    Private Function SetString(ByVal inStr1 As String, ByVal intLimitByte As Integer) As String
        Dim sWk As String = String.Empty
        Dim validLen As Integer = 0

        SetString = inStr1
        'バイト数sWk
        validLen = System.Text.Encoding.GetEncoding(932).GetByteCount(SetString)

        ' 最大文字数 を超える場合、最大文字数までの値を取得
        If validLen > intLimitByte Then

            'If combStringWk.Length > intLimitByte Then

            For i As Integer = 1 To inStr1.Length
                sWk = Mid(inStr1, 1, i)
                validLen = System.Text.Encoding.GetEncoding(932).GetByteCount(sWk)
