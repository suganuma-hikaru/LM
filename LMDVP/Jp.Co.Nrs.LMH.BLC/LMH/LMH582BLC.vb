...
'  システム名       :  LM
'  サブシステム名   :  LMH       : EDI
'  プログラムID     :  LMH582    : EDI納品書(埼玉 BP タクティー)
'  作  成  者       :  渡部　剛
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH582DAC = New LMH582DAC()
...
#Region "Const"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_IN As String = "LMH582IN"

    ''' <summary>
    ''' OUTテーブル名
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_OUT As String = "LMH582OUT"
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = LMH582BLC.TABLE_NM_IN
        Dim tableNmOut As String = LMH582BLC.TABLE_NM_OUT
        Dim tableNmRpt As String = LMH582BLC.TABLE_NM_M_RPT
        ...
        workDtRpt = view.ToTable(True, LMH582BLC.NRS_BR_CD, LMH582BLC.PTN_ID, LMH582BLC.PTN_CD, LMH582BLC.RPT_ID)
        ...
        For Each dr As DataRow In ds.Tables(tableNmRpt).Rows
            ...
            'prtDs = comPrt.CallDataSet(ds.Tables(tableNmOut), dr.Item(LMH582BLC.RPT_ID).ToString())
            prtDs = comPrt.CallDataSet(dtOut, dr.Item(LMH582BLC.RPT_ID).ToString())
            '(2012.06.11) 要望番号1102 並び換えしたDateTableを設定 ---  END  ---

            '帳票ごとの編集があるなら行う。
            prtDs = Me.EditPrintDataSet(dr.Item(LMH582BLC.RPT_ID).ToString(), prtDs)

            '帳票CSV出力
            comPrt.StartPrint(Me, _
                              dr.Item(LMH582BLC.NRS_BR_CD).ToString(), _
                              dr.Item(LMH582BLC.PTN_ID).ToString(), _
                              dr.Item(LMH582BLC.PTN_CD).ToString(), _
                              dr.Item(LMH582BLC.RPT_ID).ToString(), _
    ...
    Private Function EditPrintDataSet(ByVal rptId As String, ByVal ds As DataSet) As DataSet
        ...
        '要望管理1983 Start
        Dim outDt As DataTable = ds.Tables("LMH582OUT") 'データ取得
    ...
    Private Function SelectMPrt(ByVal ds As DataSet, ByVal intbl As DataTable) As DataSet
        ...
        ds = MyBase.CallDAC(Me._Dac, LMH582BLC.ACTION_ID_SELECT_MPRT, ds)
        ...
        If count < 1 Then
            ...
            Else
                If outType.Equals(ALL_PRINT) = True Then
                    ...
                    MyBase.SetMessageStore("00", "G053", New String() {"　【EDI納品書】　"}, rowNo, "帳票名", "EDI納品書 BPﾀｸﾃｨｰ")
                End If
            End If
        End If
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        Return MyBase.CallDAC(Me._Dac, LMH582BLC.ACTION_ID_SELECT_PRT_DATA, ds)
