...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC119 = New LMH030DAC119()
...
#Else
                    ...
                    ds = Me._Blc.SetComWarningM("W281", LMH030BLC.NKS_WID_M001, ds, setDs, msgArray, custGoodsCd, String.Empty)
#End If
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        Else
            '名称からDEST_CDを取得 YAMAMOTO 2017/8/9
            drOutkaEdiL("EDI_DEST_CD") = setDs.Tables("LMH030_M_DEST").Rows(0).Item("DEST_CD")
        End If
        ...
        Else
            '名称からDEST_CDを取得 YAMAMOTO 2017/8/9
            drOutkaEdiL("DEST_CD") = setDs.Tables("LMH030_M_DEST").Rows(0).Item("DEST_CD")
        End If
        ...
        End If
        '
        '2017.08.10 エアウォーターゾル Start
        'オーダー番号にEDPを付加
        'If setDs.Tables("LMH030_M_CUST_DETAILS").Select("SUB_KB='0Z'").Count > 0 Then
        sKey = "M_FREE_C26_NO"
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            drOutkaEdiL("CUST_ORD_NO") = drOutkaEdiL("CUST_ORD_NO").ToString().Trim() & drRcvEdiDtl(sTempColNo).ToString().Trim()
        End If
        'End If
        '2017.08.10 エアウォーターゾル END
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            ...
            If drOutkaEdiM.Item("ALCTD_KB").Equals("01") Then
                ...
                If drSemiEdiInfo.Item("DTL_OUTKAPKGNB_CALC_FLG").ToString().Equals(LMConst.FLG.ON) Then
                    If drMgoods Is Nothing OrElse String.IsNullOrEmpty(drMgoods.Item("PKG_NB").ToString()) = True Then
        ...
        drOutkaEdiM("FREE_C04") = drRcvEdiDtl.Item("COLUMN_12").ToString().Trim()   '数量１
        drOutkaEdiM("FREE_C05") = drRcvEdiDtl.Item("COLUMN_13").ToString().Trim()   '単位１
        drOutkaEdiM("FREE_C06") = drRcvEdiDtl.Item("COLUMN_14").ToString().Trim()   'ＰＬ数
        drOutkaEdiM("FREE_C07") = drRcvEdiDtl.Item("COLUMN_17").ToString().Trim()   '便名
        drOutkaEdiM("FREE_C08") = drRcvEdiDtl.Item("COLUMN_18").ToString().Trim()   '出荷区分
        drOutkaEdiM("FREE_C09") = drRcvEdiDtl.Item("COLUMN_19").ToString().Trim()   '送り状No
        drOutkaEdiM("FREE_C10") = drRcvEdiDtl.Item("COLUMN_20").ToString().Trim()   'EDP
        drOutkaEdiM("FREE_C11") = drRcvEdiDtl.Item("COLUMN_21").ToString().Trim()   '出庫日	
        drOutkaEdiM("FREE_C12") = drRcvEdiDtl.Item("COLUMN_22").ToString().Trim()   '処理区分
...
#Region "セミEDI時  データセット設定(届先マスタ)"
    ...
    Private Sub SetInDataSelectDestMst(ByRef setDs As DataSet, ByVal rowCnt As Integer)
        ...
        'setDr.Item("DEST_NM") = drSemiInfo.Item("DEST_NM").ToString()

        'カラム指定INデータ
        '荷主商品コード
        sKey = "L_DEST_NM_NO"
        If isNullColKey(drSemiInfo, sKey, sTempColNo) <> False Then
            setDr.Item("DEST_NM") = drEdiDtl.Item(sTempColNo).ToString().Trim()
        End If
...
#Region "カラム項目の値の先頭" '"を取る"
    ...
    Public Sub ApostropheCutEdit(ByVal dr As DataRow)
        ...
        '届け先名１
        dr.Item("COLUMN_1") = ApostropheCut(dr.Item("COLUMN_1").ToString().Trim())

        '届け先名２
        dr.Item("COLUMN_2") = ApostropheCut(dr.Item("COLUMN_2").ToString().Trim())

        '住所１
        dr.Item("COLUMN_3") = ApostropheCut(dr.Item("COLUMN_3").ToString().Trim())

        '住所２
        dr.Item("COLUMN_4") = ApostropheCut(dr.Item("COLUMN_4").ToString().Trim())

        '住所３
        dr.Item("COLUMN_5") = ApostropheCut(dr.Item("COLUMN_5").ToString().Trim())

        '伝票№
        dr.Item("COLUMN_6") = ApostropheCut(dr.Item("COLUMN_6").ToString().Trim())

        '品番
        dr.Item("COLUMN_7") = ApostropheCut(dr.Item("COLUMN_7").ToString().Trim())

        '品名
        dr.Item("COLUMN_8") = ApostropheCut(dr.Item("COLUMN_8").ToString().Trim())

        'ＬＯＴ１
        dr.Item("COLUMN_9") = ApostropheCut(dr.Item("COLUMN_9").ToString().Trim())

        'ＬＯＴ２
        dr.Item("COLUMN_10") = ApostropheCut(dr.Item("COLUMN_10").ToString().Trim())

        '棚番
        dr.Item("COLUMN_11") = ApostropheCut(dr.Item("COLUMN_11").ToString().Trim())

        '数量１
        'dr.Item("COLUMN_12") = ApostropheCut(dr.Item("COLUMN_12").ToString().Trim())

        '単位１
        dr.Item("COLUMN_13") = ApostropheCut(dr.Item("COLUMN_13").ToString().Trim())

        'ＰＬ数
        'dr.Item("COLUMN_14") = ApostropheCut(dr.Item("COLUMN_14").ToString().Trim())

        '数量２
        'dr.Item("COLUMN_15") = ApostropheCut(dr.Item("COLUMN_15").ToString().Trim())

        '単位２
        dr.Item("COLUMN_16") = ApostropheCut(dr.Item("COLUMN_16").ToString().Trim())

        '便名
        dr.Item("COLUMN_17") = ApostropheCut(dr.Item("COLUMN_17").ToString().Trim())

        '出荷区分
        dr.Item("COLUMN_18") = ApostropheCut(dr.Item("COLUMN_18").ToString().Trim())

        '送り状№
        dr.Item("COLUMN_19") = ApostropheCut(dr.Item("COLUMN_19").ToString().Trim())

        'ＥＤＰ－№
        dr.Item("COLUMN_20") = ApostropheCut(dr.Item("COLUMN_20").ToString().Trim())

        '出庫日
        dr.Item("COLUMN_21") = ApostropheCut(dr.Item("COLUMN_21").ToString().Trim())

        '処理区分
        dr.Item("COLUMN_22") = ApostropheCut(dr.Item("COLUMN_22").ToString().Trim())

        '納品日
        dr.Item("COLUMN_23") = ApostropheCut(dr.Item("COLUMN_23").ToString().Trim())
