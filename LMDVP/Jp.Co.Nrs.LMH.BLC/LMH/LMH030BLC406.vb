...
'  EDI荷主ID　　　　:  135　　　        : ローム(横浜)(2018/10/16(REV.17318)のセミEDI標準仕様000からコピー)
'                                         DPP(横浜)　ADD 2020/07/27 014220  【LMS】DDPスペシャリティ_セミEDI取込機能追
'  作  成  者       :  kido 
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC406 = New LMH030DAC406()
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        If iFindOutkaEdiFlg = 1 Then
            drOutkaEdiL("DEL_KB") = "1"     'MOD 2018/11/07 要望番号002046 設定値"3"→"1"
        ...
        '文字列4（出荷伝票）
        drOutkaEdiL("FREE_C04") = drRcvEdiDtl("COLUMN_256").ToString().Trim()   'MOD 2018/11/07 要望番号002046 値を設定するよう変更
...
#If False Then   'ADD 2019/12/24 【LMS】フィルメEDI出荷登録_数量10倍誤出荷理由調査
...
#End If
        ...
        '削除区分
        If iFindOutkaEdiFlg = 1 Then
            drOutkaEdiM.Item("DEL_KB") = "1"    'MOD 2018/11/07 要望番号002046 設定値"3"→"1"
...
#Region "セミEDI時 データセット設定(キャンセル対象データ抽出)"
    ...
    Private Sub SetDatasetSelectCancelData(ByRef setDs As DataSet, ByVal RowNo As Integer)
        ...
        End If
        '出荷伝票
        setInOutDr.Item("FREE_C04") = drItems.Item("COLUMN_256").ToString().Trim()  'ADD 2018/11/07 要望番号002046
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            If bSameKeyFlg = False AndAlso drSemiInfo.Item("EDI_TORIKESI_FLG").ToString().Equals("1") = True Then
                ...
                If MyBase.GetResultCount > 0 Then
                    ...
                    Else
                        'チェック ①<レコード有 / OUTKA_CTL_NO有> = 出荷が既にできているため(後続データを削除扱いでINSERT)
                        iFindOutkaEdiFlg = 1
                        If rcvInsFlg.Equals("1") = True OrElse rcvInsFlg.Equals("2") = True Then
                            If String.IsNullOrEmpty(drSemiInfo.Item("RCV_NM_DTL").ToString()) = False Then
