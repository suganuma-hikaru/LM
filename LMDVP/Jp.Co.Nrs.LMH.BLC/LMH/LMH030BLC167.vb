...
''' </summary>
''' <remarks></remarks>
Public Class LMH030BLC167
    Inherits Jp.Co.Nrs.LM.Base.BLC.LMBaseBLC

#Region "CONST"

    Public Const WH_CD_TST As String = "200"                    '倉庫コード
    Public Const CUST_CD_L_TST As String = "00738"              '荷主コード（大）
    Public Const CUST_CD_M_TST As String = "00"                 '荷主コード（中）

    Public Const DEF_CTL_NO As String = "T00000000"             '管理番号初期値
    Public Const DEF_CTL_NO_CHU As String = "000"               '管理番号初期値
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC167 = New LMH030DAC167()
        ...
        If String.IsNullOrEmpty(ediGoodsNm) = False Then
            ...
            ElseIf mGoodsNm.Equals(ediGoodsNm) = False Then
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningM("W194", LMH030BLC.BYK_WID_M002, ds, setDs, msgArray,
...
#Region "届先マスタチェック(ビックケミー専用)"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        Dim mAdAll As String = String.Concat(mAd1, mAd2)
        Dim mZipJis As String = String.Empty
        ...
        Dim ediDestJisCd As String = dtEdi.Rows(0)("DEST_JIS_CD").ToString()
        Dim ediAdAll As String = String.Concat(ediAd1, ediAd2)
        ...
        Else
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                ds = Me._Blc.SetComWarningL("W159", LMH030BLC.BYK_WID_L003, ds, msgArray,
...
#Region "届先マスタ追加時チェック"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetInsMDestFromDest(ByVal ds As DataSet, ByVal rowNo As String,
                                         ByVal ediCtlNo As String, ByVal workDestCd As String,
...
#Region "データセット設定(出荷L)"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            outkaDr("BUYER_ORD_NO") = ediDr("FREE_C17")
...
#Region "データセット設定(オーダ番号連結)"
    ...
    Private Function ConcatOrdNo(ByVal dt As DataTable) As String
        ...
        For i As Integer = 0 To max
            If i = 0 Then
                orderNo = dt.Rows(i)("FREE_C08").ToString()
            ElseIf i = 1 AndAlso max = 1 Then
                orderNo = String.Concat(orderNo, ",", dt.Rows(i)("FREE_C08"))
            ElseIf i = max Then
                orderNo = String.Concat(orderNo, "-", dt.Rows(i)("FREE_C08"))
            Else
                'no Action
...
#Region "画面取込(セミEDI)"
    ...
    Private Function SemiEdiTorikomiChk(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To hedmax
            ...
            Else
                '取込ファイル名の重複チェック
                Call MyBase.CallDAC(Me._Dac, "SelectRcvCount", ds)
                If MyBase.GetResultCount > 0 Then
                    dtSemiHed.Rows(i).Item("ERR_FLG") = "1"
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E01U", New String() {"同一のファイル名は既に取込済みです。ファイル名を変更してください。"}, , LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    Continue For
                End If
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow) As Boolean
        ...
        With ""
            ...
            If (Not "1".Equals(sNum)) AndAlso (Not "2".Equals(sNum)) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat(sMsg, "[", sNum, "]")}, sRecNo)
                bRet = False
            End If
        End With
        ...
        With ""
            sMsg = "出荷依頼日(カラム2番目)"
            sNum = dr.Item("COLUMN_2").ToString().Trim()
            If LenB(sNum) > 8 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                bRet = False
            End If
        End With
        ...
        With ""
            sMsg = "出荷指示No(カラム3番目)"
            sNum = dr.Item("COLUMN_3").ToString().Trim()
            If LenB(sNum) > 3 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                bRet = False
            End If
        ...
        With ""
            sMsg = "出荷依頼書発行理由(カラム4番目)"
            sNum = dr.Item("COLUMN_4").ToString().Trim()
            If String.IsNullOrEmpty(sNum) Then
                sNum = "0"
            End If
            If Not {"0", "1", "9"}.Contains(sNum) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat(sMsg, "[", sNum, "]")}, sRecNo)
                bRet = False
            End If
            If {"1", "9"}.Contains(sNum) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E02Z", New String() {}, sRecNo)
                bRet = False
            End If
        End With
        ...
        With ""
            sMsg = "出荷日(カラム5番目)"
            sDate = dr.Item("COLUMN_5").ToString().Trim()
            If String.IsNullOrEmpty(sDate) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {sMsg}, sRecNo)
                bRet = False
            Else
                If Not IsDate(Me._Blc.GetSlashEditDate(sDate)) Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat(sMsg, "[", sDate, "]")}, sRecNo)
                    bRet = False
                End If
            End If
        End With
        ...
        With ""
            sMsg = "得意先CODE(カラム7番目)"
            sNum = dr.Item("COLUMN_7").ToString().Trim()
            If LenB(sNum) > 6 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                bRet = False
            End If
        ...
        With ""
            sMsg = "得意先名(カラム8番目)"
            sNum = dr.Item("COLUMN_8").ToString().Trim()
            If LenB(sNum) > 40 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                bRet = False
            End If
        End With
        ...
        With ""
            sMsg = "納入先CODE(カラム9番目)"
            sNum = dr.Item("COLUMN_9").ToString().Trim()
            If String.IsNullOrEmpty(sNum) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {sMsg}, sRecNo)
                bRet = False
            End If
        ...
        With ""
            sMsg = "納入先名２(カラム11番目)"
            sNum = dr.Item("COLUMN_11").ToString().Trim()
            If LenB(sNum) > 32 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                bRet = False
            End If
        ...
        With ""
            sMsg = "納入先名３(カラム12番目)"
            sNum = dr.Item("COLUMN_12").ToString().Trim()
            If LenB(sNum) > 32 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                bRet = False
            End If
        ...
        With ""
            sMsg = "商品CODE(カラム13番目)"
            sNum = dr.Item("COLUMN_13").ToString().Trim()
            If LenB(sNum) > 8 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                bRet = False
            End If
        ...
        With ""
            sMsg = "ロット(カラム14番目)"
            sNum = dr.Item("COLUMN_14").ToString().Trim()
            If LenB(sNum) > 20 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                bRet = False
            End If
        End With
        ...
        With ""
            ...
            If String.IsNullOrEmpty(sNum) Then
                dr.Item("COLUMN_18") = 0
            ElseIf IsNumeric(sNum) Then
                dNum = Convert.ToDouble(sNum)
                If dNum < 0 OrElse dNum > 999999.99 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                    bRet = False
                End If
            Else
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E005", New String() {String.Concat(sMsg, "[", sNum, "]")}, sRecNo)
                bRet = False
            End If
        End With
        ...
        With ""
            ...
            ElseIf IsNumeric(sNum) Then
                dNum = Convert.ToDouble(sNum)
                If dNum < 0 OrElse dNum > 99999 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                    bRet = False
                End If
            Else
        ...
        With ""
            ...
            ElseIf IsNumeric(sNum) Then
                dNum = Convert.ToDouble(sNum)
                If dNum < 0 OrElse dNum > 999999999.99 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                    bRet = False
                End If
            Else
        ...
        With ""
            sMsg = "注文番号(カラム21番目)"
            sNum = dr.Item("COLUMN_21").ToString().Trim()
            If LenB(sNum) > 20 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                bRet = False
            End If
        ...
        With ""
            sMsg = "出荷依頼書メモ(カラム22番目)"
            sNum = dr.Item("COLUMN_22").ToString().Trim()
            If LenB(sNum) > 50 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                bRet = False
            End If
        End With
        ...
        With ""
            sMsg = "納期メモ(カラム23番目)"
            sNum = dr.Item("COLUMN_23").ToString().Trim()
            If LenB(sNum) > 20 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                bRet = False
            End If
        ...
        With ""
            sMsg = "受注データNo(カラム24番目)"
            sNum = dr.Item("COLUMN_24").ToString().Trim()
            If LenB(sNum) > 10 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, "[", sNum, "]"), ""}, sRecNo)
                bRet = False
            End If
        End With
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH030_H_OUTKAEDI_DTL_TST")      'EDI受信Dtl
        ...
        Dim sNrsGoodsCd As String = String.Empty    'NRS商品コード 
        Dim sNrsGoodsNm As String = String.Empty    'NRS商品名     
        Dim sIrime As String = String.Empty         '入目           
        Dim sEdiCtlNo As String = String.Empty      'EDI管理番号(大)
        ...
        For i As Integer = 0 To dtSetDtl.Rows.Count - 1
            ...
            Dim drEdiRcvDtl As DataRow = dtRcvDtl.Rows(0)

            '---------------------------------------------------------------------------
            ' 倉庫,荷主の設定
            '---------------------------------------------------------------------------
            sWhcd = WH_CD_TST
            sCustCdL = CUST_CD_L_TST
            sCustCdM = CUST_CD_M_TST

            '---------------------------------------------------------------------------
            ' 受信EDIデータから商品の情報を取得
            '---------------------------------------------------------------------------
            sNrsGoodsCd = Right(drEdiRcvDtl.Item("CUST_GOODS_CD").ToString, 6)
            sNrsGoodsNm = drEdiRcvDtl.Item("GOODS_NM").ToString
            sIrime = drEdiRcvDtl.Item("IRIME").ToString

            '---------------------------------------------------------------------------
            ' EDI管理番号(大,中)の採番
            '---------------------------------------------------------------------------
            ds = Me.GetEdiCtlNo(ds, sEdiCtlNo, iEdiCtlNoChu)
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH030_H_OUTKAEDI_DTL_TST")
            setDtlDt.Clear()
            ...
            ' EDI出荷データ(中)の追加
            '---------------------------------------------------------------------------
            '受信DTL⇒EDI出荷(中)へのデータセット(先に取得しておいた商品情報も渡す)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function GetEdiCtlNo(ByVal ds As DataSet, ByRef sEdiCtlNo As String, ByRef iEdiCtlNoChu As Integer) As DataSet

        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH030_H_OUTKAEDI_DTL_TST")
        Dim sNrsBrCd As String = ds.Tables("LMH030_H_OUTKAEDI_DTL_TST").Rows(0).Item("NRS_BR_CD").ToString()
        ...
        '登録用EDI管理番号
        dtRcvEdiDtl.Rows(0).Item("EDI_CTL_NO") = sEdiCtlNo
        dtRcvEdiDtl.Rows(0).Item("EDI_CTL_NO_CHU") = iEdiCtlNoChu.ToString("000")
        dtRcvEdiDtl.Rows(0).Item("GYO") = iEdiCtlNoChu.ToString("000")
    ...
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_TST").NewRow()
        ...
        drEdiRcvDtl("GYO") = String.Empty                           '後でセット 
        ...
        drEdiRcvDtl("OUTKA_CTL_NO_CHU") = DEF_CTL_NO_CHU
        drEdiRcvDtl("CUST_CD_L") = CUST_CD_L_TST
        drEdiRcvDtl("CUST_CD_M") = CUST_CD_M_TST
        drEdiRcvDtl("PRTFLG") = "0"
        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "0"

        '荷主固有項目
        drEdiRcvDtl("TETSUTANI_BR_CD") = drSetDtl("COLUMN_1").ToString()
        drEdiRcvDtl("TETSUTANI_CRT_DATE") = drSetDtl("COLUMN_2").ToString()
        drEdiRcvDtl("SER_NO") = drSetDtl("COLUMN_3").ToString()
        drEdiRcvDtl("ISSUE_REASON") = drSetDtl("COLUMN_4").ToString()
        drEdiRcvDtl("OUTKA_PLAN_DATE") = drSetDtl("COLUMN_5").ToString()
        drEdiRcvDtl("ARR_PLAN_DATE") = drSetDtl("COLUMN_6").ToString()
        drEdiRcvDtl("TOKUI_CD") = drSetDtl("COLUMN_7").ToString()
        drEdiRcvDtl("TOKUI_NM") = drSetDtl("COLUMN_8").ToString()
        drEdiRcvDtl("DEST_CD") = drSetDtl("COLUMN_9").ToString()
        drEdiRcvDtl("DEST_NM1") = drSetDtl("COLUMN_10").ToString()
        drEdiRcvDtl("DEST_NM2") = drSetDtl("COLUMN_11").ToString()
        drEdiRcvDtl("DEST_NM3") = drSetDtl("COLUMN_12").ToString()
        drEdiRcvDtl("TETSUTANI_GOODS_CD") = drSetDtl("COLUMN_13").ToString()
        drEdiRcvDtl("LOT_NO") = drSetDtl("COLUMN_14").ToString()
        drEdiRcvDtl("SUB_LOT_NO") = drSetDtl("COLUMN_15").ToString()
        drEdiRcvDtl("CUST_GOODS_CD") = drSetDtl("COLUMN_16").ToString()
        drEdiRcvDtl("GOODS_NM") = drSetDtl("COLUMN_17").ToString()
        drEdiRcvDtl("IRIME") = drSetDtl("COLUMN_18").ToString()
        drEdiRcvDtl("OUTKA_NB") = drSetDtl("COLUMN_19").ToString()
        drEdiRcvDtl("OUTKA_QT") = drSetDtl("COLUMN_20").ToString()
        drEdiRcvDtl("BUYER_ORD_NO") = drSetDtl("COLUMN_21").ToString()
        drEdiRcvDtl("OUTKA_REMARK") = drSetDtl("COLUMN_22").ToString()
        drEdiRcvDtl("ARR_DATE_REMARK") = drSetDtl("COLUMN_23").ToString()
        drEdiRcvDtl("CUST_ORD_NO") = drSetDtl("COLUMN_24").ToString()
        drEdiRcvDtl("TETSUTANI_BR_CD") = drSetDtl("COLUMN_1").ToString().Trim()
        drEdiRcvDtl("TETSUTANI_CRT_DATE") = drSetDtl("COLUMN_2").ToString().Trim()
        drEdiRcvDtl("SER_NO") = drSetDtl("COLUMN_3").ToString().Trim()
        drEdiRcvDtl("ISSUE_REASON") = drSetDtl("COLUMN_4").ToString().Trim()
        drEdiRcvDtl("OUTKA_PLAN_DATE") = drSetDtl("COLUMN_5").ToString().Trim()
        drEdiRcvDtl("ARR_PLAN_DATE") = drSetDtl("COLUMN_6").ToString().Trim()
        drEdiRcvDtl("TOKUI_CD") = drSetDtl("COLUMN_7").ToString().Trim()
        drEdiRcvDtl("TOKUI_NM") = drSetDtl("COLUMN_8").ToString().Trim()
        drEdiRcvDtl("DEST_CD") = drSetDtl("COLUMN_9").ToString().Trim()
        drEdiRcvDtl("DEST_NM1") = drSetDtl("COLUMN_10").ToString().Trim()
        drEdiRcvDtl("DEST_NM2") = drSetDtl("COLUMN_11").ToString().Trim()
        drEdiRcvDtl("DEST_NM3") = drSetDtl("COLUMN_12").ToString().Trim()
        drEdiRcvDtl("TETSUTANI_GOODS_CD") = drSetDtl("COLUMN_13").ToString().Trim()
        drEdiRcvDtl("LOT_NO") = drSetDtl("COLUMN_14").ToString().Trim()
        drEdiRcvDtl("SUB_LOT_NO") = drSetDtl("COLUMN_15").ToString().Trim()
        drEdiRcvDtl("CUST_GOODS_CD") = drSetDtl("COLUMN_16").ToString().Trim()
        drEdiRcvDtl("GOODS_NM") = drSetDtl("COLUMN_17").ToString().Trim()
        drEdiRcvDtl("IRIME") = drSetDtl("COLUMN_18").ToString().Trim()
        drEdiRcvDtl("OUTKA_NB") = drSetDtl("COLUMN_19").ToString().Trim()
        drEdiRcvDtl("OUTKA_QT") = drSetDtl("COLUMN_20").ToString().Trim()
        drEdiRcvDtl("BUYER_ORD_NO") = drSetDtl("COLUMN_21").ToString().Trim()
        drEdiRcvDtl("OUTKA_REMARK") = drSetDtl("COLUMN_22").ToString().Trim()
        drEdiRcvDtl("ARR_DATE_REMARK") = drSetDtl("COLUMN_23").ToString().Trim()
        drEdiRcvDtl("CUST_ORD_NO") = drSetDtl("COLUMN_24").ToString().Trim()

        'データセットに設定
        ds.Tables("LMH030_H_OUTKAEDI_DTL_TST").Rows.Add(drEdiRcvDtl)
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH030_H_OUTKAEDI_DTL_TST").Rows(0)
        Dim drSemiEdiInfo As DataRow = setDs.Tables("LMH030_SEMIEDI_INFO").Rows(0)

        drOutkaEdiL("DEL_KB") = drRcvEdiDtl("DEL_KB")
        ...
        drOutkaEdiL("OUTKA_KB") = "10"              '通常 
        drOutkaEdiL("SYUBETU_KB") = "10"            '通常
        drOutkaEdiL("NAIGAI_KB") = String.Empty
        drOutkaEdiL("OUTKA_STATE_KB") = "10"        '予定入力済
        drOutkaEdiL("OUTKAHOKOKU_YN") = "1"         '有
        drOutkaEdiL("PICK_KB") = "01"               'まとめ無し
        ...
        drOutkaEdiL("TOUKI_HOKAN_YN") = "1"         '有
        ...
        drOutkaEdiL("EDI_DEST_CD") = drRcvEdiDtl("DEST_CD").ToString
        drOutkaEdiL("DEST_CD") = drRcvEdiDtl("DEST_CD").ToString
        drOutkaEdiL("DEST_NM") = drRcvEdiDtl("DEST_NM1").ToString
        drOutkaEdiL("DEST_AD_3") = Me._Blc.LeftB(String.Concat(drRcvEdiDtl("DEST_NM2").ToString, Space(1),
                                                               drRcvEdiDtl("DEST_NM3").ToString).Trim, 60)
        ...
        If setDs.Tables("LMH030_M_DEST").Rows.Count = 1 Then
            ...
            If drDest("MST_CNT").ToString = "1" Then
                '取得できた届先が1件の場合のみ（住所3は取込ファイル優先なので持ってこない）
        ...
        drOutkaEdiL("CUST_ORD_NO") = drRcvEdiDtl("CUST_ORD_NO")                     '⇒オーダー番号
        drOutkaEdiL("BUYER_ORD_NO") = drRcvEdiDtl("BUYER_ORD_NO")                   '⇒注文番号
        drOutkaEdiL("UNSO_MOTO_KB") = "10"          '自社手配
        ...
        drOutkaEdiL("REMARK") = drRcvEdiDtl("OUTKA_REMARK")                         '⇒出荷時注意事項
        drOutkaEdiL("UNSO_ATT") = drRcvEdiDtl("ARR_DATE_REMARK")                    '⇒配送時注意事項
        ...
        drOutkaEdiL("OUT_FLAG") = "0"               '未書込
        drOutkaEdiL("AKAKURO_KB") = "0"             '黒
        drOutkaEdiL("JISSEKI_FLAG") = "0"           '未出力
        ...
        drOutkaEdiL("FREE_C01") = drRcvEdiDtl("TETSUTANI_BR_CD")
        drOutkaEdiL("FREE_C02") = drRcvEdiDtl("TETSUTANI_CRT_DATE")
        drOutkaEdiL("FREE_C03") = drRcvEdiDtl("SER_NO")
        drOutkaEdiL("FREE_C04") = String.Empty
        drOutkaEdiL("FREE_C05") = drRcvEdiDtl("OUTKA_PLAN_DATE")
        drOutkaEdiL("FREE_C06") = drRcvEdiDtl("ARR_PLAN_DATE")
        drOutkaEdiL("FREE_C07") = drRcvEdiDtl("TOKUI_CD")
        drOutkaEdiL("FREE_C08") = drRcvEdiDtl("TOKUI_NM")
        drOutkaEdiL("FREE_C09") = drRcvEdiDtl("DEST_CD")
        drOutkaEdiL("FREE_C10") = drRcvEdiDtl("DEST_NM1")
        drOutkaEdiL("FREE_C11") = drRcvEdiDtl("DEST_NM2")
        drOutkaEdiL("FREE_C12") = String.Empty
        drOutkaEdiL("FREE_C13") = drRcvEdiDtl("OUTKA_REMARK")
        drOutkaEdiL("FREE_C14") = drRcvEdiDtl("ARR_DATE_REMARK")
        drOutkaEdiL("FREE_C15") = drRcvEdiDtl("DEST_NM3")
        drOutkaEdiL("FREE_C16") = String.Empty
        drOutkaEdiL("FREE_C17") = drRcvEdiDtl("BUYER_ORD_NO")
        drOutkaEdiL("FREE_C18") = drRcvEdiDtl("CUST_ORD_NO")
    ...
    ''' <summary>
    ''' 画面取込(セミEDI) データセット設定(EDI出荷(中)
    ''' </summary>
    ''' <param name="setDs"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet) As DataSet
        ...
        drOutkaEdiM("CUST_ORD_NO_DTL") = drRcvEdiDtl("CUST_ORD_NO").ToString        '⇒オーダー番号
        drOutkaEdiM("BUYER_ORD_NO_DTL") = drRcvEdiDtl("BUYER_ORD_NO").ToString      '⇒注文番号
        drOutkaEdiM("CUST_GOODS_CD") = drRcvEdiDtl("CUST_GOODS_CD").ToString        '⇒商品コード
        drOutkaEdiM("NRS_GOODS_CD") = String.Empty
        drOutkaEdiM("GOODS_NM") = drRcvEdiDtl("GOODS_NM")                           '⇒商品名
        drOutkaEdiM("RSV_NO") = String.Empty
        drOutkaEdiM("LOT_NO") = Me._Blc.LeftB(drRcvEdiDtl("LOT_NO").ToString, 10)   '⇒ロットNo
        drOutkaEdiM("SERIAL_NO") = String.Empty
        drOutkaEdiM("ALCTD_KB") = "01"              '個数引当
        drOutkaEdiM("OUTKA_PKG_NB") = drRcvEdiDtl("OUTKA_NB")                       '⇒梱数（流用元030BLC020では1固定だが、今回はちゃんとセットするよう変更）
        drOutkaEdiM("OUTKA_HASU") = 0                                               '⇒端数
        drOutkaEdiM("OUTKA_QT") = drRcvEdiDtl("OUTKA_QT")                           '⇒数量
        drOutkaEdiM("OUTKA_TTL_NB") = drRcvEdiDtl("OUTKA_NB")                       '⇒個数
        drOutkaEdiM("OUTKA_TTL_QT") = drRcvEdiDtl("OUTKA_QT")                       '⇒総数量
        drOutkaEdiM("IRIME") = drRcvEdiDtl("IRIME")                                 '⇒入目
        drOutkaEdiM("REMARK") = String.Empty
        drOutkaEdiM("OUT_KB") = "0"                 '対象
        drOutkaEdiM("AKAKURO_KB") = "0"             '黒
        drOutkaEdiM("JISSEKI_FLAG") = "0"           '未出力
        ...
        drOutkaEdiM("FREE_N01") = drRcvEdiDtl("IRIME")
        ...
        drOutkaEdiM("FREE_C02") = drRcvEdiDtl("CUST_GOODS_CD")
        drOutkaEdiM("FREE_C03") = String.Empty
        drOutkaEdiM("FREE_C04") = drRcvEdiDtl("LOT_NO")
        drOutkaEdiM("FREE_C05") = drRcvEdiDtl("SUB_LOT_NO")
        drOutkaEdiM("FREE_C06") = drRcvEdiDtl("GOODS_NM")
        drOutkaEdiM("FREE_C07") = drRcvEdiDtl("BUYER_ORD_NO")
        drOutkaEdiM("FREE_C08") = drRcvEdiDtl("CUST_ORD_NO")
        drOutkaEdiM("FREE_C09") = String.Empty
        drOutkaEdiM("FREE_C10") = drRcvEdiDtl("SER_NO")
