...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC111 = New LMH030DAC111()
...
#Region "ジェイティ物流用CONST"
    ...
    ''' <remarks></remarks>
#If False Then
    Public Const WH_CD_JT As String = "101"                '倉庫コード
    Public Const CUST_CD_L_JT As String = "00444"          '荷主コード（大）
    Public Const CUST_CD_M_JT As String = "00"             '荷主コード（中）
#End If
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
            ...
            ElseIf MyBase.GetResultCount > 1 Then
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.JT_WID_L001, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningL("W199", LMH030BLC.JT_WID_L001, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("0") = True Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.JT_WID_L002, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = String.Empty
                    matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                    ds = Me._Blc.SetComWarningL("W168", LMH030BLC.JT_WID_L002, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("1") = True Then
                ...
                If matomeStatus.Equals("10") = False Then
                    ...
                    choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.JT_WID_L003, 0)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(4) = String.Empty
                        matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                        ds = Me._Blc.SetComWarningL("W198", LMH030BLC.JT_WID_L003, ds, msgArray, matomeNo, String.Empty)
                        Return ds
...
#Region "EDI_Lの初期値設定(出荷登録処理)"
    ...
    Private Function SetEdiLShoki(ByVal ds As DataSet) As DataSet
        ...
        '要望番号:1582 terakawa 2012.11.14 Start
        'ジェイティ物流特殊処理（届先名、住所、電話番号を届け先マスタからセット）
        If mDestFlgYN = True Then
            '届先名
        ...
        End If
        '要望番号:1582 terakawa 2012.11.14 End
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.JT_WID_M001, 0)
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    Else
                        ...
                        '2012.03.19 修正START
                        ds = Me._Blc.SetComWarningM("W162", LMH030BLC.JT_WID_M001, ds, setDs, msgArray, custGoodsCd, String.Empty)
                        '2012.03.19 修正END
                ...
                If Me.EdiMDefaultSet(ds, setDs, i, unsoData, rowNo, ediCtlNo) = False Then
                    ...
                    'ジェイティ物流は現段階ではワーニングが存在するが、エラーはなし。共通のロジックを組み込む為入れておく
                    Dim chkRowNo As Integer = Convert.ToInt32(rowNo)
                    If MyBase.IsMessageStoreExist(chkRowNo) = True Then
                        '整合性チェックでエラーがあった場合は処理終了
                        Return False
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        'Dim drEdiRcvHed As DataRow = ds.Tables("LMH030_OUTKAEDI_HED_JT").NewRow()
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_OUTKAEDI_DTL_JT").NewRow()
        ...
        'drEdiRcvDtl("CUST_CD_L") = CUST_CD_L_JT                                                        '荷主コード（大）
        'drEdiRcvDtl("CUST_CD_M") = CUST_CD_M_JT                                                        '荷主コード（中）
        drEdiRcvDtl("PRTFLG") = "0"                                                                     'プリントフラグ
        drEdiRcvDtl("CANCEL_FLG") = "0"                                                                 'キャンセルフラグ
        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "1"                                                          '実績処理フラグ

        '荷主固有データ
        drEdiRcvDtl("SAKUSEI_BI") = Me._Blc.LeftB(drSetDtl("COLUMN_1").ToString().Trim(), 12)           '作成日
        drEdiRcvDtl("HASSO_BI") = Me._Blc.LeftB(drSetDtl("COLUMN_2").ToString().Trim(), 6)              '発送日
        drEdiRcvDtl("CUST_ORD_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 300)         '発送番号
        drEdiRcvDtl("HIBETSU_DAISU") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 1)         '日別台数
        drEdiRcvDtl("OUTKA_PLAN_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 8)       '発送予定日
        drEdiRcvDtl("ARR_PLAN_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 8)         '納入予定日
        drEdiRcvDtl("HASSO_MOTO") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 4)            '発送元
        drEdiRcvDtl("DEST_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 15)              '納入先
        drEdiRcvDtl("KOJO_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 40)              '工場名称
        drEdiRcvDtl("YUSO_KT") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 40)             '輸送形態
        drEdiRcvDtl("ZAIRO_KBN") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 40)           '材料区分
        drEdiRcvDtl("SUPPLIER") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 40)            'サプライヤー
        drEdiRcvDtl("CUST_GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 20)       '材料コード
        drEdiRcvDtl("ZAIRYO_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_14").ToString().Trim(), 40)           '材料名称
        drEdiRcvDtl("OUTKA_TTL_QT") = Me._Blc.LeftB(drSetDtl("COLUMN_15").ToString().Trim(), 15)        '納品重量
        drEdiRcvDtl("IRIME_UT") = Me._Blc.LeftB(drSetDtl("COLUMN_16").ToString().Trim(), 2)             '出納単位
        drEdiRcvDtl("OUTKA_TTL_NB") = Me._Blc.LeftB(drSetDtl("COLUMN_17").ToString().Trim(), 8)         '梱包個数
        drEdiRcvDtl("KONPO_KT") = Me._Blc.LeftB(drSetDtl("COLUMN_18").ToString().Trim(), 40)            '梱包形態
        drEdiRcvDtl("IRIME") = Me._Blc.LeftB(drSetDtl("COLUMN_19").ToString().Trim(), 12)               'NET重量／梱包
        drEdiRcvDtl("YUSOFT_WT_KG") = Me._Blc.LeftB(drSetDtl("COLUMN_20").ToString().Trim(), 15)        '輸送重量KG（風袋込）
        drEdiRcvDtl("BETU_WT") = Me._Blc.LeftB(drSetDtl("COLUMN_21").ToString().Trim(), 15)             'GROSS重量／（梱包）
        drEdiRcvDtl("LOT_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_22").ToString().Trim(), 40)              '製造ロット
        drEdiRcvDtl("YUSO_SYSU") = Me._Blc.LeftB(drSetDtl("COLUMN_23").ToString().Trim(), 40)           '輸送車種
        drEdiRcvDtl("CNTNER_OR_NOT") = Me._Blc.LeftB(drSetDtl("COLUMN_24").ToString().Trim(), 40)       'コンテナ or Not

        'データセットに設定
        'ds.Tables("LMH030_OUTKAEDI_HED_JT").Rows.Add(drEdiRcvHed)
        ds.Tables("LMH030_OUTKAEDI_DTL_JT").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
        ...
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH030_OUTKAEDI_DTL_JT").Rows(0)
        'Dim drRcvEdiHed As DataRow = setDs.Tables("LMH030_OUTKAEDI_HED_JT").Rows(0)
        ...
        drOutkaEdiM("CUST_ORD_NO_DTL") = drRcvEdiDtl("CUST_ORD_NO").ToString
        drOutkaEdiM("BUYER_ORD_NO_DTL") = String.Empty
        drOutkaEdiM("CUST_GOODS_CD") = drRcvEdiDtl("CUST_GOODS_CD").ToString
        drOutkaEdiM("NRS_GOODS_CD") = String.Empty
        drOutkaEdiM("GOODS_NM") = String.Empty

        drOutkaEdiM("RSV_NO") = String.Empty
        drOutkaEdiM("LOT_NO") = drRcvEdiDtl("LOT_NO").ToString
        drOutkaEdiM("SERIAL_NO") = String.Empty
        drOutkaEdiM("ALCTD_KB") = "01"                              '個数引当

        drOutkaEdiM("OUTKA_PKG_NB") = 0                             '出荷包装個数
        drOutkaEdiM("OUTKA_HASU") = drRcvEdiDtl("OUTKA_TTL_NB")     '出荷端数 

        drOutkaEdiM("OUTKA_QT") = drRcvEdiDtl("OUTKA_TTL_QT")       '出荷数量
        drOutkaEdiM("OUTKA_TTL_NB") = drRcvEdiDtl("OUTKA_TTL_NB")   '出荷総個数
        drOutkaEdiM("OUTKA_TTL_QT") = drRcvEdiDtl("OUTKA_TTL_QT")   '出荷総数量
        ...
        '要望番号1552:(【セミEDI】JT物流　EXCEL取込時 入目単位を大文字に変換する) 2012/11/09 本明 Start
        'drOutkaEdiM("IRIME_UT") = drRcvEdiDtl("IRIME_UT")
        drOutkaEdiM("IRIME_UT") = UCase(drRcvEdiDtl("IRIME_UT").ToString)   '入目単位（大文字）
        '要望番号1552:(【セミEDI】JT物流　EXCEL取込時 入目単位を大文字に変換する) 2012/11/09 本明 End
        ...
        drOutkaEdiM("FREE_N01") = drRcvEdiDtl("YUSOFT_WT_KG")
        drOutkaEdiM("FREE_N02") = drRcvEdiDtl("BETU_WT")
        ...
        drOutkaEdiM("FREE_C01") = drRcvEdiDtl("ZAIRO_KBN")
        drOutkaEdiM("FREE_C02") = drRcvEdiDtl("SUPPLIER")
        drOutkaEdiM("FREE_C03") = drRcvEdiDtl("ZAIRYO_NM")
        drOutkaEdiM("FREE_C04") = drRcvEdiDtl("KONPO_KT")
        drOutkaEdiM("FREE_C05") = drRcvEdiDtl("YUSO_SYSU")
        drOutkaEdiM("FREE_C06") = drRcvEdiDtl("CNTNER_OR_NOT")
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        drOutkaEdiL("DEST_CD") = drRcvEdiDtl("DEST_CD")
        drOutkaEdiL("DEST_NM") = String.Empty
        ...
        drOutkaEdiL("CUST_ORD_NO") = drRcvEdiDtl("CUST_ORD_NO") '荷主注文番号（全体）
        drOutkaEdiL("BUYER_ORD_NO") = String.Empty              '買主注文番号（全体）

        drOutkaEdiL("UNSO_MOTO_KB") = "20"                      '運送元区分 20：引き取り
        drOutkaEdiL("UNSO_TEHAI_KB") = "10"                     '10：混載
        drOutkaEdiL("SYARYO_KB") = String.Empty
        drOutkaEdiL("BIN_KB") = "01"
        drOutkaEdiL("UNSO_CD") = "60"
        ...
        drOutkaEdiL("FREE_C01") = drRcvEdiDtl("SAKUSEI_BI")
        drOutkaEdiL("FREE_C02") = drRcvEdiDtl("HASSO_BI")
        drOutkaEdiL("FREE_C03") = drRcvEdiDtl("HIBETSU_DAISU")
        drOutkaEdiL("FREE_C04") = drRcvEdiDtl("HASSO_MOTO")
        drOutkaEdiL("FREE_C05") = drRcvEdiDtl("KOJO_NM")
        drOutkaEdiL("FREE_C06") = drRcvEdiDtl("YUSO_KT")
...
#If False Then
        drOutkaEdiL("FREE_C20") = CUST_CD_L_JT & CUST_CD_M_JT
#Else
        drOutkaEdiL("FREE_C20") = String.Concat(drSemiEdiInfo("CUST_CD_L"), drSemiEdiInfo("CUST_CD_M"))
#End If
...
#If False Then
    ...
    Private Function GetCustCd(ByVal ds As DataSet _
        ...
        Select Case iMstGoodsCnt
            ...
            Case 0      '商品マスタ取得０件
                '荷主はジェイティ物流とする
                sCustCdL = CUST_CD_L_JT
                sCustCdM = CUST_CD_M_JT
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiHed As DataTable = ds.Tables("LMH030_OUTKAEDI_HED_JT")
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH030_OUTKAEDI_DTL_JT")
        Dim drRcvEdiDtl As DataRow = ds.Tables("LMH030_OUTKAEDI_DTL_JT").Rows(0)
        Dim sNrsBrCd As String = ds.Tables("LMH030_OUTKAEDI_DTL_JT").Rows(0).Item("NRS_BR_CD").ToString()
        ...
        '    Dim dtRcvHedDel As DataTable = ds.Tables("LMH030_DTL_JT_CANCELOUT")
        '    Dim drRcvHedDel As DataRow = ds.Tables("LMH030_DTL_JT_CANCELOUT").Rows(0)

        '    Dim dtRcvDtlDel As DataTable = ds.Tables("LMH030_DTL_JT_CANCELOUT")
        '    Dim drRcvDtlDel As DataRow = ds.Tables("LMH030_DTL_JT_CANCELOUT").Rows(0)
...
#Region "画面取込(セミEDI)チェック処理"
    ...
    Private Function SemiEdiTorikomiChk(ByVal ds As DataSet) As DataSet
        ...
        Dim sFileDate As String = String.Empty          'ファイル名称の日付
        Dim sNextDate As String = String.Empty          '翌日
        Dim sNextEigyoDate As String = String.Empty     '翌営業日
        ...
        For i As Integer = 0 To hedmax
            ...
            Else
                ...
                'sFileDate = Mid(dtSemiHed.Rows(i).Item("FILE_NAME_RCV").ToString(), 11, 8)
                sFileDate = Mid(dtSemiHed.Rows(i).Item("FILE_NAME_RCV").ToString(), 1, 8)
                '要望番号1593:(【セミEDI】JT物流　取込ファイル名変更) 2012/11/14 本明 End

                sNextDate = Format(DateSerial(Convert.ToInt32(Left(sFileDate, 4)), Convert.ToInt32(Mid(sFileDate, 5, 2)), Convert.ToInt32(Right(sFileDate, 2)) + 1), "yyyyMMdd")
                sNextEigyoDate = Format(Me.GetBussinessDay(sFileDate, +1, ds), "yyyyMMdd")

                'dtSemiDtlのleft(COLUMN_1,8)をCOLUMN_26にセットする（.SelectでSUBSTRINGが使用できないので）
                For j As Integer = 0 To dtSemiDtl.Rows.Count - 1
                    dtSemiDtl.Rows(j).Item("COLUMN_26") = Left(dtSemiDtl.Rows(j).Item("COLUMN_1").ToString, 8)
                Next

                '対象データのみソートして抜き出す
                Dim drSelect As DataRow() = dtSemiDtl.Select(" COLUMN_26 =  '" & sFileDate & "' AND " &
                                                        "(COLUMN_5  >= '" & sNextDate & "' AND  COLUMN_5 <= '" & sNextEigyoDate & "' ) AND " &
                                                        " COLUMN_7  =  '日陸'",
                                                        " COLUMN_5 ASC, COLUMN_3 ASC, COLUMN_13 ASC, REC_NO ASC")   'ソート順
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        'Dim dtRcvHed As DataTable = ds.Tables("LMH030_OUTKAEDI_HED_JT")         'EDI受信Hed
        Dim dtRcvDtl As DataTable = ds.Tables("LMH030_OUTKAEDI_DTL_JT")         'EDI受信Dtl
        Dim dtRcvDtlCancel As DataTable = ds.Tables("LMH030_DTL_JT_CANCELOUT")  'EDI受信Hed
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            ds.Tables("LMH030_OUTKAEDI_DTL_JT").Clear() '受信DTLをクリア
            ds = Me.SetSemiOutkaEdiRcv(ds, i)
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_OUTKAEDI_DTL_JT").Rows(0)
...
#Else
            ...
            sCustCdL = ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("CUST_CD_L").ToString()
            sCustCdM = ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("CUST_CD_M").ToString()
#End If
            ' ''受信EDIデータから取得
            ...
            ''    '受信DTL取消データ取得処理
            ''    ds.Tables("LMH030_DTL_JT_CANCELOUT").Clear()    '取得用DSをクリア
            ...
            ''        Dim drRcvDtlCancel As DataRow = ds.Tables("LMH030_DTL_JT_CANCELOUT").Rows(0)
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH030_OUTKAEDI_DTL_JT")
            setDtlDt.Clear()
