...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC202 = New LMH010DAC202()
...
#Region "入荷登録マスタ存在チェック(中)"
    Private Function SetGoodsMst(ByRef ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            '条件の再設定
            setDtM = Me.SetGoodsCdFromWarning(setDtM, ds, LMH010BLC.DOW_WID_M001)
            ...
            ElseIf GetResultCount() > 1 Then
                ...
                Else
                    ...
                    ds = Me._ChkBlc.SetWarningM("W162", LMH010BLC.DOW_WID_M001, ds, setDs, msgArray)
            ...
            If String.IsNullOrEmpty(dtM.Rows(i)("GOODS_NM").ToString()) = False Then
                If dtM.Rows(i)("GOODS_NM").ToString().Equals(goodsDt.Rows(0)("GOODS_NM_1").ToString()) = False Then
                    choiceKb = Me._ChkBlc.GetWarningChoiceKbM(ds.Tables("LMH010_INKAEDI_M"), ds, LMH010BLC.DOW_WID_M002, i)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        ds = Me._ChkBlc.SetWarningM2("W194", LMH010BLC.DOW_WID_M002, ds, setDs, msgArray, ediName, ediValue, mustValue)
                        flgWarning = True 'ワーニングフラグをたてて処理続行
                    End If
...
#Region "データセット設定(運送L)"
    ...
    Private Function SetDatasetUnsoL(ByVal ds As DataSet) As DataSet
        ...
        unsoDr("TAX_KB") = "01"                             'ダウケミ追加箇所 20120223
