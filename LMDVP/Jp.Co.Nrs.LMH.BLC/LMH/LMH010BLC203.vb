...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC203 = New LMH010DAC203()
...
#Region "入荷登録DB存在チェック(大)"
    Private Function DbCheckL(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        If String.IsNullOrEmpty(drEdiL("OUTKA_FROM_ORD_NO").ToString()) = False Then
            '浮間追加箇所(オーダー番号重複） 20120217 start
            If drIn("ORDER_CHECK_FLG").Equals("0") = False Then
                '浮間追加箇所(オーダー番号重複） 20120217 end 
...
#Region "入荷登録マスタ存在チェック(中)"
    Private Function SetGoodsMst(ByRef ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            '条件の再設定
            setDtM = Me.SetGoodsCdFromWarning(setDtM, ds, LMH010BLC.UKM_WID_M001)
            ...
            If MyBase.GetResultCount = 0 Then
                '浮間追加箇所 20120213 start
                MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E440", New String() {"商品マスタ", "商品", _
                    String.Concat(dtM.Rows(i).Item("CUST_GOODS_CD"), "：", dtM.Rows(i).Item("GOODS_NM")), _
                    String.Concat(dtM.Rows(i).Item("IRIME"), " ", dtM.Rows(i).Item("IRIME_UT")), _
                    CStr(dtM.Rows(i).Item("FREE_C07"))}, rowNo, LMH010BLC.EXCEL_COLTITLE, ediCtlNo)
                'このレコードのワーニングをクリア
            ...
            ElseIf GetResultCount() > 1 Then
                ...
                Else
                    ...
                    ds = Me._ChkBlc.SetWarningM("W162", LMH010BLC.UKM_WID_M001, ds, setDs, msgArray)
            ...
            ElseIf GetResultCount() = 1 Then
                ...
                If Me._SetWarningFlg = False AndAlso String.IsNullOrEmpty(dtM.Rows(i)("NRS_GOODS_CD").ToString()) = True AndAlso _
                   Convert.ToDecimal(dtM.Rows(i)("IRIME")).Equals(Convert.ToDecimal(goodsDt.Rows(0)("STD_IRIME_NB"))) = False Then
                    '再検索で商品が確定できない場合はワーニング設定して、次の中レコードへ
                    ds = Me._ChkBlc.SetWarningM("W192", LMH010BLC.UKM_WID_M001, ds, setDs, msgArray)
                    flgWarning = True 'ワーニングフラグをたてて処理続行
                    Continue For
                End If
            ...
            '浮間追加箇所 20120213 start
            '追加①↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
            '類別コードチェック(EDI入荷(中)FREE_C07と商品明細マスタ(サブ区分="07")セット内容)

            If String.IsNullOrEmpty(dtM.Rows(i)("NRS_GOODS_CD").ToString()) = False Then
                '商品明細マスタの取得
                setDs = (MyBase.CallDAC(Me._Dac, "SelectDataGoodsMeisai", setDs))
                If MyBase.GetResultCount <> 0 Then
                    Dim setNaiyo As String = setDs.Tables("LMH010_M_GOODS_DETAILS").Rows(0)("SET_NAIYO").ToString()
                    '商品明細マスタと種別のデータが同一でない場合
                    If Convert.ToString(dtM.Rows(i)("FREE_C07")).Equals(setNaiyo) = False Then

                        choiceKb = Me._ChkBlc.GetWarningChoiceKbM(ds.Tables("LMH010_INKAEDI_M"), ds, LMH010BLC.UKM_WID_M002, i)
                        If String.IsNullOrEmpty(choiceKb) = True Then

                            ediName = "類別コード(FREE_C07)"
                            ediValue = dtM.Rows(i)("FREE_C07").ToString()
                            mustValue = setNaiyo
                            ...
                            ds = Me._ChkBlc.SetWarningM2("W195", LMH010BLC.UKM_WID_M002, ds, setDs, msgArray, ediName, ediValue, mustValue)
                            flgWarning = True 'ワーニングフラグをたてて処理続行
                        Else
                            '処理区分が"はい"の場合は、処理続行(ワーニング画面で選択後)
            ...
            If String.IsNullOrEmpty(dtM.Rows(i)("GOODS_NM").ToString()) = False Then
                If dtM.Rows(i)("GOODS_NM").ToString().Equals(goodsDt.Rows(0)("GOODS_NM_1").ToString()) = False Then
                    choiceKb = Me._ChkBlc.GetWarningChoiceKbM(ds.Tables("LMH010_INKAEDI_M"), ds, LMH010BLC.UKM_WID_M003, i)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        ds = Me._ChkBlc.SetWarningM2("W194", LMH010BLC.UKM_WID_M003, ds, setDs, msgArray, ediName, ediValue, mustValue)
                        flgWarning = True 'ワーニングフラグをたてて処理続行
                    Else
            ...
            '【浮間（岩槻）のみ】荷主コード（小）とFREE_C07で浮間請求分類の存在チェック
            If custIndex = LMH010BLC.EdiCustIndex.UkimaSai00856_00 Then
                If String.IsNullOrEmpty(dtM.Rows(i)("FREE_C07").ToString()) = False Then
                    '浮間請求分類の存在
                    Call MyBase.CallDAC(Me._Dac, "SelectUkimaBunruiM", setDs)
                    If MyBase.GetResultCount = 0 Then
                        choiceKb = Me._ChkBlc.GetWarningChoiceKbM(ds.Tables("LMH010_INKAEDI_M"), ds, LMH010BLC.UKM_WID_M004, i)
                        If String.IsNullOrEmpty(choiceKb) = True Then

                            ediName = "類別コード(FREE_C07)"
                            ediValue = dtM.Rows(i)("FREE_C07").ToString()
                            mustValue = String.Empty
                            msgArray(1) = "類別"
                            msgArray(2) = "浮間請求分類マスタ"
                            msgArray(3) = dtM.Rows(i)("EDI_CTL_NO").ToString
                            msgArray(4) = dtM.Rows(i)("FREE_C07").ToString
                            ds = Me._ChkBlc.SetWarningM2("W206", LMH010BLC.UKM_WID_M004, ds, setDs, msgArray, ediName, ediValue, mustValue)
                            flgWarning = True 'ワーニングフラグをたてて処理続行
                        Else
                            '処理区分が"はい"の場合は、処理続行(ワーニング画面で選択後)
...
#Region "データセット設定(入荷S)"
    ...
    Private Function SetDatasetInkaS(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            '要望番号1003 2012.05.08 修正START(FREE_C17コメント⇒商品明細マスタより取得)
            '浮間追加箇所 20120213 start
            'inkaDr("TOU_NO") = Left((ediDr("FREE_C17").ToString), 2)
            'inkaDr("SITU_NO") = Mid((ediDr("FREE_C17").ToString), 3, 1)
            'inkaDr("ZONE_CD") = Mid((ediDr("FREE_C17").ToString), 4, 1)
            '浮間追加箇所 20120213 end
            inkaDr("TOU_NO") = ediDr("TOU_NO")
            inkaDr("SITU_NO") = ediDr("SITU_NO")
            inkaDr("ZONE_CD") = ediDr("ZONE_CD")
            '要望番号1003 2012.05.08 修正END
...
#Region "実績作成処理"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        If Me.JissekiWarningSet(ds, LMH010BLC.UKM_WID_L001) = False Then
    ...
    Private Function SetSendDs(ByVal ds As DataSet) As DataSet
        ...
        Dim dr As DataRow = ds.Tables("H_SENDINEDI_UKM").Rows(0)
    ...
    Private Function ChkOrderNo(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        If MyBase.GetResultCount <> 0 Then
            ...
            msgArray(5) = String.Empty
            ds = Me._ChkBlc.SetWarningL("W181", LMH010BLC.UKM_WID_L001, ds, msgArray)
            Return False
        End If
