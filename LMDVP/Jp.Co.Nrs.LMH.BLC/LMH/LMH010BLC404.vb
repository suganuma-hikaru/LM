...
'  システム名       :  LM
'  サブシステム名   :  LMH              : EDI
'  プログラムID     :  LMH010HBLC404    : EDI入荷検索:ローム(209)から複写
'  EDI荷主ID　　　　:  108　　　        : フィルメニッヒ
'  作  成  者       :  inoue
' ==========================================================================
Imports Jp.Co.Nrs.LM.Const
Imports Jp.Co.Nrs.LM.DAC
Imports Jp.Co.Nrs.LM.DAC.LMH010DAC404
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC404 = New LMH010DAC404()
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    Private Function SetSemiInkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_FIR").NewRow()
        ...
        ' 荷主固有データ
        drEdiRcvDtl("DELIVERY_DATE") = Format(Convert.ToDateTime(drSetDtl(GetColumnName(FirmeArrivalColumns.DELIVERY_DATE))), "yyyyMMdd")
        drEdiRcvDtl("AWB_BL_NO") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.AWB_BL_NO)).ToString().Trim(), 30)
        drEdiRcvDtl("SHIPPING_INSTR") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.SHIPPING_INSTR)).ToString().Trim(), 5)
        drEdiRcvDtl("MANUFACTURER") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.MANUFACTURER)).ToString().Trim(), 20)
        drEdiRcvDtl("MATERIAL") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.MATERIAL)).ToString().Trim(), 18)
        drEdiRcvDtl("MATERIAL_DESCRIPTION") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.MATERIAL_DESCRIPTION)).ToString().Trim(), 80)
        drEdiRcvDtl("OBD_SHIPMENT_NO") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.OBD_SHIPMENT_NO)).ToString().Trim(), 50)
        drEdiRcvDtl("DELIVERY") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.DELIVERY)).ToString().Trim(), 10)
        drEdiRcvDtl("QUANTITY_IN_OPUN") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.QUANTITY_IN_OPUN)).ToString().Trim(), 13)
        drEdiRcvDtl("OPEN_QUANTITY") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.OPEN_QUANTITY)).ToString().Trim(), 13)
        drEdiRcvDtl("ORDER_UNIT") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.ORDER_UNIT)).ToString().Trim(), 6)
        drEdiRcvDtl("PURCHASING_DOCUMENT") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.PURCHASING_DOCUMENT)).ToString().Trim(), 10)
        drEdiRcvDtl("ITEM") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.ITEM)).ToString().Trim(), 6)
        drEdiRcvDtl("PO_PLUS_LINE") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.PO_PLUS_LINE)).ToString().Trim(), 20)
        drEdiRcvDtl("BATCH") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.BATCH)).ToString().Trim(), 10)
        drEdiRcvDtl("P_DATE") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.P_DATE)).ToString().Trim(), 30)
        drEdiRcvDtl("VENDOR_BATCH") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.VENDOR_BATCH)).ToString().Trim(), 30)
        drEdiRcvDtl("SHIP_TO") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.SHIP_TO)).ToString().Trim(), 30)
        drEdiRcvDtl("SO_NO") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.SO_NO)).ToString().Trim(), 30)
        drEdiRcvDtl("CUSTOMER") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.CUSTOMER)).ToString().Trim(), 30)
        drEdiRcvDtl("BIN") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.BIN)).ToString().Trim(), 30)
        drEdiRcvDtl("GR_NO") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.GR_NO)).ToString().Trim(), 20)
        drEdiRcvDtl("REMARK") = Left(drSetDtl(GetColumnName(FirmeArrivalColumns.REMARK)).ToString().Trim(), 100)

        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "1"                                                          '実績処理フラグ

        'データセットに設定
        ds.Tables("LMH010_INKAEDI_DTL_FIR").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI入荷(中))"
    ...
    Private Function SetSemiInkaEdiM(ByVal setDs As DataSet _
        ...
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH010_INKAEDI_DTL_FIR").Rows(0)
        ...
        drInkaEdiM("GOODS_NM") = drRcvEdiDtl("MATERIAL_DESCRIPTION")
...
#If True Then   ' 入目対応 20160707 added inoue

        ' 数量
        Dim quantity As Double = 0.0

        ' 総数量
        Dim totalQuantity As Double = 0.0

        ' 入目算出
        If (Double.TryParse(TryCast(drRcvEdiDtl("QUANTITY_IN_OPUN"), String), totalQuantity) AndAlso _
            Double.TryParse(TryCast(drRcvEdiDtl("OPEN_QUANTITY"), String), quantity) AndAlso _
            totalQuantity > 0 AndAlso _
            quantity > 0) Then

            ' 算出した入目を設定(有効桁小数点第三位)
            ' 単位は、商品マスターと同単位で入力されていることを前提とする。
            drInkaEdiM("IRIME") = Math.Round(totalQuantity / quantity, 3, MidpointRounding.AwayFromZero)
        End If

#End If

        drInkaEdiM("NB") = Convert.ToInt32(drRcvEdiDtl("OPEN_QUANTITY").ToString())
        drInkaEdiM("INKA_PKG_NB") = Convert.ToInt32(drRcvEdiDtl("OPEN_QUANTITY").ToString())
        drInkaEdiM("HASU") = 0
        drInkaEdiM("OUTKA_FROM_ORD_NO") = drRcvEdiDtl("PO_PLUS_LINE")
        drInkaEdiM("BUYER_ORD_NO") = String.Empty
        drInkaEdiM("REMARK") = drRcvEdiDtl("REMARK")
        ...
        drInkaEdiM("FREE_C01") = drRcvEdiDtl("AWB_BL_NO")
        drInkaEdiM("FREE_C02") = drRcvEdiDtl("SHIPPING_INSTR")
        drInkaEdiM("FREE_C03") = drRcvEdiDtl("MANUFACTURER")
        drInkaEdiM("FREE_C04") = drRcvEdiDtl("OBD_SHIPMENT_NO")
        drInkaEdiM("FREE_C05") = drRcvEdiDtl("DELIVERY")
        drInkaEdiM("FREE_C06") = drRcvEdiDtl("QUANTITY_IN_OPUN")
        drInkaEdiM("FREE_C07") = drRcvEdiDtl("OPEN_QUANTITY")
        drInkaEdiM("FREE_C08") = drRcvEdiDtl("ORDER_UNIT")
        drInkaEdiM("FREE_C09") = drRcvEdiDtl("PURCHASING_DOCUMENT")
        drInkaEdiM("FREE_C10") = drRcvEdiDtl("ITEM")
        drInkaEdiM("FREE_C11") = drRcvEdiDtl("PO_PLUS_LINE")
        drInkaEdiM("FREE_C12") = drRcvEdiDtl("BATCH")
        drInkaEdiM("FREE_C13") = drRcvEdiDtl("P_DATE")
        drInkaEdiM("FREE_C14") = drRcvEdiDtl("VENDOR_BATCH")
        drInkaEdiM("FREE_C15") = drRcvEdiDtl("SHIP_TO")
        drInkaEdiM("FREE_C16") = drRcvEdiDtl("SO_NO")
        drInkaEdiM("FREE_C17") = drRcvEdiDtl("CUSTOMER")
        drInkaEdiM("FREE_C18") = drRcvEdiDtl("BIN")
        drInkaEdiM("FREE_C19") = drRcvEdiDtl("GR_NO")
        drInkaEdiM("FREE_C20") = String.Empty
        drInkaEdiM("FREE_C21") = drRcvEdiDtl("REMARK")
        drInkaEdiM("FREE_C22") = drRcvEdiDtl("MATERIAL_DESCRIPTION")
...
#Region "セミEDI時　データセット設定(EDI入荷(大))"
    ...
    Private Function SetSemiinkaediL(ByVal setDs As DataSet) As DataSet
        ...
        drInkaEdiL("INKA_DATE") = drRcvEdiDtl("DELIVERY_DATE")
        drInkaEdiL("INKA_TIME") = "0900"                                        ' 入荷(予定)時刻
        ...
        drInkaEdiL("NAIGAI_KB") = "01"                                          ' 内外貨区分:内貨
        drInkaEdiL("BUYER_ORD_NO") = String.Empty
        drInkaEdiL("OUTKA_FROM_ORD_NO") = drRcvEdiDtl("PO_PLUS_LINE")
        ...
        drInkaEdiL("HOKAN_STR_DATE") = drRcvEdiDtl("DELIVERY_DATE")
        drInkaEdiL("NIYAKU_YN") = "1"                                           ' 荷役料有無:有
        drInkaEdiL("TAX_KB") = "01"                                             ' 税区分:課税
        drInkaEdiL("REMARK") = drRcvEdiDtl("REMARK")
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_FIR")
...
#If False Then
        sMsg = "注文番号(カラム" & CType(FirmeArrivalColumns.DELIVERY, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(FirmeArrivalColumns.DELIVERY)).ToString()
...
#End If
        ...
        sMsg = "入荷日(カラム" & CType(FirmeArrivalColumns.DELIVERY_DATE, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(FirmeArrivalColumns.DELIVERY_DATE)).ToString().Trim()
        If IsDate(sStr) = False Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E445", New String() {String.Concat(sMsg, sStr, "]")}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If


        sMsg = "商品コード(カラム" & CType(FirmeArrivalColumns.MATERIAL, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(FirmeArrivalColumns.MATERIAL)).ToString().Trim()

#If True Then   ' 商品コードから_以下を削除 20160517 changed inoue
        If (String.IsNullOrEmpty(sStr) = False AndAlso _
            sStr.Split(CChar("_")).Length > 0) Then
            sStr = sStr.Split(CChar("_"))(0)
            dr.Item(GetColumnName(FirmeArrivalColumns.MATERIAL)) = sStr
        End If
#End If
        ...
        sMsg = "商品名(カラム" & CType(FirmeArrivalColumns.MATERIAL_DESCRIPTION, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(FirmeArrivalColumns.MATERIAL_DESCRIPTION)).ToString().Trim()
        If sStr.Length > 80 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E445", New String() {String.Concat(sMsg, sStr, "]")}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        sMsg = "入荷個数量(カラム" & CType(FirmeArrivalColumns.OPEN_QUANTITY, Int32) & "番目)["
        sNum = dr.Item(GetColumnName(FirmeArrivalColumns.OPEN_QUANTITY)).ToString().Trim()
        If String.IsNullOrEmpty(sNum) = True Then
            '空の場合はゼロをセット
            dr.Item(GetColumnName(FirmeArrivalColumns.OPEN_QUANTITY)) = 0
        Else
        ...
        sMsg = "ロット№(カラム" & CType(FirmeArrivalColumns.OPEN_QUANTITY, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(FirmeArrivalColumns.BATCH)).ToString().Trim()
...
#Region "商品明細マスタ"
    ...
    Public Function TorikomiMstChk(ByVal dr As DataRow, ByVal ds As DataSet) As Boolean
        ...
        sMsg = "荷主商品コード(カラム" & CType(FirmeArrivalColumns.MATERIAL, Int32) & "番目)"
        sGoodsCd = dr.Item(GetColumnName(FirmeArrivalColumns.MATERIAL)).ToString()      '荷主商品コード
        sGoodsNm = dr.Item(GetColumnName(FirmeArrivalColumns.MATERIAL_DESCRIPTION)).ToString()       '商品名
        If MyBase.GetResultCount() = 0 Then
            ...
            bRet = False
            Return bRet
#If False Then      ' 商品が特定できない場合は、入荷登録時に選択させるため許可とする
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_FIR")         'EDI受信Dtl
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            ds.Tables("LMH010_INKAEDI_DTL_FIR").Clear() '受信DTLをクリア
            ds = Me.SetSemiInkaEdiRcv(ds, i)
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_FIR").Rows(0)
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH010_INKAEDI_DTL_FIR")
            setDtlDt.Clear()
