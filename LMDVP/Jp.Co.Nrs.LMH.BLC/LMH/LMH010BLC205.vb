...
'  EDI荷主ID　　　　:  205　　　 : 日立ファインネクスト(全営業所・全荷主)
'  作  成  者       :  umano
' ==========================================================================
...
''' </summary>
''' <remarks></remarks>
Public Class LMH010BLC205
    Inherits Jp.Co.Nrs.LM.Base.BLC.LMBaseBLC

    '2018/04/18 001528 【LMS】日立FN_EDI入荷登録時、入荷大でまとめる(千葉BC物管受注１松本) Annen add start
#Region "Const"
    Private Const NRS_BR_CD_CHIBA As String = "10"

    Private Const CUST_CD_L_DIC_OUTKA As String = "00010"

#End Region
    '2018/04/18 001528 【LMS】日立FN_EDI入荷登録時、入荷大でまとめる(千葉BC物管受注１松本) Annen add end
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC205 = New LMH010DAC205()
...
#Region "入荷登録"
    ...
    Private Function InkaToroku(ByVal ds As DataSet) As DataSet
        ...
        '要望番号2100 修正START 日立FNの入荷はまとめ処理を行う 2013.10.07
        ...
        If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
            ...
            ElseIf autoMatomeF.Equals("0") = True Then
                Dim choiceKb As String = Me._ChkBlc.GetWarningChoiceKb(ds.Tables("LMH010_INKAEDI_L"), ds, LMH010BLC.DIC_WID_L001, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._ChkBlc.SetWarningL("W184", LMH010BLC.DIC_WID_L001, ds, msgArray, matomesakiInkaNo)
                    Return ds
        ...
        Return ds
        '要望番号2100 修正END 日立FNの入荷はまとめ処理を行う 2013.10.07
...
#If False Then      'UPD2018/06/29 依頼番号 : 001931   【LMS】日立FN_入荷ORDER_CHECK_FLGの判別ロジック修正
                '2018/04/18 001528 【LMS】日立FN_EDI入荷登録時、入荷大でまとめる(千葉BC物管受注１松本) Annen upd start
        'オーダー番号重複チェック
        If drEdiL("NRS_BR_CD").ToString <> NRS_BR_CD_CHIBA OrElse drEdiL("CUST_CD_L").ToString <> CUST_CD_L_DIC_OUTKA Then
        ...
        '    End If
        'End If
        '2018/04/18 001528 【LMS】日立FN_EDI入荷登録時、入荷大でまとめる(千葉BC物管受注１松本) Annen upd end
...
#Region "入荷登録マスタ存在チェック(中)"
    Private Function SetGoodsMst(ByRef ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            '条件の再設定
            setDtM = Me.SetGoodsCdFromWarning(setDtM, ds, LMH010BLC.DIC_WID_M001)
            ...
            ElseIf GetResultCount() > 1 Then
                ...
                Else
                    ...
                    ds = Me._ChkBlc.SetWarningM("W162", LMH010BLC.DIC_WID_M001, ds, setDs, msgArray)
            ...
            '2013.10.10 追加START
            '①商品名
            Dim mGoodsNm As String = goodsDt.Rows(0)("GOODS_NM_1").ToString()
            Dim ediGoodsNm As String = dtM.Rows(i)("GOODS_NM").ToString()

            'スペース除去チェックの追加
            mGoodsNm = Replace(Me._ChkBlc.SpaceCutChk(mGoodsNm), "ツ", "ｯ")
            ediGoodsNm = Replace(Me._ChkBlc.SpaceCutChk(ediGoodsNm), "ツ", "ｯ")
            '2013.10.10 追加END

            '2013.10.10 修正START
            If String.IsNullOrEmpty(dtM.Rows(i)("GOODS_NM").ToString()) = False Then
                If ediGoodsNm.Equals(mGoodsNm) = False Then
                    '2013.10.10 修正END
                    'If dtM.Rows(i)("GOODS_NM").ToString().Equals(goodsDt.Rows(0)("GOODS_NM_1").ToString()) = False Then
                    choiceKb = Me._ChkBlc.GetWarningChoiceKbM(ds.Tables("LMH010_INKAEDI_M"), ds, LMH010BLC.DIC_WID_M002, i)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(2) = "商品マスタ"
                        msgArray(3) = "商品名"
                        msgArray(4) = "（受信した品名は「商品別注意事項」に記載します。）"
                        msgArray(5) = String.Empty

                        ds = Me._ChkBlc.SetWarningM2("W194", LMH010BLC.DIC_WID_M002, ds, setDs, msgArray, ediName, ediValue, mustValue)
                        flgWarning = True 'ワーニングフラグをたてて処理続行
                    Else
                        '処理区分が"はい"の場合、REMARKを入れ替えて処理続行
                        If String.IsNullOrEmpty(dtM.Rows(i)("REMARK").ToString().Trim) Then
                            'dtM.Rows(i)("REMARK") = String.Concat("受信名＝", dtM.Rows(i)("GOODS_NM"))
                            dtM.Rows(i)("REMARK") = String.Concat("受信名＝", ReplaceSingleQuotation(dtM.Rows(i)("GOODS_NM").ToString()))
                        Else
                            'dtM.Rows(i)("REMARK") = Left(String.Concat(dtM.Rows(i)("REMARK"), Space(2), "受信名＝", dtM.Rows(i)("GOODS_NM").ToString()), 100)
                            dtM.Rows(i)("REMARK") = Left(String.Concat(dtM.Rows(i)("REMARK"), Space(2), "受信名＝", ReplaceSingleQuotation(dtM.Rows(i)("GOODS_NM").ToString())), 100)
                        End If
                    End If
            ...
            '個数単位
            If String.IsNullOrEmpty(dtM.Rows(i)("NB_UT").ToString()) = False Then
                If dtM.Rows(i)("NB_UT").ToString().Equals(goodsDt.Rows(0)("NB_UT")) = False Then
                    MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E332", New String() {"個数単位", "商品マスタ", "個数単位"}, rowNo, LMH010BLC.EXCEL_COLTITLE, ediCtlNo)
                    Return False
                End If
            Else
                '個数単位
            ...
            '包装個数(入数)
            '2013.10.10 修正START
            If Convert.ToDecimal(dtM.Rows(i)("PKG_NB")) <> 0 Then
                '2013.10.10 修正END
                If dtM.Rows(i)("PKG_NB").ToString().Equals(goodsDt.Rows(0)("PKG_NB")) = False Then
                    MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E332", New String() {"包装個数(入数)", "商品マスタ", "包装個数(入数)"}, rowNo, LMH010BLC.EXCEL_COLTITLE, ediCtlNo)
                    Return False
                End If
            Else
                '包装個数(入数)
            ...
            '包装単位
            If String.IsNullOrEmpty(dtM.Rows(i)("PKG_UT").ToString()) = False Then
                If dtM.Rows(i)("PKG_UT").ToString().Equals(goodsDt.Rows(0)("PKG_UT")) = False Then
                    MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E332", New String() {"包装単位", "商品マスタ", "包装単位"}, rowNo, LMH010BLC.EXCEL_COLTITLE, ediCtlNo)
                    Return False
                End If
            Else
                '包装単位
            ...
            If Convert.ToDecimal(dtM.Rows(i)("STD_IRIME")) <> 0 Then
                If dtM.Rows(i)("STD_IRIME").ToString().Equals(goodsDt.Rows(0)("STD_IRIME_NB")) = False Then
                    MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E332", New String() {"標準入目", "商品マスタ", "標準入目"}, rowNo, LMH010BLC.EXCEL_COLTITLE, ediCtlNo)
                    Return False
                End If
            Else
                '標準入目
            ...
            '標準入目単位
            If String.IsNullOrEmpty(dtM.Rows(i)("STD_IRIME_UT").ToString()) = False Then
                If dtM.Rows(i)("STD_IRIME_UT").ToString().Equals(goodsDt.Rows(0)("STD_IRIME_UT")) = False Then
                    MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E332", New String() {"標準入目単位", "商品マスタ", "標準入目単位"}, rowNo, LMH010BLC.EXCEL_COLTITLE, ediCtlNo)
                    Return False
                End If
            Else
                '標準入目単位
            ...
            dtM.Rows(i)("LOCA") = String.Empty      'ADD 2020/08/05 014005   【LMS】商品マスタ_入荷仮置場機能の追加
...
#If False Then   'ADD 2020/08/05 014005   【LMS】商品マスタ_入荷仮置場機能の追加
...
#Else
                    If setOkiba.Length > 0 Then
                        setOkiba = String.Concat(setOkiba.ToString, Space(16))
                        ...
                        TOU_NO = setOkiba.Substring(0, 2)
                        SITU_NO = setOkiba.Substring(2, 2)
                        ZONE_CD = setOkiba.Substring(4, 2)
                        LOCA = setOkiba.Substring(6, 10)
                        ...
                        dtM.Rows(i)("TOU_NO") = TOU_NO.ToString
                        dtM.Rows(i)("SITU_NO") = SITU_NO.ToString
                        dtM.Rows(i)("ZONE_CD") = ZONE_CD.ToString
                        dtM.Rows(i)("LOCA") = LOCA.ToString
                    End If
#End If
...
#Region "データセット設定(入荷L)"
    ...
    Private Function SetDatasetInkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        Else
            ...
            inkaDr("OUTKA_FROM_ORD_NO_L") = String.Empty        'ADD 2018/10/03 纏め時は設定しない　依頼番号 : 002498   【LMS】【既存バグ】入荷まとめ機能バグ対応
        End If
...
#Region "データセット設定(入荷S)"
    ...
    Private Function SetDatasetInkaS(ByVal ds As DataSet) As DataSet
        ...
        Dim ediLDr As DataRow = ds.Tables("LMH010_INKAEDI_L").Rows(0)
        ...
        For i As Integer = 0 To max
            ...
            inkaDr("LOCA") = ediDr("LOCA")       'ADD 2020/08/05 014005   【LMS】商品マスタ_入荷仮置場機能の追加
            ...
            If ediLDr("INKA_TP").Equals("20") = True Then
                inkaDr("SPD_KB") = "02"
                inkaDr("GOODS_COND_KB_1") = "15"
            Else
                inkaDr("SPD_KB") = "01"
            End If
            inkaDr("OFB_KB") = "01"
...
#Region "リプレース"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function ReplaceSingleQuotation(ByVal val As String) As String
