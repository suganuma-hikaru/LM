...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC612 = New LMH030DAC612()

    ''' <summary>
    ''' 使用するDACクラスの生成
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac000 As LMH030DAC000 = New LMH030DAC000()
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        '物産アニマルヘルスはまとめ対象外荷主(共通のまとめSQLを使用)
        ...
        If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
            ...
            ElseIf MyBase.GetResultCount > 1 Then
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.NIK_WID_L001, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningL("W199", LMH030BLC.NIK_WID_L001, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("0") = True Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.NIK_WID_L002, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = String.Empty
                    matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                    ds = Me._Blc.SetComWarningL("W168", LMH030BLC.NIK_WID_L002, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("1") = True Then
                ...
                If matomeStatus.Equals("10") = False Then
                    ...
                    choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.NIK_WID_L003, 0)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(4) = String.Empty
                        matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                        ds = Me._Blc.SetComWarningL("W198", LMH030BLC.NIK_WID_L003, ds, msgArray, matomeNo, String.Empty)
                        Return ds
        ...
        'If matomeFlg Then
        '    ' まとめ元・まとめ先の 案内書_取引区分 の取得
        '    Dim dsSelA_TorihikiKbn As DataSet = ds.Clone()
        '
        '    dsSelA_TorihikiKbn.Tables("H_OUTKAEDI_DTL_BAH").Clear()
        '    Dim aTorihikiKbnMoto As String = ""
        '    dsSelA_TorihikiKbn.Tables("LMH030_OUTKAEDI_L").Merge(ds.Tables("LMH030_OUTKAEDI_L"))
        '    dsSelA_TorihikiKbn = MyBase.CallDAC(Me._Dac, "SelectA_TorihikiKbnMoto", dsSelA_TorihikiKbn)
        '    If MyBase.GetResultCount > 0 Then
        '        aTorihikiKbnMoto = dsSelA_TorihikiKbn.Tables("H_OUTKAEDI_DTL_BAH").Rows(0).Item("A_TORIHIKI_KBN").ToString()
        '    End If
        '
        '    dsSelA_TorihikiKbn.Tables("H_OUTKAEDI_DTL_BAH").Clear()
        '    Dim aTorihikiKbnSaki As String = ""
        '    dsSelA_TorihikiKbn.Tables("LMH030_MATOMESAKI_EDIL").Merge(ds.Tables("LMH030_MATOMESAKI_EDIL"))
        '    dsSelA_TorihikiKbn = MyBase.CallDAC(Me._Dac, "SelectA_TorihikiKbnSaki", dsSelA_TorihikiKbn)
        '    If MyBase.GetResultCount > 0 Then
        '        aTorihikiKbnSaki = dsSelA_TorihikiKbn.Tables("H_OUTKAEDI_DTL_BAH").Rows(0).Item("A_TORIHIKI_KBN").ToString()
        '    End If
        '
        '    If aTorihikiKbnMoto <> "" AndAlso aTorihikiKbnSaki <> "" AndAlso aTorihikiKbnMoto <> aTorihikiKbnSaki Then
        '        Dim outkaKanriNo As String = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0)("OUTKA_CTL_NO").ToString()
        '        Dim msgParam3 As String =
        '            String.Concat("(", "取引区分:", aTorihikiKbnMoto, ", まとめ先取引区分:", aTorihikiKbnSaki, ", まとめ先出荷管理番号:", outkaKanriNo, ")")
        '        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E454",
        '                               New String() {"まとめ対象が異なる取引区分", "まとめ出荷", msgParam3}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
        '        Return ds
        '    End If
        'End If
...
#Region "実績作成処理"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        'EDI実績の値設定(EDI受信TBLより)
        ds = MyBase.CallDAC(Me._Dac, "SelectEdiSend", ds)

        If MyBase.GetResultCount = 0 Then
            'メッセージ「納品案内書未出力のため出力してください。」
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E03O", New String() {}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            Return ds
        End If

        'データの編集
        Const SEP As String = ";"
        For Each dr As DataRow In ds.Tables("LMH030_H_SENDOUTEDI_BAH").Rows
            '納品案内書の出力時内容を分割して各項目にセット
            '分割内容は左から・・・識別文字(無視)、伝票番号、整理番号(無視)、行番号、数量、元梱、端数、ロット番号、使用期限
            Dim NhsInfo As String() = Split(dr.Item("NHS_INFO").ToString, SEP, -1, CompareMethod.Binary)
            dr.Item("NA_NO") = NhsInfo(1)
            dr.Item("NA_GYO") = NhsInfo(3)
            dr.Item("NA_NB") = NhsInfo(4)
            dr.Item("NA_PKG") = NhsInfo(5)
            dr.Item("NA_HASU") = NhsInfo(6)
            dr.Item("NA_LOT_NO") = NhsInfo(7)
            dr.Item("NA_LT_DATE") = NhsInfo(8)
        Next

        'EDI実績データの作成
        ds = MyBase.CallDAC(Me._Dac, "InsertEdiSendData", ds)
...
#Region "EDI出荷(大)のDAC側でのチェック"
    ...
    Private Function EdiLDbExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String, _
        ...
        If (String.IsNullOrEmpty(shipCdL) = False AndAlso shipCdL.Equals(destCd) = True AndAlso shipCdL.Equals(ediDestCd) = False) _
            ...
            ds = MyBase.CallDAC(Me._Dac, "SelectDataNikMdest", ds)
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.NIK_WID_M001, 0)
...
#Region "届先マスタチェック(日医工専用)"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        If mDestNm.Equals(ediDestNm) = True Then
            'チェックなし
        Else
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.NIK_WID_L004, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                ds = Me._Blc.SetComWarningL("W166", LMH030BLC.NIK_WID_L004, ds, msgArray, _
        ...
        Else
            ...
            If mAdAll.Equals(ediDestAdAll) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.NIK_WID_L005, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.NIK_WID_L005, ds, msgArray, ediDestAdAll, mAdAll)
        ...
        Else
            If mTel.Equals(ediTel) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.NIK_WID_L006, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.NIK_WID_L006, ds, msgArray, ediTel, mTel)
        ...
        If String.IsNullOrEmpty(mZipJis) = True AndAlso String.IsNullOrEmpty(ediDestJisCd) = False Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.NIK_WID_L007, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                ds = Me._Blc.SetComWarningL("W197", LMH030BLC.NIK_WID_L007, ds, msgArray, ediDestJisCd, mJis)
        ...
        ElseIf String.IsNullOrEmpty(mZipJis) = True AndAlso String.IsNullOrEmpty(ediDestJisCd) = True Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.NIK_WID_L008, 0)

            If String.IsNullOrEmpty(choiceKb) = True Then

                ds = Me._Blc.SetComWarningL("W188", LMH030BLC.NIK_WID_L008, ds, msgArray, ediDestJisCd, mJis)
        ...
        ElseIf String.IsNullOrEmpty(ediDestJisCd) = False AndAlso ediDestJisCd.Equals(mJis) = False Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.NIK_WID_L009, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(4) = "EDIデータ"
                msgArray(5) = String.Empty
                ds = Me._Blc.SetComWarningL("W187", LMH030BLC.NIK_WID_L009, ds, msgArray, ediDestJisCd, mJis)
        ...
        ElseIf String.IsNullOrEmpty(ediDestJisCd) = True AndAlso mZipJis.Equals(mJis) = False Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.NIK_WID_L010, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(5) = String.Empty
                ds = Me._Blc.SetComWarningL("W187", LMH030BLC.NIK_WID_L010, ds, msgArray, mZipJis, mJis)
...
#Region "届先マスタ追加時チェック"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function ZipCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, _
                                     ByVal ediCtlNo As String, ByVal workDestCd As String, _
                                     ByVal workDestString As String, genericInsFlg As Integer) As Boolean
        ...
        choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.NIK_WID_L011, 0)
        ...
        If String.IsNullOrEmpty(choiceKb) = True AndAlso genericInsFlg = 0 Then
            ...
            ds = Me._Blc.SetComWarningL("W186", LMH030BLC.NIK_WID_L011, ds, msgArray, workDestCd, String.Empty) '追加箇所 20110222
...
#Region "届先マスタ(荷送人コード)追加時チェック"
    ...
    Private Function SetInsMDestFromShip(ByVal ds As DataSet) As Boolean
        ...
        Dim dtMS As DataTable = ds.Tables("LMH030_M_DEST_SHIP_CD_L")
        Dim msgArray(5) As String
        Dim choiceKb As String = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.NIK_WID_L012, 0)
        Dim drEdiL As DataRow = ds.Tables("LMH030_OUTKAEDI_L").Rows(0)
        ...
        If String.IsNullOrEmpty(choiceKb) = True Then
            ...
            ds = Me._Blc.SetComWarningL("W182", LMH030BLC.NIK_WID_L012, ds, msgArray, shipCdL, String.Empty)
...
#Region "届先マスタ(荷送人コード)更新時チェック"
    ...
    Private Function SetUpdMDestFromShip(ByVal ds As DataSet) As Boolean
        ...
        Dim dtMS As DataTable = ds.Tables("LMH030_M_DEST_SHIP_CD_L")
        Dim msgArray(5) As String
        Dim choiceKb As String = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.NIK_WID_L013, 0)
        Dim drEdiL As DataRow = ds.Tables("LMH030_OUTKAEDI_L").Rows(0)
        Dim shipCdL As String = drEdiL("SHIP_CD_L").ToString()
        ...
        If String.IsNullOrEmpty(choiceKb) = True Then
            ...
            ds = Me._Blc.SetComWarningL("W166", LMH030BLC.NIK_WID_L013, ds, msgArray, drEdiL("SHIP_NM_L").ToString(), dtMS.Rows(0).Item("DEST_NM").ToString())
...
#Region "データセット設定(出荷L)"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        Else
            ...
            ds = MyBase.CallDAC(Me._Dac000, "SelectDataZkbn", ds)
...
#Region "データセット設定(作業)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetDatasetSagyo(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        'まとめ処理の場合（物産アニマルヘルスはまとめ対象外だがロジックは残しておく）
