...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC107 = New LMH030DAC107()
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
            ...
            ElseIf MyBase.GetResultCount > 1 Then
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.CSO_WID_L001, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningL("W199", LMH030BLC.CSO_WID_L001, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("0") = True Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.CSO_WID_L002, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = String.Empty
                    matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                    ds = Me._Blc.SetComWarningL("W168", LMH030BLC.CSO_WID_L002, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("1") = True Then
                ...
                If matomeStatus.Equals("10") = False Then
                    ...
                    choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.CSO_WID_L003, 0)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(4) = String.Empty
                        matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                        ds = Me._Blc.SetComWarningL("W198", LMH030BLC.CSO_WID_L003, ds, msgArray, matomeNo, String.Empty)
                        Return ds
        ...
        '要望番号:1887 terakawa 2013.03.25 Start
        '出荷(中)データセット設定
        'ds = Me.SetDatasetOutkaM(ds)
        ds = Me.SetDatasetOutkaM(ds, matomeFlg)
        '要望番号:1887 terakawa 2013.03.25 End
...
#Region "実績作成処理"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        Dim dt As DataTable = ds.Tables("LMH030_SEARCH_HED_CSO")
        Dim dtZ As DataTable = ds.Tables("LMH030_Z_KBN")
        Dim dtE As DataTable = ds.Tables("LMH030_JUDGE")

        ''★★★2011.09.14 追加START
        'Dim msgArray(5) As String
        'Dim choiceKb As String = String.Empty

        'ORACLE接続の確立 区分マスタより接続情報を取得
        ds = MyBase.CallDAC(Me._Dac, "SelectOracleSchma", ds)

        ''▼▼▼(チッソ専用チェック)
        '出荷データ（中）追加分の出荷ＥＤＩデータ（中）特定チェック

        ds = MyBase.CallDAC(Me._Dac, "SelectChissoOutkaAdd", ds)
        If MyBase.GetResultCount > 1 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E232", New String() {"出荷データで追加", "実績作成"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            Return ds
        End If

        ''▲▲▲(チッソ専用チェック)

        'チッソEDI実績の値設定(EDI受信TBLより)
        ds = MyBase.CallDAC(Me._Dac, "SelectChissoEdiSend", ds)

        If MyBase.GetResultCount = 0 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E326", New String() {"出荷データ", "該当レコード"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            Return ds

        ElseIf MyBase.GetResultCount > 9 Then
            '
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E504", New String() {"実績データ", "明細", "9件"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            Return ds
        ...
        Dim setDt As DataTable = setDs.Tables("LMH030_SEARCH_HED_CSO")
        Dim zkbnDt As DataTable = setDs.Tables("LMH030_Z_KBN")
        Dim inDt As DataTable = setDs.Tables("LMH030INOUT")
        Dim eventDt As DataTable = setDs.Tables("LMH030_JUDGE")

        Dim preAlctdQt As Decimal = 0
        Dim preFreeC01 As String = String.Empty
        Dim preFreeC02 As String = String.Empty
        ...
        For i As Integer = 0 To max
            ...
            zkbnDt.ImportRow(dtZ.Rows(0))
            inDt.ImportRow(dtIn.Rows(0))
            eventDt.ImportRow(dtE.Rows(0))

            'EDI出荷(中)の更新
            setDs = MyBase.CallDAC(Me._Dac, "UpdateOutkaEdiMDataJisseki", setDs)

            If preFreeC01.Equals(setDt.Rows(0).Item("FREE_C01").ToString()) = True AndAlso _
                preFreeC02.Equals(setDt.Rows(0).Item("FREE_C02").ToString()) = True Then

                '前行の数量を次行へ足しこみ
                setDt.Rows(0).Item("ALCTD_QT") = Convert.ToDecimal(setDt.Rows(0).Item("ALCTD_QT")) + preAlctdQt

            End If

            'チッソ専用EDI受信(HED)の検索処理
            setDs = MyBase.CallDAC(Me._Dac, "SelectEdiRcvHedChissoData", setDs)

            '前行の数量を初期化
            preAlctdQt = 0
            If (preFreeC01.Equals(setDt.Rows(0).Item("FREE_C01").ToString()) = True AndAlso _
               preFreeC02.Equals(setDt.Rows(0).Item("FREE_C02").ToString()) = True) OrElse i = 0 Then

                '前行の数量を格納
                preAlctdQt = Convert.ToDecimal(setDt.Rows(0).Item("ALCTD_QT"))
                preFreeC01 = setDt.Rows(0).Item("FREE_C01").ToString()
                preFreeC02 = setDt.Rows(0).Item("FREE_C02").ToString()

            End If

            'Dim setDsHed As DataTable = setDs.Tables("LMH030_H_OUTKAEDI_HED_CSO")

            'If MyBase.GetResultCount = 0 OrElse String.IsNullOrEmpty(setDsHed.Rows(0).Item("HED_REC_NO").ToString()) = True Then
            '    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E011", , rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            '    Return ds
            ...
            'ElseIf ds.Tables("LMH030_H_OUTKAEDI_HED_CSO").Rows(0).Item("OLD_DATA_FLG").ToString().Equals("Y") = False Then
            '    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E011", , rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            '    Return ds
            ...
            ''チッソ専用EDI受信(HED)の更新
            'setDs = MyBase.CallDAC(Me._Dac, "UpdateEdiRcvHedData", setDs)
            'If MyBase.GetResultCount = 0 Then
            '    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E011", , rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            '    Return ds
            'End If

            ''チッソ専用EDI受信(DTL)の追加処理
...
#Region "EDI出荷(中)の初期値設定"
    ...
    Private Function EdiMDefaultSet(ByVal ds As DataSet, ByVal setDs As DataSet, _
        ...
        '商品名１
        Dim mGoodsNm1 As String = mGoodsDr("GOODS_NM_1").ToString()
        Dim ediGoodsNm1 As String = ediMDr("GOODS_NM").ToString()

        'スペース除去チェックの追加
        mGoodsNm1 = Me.SpaceCutChk(mGoodsNm1)
        ediGoodsNm1 = Me.SpaceCutChk(ediGoodsNm1)

        If mGoodsNm1.Equals(ediGoodsNm1) = True Then
            'チェックなし
        Else

            choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.CSO_WID_M002, count)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                ds = Me._Blc.SetComWarningM("W159", LMH030BLC.CSO_WID_M002, ds, setDs, msgArray, _
        ...
        '商品名２
        Dim mGoodsNm2 As String = mGoodsDr("GOODS_NM_2").ToString()
        Dim ediFreeC05 As String = ediMDr("FREE_C05").ToString()

        'スペース除去チェックの追加
        mGoodsNm2 = Me.SpaceCutChk(mGoodsNm2)
        ediFreeC05 = Me.SpaceCutChk(ediFreeC05)

        If mGoodsNm2.Equals(ediFreeC05) = True Then
            'チェックなし
        Else

            choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.CSO_WID_M003, count)

            If String.IsNullOrEmpty(choiceKb) = True Then
                msgArray(1) = "商品名(FREE_C05)"
                msgArray(2) = "商品マスタ"
                msgArray(3) = "商品名２"
                msgArray(4) = "EDIデータ"
                msgArray(5) = String.Empty

                ds = Me._Blc.SetComWarningM("W159", LMH030BLC.CSO_WID_M003, ds, setDs, msgArray, _
                                            ediMDr("FREE_C05").ToString(), mGoodsDr("GOODS_NM_2").ToString())

                compareWarningFlg = True

            ElseIf choiceKb.Equals("01") = True Then
                '全角変換したままの場合、桁あふれを起こす事がある
                ediMDr("GOODS_NM") = mGoodsDr("GOODS_NM_2").ToString()
        ...
        Else
            If ediMDr("KB_UT").Equals(mGoodsDr("NB_UT")) = False Then
                'EDI(中)と商品マスタで個数単位が異なる場合、エラー
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E332", New String() {"個数単位", "商品マスタ", "個数単位"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                Return False
            End If
        End If
        ...
        Else
            If ediMDr("IRIME_UT").Equals(mGoodsDr("STD_IRIME_UT")) = False Then
                'EDI(中)と商品マスタで数量単位が異なる場合、エラー
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E332", New String() {"数量単位", "商品マスタ", "数量単位"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                Return False
            End If
        End If
        ...
        Select Case alctdKb
            ...
            Case "02"
                ...
                If outkaTtlQt Mod irime = 0 Then
                    baraNb = outkaTtlQt / irime
                Else

                    choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.CSO_WID_M004, count)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(4) = String.Empty
                        msgArray(5) = String.Empty
                        'ds = Me._Blc.SetComWarningM("W228", LMH030BLC.CSO_WID_M004, ds, setDs, msgArray, _
                        ds = Me._Blc.SetComWarningM("W306", LMH030BLC.CSO_WID_M004, ds, setDs, msgArray,
                        Convert.ToString(outkaTtlQt / irime), String.Empty)

                        compareWarningFlg = True

                    ElseIf choiceKb.Equals("01") = True Then
                        'ワーニングで"はい"を選択時
                        baraNb = Math.Floor(outkaTtlQt / irime) + 1

                    End If

                End If

                ediMDr("OUTKA_PKG_NB") = Math.Floor(baraNb / pkgNb)
                ediMDr("OUTKA_HASU") = baraNb Mod pkgNb

                ediMDr("OUTKA_TTL_NB") = baraNb
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.CSO_WID_M001, 0)
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    '    '2012.03.19 修正START
                    '    ds = Me._Blc.SetComWarningM("W162", LMH030BLC.CSO_WID_M001, ds, setDs, msgArray, custGoodsCd, String.Empty)
                    '    '2012.03.19 修正END
                ...
                If Me.EdiMDefaultSet(ds, setDs, i, unsoData, rowNo, ediCtlNo) = False Then
                    ...
                    'チッソは現段階ではワーニング・エラー両方存在。共通のロジックを組み込む為入れておく
                    Dim chkRowNo As Integer = Convert.ToInt32(rowNo)
                    If MyBase.IsMessageStoreExist(chkRowNo) = True Then
                        '整合性チェックでエラーがあった場合は処理終了
                        Return False
...
#Region "届先マスタチェック(チッソ専用)"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        Dim ediDestNm As String = dtEdi.Rows(0)("FREE_C05").ToString()
        ...
        'Dim ediDestAdAll As String = String.Concat(ediDestAd1, ediDestAd2)
        ...
        '届先名称(マスタ値が完全一致でなければワーニング)※チッソはFREE_C05を使用
        If mDestNm.Equals(ediDestNm) = True Then
            'チェックなし
        Else
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.CSO_WID_L004, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(4) = "EDIデータ"
                msgArray(5) = String.Empty
                ds = Me._Blc.SetComWarningL("W166", LMH030BLC.CSO_WID_L004, ds, msgArray, _
        ...
        If String.IsNullOrEmpty(ediZip) = True Then
            'チェックなし
            dtEdi.Rows(0)("DEST_ZIP") = mZip
        Else
            If mZip.Equals(ediZip) = False Then

                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.CSO_WID_L005, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W159", LMH030BLC.CSO_WID_L005, ds, msgArray, ediZip, mZip)
        ...
        Else
            ...
            If mAdAll.Equals(ediFreeC21) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.CSO_WID_L006, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W159", LMH030BLC.CSO_WID_L006, ds, msgArray, ediFreeC21, mAdAll)
        ...
        If String.IsNullOrEmpty(ediTel) = True Then
            'チェックなし
            dtEdi.Rows(0)("DEST_TEL") = mTel
        Else
            If mTel.Equals(ediTel) = False Then

                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.CSO_WID_L007, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W159", LMH030BLC.CSO_WID_L007, ds, msgArray, ediTel, mTel)
        ...
        Else
            If mJis.Equals(ediDestJisCd) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.CSO_WID_L008, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W159", LMH030BLC.CSO_WID_L008, ds, msgArray, ediDestJisCd, mJis)
...
#Region "データセット設定(出荷L)"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            End If
            'チッソ特殊処理START
            If String.IsNullOrEmpty(ediDr("FREE_C06").ToString()) = True Then

                outkaDr("NHS_REMARK") = ediDr("REMARK")
            ElseIf String.IsNullOrEmpty(ediDr("REMARK").ToString()) = True Then

                outkaDr("NHS_REMARK") = ediDr("FREE_C06")
            Else
                outkaDr("NHS_REMARK") = Me._Blc.LeftB(String.Concat(ediDr("REMARK"), Space(2), ediDr("FREE_C06")), 100)
            End If
    ...
    Private Function SetDatasetOutkaM(ByVal ds As DataSet, ByVal matomeflg As Boolean) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            Else
                outkaDr("CUST_ORD_NO_DTL") = ediDr("CUST_ORD_NO_DTL")
            End If
            '要望番号:1887 honmyo2012.12.26 End
            ...
            Else
                outkaDr("BUYER_ORD_NO_DTL") = ediDr("BUYER_ORD_NO_DTL")
            End If
            '要望番号:1712 terakawa 2013.03.25 End
