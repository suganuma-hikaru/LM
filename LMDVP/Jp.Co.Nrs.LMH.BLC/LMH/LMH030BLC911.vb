...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC911 = New LMH030DAC911()
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        '2015.06.11 修正START 住化バイエル対応
        '荷主コード、倉庫コード振り分け(区分マスタ)
        Dim drJudge As DataRow = setDs.Tables("LMH030_JUDGE").NewRow()
        drJudge("KBN_GROUP_CD") = "B022"
        '文字列3
        sKey = "L_FREE_C03_NO"
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            drJudge("KBN_CD") = drRcvEdiDtl(sTempColNo).ToString().Trim()
        End If
        ...
        If MyBase.GetResultCount = 1 Then
            '営業所
            drOutkaEdiL("NRS_BR_CD") = Left(setDs.Tables("LMH030_Z_KBN").Rows(0).Item("NISUGATA").ToString(), 2)
            '倉庫コード
            drOutkaEdiL("WH_CD") = Right(setDs.Tables("LMH030_Z_KBN").Rows(0).Item("NISUGATA").ToString(), 3)
            '荷主L
            drOutkaEdiL("CUST_CD_L") = setDs.Tables("LMH030_Z_KBN").Rows(0).Item("NISUGATA").ToString().Substring(2, 5)
            '荷主M
            drOutkaEdiL("CUST_CD_M") = setDs.Tables("LMH030_Z_KBN").Rows(0).Item("NISUGATA").ToString().Substring(7, 2)
        Else
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E454", New String() {"プラントより荷主が特定できない", "処理", String.Concat("単位", drRcvEdiDtl.Item(sTempColNo).ToString().Trim())}, Convert.ToString(rowNo), LMH030BLC.EXCEL_COLTITLE, sEdiCtlNo)
            Return setDs
        End If
        '2015.06.11 修正END 住化バイエル対応
