...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC105 = New LMH030DAC105()
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
            ...
            ElseIf MyBase.GetResultCount > 1 Then
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.HON_WID_L013, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningL("W199", LMH030BLC.HON_WID_L013, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("0") = True Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.HON_WID_L005, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = String.Empty
                    matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                    ds = Me._Blc.SetComWarningL("W168", LMH030BLC.HON_WID_L005, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("1") = True Then
                ...
                If matomeStatus.Equals("10") = False Then
                    ...
                    choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.HON_WID_L014, 0)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(4) = String.Empty
                        matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                        ds = Me._Blc.SetComWarningL("W198", LMH030BLC.HON_WID_L014, ds, msgArray, matomeNo, String.Empty)
                        Return ds
...
#Region "EDI_Lの初期値設定(出荷登録処理)"
    ...
    Private Function SetEdiLShoki(ByVal ds As DataSet) As DataSet
        ...
        'ハネウェル対応 terakawa 2013.01.28 Start
        '届先M取得
        If ediDr("EDI_DEST_CD").ToString().Trim().Equals(LMH030BLC105.EDI_DEST_CD_HANYO.ToString.Trim()) = True Then
            If String.IsNullOrEmpty(ediDr("DEST_CD").ToString().Trim()) = True Then
                '届先汎用コード("0000000001")の場合、後続で届先Mの値を設定する為、強制的にDATAROWを作成
                ds = Me.setEdidestHanyo(ds)
            Else
                ds = MyBase.CallDAC(Me._Dac, "SelectDataMdest", ds)
            End If
        ElseIf String.IsNullOrEmpty(ediDr("DEST_CD").ToString().Trim()) = False AndAlso _
            String.IsNullOrEmpty(ediDr("EDI_DEST_CD").ToString().Trim()) = False Then
            ds = MyBase.CallDAC(Me._Dac, "SelectDataMdest", ds)

            If ds.Tables("LMH030_M_DEST").Rows.Count = 0 Then
                'EDI届先コードを空にし、届先コードで再度マスタに取りに行く。
                Dim ediDestCdBk As String = ediDr("EDI_DEST_CD").ToString
                ediDr("EDI_DEST_CD") = String.Empty
                ds = MyBase.CallDAC(Me._Dac, "SelectDataMdest", ds)
                'EDI届先コードを再設定する
                ediDr("EDI_DEST_CD") = ediDestCdBk
            End If
        ...
        Else
            ...
            If ds.Tables("LMH030_M_DEST_SHIP_CD_L").Rows.Count <> 0 Then
                ediDr("FREE_C23") = ds.Tables("LMH030_M_DEST_SHIP_CD_L").Rows(0).Item("DEST_NM").ToString().Trim()
            End If
        End If
        'ハネウェル対応 terakawa 2013.01.28 End
    ...
    Private Function SetDestCd(ByVal ds As DataSet) As DataSet
        ...
        If String.IsNullOrEmpty(ediDr("DEST_CD").ToString().Trim()) = True _
           AndAlso (LMH030BLC105.EDI_DEST_CD_HANYO.ToString).Equals(ediDr("EDI_DEST_CD").ToString().Trim()) = False Then
            If ds.Tables("LMH030_M_DEST").Rows.Count > 0 Then
...
#Region "EDI出荷(大)のDAC側でのチェック"
    ...
    Private Function EdiLDbExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        '
        If ediDestCd.Equals(LMH030BLC105.EDI_DEST_CD_HANYO.ToString) = True AndAlso _
        ...
        If mDestCount = 1 Then
            If destHanyoFlg = True Then
                ...
                Select Case custIndex
                    Case LMH030BLC.EdiCustIndex.HonChb00630_00
                        '市原の場合は届先マスタの追加が可能
                    Case Else
                        '市原以外はエラー
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.HON_WID_M001, 0)
                ...
                'ハネウェル対応 terakawa 2013.01.28 Start
                'ハネウェルはNRS商品コードで検索を行わない
                setDtM.Rows(0).Item("NRS_GOODS_CD") = String.Empty
                'ハネウェル対応 terakawa 2013.01.28 End
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    Else
                        ...
                        ds = Me._Blc.SetComWarningM("W162", LMH030BLC.HON_WID_M001, ds, setDs, msgArray, custGoodsCd, String.Empty)
                ...
                If Me.EdiMDefaultSet(ds, setDs, i, unsoData, rowNo, ediCtlNo) = False Then
                    ...
                    'ハネウェルは現段階ではワーニングが存在するが、エラーはなし。共通のロジックを組み込む為入れておく
                    Dim chkRowNo As Integer = Convert.ToInt32(rowNo)
                    If MyBase.IsMessageStoreExist(chkRowNo) = True Then
                        '整合性チェックでエラーがあった場合は処理終了
                        Return False
...
#Region "届先マスタチェック"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String _
        ...
        If mDestNm.Equals(ediDestNm) = True Then
            'チェックなし
        Else
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.HON_WID_L006, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(4) = "EDIデータ"
                If destHanyoFlg = True Then
                    msgArray(5) = "注意) この届先データは汎用届先コード(0000000001)を使用しています。"
                Else
                    msgArray(5) = String.Empty
                End If
                ds = Me._Blc.SetComWarningL("W197", LMH030BLC.HON_WID_L006, ds, msgArray, _
        ...
        'FREE_C21:届先住所(EDI値にマスタ値が含まれていなければワーニング)
        ...
        Else
            ...
            If InStr(ediFreeC21.ToString().Trim(), mAdAll.ToString().Trim()) = 0 Then

                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.HON_WID_L007, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    Else
                        msgArray(5) = String.Empty
                    End If
                    ds = Me._Blc.SetComWarningL("W197", LMH030BLC.HON_WID_L007, ds, msgArray, ediFreeC21, mAdAll)
        ...
        Else
            If mZip.Equals(Replace(ediZip, "-", String.Empty)) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.HON_WID_L008, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningL("W197", LMH030BLC.HON_WID_L008, ds, msgArray, ediZip, mZip)
        ...
        If String.IsNullOrEmpty(ediTel) = True _
           AndAlso compareWarningFlg = False Then
            If destHanyoFlg = False Then
            ...
            If mTel.Equals(ediTel) = False Then
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    Else
                        msgArray(5) = String.Empty
                    End If
                    ds = Me._Blc.SetComWarningL("W197", LMH030BLC.HON_WID_L008, ds, msgArray, ediTel, mTel)
        ...
        ElseIf String.IsNullOrEmpty(ediDestJisCd) = False AndAlso ediDestJisCd.Equals(mJis) = False Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.HON_WID_L011, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                Else
                    msgArray(5) = String.Empty
                End If
                ds = Me._Blc.SetComWarningL("W197", LMH030BLC.HON_WID_L011, ds, msgArray, ediDestJisCd, mJis)
        ...
        ElseIf String.IsNullOrEmpty(ediDestJisCd) = True AndAlso mZipJis.Equals(mJis) = False Then
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(5) = String.Empty
                ds = Me._Blc.SetComWarningL("W187", LMH030BLC.HON_WID_L011, ds, msgArray, mZipJis, mJis)
...
#Region "届先マスタ追加時チェック"
    ...
    Private Function ZipCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String, ByVal workDestCd As String, ByVal workDestString As String) As Boolean
        ...
        choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.HON_WID_L012, 0)
        ...
        If String.IsNullOrEmpty(choiceKb) = True Then
            ...
            ds = Me._Blc.SetComWarningL("W186", LMH030BLC.HON_WID_L012, ds, msgArray, workDestCd, String.Empty) '追加箇所 20110222
...
#Region "データセット設定(出荷L)"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            If (LMH030BLC105.EDI_DEST_CD_HANYO.ToString).Equals(ediDr("EDI_DEST_CD").ToString().Trim()) = True AndAlso _
               String.IsNullOrEmpty(ediDr("DEST_CD").ToString().Trim()) Then
                outkaDr("DEST_KB") = "01"
            Else
                outkaDr("DEST_KB") = "02"
            End If
...
#Region "データセット設定(EDI受信DTL)"
    ...
    Private Function SetDatasetEdiRcvDtl(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            'ハネウェル専用機能
            rcvDr("CUST_CD_L") = outkaedilDr("CUST_CD_L") '荷主コード(大)の設定
            rcvDr("CUST_CD_M") = outkaedilDr("CUST_CD_M") '荷主コード(中)の設定
            rcvDr("NRS_WH_CD") = outkaedilDr("WH_CD") '日陸倉庫コードの設定
