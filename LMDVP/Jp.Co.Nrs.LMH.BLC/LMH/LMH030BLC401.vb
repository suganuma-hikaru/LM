...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC401 = New LMH030DAC401()
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        'サクラ出荷梱包個数自動計算（出荷(大)の更新）
        ds = MyBase.CallDAC(Me._Dac, "SelectCustDtlAutoCalcFlg", ds)    '2019/01/25 ADD 要望管理004402
        If ds.Tables("LMH030_CALC_OUTKA_PKG_NB_IN").Rows.Count > 0 _
        AndAlso ds.Tables("LMH030_CALC_OUTKA_PKG_NB_IN").Rows(0).Item("AUTO_CALC_FLG").ToString() = "1" Then    '2019/01/25 MOD 要望管理004402
            Dim bErr As Boolean = False
            ds = Me.CalcOutkaPkgNbSakura(ds, bErr)
            If bErr Then
                'MyBase.SetMessageStoreでメッセージを設定するとコミットされなくなるため、メッセージは設定しない。
            End If
        End If
...
#Region "サクラ出荷梱包個数自動計算"
    ...
    ''' <returns>DataSet</returns>
    ''' <remarks></remarks>
    Private Function CalcOutkaPkgNbSakura(ByVal ds As DataSet, ByRef bErr As Boolean) As DataSet
        ...
        '自動計算した出荷梱包個数
        Dim outkaPkgNbCalc As Integer = Convert.ToInt32(ds.Tables("LMH030_CALC_OUTKA_PKG_NB_SAKURA").Rows(0).Item("OUTKA_PKG_NB"))

        '出荷(大)テーブルの出荷梱包個数
        Dim outkaPkgNbOrg As Integer = Convert.ToInt32(ds.Tables("LMH030_C_OUTKA_L").Rows(0).Item("OUTKA_PKG_NB"))

        If outkaPkgNbCalc <> outkaPkgNbOrg Then
            '差異があった場合のみ自動計算した出荷梱包個数で更新
            ds.Tables("LMH030_C_OUTKA_L").Rows(0).Item("OUTKA_PKG_NB") = ds.Tables("LMH030_CALC_OUTKA_PKG_NB_SAKURA").Rows(0).Item("OUTKA_PKG_NB")
