...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC109 = New LMH030DAC109()
...
#Region "美浜用CONST"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Const WH_CD_MHM As String = "101"                '倉庫コード
    Public Const CUST_CD_L_MHM As String = "00117"          '荷主コード（大）
    Public Const CUST_CD_M_MHM As String = "00"             '荷主コード（中）
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        '追加箇所 20120224 start
        '美浜はまとめ対象荷主(まとめFLAG = "0"を使用)
        '基本、自動まとめフラグは"0"なのでここには入らない
        ...
        If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
            ...
            ElseIf MyBase.GetResultCount > 1 Then
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.MHM_WID_L001, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningL("W199", LMH030BLC.MHM_WID_L001, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("0") = True Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.MHM_WID_L002, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = String.Empty
                    matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                    ds = Me._Blc.SetComWarningL("W168", LMH030BLC.MHM_WID_L002, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("1") = True Then
                ...
                If matomeStatus.Equals("10") = False Then
                    ...
                    choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.MHM_WID_L003, 0)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(4) = String.Empty
                        matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                        ds = Me._Blc.SetComWarningL("W198", LMH030BLC.MHM_WID_L003, ds, msgArray, matomeNo, String.Empty)
                        Return ds
...
#Region "EDI_Lの初期値設定(出荷登録処理)"
    ...
    Private Function SetEdiLShoki(ByVal ds As DataSet) As DataSet
        ...
        '要望番号:1624 terakawa 2012.11.26 Start
        '美浜特殊処理（届先名、住所、電話番号を届け先マスタからセット）
        If mDestFlgYN = True Then
            '届先名
            ediDr("DEST_NM") = mDestDr("DEST_NM").ToString().Trim()
            '届先住所1
            ediDr("DEST_AD_1") = mDestDr("AD_1").ToString().Trim()
            '届先住所2
            ediDr("DEST_AD_2") = mDestDr("AD_2").ToString().Trim()
            '届先電話番号
            ediDr("DEST_TEL") = mDestDr("TEL").ToString().Trim()

            '要望番号:1634 2012.11.28 追加START
            '届先JISコード
            ediDr("DEST_JIS_CD") = mDestDr("JIS").ToString().Trim()
            '要望番号:1634 2012.11.28 追加END

        End If
        '要望番号:1624 terakawa 2012.11.26 End
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.MHM_WID_M001, 0)
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    Else
                        ...
                        '2012.03.19 修正START
                        ds = Me._Blc.SetComWarningM("W162", LMH030BLC.MHM_WID_M001, ds, setDs, msgArray, custGoodsCd, String.Empty)
                        '2012.03.19 修正END
                ...
                If Me.EdiMDefaultSet(ds, setDs, i, unsoData, rowNo, ediCtlNo) = False Then
                    ...
                    '美浜は現段階ではワーニングが存在するが、エラーはなし。共通のロジックを組み込む為入れておく
                    Dim chkRowNo As Integer = Convert.ToInt32(rowNo)
                    If MyBase.IsMessageStoreExist(chkRowNo) = True Then
                        '整合性チェックでエラーがあった場合は処理終了
                        Return False
...
#Region "データセット設定(出荷L)"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            outkaDr("CUST_ORD_NO") = String.Concat(ediDr("FREE_C06"), "-", ediDr("CUST_ORD_NO"))
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        'Dim drEdiRcvHed As DataRow = ds.Tables("LMH030_OUTKAEDI_HED_MHM").NewRow()
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_OUTKAEDI_DTL_MHM").NewRow()
        ...
        'drEdiRcvDtl("CUST_CD_L") = CUST_CD_L_MHM                                                        '荷主コード（大）
        'drEdiRcvDtl("CUST_CD_M") = CUST_CD_M_MHM                                                        '荷主コード（中）
        drEdiRcvDtl("PRTFLG") = "0"                                                                     'プリントフラグ
        'drEdiRcvDtl("CANCEL_FLG") = "0"                                                                 'キャンセルフラグ
        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "1"                                                          '実績処理フラグ

        '荷主固有データ
        drEdiRcvDtl("ARR_PLAN_DATE") = Format(Convert.ToDateTime(drSetDtl("COLUMN_160")), "yyyyMMdd")   '納入期日
        drEdiRcvDtl("OUTKA_PLAN_DATE") = Format(Convert.ToDateTime(drSetDtl("COLUMN_161")), "yyyyMMdd") '出荷予定日
        drEdiRcvDtl("JYUCYU_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_145").ToString().Trim(), 12)          '受注NO
        drEdiRcvDtl("JYUCYU_NO_GYO") = Me._Blc.LeftB(drSetDtl("COLUMN_144").ToString().Trim(), 3)       '受注NO　行
        drEdiRcvDtl("M_TEKIYO") = Me._Blc.LeftB(drSetDtl("COLUMN_182").ToString().Trim(), 60)           '明細摘要
        drEdiRcvDtl("GOODS_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_104").ToString().Trim(), 50)           '品目
        drEdiRcvDtl("GOODS_REMARKS") = Me._Blc.LeftB(drSetDtl("COLUMN_168").ToString().Trim(), 60)      '品目備考
        drEdiRcvDtl("OUTKA_QT") = Me._Blc.LeftB(drSetDtl("COLUMN_128").ToString().Trim(), 15)           '数量
        drEdiRcvDtl("NISUGATA_QT") = Me._Blc.LeftB(drSetDtl("COLUMN_131").ToString().Trim(), 15)        '荷姿数量
        drEdiRcvDtl("NISUGATA_UT") = Me._Blc.LeftB(drSetDtl("COLUMN_127").ToString().Trim(), 20)        '荷姿単位
        drEdiRcvDtl("TOKUISAKI_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 100)       '得意先
        drEdiRcvDtl("NOUHINSAKI_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_100").ToString().Trim(), 100)     '納品先
        drEdiRcvDtl("NOUHINSAKI_BIKOU") = Me._Blc.LeftB(drSetDtl("COLUMN_164").ToString().Trim(), 60)   '納品先備考
        drEdiRcvDtl("OUTKA_IRAISYO") = Me._Blc.LeftB(drSetDtl("COLUMN_48").ToString().Trim(), 20)       '出荷依頼書
        drEdiRcvDtl("OUTKA_KBN") = Me._Blc.LeftB(drSetDtl("COLUMN_197").ToString().Trim(), 20)          '出荷区分
        drEdiRcvDtl("GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_103").ToString().Trim(), 20)           '品目ｺｰﾄﾞ
        drEdiRcvDtl("TOKUISAKI_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 20)        '得意先ｺｰﾄﾞ
        drEdiRcvDtl("NOUHINSAKI_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_99").ToString().Trim(), 20)       '納品先ｺｰﾄﾞ
        drEdiRcvDtl("NISUGATA_KBN") = Me._Blc.LeftB(drSetDtl("COLUMN_125").ToString().Trim(), 20)       '荷姿区分
        drEdiRcvDtl("OUTKA_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_85").ToString().Trim(), 20)            '出荷NO
        drEdiRcvDtl("OUTKA_NO_GYO") = Me._Blc.LeftB(drSetDtl("COLUMN_86").ToString().Trim(), 3)         '出荷Ｎo.行
        drEdiRcvDtl("OUTKA_SIJI_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_142").ToString().Trim(), 20)      '出荷指示NO
        drEdiRcvDtl("SIJI_GYO") = Me._Blc.LeftB(drSetDtl("COLUMN_144").ToString().Trim(), 3)            '出荷指示No.行
        drEdiRcvDtl("URIAGE_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_151").ToString().Trim(), 20)          '売上No.
        drEdiRcvDtl("URIAGE_GYO") = Me._Blc.LeftB(drSetDtl("COLUMN_153").ToString().Trim(), 3)          '売上No.行

        drEdiRcvDtl("OUTKA_DEN_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 20)         '出荷伝票No.(採番番号) DBには登録しないが、NEW_KEYとして使用する

        '修正開始 --- 2014.02.13 要望番号:2270
        drEdiRcvDtl("URIAGE_KEIJYO_BUMON") = Me._Blc.LeftB(drSetDtl("COLUMN_54").ToString().Trim(), 5)  '受注計上部門ｺｰﾄﾞ      DBには登録しないが、OUTKA_EDI_L登録時に使用する
        '修正終了 --- 2014.02.13 要望番号:2270

        'データセットに設定
        'ds.Tables("LMH030_OUTKAEDI_HED_MHM").Rows.Add(drEdiRcvHed)
        ds.Tables("LMH030_OUTKAEDI_DTL_MHM").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
        ...
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH030_OUTKAEDI_DTL_MHM").Rows(0)
        'Dim drRcvEdiHed As DataRow = setDs.Tables("LMH030_OUTKAEDI_HED_MHM").Rows(0)
        ...
        drOutkaEdiM("CUST_ORD_NO_DTL") = Me._Blc.LeftB(drRcvEdiDtl("JYUCYU_NO").ToString().Trim(), 30)
        drOutkaEdiM("BUYER_ORD_NO_DTL") = drRcvEdiDtl("M_TEKIYO").ToString
        ...
        Select Case drRcvEdiDtl("NISUGATA_KBN").ToString

            Case "単位計上"
                drOutkaEdiM("OUTKA_HASU") = drRcvEdiDtl("OUTKA_QT")         '出荷端数 
                drOutkaEdiM("OUTKA_QT") = 0                                 '出荷数量
                drOutkaEdiM("OUTKA_TTL_NB") = drRcvEdiDtl("OUTKA_QT")       '出荷総個数
                drOutkaEdiM("OUTKA_TTL_QT") = 0                             '出荷総数量

            Case "荷姿計上"
                drOutkaEdiM("OUTKA_HASU") = drRcvEdiDtl("NISUGATA_QT")      '出荷端数 
                drOutkaEdiM("OUTKA_QT") = 0                                 '出荷数量
                drOutkaEdiM("OUTKA_TTL_NB") = drRcvEdiDtl("NISUGATA_QT")    '出荷総個数
                drOutkaEdiM("OUTKA_TTL_QT") = 0                             '出荷総数量

            Case Else
                drOutkaEdiM("OUTKA_HASU") = 0                               '出荷端数 
                drOutkaEdiM("OUTKA_QT") = 0                                 '出荷数量
                drOutkaEdiM("OUTKA_TTL_NB") = 0                             '出荷総個数
                drOutkaEdiM("OUTKA_TTL_QT") = 0                             '出荷総数量
        End Select
        ...
        drOutkaEdiM("REMARK") = String.Empty                        '注意事項
        ...
        drOutkaEdiM("SET_KB") = "0"                                 '"0"：対象

        drOutkaEdiM("FREE_N01") = drRcvEdiDtl("OUTKA_QT")
        drOutkaEdiM("FREE_N02") = drRcvEdiDtl("NISUGATA_QT")
        ...
        drOutkaEdiM("FREE_C01") = drRcvEdiDtl("FILE_NAME")
        drOutkaEdiM("FREE_C02") = drRcvEdiDtl("REC_NO")
        drOutkaEdiM("FREE_C03") = drRcvEdiDtl("JYUCYU_NO")
        drOutkaEdiM("FREE_C04") = drRcvEdiDtl("JYUCYU_NO_GYO")
        drOutkaEdiM("FREE_C05") = drRcvEdiDtl("OUTKA_NO")
        drOutkaEdiM("FREE_C06") = drRcvEdiDtl("OUTKA_NO_GYO")
        drOutkaEdiM("FREE_C07") = drRcvEdiDtl("OUTKA_SIJI_NO")
        drOutkaEdiM("FREE_C08") = drRcvEdiDtl("SIJI_GYO")
        drOutkaEdiM("FREE_C09") = drRcvEdiDtl("URIAGE_NO")
        drOutkaEdiM("FREE_C10") = drRcvEdiDtl("URIAGE_GYO")
        drOutkaEdiM("FREE_C11") = drRcvEdiDtl("NISUGATA_KBN")
        drOutkaEdiM("FREE_C12") = drRcvEdiDtl("NISUGATA_UT")
        drOutkaEdiM("FREE_C13") = drRcvEdiDtl("GOODS_REMARKS")
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        '納入予定日
        If drRcvEdiDtl("OUTKA_PLAN_DATE").ToString = drRcvEdiDtl("ARR_PLAN_DATE").ToString Then
            'OUTKA_PLAN_DATEとARR_PLAN_DATEが同日の場合はシステム日付の翌日をセット
            Dim sDate As String = MyBase.GetSystemDate()
            drOutkaEdiL("ARR_PLAN_DATE") = Format(DateSerial(Convert.ToInt32(Left(sDate, 4)), Convert.ToInt32(Mid(sDate, 5, 2)), Convert.ToInt32(Right(sDate, 2))).AddDays(+1), "yyyyMMdd")
        Else
        ...
        drOutkaEdiL("EDI_DEST_CD") = drRcvEdiDtl("NOUHINSAKI_CD")
        drOutkaEdiL("DEST_CD") = drRcvEdiDtl("NOUHINSAKI_CD")
        drOutkaEdiL("DEST_NM") = Me._Blc.LeftB(drRcvEdiDtl("NOUHINSAKI_NM").ToString().Trim(), 60)
        ...
        drOutkaEdiL("CUST_ORD_NO") = drRcvEdiDtl("JYUCYU_NO")
        drOutkaEdiL("BUYER_ORD_NO") = drRcvEdiDtl("M_TEKIYO")           '買主注文番号（全体）
        ...
        ''注意事項
        drOutkaEdiL("REMARK") = drRcvEdiDtl("NOUHINSAKI_BIKOU")
        drOutkaEdiL("UNSO_ATT") = drRcvEdiDtl("TOKUISAKI_NM")
        ...
        drOutkaEdiL("FREE_C02") = drRcvEdiDtl("REC_NO")
        drOutkaEdiL("FREE_C03") = drRcvEdiDtl("JYUCYU_NO")
        drOutkaEdiL("FREE_C04") = drRcvEdiDtl("OUTKA_NO")
        drOutkaEdiL("FREE_C05") = drRcvEdiDtl("OUTKA_SIJI_NO")
        drOutkaEdiL("FREE_C06") = drRcvEdiDtl("URIAGE_KEIJYO_BUMON")
        ...
        drOutkaEdiL("FREE_C20") = CUST_CD_L_MHM & CUST_CD_M_MHM
...
#Region "セミEDI時　商品マスタからCustCd等を取得する"
    ...
    Private Function GetCustCd(ByVal ds As DataSet _
        ...
        Select Case iMstGoodsCnt
            ...
            Case 0      '商品マスタ取得０件
                '荷主は美浜とする
                sCustCdL = CUST_CD_L_MHM
                sCustCdM = CUST_CD_M_MHM
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiHed As DataTable = ds.Tables("LMH030_OUTKAEDI_HED_MHM")
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH030_OUTKAEDI_DTL_MHM")
        Dim drRcvEdiDtl As DataRow = ds.Tables("LMH030_OUTKAEDI_DTL_MHM").Rows(0)
        Dim sNrsBrCd As String = ds.Tables("LMH030_OUTKAEDI_DTL_MHM").Rows(0).Item("NRS_BR_CD").ToString()
        ...
        '    Dim dtRcvHedDel As DataTable = ds.Tables("LMH030_DTL_MHM_CANCELOUT")
        '    Dim drRcvHedDel As DataRow = ds.Tables("LMH030_DTL_MHM_CANCELOUT").Rows(0)

        '    Dim dtRcvDtlDel As DataTable = ds.Tables("LMH030_DTL_MHM_CANCELOUT")
        '    Dim drRcvDtlDel As DataRow = ds.Tables("LMH030_DTL_MHM_CANCELOUT").Rows(0)
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow, ByVal ediCustIndex As String) As Boolean
        ...
        sMsg = "納入期日(カラム160番目)["
        sDate = dr.Item("COLUMN_160").ToString()
        ...
        sMsg = "出荷予定日(カラム161番目)["
        sDate = dr.Item("COLUMN_161").ToString()
        'sDate = Me._Blc.GetSlashEditDate(sDate)    'スラッシュ付日付に編集
        If IsDate(sDate) = True Then
        Else
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat(sMsg, sDate, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
        ...
        sMsg = "受注NO行(カラム144番目)["
        sNum = dr.Item("COLUMN_144").ToString().Trim()
        If String.IsNullOrEmpty(sNum) = True Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_144") = 0
        Else
        ...
        sMsg = "数量(カラム128番目)["
        sNum = dr.Item("COLUMN_128").ToString().Trim()
        If String.IsNullOrEmpty(sNum) = True Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_128") = 0
        Else
            If IsNumeric(sNum) Then
                '数値の場合
                dNum = Convert.ToDouble(sNum)
                If dNum > 99999999999.99 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat(sMsg, dNum.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If
            Else
        ...
        sMsg = "荷姿数量(カラム131番目)["
        sNum = dr.Item("COLUMN_131").ToString().Trim()
        If String.IsNullOrEmpty(sNum) = True Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_131") = 0
        Else
            If IsNumeric(sNum) Then
                '数値の場合
                dNum = Convert.ToDouble(sNum)
        ...
        sMsg = "出荷Ｎo.行(カラム86番目)["
        sNum = dr.Item("COLUMN_86").ToString().Trim()
        If String.IsNullOrEmpty(sNum) = True Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_86") = 0
        Else
        ...
        sMsg = "売上No.行(カラム153番目)["
        sNum = dr.Item("COLUMN_153").ToString().Trim()
        If String.IsNullOrEmpty(sNum) = True Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_153") = 0
        Else
            If IsNumeric(sNum) Then
                '数値の場合
                dNum = Convert.ToDouble(sNum)
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        'Dim dtRcvHed As DataTable = ds.Tables("LMH030_OUTKAEDI_HED_MHM")         'EDI受信Hed
        Dim dtRcvDtl As DataTable = ds.Tables("LMH030_OUTKAEDI_DTL_MHM")         'EDI受信Dtl
        Dim dtRcvDtlCancel As DataTable = ds.Tables("LMH030_DTL_MHM_CANCELOUT")  'EDI受信Hed
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            ds.Tables("LMH030_OUTKAEDI_DTL_MHM").Clear() '受信DTLをクリア
            ds = Me.SetSemiOutkaEdiRcv(ds, i)
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_OUTKAEDI_DTL_MHM").Rows(0)
            ...
            sWhcd = WH_CD_MHM   '倉庫コードは既定値使用

            '---------------------------------------------------------------------------
            ' OUTKA_EDI_Mを検索して、既に同一番号が存在する場合はスルーする
            '---------------------------------------------------------------------------
            ...
            ' キー項目設定
            '---------------------------------------------------------------------------
            sNewKey = drEdiRcvDtl.Item("OUTKA_DEN_NO").ToString
            ...
            ''    '受信DTL取消データ取得処理
            ''    ds.Tables("LMH030_DTL_MHM_CANCELOUT").Clear()    '取得用DSをクリア
            ...
            ''        Dim drRcvDtlCancel As DataRow = ds.Tables("LMH030_DTL_MHM_CANCELOUT").Rows(0)
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH030_OUTKAEDI_DTL_MHM")
            setDtlDt.Clear()
