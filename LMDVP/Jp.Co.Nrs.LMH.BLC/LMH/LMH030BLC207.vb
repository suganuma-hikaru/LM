...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC207 = New LMH030DAC207()
...
#Region "ジャパンコンポジット用CONST"

    ''' <summary>
    ''' ジャパンコンポジット
    ''' </summary>
    ''' <remarks></remarks>
    Public Const WH_CD_JC As String = "200"                 '倉庫コード
    Public Const CUST_CD_L_JC As String = "31022"           '荷主コード（大）
    Public Const CUST_CD_M_JC As String = "00"              '荷主コード（中）

    ''' <summary>
    ''' ジャパンコンポジット大泰化工
    ''' </summary>
    ''' <remarks></remarks>
    Public Const WH_CD_TAITAI As String = "202"             '倉庫コード
    Public Const CUST_CD_L_TAITAI As String = "31022"       '荷主コード（大）
    Public Const CUST_CD_M_TAITAI As String = "01"          '荷主コード（中）

    '馬野さん指示により"T0000999999999999999"→""に変更
    'Public Const NRS_GOODS_CD_TAITAI As String = "T0000999999999999999"
    Public Const NRS_GOODS_CD_TAITAI As String = ""         '商品コード
    Public Const IRIME_TAITAI As String = "0"               '入目値

    Public Const SOUKO_CD_TAITAI As String = "1050"         '受信データの倉庫コード（大泰化工）

    ''' <summary>
    ''' アイカ工業
    ''' </summary>
    ''' <remarks></remarks>
    Public Const WH_CD_AIKA As String = "200"               '倉庫コード
    Public Const CUST_CD_L_AIKA As String = "31023"         '荷主コード（大）
    Public Const CUST_CD_M_AIKA As String = "01"            '荷主コード（中）
...
#Region "汎用商品コード(JC大秦化工)CONST"

    ''' <summary>
    ''' 汎用商品コード(JC大秦化工)(商品キー)
    ''' </summary>
    ''' <remarks></remarks>
    Public Const GENERIC_GOODS_CD_NRS As String = "T0000999999999999999"

    ''' <summary>
    ''' 汎用商品コード(JC大秦化工)(荷主商品コード)
    ''' </summary>
    ''' <remarks></remarks>
    Public Const GENERIC_GOODS_CD_CUST As String = "31022001"
...
#Region "出荷登録：JC,アイカの場合(運送登録：JC大秦化工の場合)処理"
    ''' <summary>
    ''' 出荷登録処理
    ''' </summary>
    ''' <returns></returns>
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        '出荷管理番号(大)の採番
        '2012.03.20 JC大秦化工用 修正START
        'JC大秦化工の場合は運送登録なので、出荷は作成しない
        'If ediCustIdx.Equals("42") = False Then
        'If unsoFlg.Equals("1") = False Then
        ds = Me.GetOutkaNoL(ds, matomeFlg)
        'End If
        '2012.03.20 JC大秦化工用 修正END
        ...
        '2012.03.20 JC大秦化工用 修正START
        '注意)JC大秦化工の場合は運送登録なので、出荷は作成しないがデータセット設定は行う
...
#Region "EDI_Lの初期値設定(出荷登録:運送登録処理)"
    ''' <summary>
    ''' EDI_Lの初期設定
    ''' </summary>
    ''' <param name="ds"></param>
...
#Region "EDI出荷(中)の初期値設定"
    ...
    Private Function EdiMDefaultSet(ByVal ds As DataSet, ByVal setDs As DataSet, _
        ...
        '商品KEY
        '2012.04.02 修正START JC大秦化工(注意事項)
        'JC大秦化工の場合、通常の出荷データと同じで商品キーは商品マスタの値に入れ替え
        ...
        'JC大秦化工の場合、通常の出荷データと同じで以下の４項目は商品マスタの値に入れ替え
        '↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
        '個数単位
        ...
        Else
            '2012.04.02 修正START
            If (ediMDr("NRS_GOODS_CD").ToString()).Equals(LMH030BLC207.GENERIC_GOODS_CD_NRS) = False AndAlso ediMDr("IRIME_UT").Equals(mGoodsDr("STD_IRIME_UT")) = False Then
                '運送データで、EDI(中)の商品コードが汎用商品コード("T0000999999999999999")と商品マスタで入目単位が異なる場合、エラー
        ...
        '個別重量(KGS)
        '2012.03.27 修正START
        'JC大秦化工の場合、通常の出荷データと同じで商品マスタの値に入れ替え
        '現LMSと異なり、この時点で重量計算を行う為
        'If unsodata.Equals("01") = False Then
        If (ediMDr("NRS_GOODS_CD").ToString()).Equals(LMH030BLC207.GENERIC_GOODS_CD_NRS) = False Then
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.JPNCOMP_WID_M001, 0)
                ...
                If MyBase.GetResultCount = 0 Then
                    '2012.03.27 JC大秦化工対応 修正START
                    'JC大秦化工の場合、荷主商品コードが商品マスタに存在しない場合は、汎用商品コードを使用する
                    If unsoData.Equals("01") = True Then

                        choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.JPNCOMP_WID_M002, 0)

                        If String.IsNullOrEmpty(choiceKb) = True Then

                            msgArray(1) = "商品コード"
                            msgArray(2) = "商品マスタ"
                            msgArray(3) = String.Concat("(荷主)汎用商品コード", "：(", LMH030BLC207.GENERIC_GOODS_CD_CUST, ")")
                            msgArray(4) = "運送登録"
                            msgArray(5) = String.Empty

                            ds = Me._Blc.SetComWarningM("W202", LMH030BLC.JPNCOMP_WID_M002, ds, setDs, msgArray, custGoodsCd, String.Empty)
                        ...
                        ElseIf choiceKb.Equals("01") = True Then
                            ...
                            setDs.Tables("LMH030_OUTKAEDI_M").Rows(0).Item("NRS_GOODS_CD") = LMH030BLC207.GENERIC_GOODS_CD_NRS
                            setDs = (MyBase.CallDAC(Me._DacCom, "SelectDataMgoodsOutka", setDs))
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    Else
                        ...
                        ds = Me._Blc.SetComWarningM("W162", LMH030BLC.JPNCOMP_WID_M001, ds, setDs, msgArray, custGoodsCd, String.Empty)
                ...
                If Me.EdiMDefaultSet(ds, setDs, i, unsoData, rowNo, ediCtlNo) = False Then
                    ...
                    '2012.03.01 大阪対応START
                    'JC,JC大秦化工,アイカは現段階ではワーニングはないが、共通のロジックを組み込む為入れておく
                    Dim chkRowNo As Integer = Convert.ToInt32(rowNo)
                    If MyBase.IsMessageStoreExist(chkRowNo) = True Then
                        '整合性チェックでエラーがあった場合は処理終了
                        Return False
...
#Region "届先マスタチェック(ジャパンコンポジット、大秦化工、アイカ専用)"
    ''' <summary>
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        '    'チェックなし
        'Else
        '    choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.JPNCOMP_WID_LXXX, 0)
        ...
        '        msgArray(4) = "EDIデータ"
        '        msgArray(5) = String.Empty
        '        ds = Me._Blc.SetComWarningL("W166", LMH030BLC.JPNCOMP_WID_LXXX, ds, msgArray, ediDestNm, mDestNm)
        ...
        '            msgArray(4) = "EDIデータ"
        '            msgArray(5) = String.Empty
        '            ds = Me._Blc.SetComWarningL("W166", LMH030BLC.JPNCOMP_WID_LXXX, ds, msgArray, ediFreeC21, mAdAll)
        ...
        Else
            If mZip.Equals(Replace(ediZip, "-", String.Empty)) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.JPNCOMP_WID_L001, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.JPNCOMP_WID_L001, ds, msgArray, ediZip, mZip)
        ...
        Else
            If mTel.Equals(ediTel) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.JPNCOMP_WID_L002, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.JPNCOMP_WID_L002, ds, msgArray, ediTel, mTel)
        ...
        If String.IsNullOrEmpty(mZipJis) = True AndAlso String.IsNullOrEmpty(ediDestJisCd) = False Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.JPNCOMP_WID_L003, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                ds = Me._Blc.SetComWarningL("W197", LMH030BLC.JPNCOMP_WID_L003, ds, msgArray, ediDestJisCd, mJis)
        ...
        ElseIf String.IsNullOrEmpty(mZipJis) = True AndAlso String.IsNullOrEmpty(ediDestJisCd) = True Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.JPNCOMP_WID_L004, 0)

            If String.IsNullOrEmpty(choiceKb) = True Then

                ds = Me._Blc.SetComWarningL("W188", LMH030BLC.JPNCOMP_WID_L004, ds, msgArray, ediDestJisCd, mJis)
        ...
        ElseIf String.IsNullOrEmpty(ediDestJisCd) = False AndAlso ediDestJisCd.Equals(mJis) = False Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.JPNCOMP_WID_L005, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(4) = "EDIデータ"
                msgArray(5) = String.Empty
                ds = Me._Blc.SetComWarningL("W187", LMH030BLC.JPNCOMP_WID_L005, ds, msgArray, ediDestJisCd, mJis)
        ...
        ElseIf String.IsNullOrEmpty(ediDestJisCd) = True AndAlso mZipJis.Equals(mJis) = False Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.JPNCOMP_WID_L006, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(5) = String.Empty
                ds = Me._Blc.SetComWarningL("W187", LMH030BLC.JPNCOMP_WID_L006, ds, msgArray, mZipJis, mJis)
...
#Region "届先マスタ追加時チェック"
    ...
    Private Function ZipCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String, ByVal workDestCd As String, ByVal workDestString As String) As Boolean
        ...
        choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.JPNCOMP_WID_L007, 0)
        ...
        If String.IsNullOrEmpty(choiceKb) = True Then
            ...
            ds = Me._Blc.SetComWarningL("W186", LMH030BLC.JPNCOMP_WID_L007, ds, msgArray, workDestCd, String.Empty) '追加箇所 20110222
        ...
        ElseIf choiceKb.Equals("01") = True Then
            ...
            '2012.03.20 修正START JCは備考で使用したい為、空白
...
#Region "データセット設定(出荷管理番号L,運送番号(運送登録の場合))"

    ''' <summary>
    ''' データセット設定(出荷管理番号L,運送番号(運送登録の場合))
    ''' </summary>
    ''' <param name="ds"></param>
...
#Region "データセット設定(運送L)"
    ...
    Private Function SetDatasetUnsoL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            '2012.03.20 JC大秦化工用 修正START
            'JC大秦化工の場合は運送登録なので、前の処理でFREE_C30で取得した運送番号を使用
            ...
            '2012.04.05 JC関連荷主対応 修正START
            'JC,アイカの場合は空、JC大泰化工の場合は荷主注文番号をセット
            If unsoFlg.Equals("1") = True Then
                unsoDr("DENP_NO") = ediDr("CUST_ORD_NO")
            Else
                unsoDr("DENP_NO") = String.Empty
            End If
            '2012.04.05 JC関連荷主対応 修正END
            ...
            '2012.03.20 JC大秦化工用 修正START
            'JC大秦化工の場合は運送登録なので、元データ区分が変わる
            'If ediCustIdx.Equals("42") = True Then
            If unsoFlg.Equals("1") = True Then
...
#Region "データセット設定(運送M)"
    ...
    Private Function SetDatasetUnsoM(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            '2012.03.20 JC大秦化工用 修正START
            'JC大秦化工の場合は運送登録なので、出荷管理番号(中)が存在しないので採番する
            If unsoFlg.Equals("1") = True Then
                '運送登録処理の場合
            ...
            '2012.03.20 JC大秦化工用 修正START
            'JC大秦化工の場合は運送登録なので、運送温度区分が"90"の場合は入替
            If unsoFlg.Equals("1") = True Then
...
#Region "データセット設定(EDI出荷M：運送重量必要項目)"
    ...
    Private Function SetDatasetEdiMUnsoJyuryo(ByVal ds As DataSet, ByVal setDs As DataSet, ByVal count As Integer, _
        ...
        Dim ediCustIdx As String = ds.Tables("LMH030INOUT").Rows(0).Item("EDI_CUST_INDEX").ToString()
        Dim goodsCdNrs As String = mGoodsDr("GOODS_CD_NRS").ToString()
        '2012.03.27 大阪対応END
        ...
        '2012.03.27 大阪暫定対応START
        'If ediCustIdx.Equals("42") = False Then

        If goodsCdNrs.Equals(LMH030BLC207.GENERIC_GOODS_CD_NRS) = False Then
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        Dim drEdiRcvHed As DataRow = ds.Tables("LMH030_OUTKAEDI_HED_JC").NewRow()
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_OUTKAEDI_DTL_JC").NewRow()
        ...
        ''2015.09.08 要望対応2402 修正START
        'drEdiRcvHed("NONYU_AD_2") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 40)      '納入先住所２
        drEdiRcvHed("NONYU_AD_2") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 50)      '納入先住所２
        ''2015.09.08 要望対応2402 修正END

        '要望番号1021対応 2012/05/14 honmyo Start
        'drEdiRcvHed("NONYU_TEL") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 15)       '納入先電話番号
        ...
        drEdiRcvDtl("DENP_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_15").ToString().Trim(), 10)         '受注伝票番号
        drEdiRcvDtl("GYO_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_16").ToString().Trim(), 4)           '行番号
        drEdiRcvDtl("GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_17").ToString().Trim(), 20)        '商品コード
        drEdiRcvDtl("GOODS_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_18").ToString().Trim(), 40)        '商品名
        drEdiRcvDtl("QT") = Me._Blc.LeftB(drSetDtl("COLUMN_19").ToString().Trim(), 15)              '数量
        drEdiRcvDtl("QT_UT") = Me._Blc.LeftB(drSetDtl("COLUMN_20").ToString().Trim(), 6)            '単位名
        drEdiRcvDtl("NOUKI") = Me._Blc.LeftB(drSetDtl("COLUMN_21").ToString().Trim(), 8)            '受注納期
        drEdiRcvDtl("ORDER_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_22").ToString().Trim(), 20)        '注文番号
        drEdiRcvDtl("LOT") = Me._Blc.LeftB(drSetDtl("COLUMN_23").ToString().Trim(), 40)             'ロット番号
        drEdiRcvDtl("REMARK") = Me._Blc.LeftB(drSetDtl("COLUMN_24").ToString().Trim(), 40)          '明細摘要
        drEdiRcvDtl("DENP_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_25").ToString().Trim(), 8)          '指定伝票名
        drEdiRcvDtl("IRISU") = Me._Blc.LeftB(drSetDtl("COLUMN_26").ToString().Trim(), 13)           '入数
        drEdiRcvDtl("TANKA") = Me._Blc.LeftB(drSetDtl("COLUMN_27").ToString().Trim(), 16)           '発注単価
        drEdiRcvDtl("KINGAKU") = Me._Blc.LeftB(drSetDtl("COLUMN_28").ToString().Trim(), 12)         '発注金額
        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "1"                                                      '実績処理フラグ
        'データセットに設定
        ds.Tables("LMH030_OUTKAEDI_HED_JC").Rows.Add(drEdiRcvHed)
        ds.Tables("LMH030_OUTKAEDI_DTL_JC").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
        ...
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH030_OUTKAEDI_DTL_JC").Rows(0)
        Dim drRcvEdiHed As DataRow = setDs.Tables("LMH030_OUTKAEDI_HED_JC").Rows(0)
        ...
        drOutkaEdiM("CUST_ORD_NO_DTL") = drRcvEdiDtl("DENP_NO")
        If sCustCdL = CUST_CD_L_JC Then
            drOutkaEdiM("BUYER_ORD_NO_DTL") = drRcvEdiDtl("ORDER_NO")
        Else
            drOutkaEdiM("BUYER_ORD_NO_DTL") = drRcvEdiDtl("ORDER_NO").ToString & Left(drRcvEdiDtl("REMARK").ToString, 5)
        End If
        ...
        drOutkaEdiM("LOT_NO") = drRcvEdiDtl("LOT")
        ...
        '出荷総個数
        '出荷総数量
        Dim dQt As Double = Convert.ToDouble(drRcvEdiDtl("QT").ToString)
        Dim dIrisu As Double = Convert.ToDouble(drRcvEdiDtl("IRISU").ToString)
        Dim dIrime As Double = Convert.ToDouble(sIrime)

        If dIrisu = 0 Then
            If dIrime = 0 Then
                drOutkaEdiM("OUTKA_HASU") = dQt
                drOutkaEdiM("OUTKA_QT") = 0
                drOutkaEdiM("OUTKA_TTL_NB") = dQt
                drOutkaEdiM("OUTKA_TTL_QT") = 0
            Else
                drOutkaEdiM("OUTKA_HASU") = dQt
                drOutkaEdiM("OUTKA_QT") = dQt * dIrime
                drOutkaEdiM("OUTKA_TTL_NB") = dQt
                drOutkaEdiM("OUTKA_TTL_QT") = dQt * dIrime
            End If
        Else
            drOutkaEdiM("OUTKA_HASU") = dQt
            drOutkaEdiM("OUTKA_QT") = dQt * dIrisu
            drOutkaEdiM("OUTKA_TTL_NB") = dQt
            drOutkaEdiM("OUTKA_TTL_QT") = dQt * dIrisu
        End If
        ...
        Else
            drOutkaEdiM("REMARK") = Mid(drRcvEdiDtl("REMARK").ToString, 7)
        End If
        ...
        drOutkaEdiM("FREE_N01") = drRcvEdiDtl("GYO_NO")
        drOutkaEdiM("FREE_N02") = drRcvEdiDtl("QT")
        drOutkaEdiM("FREE_N03") = drRcvEdiDtl("IRISU")
        drOutkaEdiM("FREE_N04") = drRcvEdiDtl("TANKA")
        drOutkaEdiM("FREE_N05") = drRcvEdiDtl("KINGAKU")
        drOutkaEdiM("FREE_N06") = sIrime
        drOutkaEdiM("FREE_N07") = 0
        drOutkaEdiM("FREE_N08") = 0
        drOutkaEdiM("FREE_N09") = 0
        drOutkaEdiM("FREE_N10") = 0
        drOutkaEdiM("FREE_C01") = drRcvEdiDtl("QT_UT")
        drOutkaEdiM("FREE_C02") = drRcvEdiDtl("NOUKI")
        drOutkaEdiM("FREE_C03") = drRcvEdiDtl("DENP_NM")

        Select Case drRcvEdiHed("SOUKO_CD").ToString
            Case SOUKO_CD_TAITAI
                '運送時の商品荷主コード
                drOutkaEdiM("FREE_C04") = String.Empty
                drOutkaEdiM("FREE_C05") = String.Empty
            Case Else
                drOutkaEdiM("FREE_C04") = sCustCdL
                drOutkaEdiM("FREE_C05") = sCustCdM
        End Select

        drOutkaEdiM("FREE_C06") = sNrsGoodsCd
        drOutkaEdiM("FREE_C07") = drRcvEdiDtl("ORDER_NO")
        drOutkaEdiM("FREE_C08") = drRcvEdiDtl("REMARK")
        ...
        Select Case drRcvEdiHed("SOUKO_CD").ToString
            Case SOUKO_CD_TAITAI
                '運送管理番号の既定値（DTL）
                drOutkaEdiM("FREE_C30") = "01-T00000000000"
            Case Else
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        'drOutkaEdiL("OUTKA_KB") = String.Empty
        ...
        '出荷予定日
        drOutkaEdiL("OUTKA_PLAN_DATE") = drRcvEdiHed("OUTKA_BI")
        '出庫日
        drOutkaEdiL("OUTKO_DATE") = drRcvEdiHed("OUTKA_BI")
        '納入予定日
        drOutkaEdiL("ARR_PLAN_DATE") = drRcvEdiDtl("NOUKI")
        '納入予定時刻
        Dim sRemark As String = drRcvEdiDtl("REMARK").ToString
        ...
        drOutkaEdiL("EDI_DEST_CD") = drRcvEdiHed("NONYU_CD")
        drOutkaEdiL("DEST_CD") = drRcvEdiHed("NONYU_CD")
        drOutkaEdiL("DEST_NM") = drRcvEdiHed("NONYU_NM")
        drOutkaEdiL("DEST_ZIP") = drRcvEdiHed("NONYU_ZIP").ToString.Replace("-", "")
        drOutkaEdiL("DEST_AD_1") = Me._Blc.LeftB(drRcvEdiHed("NONYU_AD_1").ToString(), 40)
        drOutkaEdiL("DEST_AD_2") = Me._Blc.MidB(drRcvEdiHed("NONYU_AD_1").ToString(), 41)   '41バイト目以降すべて
        ''2015.09.08 要望対応2402 修正START
        'drOutkaEdiL("DEST_AD_3") = Me._Blc.LeftB(drRcvEdiHed("NONYU_AD_2").ToString(), 40)
        drOutkaEdiL("DEST_AD_3") = Me._Blc.LeftB(drRcvEdiHed("NONYU_AD_2").ToString(), 50)
        ''2015.09.08 要望対応2402 修正END
        drOutkaEdiL("DEST_AD_4") = String.Empty
        drOutkaEdiL("DEST_AD_5") = String.Empty
        drOutkaEdiL("DEST_TEL") = drRcvEdiHed("NONYU_TEL")
        ...
        drOutkaEdiL("CUST_ORD_NO") = drRcvEdiHed("DENP_NO")

        If sCustCdL = CUST_CD_L_JC Then
            drOutkaEdiL("BUYER_ORD_NO") = drRcvEdiDtl("ORDER_NO")
        Else
            drOutkaEdiL("BUYER_ORD_NO") = drRcvEdiDtl("ORDER_NO").ToString & Left(drRcvEdiDtl("REMARK").ToString, 5)
        End If
        ...
        Else
            drOutkaEdiL("REMARK") = Mid(drRcvEdiDtl("REMARK").ToString, 7)
        End If
        ...
        drOutkaEdiL("FREE_C01") = drRcvEdiHed("HAKKO_KB_NM")
        drOutkaEdiL("FREE_C02") = drRcvEdiHed("OUT_KB_NM")
        drOutkaEdiL("FREE_C03") = drRcvEdiHed("JIGYO_CD")
        drOutkaEdiL("FREE_C04") = drRcvEdiHed("JIGYO_NM")
        drOutkaEdiL("FREE_C05") = drRcvEdiHed("SOUKO_CD")
        drOutkaEdiL("FREE_C06") = drRcvEdiHed("SOUKO_NM")
        drOutkaEdiL("FREE_C07") = drRcvEdiHed("TORIKESHI_KB_NM")
        ...
        drOutkaEdiL("FREE_C20") = CUST_CD_L_JC & CUST_CD_M_JC
        drOutkaEdiL("FREE_C21") = drRcvEdiHed("NONYU_AD_1").ToString & drRcvEdiHed("NONYU_AD_2").ToString
        drOutkaEdiL("FREE_C22") = drRcvEdiHed("NONYU_AD_1")
        drOutkaEdiL("FREE_C23") = drRcvEdiHed("NONYU_AD_2")
        ...
        Select Case drRcvEdiHed("SOUKO_CD").ToString
            Case SOUKO_CD_TAITAI
                '運送管理番号の既定値（HED）
                drOutkaEdiL("FREE_C30") = "01-T00000000"
            Case Else
...
#Region "セミEDI時　商品マスタからCustCd等を取得する"
    ...
    Private Function GetCustCd(ByVal ds As DataSet _
        ...
        Select Case iMstGoodsCnt
            ...
            Case 0      '商品マスタ取得０件
                '荷主はジャパンコンポジットとする
                sCustCdL = CUST_CD_L_JC
                sCustCdM = CUST_CD_M_JC
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiHed As DataTable = ds.Tables("LMH030_OUTKAEDI_HED_JC")
        Dim drRcvEdiHed As DataRow = ds.Tables("LMH030_OUTKAEDI_HED_JC").Rows(0)
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH030_OUTKAEDI_DTL_JC")
        Dim drRcvEdiDtl As DataRow = ds.Tables("LMH030_OUTKAEDI_DTL_JC").Rows(0)
        Dim sNrsBrCd As String = ds.Tables("LMH030_OUTKAEDI_DTL_JC").Rows(0).Item("NRS_BR_CD").ToString()
        ...
        '    Dim dtRcvHedDel As DataTable = ds.Tables("LMH030_HED_JC_CANCELOUT")
        '    Dim drRcvHedDel As DataRow = ds.Tables("LMH030_DTL_JC_CANCELOUT").Rows(0)

        '    Dim dtRcvDtlDel As DataTable = ds.Tables("LMH030_DTL_JC_CANCELOUT")
        '    Dim drRcvDtlDel As DataRow = ds.Tables("LMH030_DTL_JC_CANCELOUT").Rows(0)
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow, ByVal ediCustIndex As String) As Boolean
        ...
        '出荷日(カラム7番目)
        Dim sOutkaDate As String = dr.Item("COLUMN_7").ToString()
        Dim sOutkaDateWithSlash As String = Me._Blc.GetSlashEditDate(sOutkaDate) 'スラッシュ付加

        If IsDate(sOutkaDateWithSlash) = True Then
        Else
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat("出荷日(カラム7番目)[", sOutkaDate, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        '行番号(カラム16番目)
        Dim sGyoNo As String = dr.Item("COLUMN_16").ToString().Trim()

        If String.IsNullOrEmpty(sGyoNo) = True Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_63") = 0
        Else
            If IsNumeric(sGyoNo) = True Then
                '数値の場合
                Dim iGyoNo As Integer = Convert.ToInt32(sGyoNo)

                If iGyoNo > 9999 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("行番号(カラム16番目)[", iGyoNo.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If
            Else
                '数値でない場合
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("行番号(カラム16番目)[", sGyoNo, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
        End If
        ...
        '受注納期(カラム21番目)
        Dim sNouki As String = dr.Item("COLUMN_21").ToString()
        Dim sNoukiWithSlash As String = Me._Blc.GetSlashEditDate(sOutkaDate) 'スラッシュ付加

        If IsDate(sNoukiWithSlash) = True Then
        Else
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat("受注納期(カラム21番目)[", sNouki, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        '入数(カラム26番目)
        Dim sIRISU As String = dr.Item("COLUMN_26").ToString().Trim()
        If String.IsNullOrEmpty(sIRISU) = True Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_26") = 0
        Else
            If IsNumeric(sIRISU) Then
                '数値の場合
                Dim dIRISU As Double = Convert.ToDouble(sIRISU)
                If dIRISU > 9999999999.999 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("入数(カラム26番目)[", dIRISU.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If
            Else
                '数値でない場合
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("入数(カラム26番目)[", sIRISU.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            End If
        End If

        '発注単価(カラム27番目)
        Dim sTanka As String = dr.Item("COLUMN_27").ToString().Trim()
        If String.IsNullOrEmpty(sTanka) = True Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_27") = 0
        Else
            If IsNumeric(sTanka) Then
                '数値の場合
                Dim dTanka As Double = Convert.ToDouble(sTanka)
                If dTanka > 999999999999.999 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("発注単価(カラム27番目)[", dTanka.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If
            Else
                '数値でない場合
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("発注単価(カラム27番目)[", sTanka.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            End If
        End If

        '発注金額(カラム28番目)
        Dim sKingaku As String = dr.Item("COLUMN_28").ToString().Trim()
        If String.IsNullOrEmpty(sKingaku) = True Then
            '空の場合はゼロをセット
            dr.Item("COLUMN_28") = 0
        Else
            If IsNumeric(sKingaku) Then
                '数値の場合
                Dim dKingaku As Double = Convert.ToDouble(sKingaku)
                If dKingaku > 999999999999 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("発注金額(カラム28番目)[", dKingaku.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If
            Else
                '数値でない場合
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("発注金額(カラム28番目)[", sKingaku.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            End If
        End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvHed As DataTable = ds.Tables("LMH030_OUTKAEDI_HED_JC")         'EDI受信Hed
        Dim dtRcvDtl As DataTable = ds.Tables("LMH030_OUTKAEDI_DTL_JC")         'EDI受信Dtl
        Dim dtRcvHedCancel As DataTable = ds.Tables("LMH030_HED_JC_CANCELOUT")  'EDI受信Hed
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            ds.Tables("LMH030_OUTKAEDI_HED_JC").Clear() '受信HEDをクリア
            ds.Tables("LMH030_OUTKAEDI_DTL_JC").Clear() '受信DTLをクリア
            ds = Me.SetSemiOutkaEdiRcv(ds, i)

            Dim drEdiRcvHed As DataRow = ds.Tables("LMH030_OUTKAEDI_HED_JC").Rows(0)
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_OUTKAEDI_DTL_JC").Rows(0)
            ...
            Select Case drEdiRcvHed.Item("SOUKO_CD").ToString
                ...
                Case SOUKO_CD_TAITAI     '運送指示データの場合
                    ...
                    sNrsGoodsCd = NRS_GOODS_CD_TAITAI
                    sNrsGoodsNm = String.Empty
                    sIrime = "0"
            ...
            ' キー項目設定
            '---------------------------------------------------------------------------
            sNewKey = String.Concat(drEdiRcvHed.Item("SOUKO_CD").ToString _
                                      , iHakkoFlg.ToString _
                                      , iAkakuroFlg.ToString _
            ...
            If iSkipFlg = 0 AndAlso bSameKeyFlg = False Then
                ...
                '受信DTL取消データ取得処理
                ds.Tables("LMH030_HED_JC_CANCELOUT").Clear()    '取得用DSをクリア
                ...
                If MyBase.GetResultCount > 0 Then
                    ...
                    Dim drRcvHedCancel As DataRow = ds.Tables("LMH030_HED_JC_CANCELOUT").Rows(0)
                    '取得したデータのキャンセルフラグを設定
            ...
            Dim setHedDt As DataTable = setDs.Tables("LMH030_OUTKAEDI_HED_JC")
            Dim setDtlDt As DataTable = setDs.Tables("LMH030_OUTKAEDI_DTL_JC")
