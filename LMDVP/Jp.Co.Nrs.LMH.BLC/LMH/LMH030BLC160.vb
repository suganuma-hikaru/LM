...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC160 = New LMH030DAC160()
    ...
    ''' <summary>
    ''' 商品マスタの入数によらず、1～(商品明細の設定値)/1梱包の出荷とする特定商品の
    ''' 商品KEY(Key)と 1梱包とする数(Value)
    ''' </summary>
    Private _KonpoSuOfTokuteiGoods As Dictionary(Of String, Integer)

#End Region

#Region "DHLサプライチェーン CONST"
...
#Region "画面取込(セミEDI)チェック"
    ...
    Private Function SemiEdiTorikomiChk(ByVal ds As DataSet) As DataSet
        ...
        setDs.Tables("LMH030_SEMIEDI_INFO").ImportRow(dtSemiInfo.Rows(0))
        Dim oneBussinessDayAfter As String = Left(Convert.ToString(Me.GetBussinessDayKrt(MyBase.GetSystemDate(), 1, setDs)).Replace("/", String.Empty), 8)
            ...
            Else
                ...
                For j As Integer = iRowCnt To max
                    ...
                    If (dr.Item("FILE_NAME_RCV").ToString().Trim()).Equals(dtSemiHed.Rows(i).Item("FILE_NAME_RCV").ToString().Trim()) = True Then
                        ...
                        If Me.TorikomiValChk(dr, oneBussinessDayAfter) = False Then
...
#Region "カラム項目の値・日付チェック"
    ...
    ''' <param name="dr"></param>
    ''' <returns></returns>
    Public Function TorikomiValChk(ByVal dr As DataRow, oneBussinessDayAfter As String) As Boolean
        ...
        If LenB(targetStr) > 30 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("Shipment Number(カラム1番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' （【買主注文番号（全体）】→画面.注文番号）
        If LenB(targetStr) > 30 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("Reference #1(カラム2番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("Ship To Zipcode(カラム6番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 10, False).Equals(False) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("Ship To Zipcode(カラム6番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("Ship To Contact Number(カラム7番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 20, False).Equals(False) Or targetStr.Replace("-", "").Length > 15 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("Ship To Contact Number(カラム7番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        Dim nbStr As String = GetNbStr(targetStr)
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("Pack Instruction(カラム8番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        Else
            If String.IsNullOrEmpty(nbStr) Then
                If targetStr.IndexOf(" ") >= 1 Then
                    nbStr = targetStr.Substring(0, targetStr.IndexOf(" "))
                Else
                    nbStr = targetStr.Substring(0, 1)
                End If
            End If
            If IsConvertDbl(nbStr).Equals(False) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E005", New String() {String.Concat("Pack Instruction(カラム8番目)[", targetStr, "]の左側数字部分[", nbStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            Else
                dNum = Convert.ToDouble(nbStr)
                dNum = System.Math.Abs(dNum)
                If Convert.ToDouble(nbStr) = 0 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("Pack Instruction(カラム8番目)[", targetStr, "]の左側数字部分[", nbStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                ElseIf Convert.ToDouble(nbStr) < 0 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E185", New String() {String.Concat("Pack Instruction(カラム8番目)[", targetStr, "]の左側数字部分[", nbStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                ElseIf dNum > 9999999999 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("Pack Instruction(カラム8番目)[", targetStr, "]の左側数字部分[", nbStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                Else
                    If nbStr.IndexOf(".") >= 0 AndAlso Convert.ToDecimal(nbStr.Substring(nbStr.IndexOf(".") + 1)) > 0 Then
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E025", New String() {String.Concat("Pack Instruction(カラム8番目)[", targetStr, "]の左側数字部分[", nbStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                        bRet = False
                    End If
                End If
            End If
        ...
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("Part Number(カラム9番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 20, False).Equals(False) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("Part Number(カラム9番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("Part Description(カラム10番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 文字列長チェック
        ElseIf LenB(targetStr) > 60 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("Part Description(カラム10番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        ' 取込Dtlの並べ替え（1列目: Shipment Number 昇順）
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_DHL").NewRow()
            gyo += 1
            ds.Tables("LMH030_H_OUTKAEDI_DTL_DHL").Clear() ' 受信DTLのクリア
            ...
            If drEdiRcvDtl.Item("CUST_ORD_NO").ToString().Trim().Equals("") = False Then
                ...
                '---------------------------------------------------------------------------
                setDs.Tables("LMH030_H_OUTKAEDI_DTL_DHL").ImportRow(drEdiRcvDtl)
                setDs = MyBase.CallDAC(Me._Dac, "GetOutkaCntAndNoL", setDs)

                ediRecCnt = MyBase.GetResultCount()
                Dim outkaNoL As String = setDs.Tables("LMH030_H_OUTKAEDI_DTL_DHL").Rows(0).Item("OUTKA_CTL_NO").ToString()
            ...
            ' EDI受信データの新規追加
            '---------------------------------------------------------------------------
            setDs.Tables("LMH030_H_OUTKAEDI_DTL_DHL").Clear()
            setDs.Tables("LMH030_H_OUTKAEDI_DTL_DHL").ImportRow(drEdiRcvDtl)
...
#Region "データセット設定 EDI出荷受信テーブル(明細)"
    ...
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer, ByVal gyo As Integer, ByVal drEdiRcvDtl As DataRow) As DataSet
        ...
        drEdiRcvDtl("CUST_ORD_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_1").ToString().Trim(), 30)              ' 荷主注文番号（全体）　←受信データ(Excel) Shipment Number
        drEdiRcvDtl("BUYER_ORD_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_2").ToString().Trim(), 30)             ' 買主注文番号（全体）　←受信データ(Excel) Reference #1
        drEdiRcvDtl("DEST_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 80)                  ' 届先名　　　　　　　　←受信データ(Excel) Ship To Name
        drEdiRcvDtl("DEST_AD_2") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 100)               ' 届先住所2　　　　　　 ←受信データ(Excel) Ship To Address ※住所は届先名称までを含んでしまい純粋に住所部分をロジックで切り出せないため、EDI出荷データの住所(さらには同データを介して届先マスタ)に設定せず、文字列0n項目に退避するのみという結論となったため、文字列01～20の上限である100バイトまでの設定とする。
        drEdiRcvDtl("DEST_AD_1") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 100)               ' 届先住所1　　　　　　 ←受信データ(Excel) Ship To City
        drEdiRcvDtl("DEST_ZIP") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 10)                 ' 届先郵便番号　　　　　←受信データ(Excel) Ship To Zipcode
        drEdiRcvDtl("DEST_TEL") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 20)                 ' 届先電話番号　　　　　←受信データ(Excel) Ship To Contact Number
        drEdiRcvDtl("PACK_INSTRUCTION") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 100)        ' Pack Instruction　　　←受信データ(Excel) Pack Instruction
        drEdiRcvDtl("CUST_GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 20)            ' 商品コード　　　　　　←受信データ(Excel) Part Number
        drEdiRcvDtl("GOODS_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 60)                ' 商品名　　　　　　　　←受信データ(Excel) Part Description
        drEdiRcvDtl("REQUEST_SHIP_LINE_QTY") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 100)  ' Request Ship Line Qty ←受信データ(Excel) Request Ship Line Qty
        drEdiRcvDtl("CARRIER") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 100)                ' 運送会社　　　　　　　←受信データ(Excel) Carrier
        drEdiRcvDtl("OUTKA_PLAN_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 10)         ' 出荷日　　　　　　　　←受信データ(Excel) Pickup
        drEdiRcvDtl("ARR_PLAN_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_14").ToString().Trim(), 10)           ' 納入予定日　　　　　　←受信データ(Excel) Consignee

        'データセットに設定
        ds.Tables("LMH030_H_OUTKAEDI_DTL_DHL").Rows.Add(drEdiRcvDtl)
...
#Region "データセット設定 EDI出荷データ 用 EDI管理番号(大・中)"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim sNrsBrCd As String = ds.Tables("LMH030_H_OUTKAEDI_DTL_DHL").Rows(0).Item("NRS_BR_CD").ToString()
...
#Region "データセット設定(EDI出荷(中)"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet, ByVal drEdiRcvDtl As DataRow) As DataSet
        ...
        drOutkaEdiM("CUST_ORD_NO_DTL") = drEdiRcvDtl("CUST_ORD_NO")             ' 荷主注文番号（明細単位）←EDI出荷受信テーブル.荷主注文番号（全体）　←受信データ(Excel) Shipment Number
        drOutkaEdiM("BUYER_ORD_NO_DTL") = String.Empty
        drOutkaEdiM("CUST_GOODS_CD") = drEdiRcvDtl("CUST_GOODS_CD")             ' 商品コード　　　　　　　←EDI出荷受信テーブル.商品コード　　　　　　←受信データ(Excel) Part Number
        drOutkaEdiM("NRS_GOODS_CD") = String.Empty
        drOutkaEdiM("GOODS_NM") = drEdiRcvDtl("GOODS_NM")                       ' 商品名　　　　　　　　　←EDI出荷受信テーブル.商品名　　　　　　　　←受信データ(Excel) Part Description
        ...
        ' 出荷総個数←(左側の部分文字列)←EDI出荷受信テーブル.Pack Instruction←受信データ(Excel) Pack Instruction
        Dim outkaTtlNb As String = GetNbStr(drEdiRcvDtl("PACK_INSTRUCTION").ToString())
        ...
        drOutkaEdiM("FREE_C01") = drEdiRcvDtl("PACK_INSTRUCTION").ToString()    ' 文字列01　　　　　　　　←EDI出荷受信テーブル.Pack Instruction　　　←受信データ(Excel) Pack Instruction
...
#Region "データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        ' 取込処理日の 1営業日後の取得
        Dim oneBussinessDayAfter As String = Left(Convert.ToString(Me.GetBussinessDayKrt(MyBase.GetSystemDate(), 1, setDs)).Replace("/", String.Empty), 8)
        drOutkaEdiL("OUTKA_PLAN_DATE") = oneBussinessDayAfter                       ' 出荷日　　　　　　　←取込処理日の 1営業日前
        drOutkaEdiL("OUTKO_DATE") = oneBussinessDayAfter                            ' 出庫日　　　　　　　←取込処理日の 1営業日前

#If True Then   'ADD 2022/08/25 031997   【LMS】翌々営業日を納入日に設定してほしい
        ' 取込処理日の 翌々営業日の取得
        Dim oneBussinessDay2After As String = Left(Convert.ToString(Me.GetBussinessDayKrt(MyBase.GetSystemDate(), 2, setDs)).Replace("/", String.Empty), 8)
        drOutkaEdiL("ARR_PLAN_DATE") = oneBussinessDay2After                        '納入日　　　　　　　←取込処理日の 翌々営業日
#Else
...
#End If
        ...
        ' 届先コード←(ハイフン抜き)←EDI出荷受信テーブル.届先電話番号←受信データ(Excel) Ship To Contact Number
        Dim destCd As String = destTel.Replace("-", "")
        drOutkaEdiL("EDI_DEST_CD") = destCd                                         ' EDI届先コード 　　　←上記 届先コード
        drOutkaEdiL("DEST_CD") = destCd                                             ' 届先コード　　　　　←上記 届先コード
        drOutkaEdiL("DEST_NM") = String.Empty                                       ' 届先名　　　　　　　←設定不要
        ...
        drOutkaEdiL("DEST_TEL") = String.Empty                                      ' 届先電話番号　　　　←設定不要
        ...
        drOutkaEdiL("CUST_ORD_NO") = drEdiRcvDtl("CUST_ORD_NO")                     ' 荷主注文番号（全体）←EDI出荷受信テーブル.荷主注文番号（全体）　←受信データ(Excel) Shipment Number
        drOutkaEdiL("BUYER_ORD_NO") = drEdiRcvDtl("BUYER_ORD_NO")                   ' 買主注文番号（全体）←EDI出荷受信テーブル.買主注文番号（全体）　←受信データ(Excel) Reference #1
        ...
        drOutkaEdiL("FREE_C01") = drEdiRcvDtl("CUST_ORD_NO")                        ' 文字列01　　　　　　←EDI出荷受信テーブル.荷主注文番号（全体）　←受信データ(Excel) Shipment Number
        drOutkaEdiL("FREE_C02") = drEdiRcvDtl("BUYER_ORD_NO")                       ' 文字列02　　　　　　←EDI出荷受信テーブル.買主注文番号（全体）　←受信データ(Excel) Reference #1
        drOutkaEdiL("FREE_C03") = drEdiRcvDtl("DEST_NM")                            ' 文字列03　　　　　　←EDI出荷受信テーブル.届先名　　　　　　　　←受信データ(Excel) Ship To Name
        drOutkaEdiL("FREE_C04") = drEdiRcvDtl("DEST_AD_2")                          ' 文字列04　　　　　　←EDI出荷受信テーブル.届先住所2　　　　　　 ←受信データ(Excel) Ship To Address
        drOutkaEdiL("FREE_C05") = drEdiRcvDtl("DEST_AD_1")                          ' 文字列05　　　　　　←EDI出荷受信テーブル.届先住所1　　　　　　 ←受信データ(Excel) Ship To City
        drOutkaEdiL("FREE_C06") = drEdiRcvDtl("DEST_TEL")                           ' 文字列06　　　　　　←EDI出荷受信テーブル.届先届先電話番号　　　←受信データ(Excel) Ship To Contact Number
        drOutkaEdiL("FREE_C07") = drEdiRcvDtl("REQUEST_SHIP_LINE_QTY")              ' 文字列07　　　　　　←EDI出荷受信テーブル.Request Ship Line Qty ←受信データ(Excel) Request Ship Line Qty
        drOutkaEdiL("FREE_C08") = drEdiRcvDtl("CARRIER")                            ' 文字列08　　　　　　←EDI出荷受信テーブル.運送会社　　　　　　　←受信データ(Excel) Carrier
        drOutkaEdiL("FREE_C09") = drEdiRcvDtl("OUTKA_PLAN_DATE")                    ' 文字列09　　　　　　←EDI出荷受信テーブル.出荷日　　　　　　　　←受信データ(Excel) Pickup
        drOutkaEdiL("FREE_C10") = drEdiRcvDtl("ARR_PLAN_DATE")                      ' 文字列10　　　　　　←EDI出荷受信テーブル.納入予定日　　　　　　←受信データ(Excel) Consignee
...
#Region "Pack Instruction よりの「個数」(左側の部分文字列)取り出し"

    Private Function GetNbStr(ByVal packInstruction As String) As String

        Dim ret As String = ""
        Dim dummy As Double

        For i As Integer = 1 To packInstruction.Length()
            If Not Double.TryParse(packInstruction.Substring(0, i), dummy) Then
                ret = packInstruction.Substring(0, i - 1).Trim()
                Exit For
            End If
        Next
...
#End Region '  "Pack Instruction よりの「個数」取り出し"
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        ' 出荷梱包個数計算単位取得(商品明細マスタ)
        _KonpoSuOfTokuteiGoods = New Dictionary(Of String, Integer)
        ds = MyBase.CallDAC(Me._Dac, "SelectGoodsDetails", ds)
        For i As Integer = 0 To ds.Tables("LMH030_M_GOODS_DETAILS").Rows.Count - 1
            Dim drGoodsDetails As DataRow = ds.Tables("LMH030_M_GOODS_DETAILS").Rows(i)
            Dim setNaiyo As Integer
            If Integer.TryParse(drGoodsDetails.Item("SET_NAIYO").ToString(), setNaiyo) Then
                _KonpoSuOfTokuteiGoods(drGoodsDetails.Item("GOODS_CD_NRS").ToString()) = setNaiyo
            End If
        Next
...
#Region "データセット設定(出荷包装個数)"
    Private Function SumPkgNb(ByVal dt As DataTable) As Double
        ...
        For i As Integer = 0 To max
            ...
            If _KonpoSuOfTokuteiGoods.ContainsKey(dt.Rows(i)("NRS_GOODS_CD").ToString()) Then
                ' 商品マスタの入数によらず、1～(商品明細の設定値)/1梱包の出荷とする特定商品の場合
                Dim konpoSu As Integer = _KonpoSuOfTokuteiGoods(dt.Rows(i)("NRS_GOODS_CD").ToString())
                sumNb += GetOutkaMPkgNg(dt.Rows(i), konpoSu)
            Else
...
#Region "データセット設定(出荷M)"
    ...
    Private Function SetDatasetOutkaM(ByVal ds As DataSet, ByVal matomeflg As Boolean) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            If _KonpoSuOfTokuteiGoods.ContainsKey(ediDr("NRS_GOODS_CD").ToString()) Then
                ' 商品マスタの入数によらず、1～(商品明細の設定値)/1梱包の出荷とする特定商品の場合
                Dim konpoSu As Integer = _KonpoSuOfTokuteiGoods(ediDr("NRS_GOODS_CD").ToString())
                outkaDr("OUTKA_M_PKG_NB") = GetOutkaMPkgNg(ediDr, konpoSu)
            Else
...
#Region "データセット設定補助処理(出荷L, M 共通)"

    ''' <summary>
    ''' 商品マスタの入数によらず、1～(商品明細の設定値)/1梱包の出荷とする特定商品の出荷梱包個数 計算
    ''' </summary>
    ''' <param name="ediMDr"></param>
    ''' <param name="konpoSu">商品明細の設定値</param>
    ''' <returns></returns>
    Private Function GetOutkaMPkgNg(ByVal ediMDr As DataRow, ByVal konpoSu As Integer) As Decimal

        Dim outkaTtlNb As Decimal = Convert.ToDecimal(ediMDr("OUTKA_TTL_NB").ToString())
        Return Math.Floor(outkaTtlNb / konpoSu) + If(outkaTtlNb Mod konpoSu > 0, 1, 0)
