...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC101 = New LMH010DAC101()
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(dtM.Rows(i)("GOODS_NM").ToString()) = False Then
                If dtM.Rows(i)("GOODS_NM").ToString().Equals(goodsDt.Rows(0)("GOODS_NM_1").ToString()) = False Then
                    choiceKb = Me._ChkBlc.GetWarningChoiceKbM(ds.Tables("LMH010_INKAEDI_M"), ds, LMH010BLC.NIK_WID_M002, i)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        ds = Me._ChkBlc.SetWarningM2("W195", LMH010BLC.NIK_WID_M002, ds, setDs, msgArray, ediName, ediValue, mustValue)
                        flgWarning = True 'ワーニングフラグをたてて処理続行
                    End If
            ...
            '        choiceKb = Me._ChkBlc.GetWarningChoiceKbM(ds.Tables("LMH010_INKAEDI_M"), ds, LMH010BLC.NIK_WID_MXXX, i)
            ...
            '            ds = Me._ChkBlc.SetWarningM2("W194", LMH010BLC.NIK_WID_MXXX, ds, setDs, msgArray, ediName, ediValue, mustValue)
            '            flgWarning = True 'ワーニングフラグをたてて処理続行
            '        End If
    ...
    Private Function SetDatasetInkaS(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(ediDr("FREE_C01").ToString()) = False Then
                inkaDr("LT_DATE") = (ediDr("FREE_C01").ToString()).Substring(0, 6)
            End If
            '日医工 特殊設定　追加END
            inkaDr("SPD_KB") = "01"
            inkaDr("OFB_KB") = "01"
...
#Region "実績作成処理(日医工)"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        '(日医工)送信データ作成用情報取得
        ds = MyBase.CallDAC(Me._Dac, "SelectSendNichiiko", ds)
        ...
        '(日医工)送信データの更新
        ds = MyBase.CallDAC(Me._Dac, "InsertSendNichiiko", ds)
