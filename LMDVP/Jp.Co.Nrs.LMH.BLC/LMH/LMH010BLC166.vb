...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC166 = New LMH010DAC166()
...
#Region "データセット設定(入荷L)"
    ...
    Private Function SetDatasetInkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_TST").NewRow()
        ...
        If matomeFlg = False Then
            ...
            inkaDr("REMARK_OUT") = String.Empty     'drEdiRcvDtl("PURCHASING_DOCUMENT") 'ediDr("NYUBAN_L") '20151106 tsunehira add 
...
#Region "データセット設定(入荷M)"
    ...
    Private Function SetDatasetInkaM(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            inkaDr("OUTKA_FROM_ORD_NO_M") = ediDr("OUTKA_FROM_ORD_NO")  'drEdiRcvDtl("PURCHASING_DOCUMENT") 'ediDr("PURCHASING_DOCUMENT")
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    Private Function SetSemiInkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        '荷主固有データ
        drEdiRcvDtl("DELIVERY") = drSetDtl("COLUMN_1").ToString().Trim()
        drEdiRcvDtl("PUR_DOC") = drSetDtl("COLUMN_2").ToString().Trim()
        drEdiRcvDtl("NAME_OF_SHIP_TO") = drSetDtl("COLUMN_3").ToString().Trim()
        drEdiRcvDtl("SHIP_TO") = drSetDtl("COLUMN_4").ToString().Trim()
        drEdiRcvDtl("MATERIAL") = drSetDtl("COLUMN_5").ToString().Trim()
        drEdiRcvDtl("DESCRIPTION") = drSetDtl("COLUMN_6").ToString().Trim()
        drEdiRcvDtl("BATCH") = drSetDtl("COLUMN_7").ToString().Trim()
        drEdiRcvDtl("DELIVERY_QUANTITY") = drSetDtl("COLUMN_8").ToString().Trim()
        drEdiRcvDtl("SU") = drSetDtl("COLUMN_9").ToString().Trim()
        drEdiRcvDtl("NET_WEIGHT") = drSetDtl("COLUMN_10").ToString().Trim()
        drEdiRcvDtl("BUN") = drSetDtl("COLUMN_11").ToString().Trim()

        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "1"                                                                 '実績処理フラグ

        'データセットに設定
        ds.Tables("LMH010_INKAEDI_DTL_TST").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI入荷(中))"
    ...
    Private Function SetSemiInkaEdiM(ByVal setDs As DataSet _
        ...
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH010_INKAEDI_DTL_TST").Rows(0)
        ...
        If iMSkipFlg = 0 Then
            drInkaEdiM("NB") = drRcvEdiDtl("DELIVERY_QUANTITY")
            drInkaEdiM("INKA_PKG_NB") = drRcvEdiDtl("DELIVERY_QUANTITY")
        End If
        drInkaEdiM("HASU") = 0

        drInkaEdiM("OUTKA_FROM_ORD_NO") = drRcvEdiDtl("PUR_DOC")
        ...
        drInkaEdiM("FREE_C01") = drRcvEdiDtl("DELIVERY")
        drInkaEdiM("FREE_C02") = drRcvEdiDtl("PUR_DOC")
        drInkaEdiM("FREE_C03") = drRcvEdiDtl("NAME_OF_SHIP_TO")
        drInkaEdiM("FREE_C04") = drRcvEdiDtl("SHIP_TO")
        drInkaEdiM("FREE_C05") = drRcvEdiDtl("SU")
        drInkaEdiM("FREE_C06") = drRcvEdiDtl("BUN")
...
#Region "セミEDI時　データセット設定(EDI入荷(大))"
    ...
    Private Function SetSemiinkaediL(ByVal setDs As DataSet) As DataSet
        ...
        drInkaEdiL("NRS_WH_CD") = WH_CD_TST
        drInkaEdiL("CUST_CD_L") = CUST_CD_L_TST
        drInkaEdiL("CUST_CD_M") = CUST_CD_M_TST
        ...
        drInkaEdiL("BUYER_ORD_NO") = String.Empty   'drRcvEdiDtl("EXTERNAL_DELIVERY_ID")
        ...
        drInkaEdiL("FREE_C20") = String.Empty   'String.Concat(drSemiEdiInfo.Item("CUST_CD_L").ToString(), drSemiEdiInfo.Item("CUST_CD_M").ToString())
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_TST")
...
#Region "商品マスタ"
    ...
    Public Function TorikomiMstChk(ByVal dr As DataRow, ByVal ds As DataSet) As Boolean
        ...
        sMsg = "Material(カラム5番目)"
        sGoodsCd = dr.Item("COLUMN_5").ToString()
        sGoodsNm = dr.Item("COLUMN_6").ToString()

        If ds.Tables("LMH010_M_GOODS").Rows.Count = 0 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E493", New String() {"荷主商品コード", "商品マスタ", String.Concat("Material：", sGoodsCd, " Description：", sGoodsNm)}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            Return bRet

        ElseIf ds.Tables("LMH010_M_GOODS").Rows.Count > 1 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E494", New String() {"荷主商品コード", "商品マスタ", String.Concat("Material：", sGoodsCd)}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            Return bRet
        End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_TST")         'EDI受信Dtl
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            ds.Tables("LMH010_INKAEDI_DTL_TST").Clear() '受信DTLをクリア
            ds = Me.SetSemiInkaEdiRcv(ds, i)
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_TST").Rows(0)
            ...
            '数量が0.0の場合は取り込まずに次のレコードへ
            If Convert.ToDecimal(drEdiRcvDtl.Item("DELIVERY_QUANTITY").ToString()) = 0 Then
                Continue For
            End If
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH010_INKAEDI_DTL_TST")
            setDtlDt.Clear()
            ...
            preDeliverNo = setDs.Tables("LMH010_INKAEDI_DTL_TST").Rows(0).Item("DELIVERY").ToString()
            preGoodsCdCust = setDs.Tables("LMH010_INKAEDI_DTL_TST").Rows(0).Item("MATERIAL").ToString()
            preLotNo = setDs.Tables("LMH010_INKAEDI_DTL_TST").Rows(0).Item("BATCH").ToString()
