...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC158 = New LMH030DAC158()
...
#Region "テルモ サンプルCONST"

    ''' <summary>
    ''' その他
    ''' </summary>
    ''' <remarks></remarks>
    Public Const DEF_CTL_NO As String = "U00000000"             '管理番号初期値
    Public Const DEF_UNSO_NO_L As String = "01-U00000000"       '運送番号初期値
    Public Const DEF_UNSO_NO_M As String = "01-U00000000000"    '運送番号初期値

#End Region ' "テルモ サンプルCONST"
...
#Region "画面取込(セミEDI)チェック"
    ...
    Private Function SemiEdiTorikomiChk(ByVal ds As DataSet) As DataSet
        ...
        Dim denpyoSb As String = ""
        Dim lastDenpyoSb As String = ""
        Dim lastDenpyoNo As String = ""
        Dim recNo As Integer = 0
        Dim recNoLen As Integer = dtSemiDtl.Rows.Count.ToString().Length
        Dim preZeroStr As String = New String("0"c, recNoLen)
        ...
        For k As Integer = 0 To max
            dr = dtSemiDtl.Rows(k)
            Dim sData As String = dr.Item("COLUMN_1").ToString()                    ' 固定長レコード取得

            denpyoSb = Me._Blc.MidB(sData, 26, 2).Trim()                            ' 伝票種別(ヘッダ、明細の判定用に、先に取り出す)

            dr.Item("COLUMN_1") = sData.Length.ToString()                           ' 先頭列はレコード長に置換する。
            If denpyoSb = "H1" Then
                ' ヘッダ
                recNo = k + 1                                                       ' 入力ファイルの行番号の算出
                dr.Item("COLUMN_2") = recNo.ToString()                              ' レコードナンバーとして番号を保持する。
            ElseIf denpyoSb = "B1" Then
                ' 明細
                dr.Item("COLUMN_2") = recNo.ToString()                              ' (ヘッダのレコードナンバーを)レコードナンバーとして保持する。
            End If
            If True Then
                ' ソート用に前ゼロを付加したファイル内行数を設定する。
                dr.Item("COLUMN_3") = Right(String.Concat(preZeroStr, dr.Item("REC_NO").ToString()), recNoLen)
                ' ヘッダ、明細共通部
                dr.Item("COLUMN_4") = Me._Blc.MidB(sData, 1, 10).Trim()             ' 伝票番号
                dr.Item("COLUMN_5") = Me._Blc.MidB(sData, 11, 8).Trim()             ' 伝票日付
                dr.Item("COLUMN_6") = Me._Blc.MidB(sData, 19, 5).Trim()             ' 依頼先倉庫コード
                dr.Item("COLUMN_7") = Me._Blc.MidB(sData, 24, 2).Trim()             ' 伝票区分
                dr.Item("COLUMN_8") = denpyoSb                                      ' 伝票種別
            End If
            If denpyoSb = "H1" Then
                ' ヘッダ
                dr.Item("COLUMN_9") = Me._Blc.MidB(sData, 28, 5).Trim()             ' 送り先倉庫コード
                dr.Item("COLUMN_10") = Me._Blc.MidB(sData, 33, 40).Trim()           ' 送り先名
                dr.Item("COLUMN_11") = Me._Blc.MidB(sData, 73, 7).Trim()            ' 送り先郵便番号
                dr.Item("COLUMN_12") = Me._Blc.MidB(sData, 80, 40).Trim()           ' 送り先住所１
                dr.Item("COLUMN_13") = Me._Blc.MidB(sData, 120, 40).Trim()          ' 送り先住所２
                dr.Item("COLUMN_14") = Me._Blc.MidB(sData, 160, 15).Trim()          ' 送り先電話番号
                dr.Item("COLUMN_15") = Me._Blc.MidB(sData, 175, 40).Trim()          ' 受取人名
                dr.Item("COLUMN_16") = Me._Blc.MidB(sData, 215, 40).Trim()          ' 送り元名
                dr.Item("COLUMN_17") = Me._Blc.MidB(sData, 255, 7).Trim()           ' 送り元郵便番号
                dr.Item("COLUMN_18") = Me._Blc.MidB(sData, 262, 40).Trim()          ' 送り元住所１
                dr.Item("COLUMN_19") = Me._Blc.MidB(sData, 302, 40).Trim()          ' 送り元住所２
                dr.Item("COLUMN_20") = Me._Blc.MidB(sData, 342, 15).Trim()          ' 送り元電話番号
                dr.Item("COLUMN_21") = Me._Blc.MidB(sData, 357, 40).Trim()          ' 送り人名
                dr.Item("COLUMN_22") = Me._Blc.MidB(sData, 397, 8).Trim()           ' 出荷日
                dr.Item("COLUMN_23") = Me._Blc.MidB(sData, 405, 8).Trim()           ' 着荷予定日
                dr.Item("COLUMN_24") = Me._Blc.MidB(sData, 413, 5).Trim()           ' 着荷予定時刻
                dr.Item("COLUMN_25") = Me._Blc.MidB(sData, 418, 1).Trim()           ' 出荷方法
                dr.Item("COLUMN_26") = Me._Blc.MidB(sData, 419, 1).Trim()           ' 送り状出力有無
                dr.Item("COLUMN_27") = Me._Blc.MidB(sData, 420, 1).Trim()           ' 返却用送り状出力有無
                dr.Item("COLUMN_28") = Me._Blc.MidB(sData, 421, 1).Trim()           ' 修理品送り状出力有無
                dr.Item("COLUMN_29") = Me._Blc.MidB(sData, 422, 1).Trim()           ' 受領書出力有無
                dr.Item("COLUMN_30") = Me._Blc.MidB(sData, 423, 1).Trim()           ' アセンブリ指示有無
                dr.Item("COLUMN_31") = Me._Blc.MidB(sData, 424, 1).Trim()           ' 振り分けＩＤ
                dr.Item("COLUMN_32") = Me._Blc.MidB(sData, 425, 99).Trim()          ' 備考
                dr.Item("COLUMN_33") = Me._Blc.MidB(sData, 524, 10).Trim()          ' 配送業者デポコード
                dr.Item("COLUMN_34") = Me._Blc.MidB(sData, 534, 12).Trim()          ' 配送業者問合せ番号
                dr.Item("COLUMN_35") = Me._Blc.MidB(sData, 546, 10).Trim()          ' 製品出荷伝票番号
                dr.Item("COLUMN_36") = Me._Blc.MidB(sData, 556, 8).Trim()           ' 配送業者コード
                dr.Item("COLUMN_37") = Me._Blc.MidB(sData, 564, 20).Trim()          ' 仕分けコード
                dr.Item("COLUMN_38") = Me._Blc.MidB(sData, 584, 5).Trim()           ' 温度区分
                dr.Item("COLUMN_39") = Me._Blc.MidB(sData, 589, 10).Trim()          ' 集荷指定時刻条件
                dr.Item("COLUMN_40") = Me._Blc.MidB(sData, 599, 5).Trim()           ' サービスレベル区分
                dr.Item("COLUMN_41") = Me._Blc.MidB(sData, 604, 1).Trim()           ' データ識別区分
                dr.Item("COLUMN_42") = Me._Blc.MidB(sData, 605, 1).Trim()           ' 事前通知希望フラグ
                dr.Item("COLUMN_43") = Me._Blc.MidB(sData, 606, 60).Trim()          ' 事前通知e-mailアドレス
                dr.Item("COLUMN_44") = Me._Blc.MidB(sData, 666, 1).Trim()           ' 入力機種種別
                dr.Item("COLUMN_45") = Me._Blc.MidB(sData, 667, 100).Trim()         ' メッセージエリア
            ElseIf denpyoSb = "B1" Then
                ' 明細
                dr.Item("COLUMN_9") = Me._Blc.MidB(sData, 28, 3).Trim()             ' 明細番号
                dr.Item("COLUMN_10") = Me._Blc.MidB(sData, 31, 15).Trim()           ' 商品コード
                dr.Item("COLUMN_11") = Me._Blc.MidB(sData, 46, 15).Trim()           ' ロット番号
                dr.Item("COLUMN_12") = Me._Blc.MidB(sData, 61, 12).Trim()           ' 製造番号
                dr.Item("COLUMN_13") = Me._Blc.MidB(sData, 73, 5).Trim()            ' バージョン
                dr.Item("COLUMN_14") = Me._Blc.MidB(sData, 78, 1).Trim()            ' 商品種別
                dr.Item("COLUMN_15") = Me._Blc.MidB(sData, 79, 40).Trim()           ' 商品名
                dr.Item("COLUMN_16") = Me._Blc.MidB(sData, 119, 2).Trim()           ' 使用目的
                dr.Item("COLUMN_17") = Me._Blc.MidB(sData, 121, 10).Trim()          ' 数量
                dr.Item("COLUMN_18") = Me._Blc.MidB(sData, 131, 10).Trim()          ' 単価
                dr.Item("COLUMN_19") = Me._Blc.MidB(sData, 141, 10).Trim()          ' 金額
                dr.Item("COLUMN_20") = Me._Blc.MidB(sData, 151, 40).Trim()          ' ユーザ名
                dr.Item("COLUMN_21") = Me._Blc.MidB(sData, 191, 40).Trim()          ' 診療科
                dr.Item("COLUMN_22") = Me._Blc.MidB(sData, 231, 20).Trim()          ' ドクター名
                dr.Item("COLUMN_23") = Me._Blc.MidB(sData, 251, 8).Trim()           ' 貸出期間FROM
                dr.Item("COLUMN_24") = Me._Blc.MidB(sData, 259, 8).Trim()           ' 貸出期間TO
                dr.Item("COLUMN_25") = Me._Blc.MidB(sData, 267, 1).Trim()           ' 貸出機区分
                dr.Item("COLUMN_26") = Me._Blc.MidB(sData, 268, 1).Trim()           ' 正梱・バラ区分
                dr.Item("COLUMN_27") = Me._Blc.MidB(sData, 269, 20).Trim()          ' 学術管理Ｎｏ．
                dr.Item("COLUMN_28") = Me._Blc.MidB(sData, 289, 1).Trim()           ' クラス分類
                dr.Item("COLUMN_29") = Me._Blc.MidB(sData, 290, 1).Trim()           ' 保守特定
                dr.Item("COLUMN_30") = Me._Blc.MidB(sData, 291, 1).Trim()           ' ユーザ名出力フラグ
                dr.Item("COLUMN_31") = Me._Blc.MidB(sData, 292, 80).Trim()          ' ユーザ住所
                dr.Item("COLUMN_32") = Me._Blc.MidB(sData, 372, 10).Trim()          ' 患者ID
                dr.Item("COLUMN_33") = Me._Blc.MidB(sData, 382, 30).Trim()          ' 設置場所
                dr.Item("COLUMN_34") = Me._Blc.MidB(sData, 412, 2).Trim()           ' 交換対応者
                dr.Item("COLUMN_35") = Me._Blc.MidB(sData, 414, 10).Trim()          ' 宅配日
                dr.Item("COLUMN_36") = Me._Blc.MidB(sData, 424, 12).Trim()          ' 交換対象機
                dr.Item("COLUMN_37") = Me._Blc.MidB(sData, 436, 50).Trim()          ' 患者名称
                dr.Item("COLUMN_38") = Me._Blc.MidB(sData, 486, 7).Trim()           ' お届け先郵便番号
                dr.Item("COLUMN_39") = Me._Blc.MidB(sData, 493, 100).Trim()         ' お届け先住所
                dr.Item("COLUMN_40") = Me._Blc.MidB(sData, 593, 15).Trim()          ' お届け先電話番号
            End If
        Next
            ...
            Else
                ...
                For j As Integer = iRowCnt To max
                    ...
                    If (dr.Item("FILE_NAME_RCV").ToString().Trim()).Equals(dtSemiHed.Rows(i).Item("FILE_NAME_RCV").ToString().Trim()) = True Then
                        ...
                        '入力チェック(数値,日付チェック)
                        If Me.TorikomiValChk(dr, j, max, lastDenpyoSb, lastDenpyoNo) = False Then
...
#Region "カラム項目の値・日付チェック"
    ...
    ''' </summary>
    ''' <param name="dr"></param>
    ''' <param name="dtlIdx"></param>
    ''' <param name="dtlIdxMax"></param>
    ''' <param name="lastDenpyoSb"></param>
    ''' <param name="lastDenpyoNo"></param>
    ''' <returns></returns>
    Public Function TorikomiValChk(ByVal dr As DataRow,
                                   ByVal dtlIdx As Integer,
                                   dtlIdxMax As Integer,
                                   ByRef lastDenpyoSb As String,
                                   ByRef lastDenpyoNo As String) As Boolean

        Dim sFileNm As String = dr.Item("FILE_NAME_RCV").ToString()
        Dim sRecNo As String = dr.Item("REC_NO").ToString()
        Dim denpyoSb As String = GetDenpyoSb(dr)
        ...
        If denpyoSb = "H1" Then
            ...
            If dtlIdx = dtlIdxMax Then
                ' 最終行がヘッダ
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("最終行の伝票種別[", denpyoSb, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            ElseIf dtlIdx <> 0 AndAlso lastDenpyoSb = "H1" Then
                ' 2行目以降
                ' ヘッダが続く
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("、直上のヘッダ行と同じ伝票種別[", denpyoSb, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
            ...
            If String.IsNullOrEmpty(targetStr) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("送り先名[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
                ' 文字列長チェック
            ElseIf LenB(targetStr) > 80 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("送り先名[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
            ...
            '必須チェック
            If String.IsNullOrEmpty(targetStr) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("送り先郵便番号[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
                ' 半角文字列長チェック
            ElseIf IsHalfSize(targetStr, 10, False).Equals(False) Or targetStr.Replace("-", "").Length < 7 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("送り先郵便番号[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If

            ' 送り先住所１、送り先住所２
            targetStr = String.Concat(dr.Item("COLUMN_12").ToString.Trim(), dr.Item("COLUMN_13").ToString.Trim())
            '必須チェック
            If String.IsNullOrEmpty(targetStr) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("送り先住所１、２[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
                ' 文字列長チェック
            Else
                Dim ad() As String = editAddr(targetStr)
                If ad(0).Equals(targetStr) = False Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("送り先住所１、２[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If
            End If
            ...
            '必須チェック
            If String.IsNullOrEmpty(targetStr) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("送り先電話番号[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
                ' 半角文字列長チェック
            ElseIf IsHalfSize(targetStr, 20, False).Equals(False) Or targetStr.Replace("-", "").Length < 8 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("送り先電話番号[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If

            ' 出荷日
            sDate = dr.Item("COLUMN_22").ToString.Trim()
            ' 年月日チェック
            If sDate.Length <> 8 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat("出荷日[", sDate, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            Else
                If IsDate(String.Concat(sDate.Substring(0, 4), "/", sDate.Substring(4, 2), "/", sDate.Substring(6, 2))) = True Then
                Else
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat("出荷日[", sDate, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If
            ...
            ' 着荷予定日
            sDate = dr.Item("COLUMN_23").ToString.Trim()
            ' 年月日チェック
            If sDate.Length <> 8 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat("着荷予定日[", sDate, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            Else
                If IsDate(String.Concat(sDate.Substring(0, 4), "/", sDate.Substring(4, 2), "/", sDate.Substring(6, 2))) = True Then
                Else
        ...
        ElseIf denpyoSb = "B1" Then
            ...
            If dtlIdx = 0 Then
                ' 先頭がヘッダ以外
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("先頭行の伝票種別[", denpyoSb, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            Else
                ' 伝票番号
                targetStr = dr.Item("COLUMN_4").ToString.Trim()
                ' 直前行と不一致
                If targetStr <> lastDenpyoNo Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("、直上行と一致しない伝票番号[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                End If
            End If
            ...
            ' 必須チェック
            If String.IsNullOrEmpty(targetStr) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("商品コード[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
                ' 半角文字列長チェック
            ElseIf IsHalfSize(targetStr, 20, False).Equals(False) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("商品コード[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
            ...
            ' 必須チェック
            If String.IsNullOrEmpty(targetStr) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("数量[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            Else
                If IsConvertDbl(targetStr).Equals(False) Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E005", New String() {String.Concat("数量[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                Else
                    ...
                    ElseIf Convert.ToDouble(targetStr) < 0 Then
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E185", New String() {String.Concat("数量[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                        bRet = False
                    ElseIf dNum > 9999999999 Then
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("数量[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                        bRet = False
                    Else
                        If targetStr.IndexOf(".") >= 0 AndAlso Convert.ToDecimal(targetStr.Substring(targetStr.IndexOf(".") + 1)) > 0 Then
                            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E025", New String() {String.Concat("数量[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                            bRet = False
                        End If
                    End If
                End If
        ...
        Else
            ' 伝票種別 範囲外
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat("伝票種別[", denpyoSb, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        ' 次回比較用直前値の退避
        ' 伝票種別
        lastDenpyoSb = denpyoSb
        ' 伝票番号
        lastDenpyoNo = dr.Item("COLUMN_4").ToString()
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        ' 区分マスタ(テルモ サンプル 使用目的)
        Dim dsZKbnPurpose As DataSet = New DataSet()
        dsZKbnPurpose.Merge(ds.Tables("LMH030_SEMIEDI_INFO"))
        dsZKbnPurpose.Merge(ds.Tables("LMH030_Z_KBN_OUT"))
        dsZKbnPurpose.Merge(ds.Tables("LMH030_Z_KBN_IN").Clone())
        dsZKbnPurpose.Tables("LMH030_Z_KBN_IN").Clear()
        Dim drZKbnIn As DataRow = dsZKbnPurpose.Tables("LMH030_Z_KBN_IN").NewRow()
        drZKbnIn.Item("KBN_GROUP_CD") = "T034"
        dsZKbnPurpose.Tables("LMH030_Z_KBN_IN").Rows.Add(drZKbnIn)
        dsZKbnPurpose = MyBase.CallDAC(Me._Dac, "SelectDataZkbn2", dsZKbnPurpose)
        If MyBase.GetResultCount = 0 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E326", New String() {"テルモ サンプル 使用目的", "区分マスタ"})
            Return ds
        End If

        ' 取込Dtlの並べ替え（4列目: 伝票番号 昇順、3列目: 前ゼロ付加で桁を揃えたファイル内行数 昇順）
        ' ヘッダ、1件以上の明細（ヘッダ、1件以上の明細……）の順序および、
        ' 明細の伝票番号はヘッダの伝票番号と一致することは保証されている（入力チェック済み）前提
        Dim tmpDt As DataTable = dtSetDtl.Clone()
        Dim tmpDr() As DataRow = Nothing
        tmpDr = dtSetDtl.Select(String.Empty, "COLUMN_4 ASC, COLUMN_3 ASC")
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            ' 伝票種別取得
            '---------------------------------------------------------------------------
            denpyoSb = GetDenpyoSb(ds.Tables("LMH030_EDI_TORIKOMI_DTL").Rows(i))

            '---------------------------------------------------------------------------
            ' セミEDI取込(共通)⇒EDI受信データセット
            '---------------------------------------------------------------------------
            Dim drEdiRcvHed As DataRow = ds.Tables("LMH030_H_OUTKAEDI_HED_TRMSMPL").NewRow()
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_TRMSMPL").NewRow()
            If denpyoSb = "H1" Then
                ' ヘッダ
                gyo = 0
                ds.Tables("LMH030_H_OUTKAEDI_HED_TRMSMPL").Clear() ' 受信HEDのクリア
                ds = Me.SetSemiOutkaEdiRcvHed(ds, i, drEdiRcvHed)
            ElseIf denpyoSb = "B1" Then
                ' 明細
                gyo += 1
                ds.Tables("LMH030_H_OUTKAEDI_DTL_TRMSMPL").Clear() ' 受信DTLのクリア
                ds = Me.SetSemiOutkaEdiRcvDtl(ds, i, gyo, drEdiRcvDtl)
            End If
            ...
            If denpyoSb = "H1" Then
                ' ヘッダ
                sNewKey = drEdiRcvHed.Item("CUST_ORD_NO").ToString()
            ElseIf denpyoSb = "B1" Then
                ' 明細
                sNewKey = drEdiRcvDtl.Item("DENPYO_NO").ToString()
            End If
            ...
            ' 区分マスタ(テルモ サンプル 使用目的) マージ
            setDs.Merge(dsZKbnPurpose.Tables("LMH030_Z_KBN_OUT"))

            If _
                (denpyoSb = "H1" AndAlso drEdiRcvHed.Item("CUST_ORD_NO").ToString().Trim().Equals("") = False) OrElse
                (denpyoSb = "B1" AndAlso drEdiRcvDtl.Item("DENPYO_NO").ToString().Trim().Equals("") = False) Then
                '---------------------------------------------------------------------------
                ' EDI出荷データ件数および出荷データL 出荷管理番号L 等 取得処理
                '---------------------------------------------------------------------------
                Dim drEdiRcvHedTemp As DataRow = setDs.Tables("LMH030_H_OUTKAEDI_HED_TRMSMPL").NewRow()
                If denpyoSb = "H1" Then
                    ' ヘッダ
                    drEdiRcvHedTemp.Item("NRS_BR_CD") = drEdiRcvHed.Item("NRS_BR_CD")
                    drEdiRcvHedTemp.Item("CUST_CD_L") = drEdiRcvHed.Item("CUST_CD_L")
                    drEdiRcvHedTemp.Item("CUST_CD_M") = drEdiRcvHed.Item("CUST_CD_M")
                    drEdiRcvHedTemp.Item("CUST_ORD_NO") = drEdiRcvHed.Item("CUST_ORD_NO")
                ElseIf denpyoSb = "B1" Then
                    ' 明細
                    drEdiRcvHedTemp.Item("NRS_BR_CD") = drEdiRcvDtl.Item("NRS_BR_CD")
                    drEdiRcvHedTemp.Item("CUST_CD_L") = drEdiRcvDtl.Item("CUST_CD_L")
                    drEdiRcvHedTemp.Item("CUST_CD_M") = drEdiRcvDtl.Item("CUST_CD_M")
                    drEdiRcvHedTemp.Item("CUST_ORD_NO") = drEdiRcvDtl.Item("DENPYO_NO")
                End If
                setDs.Tables("LMH030_H_OUTKAEDI_HED_TRMSMPL").Rows.Add(drEdiRcvHedTemp)

                setDs = MyBase.CallDAC(Me._Dac, "GetOutkaCntAndNoL", setDs)

                ediRecCnt = MyBase.GetResultCount()
                Dim outkaNoL As String = setDs.Tables("LMH030_H_OUTKAEDI_HED_TRMSMPL").Rows(0).Item("OUTKA_CTL_NO").ToString()
            ...
            If denpyoSb = "H1" Then
                ' ヘッダ
                ds = Me.GetEdiCtlNo(ds, iDeleteFlg, iSkipFlg, bSameKeyFlg, sEdiCtlNo, iEdiCtlNoChu, drEdiRcvHed)
            ElseIf denpyoSb = "B1" Then
                ' 明細
            ...
            If denpyoSb = "H1" Then
                ' ヘッダ
                setDs.Tables("LMH030_H_OUTKAEDI_HED_TRMSMPL").Clear()
                setDs.Tables("LMH030_H_OUTKAEDI_HED_TRMSMPL").ImportRow(drEdiRcvHed)
                setDs.Tables("LMH030_SEMIEDI_INFO").ImportRow(ds.Tables("LMH030_SEMIEDI_INFO").Rows(0))

                ' 新規追加 EDI受信データ(HED) と同一キーレコードの件数取得
                setDs = MyBase.CallDAC(Me._Dac, "GetOutkaEdiHedCnt", setDs)
                If MyBase.GetResultCount() > 0 Then
                    ' 同一キーレコード存在時の物理削除
                    setDs = MyBase.CallDAC(Me._Dac, "DeleteOutkaEdiHed", setDs)
            ...
            ElseIf denpyoSb = "B1" Then
                ' 明細
                setDs.Tables("LMH030_H_OUTKAEDI_DTL_TRMSMPL").Clear()
                setDs.Tables("LMH030_H_OUTKAEDI_DTL_TRMSMPL").ImportRow(drEdiRcvDtl)
                setDs.Tables("LMH030_SEMIEDI_INFO").ImportRow(ds.Tables("LMH030_SEMIEDI_INFO").Rows(0))
            ...
            If iSkipFlg = 0 Then
                ...
                If denpyoSb = "H1" Then
                    ...
                    If bSameKeyFlg = False Then
                        ' 使用目的取得
                        ' ヘッダにはない項目のため、次の行
                        ' （ヘッダ、1件以上の明細（ヘッダ、1件以上の明細……）の順序は保証されている（入力チェック済み）前提）
                        Dim purpose As String = GetPurpose(ds.Tables("LMH030_EDI_TORIKOMI_DTL").Rows(i + 1))

                        ' 受信DTL⇒EDI出荷(大)へのデータセット
                        setDs = Me.SetSemiOutkaEdiL(setDs, sWhcd, purpose, drEdiRcvHed)
...
#Region "データセット設定 EDI出荷受信テーブル(ヘッダ)"

    ''' <summary>
    ''' データセット設定 EDI出荷受信テーブル(ヘッダ)
    ''' </summary>
    ''' <param name="ds"></param>
    ''' <param name="i"></param>
    ''' <param name="drEdiRcvHed"></param>
    ''' <returns></returns>
    Private Function SetSemiOutkaEdiRcvHed(ByVal ds As DataSet, ByVal i As Integer, ByVal drEdiRcvHed As DataRow) As DataSet
        ...
        ' EDI受信DTL設定
        drEdiRcvHed("DEL_KB") = "0"
        drEdiRcvHed("CRT_DATE") = MyBase.GetSystemDate()
        drEdiRcvHed("FILE_NAME") = drSetDtl("FILE_NAME_RCV")
        drEdiRcvHed("REC_NO") = Convert.ToInt32(drSetDtl("REC_NO")).ToString("00000")
        drEdiRcvHed("NRS_BR_CD") = drSemiEdiInfo("NRS_BR_CD")
        drEdiRcvHed("EDI_CTL_NO") = String.Empty                                                            ' 後でセットする。
        drEdiRcvHed("EDI_CTL_NO_CHU") = String.Empty                                                        ' 後でセットする。
        drEdiRcvHed("OUTKA_CTL_NO") = DEF_CTL_NO
        drEdiRcvHed("OUTKA_CTL_NO_CHU") = "000"
        drEdiRcvHed("CUST_CD_L") = drSemiEdiInfo("CUST_CD_L")                                               ' 荷主コード（大）
        drEdiRcvHed("CUST_CD_M") = drSemiEdiInfo("CUST_CD_M")                                               ' 荷主コード（中）
        drEdiRcvHed("PRTFLG") = "0"                                                                         ' プリントフラグ

        drEdiRcvHed("CUST_ORD_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 30)              ' 荷主注文番号（全体）　←受信データ(固定長テキスト) 伝票番号
        drEdiRcvHed("DENPYO_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 8)               ' 伝票日付　　　　　　　←受信データ(固定長テキスト) 伝票日付
        drEdiRcvHed("IRAI_SAKI_WH_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 5)           ' 依頼先倉庫コード　　　←受信データ(固定長テキスト) 依頼先倉庫コード
        drEdiRcvHed("DENPYO_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 2)                 ' 伝票区分　　　　　　　←受信データ(固定長テキスト) 伝票区分
        drEdiRcvHed("DENPYO_SB") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 2)                 ' 伝票種別　　　　　　　←受信データ(固定長テキスト) 伝票種別
        drEdiRcvHed("SEND_WH_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 5)                ' 送り先倉庫コード　　　←受信データ(固定長テキスト) 送り先倉庫コード
        drEdiRcvHed("DEST_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 80)                 ' 届先名　　　　　　　　←受信データ(固定長テキスト) 送り先名
        drEdiRcvHed("DEST_ZIP") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 10)                ' 届先郵便番号　　　　　←受信データ(固定長テキスト) 送り先郵便番号
        drEdiRcvHed("DEST_AD_1") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 240)              ' 届先住所1 　　　　　　←受信データ(固定長テキスト) 送り先住所１
        drEdiRcvHed("DEST_AD_2") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 240)              ' 届先住所2 　　　　　　←受信データ(固定長テキスト) 送り先住所２
        drEdiRcvHed("DEST_TEL") = Me._Blc.LeftB(drSetDtl("COLUMN_14").ToString().Trim(), 20)                ' 届先電話番号　　　　　←受信データ(固定長テキスト) 送り先電話番号
        drEdiRcvHed("REMARK") = Me._Blc.LeftB(drSetDtl("COLUMN_15").ToString().Trim(), 100)                 ' 出荷時注意事項　　　　←受信データ(固定長テキスト) 受取人名
        drEdiRcvHed("SEND_MOTO_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_16").ToString().Trim(), 40)            ' 送り元名　　　　　　　←受信データ(固定長テキスト) 送り元名
        drEdiRcvHed("SEND_MOTO_ZIP") = Me._Blc.LeftB(drSetDtl("COLUMN_17").ToString().Trim(), 7)            ' 送り元郵便番号　　　　←受信データ(固定長テキスト) 送り元郵便番号
        drEdiRcvHed("SEND_MOTO_AD_1") = Me._Blc.LeftB(drSetDtl("COLUMN_18").ToString().Trim(), 40)          ' 送り元住所１　　　　　←受信データ(固定長テキスト) 送り元住所１
        drEdiRcvHed("SEND_MOTO_AD_2") = Me._Blc.LeftB(drSetDtl("COLUMN_19").ToString().Trim(), 40)          ' 送り元住所２　　　　　←受信データ(固定長テキスト) 送り元住所２
        drEdiRcvHed("SEND_MOTO_TEL") = Me._Blc.LeftB(drSetDtl("COLUMN_20").ToString().Trim(), 15)           ' 送り元電話番号　　　　←受信データ(固定長テキスト) 送り元電話番号
        drEdiRcvHed("SEND_NIN_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_21").ToString().Trim(), 40)             ' 送り人名　　　　　　　←受信データ(固定長テキスト) 送り人名
        drEdiRcvHed("OUTKA_PLAN_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_22").ToString().Trim(), 8)          ' 出荷日　　　　　　　　←受信データ(固定長テキスト) 出荷日
        drEdiRcvHed("ARR_PLAN_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_23").ToString().Trim(), 8)            ' 納入予定日　　　　　　←受信データ(固定長テキスト) 着荷予定日
        drEdiRcvHed("ARR_PLAN_TIME") = Me._Blc.LeftB(drSetDtl("COLUMN_24").ToString().Trim(), 5)            ' 着荷予定時刻　　　　　←受信データ(固定長テキスト) 着荷予定時刻
        drEdiRcvHed("OUTKA_METHOD") = Me._Blc.LeftB(drSetDtl("COLUMN_25").ToString().Trim(), 1)             ' 出荷方法　　　　　　　←受信データ(固定長テキスト) 出荷方法
        drEdiRcvHed("DENP_YN") = Me._Blc.LeftB(drSetDtl("COLUMN_26").ToString().Trim(), 1)                  ' 送り状出力有無　　　　←受信データ(固定長テキスト) 送り状出力有無
        drEdiRcvHed("RETURN_DENP_YN") = Me._Blc.LeftB(drSetDtl("COLUMN_27").ToString().Trim(), 1)           ' 返却用送り状出力有無　←受信データ(固定長テキスト) 返却用送り状出力有無
        drEdiRcvHed("REPAIR_DENP_YN") = Me._Blc.LeftB(drSetDtl("COLUMN_28").ToString().Trim(), 1)           ' 修理品送り状出力有無　←受信データ(固定長テキスト) 修理品送り状出力有無
        drEdiRcvHed("RECEIPT_YN") = Me._Blc.LeftB(drSetDtl("COLUMN_29").ToString().Trim(), 1)               ' 受領書出力有無　　　　←受信データ(固定長テキスト) 受領書出力有無
        drEdiRcvHed("ASSEMBLY_YN") = Me._Blc.LeftB(drSetDtl("COLUMN_30").ToString().Trim(), 1)              ' アセンブリ指示有無　　←受信データ(固定長テキスト) アセンブリ指示有無
        drEdiRcvHed("FURIWAKE_ID") = Me._Blc.LeftB(drSetDtl("COLUMN_31").ToString().Trim(), 1)              ' 振り分けＩＤ　　　　　←受信データ(固定長テキスト) 振り分けＩＤ
        drEdiRcvHed("NOTES") = Me._Blc.LeftB(drSetDtl("COLUMN_32").ToString().Trim(), 99)                   ' 備考　　　　　　　　　←受信データ(固定長テキスト) 備考
        drEdiRcvHed("UNSOCO_DEPOT_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_33").ToString().Trim(), 10)         ' 配送業者デポコード　　←受信データ(固定長テキスト) 配送業者デポコード
        drEdiRcvHed("UNSOCO_INQ_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_34").ToString().Trim(), 12)           ' 配送業者問合せ番号　　←受信データ(固定長テキスト) 配送業者問合せ番号
        drEdiRcvHed("PROD_OUTKA_DENPYO_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_35").ToString().Trim(), 10)    ' 製品出荷伝票番号　　　←受信データ(固定長テキスト) 製品出荷伝票番号
        drEdiRcvHed("UNSOCO_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_36").ToString().Trim(), 8)                ' 配送業者コード　　　　←受信データ(固定長テキスト) 配送業者コード
        drEdiRcvHed("ASSORT_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_37").ToString().Trim(), 20)               ' 仕分けコード　　　　　←受信データ(固定長テキスト) 仕分けコード
        drEdiRcvHed("ONDO_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_38").ToString().Trim(), 5)                  ' 温度区分　　　　　　　←受信データ(固定長テキスト) 温度区分
        drEdiRcvHed("SYUKA_TIME_JOKEN") = Me._Blc.LeftB(drSetDtl("COLUMN_39").ToString().Trim(), 10)        ' 集荷指定時刻条件　　　←受信データ(固定長テキスト) 集荷指定時刻条件
        drEdiRcvHed("SERVICE_LEVEL_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_40").ToString().Trim(), 5)         ' サービスレベル区分　　←受信データ(固定長テキスト) サービスレベル区分
        drEdiRcvHed("DATA_SKBT_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_41").ToString().Trim(), 1)             ' データ識別区分　　　　←受信データ(固定長テキスト) データ識別区分
        drEdiRcvHed("PRIOR_NOTICE_FLG") = Me._Blc.LeftB(drSetDtl("COLUMN_42").ToString().Trim(), 1)         ' 事前通知希望フラグ　　←受信データ(固定長テキスト) 事前通知希望フラグ
        drEdiRcvHed("PRIOR_NOTICE_EMAIL_AD") = Me._Blc.LeftB(drSetDtl("COLUMN_43").ToString().Trim(), 60)   ' 事前通知e-mailアドレス←受信データ(固定長テキスト) 事前通知e-mailアドレス
        drEdiRcvHed("IMP_DEV_SB") = Me._Blc.LeftB(drSetDtl("COLUMN_44").ToString().Trim(), 1)               ' 入力機種種別　　　　　←受信データ(固定長テキスト) 入力機種種別
        drEdiRcvHed("MESSAGE_AREA") = Me._Blc.LeftB(drSetDtl("COLUMN_45").ToString().Trim(), 100)           ' メッセージエリア　　　←受信データ(固定長テキスト) メッセージエリア

        'データセットに設定
        ds.Tables("LMH030_H_OUTKAEDI_HED_TRMSMPL").Rows.Add(drEdiRcvHed)

        Return ds

    End Function

#End Region ' "データセット設定 EDI出荷受信テーブル(ヘッダ)"
...
#Region "データセット設定 EDI出荷受信テーブル(明細)"
    ...
    ''' <returns></returns>
    Private Function SetSemiOutkaEdiRcvDtl(ByVal ds As DataSet, ByVal i As Integer, ByVal gyo As Integer, ByVal drEdiRcvDtl As DataRow) As DataSet
        ...
        drEdiRcvDtl("DENPYO_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 10)                ' 伝票番号　　　　　　　←受信データ(固定長テキスト) 伝票番号
        drEdiRcvDtl("DENPYO_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 8)               ' 伝票日付　　　　　　　←受信データ(固定長テキスト) 伝票日付
        drEdiRcvDtl("IRAI_SAKI_WH_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 5)           ' 依頼先倉庫コード　　　←受信データ(固定長テキスト) 依頼先倉庫コード
        drEdiRcvDtl("DENPYO_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 2)                 ' 伝票区分　　　　　　　←受信データ(固定長テキスト) 伝票区分
        drEdiRcvDtl("DENPYO_SB") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 2)                 ' 伝票種別　　　　　　　←受信データ(固定長テキスト) 伝票種別
        drEdiRcvDtl("DTL_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 3)                    ' 明細番号　　　　　　　←受信データ(固定長テキスト) 明細番号
        drEdiRcvDtl("CUST_GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 20)           ' 商品コード　　　　　　←受信データ(固定長テキスト) 商品コード
        drEdiRcvDtl("LOT_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 15)                  ' ロット番号　　　　　　←受信データ(固定長テキスト) ロット番号
        drEdiRcvDtl("PRODUCT_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 12)              ' 製造番号　　　　　　　←受信データ(固定長テキスト) 製造番号
        drEdiRcvDtl("VERSION") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 5)                  ' バージョン　　　　　　←受信データ(固定長テキスト) バージョン
        drEdiRcvDtl("GOODS_SB") = Me._Blc.LeftB(drSetDtl("COLUMN_14").ToString().Trim(), 1)                 ' 商品種別　　　　　　　←受信データ(固定長テキスト) 商品種別
        drEdiRcvDtl("GOODS_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_15").ToString().Trim(), 60)                ' 商品名　　　　　　　　←受信データ(固定長テキスト) 商品名
        drEdiRcvDtl("PURPOSE") = Me._Blc.LeftB(drSetDtl("COLUMN_16").ToString().Trim(), 2)                  ' 使用目的　　　　　　　←受信データ(固定長テキスト) 使用目的
        drEdiRcvDtl("OUTKA_TTL_NB") = Me._Blc.LeftB(drSetDtl("COLUMN_17").ToString().Trim(), 10)            ' 出荷総個数　　　　　　←受信データ(固定長テキスト) 数量
        drEdiRcvDtl("TANKA") = Me._Blc.LeftB(drSetDtl("COLUMN_18").ToString().Trim(), 10)                   ' 単価　　　　　　　　　←受信データ(固定長テキスト) 単価
        drEdiRcvDtl("GK") = Me._Blc.LeftB(drSetDtl("COLUMN_19").ToString().Trim(), 10)                      ' 金額　　　　　　　　　←受信データ(固定長テキスト) 金額
        drEdiRcvDtl("USER_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_20").ToString().Trim(), 40)                 ' ユーザ名　　　　　　　←受信データ(固定長テキスト) ユーザ名
        drEdiRcvDtl("HOSP_DEPT") = Me._Blc.LeftB(drSetDtl("COLUMN_21").ToString().Trim(), 40)               ' 診療科　　　　　　　　←受信データ(固定長テキスト) 診療科
        drEdiRcvDtl("DOCTOR_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_22").ToString().Trim(), 20)               ' ドクター名　　　　　　←受信データ(固定長テキスト) ドクター名
        drEdiRcvDtl("RENT_FROM") = Me._Blc.LeftB(drSetDtl("COLUMN_23").ToString().Trim(), 8)                ' 貸出期間FROM　　　　　←受信データ(固定長テキスト) 貸出期間FROM
        drEdiRcvDtl("RENT_TO") = Me._Blc.LeftB(drSetDtl("COLUMN_24").ToString().Trim(), 8)                  ' 貸出期間TO　　　　　　←受信データ(固定長テキスト) 貸出期間TO
        drEdiRcvDtl("RENT_DEV_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_25").ToString().Trim(), 1)              ' 貸出機区分　　　　　　←受信データ(固定長テキスト) 貸出機区分
        drEdiRcvDtl("BOX_PIECE_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_26").ToString().Trim(), 1)             ' 正梱・バラ区分　　　　←受信データ(固定長テキスト) 正梱・バラ区分
        drEdiRcvDtl("SCI_MGMT_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_27").ToString().Trim(), 20)             ' 学術管理Ｎｏ．　　　　←受信データ(固定長テキスト) 学術管理Ｎｏ．
        drEdiRcvDtl("CLASS_BR") = Me._Blc.LeftB(drSetDtl("COLUMN_28").ToString().Trim(), 1)                 ' クラス分類　　　　　　←受信データ(固定長テキスト) クラス分類
        drEdiRcvDtl("MAINT_SPECIFIC") = Me._Blc.LeftB(drSetDtl("COLUMN_29").ToString().Trim(), 1)           ' 保守特定　　　　　　　←受信データ(固定長テキスト) 保守特定
        drEdiRcvDtl("USER_NM_FLG") = Me._Blc.LeftB(drSetDtl("COLUMN_30").ToString().Trim(), 1)              ' ユーザ名出力フラグ　　←受信データ(固定長テキスト) ユーザ名出力フラグ
        drEdiRcvDtl("USER_AD") = Me._Blc.LeftB(drSetDtl("COLUMN_31").ToString().Trim(), 80)                 ' ユーザ住所　　　　　　←受信データ(固定長テキスト) ユーザ住所
        drEdiRcvDtl("PATIENT_ID") = Me._Blc.LeftB(drSetDtl("COLUMN_32").ToString().Trim(), 10)              ' 患者ID　　　　　　　　←受信データ(固定長テキスト) 患者ID
        drEdiRcvDtl("INSTALL_PLACE") = Me._Blc.LeftB(drSetDtl("COLUMN_33").ToString().Trim(), 30)           ' 設置場所　　　　　　　←受信データ(固定長テキスト) 設置場所
        drEdiRcvDtl("REPL_PIC") = Me._Blc.LeftB(drSetDtl("COLUMN_34").ToString().Trim(), 2)                 ' 交換対応者　　　　　　←受信データ(固定長テキスト) 交換対応者
        drEdiRcvDtl("DELIV_DT") = Me._Blc.LeftB(drSetDtl("COLUMN_35").ToString().Trim(), 10)                ' 宅配日　　　　　　　　←受信データ(固定長テキスト) 宅配日
        drEdiRcvDtl("REPL_DEV") = Me._Blc.LeftB(drSetDtl("COLUMN_36").ToString().Trim(), 12)                ' 交換対象機　　　　　　←受信データ(固定長テキスト) 交換対象機
        drEdiRcvDtl("PATIENT_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_37").ToString().Trim(), 50)              ' 患者名称　　　　　　　←受信データ(固定長テキスト) 患者名称
        drEdiRcvDtl("DEST_ZIP") = Me._Blc.LeftB(drSetDtl("COLUMN_38").ToString().Trim(), 7)                 ' お届け先郵便番号　　　←受信データ(固定長テキスト) お届け先郵便番号
        drEdiRcvDtl("DEST_AD_1") = Me._Blc.LeftB(drSetDtl("COLUMN_39").ToString().Trim(), 100)              ' お届け先住所　　　　　←受信データ(固定長テキスト) お届け先住所
        drEdiRcvDtl("DEST_TEL") = Me._Blc.LeftB(drSetDtl("COLUMN_40").ToString().Trim(), 15)                ' お届け先電話番号　　　←受信データ(固定長テキスト) お届け先電話番号

        'データセットに設定
        ds.Tables("LMH030_H_OUTKAEDI_DTL_TRMSMPL").Rows.Add(drEdiRcvDtl)
...
#Region "データセット設定 EDI出荷データ 用 EDI管理番号(大・中)"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
                               , ByVal iDeleteFlg As Integer, ByVal iSkipFlg As Integer, ByVal bSameKeyFlg As Boolean _
                                ...
                                , ByRef drEdiRcv As DataRow) As DataSet

        Dim sNrsBrCd As String = drEdiRcv.Item("NRS_BR_CD").ToString()


        If bSameKeyFlg Then
            ' 前行とキーが等しい場合　
            If drEdiRcv.Table.Columns.Contains("GYO") Then
                ' 受信データ(固定長テキスト) 明細レコードの場合
                ' EDI管理番号(中) をカウントアップ
                iEdiCtlNoChu = iEdiCtlNoChu + 1
            End If
        Else
            ' 前行とキーが異なる場合
            ' 受信データ(固定長テキスト) ヘッダレコードの場合のみの想定(それ以外のケースはエラーとしている)
            ' 受信データ(固定長テキスト) ヘッダレコードの処理タイミングでは EDI管理番号(中) はゼロクリアのみ。
            ' 受信データ(固定長テキスト) 明細レコードの処理タイミング(前行とキーが等しいチェック済み) で EDI管理番号(中) を加算する。
            iEdiCtlNoChu = 0    ' ゼロクリア    
        End If
        ...
        If iSkipFlg = 0 Then
            ...
            ' 登録用EDI管理番号
            drEdiRcv.Item("EDI_CTL_NO") = sEdiCtlNo              ' DTLにセット
            If drEdiRcv.Table.Columns.Contains("GYO") Then
                ' 受信データ(固定長テキスト) 明細レコードの場合
                drEdiRcv.Item("EDI_CTL_NO_CHU") = iEdiCtlNoChu.ToString("000")  ' EDI_CHUにセット
            Else
                drEdiRcv.Item("EDI_CTL_NO_CHU") = "000"                         ' EDI_CHUに固定値をセット
            End If
        Else
            drEdiRcv.Item("EDI_CTL_NO") = DEF_CTL_NO             ' DTLに固定値をセット
            drEdiRcv.Item("EDI_CTL_NO_CHU") = "000"              ' EDI_CHUに固定値をセット
        End If
...
#Region "データセット設定(EDI出荷(中)"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet, ByVal drEdiRcvDtl As DataRow) As DataSet
        ...
        drOutkaEdiM("CUST_ORD_NO_DTL") = drEdiRcvDtl("DENPYO_NO")       ' 荷主注文番号（明細単位）←EDI出荷受信テーブル（明細）.伝票番号　　←受信データ(固定長テキスト) 伝票番号　※ヘッダとの一致はチェック済みの前提
        drOutkaEdiM("BUYER_ORD_NO_DTL") = String.Empty
        drOutkaEdiM("CUST_GOODS_CD") = drEdiRcvDtl("CUST_GOODS_CD")     ' 商品コード　　　　　　　←EDI出荷受信テーブル（明細）.商品コード　←受信データ(固定長テキスト) 商品コード
        drOutkaEdiM("NRS_GOODS_CD") = drGoods("GOODS_CD_NRS")
        drOutkaEdiM("GOODS_NM") = drEdiRcvDtl("GOODS_NM")               ' 商品名　　　　　　　　　←EDI出荷受信テーブル（明細）.商品名　　　←受信データ(固定長テキスト) 商品名

        drOutkaEdiM("RSV_NO") = String.Empty
        drOutkaEdiM("LOT_NO") = drEdiRcvDtl("LOT_NO")                   ' ロット№　　　　　　　　←EDI出荷受信テーブル（明細）.ロット番号　←受信データ(固定長テキスト) ロット番号
        drOutkaEdiM("SERIAL_NO") = String.Empty
        drOutkaEdiM("ALCTD_KB") = "01"

        drOutkaEdiM("OUTKA_PKG_NB") = 0
        drOutkaEdiM("OUTKA_HASU") = drEdiRcvDtl("OUTKA_TTL_NB")         ' 出荷端数　　　　　　　　←EDI出荷受信テーブル（明細）.出荷総個数　←受信データ(固定長テキスト) 数量
        drOutkaEdiM("OUTKA_QT") = 0
        drOutkaEdiM("OUTKA_TTL_NB") = drEdiRcvDtl("OUTKA_TTL_NB")       ' 出荷総個数　　　　　　　←EDI出荷受信テーブル（明細）.出荷総個数　←受信データ(固定長テキスト) 数量
        If String.IsNullOrEmpty(drGoods("STD_IRIME_NB").ToString()) Then
            drOutkaEdiM("OUTKA_TTL_QT") = 0
        Else
            ' 出荷総数量 = 出荷総個数 * 入目
            drOutkaEdiM("OUTKA_TTL_QT") = Convert.ToDecimal(drEdiRcvDtl("OUTKA_TTL_NB")) * Convert.ToDecimal(drGoods("STD_IRIME_NB"))
        End If
        ...
        drOutkaEdiM("FREE_C01") = drEdiRcvDtl("PURPOSE")                ' 文字列01　　　　　　　　←EDI出荷受信テーブル（明細）.使用目的　　←受信データ(固定長テキスト) 使用目的
...
#Region "データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
                                    ...
                                    , ByVal drEdiRcvHed As DataRow) As DataSet

        Dim drOutkaEdiL As DataRow = setDs.Tables("LMH030_OUTKAEDI_L").NewRow()

        drOutkaEdiL("DEL_KB") = "0"
        drOutkaEdiL("NRS_BR_CD") = drEdiRcvHed("NRS_BR_CD")
        drOutkaEdiL("EDI_CTL_NO") = drEdiRcvHed("EDI_CTL_NO")
        ...
        drOutkaEdiL("OUTKA_PLAN_DATE") = drEdiRcvHed("OUTKA_PLAN_DATE").ToString()  ' 出定日　　　　　　　←EDI出荷受信テーブル（ヘッダ）.出荷日　　　　　　　　　　　←受信データ(固定長テキスト) 出荷日
        drOutkaEdiL("OUTKO_DATE") = drEdiRcvHed("OUTKA_PLAN_DATE").ToString()       ' 出庫日　　　　　　　←EDI出荷受信テーブル（ヘッダ）.出荷日　　　　　　　　　　　←受信データ(固定長テキスト) 出荷日
        drOutkaEdiL("ARR_PLAN_DATE") = drEdiRcvHed("ARR_PLAN_DATE").ToString()      ' 納入予定日　　　　　←EDI出荷受信テーブル（ヘッダ）.納入予定日　　　　　　　　　←受信データ(固定長テキスト) 着荷予定日
        drOutkaEdiL("ARR_PLAN_TIME") = String.Empty

        drOutkaEdiL("HOKOKU_DATE") = String.Empty

        drOutkaEdiL("TOUKI_HOKAN_YN") = "1"
        drOutkaEdiL("CUST_CD_L") = drEdiRcvHed("CUST_CD_L")
        drOutkaEdiL("CUST_CD_M") = drEdiRcvHed("CUST_CD_M")
        ...
        Dim destZip As String = drEdiRcvHed("DEST_ZIP").ToString()
        Dim destTel As String = drEdiRcvHed("DEST_TEL").ToString()

        Dim destZip2 As String = destZip.Replace("-", "")
        If destZip2.Length < 7 Then
            destZip2 = destZip2.PadRight(7, " "c)
        End If
        Dim destTel2 As String = destTel.Replace("-", "")
        If destTel2.Length < 8 Then
            destTel2 = destTel2.PadLeft(8, " "c)
        End If
        ' 届先コード ←
        ' 　EDI出荷受信テーブル（ヘッダ）.届先郵便番号←受信データ(固定長テキスト) 送り先郵便番号 (ハイフン抜き上7桁) + 
        ' 　EDI出荷受信テーブル（ヘッダ）.届先電話番号←受信データ(固定長テキスト) 送り先電話番号 (ハイフン抜き下8桁)
        Dim destCd As String = String.Concat(
            Me._Blc.LeftB(destZip2, 7), destTel2.Substring(destTel2.Length - 8, 8))
        drOutkaEdiL("EDI_DEST_CD") = destCd                                         ' EDI届先コード 　　　←上記 届先郵便番号 および 届先電話番号 より編集した届先コード
        drOutkaEdiL("DEST_CD") = destCd                                             ' 届先コード　　　　　←上記 届先郵便番号 および 届先電話番号 より編集した届先コード
        drOutkaEdiL("DEST_NM") = drEdiRcvHed("DEST_NM")                             ' 届先名　　　　　　　←EDI出荷受信テーブル（ヘッダ）.届先名　　　　　　　　　　　←受信データ(固定長テキスト) 送り先名
        drOutkaEdiL("DEST_ZIP") = destZip.Replace("-", "")                          ' 届先郵便番号　　　　←EDI出荷受信テーブル（ヘッダ）.届先郵便番号 (ハイフン抜き) ←受信データ(固定長テキスト) 送り先郵便番号

        If LenB(drEdiRcvHed("DEST_AD_1").ToString()) <= 40 AndAlso
            LenB(drEdiRcvHed("DEST_AD_2").ToString()) <= 40 Then
            ' 受信データ(固定長テキスト) 送り先住所１, ２をそのままの桁数で設定できる場合
            drOutkaEdiL("DEST_AD_1") = drEdiRcvHed("DEST_AD_1")                     ' 届先住所1 　　　　　←EDI出荷受信テーブル（ヘッダ）.届先住所1 　　　　　　　　　←受信データ(固定長テキスト) 送り先住所１
            drOutkaEdiL("DEST_AD_2") = drEdiRcvHed("DEST_AD_2")                     ' 届先住所2 　　　　　←EDI出荷受信テーブル（ヘッダ）.届先住所2 　　　　　　　　　←受信データ(固定長テキスト) 送り先住所２
        ...
        Else
            ' 住所編集（住所を格納）
            ' 届先住所1～5 ←
            ' 　EDI出荷受信テーブル（ヘッダ）.届先住所1←受信データ(固定長テキスト) 送り先住所１ +
            ' 　EDI出荷受信テーブル（ヘッダ）.届先住所2←受信データ(固定長テキスト) 送り先住所２の分割文字列
            Dim mergeAddr As String = String.Concat(drEdiRcvHed("DEST_AD_1").ToString(),
                                                drEdiRcvHed("DEST_AD_2").ToString())
            Dim ad() As String = editAddr(mergeAddr)
            drOutkaEdiL("DEST_AD_1") = ad(1)
            drOutkaEdiL("DEST_AD_2") = ad(2)
            drOutkaEdiL("DEST_AD_3") = ad(3)
            drOutkaEdiL("DEST_AD_4") = ad(4)
            drOutkaEdiL("DEST_AD_5") = ad(5)
        End If

        drOutkaEdiL("DEST_TEL") = destTel                                           ' 届先電話番号　　　　←EDI出荷受信テーブル（ヘッダ）.届先電話番号　　　　　　　　←受信データ(固定長テキスト) 送り先電話番号
        ...
        drOutkaEdiL("CUST_ORD_NO") = drEdiRcvHed("CUST_ORD_NO")                     ' 荷主注文番号（全体）←EDI出荷受信テーブル（ヘッダ）.荷主注文番号（全体）　　　　←受信データ(固定長テキスト) 伝票番号
        ...
        ' 注意事項 ←
        '   EDI出荷受信テーブル（ヘッダ）.出荷時注意事項←受信データ(固定長テキスト) 受取人名 + " " +
        '   EDI出荷受信テーブル（明細）.使用目的←受信データ(固定長テキスト) 使用目的 の区分値が指す使用目的(名称)
        Dim purposeNm As String = ""
        Dim where As String = String.Concat("KBN_GROUP_CD = 'T034' AND KBN_CD = '", purpose, "'")
        Dim drPurpose As DataRow() = setDs.Tables("LMH030_Z_KBN_OUT").Select(where)
        If drPurpose.Count() > 0 Then
            purposeNm = String.Concat(" ", drPurpose(0).Item("KBN_NM1").ToString())
        End If
        Dim remark As String = String.Concat(drEdiRcvHed("REMARK").ToString(), purposeNm)
        drOutkaEdiL("REMARK") = remark
        ...
        drOutkaEdiL("FREE_C01") = drEdiRcvHed("CUST_ORD_NO")            ' 文字列01　　　　　　←EDI出荷受信テーブル（ヘッダ）.荷主注文番号（全体）　　　　←受信データ(固定長テキスト) 伝票番号
...
#Region "固定長レコード分割後のカラムよりの伝票種別取り出し"

    Private Function GetDenpyoSb(ByVal dr As DataRow) As String

        Return dr.Item("COLUMN_8").ToString()

    End Function

#End Region ' "固定長レコード分割後のカラムよりの伝票種別取り出し"

#Region "固定長レコード分割後のカラムよりの使用目的取り出し"

    Private Function GetPurpose(ByVal dr As DataRow) As String

        If GetDenpyoSb(dr).ToString() = "H1" Then
            ' ヘッダ
        ...
        Return dr.Item("COLUMN_16").ToString()

    End Function

#End Region ' "固定長レコード分割後のカラムよりの使用目的取り出し"
...
#Region "住所編集"
    ...
    ''' 住所編集
    ''' </summary>
    ''' <param name="destAd"></param>
    ''' <returns>文字配列 (1)～(5): 住所1～5, (0): 分割した住所 1～5 の結合文字列</returns>
    Private Function editAddr(ByVal destAd As String) As String()

        Dim ad(5) As String
        ad(0) = String.Empty

        Dim mergeAddr As String = destAd
        Dim cutAddr As String = String.Empty
        ' 1
        cutAddr = Me._Blc.LeftB(mergeAddr, 40)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        ad(1) = cutAddr
        ' 2
        cutAddr = Me._Blc.LeftB(mergeAddr, 40)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        ad(2) = cutAddr
        ' 3
        cutAddr = Me._Blc.LeftB(mergeAddr, 80)
        mergeAddr = mergeAddr.Substring(cutAddr.Length())
        ad(3) = cutAddr
        ' 4
        cutAddr = Me._Blc.LeftB(mergeAddr, 40)
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        ' 作業レコードデータセット設定
        Dim retSts As Boolean = True
        ds = Me.SetDatasetSagyo(ds, rowNo, ediCtlNo, retSts)
        If Not retSts Then
            Return ds
        End If
...
#Region "データセット設定(出荷L)"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            outkaDr("NHS_REMARK") = ediDr("FREE_C03")     'UPD 2020/06/15  
...
#Region "データセット設定(作業)"
    ...
    ''' <param name="retSts"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetDatasetSagyo(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String, ByRef retSts As Boolean) As DataSet
        ...
        ' 作業項目マスタの取得(営業所コード、荷主コード(大) 単位)
        Dim sagyomInDt As DataTable = ds.Tables("LMH030_M_SAGYO_IN")
        sagyomInDt.Rows.Clear()
        Dim sagyomInDr As DataRow = sagyomInDt.NewRow()
        sagyomInDr("NRS_BR_CD") = ediDrL("NRS_BR_CD")
        sagyomInDr("CUST_CD_L") = ediDrL("CUST_CD_L")
        sagyomInDt.Rows.Add(sagyomInDr)
        ds = MyBase.CallDAC(Me._Dac, "SelectMSagyo", ds)
        Dim sagyomOutDt As DataTable = ds.Tables("LMH030_M_SAGYO")
        ...
        For i As Integer = 0 To max
            ...
            For j As Integer = 1 To 5
                ...
                If String.IsNullOrEmpty(sagyoCD) = False Then
                    ' 作業項目マスタの取得
                    Dim sagyomDr As DataRow = GetSagyoMRec(sagyomOutDt, sagyoCD, rowNo, ediCtlNo)
                    If sagyomDr Is Nothing Then
                        retSts = False
                        Return ds
                    End If
                    ...
                    sagyoDr("SAGYO_NB") = ediDrM("OUTKA_TTL_NB")
                    sagyoDr("SAGYO_GK") = Math.Round(Convert.ToDecimal(sagyoDr("SAGYO_NB").ToString()) * Convert.ToDecimal(sagyomDr("SAGYO_UP").ToString()), MidpointRounding.AwayFromZero)
    ...
    ''' <summary>
    ''' 作業項目マスタの取得(作業コードで1件に絞り込む)
    ''' </summary>
    ''' <param name="sagyomOutDt"></param>
    ''' <param name="sagyoCD"></param>
    ''' <param name="rowNo"></param>
    ''' <param name="ediCtlNo"></param>
    ''' <returns></returns>
    Private Function GetSagyoMRec(ByVal sagyomOutDt As DataTable, ByVal sagyoCD As String, ByVal rowNo As String, ByVal ediCtlNo As String) As DataRow

        ' 作業項目マスタの取得(作業コードで1件に絞り込む)
        Dim sagyomDrs As DataRow() = sagyomOutDt.Select(String.Concat("SAGYO_CD = '", sagyoCD, "'"))
        Dim sagyomDr As DataRow = Nothing

        If sagyomDrs.Length = 0 Then
            ' 作業マスタに作業項目コードが登録されていない場合はエラー
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E492", New String() {"作業項目マスタ", "作業項目コード",
                                                                                             String.Concat("作業項目コード: ", sagyoCD)},
                                                                                            rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
        Else
            sagyomDr = sagyomDrs(0)
        End If
