...
'  EDI荷主ID　　　　:  120　　　        : インターコンチ（横浜）(000ベース生産2017/08/25)
'  作  成  者       :  daikoku　       
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC405 = New LMH030DAC405()
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
                                    ...
                                    , ByVal itargetUpdFlg As Integer _
                                    , ByVal iCntDiffFlg As Integer _
                                    , ByRef bNoErr As Boolean) As DataSet
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            ...
            '荷主明細マスタ（SUB_KB=87の取得:受信個数が、端数の場合）
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            ...
            If drOutkaEdiM.Item("ALCTD_KB").Equals("01") Then
                ...
                If drMgoods Is Nothing OrElse String.IsNullOrEmpty(drMgoods.Item("STD_IRIME_NB").ToString()) = True Then
                    ...
                    '2015.03.23 横浜・古川エージェンシー対応 END
                Else
                    '2018/01/18 Annen 【LMS】セミEDI_YCCインターコンチネンタル-本数→ケース数へ変更対応 upd start
                    drOutkaEdiM("OUTKA_QT") = Convert.ToDecimal(drRcvEdiDtl.Item(sTempColNo).ToString().Trim())
                    ...
                    '2018/01/18 Annen 【LMS】セミEDI_YCCインターコンチネンタル-本数→ケース数へ変更対応 upd end
                End If
            Else
                '②そのまま
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            ...
            If drOutkaEdiM.Item("ALCTD_KB").Equals("01") Then
                '2018/01/18 Annen 【LMS】セミEDI_YCCインターコンチネンタル-本数→ケース数へ変更対応 upd start
                '①出荷総個数 = ファイルから取得した個数 ÷ 入目
                If Convert.ToDecimal(drRcvEdiDtl.Item(sTempColNo).ToString().Trim()) Mod Convert.ToDecimal(drOutkaEdiM("IRIME")) <> 0 Then
                    '端数が発生したらエラーとする
                    MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E976", , , LMH010BLC.EXCEL_COLTITLE, sEdiCtlNo)
                    bNoErr = False
                    Return setDs
                End If
                Dim outokaTtlNb As Decimal = Math.Floor(Convert.ToDecimal(drRcvEdiDtl.Item(sTempColNo).ToString().Trim()) / Convert.ToDecimal(drOutkaEdiM("IRIME")))
                drOutkaEdiM("OUTKA_TTL_NB") = outokaTtlNb
                ''①そのまま
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            ...
            Else
                ...
                Else
                    ...
                    If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
                        '2018/01/18 Annen 【LMS】セミEDI_YCCインターコンチネンタル-本数→ケース数へ変更対応 upd start
                        drOutkaEdiM("OUTKA_TTL_QT") = Convert.ToDecimal(drRcvEdiDtl.Item(sTempColNo).ToString().Trim())
...
#End If
            ...
            '2018/01/19 Annen 【LMS】セミEDI_YCCインターコンチネンタル-本数→ケース数へ変更対応 upd start
            'setDs = Me.SetSemiOutkaEdiM(setDs, i, sEdiCtlNo, iEdiCtlNoChu, iFindOutkaEdiFlg)
            setDs = Me.SetSemiOutkaEdiM(setDs, i, sEdiCtlNo, iEdiCtlNoChu, iFindOutkaEdiFlg, itargetUpdFlg, iCntDiffFlg, bNoErr)
            If bNoErr.Equals(False) Then
                Continue For
            End If
            '2018/01/19 Annen 【LMS】セミEDI_YCCインターコンチネンタル-本数→ケース数へ変更対応 upd end
