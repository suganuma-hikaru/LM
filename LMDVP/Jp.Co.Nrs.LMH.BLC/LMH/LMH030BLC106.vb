...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC106 = New LMH030DAC106()
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        '東レダウ追加箇所：共通　terakawa 2012.10.15 Start
        ''届先コードの初期値設定
        'ds = Me.SetDestCd(ds)
        '東レダウ追加箇所：共通　terakawa 2012.10.15 End
        ...
        '東レダウ追加箇所 honmyo 2012.09.28 Start
        '東レダウはまとめ対象荷主(チェックして出荷登録できたデータだけでまとめを行う)
        ...
        If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
            ...
            ElseIf MyBase.GetResultCount > 1 Then
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.TOR_WID_L001, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningL("W199", LMH030BLC.TOR_WID_L001, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("0") = True Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.TOR_WID_L002, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = String.Empty
                    matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                    ds = Me._Blc.SetComWarningL("W168", LMH030BLC.TOR_WID_L002, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("1") = True Then
                ...
                If matomeStatus.Equals("10") = False Then
                    ...
                    choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.TOR_WID_L003, 0)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(4) = String.Empty
                        matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                        ds = Me._Blc.SetComWarningL("W198", LMH030BLC.TOR_WID_L003, ds, msgArray, matomeNo, String.Empty)
                        Return ds
        ...
        Select Case custIndex
            Case LMH030BLC.EdiCustIndex.TorChb00041_00
...
#Region "実績作成処理"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        '東レダウEDI実績データの作成
        ds = MyBase.CallDAC(Me._Dac, "InsertTOREdiSendData", ds)
...
#End Region
    ...
    Private Function SetSendDs(ByVal ds As DataSet) As DataSet
        ...
        Dim sendDr As DataRow = ds.Tables("LMH030_H_SENDOUTEDI_TOR").Rows(0)
        Dim sDt As DataTable = ds.Tables("LMH030_TOR_JISSEKI_OUTKAS")
...
#Region "EDI出荷(中)の初期値設定"
    ...
    Private Function EdiMDefaultSet(ByVal ds As DataSet, ByVal setDs As DataSet, _
        ...
        '東レダウ追加箇所：東レダウ　terakawa 2012.10.15 Start
        '入目
        Select Case custIndex
            Case LMH030BLC.EdiCustIndex.TorChb00041_00
                Dim ediIrime As String = ediMDr("IRIME").ToString()
                Dim mIrime As String = mGoodsDr("STD_IRIME_NB").ToString()
                Dim ediTtlQt As String = ediMDr("OUTKA_TTL_QT").ToString()

                '要望番号:1676 terakawa 2012.12.07 Start
                ...
                Else
                    If (ediIrime).Equals(mIrime) = False Then
                        ...
                        choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.TOR_WID_M004, count)

                        If String.IsNullOrEmpty(choiceKb) = True Then
                            msgArray(1) = "入目"
                            msgArray(2) = "商品マスタ"
                            msgArray(3) = "標準入目"
                            msgArray(4) = "数量=" & ediTtlQt & "、入目" & ediIrime & "、商品マスタ標準入目=" & mIrime
                            msgArray(5) = String.Empty
                            ds = Me._Blc.SetComWarningM("W195", LMH030BLC.TOR_WID_M004, ds, setDs, msgArray, ediIrime, mIrime)
                    ...
                    End If
                End If
                '要望番号:1676 terakawa 2012.12.07 End
            Case Else
                If Convert.ToDecimal(ediMDr("IRIME")) = 0 _
                AndAlso Convert.ToDecimal(mGoodsDr("STD_IRIME_NB")) <> 0 Then
                    ediMDr("IRIME") = mGoodsDr("STD_IRIME_NB")
                End If
        End Select
        '東レダウ追加箇所：東レダウ　terakawa 2012.10.15 End
        ...
        Dim freeN02 As Double = Convert.ToDouble(ediMDr("FREE_N02"))
        Dim freeN03 As Double = Convert.ToDouble(ediMDr("FREE_N03"))
        Dim freeC01 As String = ediMDr("FREE_C01").ToString()
        Dim freeC02 As String = ediMDr("FREE_C02").ToString()
        Dim hasu As Double = freeN02 Mod (irime * 1000)

        '東レダウ追加箇所：日通　terakawa 2012.10.15 Start
        Select Case custIndex
            Case LMH030BLC.EdiCustIndex.TorChb00041_00
                '東レダウの場合
                If freeN03 = 0 AndAlso freeC01.Equals("KGM") = True AndAlso String.IsNullOrEmpty(freeC02) = True Then
                    If hasu <= 0 Then
                        ediMDr("OUTKA_HASU") = freeN02 / (irime * 1000)
                        ediMDr("OUTKA_TTL_NB") = ediMDr("OUTKA_HASU")
                        ediMDr("FREE_C02") = "個数再計算済"

                    Else
                        'ワーニング
                        choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.TOR_WID_M003, count)
                        ...
                        If String.IsNullOrEmpty(choiceKb) = True Then
                            ...
                            ds = Me._Blc.SetComWarningM("W167", LMH030BLC.TOR_WID_M003, ds, setDs, msgArray, ediValue, String.Empty)
...
#Region "EDI出荷(大)のBLC側でのチェック"
    ...
    Private Function EdiLKanrenCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        '2018/03/05 001286【EDI出荷データ検索】保管・荷役料最終計算日より前の出荷データを登録できないようにする 対応 Annen add start
        Dim custrow As DataRow = ds.Tables("LMH030_M_CUST").Rows(0)
        '2018/03/05 001286【EDI出荷データ検索】保管・荷役料最終計算日より前の出荷データを登録できないようにする 対応 Annen add end
        ...
        '2018/03/05 001286【EDI出荷データ検索】保管・荷役料最終計算日より前の出荷データを登録できないようにする 対応 Annen add start
        '出荷予定日が保管・荷役料最終計算日以前の場合、エラーメッセージを出力し、Falseを返却する
        If Convert.ToInt32(dt.Rows(0).Item("OUTKA_PLAN_DATE")) <= Convert.ToInt32(custrow("HOKAN_NIYAKU_CALCULATION")) Then
            Dim targetOrderNo As String = dt.Rows(0).Item("CUST_ORD_NO").ToString
            'エラーメッセージ：
            'オーダー番号[%1]の出荷予定日が保管・荷役料最終計算日以前の日付のため出荷登録できません。([%1]は荷主注文番号)
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E977", New String() {String.Concat("【", targetOrderNo, "】")}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            Return False
        End If
        '2018/03/05 001286【EDI出荷データ検索】保管・荷役料最終計算日より前の出荷データを登録できないようにする 対応 Annen add start
...
#Region "EDI出荷(大)のDAC側でのチェック"
    ...
    Private Function EdiLDbExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        If (String.IsNullOrEmpty(shipCdL) = False AndAlso shipCdL.Equals(destCd) = True AndAlso shipCdL.Equals(ediDestCd) = False) _
            ...
            ds = MyBase.CallDAC(Me._Dac, "SelectDataTORMdest", ds)
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.TOR_WID_M001, 0)

                If choiceKb.Equals("03") = True Then
                    'ワーニングで"キャンセル"を選択時
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "G042", , rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                End If
                '2018/03/26 セミEDI_千葉ITW_新規登録対応 Annen upd start
                If custIndex = LMH030BLC.EdiCustIndex.ITW00750_00 Then
                ...
                End If
                '2018/03/26 セミEDI_千葉ITW_新規登録対応 Annen upd end
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    '東レダウ追加箇所：日通　terakawa 2012.10.15 Start
                    Select Case custIndex
                        Case LMH030BLC.EdiCustIndex.TorChb00637_00
                            '日通(東レダウ)の場合は、商品が複数存在した場合エラー
                            Dim sErrMsg As String = Me._Blc.GetErrMsgE493(setDs)
                        ...
                        Case Else
                            '入目 + 荷主商品コードで再検索
                            setDs = (MyBase.CallDAC(Me._Dac, "SelectDataMgoodsIrimeOutka", setDs))
                            ...
                            Else
                                ...
                                ds = Me._Blc.SetComWarningM("W162", LMH030BLC.TOR_WID_M001, ds, setDs, msgArray, custGoodsCd, String.Empty)
...
#Region "届先マスタチェック(東レダウ専用)"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        If mDestNm.Equals(ediDestNm) = True Then
            'チェックなし
        Else
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.TOR_WID_L004, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                ds = Me._Blc.SetComWarningL("W166", LMH030BLC.TOR_WID_L004, ds, msgArray, _
        ...
        Else
            ...
            If mAdAll.Equals(ediDestAdAll) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.TOR_WID_L005, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.TOR_WID_L005, ds, msgArray, ediDestAdAll, mAdAll)
        ...
        Else
            If mTel.Equals(ediTel) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.TOR_WID_L006, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.TOR_WID_L006, ds, msgArray, ediTel, mTel)
        ...
        If String.IsNullOrEmpty(mZipJis) = True AndAlso String.IsNullOrEmpty(ediDestJisCd) = False Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.TOR_WID_L007, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                ds = Me._Blc.SetComWarningL("W197", LMH030BLC.TOR_WID_L007, ds, msgArray, ediDestJisCd, mJis)
        ...
        ElseIf String.IsNullOrEmpty(mZipJis) = True AndAlso String.IsNullOrEmpty(ediDestJisCd) = True Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.TOR_WID_L008, 0)

            If String.IsNullOrEmpty(choiceKb) = True Then

                ds = Me._Blc.SetComWarningL("W188", LMH030BLC.TOR_WID_L008, ds, msgArray, ediDestJisCd, mJis)
        ...
        ElseIf String.IsNullOrEmpty(ediDestJisCd) = False AndAlso ediDestJisCd.Equals(mJis) = False Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.TOR_WID_L009, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(4) = "EDIデータ"
                msgArray(5) = String.Empty
                ds = Me._Blc.SetComWarningL("W187", LMH030BLC.TOR_WID_L009, ds, msgArray, ediDestJisCd, mJis)
        ...
        ElseIf String.IsNullOrEmpty(ediDestJisCd) = True AndAlso mZipJis.Equals(mJis) = False Then
            ...
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.TOR_WID_L010, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(5) = String.Empty
                ds = Me._Blc.SetComWarningL("W187", LMH030BLC.TOR_WID_L010, ds, msgArray, mZipJis, mJis)
...
#Region "届先マスタ追加時チェック"
    ...
    Private Function ZipCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String, ByVal workDestCd As String, ByVal workDestString As String) As Boolean
        ...
        choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.TOR_WID_L011, 0)
        ...
        If String.IsNullOrEmpty(choiceKb) = True Then
            ...
            ds = Me._Blc.SetComWarningL("W186", LMH030BLC.TOR_WID_L011, ds, msgArray, workDestCd, String.Empty) '追加箇所 20110222
...
#Region "届先マスタ(荷送人コード)追加時チェック"
    ...
    Private Function SetInsMDestFromShip(ByVal ds As DataSet) As Boolean
        ...
        Dim dtMS As DataTable = ds.Tables("LMH030_M_DEST_SHIP_CD_L")
        Dim msgArray(5) As String
        Dim choiceKb As String = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.TOR_WID_L012, 0)
        Dim drEdiL As DataRow = ds.Tables("LMH030_OUTKAEDI_L").Rows(0)
        ...
        If String.IsNullOrEmpty(choiceKb) = True Then
            ...
            ds = Me._Blc.SetComWarningL("W182", LMH030BLC.TOR_WID_L012, ds, msgArray, shipCdL, String.Empty)
...
#Region "届先マスタ(荷送人コード)更新時チェック"
    ...
    Private Function SetUpdMDestFromShip(ByVal ds As DataSet) As Boolean
        ...
        Dim dtMS As DataTable = ds.Tables("LMH030_M_DEST_SHIP_CD_L")
        Dim msgArray(5) As String
        Dim choiceKb As String = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.TOR_WID_L013, 0)
        Dim drEdiL As DataRow = ds.Tables("LMH030_OUTKAEDI_L").Rows(0)
        Dim shipCdL As String = drEdiL("SHIP_CD_L").ToString()
        ...
        If String.IsNullOrEmpty(choiceKb) = True Then
            ...
            ds = Me._Blc.SetComWarningL("W166", LMH030BLC.TOR_WID_L013, ds, msgArray, drEdiL("SHIP_NM_L").ToString(), dtMS.Rows(0).Item("DEST_NM").ToString())
