...
'  EDI荷主ID　　　　:  136　　　        : ローム(大阪)(2022/10/28(REV.1910)のセミEDI標準仕様000からコピー)
'  作  成  者       :  
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC407 = New LMH030DAC407()
...
#Else
                        ...
                        If (_IsFirmeType AndAlso _
                            Double.TryParse(TryCast(dtM.Rows(i)("IRIME"), String), warningIrime) AndAlso _
                            warningIrime > 0) Then
...
#End If
        ...
        drOutkaEdiM("FREE_C04") = drRcvEdiDtl("COLUMN_256").ToString().Trim()   'UPD 2022/10/28 033290 大阪ロームEDI改修
...
#Region "セミEDI時 データセット設定(キャンセル対象データ抽出)"
    ...
    Private Sub SetDatasetSelectCancelData(ByRef setDs As DataSet, ByVal RowNo As Integer)
        ...
        End If
        '出荷伝票
        setInOutDr.Item("FREE_C04") = drItems.Item("COLUMN_256").ToString().Trim()  'ADD 2022/10/28 033290 大阪ロームEDI改修
            ...
            Else
                ...
                For j As Integer = iRowCnt To max
                    ...
                    If (dr.Item("FILE_NAME_RCV").ToString().Trim()).Equals(dtSemiHed.Rows(i).Item("FILE_NAME_RCV").ToString().Trim()) = True Then
                        'ヘッダと明細のファイル名称が等しい場合
#If True Then   'ADD 2020/04/20 012056_【LMS】ローム_SAP出力EDIファイル仕様変更
                        If drSemiInfo.Item("NRS_BR_CD").ToString.Equals("40") = True AndAlso
                           drSemiInfo.Item("CUST_CD_L").ToString.Equals("00061") = True AndAlso
                           drSemiInfo.Item("CUST_CD_M").ToString.Equals("00") = True Then

                            If String.IsNullOrEmpty(dr.Item("COLUMN_22").ToString()) = True AndAlso
                        String.IsNullOrEmpty(dr.Item("COLUMN_23").ToString()) = True Then
                                'VとWが空白の場合は、今まで通りH列とI列を使用する。
                                dtSemiDtl.Rows(j).Item("COLUMN_22") = dr.Item("COLUMN_8").ToString()
                                dtSemiDtl.Rows(j).Item("COLUMN_23") = dr.Item("COLUMN_9").ToString()

                                dr.Item("COLUMN_22") = dr.Item("COLUMN_8").ToString()
                                dr.Item("COLUMN_23") = dr.Item("COLUMN_9").ToString()
                ...
                Else
                    If IsNumeric(sNum) Then
                        ...
                        If dNum > 9999999999 Then
                            ...
                            bRet = False
                        End If
                        If dNum - System.Math.Floor(dNum) <> 0 Then
                            '小数は不許可
                            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat(sMsg, dNum.ToString(), "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                            bRet = False
                        End If
