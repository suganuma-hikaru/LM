...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC104 = New LMH010DAC104()
...
#Region "入荷登録マスタ存在チェック(中)"
    Private Function SetGoodsMst(ByRef ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            '条件の再設定
            setDtM = Me.SetGoodsCdFromWarning(setDtM, ds, LMH010BLC.NSN_WID_M001)
            ...
            ElseIf GetResultCount() > 1 Then
                ...
                Else
                    ...
                    ds = Me._ChkBlc.SetWarningM("W162", LMH010BLC.NSN_WID_M001, ds, setDs, msgArray)
...
#Region "実績作成処理"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        Dim updFlg As String = ds.Tables("LMH010INOUT").Rows(0).Item("INKA_L_UPD_FLG").ToString()
        ...
        If Me.ChkSCount(ds) = False Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E504", New String() {"実績作成対象", "入荷(小)", "10件"}, rowNo, LMH010BLC.EXCEL_COLTITLE, ediCtlNo)
            Return ds
        End If
        ...
        '入荷(大)の更新
        If updFlg.Equals("1") = True Then   '要望番号:1815（千葉 日産物流 まとめた入荷を実績作成すると、排他エラーとなる) 2013/02/01 本明 Start
    ...
    Private Function ChkSCount(ByVal ds As DataSet) As Boolean
        ...
        Dim dt As DataTable = ds.Tables("LMH010_NSN_JISSEKI_INKAS")
    ...
    Private Function SetSendDs(ByVal ds As DataSet) As DataSet
        ...
        Dim sendDr As DataRow = ds.Tables("H_SENDINEDI_NSN").Rows(0)
        Dim sDt As DataTable = ds.Tables("LMH010_NSN_JISSEKI_INKAS")
