...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC159 = New LMH030DAC159()
...
#Region "丸和物産株式会社(CSV) CONST"

    ''' <summary>
    ''' その他
    ''' </summary>
    ''' <remarks></remarks>
    Public Const DEF_CTL_NO As String = "Y00000000"             '管理番号初期値
    Public Const DEF_UNSO_NO_L As String = "01-Y00000000"       '運送番号初期値
    Public Const DEF_UNSO_NO_M As String = "01-Y00000000000"    '運送番号初期値

#End Region ' "丸和物産株式会社(CSV) CONST"
...
#Region "画面取込(セミEDI)チェック"
    ...
    Private Function SemiEdiTorikomiChk(ByVal ds As DataSet) As DataSet
        ...
        Dim dtSemiDtlOrg As DataTable = dtSemiDtl.Copy()
        ...
        Dim colNoMax As Integer = Convert.ToInt32(dtSemiInfo.Rows(0).Item("RCV_FILE_COL_CNT").ToString) ' 列の最大数

        Dim iRowCnt As Integer = 0

        Dim sysChumonNoBreak As New HashSet(Of Integer)
        Dim addressBreak As New HashSet(Of Integer)

        '----------------------------------------------------------------------------------------- 
        ' 省略値を、先頭列(システム注文番号) の等しい(上方向)直近行より補う
        '----------------------------------------------------------------------------------------- 
        ' 1/2 ファイル全体を対象に値のある行から参照する列(システム注文番号、配達希望日)
        Dim targetColNo As Integer() = New Integer() {1, 67}
        For k As Integer = 1 To max
            Dim drPrev As DataRow = dtSemiDtl.Rows(k - 1)
            dr = dtSemiDtl.Rows(k)
            For m As Integer = 0 To targetColNo.Length() - 1
                Dim colNm As String = "COLUMN_" & targetColNo(m).ToString()
                If IsDBNull(dr.Item(colNm)) OrElse
                    String.IsNullOrEmpty(dr.Item(colNm).ToString()) Then
                    If IsDBNull(drPrev.Item(colNm)) Then
                        dr.Item(colNm) = ""
                    Else
                        dr.Item(colNm) = drPrev.Item(colNm)
                    End If
                End If
            Next
        Next
        For k As Integer = 1 To max
            Dim drPrev As DataRow = dtSemiDtl.Rows(k - 1)
            dr = dtSemiDtl.Rows(k)
            If drPrev.Item("COLUMN_1").ToString() = "" Then
                ' 1/2 の処理を経てなお、
                ' 直前行の先頭列(システム注文番号) が未設定の行は、
                ' 直前行とシステム注文番号が等しい場合に行う、
                ' 以降の列単位の処理を行う意味がないため、次の行の処理へ遷移する。
                Continue For
            End If
            If dr.Item("COLUMN_1").ToString() <> drPrev.Item("COLUMN_1").ToString() Then
                ' 直前行とシステム注文番号が等しくない場合、
                ' 値未設定の列は、直前行と同値で省略したのではなく、設定値なし("") であるため、
                ' 次の行の処理へ遷移する。
                sysChumonNoBreak.Add(k)
                Continue For
            End If

            ' 2/2 システム注文番号単位の先頭行を上限として値のある行から参照する列

            ' 住所(57列目「配送先都道府県」～60列目「配送先ビル等」の連結文字列) の編集
            ' 直前行, 現在行
            Dim adPrev As String = String.Concat(
                            drPrev.Item("COLUMN_57").ToString().Trim(), drPrev.Item("COLUMN_58").ToString().Trim(),
                            drPrev.Item("COLUMN_59").ToString().Trim(), drPrev.Item("COLUMN_60").ToString().Trim())
            Dim adCurrent As String = String.Concat(
                            dr.Item("COLUMN_57").ToString().Trim(), dr.Item("COLUMN_58").ToString().Trim(),
                            dr.Item("COLUMN_59").ToString().Trim(), dr.Item("COLUMN_60").ToString().Trim())
            Dim adBreak As Boolean = False
            If adCurrent <> "" AndAlso adPrev <> adCurrent Then
                addressBreak.Add(k)
                adBreak = True
            End If
            For m As Integer = 2 To colNoMax
                If 51 <= m AndAlso m <= 64 Then
                    ' 見出しに "配送先" と付く項目(51列目「配送先選択」～64列目「配送方法番号」) の場合
                    If adBreak Then
                        ' 住所(57列目「配送先都道府県」～60列目「配送先ビル等」の連結文字列) に元より値ありで直前行と異なる場合、
                        ' 見出しに "配送先" と付く項目は、直前行と同値で省略したのではなく、設定値なし("") であるため、
                        ' 次の列の処理へ遷移する。
                        Continue For
                    End If
                End If
                If (70 <= m AndAlso m <= 72) OrElse (75 <= m AndAlso m <= 77) Then
                    ' 70列目「商品コード」, 71列目「システム商品番号」, 72列目「商品名」
                    ' 75列目「価格（税込）」, 76列目「数量」, 77列目「金額
                    ' 各行固有の情報の設定が期待される列につき、直前行と同値で省略の判定/設定は行わない。
                    Continue For
                End If
                Dim colNm As String = "COLUMN_" & m.ToString()
                If IsDBNull(dr.Item(colNm)) OrElse
                    String.IsNullOrEmpty(dr.Item(colNm).ToString()) Then
                    If IsDBNull(drPrev.Item(colNm)) Then
                        dr.Item(colNm) = ""
            ...
            Else
                ...
                For j As Integer = iRowCnt To max
                    ...
                    If (dr.Item("FILE_NAME_RCV").ToString().Trim()).Equals(dtSemiHed.Rows(i).Item("FILE_NAME_RCV").ToString().Trim()) = True Then
                        ...
                        If Me.TorikomiValChk(dtSemiDtlOrg.Rows(j), j, sysChumonNoBreak, addressBreak) = False Then
...
#Region "カラム項目の値・日付チェック"
    ...
    ''' <param name="sysChumonNoBreak"></param>
    ''' <param name="addressBreak"></param>
    ''' <returns></returns>
    Public Function TorikomiValChk(ByVal dr As DataRow,
                                   ByVal dtlIdx As Integer,
                                   ByVal sysChumonNoBreak As HashSet(Of Integer),
                                   ByVal addressBreak As HashSet(Of Integer)) As Boolean
        ...
        Dim targetStr2 As String = String.Empty
        Dim targetStr3 As String = String.Empty
        Dim dNum As Double = 0
        Dim bRet As Boolean = True

        ' システム注文番号
        If dtlIdx = 0 OrElse sysChumonNoBreak.Contains(dtlIdx) Then
            ' 先頭行またはシステム注文番がブレイクした行のみチェックする。
            targetStr = dr.Item("COLUMN_1").ToString.Trim()
            ' 必須チェック
            If String.IsNullOrEmpty(targetStr) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("システム注文番号(カラム1番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
                ' 文字列長チェック
            ElseIf LenB(targetStr) > 100 Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("システム注文番号(カラム1番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
        End If
        ...
        If LenB(targetStr) > 30 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("フリー項目３(カラム47番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        ' 配送先郵便番号
        targetStr = dr.Item("COLUMN_56").ToString.Trim()
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            If dtlIdx = 0 OrElse sysChumonNoBreak.Contains(dtlIdx) OrElse addressBreak.Contains(dtlIdx) Then
                ' 先頭行またはシステム注文番がブレイクした行または住所がブレイクした行のみチェックする。
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("配送先郵便番号(カラム56番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 10, False).Equals(False) Or targetStr.Replace("-", "").Length < 7 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("配送先郵便番号(カラム56番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        ' 配送先都道府県、配送先市区町村、配送先町域・番地、配送先ビル等
        ' 配送先氏名（姓）、配送先氏名（名）
        targetStr = String.Concat(
                    dr.Item("COLUMN_57").ToString.Trim(), dr.Item("COLUMN_58").ToString.Trim(),
                    dr.Item("COLUMN_59").ToString.Trim())
        targetStr2 = dr.Item("COLUMN_60").ToString.Trim()
        targetStr3 = String.Concat(
                    dr.Item("COLUMN_52").ToString.Trim(), " ", dr.Item("COLUMN_53").ToString.Trim())
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            If dtlIdx = 0 OrElse sysChumonNoBreak.Contains(dtlIdx) OrElse addressBreak.Contains(dtlIdx) Then
                ' 先頭行またはシステム注文番がブレイクした行または住所がブレイクした行のみチェックする。
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001",
                                                   New String() {String.Concat("配送先都道府県、配送先市区町村、配送先町域・番地、配送先ビル等(カラム57-60番目)[", String.Concat(targetStr, If(targetStr2.Trim().Length() = 0, "", " "), targetStr2), "]")},
                                                   sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
            ' 文字列長チェック
        Else
            Dim ad() As String = editAddr(targetStr, targetStr2, targetStr3)
            If Not (ad(0).Equals(String.Concat(targetStr, targetStr2, targetStr3)) OrElse
                                    ad(0).Equals(String.Concat(targetStr, If(targetStr2.Trim().Length() = 0, "", " "), targetStr2, targetStr3)) OrElse
                                    ad(0).Equals(String.Concat(targetStr, If(targetStr2.Trim().Length() = 0, "", " "), targetStr2, If(targetStr3.Trim().Length() = 0, "", " "), targetStr3))) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518",
                                                       New String() {String.Concat("配送先都道府県、配送先市区町村、配送先町域・番地、配送先ビル等(カラム57-60番目)[", String.Concat(targetStr, If(targetStr2.Trim().Length() = 0, "", " "), targetStr2), "]",
                                                                                   " ", "と", "配送先氏名（姓）、配送先氏名（名）(カラム52-53番目)[", targetStr3, "]"), ""},
                                                       sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
        End If
        ...
        ' 配送先会社名、配送先部署名
        targetStr = String.Concat(dr.Item("COLUMN_61").ToString.Trim(), If(dr.Item("COLUMN_62").ToString.Trim().Length() = 0, "", " "), dr.Item("COLUMN_62").ToString.Trim())
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            If dtlIdx = 0 OrElse sysChumonNoBreak.Contains(dtlIdx) OrElse addressBreak.Contains(dtlIdx) Then
                ' 先頭行またはシステム注文番がブレイクした行または住所がブレイクした行のみチェックする。
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("配送先会社名、配送先部署名(カラム61-62番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
            ' 文字列長チェック
        ElseIf LenB(targetStr) > 80 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("配送先会社名、配送先部署名(カラム61-62番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        ' 配送先電話番号
        targetStr = dr.Item("COLUMN_63").ToString.Trim()
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            If dtlIdx = 0 OrElse sysChumonNoBreak.Contains(dtlIdx) OrElse addressBreak.Contains(dtlIdx) Then
                ' 先頭行またはシステム注文番がブレイクした行または住所がブレイクした行のみチェックする。
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("配送先電話番号(カラム63番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 20, False).Equals(False) Or targetStr.Replace("-", "").Length < 8 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("配送先電話番号(カラム63番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        ' 配達希望日
        sDate = dr.Item("COLUMN_67").ToString.Trim()
        ' 必須チェック
        If String.IsNullOrEmpty(sDate) Then
            ' 丸和物産側は必須項目ではない
            'If dtlIdx = 0 Then
            '    ' 先頭行のみチェックする。
            '    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("配達希望日(カラム67番目)[", sDate, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            '    bRet = False
            'End If
        Else
            ' 年月日チェック
            ...
            Else
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat("配達希望日(カラム67番目)[", sDate, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
        End If

        ' 商品コード
        targetStr = dr.Item("COLUMN_70").ToString.Trim()
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("商品コード(カラム70番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 20, False).Equals(False) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("商品コード(カラム70番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        ' 商品名
        targetStr = dr.Item("COLUMN_72").ToString.Trim()
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("商品名(カラム72番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 文字列長チェック
        ElseIf LenB(targetStr) > 60 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("商品名(カラム72番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        ' 数量
        targetStr = dr.Item("COLUMN_76").ToString().Trim()
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("数量(カラム76番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        Else
            If IsConvertDbl(targetStr).Equals(False) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E005", New String() {String.Concat("数量(カラム76番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            Else
                ...
                ElseIf Convert.ToDouble(targetStr) < 0 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E185", New String() {String.Concat("数量(カラム76番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                ElseIf dNum > 9999999999 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("数量(カラム76番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                Else
                    If targetStr.IndexOf(".") >= 0 AndAlso Convert.ToDecimal(targetStr.Substring(targetStr.IndexOf(".") + 1)) > 0 Then
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E025", New String() {String.Concat("数量(カラム76番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                        bRet = False
                    End If
                End If
            End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        ' 今回採番したシステム注文番号単位のEDI管理番号L
        Dim ediCtlNoDic As New Dictionary(Of String, HashSet(Of String))
        ' 上記の Data
        Dim ediCtlNoSet As HashSet(Of String)

        ' 取込Dtlの並べ替え（1列目: システム注文番号 昇順、57列目: 配送先都道府県 昇順、58列目: 配送先市区町村 昇順、59列目: 配送先町域・番地 昇順、60列目: 配送先ビル等 昇順）
        Dim tmpDt As DataTable = dtSetDtl.Clone()
        Dim tmpDr() As DataRow = Nothing
        tmpDr = dtSetDtl.Select(String.Empty, "COLUMN_1 ASC, COLUMN_57 ASC, COLUMN_58 ASC, COLUMN_59 ASC, COLUMN_60 ASC")
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_MRCCSV").NewRow()
            gyo += 1
            ds.Tables("LMH030_H_OUTKAEDI_DTL_MRCCSV").Clear() ' 受信DTLのクリア
            ds = Me.SetSemiOutkaEdiRcv(ds, i, gyo, drEdiRcvDtl)

            '---------------------------------------------------------------------------
            ' キー項目設定
            '---------------------------------------------------------------------------
            sNewKey = String.Concat(
                drEdiRcvDtl.Item("SYS_CHUMON_NO").ToString(),
                drEdiRcvDtl.Item("DEST_AD_1").ToString(),
                drEdiRcvDtl.Item("DEST_AD_2").ToString(),
                drEdiRcvDtl.Item("DEST_AD_3").ToString(),
                drEdiRcvDtl.Item("DEST_AD_4").ToString())
            ...
            If drEdiRcvDtl.Item("SYS_CHUMON_NO").ToString().Trim().Equals("") = False Then
                '---------------------------------------------------------------------------
                ' EDI出荷データ件数および出荷データL 出荷管理番号L 等 取得処理
                '---------------------------------------------------------------------------
                setDs.Tables("LMH030_H_OUTKAEDI_DTL_MRCCSV").ImportRow(drEdiRcvDtl)

                ' 今回採番したシステム注文番号単位のEDI管理番号L がある場合、条件項目に追加する。
                Dim ediCtlNoJogai As New System.Text.StringBuilder()
                If ediCtlNoDic.ContainsKey(drEdiRcvDtl.Item("SYS_CHUMON_NO").ToString()) Then
                    ediCtlNoSet = ediCtlNoDic(drEdiRcvDtl.Item("SYS_CHUMON_NO").ToString())
                    For Each ediCtlNo As String In ediCtlNoSet
                        ediCtlNoJogai.Append(String.Concat(If(ediCtlNoJogai.Length() = 0, "", ", "), "'", ediCtlNo, "'"))
                    Next
                    ediCtlNoJogai.Remove(0, 1)
                    ediCtlNoJogai.Remove(ediCtlNoJogai.Length() - 1, 1)
                End If
                setDs.Tables("LMH030_H_OUTKAEDI_DTL_MRCCSV").Rows(0).Item("EDI_CTL_NO_JOGAI") = ediCtlNoJogai.ToString()

                setDs = MyBase.CallDAC(Me._Dac, "GetOutkaCntAndNoL", setDs)

                ediRecCnt = MyBase.GetResultCount()
                Dim outkaNoL As String = setDs.Tables("LMH030_H_OUTKAEDI_DTL_MRCCSV").Rows(0).Item("OUTKA_CTL_NO").ToString()
            ...
            ' 今回採番したシステム注文番号単位のEDI管理番号L の退避
            If ediCtlNoDic.ContainsKey(drEdiRcvDtl.Item("SYS_CHUMON_NO").ToString()) Then
                ediCtlNoSet = ediCtlNoDic(drEdiRcvDtl.Item("SYS_CHUMON_NO").ToString())
            Else
                ediCtlNoSet = New HashSet(Of String)
            End If
            ediCtlNoSet.Add(sEdiCtlNo)
            ediCtlNoDic(drEdiRcvDtl.Item("SYS_CHUMON_NO").ToString()) = ediCtlNoSet
            ...
            ' EDI受信データの新規追加
            '---------------------------------------------------------------------------
            setDs.Tables("LMH030_H_OUTKAEDI_DTL_MRCCSV").Clear()
            setDs.Tables("LMH030_H_OUTKAEDI_DTL_MRCCSV").ImportRow(drEdiRcvDtl)
...
#Region "データセット設定 EDI出荷受信テーブル(明細)"
    ...
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer, ByVal gyo As Integer, ByVal drEdiRcvDtl As DataRow) As DataSet
        ...
        drEdiRcvDtl("SYS_CHUMON_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_1").ToString().Trim(), 100)               ' システム注文番号　　　　　←受信データ(CSV) システム注文番号
        drEdiRcvDtl("CHUMON_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_2").ToString().Trim(), 100)                   ' 注文番号　　　　　　　　　←受信データ(CSV) 注文番号
        drEdiRcvDtl("STATUS") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 100)                      ' ステータス　　　　　　　　←受信データ(CSV) ステータス
        drEdiRcvDtl("ORD_MAIL_SND_DT_TM") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 100)          ' 注文確認メール送信日時　　←受信データ(CSV) 注文確認メール送信日時
        drEdiRcvDtl("DLVY_MAIL_SND_DT_TM") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 100)         ' 配送通知メール送信日時　　←受信データ(CSV) 配送通知メール送信日時
        drEdiRcvDtl("DENP_CSV_DL_DT_TM") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 100)           ' 送り状CSVダウンロード日時 ←受信データ(CSV) 送り状CSVダウンロード日時
        drEdiRcvDtl("DOT") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 100)                         ' ドット　　　　　　　　　　←受信データ(CSV) .(ドット)
        drEdiRcvDtl("URI_FIXED_GK") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 100)                ' 売上確定額　　　　　　　　←受信データ(CSV) 売上確定額
        drEdiRcvDtl("URI_FIXED_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 100)              ' 売上確定日　　　　　　　　←受信データ(CSV) 売上確定日
        drEdiRcvDtl("GENPYO_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 100)                  ' 原票ナンバー　　　　　　　←受信データ(CSV) 原票ナンバー
        drEdiRcvDtl("TTL_SEIQ_GK") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 100)                ' 合計請求金額　　　　　　　←受信データ(CSV) 合計請求金額
        drEdiRcvDtl("GK_ADJUST_TITLE") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 100)            ' 金額調整タイトル　　　　　←受信データ(CSV) 金額調整タイトル
        drEdiRcvDtl("GK_ADJUST") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 100)                  ' 金額調整　　　　　　　　　←受信データ(CSV) 金額調整
        drEdiRcvDtl("WRAPPING_GK") = Me._Blc.LeftB(drSetDtl("COLUMN_14").ToString().Trim(), 100)                ' ラッピング金額　　　　　　←受信データ(CSV) ラッピング金額
        drEdiRcvDtl("WRAPPING_DTL") = Me._Blc.LeftB(drSetDtl("COLUMN_15").ToString().Trim(), 100)               ' ラッピング詳細　　　　　　←受信データ(CSV) ラッピング詳細
        drEdiRcvDtl("PAYMENT_COMMN") = Me._Blc.LeftB(drSetDtl("COLUMN_16").ToString().Trim(), 100)              ' 決済手数料　　　　　　　　←受信データ(CSV) 決済手数料
        drEdiRcvDtl("POINT_USE_GK") = Me._Blc.LeftB(drSetDtl("COLUMN_17").ToString().Trim(), 100)               ' ポイント利用額　　　　　　←受信データ(CSV) ポイント利用額
        drEdiRcvDtl("COMP_ORD_CALC_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_18").ToString().Trim(), 100)           ' 受注完了時送料計算種別　　←受信データ(CSV) 受注完了時送料計算種別
        drEdiRcvDtl("POSTAGE") = Me._Blc.LeftB(drSetDtl("COLUMN_19").ToString().Trim(), 100)                    ' 送料　　　　　　　　　　　←受信データ(CSV) 送料
        drEdiRcvDtl("DM_POSTAGE") = Me._Blc.LeftB(drSetDtl("COLUMN_20").ToString().Trim(), 100)                 ' DM便送料　　　　　　　　　←受信データ(CSV) DM便送料
        drEdiRcvDtl("NEKO_POS_POSTAGE") = Me._Blc.LeftB(drSetDtl("COLUMN_21").ToString().Trim(), 100)           ' ネコポス送料　　　　　　　←受信データ(CSV) ネコポス送料
        drEdiRcvDtl("HOME_DLVY_CCT_POSTAGE") = Me._Blc.LeftB(drSetDtl("COLUMN_22").ToString().Trim(), 100)      ' 宅急便コンパクト送料　　　←受信データ(CSV) 宅急便コンパクト送料
        drEdiRcvDtl("HOME_DLVY_POSTAGE") = Me._Blc.LeftB(drSetDtl("COLUMN_23").ToString().Trim(), 100)          ' 宅急便送料　　　　　　　　←受信データ(CSV) 宅急便送料
        drEdiRcvDtl("COOL_COMMN") = Me._Blc.LeftB(drSetDtl("COLUMN_24").ToString().Trim(), 100)                 ' クール手数料　　　　　　　←受信データ(CSV) クール手数料
        drEdiRcvDtl("ORD_DT_TM") = Me._Blc.LeftB(drSetDtl("COLUMN_25").ToString().Trim(), 100)                  ' 注文日時　　　　　　　　　←受信データ(CSV) 注文日時
        drEdiRcvDtl("PAYMENT_METHOD") = Me._Blc.LeftB(drSetDtl("COLUMN_26").ToString().Trim(), 100)             ' お支払い方法　　　　　　　←受信データ(CSV) お支払い方法
        drEdiRcvDtl("PAYMENT_METHOD_SUPPL") = Me._Blc.LeftB(drSetDtl("COLUMN_27").ToString().Trim(), 100)       ' お支払い方法補足　　　　　←受信データ(CSV) お支払い方法補足
        drEdiRcvDtl("FAMILY_NAME") = Me._Blc.LeftB(drSetDtl("COLUMN_28").ToString().Trim(), 100)                ' 氏名（姓）　　　　　　　　←受信データ(CSV) 氏名（姓）
        drEdiRcvDtl("FIRST_NAME") = Me._Blc.LeftB(drSetDtl("COLUMN_29").ToString().Trim(), 100)                 ' 氏名（名）　　　　　　　　←受信データ(CSV) 氏名（名）
        drEdiRcvDtl("FAMILY_NAME_KANA") = Me._Blc.LeftB(drSetDtl("COLUMN_30").ToString().Trim(), 100)           ' 氏名フリガナ（姓）　　　　←受信データ(CSV) 氏名フリガナ（姓）
        drEdiRcvDtl("FIRST_NAME_KANA") = Me._Blc.LeftB(drSetDtl("COLUMN_31").ToString().Trim(), 100)            ' 氏名フリガナ（名）　　　　←受信データ(CSV) 氏名フリガナ（名）
        drEdiRcvDtl("ZIP") = Me._Blc.LeftB(drSetDtl("COLUMN_32").ToString().Trim(), 100)                        ' 郵便番号　　　　　　　　　←受信データ(CSV) 郵便番号
        drEdiRcvDtl("TODOFUKEN") = Me._Blc.LeftB(drSetDtl("COLUMN_33").ToString().Trim(), 100)                  ' 都道府県　　　　　　　　　←受信データ(CSV) 都道府県
        drEdiRcvDtl("SHIKUCHOSON") = Me._Blc.LeftB(drSetDtl("COLUMN_34").ToString().Trim(), 100)                ' 市区町村　　　　　　　　　←受信データ(CSV) 市区町村
        drEdiRcvDtl("CHOIKI_BANCHI") = Me._Blc.LeftB(drSetDtl("COLUMN_35").ToString().Trim(), 100)              ' 町域・番地　　　　　　　　←受信データ(CSV) 町域・番地
        drEdiRcvDtl("BLDG_ETC") = Me._Blc.LeftB(drSetDtl("COLUMN_36").ToString().Trim(), 100)                   ' ビル等　　　　　　　　　　←受信データ(CSV) ビル等
        drEdiRcvDtl("CORP_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_37").ToString().Trim(), 100)                    ' 会社名　　　　　　　　　　←受信データ(CSV) 会社名
        drEdiRcvDtl("DEPT_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_38").ToString().Trim(), 100)                    ' 部署名　　　　　　　　　　←受信データ(CSV) 部署名
        drEdiRcvDtl("MAIL_ADDRESS") = Me._Blc.LeftB(drSetDtl("COLUMN_39").ToString().Trim(), 100)               ' メールアドレス　　　　　　←受信データ(CSV) メールアドレス
        drEdiRcvDtl("TEL") = Me._Blc.LeftB(drSetDtl("COLUMN_40").ToString().Trim(), 100)                        ' 電話番号　　　　　　　　　←受信データ(CSV) 電話番号
        drEdiRcvDtl("CELL_PHONE_NMB") = Me._Blc.LeftB(drSetDtl("COLUMN_41").ToString().Trim(), 100)             ' 携帯電話番号　　　　　　　←受信データ(CSV) 携帯電話番号
        drEdiRcvDtl("FAX") = Me._Blc.LeftB(drSetDtl("COLUMN_42").ToString().Trim(), 100)                        ' FAX番号　　　　　　　　　 ←受信データ(CSV) FAX番号
        drEdiRcvDtl("GENDER") = Me._Blc.LeftB(drSetDtl("COLUMN_43").ToString().Trim(), 100)                     ' 性別　　　　　　　　　　　←受信データ(CSV) 性別
        drEdiRcvDtl("BIRTH_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_44").ToString().Trim(), 100)                 ' 生年月日　　　　　　　　　←受信データ(CSV) 生年月日
        drEdiRcvDtl("FREE_1") = Me._Blc.LeftB(drSetDtl("COLUMN_45").ToString().Trim(), 100)                     ' フリー項目１　　　　　　　←受信データ(CSV) フリー項目１
        drEdiRcvDtl("FREE_2") = Me._Blc.LeftB(drSetDtl("COLUMN_46").ToString().Trim(), 100)                     ' フリー項目２　　　　　　　←受信データ(CSV) フリー項目２
        drEdiRcvDtl("CUST_ORD_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_47").ToString().Trim(), 30)                 ' 荷主注文番号（全体）　　　←受信データ(CSV) フリー項目３
        drEdiRcvDtl("MENBER_ID") = Me._Blc.LeftB(drSetDtl("COLUMN_48").ToString().Trim(), 100)                  ' 会員ID　　　　　　　　　　←受信データ(CSV) 会員ID
        drEdiRcvDtl("USED_POINT") = Me._Blc.LeftB(drSetDtl("COLUMN_49").ToString().Trim(), 100)                 ' 利用ポイント　　　　　　　←受信データ(CSV) 利用ポイント
        drEdiRcvDtl("ACQ_POINT") = Me._Blc.LeftB(drSetDtl("COLUMN_50").ToString().Trim(), 100)                  ' 獲得ポイント　　　　　　　←受信データ(CSV) 獲得ポイント
        drEdiRcvDtl("DEST_SELECT") = Me._Blc.LeftB(drSetDtl("COLUMN_51").ToString().Trim(), 100)                ' 配送先選択　　　　　　　　←受信データ(CSV) 配送先選択
        drEdiRcvDtl("DEST_UKETORI_NM_1") = Me._Blc.LeftB(drSetDtl("COLUMN_52").ToString().Trim(), 160)          ' 配送先氏名（姓）　　　　　←受信データ(CSV) 配送先氏名（姓）
        drEdiRcvDtl("DEST_UKETORI_NM_2") = Me._Blc.LeftB(drSetDtl("COLUMN_53").ToString().Trim(), 160)          ' 配送先氏名（名）　　　　　←受信データ(CSV) 配送先氏名（名）
        drEdiRcvDtl("DEST_FAMILY_NAME_KANA") = Me._Blc.LeftB(drSetDtl("COLUMN_54").ToString().Trim(), 100)      ' 配送先氏名フリガナ（姓）　←受信データ(CSV) 配送先氏名フリガナ（姓）
        drEdiRcvDtl("DEST_FIRST_NAME_KANA") = Me._Blc.LeftB(drSetDtl("COLUMN_55").ToString().Trim(), 100)       ' 配送先氏名フリガナ（名）　←受信データ(CSV) 配送先氏名フリガナ（名）
        drEdiRcvDtl("DEST_ZIP") = Me._Blc.LeftB(drSetDtl("COLUMN_56").ToString().Trim(), 10)                    ' 届先郵便番号　　　　　　　←受信データ(CSV) 配送先郵便番号
        drEdiRcvDtl("DEST_AD_1") = Me._Blc.LeftB(drSetDtl("COLUMN_57").ToString().Trim(), 160)                  ' 届先住所1　　　　　　　　 ←受信データ(CSV) 配送先都道府県
        drEdiRcvDtl("DEST_AD_2") = Me._Blc.LeftB(drSetDtl("COLUMN_58").ToString().Trim(), 160)                  ' 届先住所2　　　　　　　　 ←受信データ(CSV) 配送先市区町村
        drEdiRcvDtl("DEST_AD_3") = Me._Blc.LeftB(drSetDtl("COLUMN_59").ToString().Trim(), 160)                  ' 届先住所3　　　　　　　　 ←受信データ(CSV) 配送先町域・番地
        drEdiRcvDtl("DEST_AD_4") = Me._Blc.LeftB(drSetDtl("COLUMN_60").ToString().Trim(), 160)                  ' 届先住所4　　　　　　　　 ←受信データ(CSV) 配送先ビル等
        drEdiRcvDtl("DEST_NM_1") = Me._Blc.LeftB(drSetDtl("COLUMN_61").ToString().Trim(), 80)                   ' 届先名1　　　　　　　　　 ←受信データ(CSV) 配送先会社名
        drEdiRcvDtl("DEST_NM_2") = Me._Blc.LeftB(drSetDtl("COLUMN_62").ToString().Trim(), 80)                   ' 届先名2　　　　　　　　　 ←受信データ(CSV) 配送先部署名
        drEdiRcvDtl("DEST_TEL") = Me._Blc.LeftB(drSetDtl("COLUMN_63").ToString().Trim(), 15)                    ' 届先電話番号　　　　　　　←受信データ(CSV) 配送先電話番号
        drEdiRcvDtl("DLVY_METHOD_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_64").ToString().Trim(), 100)             ' 配送方法番号　　　　　　　←受信データ(CSV) 配送方法番号
        drEdiRcvDtl("DLVY_METHOD") = Me._Blc.LeftB(drSetDtl("COLUMN_65").ToString().Trim(), 100)                ' 配送方法　　　　　　　　　←受信データ(CSV) 配送方法
        drEdiRcvDtl("DENP_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_66").ToString().Trim(), 100)                    ' 送り状番号　　　　　　　　←受信データ(CSV) 送り状番号
        drEdiRcvDtl("ARR_PLAN_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_67").ToString().Trim(), 10)               ' 納入予定日　　　　　　　　←受信データ(CSV) 配達希望日
        drEdiRcvDtl("RECEIVE_TIME") = Me._Blc.LeftB(drSetDtl("COLUMN_68").ToString().Trim(), 100)               ' 受取時間帯　　　　　　　　←受信データ(CSV) 受取時間帯
        drEdiRcvDtl("SURVEY") = Me._Blc.LeftB(drSetDtl("COLUMN_69").ToString().Trim(), 100)                     ' アンケート　　　　　　　　←受信データ(CSV) アンケート
        drEdiRcvDtl("CUST_GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_70").ToString().Trim(), 20)               ' 商品コード　　　　　　　　←受信データ(CSV) 商品コード
        drEdiRcvDtl("SYS_CUST_GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_71").ToString().Trim(), 100)          ' システム商品番号　　　　　←受信データ(CSV) システム商品番号
        drEdiRcvDtl("GOODS_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_72").ToString().Trim(), 60)                    ' 商品名　　　　　　　　　　←受信データ(CSV) 商品名
        drEdiRcvDtl("VARIATION") = Me._Blc.LeftB(drSetDtl("COLUMN_73").ToString().Trim(), 100)                  ' バリエーション名　　　　　←受信データ(CSV) バリエーション名
        drEdiRcvDtl("BIN_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_74").ToString().Trim(), 100)                     ' 便種　　　　　　　　　　　←受信データ(CSV) 便種
        drEdiRcvDtl("PRICE") = Me._Blc.LeftB(drSetDtl("COLUMN_75").ToString().Trim(), 100)                      ' 価格（税込）　　　　　　　←受信データ(CSV) 価格（税込）
        drEdiRcvDtl("OUTKA_TTL_NB") = Me._Blc.LeftB(drSetDtl("COLUMN_76").ToString().Trim(), 10)                ' 出荷総個数　　　　　　　　←受信データ(CSV) 数量
        drEdiRcvDtl("GK") = Me._Blc.LeftB(drSetDtl("COLUMN_77").ToString().Trim(), 100)                         ' 金額　　　　　　　　　　　←受信データ(CSV) 金額
        drEdiRcvDtl("MEMO") = Me._Blc.LeftB(drSetDtl("COLUMN_78").ToString().Trim(), 100)                       ' メモ　　　　　　　　　　　←受信データ(CSV) メモ
        drEdiRcvDtl("RECEIVE_PLACE_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_79").ToString().Trim(), 100)           ' 受取場所名称　　　　　　　←受信データ(CSV) 受取場所名称
        drEdiRcvDtl("RECEIVE_PLACE_ZIP") = Me._Blc.LeftB(drSetDtl("COLUMN_80").ToString().Trim(), 100)          ' 受取場所郵便番号　　　　　←受信データ(CSV) 受取場所郵便番号
        drEdiRcvDtl("RECEIVE_PLACE_AD") = Me._Blc.LeftB(drSetDtl("COLUMN_81").ToString().Trim(), 100)           ' 受取場所住所　　　　　　　←受信データ(CSV) 受取場所住所
        drEdiRcvDtl("RECEIVE_PLACE_TEL") = Me._Blc.LeftB(drSetDtl("COLUMN_82").ToString().Trim(), 100)          ' 受取場所電話番号　　　　　←受信データ(CSV) 受取場所電話番号
        drEdiRcvDtl("CERT_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_83").ToString().Trim(), 100)                    ' 認証番号　　　　　　　　　←受信データ(CSV) 認証番号
        drEdiRcvDtl("ORG_STATUS") = Me._Blc.LeftB(drSetDtl("COLUMN_84").ToString().Trim(), 100)                 ' 独自ステータス　　　　　　←受信データ(CSV) 独自ステータス
        drEdiRcvDtl("PROXY_ORD_ROUTE") = Me._Blc.LeftB(drSetDtl("COLUMN_85").ToString().Trim(), 100)            ' 代理注文経路　　　　　　　←受信データ(CSV) 代理注文経路
        drEdiRcvDtl("PROXY_ORD_REGISTER") = Me._Blc.LeftB(drSetDtl("COLUMN_86").ToString().Trim(), 100)         ' 代理注文登録者　　　　　　←受信データ(CSV) 代理注文登録者
        drEdiRcvDtl("SYS_DLVY_DEST_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_87").ToString().Trim(), 100)           ' システム配送先番号　　　　←受信データ(CSV) システム配送先番号
        drEdiRcvDtl("CLIENT") = Me._Blc.LeftB(drSetDtl("COLUMN_88").ToString().Trim(), 100)                     ' ご依頼主　　　　　　　　　←受信データ(CSV) ご依頼主
        drEdiRcvDtl("SALES_DET_DENP_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_89").ToString().Trim(), 100)          ' 売上確定用送り状番号　　　←受信データ(CSV) 売上確定用送り状番号
        drEdiRcvDtl("GOODS_ATTR") = Me._Blc.LeftB(drSetDtl("COLUMN_90").ToString().Trim(), 100)                 ' 商品属性　　　　　　　　　←受信データ(CSV) 商品属性
        drEdiRcvDtl("RESERVE_ORD") = Me._Blc.LeftB(drSetDtl("COLUMN_91").ToString().Trim(), 100)                ' 予約注文　　　　　　　　　←受信データ(CSV) 予約注文
        drEdiRcvDtl("COUPON_INFO") = Me._Blc.LeftB(drSetDtl("COLUMN_92").ToString().Trim(), 100)                ' クーポン情報　　　　　　　←受信データ(CSV) クーポン情報
        drEdiRcvDtl("COUPON_DISCOUNT") = Me._Blc.LeftB(drSetDtl("COLUMN_93").ToString().Trim(), 100)            ' クーポン値引き　　　　　　←受信データ(CSV) クーポン値引き
        drEdiRcvDtl("SALES_PROMO_MAIL_DLVY") = Me._Blc.LeftB(drSetDtl("COLUMN_94").ToString().Trim(), 100)      ' 販促メール配信　　　　　　←受信データ(CSV) 販促メール配信
        drEdiRcvDtl("B2_CUSTOMER_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_95").ToString().Trim(), 100)             ' B2お客様管理番号　　　　　←受信データ(CSV) B2お客様管理番号
        drEdiRcvDtl("DENP_OUT_UNIT") = Me._Blc.LeftB(drSetDtl("COLUMN_96").ToString().Trim(), 100)              ' 送り状発行単位　　　　　　←受信データ(CSV) 送り状発行単位
        drEdiRcvDtl("BR_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_97").ToString().Trim(), 100)                      ' 営業所コード　　　　　　　←受信データ(CSV) 営業所コード
        drEdiRcvDtl("SYUKKA_PLAN_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_98").ToString().Trim(), 100)           ' 出荷予定日　　　　　　　　←受信データ(CSV) 出荷予定日
        drEdiRcvDtl("TAX_RATE_KB") = Me._Blc.LeftB(drSetDtl("COLUMN_99").ToString().Trim(), 100)                ' 税率区分　　　　　　　　　←受信データ(CSV) 税率区分
        drEdiRcvDtl("TAX_RATE") = Me._Blc.LeftB(drSetDtl("COLUMN_100").ToString().Trim(), 100)                  ' 税率　　　　　　　　　　　←受信データ(CSV) 税率
        drEdiRcvDtl("DISCOUNT_SUBTOTAL_1") = Me._Blc.LeftB(drSetDtl("COLUMN_101").ToString().Trim(), 100)       ' 値引き小計（標準税率対象）←受信データ(CSV) 値引き小計（標準税率対象）
        drEdiRcvDtl("DISCOUNT_SUBTOTAL_2") = Me._Blc.LeftB(drSetDtl("COLUMN_102").ToString().Trim(), 100)       ' 値引き小計（軽減税率対象）←受信データ(CSV) 値引き小計（軽減税率対象）
        drEdiRcvDtl("DISCOUNT_SUBTOTAL_3") = Me._Blc.LeftB(drSetDtl("COLUMN_103").ToString().Trim(), 100)       ' 値引き小計（非課税）　　　←受信データ(CSV) 値引き小計（非課税）
        drEdiRcvDtl("TOTAL_1") = Me._Blc.LeftB(drSetDtl("COLUMN_104").ToString().Trim(), 100)                   ' 合計（標準税率対象）　　　←受信データ(CSV) 合計（標準税率対象）
        drEdiRcvDtl("TOTAL_2") = Me._Blc.LeftB(drSetDtl("COLUMN_105").ToString().Trim(), 100)                   ' 合計内税（標準税率対象）　←受信データ(CSV) 合計内税（標準税率対象）
        drEdiRcvDtl("TOTAL_3") = Me._Blc.LeftB(drSetDtl("COLUMN_106").ToString().Trim(), 100)                   ' 合計（軽減税率対象）　　　←受信データ(CSV) 合計（軽減税率対象）
        drEdiRcvDtl("TOTAL_4") = Me._Blc.LeftB(drSetDtl("COLUMN_107").ToString().Trim(), 100)                   ' 合計内税（軽減税率対象）　←受信データ(CSV) 合計内税（軽減税率対象）
        drEdiRcvDtl("TOTAL_5") = Me._Blc.LeftB(drSetDtl("COLUMN_108").ToString().Trim(), 100)                   ' 合計（非課税）　　　　　　←受信データ(CSV) 合計（非課税）
        drEdiRcvDtl("SYUKKA_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_109").ToString().Trim(), 100)               ' 出荷日　　　　　　　　　　←受信データ(CSV) 出荷日
        drEdiRcvDtl("QRURL_PARAM") = Me._Blc.LeftB(drSetDtl("COLUMN_110").ToString().Trim(), 100)               ' QRURLパラメータ　　　　　 ←受信データ(CSV) QRURLパラメータ
        drEdiRcvDtl("UNSO_IRAI_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_111").ToString().Trim(), 100)              ' 運送依頼番号　　　　　　　←受信データ(CSV) 運送依頼番号
        drEdiRcvDtl("YOBI_1") = Me._Blc.LeftB(drSetDtl("COLUMN_112").ToString().Trim(), 100)                    ' 予備１　　　　　　　　　　←受信データ(CSV) 予備１
        drEdiRcvDtl("YOBI_2") = Me._Blc.LeftB(drSetDtl("COLUMN_113").ToString().Trim(), 100)                    ' 予備２　　　　　　　　　　←受信データ(CSV) 予備２

        'データセットに設定
        ds.Tables("LMH030_H_OUTKAEDI_DTL_MRCCSV").Rows.Add(drEdiRcvDtl)
...
#Region "データセット設定 EDI出荷データ 用 EDI管理番号(大・中)"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim sNrsBrCd As String = ds.Tables("LMH030_H_OUTKAEDI_DTL_MRCCSV").Rows(0).Item("NRS_BR_CD").ToString()
...
#Region "データセット設定(EDI出荷(中)"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet, ByVal drEdiRcvDtl As DataRow) As DataSet
        ...
        drOutkaEdiM("CUST_GOODS_CD") = drEdiRcvDtl("CUST_GOODS_CD")     ' 商品コード　　　　　　　←EDI出荷受信テーブル.商品コード　　　　　　　　←受信データ(CSV) 商品コード
        drOutkaEdiM("NRS_GOODS_CD") = String.Empty
        drOutkaEdiM("GOODS_NM") = drEdiRcvDtl("GOODS_NM")               ' 商品名　　　　　　　　　←EDI出荷受信テーブル.商品名　　　　　　　　　　←受信データ(CSV) 商品名
        ...
        drOutkaEdiM("OUTKA_HASU") = drEdiRcvDtl("OUTKA_TTL_NB")         ' 出荷端数　　　　　　　　←EDI出荷受信テーブル.出荷総個数　　　　　　　　←受信データ(CSV) 数量
        drOutkaEdiM("OUTKA_QT") = 0
        drOutkaEdiM("OUTKA_TTL_NB") = drEdiRcvDtl("OUTKA_TTL_NB")       ' 出荷総個数　　　　　　　←EDI出荷受信テーブル.出荷総個数　　　　　　　　←受信データ(CSV) 数量
...
#Region "データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        If Not String.IsNullOrEmpty(drEdiRcvDtl("ARR_PLAN_DATE").ToString) Then
            ' EDI出荷受信テーブル.納入予定日に値があれば使用
            arrPlanDate = Date.Parse(drEdiRcvDtl("ARR_PLAN_DATE").ToString().Trim()).ToString("yyyyMMdd")
        ElseIf Not String.IsNullOrEmpty(drEdiRcvDtl("ORD_DT_TM").ToString) Then
            ' EDI出荷受信テーブル.注文日時に値があれば+3営業日を適用して使用
            arrPlanDate = Date.Parse(drEdiRcvDtl("ORD_DT_TM").ToString().Trim()).ToString("yyyyMMdd")
            arrPlanDate = Left(Convert.ToString(Me.GetBussinessDayKrt(arrPlanDate, +3, setDs)).Replace("/", String.Empty), 8)
        End If
        If String.IsNullOrEmpty(arrPlanDate) Then
            drOutkaEdiL("OUTKA_PLAN_DATE") = String.Empty
            drOutkaEdiL("OUTKO_DATE") = String.Empty
            drOutkaEdiL("ARR_PLAN_DATE") = String.Empty
        Else
            Dim oneBussinessDayAgo As String = Left(Convert.ToString(Me.GetBussinessDayKrt(arrPlanDate, -1, setDs)).Replace("/", String.Empty), 8)
            drOutkaEdiL("OUTKA_PLAN_DATE") = oneBussinessDayAgo
            drOutkaEdiL("OUTKO_DATE") = oneBussinessDayAgo
            drOutkaEdiL("ARR_PLAN_DATE") = arrPlanDate
        End If
        ...
        Dim destZip As String = drEdiRcvDtl("DEST_ZIP").ToString()
        Dim destTel As String = drEdiRcvDtl("DEST_TEL").ToString()
        ...
        End If
        ' 届先コード ←
        ' 　EDI出荷受信テーブル.届先郵便番号←受信データ(CSV) 送り先郵便番号 (ハイフン抜き上7桁) + 
        ' 　EDI出荷受信テーブル.届先電話番号←受信データ(CSV) 送り先電話番号 (ハイフン抜き下8桁)
        ...
        ' 届先名←
        ' 　EDI出荷受信テーブル.届先名1←受信データ(CSV) 配送先会社名 + 半角空白 +
        ' 　EDI出荷受信テーブル.届先名2←受信データ(CSV) 配送先部署名
        Dim dstrNm As String = String.Concat(
                                drEdiRcvDtl("DEST_NM_1").ToString(),
                                If(drEdiRcvDtl("DEST_NM_2").ToString().Trim().Length = 0, "", " "),
                                drEdiRcvDtl("DEST_NM_2").ToString())
        drOutkaEdiL("DEST_NM") = dstrNm                                             ' 届先名　　　　　　　←上記 届先名
        drOutkaEdiL("DEST_ZIP") = destZip.Replace("-", "")                          ' 届先郵便番号　　　　←EDI出荷受信テーブル.届先郵便番号 (ハイフン抜き) ←受信データ(CSV) 配送先郵便番号

        ' 住所編集（住所を格納）
        ' 届先住所1～3 ←
        ' 　(A) EDI出荷受信テーブル.届先住所1←受信データ(CSV) 配送先都道府県 +
        ' 　(B) EDI出荷受信テーブル.届先住所2←受信データ(CSV) 配送先市区町村 +
        ' 　(C) EDI出荷受信テーブル.届先住所3←受信データ(CSV) 配送先町域・番地 + 半角空白 +
        ' 　(D) EDI出荷受信テーブル.届先住所4←受信データ(CSV) 配送先ビル等 +
        ' 　(E) 半角空白
        ' 　(F) EDI出荷受信テーブル.配送先氏名（姓）←受信データ(CSV) 配送先氏名（姓） + 半角空白 +
        ' 　(G) EDI出荷受信テーブル.配送先氏名（名）←受信データ(CSV) 配送先氏名（名） + "様" の分割文字列
        ' ただし、
        ' 　(A)～(D) の連結文字列を分割して、(設定先の)届先住所1～2 に収まる場合は以下とする。
        ' 　(設定先の)届先住所1～2 は (A)～(D) の連結文字列の分割文字列、
        ' 　(設定先の)届先住所3　  は (F)～(G) + "様"
        Dim destAd1 As String = String.Concat(
                                        drEdiRcvDtl("DEST_AD_1").ToString().Trim(), drEdiRcvDtl("DEST_AD_2").ToString().Trim(),
                                        drEdiRcvDtl("DEST_AD_3").ToString().Trim())
        Dim destAd2 As String = drEdiRcvDtl("DEST_AD_4").ToString().Trim()
        Dim destUketori As String = String.Concat(
                                            drEdiRcvDtl("DEST_UKETORI_NM_1").ToString().Trim(), " ",
                                            drEdiRcvDtl("DEST_UKETORI_NM_2").ToString().Trim())
        Dim ad() As String = editAddr(destAd1, destAd2, destUketori)
        drOutkaEdiL("DEST_AD_1") = ad(1)
        drOutkaEdiL("DEST_AD_2") = ad(2)
        drOutkaEdiL("DEST_AD_3") = ad(3) + If(destUketori.Trim().Length = 0, "", "様")
        drOutkaEdiL("DEST_AD_4") = String.Empty
        drOutkaEdiL("DEST_AD_5") = String.Empty

        drOutkaEdiL("DEST_TEL") = destTel                                           ' 届先電話番号　　　　←EDI出荷受信テーブル.届先電話番号　　　　　　　　←受信データ(CSV) 送り先電話番号
        ...
        drOutkaEdiL("CUST_ORD_NO") = String.Empty
        ...
        drOutkaEdiL("REMARK") = drEdiRcvDtl("CUST_ORD_NO")                          ' 出荷時注意事項←EDI出荷受信テーブル.荷主注文番号（全体）　　　　　　　←受信データ(CSV) フリー項目３
        ...
        drOutkaEdiL("FREE_C04") = drEdiRcvDtl("SYS_CHUMON_NO")                      ' 文字列04　　　　　　←EDI出荷受信テーブル.システム注文番号　　　　　　←受信データ(CSV) システム注文番号
        'drOutkaEdiL("FREE_C05") = String.Empty     'DEL 2023/07/18 038023【LMS】丸和物産らくうるカートお支払い方法（送り状種類）対応
        'ADD START 2023/07/18 038023【LMS】丸和物産らくうるカートお支払い方法（送り状種類）対応
        Dim okurijyoPtn As String = String.Empty
        If String.IsNullOrEmpty(drEdiRcvDtl("PAYMENT_METHOD").ToString.Trim) = False Then
            okurijyoPtn = drEdiRcvDtl("PAYMENT_METHOD").ToString.Substring(0, 1)
        End If
        drOutkaEdiL("FREE_C05") = okurijyoPtn
        'ADD END 2023/07/18 038023【LMS】丸和物産らくうるカートお支払い方法（送り状種類）対応

        drOutkaEdiL("FREE_C06") = String.Empty
        drOutkaEdiL("FREE_C07") = String.Empty
        'ADD START 2023/07/18 038023【LMS】丸和物産らくうるカートお支払い方法（送り状種類）対応
        If okurijyoPtn.Equals("2") Then
            '送り状種類が 2（コレクト）の場合、合計（標準税率対象）と合計内税（標準税率対象）を設定
            drOutkaEdiL("FREE_C06") = drEdiRcvDtl("TOTAL_1")
            drOutkaEdiL("FREE_C07") = drEdiRcvDtl("TOTAL_2")
        End If
        'ADD END 2023/07/18 038023【LMS】丸和物産らくうるカートお支払い方法（送り状種類）対応
        ...
        drOutkaEdiL("FREE_C27") = drEdiRcvDtl("FILE_NAME").ToString().Trim()        ' 文字列27　　　　　　←EDI出荷受信テーブル.受信ファイル名　(今回改修前に稼働の LMH030BLC404(xlsm取込) 準拠)
...
#Region "住所編集"
    ...
    ''' 住所編集
    ''' </summary>
    ''' <param name="destAd1">配送先都道府県、配送先市区町村、配送先町域・番地 の連結文字列</param>
    ''' <param name="destAd2">配送先ビル等</param>
    ''' <param name="destUketori">配送先氏名（姓）、配送先氏名（名）の連結文字列</param>
    ''' <returns>文字配列 (1)～(3): 住所1～3, (0): 分割した住所 1～3 の結合文字列</returns>
    Private Function editAddr(ByVal destAd1 As String, ByVal destAd2 As String, ByVal destUketori As String) As String()
        ...
        Dim keishoLenB As Integer = LenB(If(destUketori.Trim().Length() = 0, "", "様"))

        If LenB(destAd1) <= 40 AndAlso LenB(destAd2) <= 40 Then
            ' 配送先都道府県、配送先市区町村、配送先町域・番地 の連結文字列が 住所1 に収まり、
            ' 配送先ビル等 が住所2 に収まる場合、
            ' 配送先都道府県、配送先市区町村、配送先町域・番地 の連結文字列を 住所1 に、
            ' 配送先ビル等 を住所2 に、それぞれ設定し、
            ' 配送先氏名（姓）、半角空白、配送先氏名（名）の連結文字列 の SJIS 80バイト または 78バイト(※) を 住所3 に設定する。
            ' ※配送先氏名（姓）、配送先氏名（名）の連結文字列に値がある場合、テーブル設定時付加する "様" 分を 80から 引いたバイト数
            ...
            ad(2) = destAd2
            ' 3
            If destUketori.Trim().Length() = 0 Then
                ad(3) = ""
            Else
                ad(3) = Me._Blc.LeftB(destUketori, 80 - keishoLenB)
            End If
        ElseIf LenB(String.Concat(destAd1, If(destAd2.Trim().Length() = 0, "", " "), destAd2)) <= 80 Then
            ' 上記以外で
            ' 配送先都道府県、配送先市区町村、配送先町域・番地、半角空白、配送先ビル等 の連結文字列が 住所1, 住所2 に収まる場合、
            ' 配送先都道府県、配送先市区町村、配送先町域・番地、半角空白、配送先ビル等 の連結文字列を 住所1, 住所2 に設定し、
            ' 配送先氏名（姓）、半角空白、配送先氏名（名）の連結文字列 の SJIS 80バイト または 78バイト(※) を 住所3 に設定する。
            ' ※配送先氏名（姓）、配送先氏名（名）の連結文字列に値がある場合、テーブル設定時付加する "様" 分を 80から 引いたバイト数
            mergeAddr = String.Concat(destAd1, If(destAd2.Trim().Length() = 0, "", " "), destAd2)
            ' 1
        ...
        Else
            ' 上記以外の場合、
            ' 以下の連結文字列を、(40, 40, 80 または 78(※)) の各 SJIS バイトに分割し、(住所1, 住所2, 住所3) に設定する。
            ' ※配送先氏名（姓）、配送先氏名（名）の連結文字列に値がある場合、テーブル設定時付加する "様" 分を 80から 引いたバイト数
            '
            ' 配送先都道府県、配送先市区町村、配送先町域・番地、半角空白、配送先ビル等 の連結文字列
            ' 半角空白(以降の連結値に値がある場合)
            ' 配送先氏名（姓）、半角空白、配送先氏名（名）の連結文字列
            mergeAddr = String.Concat(
                destAd1,
                If(destAd2.Trim().Length() = 0, "", " "), destAd2,
                If(destUketori.Trim().Length() = 0, "", " "), destUketori)
            ' 1
            cutAddr = Me._Blc.LeftB(mergeAddr, 40)
            mergeAddr = mergeAddr.Substring(cutAddr.Length())
            ad(1) = cutAddr
            ...
            ad(2) = cutAddr
            ' 3
            cutAddr = Me._Blc.LeftB(mergeAddr, 80 - keishoLenB)
            mergeAddr = mergeAddr.Substring(cutAddr.Length())
            ad(3) = cutAddr
        End If
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        If ds.Tables("LMH030_OUTKAEDI_L").Rows.Count = 0 Then
            ...
            Return ds
        End If
        If ds.Tables("LMH030_SEMIEDI_INFO").Rows.Count > 0 Then
            Dim rcvFileExtention As String = ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("RCV_FILE_EXTENTION").ToString()
            Dim fileName As String = ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("FREE_C27").ToString().Trim()
            If fileName <> "" AndAlso fileName.IndexOf(".") > 0 Then
                Dim extention As String = fileName.Substring(fileName.LastIndexOf(".") + 1)
                If rcvFileExtention.ToLower() <> extention.ToLower() Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E454",
                                           New String() {
                                                String.Concat("現在の取込対象(*.", rcvFileExtention, ")と異なる形式のファイル(*.", extention, ")より取り込んだEDI出荷"),
                                                "出荷登録",
                                                "特定荷主機能のセミEDI環境切り替え処理を行ってから出荷登録してください。"},
