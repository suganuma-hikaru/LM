...
'  システム名       :  LM
'  サブシステム名   :  LMH       : EDI
'  プログラムID     :  LMH583    : EDI納品書(埼玉 BP イエローハット)
'  作  成  者       :  渡部　剛
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH583DAC = New LMH583DAC()
...
#Region "Const"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_IN As String = "LMH583IN"

    ''' <summary>
    ''' OUTテーブル名
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_OUT As String = "LMH583OUT"
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = LMH583BLC.TABLE_NM_IN
        Dim tableNmOut As String = LMH583BLC.TABLE_NM_OUT
        Dim tableNmRpt As String = LMH583BLC.TABLE_NM_M_RPT
        '2013.06.17 追加START
        Dim tableNmCnt As String = LMH583BLC.TABLE_NM_CNT
        '2013.06.17 追加END
        ...
        workDtRpt = view.ToTable(True, LMH583BLC.NRS_BR_CD, LMH583BLC.PTN_ID, LMH583BLC.PTN_CD, LMH583BLC.RPT_ID)
        ...
        For Each dr As DataRow In ds.Tables(tableNmRpt).Rows
            ...
            'prtDs = comPrt.CallDataSet(ds.Tables(tableNmOut), dr.Item(LMH583BLC.RPT_ID).ToString())
            prtDs = comPrt.CallDataSet(dtOut, dr.Item(LMH583BLC.RPT_ID).ToString())
            '(2012.06.11) 要望番号1102 並び換えしたDateTableを設定 ---  END  ---

            '帳票ごとの編集があるなら行う。
            prtDs = Me.EditPrintDataSet(dr.Item(LMH583BLC.RPT_ID).ToString(), prtDs)

            '帳票CSV出力
            comPrt.StartPrint(Me, _
                              dr.Item(LMH583BLC.NRS_BR_CD).ToString(), _
                              dr.Item(LMH583BLC.PTN_ID).ToString(), _
                              dr.Item(LMH583BLC.PTN_CD).ToString(), _
                              dr.Item(LMH583BLC.RPT_ID).ToString(), _
    ...
    Private Function EditPrintDataSet(ByVal rptId As String, ByVal ds As DataSet) As DataSet
        ...
        'BIKO_DTL変更処理
        Dim outDt As DataTable = ds.Tables("LMH583OUT") 'データ取得
        Dim max As Integer = outDt.Rows.Count - 1       '抽出データ数取得
        Dim keep As String = String.Empty               '保存用

        Dim drs As DataRow()

        For i As Integer = 0 To max

            If outDt.Rows(i).Item("ROW_NO").ToString = "001" Then
                keep = outDt.Rows(i).Item("BIKO_DTL").ToString()
            End If
            outDt.Rows(i).Item("BIKO_DTL") = keep
    ...
    Private Function SelectMPrt(ByVal ds As DataSet, ByVal intbl As DataTable) As DataSet
        ...
        ds = MyBase.CallDAC(Me._Dac, LMH583BLC.ACTION_ID_SELECT_MPRT, ds)
        ...
        If count < 1 Then
            ...
            Else
                If outType.Equals(ALL_PRINT) = True Then
                    ...
                    MyBase.SetMessageStore("00", "G053", New String() {"　【EDI納品書】　"}, rowNo, "帳票名", "EDI納品書 BPｲｴﾛｰﾊｯﾄ")
                End If
            End If
        End If
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        Return MyBase.CallDAC(Me._Dac, LMH583BLC.ACTION_ID_SELECT_PRT_DATA, ds)
    ...
    Private Function SelectOutputData(ByVal ds As DataSet) As DataSet
        ...
        Return MyBase.CallDAC(Me._Dac, LMH583BLC.ACTION_ID_SELECT_OUTPUTCNT, ds)
