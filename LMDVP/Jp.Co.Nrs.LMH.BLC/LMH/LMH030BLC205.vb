...
#Region "Const"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _divideDestStatus As DivideDestStatusType = DivideDestStatusType.None
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Enum DivideDestStatusType As Integer
    ...
    ''' <remarks></remarks>
    Class TABLE_NM
        Public Const LMH030_OUTKAEDI_L As String = LMH030DAC205.TABLE_NM.LMH030_OUTKAEDI_L
        Public Const LMH030OUT_DIVIDE_DEST As String = LMH030DAC205.TABLE_NM.LMH030OUT_DIVIDE_DEST
        Public Const LMH030_M_DEST As String = "LMH030_M_DEST"
    End Class
    ...
    Class WARNING_ITEM_NAME

        Public Const DEST_CD As String = "届先コード"
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC205 = New LMH030DAC205()
...
#Region "CONST"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Const DEF_SAITAMA_ABUKA_CD As String = "NRSBR50"

    ''' <summary>
    ''' 初期在庫部課コード(埼玉)
    ''' </summary>
    ''' <remarks></remarks>
    Public Const DEF_SAITAMA_ZBUKA_CD As String = "NRSBR50"

#End Region
    '2013.10.16 要望番号2113 修正END
...
#Region "出荷登録処理"
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        Select Case custIndex
            Case LMH030BLC.EdiCustIndex.DicGnm00039_00 _
               , LMH030BLC.EdiCustIndex.DicGnm00072_00 _
               , LMH030BLC.EdiCustIndex.DicGnm00076_00 _
               , LMH030BLC.EdiCustIndex.DicChbBC00010_00
                'ディック物流群馬の荷主は、事前チェックを行わない。
            Case Else
                'ディック（大阪）対応　20120321 Start
                '登録前の事前チェック（届先JISコード）
                ...
                If MyBase.GetResultCount = 0 Then
                    ...
                    Return ds
                End If
                'ディック（大阪）対応　20120321 End
        End Select
        'ディック（群馬）追加箇所 2012.07.11 terakawa End

        '要望番号1282 追加START 2012.07.23
        '荷主明細マスタの取得
...
#If False Then ' 届先自動変換対応(日立物流) 20170407 changed by inoue 
        ...
        'ディック（岩槻）追加箇所 20120601 terakawa Start
        'EDI出荷(大)の届先設定
        If Me.EdiLDestChk(ds, rowNo, ediCtlNo, genericInsFlg, doConvDest) = False Then
            'If Me.EdiLDestChk(ds, rowNo, ediCtlNo) = False Then
            Return ds
        End If
        'ディック（岩槻）追加箇所 20120601 terakawa End
        '要望番号1282 追加END 2012.07.23
#Else
        ...
        ' 届先振分対象荷主判定
        If (Me.IsDivideDestCust(custIndex)) Then
            ' 届先振分処理
            ds = Me.DivideDestCd(ds)
        End If

        If (Me.IsDivideDestCust(custIndex) = False OrElse _
            Me.IsEdiLDestChk()) Then

            '要望番号1282 追加START 2012.07.23
            'ディック（岩槻）追加箇所 20120601 terakawa Start
            'EDI出荷(大)の届先設定
            If Me.EdiLDestChk(ds, rowNo, ediCtlNo, genericInsFlg) = False Then
                'If Me.EdiLDestChk(ds, rowNo, ediCtlNo) = False Then
                Return ds
            End If
            'ディック（岩槻）追加箇所 20120601 terakawa End
...
#End If
        ...
        'ディック（大阪）対応 20120605 ReStart
        '届先コードの初期値設定
        If custIndex.Equals(LMH030BLC.EdiCustIndex.DicOsk00010_00) = False Then
            ds = Me.SetDestCd(ds)
        End If
        'ディック（大阪）対応 20120605 ReEnd
        ...
        Select Case custIndex
            ...
            'Case LMH030BLC.EdiCustIndex.DicOsk00010_00 _
            '    , LMH030BLC.EdiCustIndex.DicItk10001_00 _
            '    , LMH030BLC.EdiCustIndex.DicItk10002_00 _
            '    , LMH030BLC.EdiCustIndex.DicItk10007_00 _
            '    , LMH030BLC.EdiCustIndex.DicItk10009_00 _
            '    , LMH030BLC.EdiCustIndex.DicGnm00039_00 _
            '    , LMH030BLC.EdiCustIndex.DicGnm00072_00 _
            '    , LMH030BLC.EdiCustIndex.DicGnm00076_00

            Case LMH030BLC.EdiCustIndex.DicItk10009_00 _
                , LMH030BLC.EdiCustIndex.DicGnm00039_00
            ...
            Case LMH030BLC.EdiCustIndex.DicOsk00010_00 _
                ...
                , LMH030BLC.EdiCustIndex.DicItk10007_00 _
                , LMH030BLC.EdiCustIndex.DicChbBC00010_00
                '要望番号2100 追加START 2013.10.04
                If ds.Tables("LMH030INOUT").Rows(0)("HITACHI_SAP_FLG").ToString().Equals("1") = False Then
                    ds = Me.CustOrdNoConect(ds)
                End If
                '要望番号2100 追加END 2013.10.04
        ...
        'ディック（大阪）対応 20120315 Start
        'オーダー重複チェック【ディック専用】
#If False Then      'UPD 2018/05/21 千葉は別バージョンで処理する　依頼番号 : 001666   【LMS】【EDI】千葉日立FN_キャンセル修正
            If Me.CustOrderNoCheck(ds, rowNo, ediCtlNo) = False Then
                Return ds
            End If

#Else
        If custIndex.Equals(LMH030BLC.EdiCustIndex.DicChbBC00010_00) Then
            '千葉専用
            If Me.CustOrderNoCheckChiba(ds, rowNo, ediCtlNo) = False Then
                Return ds
            End If
...
#End If
        ...
        'ディック追加箇所 20120224 start
        'ディックはまとめ対象荷主(共通のまとめSQLを使用)
        ...
        If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
            ...
            ElseIf MyBase.GetResultCount > 1 Then
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.DIC_WID_L013, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningL("W199", LMH030BLC.DIC_WID_L013, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("0") = True Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.DIC_WID_L005, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = String.Empty
                    matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                    ds = Me._Blc.SetComWarningL("W168", LMH030BLC.DIC_WID_L005, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("1") = True Then
                ...
                If matomeStatus.Equals("10") = False Then
                    ...
                    choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.DIC_WID_L014, 0)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(4) = String.Empty
                        matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                        ds = Me._Blc.SetComWarningL("W198", LMH030BLC.DIC_WID_L014, ds, msgArray, matomeNo, String.Empty)
                        Return ds
...
#If True Then ' 届先自動変換対応(日立物流) 20170407 added by inoue 
        ...
        If (Me.IsDivideDestCust(custIndex) AndAlso _
            Me.IsDeivideDestCd() AndAlso _
            ds.Tables(TABLE_NM.LMH030OUT_DIVIDE_DEST).Rows.Count > 0) Then

            ' (注)誤った組み合わせが登録されると、以降のデータは全て誤った振分となる。解除するためには手動削除が必要
            ds = MyBase.CallDAC(Me._Dac, LMH030DAC205.FUNCTION_NM.InsertDivideDestData, ds)
        End If
#End If
...
#Region "EDI_Lの初期値設定(出荷登録処理)"
    ...
    Private Function SetEdiLShoki(ByVal ds As DataSet) As DataSet
        ...
        'ディック（岩槻）追加箇所 20120321 terakawa Start
        Dim custIndex As Integer = Convert.ToInt32(ds.Tables("LMH030INOUT").Rows(0).Item("EDI_CUST_INDEX"))
        'ディック（岩槻）追加箇所 20120321 terakawa End
        ...
        'ディック（岩槻）追加箇所 20120604 terakawa Start
        '届先M取得()
        'Select Case custIndex
        '    （大阪）ディック大阪
        '    Case LMH030BLC.EdiCustIndex.DicOsk00010_00
        '        ディック大阪では届先マスタを取得しない。
        '    Case Else
        ...
        If ds.Tables("LMH030_M_DEST").Rows.Count > 0 Then
            ...
            mDestFlgYN = True
        End If
        'ディック（岩槻）追加箇所 20120604 terakawa End
        ...
        If String.IsNullOrEmpty(ediDr("PICK_KB").ToString().Trim()) = True Then
            If mDestFlgYN = True Then
                ...
                '（群馬）ディック物流群馬
                Select Case custIndex
                    Case LMH030BLC.EdiCustIndex.DicGnm00076_00
                        If ediDr("BIN_KB").ToString().Trim() = "03" Then
                            ediDr("PICK_KB") = "01"
                        Else
        ...
        'ディック（岩槻）追加箇所 terakawa 20120601 Start
        '荷送人コード(大)
        '（岩槻）ディック物流埼玉 東ケミ
        '（岩槻）ディック物流埼玉 東ケミ01
        Select Case custIndex
            Case LMH030BLC.EdiCustIndex.DicItk00899_00 _
                , LMH030BLC.EdiCustIndex.DicItk00899_01
                '注意事項
                ediDr("SHIP_CD_L") = "ZZZZZ000"
        End Select
        'ディック（岩槻）追加箇所 terakawa 20120601 End
        ...
        If String.IsNullOrEmpty(ediDr("COA_YN").ToString().Trim()) = True Then
            ...
            Else
                ...
                Select Case custIndex
                    Case LMH030BLC.EdiCustIndex.DicItk10008_00
...
#Region "EDI出荷(中)の初期値設定"
    ...
    Private Function EdiMDefaultSet(ByVal ds As DataSet, ByVal setDs As DataSet, _
        ...
        Select Case custIndex
            Case LMH030BLC.EdiCustIndex.DicItk00899_00 _
                , LMH030BLC.EdiCustIndex.DicItk00899_01 _
                , LMH030BLC.EdiCustIndex.DicItk10001_00 _
                , LMH030BLC.EdiCustIndex.DicItk10002_00 _
                , LMH030BLC.EdiCustIndex.DicItk10003_00 _
                , LMH030BLC.EdiCustIndex.DicItk10007_00 _
                , LMH030BLC.EdiCustIndex.DicItk10008_00 _
                , LMH030BLC.EdiCustIndex.DicItk10009_00 _
                '注意事項
                ediMDr("REMARK") = mGoodsDr("OUTKA_ATT")

                '(2013.04.11)要望番号2017 ディック（群馬）追加START
                'ＤＩＣ物流群馬日パケ
                'ＤＩＣ物流群馬トートタンク
                'ＤＩＣ物流群馬
            ...
            Case LMH030BLC.EdiCustIndex.DicGnm00039_00 _
                ...
                , LMH030BLC.EdiCustIndex.DicGnm00076_00 _
                , LMH030BLC.EdiCustIndex.DicChbBC00010_00
                If String.IsNullOrEmpty(ediMDr("LOT_NO").ToString()) = True Then

                    choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.DIC_WID_M004, count)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        ds = Me._Blc.SetComWarningM("W139", LMH030BLC.DIC_WID_M004, ds, setDs, msgArray, _
                                                    ediMDr("LOT_NO").ToString(), String.Empty)
                ...
                End If
                '(2013.04.11)要望番号2017 ディック（群馬）追加END
        ...
        'ディック（大阪）対応　20120315  Start
        '商品名
        Dim mGoodsNm As String = Me._Blc.ReplaceHyphen(mGoodsDr("GOODS_NM_1").ToString().Trim())
        mGoodsNm = Me._Blc.ReplaceSokuon(mGoodsNm.Trim())
        Dim ediGoodsNm As String = Me._Blc.ReplaceHyphen(ediMDr("GOODS_NM").ToString().Trim())
        ediGoodsNm = Me._Blc.ReplaceSokuon(ediGoodsNm.Trim())
        Dim mDestGoodsNm As String = String.Empty
        If String.IsNullOrEmpty(ediMDr("DEST_GOODS_NM").ToString()) = False Then
            mDestGoodsNm = Me._Blc.ReplaceHyphen(ediMDr("DEST_GOODS_NM").ToString().Trim())
            mDestGoodsNm = Me._Blc.ReplaceSokuon(mDestGoodsNm.Trim())
        End If

        'スペース除去チェックの追加
        mGoodsNm = Me._Blc.SpaceCutChk(mGoodsNm)
        ediGoodsNm = Me._Blc.SpaceCutChk(ediGoodsNm)
        mDestGoodsNm = Me._Blc.SpaceCutChk(mDestGoodsNm)
        'ディック（岩槻）追加箇所 20120601 End
        ...
        ElseIf String.IsNullOrEmpty(mGoodsNm) = False AndAlso mDestGoodsNm.Equals(ediGoodsNm) = False AndAlso _
               String.IsNullOrEmpty(mDestGoodsNm) = False Then
            choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.DIC_WID_M002, count)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                msgArray(5) = String.Empty
                ds = Me._Blc.SetComWarningM("W194", LMH030BLC.DIC_WID_M002, ds, setDs, msgArray, _
                                            ediMDr("GOODS_NM").ToString(), ediMDr("DEST_GOODS_NM").ToString())
            ...
            ElseIf choiceKb.Equals("01") = True Then
                ...
                Else
                    ediMDr("REMARK") = Me.LeftB(String.Concat(ediMDr("REMARK"), Space(2), "受信品名＝", ediMDr("GOODS_NM")), 100)
                End If
        ...
        '        '2012.03.08 修正START
        '        'ds = Me._Blc.SetComWarningM("W159", LMH030BLC.DIC_WID_M002, ds, setDs, msgArray, ediGOODSNm, mGOODSNm)
        '        ds = Me._Blc.SetComWarningM("W159", LMH030BLC.DIC_WID_M002, ds, setDs, msgArray, _
        '                                    ediMDr("GOODS_NM").ToString(), mGoodsDr("GOODS_NM_1").ToString())
        '        '2012.03.08 修正END
        ...
        '        'チェックなし
        '    Else
        '        choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.DIC_WID_M003, count)
        ...
        '            msgArray(4) = "EDIデータ"
        '            msgArray(5) = String.Empty
        '            ds = Me._Blc.SetComWarningM("W159", LMH030BLC.DIC_WID_M003, ds, setDs, msgArray, ediIRIMENm, mIRIMENm)
...
#Region "EDI出荷(大)のDAC側でのチェック"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function EdiLDestChk(ByVal ds As DataSet, ByVal rowNo As String, _
                                 ByVal ediCtlNo As String, ByVal genericInsFlg As Integer) As Boolean
        '要望番号1282 修正END 2012.07.23
        ...
        Select Case custIndex
            ...
            Case Else
                ...
                'ディック（岩槻）追加箇所 20120604 terakawa Start
                'EDI届先コードを使って⇒DEST_CDが空の場合はEDI_DEST_CDを使用する
                ...
                '■届先コードをセット■
                choiceKb = Me.SetDestWarningChoiceKb(dtEdiL, ds, LMH030BLC.DIC_WID_L015, 0)

#If False Then ' 届先自動変換対応(日立物流) 20170407 changed by inoue 

                    '■届先マスタを取得■
                    If String.IsNullOrEmpty(drEdiL("DEST_CD").ToString().Trim()) = False OrElse _
                      String.IsNullOrEmpty(drEdiL("EDI_DEST_CD").ToString().Trim()) = False Then
                        ds = MyBase.CallDAC(Me._DacCom, "SelectDataMdest", ds)
                    End If
#Else
                If (Me.IsDeivideDestCd = False) Then
...
#End If
                ...
                ElseIf mDestCount = 0 Then
                    ...
                    '要望番号1282 修正START 2012.07.23
                    If Me.SetInsMDestFromDest(ds, rowNo, ediCtlNo, workDestCd, workDestString, genericInsFlg) = False Then
                        'If Me.SetInsMDestFromDest(ds, rowNo, ediCtlNo, workDestCd, workDestString) = False Then
                        Dim chkRowNo As Integer = Convert.ToInt32(rowNo)
                        If MyBase.IsMessageStoreExist(chkRowNo) = True Then
                            'チェックでエラーがあった場合は処理終了
                            Return False
                ...
                Else
                    'ディック（群馬）追加箇所 terakawa 20120711 Start
                    'ディック（岩槻）追加箇所 terakawa 20120604 Start
                    ...
                    Select Case custIndex
                        ...
                        Case Else
                            ...
                            If String.IsNullOrEmpty(choiceKb) = True Then
                                ...
                                '届先が確定できない場合はワーニング
                                ds = Me._Blc.SetComWarningL("W208", LMH030BLC.DIC_WID_L015, ds, msgArray, workDestCd, String.Empty)
                                flgWarning = True 'ワーニングフラグをたてて処理続行
                                'ディック（岩槻）追加箇所 terakawa 20120604 End
                            End If
                            ...
                            End If
                    End Select
                    'ディック（群馬）追加箇所 terakawa 20120711 End
                End If
        End Select
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.DIC_WID_M001, 0)
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    Else
                        ...
                        ds = Me._Blc.SetComWarningM("W162", LMH030BLC.DIC_WID_M001, ds, setDs, msgArray, custGoodsCd, String.Empty)
                ...
                If Me.EdiMDefaultSet(ds, setDs, i, unsoData, rowNo, ediCtlNo) = False Then
                    ...
                    '2012.03.01 大阪対応START
                    'ディックは現段階ではワーニングが存在するが、エラーはなし。共通のロジックを組み込む為入れておく
                    Dim chkRowNo As Integer = Convert.ToInt32(rowNo)
                    If MyBase.IsMessageStoreExist(chkRowNo) = True Then
                        '整合性チェックでエラーがあった場合は処理終了
                        Return False
...
#Region "届先マスタチェック"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        'ディック（岩槻）追加箇所 terakawa 20120604 Start
        Dim custIndex As Integer = Convert.ToInt32(ds.Tables("LMH030INOUT").Rows(0).Item("EDI_CUST_INDEX"))
        Dim ediLNo As String = dtEdi.Rows(0)("EDI_CTL_NO").ToString()
        'ディック（岩槻）追加箇所 terakawa 20120604 End
        ...
        '要望番号:1330 terkawa 2012.08.10 Start
        Dim ediKanaNm As String = dtEdi.Rows(0).Item("DEST_AD_4").ToString()
        '要望番号:1330 terkawa 2012.08.10 End
        ...
        'ディック（岩槻）追加箇所 terakawa 20120604 Start
        '諸口対応(届先マスタ)
        If Right(mDestCd, 8).Equals("SOEP9999") = True Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E487", New String() {ediLNo}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            Return False
        End If
        ...
        Select Case custIndex
            Case LMH030BLC.EdiCustIndex.DicItk10008_00
                ...
                If String.IsNullOrEmpty(mDestCd) = True Then
                    'チェックなし
                Else
                    If Right(mDestCd, 8).Equals("NIBY4700") = True Then

                        choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DIC_WID_L010, 0)
                        ...
                        If String.IsNullOrEmpty(choiceKb) = True Then
                            ...
                            msgArray(4) = String.Empty
                            msgArray(5) = String.Empty
                            ds = Me._Blc.SetComWarningL("W209", LMH030BLC.DIC_WID_L010, ds, msgArray, mDestCd, String.Empty)
        ...
        If mDestNm.Equals(ediDestNm) = True Then
            'チェックなし
        Else
            choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DIC_WID_L006, 0)
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                ds = Me._Blc.SetComWarningL("W166", LMH030BLC.DIC_WID_L006, ds, msgArray, _
        ...
        Else
            ...
            If mAdAll.Equals(ediFreeC21) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DIC_WID_L007, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.DIC_WID_L007, ds, msgArray, ediFreeC21, mAdAll)
        ...
        Else
            If mTel.Equals(ediTel) = False Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DIC_WID_L008, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.DIC_WID_L008, ds, msgArray, ediTel, mTel)
        ...
        'ディック（岩槻）追加箇所 terakawa 20120604 Start
        '届先JISコード(マスタ値が完全一致でなければワーニング)
        If String.IsNullOrEmpty(ediDestJisCd) = True OrElse _
           ediDestJisCd.Equals("XXXX") = True Then
            'チェックなし
        Else
            If mJis.Equals(ediDestJisCd) = False Then

                choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DIC_WID_L009, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = "EDIデータ"
                    msgArray(5) = String.Empty
                    ds = Me._Blc.SetComWarningL("W166", LMH030BLC.DIC_WID_L009, ds, msgArray, ediDestJisCd, mJis)
        ...
        '要望番号:1330 terakawa 2012.08.10 Start
        '（群馬）ディック物流群馬トートタンク
        '（群馬）ディック物流群馬
        Select Case custIndex
            Case LMH030BLC.EdiCustIndex.DicGnm00072_00 _
               , LMH030BLC.EdiCustIndex.DicGnm00076_00
                '届先カナ名(マスタ値が完全一致でなければワーニング)
                If String.IsNullOrEmpty(ediKanaNm) = True Then
                    'チェックなし
                Else
                    If mKanaNm.Equals(ediKanaNm) = False Then

                        choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DIC_WID_L016, 0)
                        ...
                        If String.IsNullOrEmpty(choiceKb) = True Then
                            ...
                            msgArray(4) = "EDIデータ"
                            msgArray(5) = String.Empty
                            ds = Me._Blc.SetComWarningL("W166", LMH030BLC.DIC_WID_L016, ds, msgArray, ediKanaNm, mKanaNm)

                            compareWarningFlg = True

                        ElseIf choiceKb.Equals("01") = True Then
                            'ワーニングで"はい"を選択時
                            dtMdest.Rows(0).Item("KANA_NM") = dtEdi.Rows(0)("DEST_AD_4").ToString()
                            'マスタ更新対象フラグ
                            dtMdest.Rows(0).Item("MST_UPDATE_FLG") = "1"
                        End If
                    End If
            ...
            Case Else
        End Select
        '要望番号:1330 terakawa 2012.08.10 End
        ...
        '        ds = Me._Blc.SetComWarningL("W197", LMH030BLC.DIC_WID_L009, ds, msgArray, ediDestJisCd, mJis)
        ...
        '        ds = Me._Blc.SetComWarningL("W188", LMH030BLC.DIC_WID_L010, ds, msgArray, ediDestJisCd, mJis)
        ...
        '    choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DIC_WID_L011, 0)
        ...
        '        msgArray(4) = "EDIデータ"
        '        msgArray(5) = String.Empty
        '        ds = Me._Blc.SetComWarningL("W187", LMH030BLC.DIC_WID_L011, ds, msgArray, ediDestJisCd, mJis)
        ...
        '        msgArray(5) = String.Empty
        '        ds = Me._Blc.SetComWarningL("W187", LMH030BLC.DIC_WID_L011, ds, msgArray, mZipJis, mJis)
...
#Region "届先マスタ追加時チェック"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetInsMDestFromDest(ByVal ds As DataSet, ByVal rowNo As String, _
                                         ByVal ediCtlNo As String, ByVal workDestCd As String, _
                                         ByVal workDestString As String, ByVal genericInsFlg As Integer) As Boolean
        ...
        choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DIC_WID_L012, 0)
        ...
        If String.IsNullOrEmpty(choiceKb) = True AndAlso genericInsFlg = 0 Then
            ...
            ds = Me._Blc.SetComWarningL("W182", LMH030BLC.DIC_WID_L012, ds, msgArray, workDestCd, String.Empty) '追加箇所 20110222
        ...
        ElseIf choiceKb.Equals("01") = True OrElse genericInsFlg = 1 Then
            ...
            End If
            '要望番号:1330 terakawa 2012.08.10 Start
            If String.IsNullOrEmpty(drEdiL("DEST_AD_4").ToString()) = False Then
                drMD("KANA_NM") = drEdiL("DEST_AD_4").ToString()
            End If
            '要望番号:1330 terakawa 2012.08.10 End
            ...
            Select Case custIndex
                ...
                Case LMH030BLC.EdiCustIndex.DicOsk00010_00 _
                    ...
                    , LMH030BLC.EdiCustIndex.DicGnm00072_00 _
                    , LMH030BLC.EdiCustIndex.DicGnm00076_00 _
                    , LMH030BLC.EdiCustIndex.DicItk00899_00 _
                    , LMH030BLC.EdiCustIndex.DicItk10007_00 _
                    , LMH030BLC.EdiCustIndex.DicItk10008_00 _
                    , LMH030BLC.EdiCustIndex.DicChbBC00010_00
...
#Region "データセット設定(出荷L)"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            'ADD 2018/04/25 依頼番号 1585 千葉BC_日立FN_EDI出荷登録時修正対応
            If ("10").Equals(ediDr("NRS_BR_CD")) _
                AndAlso ("00010").Equals(ediDr("CUST_CD_L")) Then

                outkaDr("CUST_ORD_NO") = String.Empty       'オーダー番号
                outkaDr("BUYER_ORD_NO") = String.Empty      '注文番号
                outkaDr("PICK_KB") = "02".ToString          'ピック区分  02:まとめﾋﾟｯｷﾝｸﾞﾘｽﾄ
            End If
...
#Region "データセット設定(運送L)"
    ...
    Private Function SetDatasetUnsoL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        Dim custIndex As Integer = Convert.ToInt32(ds.Tables("LMH030INOUT").Rows(0).Item("EDI_CUST_INDEX")) '2019/05/22 要望番号005128 ADD
        Dim ediDr As DataRow = ds.Tables("LMH030_OUTKAEDI_L").Rows(0)
        ...
        If matomeFlg = False Then
            ...
            '2019/07/01 要望番号006435 DEL START
            ''2019/05/22 要望番号005128 ADD START
            'Select Case custIndex
            '    Case LMH030BLC.EdiCustIndex.DicGnm00076_00
            '        unsoDr("BIN_KB") = GetBinKbDicGnm00076(ds)
            '    Case Else
            '        '2019/05/22 要望番号005128 ADD END
            '2019/07/01 要望番号006435 DEL END
            unsoDr("BIN_KB") = ediDr("BIN_KB")
            'End Select    '2019/05/22 要望番号005128 ADD    '2019/07/01 要望番号006435 DEL
            ...
            '要望番号1457:(群馬：日立(00076) 出荷登録時、元着払いが〔着払い〕になる) 2012/09/21 本明 Start
            'unsoDr("PC_KB") = ediDr("PICK_KB")
            unsoDr("PC_KB") = ediDr("PC_KB")
            '要望番号1457:(群馬：日立(00076) 出荷登録時、元着払いが〔着払い〕になる) 2012/09/21 本明 End
            ...
            If ("10").Equals(ediDr("NRS_BR_CD")) _
                ...
                unsoDr("TARIFF_BUNRUI_KB") = "40".ToString      'タリフ分類区分 40:横持ち
                unsoDr("UNSO_TEHAI_KB") = "20".ToString         '運送手配       20:先方手配
                unsoDr("SEIQ_TARIFF_CD") = String.Empty         'タリフコード
                unsoDr("SEIQ_ETARIFF_CD") = String.Empty        '割増タリフコード
    ...
    ''2019/05/22 要望番号005128 ADD START
    ' ''' <summary>
    ' ''' 便区分取得(日立物流ファインネクスト(群馬))
    ' ''' </summary>
    ' ''' <param name="ds">データセット</param>
    ' ''' <returns>便区分</returns>
    ' ''' <remarks>出荷予定日と納入予定日から便区分を決定する</remarks>
    'Private Function GetBinKbDicGnm00076(ByVal ds As DataSet) As String

    '    Dim outkaEdiLRow As DataRow = ds.Tables("LMH030_OUTKAEDI_L").Rows(0)

    '    '2019/06/04 要望番号005128 ADD START
    '    Dim sDestAdr As String = outkaEdiLRow.Item("DEST_AD_1").ToString()

    '    If sDestAdr.StartsWith("東京都") OrElse _
    '       sDestAdr.StartsWith("神奈川県") OrElse _
    '       sDestAdr.StartsWith("千葉県") OrElse _
    '       sDestAdr.StartsWith("埼玉県") OrElse _
    '       sDestAdr.StartsWith("群馬県") OrElse _
    '       sDestAdr.StartsWith("栃木県") OrElse _
    '       sDestAdr.StartsWith("茨城県") Then

    '        '1都6県の場合、便区分の変更に進む

    '    Else

    '        '1都6県でない場合、EDI出荷データ.便区分
    '        Return outkaEdiLRow.Item("BIN_KB").ToString()

    '    End If
    '    '2019/06/04 要望番号005128 ADD END

    '    Dim sOutkaDate As String = outkaEdiLRow.Item("OUTKA_PLAN_DATE").ToString()
    '    Dim sArrDate As String = outkaEdiLRow.Item("ARR_PLAN_DATE").ToString()

    '    Dim outkaDate As New DateTime(CInt(Left(sOutkaDate, 4)), CInt(Mid(sOutkaDate, 5, 2)), CInt(Right(sOutkaDate, 2)))
    '    Dim arrDate As New DateTime(CInt(Left(sArrDate, 4)), CInt(Mid(sArrDate, 5, 2)), CInt(Right(sArrDate, 2)))


    '    Dim bHoliday As Boolean = False
    '    '祝日マスタの検索条件設定
    '    Dim mHolInTbl As DataTable = ds.Tables("LMH030_M_HOL_IN")
    '    Dim mHolInRow As DataRow = mHolInTbl.NewRow
    '    mHolInRow.Item("NRS_BR_CD") = outkaEdiLRow.Item("NRS_BR_CD")
    '    mHolInRow.Item("DATE") = sArrDate
    '    mHolInTbl.Rows.Add(mHolInRow)

    '    '祝日マスタの検索
    '    ds = MyBase.CallDAC(Me._Dac, "SelectCountMHol", ds)
    ...
    '    Dim bSaturday As Boolean = False
    '    If arrDate.DayOfWeek = DayOfWeek.Saturday Then
    '        '土曜の場合
    '        bSaturday = True
    '    End If

    '    '出荷予定日と納入予定日の差
    '    Dim span As TimeSpan = arrDate - outkaDate
    '    Dim spanDays As Integer = span.Days

    '    '便区分（初期値：EDI出荷データ.便区分）
    '    Dim binKB As String = outkaEdiLRow.Item("BIN_KB").ToString()

    '    '便区分の変更（変更しない場合は上書きしない）
    '    If spanDays = 0 Then
    '        '当日
    '        binKB = "03"    '03:2便
    '    ElseIf bHoliday Then
    '        '祝日
    '        binKB = "09"    '09:チャーター便
    '    ElseIf bSaturday Then
    '        '土曜
    '        binKB = "06"    '06:3便
    '    Else

    '        '出荷予定日から納入予定日までの休業日の日数をカウント
    '        Dim iCntDaysOff As Integer = 0
    '        Dim tmpDate As DateTime = outkaDate.AddDays(1)
    '        While tmpDate < arrDate

    '            '休業日フラグ
    '            Dim bDayOff As Boolean = False

    '            '祝日マスタの検索条件設定
    '            mHolInTbl.Rows(0).Item("DATE") = tmpDate.ToString("yyyyMMdd")
    ...
    '            If tmpDate.DayOfWeek = DayOfWeek.Saturday OrElse tmpDate.DayOfWeek = DayOfWeek.Sunday Then
    '                '土曜・日曜の場合
    '                bDayOff = True
    '            End If
    ...
    '        If spanDays - iCntDaysOff = 1 Then
    '            '納入予定日が出荷予定日の翌営業日の場合
    '            binKB = "02"    '02:1便
    '        End If
...
#Region "データセット設定(運送M)"
    ...
    Private Function SetDatasetUnsoM(ByVal ds As DataSet) As DataSet
        Dim ediDr As DataRow
        'ディック（大阪）対応 20120215 Start
        Dim ediDrL As DataRow
        'ディック（大阪）対応 20120215 End
        ...
        For i As Integer = 0 To max
            'ディック（大阪）対応 20120215 Start
            ediDrL = ds.Tables("LMH030_OUTKAEDI_L").Rows(0)
            'ディック（大阪）対応 20120215 End
            ediDr = ds.Tables("LMH030_OUTKAEDI_M").Rows(i)
            ...
            '要望番号2113 修正START 2013.10.16
            Select Case custIndex

                '（岩槻）ディック物流春日部
                '（岩槻）ディック物流千葉－岩槻分
                Case LMH030BLC.EdiCustIndex.DicItk10001_00 _
                    , LMH030BLC.EdiCustIndex.DicItk10002_00 _
                    , LMH030BLC.EdiCustIndex.DicItk10007_00 _
                    , LMH030BLC.EdiCustIndex.DicItk00899_00 _
                    , LMH030BLC.EdiCustIndex.DicItk10008_00

                    If ds.Tables("LMH030INOUT").Rows(0)("HITACHI_SAP_FLG").ToString().Equals("1") = True Then
                        unsoMDr("ZBUKA_CD") = LMH030BLC205.DEF_SAITAMA_ZBUKA_CD
                        unsoMDr("ABUKA_CD") = LMH030BLC205.DEF_SAITAMA_ABUKA_CD
                    Else
                        unsoMDr("ZBUKA_CD") = ediDrL("FREE_C02")
                        unsoMDr("ABUKA_CD") = ediDrL("FREE_C01")
                    End If
...
#Region "CUST_ORD_NO連結処理"
    ...
    Private Function CustOrdNoConect(ByVal ds As DataSet) As DataSet
        ...
        Dim dtM As DataTable = ds.Tables("LMH030_OUTKAEDI_M")
        Dim max As Integer = dtM.Rows.Count - 1
        '2018/03/05 001074【EDI】千葉DIC_出荷EDI振分条件の変更 対応 Annen add start
        'DIC、千葉BC物管の場合、処理対象としない。
        Dim custIndex As Integer = Convert.ToInt32(ds.Tables("LMH030INOUT").Rows(0).Item("EDI_CUST_INDEX"))
        If custIndex.Equals(102) AndAlso _
           dtL.Rows(0).Item("NRS_BR_CD").ToString.Equals("10") AndAlso _
           dtL.Rows(0).Item("CUST_CD_L").ToString.Equals("00010") Then
            Return ds
        End If
        '2018/03/05 001074【EDI】千葉DIC_出荷EDI振分条件の変更 対応 Annen add end 

        For i As Integer = 0 To max
            '出荷(中)の中盤が"001"の場合はEDI出荷(大)のCUST_ORD_NOにEDI出荷(中)のCUST_ORD_NO_DTLをセット
            ...
            Else
                '出荷(中)の中盤が"001"以外の場合は、EDI出荷(大)のCUST_ORD_NOにEDI出荷(中)のCUST_ORD_NO_DTLが含まれていない場合のみ連結
...
#Region "オーダーNO重複チェック"
    ...
    Private Function CustOrderNoCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        'ディック（群馬）追加箇所 20120711 terakawa Start
        Dim setDtInOut As DataTable = setDs.Tables("LMH030INOUT")
        'ディック（群馬）追加箇所 20120711 terakawa End
        ...
        For i As Integer = 0 To edimCount
            ...
            'ディック（群馬）追加箇所 20120711 terakawa Start
            setDtInOut.ImportRow(drIn)
            'ディック（群馬）追加箇所 20120711 terakawa End

            'オーダー番号重複チェック処理
            If String.IsNullOrEmpty(drEdiL.Item("CUST_ORD_NO").ToString()) = False Then
                If drIn("ORDER_CHECK_FLG").Equals("1") = True Then
                    Call MyBase.CallDAC(Me._Dac, "SelectOrderCheckData", setDs)
                    If MyBase.GetResultCount > 0 Then
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E377", New String() {"出荷データ"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                        Return False
                    End If
...
#Region "オーダーNO重複チェック千葉専用"
    ''' <summary>
    ''' オーダーNO重複チェック千葉専用　ADD 2018/05/21 2018/05/21 千葉は別バージョンで処理する　依頼番号 : 001666   【LMS】【EDI】千葉日立FN_キャンセル修正
    ''' </summary>
    ''' <param name="ds"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function CustOrderNoCheckChiba(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To edimCount
            ...
            If String.IsNullOrEmpty(drEdiL.Item("CUST_ORD_NO").ToString()) = False Then
                If drIn("ORDER_CHECK_FLG").Equals("1") = True Then
                    Call MyBase.CallDAC(Me._Dac, "SelectOrderCheckDataChiba", setDs)
                    If MyBase.GetResultCount > 0 Then
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E377", New String() {"出荷データ"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                        Return False
                    End If
...
#Region "届先振分"
    ...
    ''' 届先振り分け対象の荷主か判定する。
    ''' </summary>
    ''' <param name="custIndex"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function IsDivideDestCust(ByVal custIndex As Integer) As Boolean
        ...
        ' 届先振分対象外
        _divideDestStatus = DivideDestStatusType.None
    ...
    ''' <summary>
    ''' 振り分け先が設定された届先コードか判定する。
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private ReadOnly Property IsDeivideDestCd() As Boolean
        Get
            Return (_divideDestStatus <> DivideDestStatusType.None)
        End Get
    End Property
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private ReadOnly Property IsEdiLDestChk() As Boolean
        Get

            Return Not (DivideDestStatusType.NotExistsUnsoAtt = _divideDestStatus)
    ...
    ''' <returns></returns>
    ''' <remarks>
    ''' EDIバッチでも別条件で同処理を行っている
    ''' </remarks>
    Private Function DivideDestCd(ByVal ds As DataSet) As DataSet

        _divideDestStatus = DivideDestStatusType.None

        Dim warningId As String = LMH030BLC.DIC_WID_L017

        Dim outkaEdiL As DataTable = ds.Tables(TABLE_NM.LMH030_OUTKAEDI_L)
        Dim mDestData As DataTable = ds.Tables(TABLE_NM.LMH030_M_DEST)
        Dim ediOutkaRow As DataRow = ds.Tables(TABLE_NM.LMH030_OUTKAEDI_L).Rows(0)

        ' 配送時注意事項
        Dim unsoAtt As String = If(ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.UNSO_ATT), "").ToString().Trim()

        ' 変換一覧取得
        Dim wkData As DataSet = ds.Clone

#If False Then ' 変換対象の条件を変更(既定のWMS届先コード => 配送注意事項が設定された全届先)
        wkData.Tables(TABLE_NM.LMH030_OUTKAEDI_L).Merge(ds.Tables(TABLE_NM.LMH030_OUTKAEDI_L))
        wkData = MyBase.CallDAC(_Dac, LMH030DAC205.FUNCTION_NM.SelectDivideDestByDestCd, wkData)

        Dim isDivideDestCd As Boolean _
            = (wkData.Tables(TABLE_NM.LMH030OUT_DIVIDE_DEST).Rows.Count > 0 AndAlso _
               String.IsNullOrEmpty(unsoAtt) = False)
#Else
        Dim isDivideDestCd As Boolean _
            = (String.IsNullOrEmpty(unsoAtt) = False)
...
#End If
        ...
        If (isDivideDestCd) Then

            _divideDestStatus = DivideDestStatusType.NotExistsUnsoAtt

            wkData.Clear()
            wkData.Tables(TABLE_NM.LMH030_OUTKAEDI_L).Merge(ds.Tables(TABLE_NM.LMH030_OUTKAEDI_L))
            wkData = MyBase.CallDAC(_Dac, LMH030DAC205.FUNCTION_NM.SelectDivideDestByDestCdAndUnsoAtt, wkData)

            Dim divideDestRow As DataRow = Nothing

            If (wkData.Tables(TABLE_NM.LMH030OUT_DIVIDE_DEST).Rows.Count = 1) Then
                divideDestRow = wkData.Tables(TABLE_NM.LMH030OUT_DIVIDE_DEST).Rows(0)
            End If


            If (divideDestRow IsNot Nothing) Then


                ' LMH030_OUTKAEDI_Lへ届先コードを上書
                ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_CD) = divideDestRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.DIVIDE_DEST_CD)
                ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_NM) = divideDestRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.DIVIDE_DEST_NM)
                ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_AD_4) = divideDestRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.DIVIDE_KANA_NM)
                ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_JIS_CD) = divideDestRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.DIVIDE_JIS)
                ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_TEL) = divideDestRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.DIVIDE_TEL)
                ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_AD_1) = divideDestRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.DIVIDE_AD_1)
                ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_AD_2) = divideDestRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.DIVIDE_AD_2)
                ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_AD_3) = divideDestRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.DIVIDE_AD_3)

                ' 設定値クリア
                ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.SP_NHS_KB) = ""
                ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.COA_YN) = ""
                ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.PICK_KB) = ""

                _divideDestStatus = DivideDestStatusType.Divided

                ' M_DEST 検索
                '■届先マスタを取得■
                If (String.IsNullOrEmpty(ediOutkaRow(LMH030DAC205.COLUMN_NM.DEST_CD).ToString().Trim()) = False OrElse _
                    String.IsNullOrEmpty(ediOutkaRow(LMH030DAC205.COLUMN_NM.EDI_DEST_CD).ToString().Trim()) = False) Then
                    ds = MyBase.CallDAC(Me._DacCom, LMH030DAC205.FUNCTION_NM.SelectDataMdest, ds)
                End If

            Else

                ' ワーニング画面の検索用に元の届先コードを保持
                Dim originDestCd As String = If(ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_CD), "").ToString().Trim()

                ' ワーニング表示
                Dim choiceKb As String = Me.SetDestWarningChoiceKb(outkaEdiL, ds, warningId, 0)
                If (String.IsNullOrEmpty(choiceKb) = True) Then

                    ' 初期化
                    Dim warnMessages() As String = Enumerable.Repeat(String.Empty, 6).ToArray()

                    warnMessages(1) = WARNING_ITEM_NAME.DEST_CD
                    warnMessages(2) = originDestCd
                    warnMessages(3) = unsoAtt

                    Dim ediValue As String = originDestCd
                    Dim mstValue As String = ediValue

                    ' [%1]は画面の項目名で使用されるため、メッセージから除外
                    ' 届先コード([%2])の振分先が見つかりません。振分先として届先を選択してください。配送時注意事項([%3])
                    ds = Me._Blc.SetComWarningL("W266" _
                                              , warningId _
                                              , ds _
                                              , warnMessages _
                                              , ediValue _
                ...
                ElseIf (CHOICE_KB.YES.Equals(choiceKb) = True) Then

                    If (originDestCd.Equals(ediOutkaRow.Item(LMH030DAC205.M_DEST_COL_NM.DEST_CD)) = False) Then
                        ...
                        If (mDestData.Rows.Count > 0) Then

                            ' WMS届先振分テーブルへの追加を判定
                            If (IsInsertDivideDest(originDestCd, ediOutkaRow, mDestData.Rows(0))) Then

                                ' 届先振分テーブルへ追加
                                Dim newRow As DataRow = ds.Tables(TABLE_NM.LMH030OUT_DIVIDE_DEST).NewRow
                                newRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.NRS_BR_CD) = ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.NRS_BR_CD)
                                newRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.CUST_CD_L) = ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.CUST_CD_L)
                                newRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.CUST_CD_M) = ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.CUST_CD_M)
                                newRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.DEST_CD_WMS) = originDestCd
                                newRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.UNSO_ATT) = ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.UNSO_ATT)
                                newRow.Item(LMH030DAC205.DIVIDE_DEST_COL_NM.DIVIDE_DEST_CD) = ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_CD)
                                ds.Tables(TABLE_NM.LMH030OUT_DIVIDE_DEST).Rows.Add(newRow)

                            End If

                            ' LMH030_OUTKAEDI_Lへ選択された届先のデータを上書
                            ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_NM) = mDestData.Rows(0).Item(LMH030DAC205.M_DEST_COL_NM.DEST_NM)
                            ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_AD_4) = mDestData.Rows(0).Item(LMH030DAC205.M_DEST_COL_NM.KANA_NM)
                            ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_JIS_CD) = mDestData.Rows(0).Item(LMH030DAC205.M_DEST_COL_NM.JIS)
                            ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_TEL) = mDestData.Rows(0).Item(LMH030DAC205.M_DEST_COL_NM.TEL)
                            ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_AD_1) = mDestData.Rows(0).Item(LMH030DAC205.M_DEST_COL_NM.AD_1)
                            ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_AD_2) = mDestData.Rows(0).Item(LMH030DAC205.M_DEST_COL_NM.AD_2)
                            ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_AD_3) = mDestData.Rows(0).Item(LMH030DAC205.M_DEST_COL_NM.AD_3)
                    ...
                    Else
                        ...
                        ' ワーニング表示後、届先の変更されていない場合は、ユーザーが振分対象外として選択したと判断する
                        _divideDestStatus = DivideDestStatusType.None
    ...
    ''' 届先振分テーブルへ追加するか判定する
    ''' </summary>
    ''' <param name="ediOutkaRow"></param>
    ''' <param name="mDestRow"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function IsInsertDivideDest(ByVal originDestCd As String _
                                      , ByVal ediOutkaRow As DataRow _
                                      , ByVal mDestRow As DataRow) As Boolean

        Return (ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_AD_1) _
                    .Equals(mDestRow.Item(LMH030DAC205.M_DEST_COL_NM.AD_1)) AndAlso _
                String.IsNullOrEmpty(ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_CD).ToString()) = False AndAlso _
                ediOutkaRow.Item(LMH030DAC205.COLUMN_NM.DEST_CD).Equals(originDestCd) = False)
