...
'  システム名       :  LM
'  サブシステム名   :  LMH              : EDI
'  プログラムID     :  LMH010HBLC151    : EDI入荷検索:ローム(404)から複写
'  EDI荷主ID　　　　:  151　　　        : ジョンソンエンドジョンソン
'  作  成  者       :  inoue
' ==========================================================================
Imports Jp.Co.Nrs.LM.Const
Imports Jp.Co.Nrs.LM.DAC
Imports Jp.Co.Nrs.LM.DAC.LMH010DAC151
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC151 = New LMH010DAC151()
...
#Region "セミEDI時　データセット設定(EDI受信HED・DTL)"
    ...
    Private Function SetSemiInkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer) As DataSet
        ...
        Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_JJ").NewRow()
        ...
        ' 荷主固有データ
        '----------------
        'drEdiRcvDtl("CATEGORY") = Left(drSetDtl(GetColumnName(JJArrivalColumns.CATEGORY)).ToString().Trim(), 30)
        'drEdiRcvDtl("VENDOR_NM") = Left(drSetDtl(GetColumnName(JJArrivalColumns.VENDOR_NM)).ToString().Trim(), 20)
        drEdiRcvDtl("PLAN_DATE") = Format(Convert.ToDateTime(drSetDtl(GetColumnName(JJArrivalColumns.PLAN_DATE))), "yyyyMMdd")
        'drEdiRcvDtl("CONT_NO") = Left(drSetDtl(GetColumnName(JJArrivalColumns.CONT_NO)).ToString().Trim(), 20)
        'drEdiRcvDtl("CONT_SIZE") = Left(drSetDtl(GetColumnName(JJArrivalColumns.CONT_SIZE)).ToString().Trim(), 10)
        'drEdiRcvDtl("OUTBOUND_NO") = Left(drSetDtl(GetColumnName(JJArrivalColumns.OUTBOUND_NO)).ToString().Trim(), 30)
        drEdiRcvDtl("GOODS_CD") = Left(drSetDtl(GetColumnName(JJArrivalColumns.GOODS_CD)).ToString().Trim(), 20)
        drEdiRcvDtl("GOODS_NM") = Left(drSetDtl(GetColumnName(JJArrivalColumns.GOODS_NM)).ToString().Trim(), 60)
        drEdiRcvDtl("CSE") = Left(drSetDtl(GetColumnName(JJArrivalColumns.CSE)).ToString().Trim(), 8)
        drEdiRcvDtl("IRISU") = Left(drSetDtl(GetColumnName(JJArrivalColumns.IRISU)).ToString().Trim(), 8)
        drEdiRcvDtl("PC") = Left(drSetDtl(GetColumnName(JJArrivalColumns.PC)).ToString().Trim(), 8)
        drEdiRcvDtl("LOT") = Left(drSetDtl(GetColumnName(JJArrivalColumns.LOT)).ToString().Trim(), 20)
        'drEdiRcvDtl("PRODUCTION_DATE") = Left(drSetDtl(GetColumnName(JJArrivalColumns.PRODUCTION_DATE)).ToString().Trim(), 20)
        'If Left(drSetDtl(GetColumnName(JJArrivalColumns.COA_UM)).ToString().Trim(), 1) = "〇" Then
        '    drEdiRcvDtl("COA_UM") = LMConst.FLG.ON
        'Else
        '    drEdiRcvDtl("COA_UM") = LMConst.FLG.OFF
        'End If
        'drEdiRcvDtl("BATCH") = Left(drSetDtl(GetColumnName(JJArrivalColumns.BATCH)).ToString().Trim(), 20)
        'drEdiRcvDtl("EXPIRETION_DATE") = Left(drSetDtl(GetColumnName(JJArrivalColumns.EXPIRETION_DATE)).ToString().Trim(), 30)
        'drEdiRcvDtl("ALREADY_RECEIVED_LOT") = Left(drSetDtl(GetColumnName(JJArrivalColumns.ALREADY_RECEIVED_LOT)).ToString().Trim(), 20)
        'drEdiRcvDtl("DESIRED_RELEASE_DATE") = Left(drSetDtl(GetColumnName(JJArrivalColumns.DESIRED_RELEASE_DATE)).ToString().Trim(), 30)
        drEdiRcvDtl("COMMENT") = Left(drSetDtl(GetColumnName(JJArrivalColumns.COMMENT)).ToString().Trim(), 30)
        'drEdiRcvDtl("QA_KENSA") = Left(drSetDtl(GetColumnName(JJArrivalColumns.QA_KENSA)).ToString().Trim(), 30)
        'drEdiRcvDtl("QA_ENDDATE") = Left(drSetDtl(GetColumnName(JJArrivalColumns.QA_ENDDATE)).ToString().Trim(), 30)

        drEdiRcvDtl("JISSEKI_SHORI_FLG") = "1"                                                          '実績処理フラグ

        'データセットに設定
        ds.Tables("LMH010_INKAEDI_DTL_JJ").Rows.Add(drEdiRcvDtl)
...
#Region "セミEDI時　データセット設定(EDI入荷(中))"
    ...
    Private Function SetSemiInkaEdiM(ByVal setDs As DataSet _
        ...
        Dim drRcvEdiDtl As DataRow = setDs.Tables("LMH010_INKAEDI_DTL_JJ").Rows(0)
        ...
        End If
#If False Then  'DEL 2020/06/10 入目対応をやめる
        '#If True Then   ' 入目対応 
        ...
        ' 入目算出
        If (Double.TryParse(TryCast(drRcvEdiDtl("PC"), String), totalQuantity) AndAlso
            Double.TryParse(TryCast(drRcvEdiDtl("CSE"), String), quantity) AndAlso
            totalQuantity > 0 AndAlso
            quantity > 0) Then
...
#End If
        ...
        drInkaEdiM("NB") = Convert.ToInt32(drRcvEdiDtl("PC").ToString())
        drInkaEdiM("INKA_PKG_NB") = Convert.ToInt32(drRcvEdiDtl("CSE").ToString())
        drInkaEdiM("HASU") = 0
        drInkaEdiM("OUTKA_FROM_ORD_NO") = ""    'drRcvEdiDtl("PO_PLUS_LINE")
        drInkaEdiM("BUYER_ORD_NO") = String.Empty
        drInkaEdiM("REMARK") = drRcvEdiDtl("COMMENT")
        drInkaEdiM("LOT_NO") = drRcvEdiDtl("LOT")
...
#Region "セミEDI時　データセット設定(EDI入荷(大))"
    ...
    Private Function SetSemiinkaediL(ByVal setDs As DataSet) As DataSet
        ...
        drInkaEdiL("INKA_DATE") = drRcvEdiDtl("PLAN_DATE")
        ...
        drInkaEdiL("HOKAN_STR_DATE") = drRcvEdiDtl("PLAN_DATE")
        drInkaEdiL("NIYAKU_YN") = "1"                                           ' 荷役料有無:有
        drInkaEdiL("TAX_KB") = "01"                                             ' 税区分:課税
        drInkaEdiL("REMARK") = drRcvEdiDtl("COMMENT")
...
#Region "セミEDI データセット設定(EDI管理番号(大・中))"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim dtRcvEdiDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_JJ")
...
#If False Then
        sMsg = "注文番号(カラム" & CType(JJArrivalColumns.DELIVERY, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(JJArrivalColumns.DELIVERY)).ToString()
...
#End If
        ...
        sMsg = "入荷日(カラム" & CType(JJArrivalColumns.PLAN_DATE, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(JJArrivalColumns.PLAN_DATE)).ToString().Trim()
        ...
        sMsg = "商品コード(カラム" & CType(JJArrivalColumns.GOODS_CD, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(JJArrivalColumns.GOODS_CD)).ToString().Trim()
#If False Then   'ADD 2020/06/05 先頭0埋めして18桁にする  2020/10/01 0埋めをやめる
        If String.IsNullOrEmpty(sStr) = False Then
            sStr = Right(String.Concat("00000000000000000", sStr), 18)
        End If
#End If

#If True Then   ' 商品コードから_以下を削除 20160517 changed inoue
        If (String.IsNullOrEmpty(sStr) = False AndAlso
            sStr.Split(CChar("_")).Length > 0) Then
            sStr = sStr.Split(CChar("_"))(0)
            dr.Item(GetColumnName(JJArrivalColumns.GOODS_CD)) = sStr
        End If
#End If
        ...
        sMsg = "商品名(カラム" & CType(JJArrivalColumns.GOODS_NM, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(JJArrivalColumns.GOODS_NM)).ToString().Trim()
        If sStr.Length > 80 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E445", New String() {String.Concat(sMsg, sStr, "]")}, sRecNo, LMH010BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        sMsg = "入荷個数量(カラム" & CType(JJArrivalColumns.PC, Int32) & "番目)["
        sNum = dr.Item(GetColumnName(JJArrivalColumns.PC)).ToString().Trim()
        If String.IsNullOrEmpty(sNum) = True Then
            '空の場合はゼロをセット
            dr.Item(GetColumnName(JJArrivalColumns.PC)) = 0
        Else
        ...
        sMsg = "ロット№(カラム" & CType(JJArrivalColumns.LOT, Int32) & "番目)["
        sStr = dr.Item(GetColumnName(JJArrivalColumns.LOT)).ToString().Trim()
...
#Region "商品明細マスタ"
    ...
    Public Function TorikomiMstChk(ByVal dr As DataRow, ByVal ds As DataSet) As Boolean
        ...
        sMsg = "荷主商品コード(カラム" & CType(JJArrivalColumns.GOODS_CD, Int32) & "番目)"
        sGoodsCd = dr.Item(GetColumnName(JJArrivalColumns.GOODS_CD)).ToString()      '荷主商品コード
        sGoodsNm = dr.Item(GetColumnName(JJArrivalColumns.GOODS_NM)).ToString()       '商品名
        If MyBase.GetResultCount() = 0 Then
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_JJ")         'EDI受信Dtl
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            ds.Tables("LMH010_INKAEDI_DTL_JJ").Clear() '受信DTLをクリア
            ds = Me.SetSemiInkaEdiRcv(ds, i)
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH010_INKAEDI_DTL_JJ").Rows(0)
            ...
            If Convert.ToDecimal(drEdiRcvDtl.Item("PC").ToString()) = 0.0 Then
                Continue For
            End If
            ...
            '別インスタンス
            Dim setDs As DataSet = ds.Copy()
            Dim setDtlDt As DataTable = setDs.Tables("LMH010_INKAEDI_DTL_JJ")
            setDtlDt.Clear()
