...
''' </summary>
''' <remarks></remarks>
Public Class LMH030BLC202
    Inherits Jp.Co.Nrs.LM.Base.BLC.LMBaseBLC

    '2012.02.28 大阪対応START
    '2012.02.28 ダウケミ専用START
#Region "Const"

#Region "届先(引き取り)コード(ダウケミ専用)パラメータ用CONST"
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC202 = New LMH030DAC202()
    ...
    Private Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        If autoMatomeF.Equals("0") OrElse autoMatomeF.Equals("1") Then
            ...
            ElseIf MyBase.GetResultCount > 1 Then
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.DOW_WID_L013, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    ds = Me._Blc.SetComWarningL("W199", LMH030BLC.DOW_WID_L013, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("0") = True Then
                ...
                choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.DOW_WID_L005, 0)
                ...
                If String.IsNullOrEmpty(choiceKb) = True Then
                    ...
                    msgArray(4) = String.Empty
                    matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                    ds = Me._Blc.SetComWarningL("W168", LMH030BLC.DOW_WID_L005, ds, msgArray, matomeNo, String.Empty)
                    Return ds
            ...
            ElseIf autoMatomeF.Equals("1") = True Then
                ...
                If matomeStatus.Equals("10") = False Then
                    ...
                    choiceKb = Me.SetDestWarningChoiceKb(ds.Tables("LMH030_OUTKAEDI_L"), ds, LMH030BLC.DOW_WID_L014, 0)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        msgArray(4) = String.Empty
                        matomeNo = ds.Tables("LMH030_MATOMESAKI_EDIL").Rows(0).Item("OUTKA_CTL_NO").ToString()
                        ds = Me._Blc.SetComWarningL("W198", LMH030BLC.DOW_WID_L014, ds, msgArray, matomeNo, String.Empty)
                        Return ds
...
#Region "実績作成処理"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        'ダウケミEDI実績の値設定(EDI受信TBLより)
        ds = MyBase.CallDAC(Me._Dac, "SelectDowEdiSend", ds)
        ...
        'ダウケミEDI実績データの更新
        ds = MyBase.CallDAC(Me._Dac, "InsertDowEdiSendData", ds)
...
#Region "EDI出荷(中)の初期値設定"
    ...
    Private Function EdiMDefaultSet(ByVal ds As DataSet, ByVal setDs As DataSet, _
        ...
        Else
            ...
            '2012.03.19 修正START
            choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.DOW_WID_M002, count)
            '2012.03.19 修正END
            ...
            If String.IsNullOrEmpty(choiceKb) = True Then
                ...
                '2012.03.19 修正START
                'ds = Me._Blc.SetComWarningM("W159", LMH030BLC.DOW_WID_M002, ds, setDs, msgArray, ediGOODSNm, mGOODSNm)
                ds = Me._Blc.SetComWarningM("W159", LMH030BLC.DOW_WID_M002, ds, setDs, msgArray, _
        ...
        '入目単位
        'ダウケミの場合、入目単位が異なる場合でもエラーまたはワーニングにしない(目欠対応)
        ...
        '        'チェックなし
        '    Else
        '        choiceKb = Me.SetGoodsWarningChoiceKb(dtEdi, ds, LMH030BLC.DOW_WID_M003, count)
        ...
        '            msgArray(4) = "EDIデータ"
        '            msgArray(5) = String.Empty
        '            ds = Me._Blc.SetComWarningM("W159", LMH030BLC.DOW_WID_M003, ds, setDs, msgArray, ediIRIMENm, mIRIMENm)
    ...
    Private Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            If String.IsNullOrEmpty(custGoodsCd) = False Then
                ...
                choiceKb = Me.SetGoodsWarningChoiceKb(setDtM, ds, LMH030BLC.DOW_WID_M001, 0)
                ...
                ElseIf GetResultCount() > 1 Then
                    ...
                    Else
                        ...
                        '2012.03.19 修正START
                        ds = Me._Blc.SetComWarningM("W162", LMH030BLC.DOW_WID_M001, ds, setDs, msgArray, custGoodsCd, String.Empty)
                        '2012.03.19 修正END
                ...
                If Me.EdiMDefaultSet(ds, setDs, i, unsoData, rowNo, ediCtlNo) = False Then
                    ...
                    '2012.03.01 大阪対応START
                    'ダウケミは現段階ではワーニングが存在するが、エラーはなし。共通のロジックを組み込む為入れておく
                    Dim chkRowNo As Integer = Convert.ToInt32(rowNo)
                    If MyBase.IsMessageStoreExist(chkRowNo) = True Then
                        '整合性チェックでエラーがあった場合は処理終了
                        Return False
...
#Region "届先マスタチェック(ダウケミ専用)"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        '2012.03.14 要望番号877 修正START
        'FREE_C21:届先住所(マスタ値が含まれていない場合はエラー)
        ...
        '    '届先マスタの住所1+2がEDI出荷(大)の届先住所(FREE_C21)に含まれていない場合はエラー
        '    If InStr(ediFreeC21.ToString().Trim(), mAdAll.ToString().Trim()) = 0 Then

        '        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E332", New String() {"届先住所", "届先マスタ", "住所1 + 住所2"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
        '        Return False
        ...
        Else
            ...
            '届先マスタの住所1+2がEDI出荷(大)の届先住所(FREE_C21)に含まれていない場合はエラー
            If InStr(ediDestAdAll.ToString().Trim(), mAdAll.ToString().Trim()) = 0 Then
...
#End Region
    ...
    '        choiceKb = Me.SetDestWarningChoiceKb(dtEdi, ds, LMH030BLC.DOW_WID_L012, 0)
    ...
    '            ds = Me._Blc.SetComWarningL("W186", LMH030BLC.DOW_WID_L012, ds, msgArray, workDestCd, String.Empty) '追加箇所 20110222
...
#Region "データセット設定(出荷L)"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            Else
                outkaDr("REMARK") = Me._Blc.LeftB(String.Concat(ediDr("REMARK"), Space(2), ediDr("UNSO_ATT")), 100)
            End If
            'ダウケミ特殊処理END
            '2012.02.28 大阪対応END
            ...
            '2012.02.28 大阪対応START
            'ダウケミ特殊処理START
            If (LMH030BLC202.EDI_DEST_CD_HIKITORI.ToString()).Equals _
               (ediDr("DEST_CD").ToString().Trim()) = True Then
                outkaDr("DEST_NM") = Me.LeftB(String.Concat(ediDr("DEST_NM"), Space(2), ediDr("FREE_C11")), 80)
            Else
...
#Region "データセット設定(運送L)"
    ...
    Private Function SetDatasetUnsoL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            'ダウケミの場合はEDI出荷(大)のFREE_C03(ＳＨ番号)をセットする
