...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH590DAC = New LMH590DAC()
...
#Region "Const"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_IN As String = "LMH590IN"

    ''' <summary>
    ''' OUTテーブル名
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_OUT As String = "LMH590OUT"
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = LMH590BLC.TABLE_NM_IN
        Dim tableNmOut As String = LMH590BLC.TABLE_NM_OUT
        Dim tableNmRpt As String = LMH590BLC.TABLE_NM_M_RPT
        ...
        If ds.Tables(tableNmIn).Rows.Count = 0 Then
            ...
            MyBase.SetMessage("G052", New String() {"EDI荷札"})
            MyBase.SetMessageStore(String.Empty, "G052", New String() {"EDI荷札"})
            Return ds
        End If
        ...
        For i As Integer = 0 To max
            ...
            For j As Integer = 0 To setDtOut.Rows.Count - 1
                ...
                For l As Integer = 0 To Convert.ToInt32(setDtOut.Rows(j).Item("OUTKA_PKG_NB").ToString()) - 1
        ...
        workDtRpt = view.ToTable(True, LMH590BLC.NRS_BR_CD, LMH590BLC.PTN_ID, LMH590BLC.PTN_CD, LMH590BLC.RPT_ID)
        ...
        For Each dr As DataRow In ds.Tables(tableNmRpt).Rows
            ...
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables(tableNmOut), dr.Item(LMH590BLC.RPT_ID).ToString())

            '帳票ごとの編集があるなら行う。
            prtDs = Me.EditPrintDataSet(dr.Item(LMH590BLC.RPT_ID).ToString(), prtDs)

            '帳票CSV出力
            comPrt.StartPrint(Me, _
                              dr.Item(LMH590BLC.NRS_BR_CD).ToString(), _
                              dr.Item(LMH590BLC.PTN_ID).ToString(), _
                              dr.Item(LMH590BLC.PTN_CD).ToString(), _
                              dr.Item(LMH590BLC.RPT_ID).ToString(), _
    ...
    Private Function SelectMPrt(ByVal ds As DataSet, ByVal intbl As DataTable) As DataSet
        ...
        ds = MyBase.CallDAC(Me._Dac, LMH590BLC.ACTION_ID_SELECT_MPRT, ds)

        'メッセージコードの設定ALL_PRINT
        Dim count As Integer = MyBase.GetResultCount()
        If count < 1 Then
            '0件の場合
            MyBase.SetMessage("G053", New String() {"　【EDI荷札】　"})

            If prtFlg.Equals("1") = True Then
                '出力済の場合、エラーメッセージはExcel出力
                MyBase.SetMessageStore("00", "G053", New String() {"　【EDI荷札】　"}, rowNo, "EDI管理番号", ediCtlNo)
            Else
                If outType.Equals(ALL_PRINT) = True Then
                    '一括印刷時、エラーメッセージはExcel出力
                    MyBase.SetMessageStore("00", "G053", New String() {"　【EDI荷札】　"}, rowNo, "帳票名", "EDI納品書 日興ｲｴﾛｰﾊｯﾄ")
                End If
            End If
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        Return MyBase.CallDAC(Me._Dac, LMH590BLC.ACTION_ID_SELECT_PRT_DATA, ds)
