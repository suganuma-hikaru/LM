...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected _Dac As Jp.Co.Nrs.LM.Base.DAC.LMBaseDAC = New LMH030DAC210()

    ''' <summary>
    ''' 使用するDACクラスの生成(共通DAC)
    ''' </summary>
    ''' <remarks></remarks>
    Protected _DacCom As LMH030DAC = New LMH030DAC()

    ''' <summary>
    ''' 使用するBLC共通クラスの生成
    ''' </summary>
    ''' <remarks></remarks>
    Protected _Blc As LMH030BLC = New LMH030BLC()

    ''' <summary>
    ''' 倉庫/荷主L/荷主M
    ''' </summary>
    ''' <remarks></remarks>
    Protected _WhCd As String = String.Empty            '倉庫コード
    Protected _CustCdL As String = String.Empty         '荷主コード（大）
    Protected _CustCdM As String = String.Empty         '荷主コード（中）

    Protected _RtnFlg As Boolean = False                  '返品フラグ

    Protected _DevideLen1 As Integer = 0                 'キー条件1　桁数
    Protected _DevideLen2 As Integer = 0                 'キー条件2　桁数
...
#Region "出荷登録処理"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function OutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        ''2015.06.25 協立化学はオーダー明細までで重複チェックを行う 
        'If Me.CustOrderNoCheckDtl(ds, rowNo, ediCtlNo) = False Then
        '    Return ds
        'End If
        ''2015.06.25 協立化学はオーダー明細までで重複チェックを行う 
        ...
        Dim hasuShoriFlg As Boolean = False

        Dim hasuDr As DataRow = ds.Tables("LMH030_HASUSHORI").NewRow()
        hasuDr("RTN_FLG") = "0"
        ds.Tables("LMH030_HASUSHORI").Rows.Add(hasuDr)

        If _RtnFlg = True Then
            hasuDr("RTN_FLG") = "1"
        End If
        ...
        '出荷(大)データセット設定
        ds = Me.SetDatasetOutkaL(ds, matomeFlg, hasuShoriFlg)

        '出荷(中)データセット設定
        ds = Me.SetDatasetOutkaM(ds, matomeFlg, hasuShoriFlg)
        ...
        ''作業レコードデータセット設定
        ds = Me.SetDatasetSagyo(ds)         'UPD 復活 006202   【LMS】商品に出荷時作業加工区分が登録されている商品のEDI出荷登録できない荷主有り調査、改修依頼

        '運送(大,中)データセット設定
        ds = Me.SetDatasetUnsoL(ds, matomeFlg, hasuShoriFlg)
        ds = Me.SetDatasetUnsoM(ds, hasuShoriFlg)
...
#Region "出荷登録処理(端数返品分)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function OutkaTorokuHasu(ByVal ds As DataSet) As DataSet
        ...
        Dim hasuShoriFlg As Boolean = True
        ...
        ''作業レコードデータセット設定
        ds = Me.SetDatasetSagyo(ds)         'UPD 2019/08/02 復活　006202   【LMS】商品に出荷時作業加工区分が登録されている商品のEDI出荷登録できない荷主有り調査、改修依頼
...
#Region "EDI出荷(大)の初期値設定(出荷登録処理)"
    ...
    Protected Function SetEdiLShoki(ByVal ds As DataSet) As DataSet
    ...
    Protected Function SetDestCd(ByVal ds As DataSet) As DataSet
...
#Region "EDI出荷(中)の初期値設定(出荷登録処理)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function EdiMDefaultSet(ByVal ds As DataSet, ByVal setDs As DataSet, _
        ...
        ds = MyBase.CallDAC(Me._Dac, "SelectGoodsDetailsSub48", ds)


        If outkaHasu Mod pkgNb > 0 AndAlso MyBase.GetResultCount > 0 Then
            _RtnFlg = True
            'ediMDr("OUTKA_HASU") = outkaHasu - outkaHasu Mod pkgNb
            ediMDr("FREE_N02") = pkgNb - (outkaHasu Mod pkgNb)
        End If
...
#Region "EDI出荷(大)のBLC側でのチェック"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function EdiLKanrenCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
...
#Region "EDI出荷(大)のDAC側でのチェック"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function EdiLDbExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        ElseIf mDestCount = 0 Then
            '0件の場合、届け先マスタエラーとする
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E078", New String() {"届先マスタ"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            Return False
...
#Region "EDI出荷(中)のBLC側でのチェック"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function EdiMKanrenCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
...
#Region "EDI出荷(中)のDAC側でのチェック + 初期値設定"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function EdiMMasterExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
...
#Region "届先マスタ追加時チェック"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function ZipCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String, ByVal workDestCd As String, ByVal workDestString As String) As Boolean
...
#Region "届先マスタチェック"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        '2015.06.29 協立化学は届け先の比較しないSTART
        ...
        '        ds = Me._Blc.SetComWarningL("W187", LMH030BLC.STD_WID_L005, ds, msgArray, ediDestNm, mDestNm)
        ...
        '            ds = Me._Blc.SetComWarningL("W187", LMH030BLC.STD_WID_L006, ds, msgArray, ediDestAdAll, mAdAll)
        ...
        '            ds = Me._Blc.SetComWarningL("W187", LMH030BLC.STD_WID_L007, ds, msgArray, ediZip, mZip)
        ...
        '            ds = Me._Blc.SetComWarningL("W187", LMH030BLC.STD_WID_L008, ds, msgArray, ediTel, mTel)
        ...
        '            ds = Me._Blc.SetComWarningL("W187", LMH030BLC.STD_WID_L009, ds, msgArray, ediDestJisCd, mJis)
...
#Region "SPACE除去 + 文字変換"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SpaceCutChk(ByVal chkFld As String) As String
...
#Region "Null変換"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function NullConvertString(ByVal value As Object) As Object
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function NullConvertZero(ByVal value As Object) As Object
...
#Region "左埋処理"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function FormatZero(ByVal val As String, ByVal keta As Integer) As String
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function FormatSpace(ByVal val As String, ByVal keta As Integer) As String
...
#Region "ワーニング処理(EDI(大)届先)選択区分の取得"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetDestWarningChoiceKb(ByVal setDt As DataTable, ByVal ds As DataSet, _
...
#Region "ワーニング処理(EDI(中)商品)選択区分の取得"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetGoodsWarningChoiceKb(ByRef setDt As DataTable, ByVal ds As DataSet, _
                                           ByVal warningId As String, ByVal count As Integer) As String
...
#Region "データセット設定(出荷管理番号L)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function GetOutkaNoL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
...
#Region "データセット設定(出荷管理番号M)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function GetOutkaNoM(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
...
#Region "データセット設定(出荷L)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean, ByVal hasuShoriFlg As Boolean) As DataSet

        Dim ediDr As DataRow = ds.Tables("LMH030_OUTKAEDI_L").Rows(0)

        If hasuShoriFlg = True Then
        ...
        If matomeFlg = False Then
            ...
            If hasuShoriFlg = True Then
                ...
                ds = MyBase.CallDAC(Me._Dac, "SelectGoodsDetailsSub48", ds)
                If MyBase.GetResultCount > 0 Then
                    ediDr("DEST_CD") = ds.Tables("LMH030_M_GOODS_DETAILS").Rows(0).Item("SET_NAIYO").ToString()
                    ds = MyBase.CallDAC(Me._DacCom, "SelectDataMdest", ds)
                    outkaDr("CUST_ORD_NO") = String.Empty
                    '2015.10.20 協立化学　返品データ修正START
                    outkaDr("BUYER_ORD_NO") = String.Empty
                    '2015.10.20 協立化学　返品データ修正END
                End If
            Else
            ...
            If ds.Tables("LMH030_M_DEST").Rows.Count > 0 Then
                ...
                '2015.10.20 協立化学　返品データ修正START
                If hasuShoriFlg = True Then
                    ediDr("UNSO_CD") = destMDr("SP_UNSO_CD")
                    ediDr("UNSO_BR_CD") = destMDr("SP_UNSO_BR_CD")
                    '運賃タリフフラグの初期化
                    ediDr("UNCHIN_TARIFF_FLG") = String.Empty
                    ediDr("UNCHIN_TARIFF_CD") = String.Empty
                    'タリフセットマスタの取得(運賃タリフ)
                    ds = MyBase.CallDAC(Me._DacCom, "SetTariffData", ds)

                    '割増タリフフラグの初期化
                    ediDr("EXTC_TARIFF_FLG") = String.Empty
                    ediDr("EXTC_TARIFF_CD") = String.Empty
                    'タリフセットマスタの取得(割増タリフ)
                    ds = MyBase.CallDAC(Me._DacCom, "SetExtcTariffData", ds)
                End If
                '2015.10.20 協立化学　返品データ修正END
            ...
            Else
                outkaDr("OUTKA_PKG_NB") = SumPkgNbKrt(dt, ds)
            End If
        ...
        Else
            ...
            outkaDr("NRS_BR_CD") = ediDr("NRS_BR_CD")
            outkaDr("OUTKA_NO_L") = ediDr("OUTKA_CTL_NO")
            outkaDr("OUTKA_PKG_NB") = SumPkgNbKrt(dt, ds) + Convert.ToDouble(matomesakiDt.Rows(0)("OUTKA_PKG_NB"))
...
#Region "データセット設定(協立化学専用：出荷包装個数計算)"
    Protected Function SumPkgNbKrt(ByVal dt As DataTable, ByVal ds As DataSet) As Double
        ...
        Dim ordTtlNb As Integer = 0
        Dim preNrsGoodsCd As String = String.Empty
        Dim preLotNo As String = String.Empty
        Dim pltPkg As Integer = 0
        Dim goodsHt As Hashtable = New Hashtable
        Dim endFlg As Boolean = False
        Dim goodsDr As DataRow
        Dim count As Integer = 0

        For Each row As DataRow In dt.Select(String.Empty, "NRS_GOODS_CD ASC ")

            If preNrsGoodsCd.Equals(row("NRS_GOODS_CD").ToString()) Then
                ordTtlNb = ordTtlNb + Convert.ToInt32(row.Item("OUTKA_HASU"))
                endFlg = True
            ElseIf String.IsNullOrEmpty(preNrsGoodsCd) = True Then
                ordTtlNb = Convert.ToInt32(row.Item("OUTKA_HASU"))
                endFlg = True
            Else
                count = count + 1
                goodsHt.Add(count, String.Concat(preNrsGoodsCd, "-", pltPkg, "-", ordTtlNb))
                ordTtlNb = Convert.ToInt32(row.Item("OUTKA_HASU"))
            End If
            preNrsGoodsCd = row("NRS_GOODS_CD").ToString()
            preLotNo = row("LOT_NO").ToString()
            pltPkg = Convert.ToInt32(row("FREE_N01").ToString())
        Next
        ...
        For i As Integer = 0 To goodsHt.Count - 1

            ds.Tables("LMH030_M_GOODS_DETAILS").Clear()
            goodsDr = ds.Tables("LMH030_M_GOODS_DETAILS").NewRow()

            tempNo = Convert.ToString(goodsHt(i + 1)).Split("-"c)

            goodsDr("GOODS_CD_NRS") = tempNo(0)
            goodsDr("NRS_BR_CD") = dt.Rows(0).Item("NRS_BR_CD").ToString()

            'データセットに設定
            ds.Tables("LMH030_M_GOODS_DETAILS").Rows.Add(goodsDr)

            ds = MyBase.CallDAC(Me._Dac, "SelectGoodsDetailsSub49", ds)

            If String.IsNullOrEmpty(ds.Tables("LMH030_M_GOODS_DETAILS").Rows(0).Item("SET_NAIYO").ToString()) = False Then

                If Convert.ToInt32(tempNo(2)) Mod Convert.ToInt32(tempNo(1)) = 0 Then
                    sumNb = sumNb + Convert.ToInt32(tempNo(2)) / Convert.ToInt32(tempNo(1))
                Else
                    ''2015.07.07 協立化学　梱包数対応 修正START
                    'sumNb = sumNb + (Math.Ceiling(Convert.ToInt32(tempNo(2)) / Convert.ToInt32(tempNo(1))) + Convert.ToInt32(tempNo(2)) Mod Convert.ToInt32(ds.Tables("LMH030_M_GOODS_DETAILS").Rows(0).Item("SET_NAIYO").ToString()))
                    sumNb = sumNb + (Math.Floor(Convert.ToInt32(tempNo(2)) / Convert.ToInt32(tempNo(1))) + Convert.ToInt32(tempNo(2)) Mod Convert.ToInt32(ds.Tables("LMH030_M_GOODS_DETAILS").Rows(0).Item("SET_NAIYO").ToString()))
                    ''2015.07.07 協立化学　梱包数対応 修正END
                End If
...
#Region "データセット設定(出荷包装個数)"
    Protected Function SumPkgNb(ByVal dt As DataTable) As Double
...
#Region "データセット設定(出荷M)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetDatasetOutkaM(ByVal ds As DataSet, ByVal matomeflg As Boolean, ByVal hasuShoriFlg As Boolean) As DataSet
        '要望番号:1712 umano 2013.01.11 START
        'Protected Function SetDatasetOutkaM(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            If hasuShoriFlg = True AndAlso _
                Convert.ToInt32(dt.Rows(i)("OUTKA_TTL_NB").ToString()) Mod Convert.ToInt32(dt.Rows(i)("PKG_NB").ToString()) = 0 Then
                ds.Tables("LMH030_C_OUTKA_M").Clear()
                Continue For
            ElseIf hasuShoriFlg = True AndAlso i = 0 Then
                ds.Tables("LMH030_C_OUTKA_M").Clear()
            End If
            ...
            If hasuShoriFlg = True Then
                outkaDr("OUTKA_QT") = Convert.ToInt32(ediDr("FREE_N02")) * Convert.ToDecimal(ediDr("IRIME"))
                outkaDr("OUTKA_TTL_QT") = Convert.ToInt32(ediDr("FREE_N02")) * Convert.ToDecimal(ediDr("IRIME"))
                outkaDr("BACKLOG_NB") = Convert.ToInt32(ediDr("FREE_N02"))
                outkaDr("BACKLOG_QT") = Convert.ToInt32(ediDr("FREE_N02")) * Convert.ToDecimal(ediDr("IRIME"))
                outkaDr("OUTKA_TTL_NB") = Convert.ToInt32(ediDr("FREE_N02"))
                outkaDr("OUTKA_HASU") = Convert.ToInt32(ediDr("FREE_N02"))
                outkaDr("CUST_ORD_NO_DTL") = String.Empty
            Else
...
#Region "データセット設定(EDI受信DTL)[使用していません]"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetDatasetEdiRcvDtl(ByVal ds As DataSet) As DataSet
...
#Region "データセット設定(作業)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetDatasetSagyo(ByVal ds As DataSet) As DataSet
...
#Region "データセット設定(運送L)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetDatasetUnsoL(ByVal ds As DataSet, ByVal matomeFlg As Boolean, ByVal hasuShoriFlg As Boolean) As DataSet
...
#Region "データセット設定(運送M)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetDatasetUnsoM(ByVal ds As DataSet, ByVal hasuShoriFlg As Boolean) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            If hasuShoriFlg = True Then
                unsoMDr("UNSO_TTL_QT") = calcPkgModNb * Convert.ToDecimal(ediDr("IRIME"))
                unsoMDr("UNSO_TTL_NB") = calcPkgModNb
                unsoMDr("HASU") = calcPkgModNb
            Else
...
#Region "データセット設定(運送L：運送重量)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetdatasetUnsoJyuryo(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetCalcJyuryo(ByVal ds As DataSet, ByVal tblNm As String) As Decimal
...
#Region "データセット設定(EDI出荷M：運送重量必要項目)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetDatasetEdiMUnsoJyuryo(ByVal ds As DataSet, ByVal setDs As DataSet, ByVal count As Integer, _
                                              ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
...
#Region "まとめ先複数件の時出荷管理番号取得"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function matomesakiOutkaNo(ByVal ds As DataSet) As String
...
#Region "出荷登録処理(運賃作成)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function UnchinSakusei(ByVal ds As DataSet) As DataSet
...
#Region "紐付け処理[使用していません]"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function Himoduke(ByVal ds As DataSet) As DataSet
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetHimodukeFlg(ByVal ds As DataSet) As DataSet
...
#Region "出荷登録処理(ｹｰｽﾏｰｸ情報作成)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function UpdateSetIniCaseMark(ByVal ds As DataSet) As DataSet

        'ﾍｯﾄﾞ設定
        Me.SelectCaseHeadDef(ds)

        '明細設定
        If ds.Tables("LMH030_C_MARK_HED").Rows.Count > 0 Then
            Me.SelectCaseDtlDef(ds)
        End If

        'ｹｰｽﾏｰｸHEDの新規登録
        If ds.Tables("LMH030_C_MARK_HED").Rows.Count > 0 Then
            ds = MyBase.CallDAC(Me._Dac, "InsertMarkHedData", ds)
        End If

        'ｹｰｽﾏｰｸDTLの新規登録
        If ds.Tables("LMH030_C_MARK_HED").Rows.Count > 0 Then
            ds = MyBase.CallDAC(Me._Dac, "InsertMarkDtlData", ds)
        End If
    ...
    ''' <summary>
    ''' ｹｰｽﾏｰｸの初期値設定(データベースより抽出されなかった場合実行)
    ''' </summary>
    ''' <param name="ds"></param>
    ''' <remarks></remarks>
    Protected Sub SelectCaseHeadDef(ByRef ds As DataSet)

        'ｹｰｽﾏｰｸﾍｯﾄﾞTBL(本物)
        Dim caseHedDt As DataTable = ds.Tables("LMH030_C_MARK_HED")
        Dim newRow As DataRow = Nothing

        '出荷MTBL(複製)
        Dim copyM As DataSet = ds.Copy()
        Dim copyMTbl As DataTable = copyM.Tables("LMH030_C_OUTKA_M")
        Dim copyMView As DataView = copyMTbl.DefaultView()
        copyMView.Sort = "GOODS_CD_NRS ASC"
        Dim sumNb As Integer = 0

        '纏め先出荷M
        Dim groupM As DataSet = ds.Clone()
        Dim groupMTbl As DataTable = groupM.Tables("LMH030_C_OUTKA_M")
        Dim groupMRow As DataRow = Nothing

        '切替り比較用商品キー
        Dim preGoodsCdNrs As String = String.Empty

        '各種データ抽出用InDs
        Dim gdDs As DataSet = ds.Clone()
        Dim gdMDt As DataTable = gdDs.Tables("LMH030_C_OUTKA_M")
        Dim gdDtRow As DataRow() = Nothing

        '行生成
        newRow = caseHedDt.NewRow()

        '出荷中の最小値を取得
        Dim minOutMRow As DataRow() = copyMTbl.Select(String.Concat("OUTKA_NO_M = MIN(OUTKA_NO_M)"))

        '個数
        Dim alctdNb As Integer = 0

        'ケース№To
        Dim caseNoTo As Integer = 0

        '出荷Mの最小番号をもとにHED作成
        If minOutMRow IsNot Nothing AndAlso minOutMRow.Count > 0 Then

            'ケース№の設定
            newRow.Item("NRS_BR_CD") = minOutMRow(0).Item("NRS_BR_CD")
            newRow.Item("OUTKA_NO_L") = minOutMRow(0).Item("OUTKA_NO_L")
            newRow.Item("OUTKA_NO_M") = minOutMRow(0).Item("OUTKA_NO_M")
            newRow.Item("SYS_DEL_FLG") = LMConst.FLG.OFF

            '出荷Mをまとめる(GOOD_CD_NRS毎)
            Dim cpMrowCnt As Integer = copyMTbl.Rows.Count - 1
            For i As Integer = 0 To cpMrowCnt

                '新規行の生成
                If String.IsNullOrEmpty(preGoodsCdNrs) OrElse (preGoodsCdNrs.Equals(copyMTbl.Rows(i).Item("GOODS_CD_NRS").ToString()) = False) Then

                    sumNb = 0

                    groupMRow = groupMTbl.NewRow()
                    groupMRow.Item("NRS_BR_CD") = copyMTbl.Rows(i).Item("NRS_BR_CD").ToString()
                    groupMRow.Item("GOODS_CD_NRS") = copyMTbl.Rows(i).Item("GOODS_CD_NRS").ToString()

                End If

                sumNb = sumNb + Convert.ToInt32(copyMTbl.Rows(i).Item("OUTKA_TTL_NB"))
                preGoodsCdNrs = copyMTbl.Rows(i).Item("GOODS_CD_NRS").ToString()

                '総個数の設定と行格納
                If i = cpMrowCnt OrElse (preGoodsCdNrs.Equals(copyMTbl.Rows(i).Item("GOODS_CD_NRS").ToString()) = False) Then

                    groupMRow.Item("OUTKA_TTL_NB") = sumNb
                    groupMTbl.Rows.Add(groupMRow)
            ...
            'ケース№設定
            preGoodsCdNrs = String.Empty
            For i As Integer = 0 To groupMTbl.Rows.Count - 1

                '商品のサイズと重量取得
                If String.IsNullOrEmpty(preGoodsCdNrs) OrElse (preGoodsCdNrs.Equals(groupMTbl.Rows(i).ToString()) = False) Then
                    gdMDt.ImportRow(groupMTbl.Rows(i))
                    '検索結果取得
                    gdDs = MyBase.CallDAC(Me._Dac, "SelectGoodsDetails", gdDs)
                End If

                '個数設定
                alctdNb = Convert.ToInt32(groupMTbl.Rows(i).Item("OUTKA_TTL_NB").ToString())

                Do While alctdNb > 0

                    gdDtRow = gdDs.Tables("LMH030_GOODS_SIZE_WT").Select(String.Concat("IRISU = '", alctdNb, "'"), " GOODS_CD_NRS_EDA DESC")

                    If gdDtRow.Count = 0 Then
                        ...
                        If gdDtRow.Count > 0 Then
                            ...
                            '個数マイナス(規定入数分)
                            alctdNb = alctdNb - Convert.ToInt32(gdDtRow(0).Item("IRISU"))
                    ...
                    preGoodsCdNrs = groupMTbl.Rows(i).Item("GOODS_CD_NRS").ToString()
                Loop
            ...
            '同一得意先発注番号が存在する場合はケース№を過去の最大値よりカウントアップする(しない場合は１よりカウントアップ)
            ds = MyBase.CallDAC(Me._Dac, "GetCaseToMaxNo", ds)

            If String.IsNullOrEmpty(ds.Tables("LMH030_Z_KBN").Rows(0).Item("NIS_CNT").ToString()) = True Then
                newRow.Item("CASE_NO_FROM") = 1
                newRow.Item("CASE_NO_TO") = caseNoTo
            Else
                caseNoFrom = Convert.ToInt32(ds.Tables("LMH030_Z_KBN").Rows(0).Item("NIS_CNT").ToString())
                newRow.Item("CASE_NO_FROM") = caseNoFrom + 1
                newRow.Item("CASE_NO_TO") = caseNoFrom + caseNoTo
            End If

            caseHedDt.Rows.Add(newRow)
    ...
    ''' <param name="ds"></param>
    ''' <remarks></remarks>
    Protected Sub SelectCaseDtlDef(ByRef ds As DataSet)

        'ｹｰｽﾏｰｸ明細 TBL(本物)
        Dim caseDtlDt As DataTable = ds.Tables("LMH030_C_MARK_DTL")
        Dim newRow As DataRow = Nothing

        '出荷LTBL(参照用)
        Dim outLTbl As DataTable = ds.Tables("LMH030_C_OUTKA_L")
        ...
        '検索用Ds
        Dim inOutDs As DataSet = ds.Clone()
        Dim inOutDt As DataTable = inOutDs.Tables("LMH030_C_OUTKA_M")
        ...
        If minOutMRow IsNot Nothing AndAlso minOutMRow.Count > 0 Then
            ...
            '必要情報の取得
            inOutDt.ImportRow(minOutMRow(0))
            inOutDs = MyBase.CallDAC(Me._Dac, "GetCaseInfoGoodsNm", inOutDs)

            '1行目設定(商品名)
            newRow = caseDtlDt.NewRow()
            '共通パラメータの設定
            Me.SetCaseDtlPublicPara(newRow, minOutMRow(0))
            '固有パラメータの設定
            newRow.Item("MARK_EDA") = "001"
            newRow.Item("REMARK_INFO") = inOutDt.Rows(0).Item("GOODS_NM").ToString()
            caseDtlDt.Rows.Add(newRow)
            ...
            Me.SetCaseDtlPublicPara(newRow, minOutMRow(0))
            '固有パラメータの設定
            newRow.Item("MARK_EDA") = "002"
            newRow.Item("REMARK_INFO") = outLTbl.Rows(0).Item("BUYER_ORD_NO").ToString()
            caseDtlDt.Rows.Add(newRow)
            ...
            Me.SetCaseDtlPublicPara(newRow, minOutMRow(0))
            '固有パラメータの設定
            newRow.Item("MARK_EDA") = "004"
            newRow.Item("REMARK_INFO") = "MADE IN JAPAN"
            caseDtlDt.Rows.Add(newRow)
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub SetCaseDtlPublicPara(ByRef newRow As DataRow, ByVal getRow As DataRow)

        '共通パラメータの設定(明細)
        newRow.Item("NRS_BR_CD") = getRow.Item("NRS_BR_CD")
        newRow.Item("OUTKA_NO_L") = getRow.Item("OUTKA_NO_L")
        newRow.Item("OUTKA_NO_M") = getRow.Item("OUTKA_NO_M")
        newRow.Item("SYS_DEL_FLG") = LMConst.FLG.OFF
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            '2015.06.29 検証一覧№8 修正START
            drOutkaEdiL("CUST_ORD_NO") = String.Concat(Left(drRcvEdiDtl(sTempColNo).ToString().Trim(), _DevideLen1), "-", Right(drRcvEdiDtl(sTempColNo).ToString().Trim(), _DevideLen2 - _DevideLen1))
            '2015.06.29 検証一覧№8 修正END
        End If
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
                                    , ByVal rowNo As Integer _
                                    , ByRef sEdiCtlNo As String _
                                    , ByRef iEdiCtlNoChu As Integer _
                                    , ByVal iFindOutkaEdiFlg As Integer _
...
#Region "セミEDI時  データセット設定(EDI管理番号(大・中))"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function GetEdiCtlNo(ByVal ds As DataSet _
...
#Region "セミEDI時  データセット設定(商品マスタ)"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub SetInDataSelectGoodsMst(ByRef setDs As DataSet, ByVal rowCnt As Integer)
...
#Region "セミEDI時  データセット設定(届先マスタ)"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub SetInDataSelectDestMst(ByRef setDs As DataSet, ByVal rowCnt As Integer)
...
#Region "セミEDI時 データセット設定(キャンセル対象データ抽出)"
    ...
    ''' <remarks></remarks>
    Protected Sub SetDatasetSelectCancelData(ByRef setDs As DataSet, ByVal RowNo As Integer)
        ...
        If isNullColKey(drSemiInfo, "L_DEST_CUST_ORD_NO", tempColNo) Then
            ...
            setInOutDr("CUST_ORD_NO") = String.Concat(Left(drItems.Item(tempColNo).ToString().Trim(), _DevideLen1), "-", Right(drItems.Item(tempColNo).ToString().Trim(), _DevideLen2 - _DevideLen1))
        End If
...
#Region "セミEDI時　商品マスタからCustCd等を取得する"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function GetCustCd(ByVal ds As DataSet _
...
#Region "画面取込(セミEDI)チェック処理"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SemiEdiTorikomiChk(ByVal ds As DataSet) As DataSet
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow, ByVal columnDt As DataTable, ByVal dtMcustD As DataTable) As Boolean
        ...
        If isNullColKey(columnDr, sKey, sTempColNo) <> False Then
            ...
            If sTempColNo <> "COLUMN_MST" Then
                ...
                If sTempColNo.Trim().Split(","c).Length > 0 Then
                    Dim tempNo As String() = sTempColNo.Trim().Split(","c)
                    For i As Integer = 0 To sTempColNo.Trim().Split(","c).Length - 1
                        If InStr(tempNo(i), "COLUMN_") = 0 Then
                            tempNo(i) = String.Concat("COLUMN_", tempNo(i))
                        End If
                        validVal = String.Concat(validVal, Space(1), dr.Item(tempNo(i)).ToString().Trim())
                    Next
                Else
                    '- ﾁｪｯｸ実行 -
                    validVal = dr.Item(sTempColNo).ToString().Trim()
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            If isNullColKey(drSemiInfo, sKey, sTempColNo) <> False Then
                '2015.06.29 検証一覧№8 追加START
                _DevideLen1 = System.Text.Encoding.GetEncoding(932).GetByteCount(drSetDtl.Item(sTempColNo).ToString().Trim())
                '2015.06.29 検証一覧№8 追加END
            ...
            If isNullColKey(drSemiInfo, sKey, sTempColNo) <> False Then
                '2015.06.29 検証一覧№8 追加START
                _DevideLen2 = System.Text.Encoding.GetEncoding(932).GetByteCount(drSetDtl.Item(sTempColNo).ToString().Trim())
                '2015.06.29 検証一覧№8 追加END
            ...
            '---------------------------------------------------------------------------
            Dim tempColNo As String = String.Empty
            'Dim tempColNoBk As String = String.Empty

            ''2015.10.21↓一旦コメントアウトSTART(保留ロジック解除)
            ''2015.10.20 協立化学　保留データ対応START
            'Dim tempDelColNo As String = String.Empty
            ''2015.10.20 協立化学　保留データ対応END
            ''2015.10.21↑一旦コメントアウトEND(保留ロジック解除)
            ...
            If bSameKeyFlg = False AndAlso drSemiInfo.Item("EDI_TORIKESI_FLG").ToString().Equals("1") = True Then
                ...
                If isNullColKey(drSemiInfo, "L_DEST_CUST_ORD_NO", tempColNo) = True Then
                    'tempColNoBk = tempColNo
                    'If isNullColKey(drSemiInfo, "M_CUST_ORD_NO_DTL_NO", tempColNo) = True Then
                    '    setDelDs.Tables("LMH030INOUT").Rows(0).Item("CUST_ORD_NO") = String.Concat(setDelDs.Tables("LMH030INOUT").Rows(0).Item("CUST_ORD_NO"), _
                    '                                                                               setDelDs.Tables("LMH030_EDI_TORIKOMI_DTL").Rows(i).Item(tempColNo).ToString().Trim())
                    'End If
                End If

                MyBase.CallDAC(Me._Dac, "SelectCancelData", setDelDs)

                'If MyBase.GetResultCount <> 0 AndAlso MyBase.GetResultCount <> ds.Tables("LMH030_EDI_TORIKOMI_DTL").Select(String.Concat(tempColNoBk, "+", tempColNo, "= '", setDelDs.Tables("LMH030_EDI_TORIKOMI_DTL").Rows(i).Item(tempColNoBk).ToString().Trim(), setDelDs.Tables("LMH030_EDI_TORIKOMI_DTL").Rows(i).Item(tempColNo).ToString().Trim(), "'")).Length Then
                '    iFindOutkaEdiFlg = 1
                '    itargetUpdFlg = 1
                '    iCntDiffFlg = 1
                'End If

                Dim len As Integer = System.Text.Encoding.GetEncoding(932).GetByteCount(setDelDs.Tables("LMH030_EDI_TORIKOMI_DTL").Rows(i).Item(tempColNo).ToString().Trim())

                ''2015.10.21↓一旦コメントアウトSTART(保留ロジック解除)
                ''2015.10.20 協立化学　保留データ対応START
                'If isNullColKey(drSemiInfo, "M_DEL_KB_NO", tempDelColNo) = True Then

                'End If
                ''2015.10.21↑一旦コメントアウトEND(保留ロジック解除)

                ' ''2015.10.21↓一旦コメントアウトSTART(保留ロジック解除)
                'If MyBase.GetResultCount <> 0 AndAlso MyBase.GetResultCount <> ds.Tables("LMH030_EDI_TORIKOMI_DTL").Select(String.Concat(tempColNo, " LIKE '", Left(setDelDs.Tables("LMH030_EDI_TORIKOMI_DTL").Rows(i).Item(tempColNo).ToString().Trim(), len - 1), "%", _
                '                                                                                                                         "' AND ", tempDelColNo, " = ''")).Length Then
                '    ''2015.10.21↑一旦コメントアウトEND(保留ロジック解除)

                ' ''2015.10.21↓復活START
                If MyBase.GetResultCount <> 0 AndAlso MyBase.GetResultCount <> ds.Tables("LMH030_EDI_TORIKOMI_DTL").Select(String.Concat(tempColNo, " LIKE '", Left(setDelDs.Tables("LMH030_EDI_TORIKOMI_DTL").Rows(i).Item(tempColNo).ToString().Trim(), len - 1), "%'")).Length Then
                    '2015.10.20 協立化学　保留データ対応END
                    ' ''2015.10.21↑復活END
            ...
            If MyBase.GetResultCount > 0 Then
                If String.IsNullOrEmpty(setDelDs.Tables("LMH030OUT").Rows(0).Item("OUTKA_CTL_NO").ToString()) Then
                    ...
                    drSetDel.Item("EDI_CTL_NO") = setDelDs.Tables("LMH030OUT").Rows(0).Item("EDI_CTL_NO")
            ...
            '2015.06.11 追加START 同一出荷日＋届け先の追加データ不具合
            If (bSameKeyFlg = False AndAlso iSkipFlg = 0) OrElse _
                (bSameKeyFlg = True AndAlso (iCntDiffFlg = 1 OrElse itargetUpdFlg = 1)) OrElse _
                (bSameKeyFlg = True AndAlso itargetUpdFlg = 0) Then
                '2015.06.11 追加END 同一出荷日＋届け先の追加データ不具合
            ...
            If itargetUpdFlg = 1 Then
                ...
                End If
                '2015.06.29 追加START 同一オーダー追加データ不具合
            ElseIf bSameKeyFlg = True AndAlso itargetUpdFlg = 0 Then
                'EDI出荷(大)データを削除⇒正常に戻す
                setDs = MyBase.CallDAC(Me._Dac, "UpdateRevivalOutkaEdiL", setDs)
                '2015.06.29 追加END 同一オーダー追加データ不具合

                ''2015.10.21↓一旦コメントアウトSTART(保留ロジック解除)
                '2015.10.20 協立化学　保留データ対応START
                ''iOutHedUpdCnt = MyBase.GetResultCount
                ''If iOutHedUpdCnt > 0 Then
                'setDs = MyBase.CallDAC(Me._Dac, "UpdateDeleteOutkaEdiL", setDs)
                ''End If
                '2015.10.20 協立化学　保留データ対応END
                ''2015.10.21↑一旦コメントアウトEND(保留ロジック解除)
...
#Region "ユーティリティ"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function isNullColKey(ByVal columnDr As DataRow, ByVal getColKey As String, ByRef setColKey As String) As Boolean
    ...
    Protected Function GetBussinessDayKrt(ByVal sStartDay As String, ByVal iBussinessDays As Integer, ByVal setDs As DataSet) As DateTime
        ...
        For i As Integer = 1 To System.Math.Abs(iBussinessDays)  'マイナス値に対応するため絶対値指定
            ...
            Do
                ...
                If Weekday(dBussinessDate) = 1 Then
                    dBussinessDate = dBussinessDate.AddDays(-1)     '前営業日
                End If
                '土日でない場合
...
#Region "荷主明細マスタ取得"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function SelectMstCustAll(ByVal ds As DataSet) As DataSet
        '---------------------------------------------------------------------------
        ' 該当する荷主明細を全取得
        '---------------------------------------------------------------------------
...
#End Region
    ...
    '2015.06.25 協立化学　追加START
#Region "オーダーNO重複チェック(荷主注文番号＋荷主注文番号明細)"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function CustOrderNoCheckDtl(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To edimCount
            ...
            If String.IsNullOrEmpty(drEdiL.Item("CUST_ORD_NO").ToString()) = False Then
                If drIn("ORDER_CHECK_FLG").Equals("1") = True Then
                    Call MyBase.CallDAC(Me._Dac, "SelectOrderCheckDataDtl", setDs)
                    If MyBase.GetResultCount > 0 Then
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E377", New String() {"出荷データ"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                        Return False
                    End If
