...
'  EDI荷主ID　　　　:  115　　　        : MRCデュポン(小池倉庫)      ※2021/08 千葉は未使用とのことで考慮しないこととした
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC115 = New LMH030DAC115()
...
#Region "EDI出荷(大)のDAC側でのチェック"
    ...
    Private Function EdiLDbExistsCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        ElseIf mDestCount = 0 Then
            '届先マスタ未登録については先にエラー判定を行っているのでここでは発生しないはず
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E047", New String() {"届先マスタ"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
            Return False
...
#Region "データセット設定(出荷M)"
    ...
    Private Function SetDatasetOutkaM(ByVal ds As DataSet, ByVal matomeflg As Boolean) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            '2015/6/16 修正 adachi
            'CUST_ORD_NO_DTLが「詰替不用品」の場合、REMARKをブランクにする
            If ediDr("CUST_ORD_NO_DTL").ToString() = "詰替不要品" Then
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            Dim str As String = drRcvEdiDtl(sTempColNo).ToString().Trim()
            If String.IsNullOrEmpty(str) Then
                drOutkaEdiL("OUTKA_PLAN_DATE") = MyBase.GetSystemDate()
            Else
                'mm/dd/yyyy ⇒ yyyy/mm/dd
                drOutkaEdiL("OUTKA_PLAN_DATE") = String.Concat(Mid(str, 7, 4), Left(str, 2), Mid(str, 4, 2))
            End If
        Else
            drOutkaEdiL("OUTKA_PLAN_DATE") = MyBase.GetSystemDate()
        End If
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            ...
            Else
                'mm/dd/yyyy ⇒ yyyy/mm/dd
                drOutkaEdiL("ARR_PLAN_DATE") = String.Concat(Mid(str, 7, 4), Left(str, 2), Mid(str, 4, 2))
            End If
        Else
            drOutkaEdiL("ARR_PLAN_DATE") = String.Empty
        End If
        ...
        Else
            drOutkaEdiL("EDI_DEST_CD") = String.Empty
        End If
        ...
        sKey = "L_DEST_NM_NO"
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            If ("COLUMN_MST".Equals(sTempColNo)) AndAlso (dtDestMst.Rows.Count > 0) Then
                drOutkaEdiL("DEST_NM") = dtDestMst.Rows(0).Item("DEST_NM").ToString()
            Else
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            If ("COLUMN_MST".Equals(sTempColNo)) AndAlso (dtDestMst.Rows.Count > 0) Then
                drOutkaEdiL("DEST_ZIP") = dtDestMst.Rows(0).Item("ZIP").ToString()
            Else
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            If ("COLUMN_MST".Equals(sTempColNo)) AndAlso (dtDestMst.Rows.Count > 0) Then
                drOutkaEdiL("DEST_AD_1") = dtDestMst.Rows(0).Item("AD_1").ToString()
            Else
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            If ("COLUMN_MST".Equals(sTempColNo)) AndAlso (dtDestMst.Rows.Count > 0) Then
                drOutkaEdiL("DEST_AD_2") = dtDestMst.Rows(0).Item("AD_2").ToString()
            Else
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            If ("COLUMN_MST".Equals(sTempColNo)) AndAlso (dtDestMst.Rows.Count > 0) Then
                drOutkaEdiL("DEST_AD_3") = dtDestMst.Rows(0).Item("AD_3").ToString()
            Else
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            If ("COLUMN_MST".Equals(sTempColNo)) AndAlso (dtDestMst.Rows.Count > 0) Then
                drOutkaEdiL("DEST_TEL") = dtDestMst.Rows(0).Item("TEL").ToString()
            Else
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            If ("COLUMN_MST".Equals(sTempColNo)) AndAlso (dtDestMst.Rows.Count > 0) Then
                drOutkaEdiL("DEST_JIS_CD") = dtDestMst.Rows(0).Item("JIS").ToString()
            Else
        ...
        If dtDestMst.Rows.Count > 0 Then
            drOutkaEdiL("SP_NHS_KB") = dtDestMst.Rows(0).Item("SP_NHS_KB").ToString().Trim()
        Else
        ...
        '分析表添付区分
        If dtDestMst.Rows.Count > 0 Then
            drOutkaEdiL("COA_YN") = Right(dtDestMst.Rows(0).Item("COA_YN").ToString().Trim(), 1)
        Else
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            If ("COLUMN_MST".Equals(sTempColNo)) AndAlso (dtDestMst.Rows.Count > 0) Then
                drOutkaEdiL("REMARK") = dtDestMst.Rows(0).Item("REMARK").ToString()
            Else
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
        ...
        '営業所コード
        drOutkaEdiM("NRS_BR_CD") = drSemiEdiInfo.Item("NRS_BR_CD").ToString()
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            drOutkaEdiM("CUST_ORD_NO_DTL") = drRcvEdiDtl.Item(sTempColNo).ToString().Trim()
        Else
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            drOutkaEdiM("CUST_GOODS_CD") = drRcvEdiDtl.Item(sTempColNo).ToString().Trim()

            'セミEDI商品コード修正フラグによる前n桁カット
            Dim drCustMst As DataRow() = setDs.Tables("LMH030_M_CUST_DETAILS").Select("SUB_KB='96'")
            If drCustMst.Length > 0 Then
                Dim cutCount As Integer = CType(drCustMst(0).Item("SET_NAIYO").ToString(), Integer)
                drOutkaEdiM("CUST_GOODS_CD") = drOutkaEdiM("CUST_GOODS_CD").ToString.Substring(cutCount)
            End If
        End If
        ...
        '引当単位区分
        drOutkaEdiM("ALCTD_KB") = drSemiEdiInfo.Item("M_DEF_ALCTD_KB").ToString().Trim()

        '出荷包装個数
        '出荷端数
        sKey = "M_OUTKA_TTL_NB_NO"
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            If LMConst.FLG.ON.Equals(drSemiEdiInfo.Item("DTL_OUTKAPKGNB_CALC_FLG").ToString) Then
                If dtMGoods.Rows.Count > 0 Then
                    drOutkaEdiM("OUTKA_PKG_NB") = Convert.ToInt32(drRcvEdiDtl.Item(sTempColNo).ToString().Trim()) \ Convert.ToInt32(dtMGoods.Rows(0).Item("PKG_NB"))
                    drOutkaEdiM("OUTKA_HASU") = Convert.ToInt32(drRcvEdiDtl.Item(sTempColNo).ToString().Trim()) Mod Convert.ToInt32(dtMGoods.Rows(0).Item("PKG_NB"))
                Else
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) Then
            If dtMGoods.Rows.Count > 0 Then
                Dim sIrime As String = dtMGoods.Rows(0).Item("STD_IRIME_NB").ToString()
                If String.IsNullOrEmpty(sIrime) Then
                    drOutkaEdiM("OUTKA_QT") = 0
                Else
                    drOutkaEdiM("OUTKA_QT") = Convert.ToDecimal(drRcvEdiDtl.Item(sTempColNo).ToString().Trim()) * Convert.ToDecimal(sIrime)
                End If
            Else
                drOutkaEdiM("OUTKA_QT") = 0
            End If
        ...
        '入目
        If dtMGoods.Rows.Count > 0 Then
            drOutkaEdiM("IRIME") = dtMGoods.Rows(0).Item("STD_IRIME_NB").ToString()
        Else
            drOutkaEdiM("IRIME") = 0
        End If
...
#Region "セミEDI時  データセット設定(EDI管理番号(大・中))"
    ...
    ''' </summary>
    ''' <param name="ds"></param>
    ''' <param name="bSameKeyFlg">キー同一フラグ</param>
    ''' <param name="sEdiCtlNo">EDI管理番号(大)</param>
    ''' <param name="iEdiCtlNoChu">EDI管理番号(中)</param>
    ''' <remarks></remarks>
    Private Sub GetEdiCtlNo(
...
#Region "セミEDI時  データセット設定(商品マスタ)"
    ...
    Private Sub SetInDataSelectGoodsMst(ByRef setDs As DataSet, ByVal rowCnt As Integer)
        ...
        '荷主商品コード
        sKey = "M_CUST_GOODS_CD_NO"
        If isNullColKey(drSemiInfo, sKey, sTempColNo) Then
...
#Region "セミEDI時 データセット設定(キャンセル対象データ抽出)"
    ...
    ''' <remarks></remarks>
    Private Function SetDatasetSelectCancelData(ByRef setDs As DataSet, ByVal RowNo As Integer) As String
        ...
        'オーダー番号
        Dim custOrdNo As String = String.Empty
        If isNullColKey(drSemiInfo, "L_DEST_CUST_ORD_NO", tempColNo) Then
            custOrdNo = drItems.Item(tempColNo).ToString().Trim()
            setInOutDr.Item("CUST_ORD_NO") = custOrdNo
        End If
...
#Region "画面取込(セミEDI)チェック処理"
    ...
    Private Function SemiEdiTorikomiChk(ByVal ds As DataSet) As DataSet
        ...
        If isNullColKey(drSemiInfo, "DEVIDE_NO_1", sTempColNo) Then
            arrSortKey.Add(sTempColNo)
        End If
        If isNullColKey(drSemiInfo, "DEVIDE_NO_2", sTempColNo) Then
            arrSortKey.Add(sTempColNo)
        End If
        If isNullColKey(drSemiInfo, "DEVIDE_NO_3", sTempColNo) Then
            arrSortKey.Add(sTempColNo)
        End If
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow, ByVal columnDt As DataTable, ByVal dtMcustD As DataTable) As Boolean
        ...
        '出荷日
        sKey = "L_OUTKA_PLAN_DATE_NO"
        If isNullColKey(columnDr, sKey, sTempColNo) Then
            sMsg = String.Concat("出荷日(カラム番号：", columnDr.Item(sKey), ")[")
        ...
        sKey = "M_LOT_NO_NO"
        If isNullColKey(columnDr, sKey, sTempColNo) Then
            sMsg = String.Concat("ロットNo(カラム番号：", columnDr.Item(sKey), ")[")

            validVal = dr.Item(sTempColNo).ToString().Trim()
            If validVal.Length > 40 Then
                '桁数不正
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E433", New String() {String.Concat(sMsg, validVal, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        Dim bSameKeyFlg As Boolean = False                                   'キー同一フラグ（前行とキーが同じ場合True、異なる場合False）
        ...
        For i As Integer = 0 To dtSetDtl.Rows.Count - 1
            ...
            sKey = "DEVIDE_NO_1"
            If isNullColKey(drSemiInfo, sKey, sTempColNo) Then
                sNewKey += drSetDtl.Item(sTempColNo).ToString().Trim()
            End If
            ...
            Else
                '2件目以降
                If sNewKey.Equals(sOldKey) Then
                    '前行とキーが同じ
                    bSameKeyFlg = True
                Else
                    '前行とキーが異なる
            ...
            If bSameKeyFlg = False AndAlso drSemiInfo.Item("EDI_TORIKESI_FLG").ToString().Equals("1") = True Then
                ...
                'INデータセット設定
                Dim custOrdNo As String = SetDatasetSelectCancelData(setDelDs, i)
                ...
                Else
                    ...
                    If String.IsNullOrEmpty(rtnDs.Tables("LMH030OUT").Rows(0).Item("OUTKA_CTL_NO").ToString()) Then
                        ...
                        '受信テーブル(DTL)の論理削除
                        If "1".Equals(rcvInsFlg) OrElse "2".Equals(rcvInsFlg) Then
                    ...
                    Else
                        ...
                        MyBase.SetMessageStore(
                                LMH030BLC.GUIDANCE_KBN,
                                "E496",
                                New String() {
                                    String.Concat("Delivery:", custOrdNo),
                                    String.Concat("出荷管理番号:", rtnDs.Tables("LMH030OUT").Rows(0).Item("OUTKA_CTL_NO").ToString(), LMH030BLC115.GUI_MSG_03)
                                    },
                                dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(),
                                LMH030BLC.EXCEL_COLTITLE_SEMIEDI,
                                dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim()
                                )
                    End If
                End If
            ...
            ' EDI管理番号の採番
            '---------------------------------------------------------------------------
            Me.GetEdiCtlNo(ds, bSameKeyFlg, sEdiCtlNo, iEdiCtlNoChu)
            ...
            '届先M自動追加・商品Mエラー判断フラグ
            Dim sFlg17 As String = drSemiInfo.Item("FLAG_17").ToString()
            ...
            If bSameKeyFlg = False Then
                ...
                '届先マスタ存在チェック
                If MyBase.GetResultCount = 0 AndAlso sFlg17.Equals("0") Then
                    Dim sDestCd As String = String.Empty
                    sKey = "L_DEST_CD_NO"
                    If isNullColKey(drSemiInfo, sKey, sTempColNo) <> False Then
                        sDestCd = drSetDtl.Item(sTempColNo).ToString().Trim()
                    End If
                    Dim sRecNo As String = drSetDtl.Item("REC_NO").ToString().Trim()

                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E493", New String() {String.Concat("届先コード:", sDestCd), "届先マスタ", String.Concat("行番号:", sRecNo)})
                    bNoErr = False
                    Continue For
                End If
            End If
            ...
            If (bSameKeyFlg = False AndAlso iSkipFlg = 0) OrElse
                    (bSameKeyFlg = True AndAlso (iCntDiffFlg = 1 OrElse itargetUpdFlg = 1)) OrElse
                    (bSameKeyFlg = True AndAlso itargetUpdFlg = 0) Then
                setDs = Me.SetSemiOutkaEdiL(setDs, i, sEdiCtlNo, iEdiCtlNoChu, itargetUpdFlg, iCntDiffFlg)
            End If
            ...
            If MyBase.GetResultCount = 0 AndAlso (sFlg17.Equals("0") OrElse sFlg17.Equals("1")) Then
                ...
                sKey = "M_CUST_GOODS_CD_NO"
                If isNullColKey(drSemiInfo, sKey, sTempColNo) <> False Then
                    sGoodsCd = drSetDtl.Item(sTempColNo).ToString().Trim()
                End If
                Dim sRecNo As String = drSetDtl.Item("REC_NO").ToString().Trim()

                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E493", New String() {String.Concat("商品コード:", sGoodsCd), "商品マスタ", String.Concat("行番号:", sRecNo)})
                bNoErr = False
                Continue For
            End If

            '取込DTL⇒EDI出荷(中)へのデータ設定
            setDs = Me.SetSemiOutkaEdiM(setDs, i, sEdiCtlNo, iEdiCtlNoChu, itargetUpdFlg, iCntDiffFlg)
