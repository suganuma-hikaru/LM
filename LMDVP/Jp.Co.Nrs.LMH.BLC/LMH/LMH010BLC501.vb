...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC501 = New LMH010DAC501()
...
#Region "CONST"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Const INDUSTRY_CD As String = "JPJS2411"

#End Region
    '2013.10.24 WIT対応 追加END
...
#Region "入荷登録"
    ...
    Private Function InkaToroku(ByVal ds As DataSet) As DataSet
        ...
        '2014.08.19 WIT廃止対応 修正START
        '2013.10.24 WIT対応 修正START
        '工業の場合は入荷Sを作成しない
        'If ds.Tables("LMH010_INKAEDI_L").Rows(0).Item("FREE_C04").ToString().Equals(LMH010BLC501.INDUSTRY_CD) = False Then

        'データセット設定処理(入荷S)
        ds = Me.SetDatasetInkaS(ds)
        'End If

        '2013.10.24 WIT対応 修正END
        '2014.08.19 WIT廃止対応 修正END
...
#Region "入荷登録マスタ存在チェック(中)"
    Private Function SetGoodsMst(ByRef ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            '条件の再設定
            setDtM = Me.SetGoodsCdFromWarning(setDtM, ds, LMH010BLC.BP_WID_M001)
            ...
            ElseIf GetResultCount() > 1 Then
                ...
                Else
                    ...
                    ds = Me._ChkBlc.SetWarningM("W162", LMH010BLC.BP_WID_M001, ds, setDs, msgArray)
            ...
            'ビーピー・カストロール追加箇所 terakawa 20120529 start
            '商品名
            If String.IsNullOrEmpty(dtM.Rows(i)("GOODS_NM").ToString()) = False Then
                If dtM.Rows(i)("GOODS_NM").ToString().Equals(goodsDt.Rows(0)("GOODS_NM_1").ToString()) = False Then
                    choiceKb = Me._ChkBlc.GetWarningChoiceKbM(ds.Tables("LMH010_INKAEDI_M"), ds, LMH010BLC.BP_WID_M002, i)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        ds = Me._ChkBlc.SetWarningM2("W194", LMH010BLC.BP_WID_M002, ds, setDs, msgArray, ediName, ediValue, mustValue)
                        flgWarning = True 'ワーニングフラグをたてて処理続行
                    Else
                        '処理区分が"はい"の場合、REMARKを入れ替えて処理続行
                        '要望対応:1965 terakawa 2013.03.26 Start
                        ...
                        End If
                        '要望対応:1965 terakawa 2013.03.26 End
                    End If
            ...
            End If
            '荷主カテゴリ１
            dtM.Rows(i)("FREE_C21") = goodsDt.Rows(0)("SEARCH_KEY_1")
            ...
            '包装個数(入数)
            If String.IsNullOrEmpty(dtM.Rows(i)("PKG_NB").ToString()) = False Then
            ...
            End If
            'ビーピー・カストロール追加箇所 terakawa 20120529 end
            ...
            If String.IsNullOrEmpty(dtM.Rows(i)("NRS_GOODS_CD").ToString()) = False Then
                ...
                If MyBase.GetResultCount <> 0 Then
                    ...
                    Dim goodsCdCust As String = goodsDt.Rows(0)("GOODS_CD_CUST").ToString()

                    '置場情報が4桁（または4バイト）でなかった場合、エラー
                    If setOkiba.Length <> 4 OrElse _
                       System.Text.Encoding.GetEncoding("Shift_JIS").GetByteCount(setOkiba) <> 4 Then
                        MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E529", _
                         New String() {"商品明細マスタの置場情報", "桁数", "荷主商品コード=" + goodsCdCust}, _
                         rowNo, LMH010BLC.EXCEL_COLTITLE, ediCtlNo)
                        Return False
                    End If

                    '置場情報が棟室マスタに存在していない場合、エラー
                    setDs = (MyBase.CallDAC(Me._Dac, "TouSituMstExistChk", setDs))
                    If MyBase.GetResultCount = 0 Then
                        MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E493", _
                         New String() {"商品明細マスタの置場情報", "棟室マスタ", "荷主商品コード=" + goodsCdCust}, _
                         rowNo, LMH010BLC.EXCEL_COLTITLE, ediCtlNo)
                        Return False
                    End If

                    '置場情報が棟室マスタに存在していない場合、エラー
                    setDs = (MyBase.CallDAC(Me._Dac, "ZoneMstExistChk", setDs))
                    If MyBase.GetResultCount = 0 Then
                        MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E493", _
                         New String() {"商品明細マスタの置場情報", "ゾーンマスタ", "荷主商品コード=" + goodsCdCust}, _
                         rowNo, LMH010BLC.EXCEL_COLTITLE, ediCtlNo)
                        Return False
                    End If
                    ...
                    dtM.Rows(i)("SITU_NO") = setOkiba.Substring(2, 1)
                    dtM.Rows(i)("ZONE_CD") = setOkiba.Substring(3, 1)
                    dtM.Rows(i)("FREE_C17") = setOkiba

                Else
                    '商品明細にレコードが存在しなかった場合、チェックなし。
                End If
            End If
...
#Region "実績作成処理"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        'ビーピー・カストロール追加箇所 terakawa 20120530 start
        '送信データ作成用情報取得
        ...
        'ビーピー・カストロール追加箇所 terakawa 20120530 end
