...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC155 = New LMH030DAC155()
...
#Region "SBS東芝ロジスティクスCONST"

    ''' <summary>
    ''' その他
    ''' </summary>
    ''' <remarks></remarks>
    Public Const DEF_CTL_NO As String = "G00000000"             '管理番号初期値
    Public Const DEF_UNSO_NO_L As String = "01-G00000000"       '運送番号初期値
    Public Const DEF_UNSO_NO_M As String = "01-G00000000000"    '運送番号初期値

#End Region ' "SBS東芝ロジスティクスCONST"
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow, ByVal ediCustIndex As String) As Boolean
        ...
        Else
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat("日付(カラム2番目)[", sDate, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        If targetStr.Length > 30 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("番号(カラム3番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        ' 伝票番号
        ' （変更処理時の既存出荷EDIデータと突合情報としてフリー項目に文字列保存するのみ）
        targetStr = dr.Item("COLUMN_4").ToString.Trim()
        If targetStr.Length > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("伝票番号(カラム4番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("荷届先_名称(カラム5番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 文字列長チェック
        ElseIf targetStr.Length > 80 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("荷届先_名称(カラム5番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        '必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("郵便番号(カラム6番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 10, False).Equals(False) Or targetStr.Replace("-", "").Length < 7 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("郵便番号(カラム6番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        '必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("荷届先_住所(カラム7番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 文字列長チェック
        Else
            Dim ad() As String = editAddr(targetStr)
            If ad(0).Equals(targetStr) = False Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("荷届先_住所(カラム7番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
        End If
        ...
        '必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("電話番号(カラム8番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 20, False).Equals(False) Or targetStr.Replace("-", "").Length < 8 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("電話番号(カラム8番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        If targetStr.Length > 60 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("品目名(カラム10番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("ロット(カラム11番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 文字列長チェック
        ElseIf targetStr.Length > 40 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("ロット(カラム11番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("明細数量(カラム12番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        Else
            If IsConvertDbl(targetStr).Equals(False) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E005", New String() {String.Concat("明細数量(カラム12番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            Else
                ...
                ElseIf Convert.ToDouble(targetStr) < 0 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E185", New String() {String.Concat("明細数量(カラム12番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                ElseIf dNum > 9999999999 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("明細数量(カラム12番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                Else
                    If targetStr.IndexOf(".") >= 0 AndAlso Convert.ToDecimal(targetStr.Substring(targetStr.IndexOf(".") + 1)) > 0 Then
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E025", New String() {String.Concat("明細数量(カラム12番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                        bRet = False
                    End If
                End If
            End If
        ...
        If targetStr.Length > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("明細重量(カラム13番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        '文字列長チェック
        If targetStr.Length > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("備考(カラム14番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_SBS").NewRow()
            ds.Tables("LMH030_H_OUTKAEDI_DTL_SBS").Clear() ' 受信DTLのクリア
            ...
            If drEdiRcvDtl("OUTKA_PLAN_DATE").ToString().Trim().Equals("") = False AndAlso
                drEdiRcvDtl("CUST_ORD_NO").ToString().Trim().Equals("") = False Then
                '---------------------------------------------------------------------------
                ' EDI出荷データ件数および出荷データL 出荷管理番号L 等 取得処理
                '---------------------------------------------------------------------------
                setDs.Tables("LMH030_H_OUTKAEDI_DTL_SBS").ImportRow(drEdiRcvDtl)

                setDs = MyBase.CallDAC(Me._Dac, "GetOutkaCntAndNoL", setDs)

                ediRecCnt = MyBase.GetResultCount()
                Dim outkaNoL As String = setDs.Tables("LMH030_H_OUTKAEDI_DTL_SBS").Rows(0).Item("OUTKA_CTL_NO").ToString()
            ...
            ' EDI受信データの新規追加
            '---------------------------------------------------------------------------
            setDs.Tables("LMH030_H_OUTKAEDI_DTL_SBS").Clear()
            setDs.Tables("LMH030_H_OUTKAEDI_DTL_SBS").ImportRow(drEdiRcvDtl)
...
#Region "データセット設定 EDI出荷受信テーブル(明細)"
    ...
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer, ByRef drEdiRcvDtl As DataRow) As DataSet
        ...
        drEdiRcvDtl("OUTKA_PLAN_DATE") = Jp.Co.Nrs.Com.Utility.DateFormatUtility.DeleteSlash(
                                            Me._Blc.LeftB(drSetDtl("COLUMN_2").ToString().Trim(), 10))  ' 出荷日　　　　　　　←受信データ(Excel) 日付
        drEdiRcvDtl("CUST_ORD_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 60)          ' 荷主注文番号（全体）←受信データ(Excel) 番号
        drEdiRcvDtl("DENPYO_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 200)           ' 伝票番号　　　　　　←受信データ(Excel) 伝票番号
        drEdiRcvDtl("DEST_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 160)             ' 届先名　　　　　　　←受信データ(Excel) 荷届先_名称
        Dim destZip As String = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 10)
        drEdiRcvDtl("DEST_ZIP") = destZip                                                               ' 届先郵便番号　　　　←受信データ(Excel) 郵便番号
        drEdiRcvDtl("DEST_AD") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 240)             ' 届先住所　　　　　　←受信データ(Excel) 荷届先_住所
        Dim destTel As String = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 20)
        drEdiRcvDtl("DEST_TEL") = destTel                                                               ' 届先電話番号　　　　←受信データ(Excel) 電話番号
        ...
        End If
        drEdiRcvDtl("DEST_CD") = String.Concat(Me._Blc.LeftB(destZip2, 7),
            destTel2.Substring(destTel2.Length - 8, 8))                                                 ' 届先コード　　　　　←受信データ(Excel) 郵便番号(ハイフン抜き上7桁) + 受信データ(Excel) 電話番号(ハイフン抜き下8桁)
        drEdiRcvDtl("CUST_GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 20)        ' 商品コード　　　　　←受信データ(Excel) 品目コード
        drEdiRcvDtl("GOODS_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 120)           ' 商品名　　　　　　　←受信データ(Excel) 品目名
        drEdiRcvDtl("LOT_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 80)              ' ロット№　　　　　　←受信データ(Excel) ロット
        drEdiRcvDtl("OUTKA_TTL_NB") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 10)        ' 出荷総個数　　　　　←受信データ(Excel) 明細数量
        drEdiRcvDtl("DTL_WT") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 200)             ' 明細重量　　　　　　←受信データ(Excel) 明細重量
        drEdiRcvDtl("REMARK") = Me._Blc.LeftB(drSetDtl("COLUMN_14").ToString().Trim(), 200)             ' 備考　　　　　　　　←受信データ(Excel) 備考

        'データセットに設定
        ds.Tables("LMH030_H_OUTKAEDI_DTL_SBS").Rows.Add(drEdiRcvDtl)
...
#Region "データセット設定 EDI出荷データ 用 EDI管理番号(大・中)"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim sNrsBrCd As String = ds.Tables("LMH030_H_OUTKAEDI_DTL_SBS").Rows(0).Item("NRS_BR_CD").ToString()
...
#Region "データセット設定(EDI出荷(中)"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet, ByVal drEdiRcvDtl As DataRow) As DataSet
        ...
        drOutkaEdiM("CUST_ORD_NO_DTL") = drEdiRcvDtl("CUST_ORD_NO")     ' 荷主注文番号（明細単位）←EDI出荷受信テーブル（明細）.荷主注文番号（全体）←受信データ(Excel) 番号
        drOutkaEdiM("BUYER_ORD_NO_DTL") = String.Empty
        drOutkaEdiM("CUST_GOODS_CD") = drEdiRcvDtl("CUST_GOODS_CD")     ' 商品コード　　　　　　　←EDI出荷受信テーブル（明細）.商品コード　　　　　←受信データ(Excel) 品目コード
        drOutkaEdiM("NRS_GOODS_CD") = String.Empty
        drOutkaEdiM("GOODS_NM") = drEdiRcvDtl("GOODS_NM")               ' 商品名　　　　　　　　　←EDI出荷受信テーブル（明細）.商品名　　　　　　　←受信データ(Excel) 品目名

        drOutkaEdiM("RSV_NO") = String.Empty
        drOutkaEdiM("LOT_NO") = drEdiRcvDtl("LOT_NO")                   ' ロット№　　　　　　　　←EDI出荷受信テーブル（明細）.ロット№　　　　　　←受信データ(Excel) ロット
        drOutkaEdiM("SERIAL_NO") = String.Empty
        drOutkaEdiM("ALCTD_KB") = "01"

        drOutkaEdiM("OUTKA_PKG_NB") = 0
        drOutkaEdiM("OUTKA_HASU") = drEdiRcvDtl("OUTKA_TTL_NB")         ' 出荷端数　　　　　　　　←EDI出荷受信テーブル（明細）.出荷総個数　　　　　←受信データ(Excel) 明細数量
        drOutkaEdiM("OUTKA_QT") = 0
        drOutkaEdiM("OUTKA_TTL_NB") = drEdiRcvDtl("OUTKA_TTL_NB")       ' 出荷総個数　　　　　　　←EDI出荷受信テーブル（明細）.出荷総個数　　　　　←受信データ(Excel) 明細数量
        ...
        drOutkaEdiM("FREE_C01") = drEdiRcvDtl("DENPYO_NO")              ' 文字列01　　　　　　　　←EDI出荷受信テーブル（明細）.伝票番号　　　　　　←受信データ(Excel) 伝票番号
        drOutkaEdiM("FREE_C02") = drEdiRcvDtl("DTL_WT")                 ' 文字列02　　　　　　　　←EDI出荷受信テーブル（明細）.明細重量　　　　　　←受信データ(Excel) 明細重量
...
#Region "データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        drOutkaEdiL("OUTKA_PLAN_DATE") = drEdiRcvDtl("OUTKA_PLAN_DATE")                 ' 出荷日　　　　　　　←EDI出荷受信テーブル（明細）.出荷日　　　　　　　←受信データ(Excel) 日付
        drOutkaEdiL("OUTKO_DATE") = drEdiRcvDtl("OUTKA_PLAN_DATE")                      ' 出荷日　　　　　　　←EDI出荷受信テーブル（明細）.出荷日　　　　　　　←受信データ(Excel) 日付
        drOutkaEdiL("ARR_PLAN_DATE") = drEdiRcvDtl("OUTKA_PLAN_DATE")                   ' 出荷日　　　　　　　←EDI出荷受信テーブル（明細）.出荷日　　　　　　　←受信データ(Excel) 日付
        ...
        drOutkaEdiL("EDI_DEST_CD") = drEdiRcvDtl("DEST_CD")                             ' EDI届先コード 　　　←EDI出荷受信テーブル（明細）.届先コード　　　　　←受信データ(Excel) 郵便番号(ハイフン抜き上7桁) + 受信データ(Excel) 電話番号(ハイフン抜き下8桁)
        drOutkaEdiL("DEST_CD") = drEdiRcvDtl("DEST_CD")                                 ' 届先コード　　　　　←EDI出荷受信テーブル（明細）.届先コード　　　　　←受信データ(Excel) 郵便番号(ハイフン抜き上7桁) + 受信データ(Excel) 電話番号(ハイフン抜き下8桁)
        drOutkaEdiL("DEST_NM") = drEdiRcvDtl("DEST_NM").ToString()                      ' 届先名　　　　　　　←EDI出荷受信テーブル（明細）.届先名　　　　　　　←受信データ(Excel) 荷届先_名称
        drOutkaEdiL("DEST_ZIP") = drEdiRcvDtl("DEST_ZIP").ToString().Replace("-", "")   ' 届先郵便番号　　　　←EDI出荷受信テーブル（明細）.届先郵便番号　　　　←受信データ(Excel) 郵便番号

        ' 住所編集（住所を格納）
        Dim mergeAddr As String = drEdiRcvDtl("DEST_AD").ToString()                     ' 届先住所1～5　　　　←EDI出荷受信テーブル（明細）.届先住所　　　　　　←受信データ(Excel) 荷届先_住所
        ...
        drOutkaEdiL("DEST_TEL") = drEdiRcvDtl("DEST_TEL")                               ' 届先電話番号　　　　←EDI出荷受信テーブル（明細）.届先電話番号　　　　←受信データ(Excel) 電話番号
        ...
        drOutkaEdiL("CUST_ORD_NO") = drEdiRcvDtl("CUST_ORD_NO")                         ' 荷主注文番号（全体）←EDI出荷受信テーブル（明細）.荷主注文番号（全体）←受信データ(Excel) 番号
        ...
        drOutkaEdiL("REMARK") = drEdiRcvDtl("REMARK")                                   ' 注意事項　　　　　　←EDI出荷受信テーブル（明細）.備考　　　　　　　　←受信データ(Excel) 備考
        ...
        drOutkaEdiL("FREE_C01") = drEdiRcvDtl("DENPYO_NO")                              ' 文字列01　　　　　　←EDI出荷受信テーブル（明細）.伝票番号　　　　　　←受信データ(Excel) 伝票番号
        drOutkaEdiL("FREE_C02") = drEdiRcvDtl("OUTKA_PLAN_DATE")                        ' 文字列02　　　　　　←EDI出荷受信テーブル（明細）.出荷日　　　　　　　←受信データ(Excel) 日付
        drOutkaEdiL("FREE_C03") = drEdiRcvDtl("CUST_ORD_NO")                            ' 文字列03　　　　　　←EDI出荷受信テーブル（明細）.荷主注文番号（全体）←受信データ(Excel) 番号
