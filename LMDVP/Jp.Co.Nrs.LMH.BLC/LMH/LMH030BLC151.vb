...
'  EDI荷主ID　　　　:  000　　　        : セミEDI標準仕様(000)をコピー(2020/04/15) ジョンソンエンドジョンソンJ&J_土気自動倉庫
'  作  成  者       :   
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC151 = New LMH030DAC151()
...
#If True Then 'ADD 2019/02/27 依頼番号 : 004291   【LMS】千葉テルモ_EDIテスト+EDI出荷改修
    ...
    ''' 
    ''' </remarks>
    Private _IsTrmType As Boolean = False
...
#Else
            If (_IsFirmeType) Then
                '出荷(大)のまとめ更新(BUYER_ORD_NO含む)
                ds = MyBase.CallDAC(Me._Dac, "UpdateOutkaLDataWithBuyerOrdNo", ds)
            Else
                '出荷(大)のまとめ更新
                ds = MyBase.CallDAC(Me._Dac, "UpdateOutkaLData", ds)
            End If
...
#If False Then      'UPD 2020/06/11 出荷包装個数は取込caseを設定する
                Select Case alctdKb
...
#Else
        ediMDr("OUTKA_PKG_NB") = ediMDr("OUTKA_PKG_NB")
        ediMDr("OUTKA_HASU") = 0
        ediMDr("OUTKA_TTL_NB") = ediMDr("OUTKA_TTL_NB")
...
#If True Then   'ADD 2019/02/27  依頼番号 : 004291   【LMS】千葉テルモ_EDIテスト+EDI出荷改修(PS宮﨑)
        If _IsTrmType = True Then
            Dim sFRR_C13 As String = dt.Rows(0).Item("FREE_C13").ToString()  '送品先コード
            If ("1400074").Equals(sFRR_C13) = True Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E208", New String() {"出荷登録"}, rowNo, LMH030BLC.EXCEL_COLTITLE, ediCtlNo)
                Return False
            End If
...
#Else
                        ...
                        Dim warningIrime As Double = 0.0
                        If (_IsFirmeType AndAlso
                            Double.TryParse(TryCast(dtM.Rows(i)("IRIME"), String), warningIrime) AndAlso
                            warningIrime > 0) Then

                            msgArray(1) = "商品コード, 入目"
                            ds = Me._Blc.SetComWarningM("W162" _
...
#If False Then
                '出荷先名称2,出荷先名称3設定時
                If ("-").Equals(drEdiL("FREE_C01").ToString()) = False Then
                    sDEST_NM = String.Concat(" ", drEdiL("FREE_C01").ToString())
                End If
                If String.IsNullOrEmpty(drEdiL("FREE_C02").ToString()) = False Then
                    sDEST_NM = String.Concat(" ", drEdiL("FREE_C02").ToString())
                End If
                drMD("DEST_NM") = String.Concat(drEdiL("DEST_NM").ToString(), sDEST_NM)
#Else
...
#If False Then  'UPD 2021/01/15 まとめ時納品書対応
                outkaDr("CUST_ORD_NO_DTL") = vbNullString#Eelse
#Else
                outkaDr("CUST_ORD_NO_DTL") = ediDr("CUST_ORD_NO_DTL")
#End If
            Else
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            If drRcvEdiDtl(sTempColNo).ToString().Trim().IndexOf("/") > 0 AndAlso
...
#Else
        drOutkaEdiL("ARR_PLAN_TIME") = "06"         'UPD 2020/10/15 02 → 06 午前指定へ
#End If
...
#End If
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            ...
            Else
                Dim sDEST_NM As String = String.Empty
                '出荷先名称2,出荷先名称3設定時
                If ("-").Equals(drRcvEdiDtl("COLUMN_29").ToString().Trim()) = False Then
                    sDEST_NM = String.Concat(" ", drRcvEdiDtl("COLUMN_29").ToString().Trim())
                End If
                If String.IsNullOrEmpty(drRcvEdiDtl("COLUMN_30").ToString().Trim()) = False Then
                    sDEST_NM = String.Concat(" ", drRcvEdiDtl("COLUMN_30").ToString().Trim())
                End If

                drOutkaEdiL("DEST_NM") = String.Concat(drRcvEdiDtl(sTempColNo).ToString().Trim(), sDEST_NM)
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            ...
            'お得意先名称２，３設定　ADD 2020/10/06
            Dim sTOKUISAKI_NM As String = drRcvEdiDtl(sTempColNo).ToString().Trim()
            'お得意先名称2,お得意先名称3設定時
            If ("-").Equals(drRcvEdiDtl("COLUMN_43").ToString().Trim()) = False Then
                sTOKUISAKI_NM = String.Concat(sTOKUISAKI_NM, " ", drRcvEdiDtl("COLUMN_43").ToString().Trim())
            End If
            If String.IsNullOrEmpty(drRcvEdiDtl("COLUMN_44").ToString().Trim()) = False Then
                sTOKUISAKI_NM = String.Concat(sTOKUISAKI_NM, " ", drRcvEdiDtl("COLUMN_44").ToString().Trim())
            End If

            drOutkaEdiL("FREE_C25") = sTOKUISAKI_NM.ToString
...
#If False Then   'ADD 2019/12/12 009299【LMS】フィルメEDI出荷登録_数量10倍誤出荷理由調査
...
#End If
        ...
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            ...
            ElseIf setDs.Tables("LMH030_M_CUST_DETAILS").Select("SUB_KB='98'").Count > 0 Then
                ...
                drOutkaEdiM.Item("CUST_GOODS_CD") = String.Concat(_Blc.LeftB(drOutkaEdiM.Item("CUST_GOODS_CD").ToString(), 6), Space(1),
                                                                  Right(drOutkaEdiM.Item("CUST_GOODS_CD").ToString(),
        ...
        '入目
        sKey = "M_IRIME_NO"
#If False Then  'UPD 2020/11/26 ケース対応
...
#End If
        ...
        '出荷包装個数
#If True Then   'UPD 2020/06/11 caseを梱数に
        sKey = "M_FREE_N01_NO"
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
            ...
            Else
                ...
                If drOutkaEdiM.Item("ALCTD_KB").Equals("01") Then
                    ...
                    If drSemiEdiInfo.Item("DTL_OUTKAPKGNB_CALC_FLG").ToString().Equals(LMConst.FLG.ON) Then
                        ...
                        If setDs.Tables("LMH030_M_CUST_DETAILS").Select("SUB_KB='9M'").Count > 0 AndAlso
        Convert.ToDecimal(drRcvEdiDtl.Item(sTempColNo).ToString().Trim()) - System.Math.Floor(Convert.ToDecimal(drRcvEdiDtl.Item(sTempColNo).ToString().Trim())) <> 0 Then
                            drOutkaEdiM("OUTKA_PKG_NB") = 0
                        Else
...
#End If
        ...
        '出荷総個数
#If False Then  'UPD 2020/06/11 個数を総個数
                'sKey = "M_OUTKA_TTL_NB_NO"
        If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
...
#End If
        ...
        '出荷総数量
        '016994 2020/11/29 ケース数*入目に変更 -Start
        'sKey = "M_OUTKA_TTL_QT_NO"
        'If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
        '016994 2020/11/29 ケース数*入目に変更 -End
        ...
        '包装個数
#If True Then   'UPD 2020/06/11 入数を入数に
        'sKey = "M_FREE_N02_NO"
        'If isNullColKey(drSemiEdiInfo, sKey, sTempColNo) <> False Then
        '    drOutkaEdiM("PKG_NB") = drRcvEdiDtl.Item(sTempColNo).ToString().Trim()
        'End If
        drOutkaEdiM("PKG_NB") = drMgoods.Item("PKG_NB").ToString()
        'Dim drMCD As DataRow = Nothing
        'If setDs.Tables("LMH030_M_CUST_DETAILS").Select("SUB_KB='68'").Count > 0 Then
        '    drMCD = setDs.Tables("LMH030_M_CUST_DETAILS").Select("SUB_KB='68'")(0)
        '    drOutkaEdiM("PKG_NB") = CType(drMCD.Item("SET_NAIYO").ToString(), Integer)
        'Else
        '    drOutkaEdiM("PKG_NB") = 0.ToString
        'End If
#Else
        '2015.06.08 横浜・古川エージェンシー対応 START
        If setDs.Tables("LMH030_M_CUST_DETAILS").Select("SUB_KB='97'").Count > 0 AndAlso
           drMgoods Is Nothing = False Then
    ...
    Private Sub SetInDataSelectGoodsMst(ByRef setDs As DataSet, ByVal rowCnt As Integer)
        ...
        If isNullColKey(drSemiInfo, sKey, sTempColNo) <> False Then
            ...
            ElseIf setDs.Tables("LMH030_M_CUST_DETAILS").Select("SUB_KB='98'").Count > 0 Then
                ...
                setDr.Item("GOODS_CD_CUST") = String.Concat(_Blc.LeftB(setDr.Item("GOODS_CD_CUST").ToString(), 6), Space(1),
                                                                  Right(setDr.Item("GOODS_CD_CUST").ToString(),
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow, ByVal columnDt As DataTable, ByVal dtMcustD As DataTable) As Boolean
        ...
        If isNullColKey(columnDr, sKey, sTempColNo) <> False Then
            ...
            '- ﾁｪｯｸ実行 -
            sDate = Mid(dr.Item(sTempColNo).ToString().Trim(), 1, 10)
...
#End If
        ...
        If isNullColKey(columnDr, sKey, sTempColNo) <> False Then
            ...
            If String.IsNullOrEmpty(validVal) Then
                ...
                bRet = False
            Else
#If True Then   'ADD 2020/10/05 先頭から0を取る
                Dim chkGoodsCD As String = validVal.ToString()

                For x As Integer = 1 To chkGoodsCD.Length
                    If ("0").Equals(Mid(chkGoodsCD, x, 1)) = False Then
                        chkGoodsCD = Mid(chkGoodsCD, x)
                        Exit For
                    End If
                Next
                validVal = chkGoodsCD.ToString.Trim

                dr.Item(sTempColNo) = chkGoodsCD.ToString.Trim
            ...
            If isNullColKey(drSemiInfo, sKey, sTempColNo) <> False Then
                ...
                If ds.Tables("LMH030_M_CUST_DETAILS").Select("SUB_KB='98'").Count > 0 Then
                    ...
                    If isNullColKey(drSemiInfo, chksKey, chkTempColNo) <> False Then
                        ...
                        If InStr(drSetDtl.Item(chkTempColNo).ToString().Trim(), "-") > 0 AndAlso
...
#End Region
    ...
    Function CalcIrime(ByVal detailRow As DataRow, ByVal semiInfoRow As DataRow) As Double
        ...
        If (isNullColKey(semiInfoRow, "M_OUTKA_TTL_NB_NO", colName) AndAlso
            Double.TryParse(TryCast(detailRow.Item(colName), String), ttlNb)) Then

            If (isNullColKey(semiInfoRow, "M_OUTKA_TTL_QT_NO", colName) AndAlso
