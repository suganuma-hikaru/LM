...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH580DAC = New LMH580DAC()
...
#Region "Const"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_IN As String = "LMH580IN"

    ''' <summary>
    ''' OUTテーブル名
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_OUT As String = "LMH580OUT"
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = LMH580BLC.TABLE_NM_IN
        Dim tableNmOut As String = LMH580BLC.TABLE_NM_OUT
        Dim tableNmRpt As String = LMH580BLC.TABLE_NM_M_RPT
        ...
        workDtRpt = view.ToTable(True, LMH580BLC.NRS_BR_CD, LMH580BLC.PTN_ID, LMH580BLC.PTN_CD, LMH580BLC.RPT_ID)
        ...
        For Each dr As DataRow In ds.Tables(tableNmRpt).Rows
            ...
            'prtDs = comPrt.CallDataSet(ds.Tables(tableNmOut), dr.Item(LMH580BLC.RPT_ID).ToString())
            prtDs = comPrt.CallDataSet(dtOut, dr.Item(LMH580BLC.RPT_ID).ToString())
            '(2012.06.11) 要望番号1102 並び換えしたDateTableを設定 ---  END  ---

            '帳票ごとの編集があるなら行う。
            prtDs = Me.EditPrintDataSet(dr.Item(LMH580BLC.RPT_ID).ToString(), ds)

            '帳票CSV出力
            comPrt.StartPrint(Me, _
                              dr.Item(LMH580BLC.NRS_BR_CD).ToString(), _
                              dr.Item(LMH580BLC.PTN_ID).ToString(), _
                              dr.Item(LMH580BLC.PTN_CD).ToString(), _
                              dr.Item(LMH580BLC.RPT_ID).ToString(), _
    ...
    Private Function EditPrintDataSet(ByVal rptId As String, ByVal ds As DataSet) As DataSet
        ...
        Dim dtOut As DataTable = ds.Tables(LMH580BLC.TABLE_NM_OUT)
        ...
        For i As Integer = 0 To dtOut.Rows.Count - 1
            ...
            If String.IsNullOrEmpty(preOrdNo.Trim()) = True OrElse preOrdNo = nowOrdNo Then
                ...
                If preDnpNo <> nowDnpNo AndAlso nowDnpNo <> "00000000" Then
...
#End If
    ...
    Private Function SelectMPrt(ByVal ds As DataSet, ByVal intbl As DataTable) As DataSet
        ...
        ds = MyBase.CallDAC(Me._Dac, LMH580BLC.ACTION_ID_SELECT_MPRT, ds)
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        Return MyBase.CallDAC(Me._Dac, LMH580BLC.ACTION_ID_SELECT_PRT_DATA, ds)
