...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH010DAC502 = New LMH010DAC502()
...
#Region "入荷登録マスタ存在チェック(中)"
    Private Function SetGoodsMst(ByRef ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        For i As Integer = 0 To max
            ...
            '条件の再設定
            setDtM = Me.SetGoodsCdFromWarning(setDtM, ds, LMH010BLC.SMK_WID_M001)
            ...
            ElseIf GetResultCount() > 1 Then
                ...
                '住化カラー追加箇所 terakawa 20120529 start
                '■住化カラーの場合、商品Mを絞れなかった場合は先頭行を用いる■
                'If MyBase.GetResultCount = 1 Then
                'Else
                ''再検索で商品が確定できない場合はワーニング設定して、次の中レコードへ
                'ds = Me._ChkBlc.SetWarningM("W162", LMH010BLC.SMK_WID_M001, ds, setDs, msgArray)
                'flgWarning = True 'ワーニングフラグをたてて処理続行
                'Continue For
                '住化カラー追加箇所 terakawa 20120529 end
                'End If
            ...
            If String.IsNullOrEmpty(dtM.Rows(i)("GOODS_NM").ToString()) = False Then
                If dtM.Rows(i)("GOODS_NM").ToString().Equals(goodsDt.Rows(0)("GOODS_NM_1").ToString()) = False Then
                    choiceKb = Me._ChkBlc.GetWarningChoiceKbM(ds.Tables("LMH010_INKAEDI_M"), ds, LMH010BLC.SMK_WID_M002, i)
                    ...
                    If String.IsNullOrEmpty(choiceKb) = True Then
                        ...
                        ds = Me._ChkBlc.SetWarningM2("W194", LMH010BLC.SMK_WID_M002, ds, setDs, msgArray, ediName, ediValue, mustValue)
                        flgWarning = True 'ワーニングフラグをたてて処理続行
                    Else
...
#Region "実績作成処理"
    ...
    Private Function JissekiSakusei(ByVal ds As DataSet) As DataSet
        ...
        '住化カラー追加箇所 umano 20120608 start
        '受信ヘッダの更新
        ds = MyBase.CallDAC(Me._Dac, "UpdateRcvHed", ds)
        If MyBase.GetResultCount = 0 Then
            MyBase.SetMessageStore(LMH010BLC.GUIDANCE_KBN, "E011", , rowNo, LMH010BLC.EXCEL_COLTITLE, ediCtlNo)
            Return ds
        End If
        '住化カラー追加箇所 umano 20120608 end

        '住化カラー追加箇所 terakawa 20120530 start
        '受信明細の更新
        ds = MyBase.CallDAC(Me._Dac, "UpdateRcvDtl", ds)
        ...
        '住化カラー追加箇所 terakawa 20120530 end
