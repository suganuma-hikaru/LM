...
'  システム名       :  LM
'  サブシステム名   :  LMH       : EDI
'  プログラムID     :  LMH560    : 入出荷EDI受信帳票(ダウ・ケミカル)
'  作  成  者       :  大貫和正
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH560DAC = New LMH560DAC()
...
#Region "Const"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_IN As String = "LMH560IN"

    ''' <summary>
    ''' OUTテーブル名
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_OUT As String = "LMH560OUT"
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = LMH560BLC.TABLE_NM_IN
        Dim tableNmOut As String = LMH560BLC.TABLE_NM_OUT
        Dim tableNmRpt As String = LMH560BLC.TABLE_NM_M_RPT
        ...
        workDtRpt = view.ToTable(True, LMH560BLC.NRS_BR_CD, LMH560BLC.PTN_ID, LMH560BLC.PTN_CD, LMH560BLC.RPT_ID)
        ...
        For Each dr As DataRow In ds.Tables(tableNmRpt).Rows
            ...
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables(tableNmOut), dr.Item(LMH560BLC.RPT_ID).ToString())

            '帳票ごとの編集があるなら行う。
            prtDs = Me.EditPrintDataSet(dr.Item(LMH560BLC.RPT_ID).ToString(), prtDs)

            '帳票CSV出力
            comPrt.StartPrint(Me, dr.Item(LMH560BLC.NRS_BR_CD).ToString(), _
                                dr.Item(LMH560BLC.PTN_ID).ToString(), _
                                dr.Item(LMH560BLC.PTN_CD).ToString(), _
                                dr.Item(LMH560BLC.RPT_ID).ToString(), _
    ...
    Private Function SelectMPrt(ByVal ds As DataSet, ByVal intbl As DataTable) As DataSet
        ...
        ds = MyBase.CallDAC(Me._Dac, LMH560BLC.ACTION_ID_SELECT_MPRT, ds)
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        Return MyBase.CallDAC(Me._Dac, LMH560BLC.ACTION_ID_SELECT_PRT_DATA, ds)

    End Function

    '要望番号1008 2012.05.09 追加START
    ''' <summary>
    '''  EDI印刷対象テーブルINデータ設定(LMH560)
    ''' </summary>
