...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC157 = New LMH030DAC157()
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow, ByVal drSemiHed As DataRow, ByRef cnt As Integer) As Boolean
        ...
        If cnt = 1 Then
            ' 以下、取込ファイル単位に初回のみのチェック
            ' 出荷日
            sDate = drSemiHed.Item("OUTKA_PLAN_DATE").ToString().Trim()
            ' 年月日チェック
            If IsDate(sDate) = True Then
            Else
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E445", New String() {String.Concat("送付日(見出し部)[", sDate, "]")}, New String("0"c, sRecNo.Length), LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
        End If
        ...
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("郵便番号(カラム1番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 10, False).Equals(False) Or targetStr.Replace("-", "").Length < 7 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("郵便番号(カラム1番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        '必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("電話番号(カラム2番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 20, False).Equals(False) Or targetStr.Replace("-", "").Length < 8 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("電話番号(カラム2番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        If LenB(targetStr) > 30 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("出荷伝票(カラム3番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("出荷先コード(カラム4番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("出荷先名称(カラム5番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 文字列長チェック
        ElseIf LenB(targetStr) > 80 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("出荷先名称(カラム5番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        '必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("出荷先住所(カラム6番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 文字列長チェック
        Else
            Dim ad() As String = editAddr(targetStr)
            If ad(0).Equals(targetStr) = False Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("出荷先住所(カラム6番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            End If
        End If
        ...
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("品目-サイズ(カラム7番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
            ' 半角文字列長チェック
        ElseIf IsHalfSize(targetStr, 20, False).Equals(False) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("品目-サイズ(カラム7番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        If LenB(targetStr) > 40 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("ロット(カラム8番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat("数量(カラム9番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        Else
            If IsConvertDbl(targetStr).Equals(False) Then
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E005", New String() {String.Concat("数量(カラム9番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                bRet = False
            Else
                ...
                ElseIf Convert.ToDouble(targetStr) < 0 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E185", New String() {String.Concat("数量(カラム9番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                ElseIf dNum > 9999999999 Then
                    MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("数量(カラム9番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                    bRet = False
                Else
                    If targetStr.IndexOf(".") >= 0 AndAlso Convert.ToDecimal(targetStr.Substring(targetStr.IndexOf(".") + 1)) > 0 Then
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E025", New String() {String.Concat("数量(カラム9番目)[", targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
                        bRet = False
                    End If
                End If
            End If
        ...
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("JFC(カラム10番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat("PDC(カラム11番目)[", targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        ' 取込Dtlの並べ替え（4列目: 出荷伝票昇順）
        Dim tmpDt As DataTable = dtSetDtl.Clone()
        Dim tmpDr() As DataRow = Nothing
        tmpDr = dtSetDtl.Select(String.Empty, "COLUMN_4 ASC")
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_SGM").NewRow()
            ds.Tables("LMH030_H_OUTKAEDI_DTL_SGM").Clear() ' 受信DTLのクリア
            ...
            If drEdiRcvDtl("CUST_ORD_NO").ToString().Trim().Equals("") = False Then
                ...
                '---------------------------------------------------------------------------
                setDs.Tables("LMH030_H_OUTKAEDI_DTL_SGM").ImportRow(drEdiRcvDtl)

                setDs = MyBase.CallDAC(Me._Dac, "GetOutkaCntAndNoL", setDs)

                ediRecCnt = MyBase.GetResultCount()
                Dim outkaNoL As String = setDs.Tables("LMH030_H_OUTKAEDI_DTL_SGM").Rows(0).Item("OUTKA_CTL_NO").ToString()
            ...
            ' EDI受信データの新規追加
            '---------------------------------------------------------------------------
            setDs.Tables("LMH030_H_OUTKAEDI_DTL_SGM").Clear()
            setDs.Tables("LMH030_H_OUTKAEDI_DTL_SGM").ImportRow(drEdiRcvDtl)
...
#Region "データセット設定 EDI出荷受信テーブル(明細)"
    ...
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer, ByRef drEdiRcvDtl As DataRow) As DataSet
        ...
        drEdiRcvDtl("OUTKA_PLAN_DATE") =
            Date.Parse(drSetHead.Item("OUTKA_PLAN_DATE").ToString().Trim()).ToString("yyyyMMdd")        ' 出荷日　　　　　　　←受信データ(Excel) 送付日 (シート ヘッダ部)
        drEdiRcvDtl("DEST_ZIP") = Me._Blc.LeftB(drSetDtl("COLUMN_2").ToString().Trim(), 10)             ' 届先郵便番号　　　　←受信データ(Excel) 郵便番号
        drEdiRcvDtl("DEST_TEL") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 20)             ' 届先電話番号　　　　←受信データ(Excel) 電話番号
        drEdiRcvDtl("CUST_ORD_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 30)          ' 荷主注文番号（全体）←受信データ(Excel) 出荷伝票
        drEdiRcvDtl("OUTKA_SAKI_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 200)       ' 出荷先コード　　　　←受信データ(Excel) 出荷先コード
        drEdiRcvDtl("DEST_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 80)              ' 届先名　　　　　　　←受信データ(Excel) 出荷先名称
        drEdiRcvDtl("DEST_AD") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 240)             ' 届先住所　　　　　　←受信データ(Excel) 出荷先住所
        drEdiRcvDtl("CUST_GOODS_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 20)        ' 商品コード　　　　　←受信データ(Excel) 品目-サイズ
        drEdiRcvDtl("LOT_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 40)               ' ロット№　　　　　　←受信データ(Excel) ロット
        drEdiRcvDtl("OUTKA_TTL_NB") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 10)        ' 出荷総個数　　　　　←受信データ(Excel) 数量
        drEdiRcvDtl("JFC") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 100)                ' JFC 　　　　　　　　←受信データ(Excel) JFC
        drEdiRcvDtl("PDC") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 100)                ' PDC 　　　　　　　　←受信データ(Excel) PDC

        'データセットに設定
        ds.Tables("LMH030_H_OUTKAEDI_DTL_SGM").Rows.Add(drEdiRcvDtl)
...
#Region "データセット設定 EDI出荷データ 用 EDI管理番号(大・中)"
    ...
    Private Function GetEdiCtlNo(ByVal ds As DataSet _
        ...
        Dim sNrsBrCd As String = ds.Tables("LMH030_H_OUTKAEDI_DTL_SGM").Rows(0).Item("NRS_BR_CD").ToString()
...
#Region "データセット設定(EDI出荷(中)"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet, ByVal drEdiRcvDtl As DataRow) As DataSet
        ...
        drOutkaEdiM("CUST_ORD_NO_DTL") = drEdiRcvDtl("CUST_ORD_NO")     ' 荷主注文番号（明細単位）←EDI出荷受信テーブル（明細）.荷主注文番号（全体）←受信データ(Excel) 出荷伝票
        drOutkaEdiM("BUYER_ORD_NO_DTL") = String.Empty
        drOutkaEdiM("CUST_GOODS_CD") = drEdiRcvDtl("CUST_GOODS_CD")     ' 商品コード　　　　　　　←EDI出荷受信テーブル（明細）.商品コード　　　　　←受信データ(Excel) 品目-サイズ
        ...
        drOutkaEdiM("OUTKA_HASU") = drEdiRcvDtl("OUTKA_TTL_NB")         ' 出荷端数　　　　　　　　←EDI出荷受信テーブル（明細）.出荷総個数　　　　　←受信データ(Excel) 数量
        drOutkaEdiM("OUTKA_QT") = 0
        drOutkaEdiM("OUTKA_TTL_NB") = drEdiRcvDtl("OUTKA_TTL_NB")       ' 出荷総個数　　　　　　　←EDI出荷受信テーブル（明細）.出荷総個数　　　　　←受信データ(Excel) 数量
        ...
        drOutkaEdiM("FREE_C01") = drEdiRcvDtl("JFC")                    ' 文字列01　　　　　　　　←EDI出荷受信テーブル（明細）.JFC 　　　　　　　　←受信データ(Excel) JFC
        drOutkaEdiM("FREE_C02") = drEdiRcvDtl("PDC")                    ' 文字列02　　　　　　　　←EDI出荷受信テーブル（明細）.PDC 　　　　　　　　←受信データ(Excel) PDC
...
#Region "データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        drOutkaEdiL("OUTKA_PLAN_DATE") = drEdiRcvDtl("OUTKA_PLAN_DATE") ' 出荷日　　　　　　　←EDI出荷受信テーブル（明細）.出荷日　　　　　　　　　　　←受信データ(Excel) 送付日 (シート ヘッダ部)
        drOutkaEdiL("OUTKO_DATE") = drEdiRcvDtl("OUTKA_PLAN_DATE")      ' 出荷日　　　　　　　←EDI出荷受信テーブル（明細）.出荷日　　　　　　　　　　　←受信データ(Excel) 送付日 (シート ヘッダ部)

        '納入予定日（出荷日の翌営業日）
        drOutkaEdiL("ARR_PLAN_DATE") = Left(Convert.ToString(Me.GetBussinessDayKrt(drEdiRcvDtl("OUTKA_PLAN_DATE").ToString(), +1, setDs)).Replace("/", String.Empty), 8)
        ...
        End If
        ' 届先コード ←
        ' 　EDI出荷受信テーブル（明細）.届先郵便番号←受信データ(Excel) 郵便番号 (ハイフン抜き上7桁) + 
        ' 　EDI出荷受信テーブル（明細）.届先電話番号←受信データ(Excel) 電話番号 (ハイフン抜き下8桁)
        ...
        drOutkaEdiL("DEST_NM") = drEdiRcvDtl("DEST_NM")                 ' 届先名　　　　　　　←EDI出荷受信テーブル（明細）.届先名　　　　　　　　　　　←受信データ(Excel) 出荷先名称
        drOutkaEdiL("DEST_ZIP") = destZip.Replace("-", "")              ' 届先郵便番号　　　　←EDI出荷受信テーブル（明細）.届先郵便番号 (ハイフン抜き) ←受信データ(Excel) 郵便番号

        If LenB(drEdiRcvDtl("DEST_AD").ToString()) <= 40 Then
            ' 受信データ(Excel) 出荷先住所 をそのままの桁数で設定できる場合
            drOutkaEdiL("DEST_AD_1") = drEdiRcvDtl("DEST_AD")           ' 届先住所1 　　　　　←EDI出荷受信テーブル（明細）.届先住所　　　　　　　　　　←受信データ(Excel) 出荷先住所
        ...
        Else
            ' 住所編集（住所を格納）
            ' 届先住所1～5 ←
            ' 　EDI出荷受信テーブル（明細）.届先住所1←受信データ(Excel) 出荷先住所 の分割文字列
            Dim mergeAddr As String = drEdiRcvDtl("DEST_AD").ToString()
        ...
        drOutkaEdiL("DEST_TEL") = destTel                               ' 届先電話番号　　　　←EDI出荷受信テーブル（明細）.届先電話番号　　　　　　　　←受信データ(Excel) 電話番号
        ...
        drOutkaEdiL("CUST_ORD_NO") = drEdiRcvDtl("CUST_ORD_NO")         ' 荷主注文番号（全体）←EDI出荷受信テーブル（明細）.荷主注文番号（全体）　　　　←受信データ(Excel) 出荷伝票
        ...
        drOutkaEdiL("FREE_C01") = drEdiRcvDtl("CUST_ORD_NO")            ' 文字列01　　　　　　←EDI出荷受信テーブル（明細）.荷主注文番号（全体）　　　　←受信データ(Excel) 出荷伝票
        drOutkaEdiL("FREE_C02") = drEdiRcvDtl("OUTKA_SAKI_CD")          ' 文字列02　　　　　　←EDI出荷受信テーブル（明細）.出荷先コード　　　　　　　　←受信データ(Excel) 出荷先コード
