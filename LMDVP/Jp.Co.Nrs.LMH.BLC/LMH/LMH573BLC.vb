...
'  システム名       :  LM
'  サブシステム名   :  LMH       : EDI
'  プログラムID     :  LMH573    : 出荷EDI受信一覧表(埼玉_大日精化)
'  作  成  者       :  大貫和正
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH573DAC = New LMH573DAC()
...
#Region "Const"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_IN As String = "LMH573IN"

    ''' <summary>
    ''' OUTテーブル名
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_OUT As String = "LMH573OUT"
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = LMH573BLC.TABLE_NM_IN
        Dim tableNmOut As String = LMH573BLC.TABLE_NM_OUT
        Dim tableNmRpt As String = LMH573BLC.TABLE_NM_M_RPT
        ...
        If ds.Tables(tableNmIn).Rows.Count = 0 Then
            ...
            MyBase.SetMessage("G052", New String() {"受信一覧表"})
            MyBase.SetMessageStore(String.Empty, "G052", New String() {"受信一覧表"})
            Return ds
        End If
        ...
        workDtRpt = view.ToTable(True, LMH573BLC.NRS_BR_CD, LMH573BLC.PTN_ID, LMH573BLC.PTN_CD, LMH573BLC.RPT_ID)
        ...
        'ソート実行
        Dim outDr As DataRow() = dtOut.Select(Nothing, "CRT_DATE ASC, FILE_NAME ASC, REC_NO ASC")
        ...
        For Each dr As DataRow In ds.Tables(tableNmRpt).Rows
            ...
            prtDs = comPrt.CallDataSet(dtOut, dr.Item(LMH573BLC.RPT_ID).ToString())
            '(2012.06.11) 要望番号1102 並び換えしたDateTableを設定 ---  END  ---

            '帳票ごとの編集があるなら行う。
            prtDs = Me.EditPrintDataSet(dr.Item(LMH573BLC.RPT_ID).ToString(), prtDs)

            '帳票CSV出力
            comPrt.StartPrint(Me, dr.Item(LMH573BLC.NRS_BR_CD).ToString(), _
                                dr.Item(LMH573BLC.PTN_ID).ToString(), _
                                dr.Item(LMH573BLC.PTN_CD).ToString(), _
                                dr.Item(LMH573BLC.RPT_ID).ToString(), _
    ...
    Private Function SelectMPrt(ByVal ds As DataSet, ByVal intbl As DataTable) As DataSet
        ...
        ds = MyBase.CallDAC(Me._Dac, LMH573BLC.ACTION_ID_SELECT_MPRT, ds)
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        Return MyBase.CallDAC(Me._Dac, LMH573BLC.ACTION_ID_SELECT_PRT_DATA, ds)
