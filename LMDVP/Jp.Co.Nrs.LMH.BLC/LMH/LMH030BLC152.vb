...
'  EDI荷主ID　　　　:  152　　　        : メルクエレクトロニクス(2023/01/11(REV.1910)のセミEDI標準仕様000からコピー)
'  作  成  者       :  
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMH030DAC152 = New LMH030DAC152()
...
#Region "届先マスタ追加時チェック"
    ...
    Private Function ZipCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String, ByVal workDestCd As String, ByVal workDestString As String) As Boolean
        ...
        ElseIf choiceKb.Equals("01") = True Then
            ...
            drMD("AD_3") = String.Empty 'AD_3は未設定
            drMD("COA_YN") = "00"
...
#Region "届先マスタチェック"
    ...
    Private Function DestCompareCheck(ByVal ds As DataSet, ByVal rowNo As String, ByVal ediCtlNo As String) As Boolean
        ...
        Dim mAdAll As String = String.Concat(mAd1, mAd2)    'AD_3を除く
        Dim mZipJis As String = String.Empty
        ...
        Dim ediDestAdAll As String = String.Concat(ediDestAd1, ediDestAd2)  'AD_3を除く
        Dim ediDestJisCd As String = dtEdi.Rows(0)("DEST_JIS_CD").ToString()
        ...
        Else
            ...
            If mAdAll.Equals(ediDestAdAll) = False Then
                ...
                ElseIf choiceKb.Equals("01") = True Then
                    ...
                    'dtMdest.Rows(0).Item("AD_3") = dtEdi.Rows(0)("DEST_AD_3").ToString()   'AD_3を除く
                    'マスタ更新対象フラグ
                    dtMdest.Rows(0).Item("MST_UPDATE_FLG") = "1"
    ...
    Private Function SetDatasetOutkaL(ByVal ds As DataSet, ByVal matomeFlg As Boolean) As DataSet
        ...
        If matomeFlg = False Then
            ...
            If ds.Tables("LMH030_M_DEST").Rows.Count > 0 Then
                ...
                outkaDr("DEST_AD_3") = ediDr("DEST_AD_3")   '届先住所3はEDI出荷Lの値を設定する
...
#If False Then  'UPD 2019/11/13  07667【LMS】エクシング運送EDIデータ出荷本数0 
...
#Region "セミEDI時　データセット設定(EDI出荷(大))"
    ...
    Private Function SetSemiOutkaEdiL(ByVal setDs As DataSet _
        ...
        '出荷日
        Dim outkaPlanDate As String = Date.Parse(drEdiRcvDtl("PGI_DT").ToString().Trim()).ToString("yyyyMMdd")
        drOutkaEdiL("OUTKA_PLAN_DATE") = outkaPlanDate
        '出庫日
        drOutkaEdiL("OUTKO_DATE") = outkaPlanDate

        '納入予定日
        Dim arrPlanDate As String = Date.Parse(drEdiRcvDtl("RDD").ToString().Trim()).ToString("yyyyMMdd")
        ...
        'EDI届先
        Dim destCd As String = drEdiRcvDtl("SHIPTO_NO").ToString()
        drOutkaEdiL("EDI_DEST_CD") = destCd
        '届先
        drOutkaEdiL("DEST_CD") = destCd

        '届先名称
        drOutkaEdiL("DEST_NM") = Me._Blc.LeftB(drEdiRcvDtl("SHIPTO_N1").ToString(), 80)
        '届先郵便番号
        drOutkaEdiL("DEST_ZIP") = drEdiRcvDtl("DEST_ZIP").ToString()

        '届先住所1～3
        drOutkaEdiL("DEST_AD_1") = Me._Blc.LeftB(String.Concat(drEdiRcvDtl("DEST_AD_1").ToString(), drEdiRcvDtl("DEST_AD_2").ToString()), 40)
        drOutkaEdiL("DEST_AD_2") = Me._Blc.LeftB(drEdiRcvDtl("DEST_AD_3").ToString(), 40)
        'UPD Start 2023/06/15 依頼番号 : 037507_【LMS】メルクエレクトロニクス セミEDIマッピング変更
        'drOutkaEdiL("DEST_AD_3") = Me._Blc.LeftB(String.Concat(drEdiRcvDtl("SHIPTO_N2").ToString(), "　", drEdiRcvDtl("ITEM_NOTE_JPCUT").ToString()), 60)

        '注意事項
        Dim remark As String _
            = drEdiRcvDtl("PACKING_INSTRUCTIONS").ToString() _
            & " " _
            & drEdiRcvDtl("ITEM_NOTE_JPCUT").ToString()

        drOutkaEdiL("DEST_AD_3") = Me._Blc.LeftB(String.Concat(drEdiRcvDtl("SHIPTO_N2").ToString(), "　", remark), 60)
        'UPD End   2023/06/15 依頼番号 : 037507_【LMS】メルクエレクトロニクス セミEDIマッピング変更
        ...
        '届先電話番号
        drOutkaEdiL("DEST_TEL") = drEdiRcvDtl("DEST_TEL").ToString()
        '届先FAX
        ...
        '荷主オーダー番号
        drOutkaEdiL("CUST_ORD_NO") = drEdiRcvDtl("DLVY_NUM").ToString()
        '買主オーダー番号
        drOutkaEdiL("BUYER_ORD_NO") = drEdiRcvDtl("PO_NUMBERIN_SO").ToString()
        ...
        'UPD Start 2023/06/15 依頼番号 : 037507_【LMS】メルクエレクトロニクス セミEDIマッピング変更
        ''注意事項
        'drOutkaEdiL("REMARK") = drEdiRcvDtl("ITEM_NOTE_JPCUT").ToString()
        ''配送時注意
        'drOutkaEdiL("UNSO_ATT") = drEdiRcvDtl("PACKING_INSTRUCTIONS").ToString()
        ...
        '文字列1
        drOutkaEdiL("FREE_C01") = drEdiRcvDtl("SLS_ORD").ToString()                 ' 文字列01     ←EDI出荷受信テーブル.受注伝票番号
        drOutkaEdiL("FREE_C02") = drEdiRcvDtl("SHIP_NO").ToString()                 ' 文字列02     ←EDI出荷受信テーブル.シップメント番号
        drOutkaEdiL("FREE_C03") = drEdiRcvDtl("INVOICE_REQUIREMENTS").ToString()    ' 文字列03     ←EDI出荷受信テーブル.Invoice Requirements
...
#Region "セミEDI時　データセット設定(EDI出荷(中))"
    ...
    Private Function SetSemiOutkaEdiM(ByVal setDs As DataSet _
        ...
        '荷主注文番号
        drOutkaEdiM("CUST_ORD_NO_DTL") = drEdiRcvDtl("DLVY_NUM").ToString().Trim()
        '買主注文番号
        drOutkaEdiM("BUYER_ORD_NO_DTL") = drEdiRcvDtl("PO_NUMBERIN_SO").ToString()
        ...
        Else
            drOutkaEdiM("OUTKA_PKG_NB") = Convert.ToInt32(Convert.ToDecimal(drEdiRcvDtl("QTY_NUM").ToString().Trim())) \ Convert.ToInt32(drMgoods("PKG_NB"))
        End If

        '出荷端数
        drOutkaEdiM("OUTKA_HASU") = Convert.ToInt32(Convert.ToDecimal(drEdiRcvDtl("QTY_NUM").ToString().Trim())) Mod Convert.ToInt32(drMgoods("PKG_NB"))

        '出荷数量
        drOutkaEdiM("OUTKA_QT") = Convert.ToDecimal(drEdiRcvDtl("QTY_NUM").ToString().Trim()) * Convert.ToDecimal(drMgoods("STD_IRIME_NB"))

        '出荷総個数
        drOutkaEdiM("OUTKA_TTL_NB") = drEdiRcvDtl("QTY_NUM").ToString().Trim()

        '出荷総数量
        drOutkaEdiM("OUTKA_TTL_QT") = drOutkaEdiM("OUTKA_QT")
        ...
        drOutkaEdiM("FREE_C01") = drEdiRcvDtl("ITEM_NR_DLVY").ToString()    ' 文字列01     ←EDI出荷受信テーブル.出荷伝票明細番号
        drOutkaEdiM("FREE_C02") = drEdiRcvDtl("QTY_STRING").ToString()      ' 文字列02     ←EDI出荷受信テーブル.数量
        drOutkaEdiM("FREE_C03") = drEdiRcvDtl("QTY_UT").ToString()          ' 文字列03     ←EDI出荷受信テーブル.数量単位
        drOutkaEdiM("FREE_C04") = drEdiRcvDtl("PO_NUMBERIN_SO").ToString()  ' 文字列04     ←EDI出荷受信テーブル.得意先発注番号
        drOutkaEdiM("FREE_C05") = drEdiRcvDtl("SLOC_TEXT").ToString()       ' 文字列05     ←EDI出荷受信テーブル.Sloc
        drOutkaEdiM("FREE_C06") = drEdiRcvDtl("SLOC_CD").ToString()         ' 文字列06     ←EDI出荷受信テーブル.保管場所
...
#Region "カラム項目の値・日付チェック"
    ...
    Public Function TorikomiValChk(ByVal dr As DataRow) As Boolean
        ...
        sMsg = "シップメント番号(カラム3番目)["
        ' 桁数チェック
        If LenB(targetStr) > 20 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If
        ...
        ' 得意先発注番号
        targetStr = dr.Item("COLUMN_25").ToString.Trim()
        sMsg = "得意先発注番号(カラム25番目)["
        ' 桁数チェック
        ...
        sMsg = "SLoc(カラム29番目)["
        ' 桁数チェック
        If LenB(targetStr) > 100 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
            bRet = False
        End If

        ' 保管場所
        targetStr = dr.Item("COLUMN_33").ToString.Trim()
        sMsg = "保管場所(カラム33番目)["
        ' 桁数チェック
        If LenB(targetStr) > 40 Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E518", New String() {String.Concat(sMsg, targetStr, "]"), ""}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
        ...
        ' 数量
        targetStr = dr.Item("COLUMN_36").ToString().Trim()
        sMsg = "数量(カラム36番目)["
        ' 必須チェック
        If String.IsNullOrEmpty(targetStr) Then
            MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E001", New String() {String.Concat(sMsg, targetStr, "]")}, sRecNo, LMH030BLC.EXCEL_COLTITLE_SEMIEDI, sFileNm)
        ...
        targetStr = dr.Item("COLUMN_55").ToString.Trim()
        sMsg = "Packing Instructions(カラム55番目)["
        If String.IsNullOrEmpty(targetStr) Then
        Else
            'バイト数チェック
            validLen = System.Text.Encoding.GetEncoding(932).GetByteCount(targetStr)
        ...
        sMsg = "Invoice Requirements(カラム56番目)["
        If String.IsNullOrEmpty(targetStr) Then
        Else
            'バイト数チェック
            validLen = System.Text.Encoding.GetEncoding(932).GetByteCount(targetStr)
...
#Region "画面取込(セミEDI)データセット＋更新処理"
    ...
    Private Function SemiEdiTorikomi(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To iSetDtlMax
            ...
            '---------------------------------------------------------------------------
            Dim drEdiRcvDtl As DataRow = ds.Tables("LMH030_H_OUTKAEDI_DTL_MERC").NewRow()
            gyo += 1
            ds.Tables("LMH030_H_OUTKAEDI_DTL_MERC").Clear() ' 受信DTLのクリア
            ...
            If drEdiRcvDtl.Item("DLVY_NUM").ToString().Trim().Equals("") = False Then
                '---------------------------------------------------------------------------
                ' EDI出荷データ件数および出荷データL 出荷管理番号L 等 取得処理
                '---------------------------------------------------------------------------
                setDs.Tables("LMH030_H_OUTKAEDI_DTL_MERC").ImportRow(drEdiRcvDtl)
                setDs = MyBase.CallDAC(Me._Dac, "GetOutkaCntAndNoL", setDs)

                ediRecCnt = MyBase.GetResultCount()
                Dim outkaNoL As String = setDs.Tables("LMH030_H_OUTKAEDI_DTL_MERC").Rows(0).Item("OUTKA_CTL_NO").ToString()
            ...
            '伝票番号設定
            Dim sDenNo As String = drEdiRcvDtl.Item("DLVY_NUM").ToString().Trim()
            ...
            If bSameKeyFlg = False Then
                ...
                Dim destCd As String = drEdiRcvDtl.Item("SHIPTO_NO").ToString().Trim()
                ...
                If destCnt = 0 Then
                    If sFlg17.Equals("0") Then
                        ...
                        MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E493",
                                               New String() {String.Concat("届先コード:", destCd), "届先マスタ", String.Concat(" 伝票管理番号:", sDenNo)},
                                               dtSetDtl.Rows(i).Item("REC_NO").ToString().Trim(),
                                               LMH030BLC.EXCEL_COLTITLE_SEMIEDI, dtSetDtl.Rows(i).Item("FILE_NAME_RCV").ToString().Trim())
                        bNoErr = False
                        Continue For
            ...
            '商品マスタ読込用INデータ作成
            Dim setDtGoods As DataTable = setDs.Tables("LMH030_M_GOODS")
            Dim setDrGoods As DataRow = setDtGoods.NewRow

            'INデータ作成前にテーブル内一掃
            setDtGoods.Clear()

            Dim goodsCnt As Integer = 0
            Dim goodsCdCust As String = drEdiRcvDtl.Item("GOODS_CD_CUST").ToString().Trim()

            If goodsDict.ContainsKey(goodsCdCust) Then
                For Each drGoods As DataRow In goodsDict(goodsCdCust).Rows
                    setDtGoods.ImportRow(drGoods)
                Next
                goodsCnt = goodsDict(goodsCdCust).Rows.Count
            ElseIf Not notFoundGoodsCdSet.Contains(goodsCdCust) Then
                'INデータ設定
                setDrGoods.Item("NRS_BR_CD") = drSemiInfo.Item("NRS_BR_CD").ToString()
                setDrGoods.Item("CUST_CD_L") = drSemiInfo.Item("CUST_CD_L").ToString()
                setDrGoods.Item("CUST_CD_M") = drSemiInfo.Item("CUST_CD_M").ToString()
                setDrGoods.Item("GOODS_CD_CUST") = goodsCdCust

                'Add処理
                setDtGoods.Rows.Add(setDrGoods)

                '商品マスタ読込
                setDs = MyBase.CallDAC(Me._Dac, "SelectMstGoods", setDs)
                goodsCnt = MyBase.GetResultCount
                If goodsCnt = 0 Then
                    notFoundGoodsCdSet.Add(goodsCdCust)
                Else
                    goodsDict(goodsCdCust) = setDs.Tables("LMH030_M_GOODS")
                End If
            End If
            ...
            If goodsCnt = 0 Then
                ...
                MyBase.SetMessageStore(LMH030BLC.GUIDANCE_KBN, "E493",
                                       New String() {String.Concat("商品コード:", goodsCdCust),
            ...
            ' EDI受信データの新規追加
            '---------------------------------------------------------------------------
            setDs.Tables("LMH030_H_OUTKAEDI_DTL_MERC").Clear()
            setDs.Tables("LMH030_H_OUTKAEDI_DTL_MERC").ImportRow(drEdiRcvDtl)
            ...
            If iSkipFlg = 0 Then
                ...
                setDs = Me.SetSemiOutkaEdiM(setDs, drEdiRcvDtl, sEdiCtlNo, iEdiCtlNoChu)
                ...
                If bSameKeyFlg = False Then
                    ...
                    setDs = Me.SetSemiOutkaEdiL(setDs, _WhCd, drEdiRcvDtl, drSetDtl, sEdiCtlNo, iEdiCtlNoChu)
...
#Region "データセット設定 EDI出荷受信テーブル(明細)"
    ...
    Private Function SetSemiOutkaEdiRcv(ByVal ds As DataSet, ByVal i As Integer, ByVal gyo As Integer, ByVal drEdiRcvDtl As DataRow) As DataSet
        ...
        drEdiRcvDtl("DLVY_NUM") = Me._Blc.LeftB(drSetDtl("COLUMN_1").ToString().Trim(), 40)                 ' 出荷伝票番号
        drEdiRcvDtl("SLS_ORD") = Me._Blc.LeftB(drSetDtl("COLUMN_2").ToString().Trim(), 40)                  ' 受注伝票番号
        drEdiRcvDtl("SHIP_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_3").ToString().Trim(), 20)                  ' シップメント番号
        drEdiRcvDtl("DLVY_TYPE") = Me._Blc.LeftB(drSetDtl("COLUMN_4").ToString().Trim(), 40)                ' 出荷タイプ
        drEdiRcvDtl("ROUTE") = Me._Blc.LeftB(drSetDtl("COLUMN_5").ToString().Trim(), 40)                    ' ルート
        drEdiRcvDtl("PGI_DT") = Me._Blc.LeftB(drSetDtl("COLUMN_6").ToString().Trim(), 20)                   ' 出庫日
        drEdiRcvDtl("RDD") = Me._Blc.LeftB(drSetDtl("COLUMN_7").ToString().Trim(), 20)                      ' 納期
        drEdiRcvDtl("ITEM_NR_DLVY") = Me._Blc.LeftB(drSetDtl("COLUMN_8").ToString().Trim(), 20)             ' 出荷伝票明細番号
        drEdiRcvDtl("SHIPTO_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_9").ToString().Trim(), 40)                ' 出荷先コード
        drEdiRcvDtl("SHIPTO_N1") = Me._Blc.LeftB(drSetDtl("COLUMN_10").ToString().Trim(), 100)              ' 出荷先名1
        drEdiRcvDtl("SHIPTO_N2") = Me._Blc.LeftB(drSetDtl("COLUMN_11").ToString().Trim(), 100)              ' 出荷先名2
        drEdiRcvDtl("DEST_ZIP") = Me._Blc.LeftB(drSetDtl("COLUMN_12").ToString().Trim(), 10)                ' 出荷先郵便番
        drEdiRcvDtl("DEST_AD_1") = Me._Blc.LeftB(drSetDtl("COLUMN_13").ToString().Trim(), 240)              ' 出荷先住所1
        drEdiRcvDtl("DEST_AD_2") = Me._Blc.LeftB(drSetDtl("COLUMN_14").ToString().Trim(), 240)              ' 出荷先住所2
        drEdiRcvDtl("DEST_AD_3") = Me._Blc.LeftB(drSetDtl("COLUMN_15").ToString().Trim(), 240)              ' 出荷先住所3
        drEdiRcvDtl("DEST_TEL") = Me._Blc.LeftB(drSetDtl("COLUMN_16").ToString().Trim(), 20)                ' 出荷先電話番号
        drEdiRcvDtl("SHIP_NOTE") = Me._Blc.LeftB(drSetDtl("COLUMN_17").ToString().Trim(), 100)              ' Shipping Note
        drEdiRcvDtl("COA") = Me._Blc.LeftB(drSetDtl("COLUMN_18").ToString().Trim(), 100)                    ' 試験成績書
        drEdiRcvDtl("REMARKS2") = Me._Blc.LeftB(drSetDtl("COLUMN_19").ToString().Trim(), 100)               ' Remarks2
        drEdiRcvDtl("GOODS_CD_CUST") = Me._Blc.LeftB(drSetDtl("COLUMN_20").ToString().Trim(), 100)          ' 商品コード
        drEdiRcvDtl("GOODS_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_21").ToString().Trim(), 100)               ' 商品名
        drEdiRcvDtl("LOT_NO") = Me._Blc.LeftB(drSetDtl("COLUMN_22").ToString().Trim(), 100)                 ' 製造番号
        drEdiRcvDtl("QTY_STRING") = Me._Blc.LeftB(drSetDtl("COLUMN_23").ToString().Trim(), 40)              ' 数量(String)
        drEdiRcvDtl("QTY_UT") = Me._Blc.LeftB(drSetDtl("COLUMN_24").ToString().Trim(), 20)                  ' 数量単位
        drEdiRcvDtl("PO_NUMBERIN_SO") = Me._Blc.LeftB(drSetDtl("COLUMN_25").ToString().Trim(), 100)         ' 得意先発注番号
        drEdiRcvDtl("CUST_MATERIAL") = Me._Blc.LeftB(drSetDtl("COLUMN_26").ToString().Trim(), 40)           ' 基本データテキスト
        drEdiRcvDtl("TODAYS_DT") = Me._Blc.LeftB(drSetDtl("COLUMN_27").ToString().Trim(), 40)               ' 標準テキスト
        drEdiRcvDtl("SALES_TEXT") = Me._Blc.LeftB(drSetDtl("COLUMN_28").ToString().Trim(), 40)              ' 販売テキスト
        drEdiRcvDtl("SLOC_TEXT") = Me._Blc.LeftB(drSetDtl("COLUMN_29").ToString().Trim(), 100)              ' Sloc(Packing note)
        drEdiRcvDtl("DESCRIPTION") = Me._Blc.LeftB(drSetDtl("COLUMN_30").ToString().Trim(), 100)            ' 顧客依頼テキスト
        drEdiRcvDtl("CUST_MAT_TEXT") = Me._Blc.LeftB(drSetDtl("COLUMN_31").ToString().Trim(), 100)          ' 顧客依頼品目テキスト
        drEdiRcvDtl("BIN_NR") = Me._Blc.LeftB(drSetDtl("COLUMN_32").ToString().Trim(), 100)                 ' 転送元棚番
        drEdiRcvDtl("SLOC_CD") = Me._Blc.LeftB(drSetDtl("COLUMN_33").ToString().Trim(), 40)                 ' 保管場所
        drEdiRcvDtl("PO_DATE") = Me._Blc.LeftB(drSetDtl("COLUMN_34").ToString().Trim(), 100)                ' 得意先品目コード
        drEdiRcvDtl("TRANSGRP") = Me._Blc.LeftB(drSetDtl("COLUMN_35").ToString().Trim(), 100)               ' 輸送手段
        drEdiRcvDtl("QTY_NUM") = Me._Blc.LeftB(drSetDtl("COLUMN_36").ToString().Trim(), 100)                ' 数量(num)
        drEdiRcvDtl("SOLDTO") = Me._Blc.LeftB(drSetDtl("COLUMN_37").ToString().Trim(), 40)                  ' 受注先
        drEdiRcvDtl("SOLDTO_NM") = Me._Blc.LeftB(drSetDtl("COLUMN_38").ToString().Trim(), 100)              ' 受注先名
        drEdiRcvDtl("CUST_CLASS") = Me._Blc.LeftB(drSetDtl("COLUMN_39").ToString().Trim(), 40)              ' 得意先分類
        drEdiRcvDtl("PROFIT_CTR") = Me._Blc.LeftB(drSetDtl("COLUMN_40").ToString().Trim(), 40)              ' 部署コード
        drEdiRcvDtl("CREATEN_DT") = Me._Blc.LeftB(drSetDtl("COLUMN_41").ToString().Trim(), 40)              ' 作成年月日
        drEdiRcvDtl("VOL") = Me._Blc.LeftB(drSetDtl("COLUMN_42").ToString().Trim(), 40)                     ' 容量
        drEdiRcvDtl("VOL_UT") = Me._Blc.LeftB(drSetDtl("COLUMN_43").ToString().Trim(), 40)                  ' 容量単位
        drEdiRcvDtl("INT_MEMO") = Me._Blc.LeftB(drSetDtl("COLUMN_44").ToString().Trim(), 40)                ' 内部メモ
        drEdiRcvDtl("PRD_ATTR_1") = Me._Blc.LeftB(drSetDtl("COLUMN_45").ToString().Trim(), 40)              ' 製品属性1
        drEdiRcvDtl("TEMPRTURE") = Me._Blc.LeftB(drSetDtl("COLUMN_46").ToString().Trim(), 80)               ' 温度(テキスト)
        drEdiRcvDtl("SRC_STR_TYPE") = Me._Blc.LeftB(drSetDtl("COLUMN_47").ToString().Trim(), 40)            ' 転送元保管域タイプ
        drEdiRcvDtl("ORG_BIN") = Me._Blc.LeftB(drSetDtl("COLUMN_48").ToString().Trim(), 40)                 ' 転送元保管区画
        drEdiRcvDtl("LEGAL_REQMT") = Me._Blc.LeftB(drSetDtl("COLUMN_49").ToString().Trim(), 100)            ' 品目該当法規
        drEdiRcvDtl("STRGE_CONDTN") = Me._Blc.LeftB(drSetDtl("COLUMN_50").ToString().Trim(), 40)            ' 品目保管条件
        drEdiRcvDtl("DESCRPTN") = Me._Blc.LeftB(drSetDtl("COLUMN_51").ToString().Trim(), 40)                ' 海貨業者
        drEdiRcvDtl("SHIP_PT") = Me._Blc.LeftB(drSetDtl("COLUMN_52").ToString().Trim(), 40)                 ' 出荷ポイント
        drEdiRcvDtl("WARE") = Me._Blc.LeftB(drSetDtl("COLUMN_53").ToString().Trim(), 40)                    ' 出荷条件
        drEdiRcvDtl("WAREHOUSE") = Me._Blc.LeftB(drSetDtl("COLUMN_54").ToString().Trim(), 40)               ' 出荷条件名称
        drEdiRcvDtl("PACKING_INSTRUCTIONS") = Me._Blc.LeftB(drSetDtl("COLUMN_55").ToString().Trim(), 100)   ' Packing Instructions(Packing Dept./Warehouse Instu.)
        drEdiRcvDtl("INVOICE_REQUIREMENTS") = Me._Blc.LeftB(drSetDtl("COLUMN_56").ToString().Trim(), 100)   ' Invoice Requirements(Invoice Requirements –Internal)
        drEdiRcvDtl("ITEM_NOTE_JPCUT") = Me._Blc.LeftB(drSetDtl("COLUMN_57").ToString().Trim(), 100)        ' Item note-JP CUT

        'データセットに設定
        ds.Tables("LMH030_H_OUTKAEDI_DTL_MERC").Rows.Add(drEdiRcvDtl)
