...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMC796DAC = New LMC796DAC()
...
#Region "印刷"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMC796IN"
        Dim tableNmOut As String = "LMC796OUT"
...
#If True Then   'ADD 2018/06/07 依頼番号 : 000574
        '部数の設定がない場合は初期値0を設定
        Dim prtNb As Integer = 0
        If String.IsNullOrEmpty(ds.Tables("LMC796IN").Rows(0).Item("PRT_NB").ToString()) = False Then
            prtNb = Convert.ToInt32(ds.Tables("LMC796IN").Rows(0).Item("PRT_NB"))
        End If
...
#End If
        ...
        For Each dr As DataRow In ds.Tables("M_RPT").Rows
            ...
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables("LMC796OUT"), dr.Item("RPT_ID").ToString())

            '帳票ごとの編集があるなら行う。
            prtDs = Me.EditPrintDataSet(dr.Item("RPT_ID").ToString(), prtDs)

            '帳票CSV出力
#If False Then      'UPD 2018/06/07 部数処理追加　依頼番号 : 000574
