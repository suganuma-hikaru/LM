...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMC810DAC = New LMC810DAC()
...
#Region "CSVデータ取得処理"
    ...
    Private Function OkakenCsv(ByVal ds As DataSet) As DataSet
        ...
        '出力対象データが0件の場合は終了
        If rtnDs.Tables("LMC810OUT").Rows.Count = 0 Then
            '処理終了メッセージの表示
    ...
    Private Function SelectOkakenCsv(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim dt As DataTable = ds.Tables("LMC810IN")
        Dim outDt As DataTable = ds.Tables("LMC810OUT")
        Dim max As Integer = dt.Rows.Count - 1

        '別インスタンス
        Dim setDs As DataSet = ds.Copy()
        Dim inTbl As DataTable = setDs.Tables("LMC810IN")
        Dim setDt As DataTable = setDs.Tables("LMC810OUT")
    ...
    Private Function UpdateOkakenCsv(ByVal ds As DataSet) As DataSet
        ...
        'DACクラス呼出
        Return MyBase.CallDAC(Me._Dac, "UpdateOkakenCsv", ds)
