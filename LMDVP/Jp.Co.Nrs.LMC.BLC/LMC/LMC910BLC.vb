...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMC910DAC = New LMC910DAC()
...
#Region "CSVデータ取得処理"
    ...
    Private Function SLineCsv(ByVal ds As DataSet) As DataSet

        '印刷対象データの取得
        Dim rtnDs As DataSet = Me.SelectSLineCsv(ds)

        '出力対象データが0件の場合は終了
        If rtnDs.Tables("LMC910OUT").Rows.Count = 0 Then
            '処理終了メッセージの表示
    ...
    Private Function SelectSLineCsv(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim dt As DataTable = ds.Tables("LMC910IN")
        Dim outDt As DataTable = ds.Tables("LMC910OUT")
        Dim max As Integer = dt.Rows.Count - 1

        '別インスタンス
        Dim setDs As DataSet = ds.Copy()
        Dim inTbl As DataTable = setDs.Tables("LMC910IN")
        Dim setDt As DataTable = setDs.Tables("LMC910OUT")
        ...
        For i As Integer = 0 To max
            ...
            'データの抽出
            rtnResult = Me.ServerChkJudge(setDs, "SelectSLineCsv")
    ...
    Private Function UpdateSLineCsv(ByVal ds As DataSet) As DataSet
        ...
        'DACクラス呼出
        Return MyBase.CallDAC(Me._Dac, "UpdateSLineCsv", ds)
