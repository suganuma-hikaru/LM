...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMC792DAC = New LMC792DAC()
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMC792IN"
        Dim tableNmOut As String = "LMC792OUT"
        ...
        For Each dr As DataRow In ds.Tables("M_RPT").Rows
            ...
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables("LMC792OUT"), dr.Item("RPT_ID").ToString())

            copyDs = prtDs.Copy()

            Dim outDr As DataRow

            For k As Integer = 1 To Convert.ToInt32(dtIn.Rows(0).Item("PRT_NB")) - 1

                For j As Integer = 0 To ds.Tables("LMC792OUT").Rows.Count - 1

                    outDr = copyDs.Tables("LMC792OUT").Rows(j)
                    'セット数分(画面で入力した枚数)だけ複写する
                    copyDs.Tables("LMC792OUT").ImportRow(outDr)
            ...
            '                      prtDs.Tables("LMC792OUT"), _
            '                      ds.Tables(LMConst.RD), _
            '                      String.Empty, _
            '                      String.Empty, _
            '                      Convert.ToInt32(dtIn.Rows(0).Item("PRT_NB")))
