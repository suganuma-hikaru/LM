...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMC840DAC = New LMC840DAC()
...
#Region "CSVデータ取得処理"
    ...
    ''' <remarks>DACのSigmaCsvメソッド呼出</remarks>
    Private Function SigmaCsv(ByVal ds As DataSet) As DataSet

        '印刷対象データの取得
        Dim rtnDs As DataSet = Me.SelectSigmaCsv(ds)

        '出力対象データが0件の場合は終了
        If rtnDs.Tables("LMC840OUT").Rows.Count = 0 Then
            '処理終了メッセージの表示
    ...
    Private Function SelectSigmaCsv(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim dt As DataTable = ds.Tables("LMC840IN")
        Dim outDt As DataTable = ds.Tables("LMC840OUT")
        Dim max As Integer = dt.Rows.Count - 1

        '別インスタンス
        Dim setDs As DataSet = ds.Copy()
        Dim inTbl As DataTable = setDs.Tables("LMC840IN")
        Dim setDt As DataTable = setDs.Tables("LMC840OUT")
        ...
        For i As Integer = 0 To max
            ...
            'データの抽出
            rtnResult = Me.ServerChkJudge(setDs, "SelectSigmaCsv")
    ...
    Private Function UpdateSigmaCsv(ByVal ds As DataSet) As DataSet
        ...
        'DACクラス呼出
        Return MyBase.CallDAC(Me._Dac, "UpdateSigmaCsv", ds)
