...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMC541DAC = New LMC541DAC()
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMC541IN"
        Dim tableNmOut As String = "LMC541OUT"
    ...
    Private Function EditPrintDataSet(ByVal rptId As String, ByVal ds As DataSet) As DataSet
        ...
        '親商品毎に項番を振る
        '抽出データテーブル取得
        Dim outDt As DataTable = ds.Tables("LMC541OUT")
        '抽出データ数取得
        Dim max As Integer = outDt.Rows.Count - 1
        '項番(初期値0)
        Dim count As Integer = 0
        '比較用キー設定用
        Dim keyOyaCustGoodsCd As String = String.Empty
        Dim keyOyaGoodsNm As String = String.Empty
        Dim keyNounyuNm As String = String.Empty
        'レコードの項目設定用
        Dim oyaCustGoodsCd As String = String.Empty
        Dim oyaGoodsNm As String = String.Empty
        Dim nounyuNm As String = String.Empty

        '親商品コード、親商品名、納入先名が直前のレコードと一致する場合は同じ項番を設定、一致しない場合はカウントアップし項番を設定
        For i As Integer = 0 To max
            'レコードの親商品コード、親商品名取得、納入先名を取得
            oyaCustGoodsCd = outDt.Rows(i).Item("OYA_CUST_GOODS_CD").ToString()
            oyaGoodsNm = outDt.Rows(i).Item("OYA_GOODS_NM").ToString()
            nounyuNm = outDt.Rows(i).Item("M_FREE_C02").ToString()

            '比較用キーとレコード項目の一致判定(空白は除く)
            If keyNounyuNm <> nounyuNm OrElse _
               keyOyaCustGoodsCd <> oyaCustGoodsCd OrElse _
               keyOyaGoodsNm <> oyaGoodsNm Then
                '一致しない場合
                '項番をカウントアップ
                count = count + 1
                '項番を設定
                outDt.Rows(i).Item("NO") = count.ToString()
                '比較用キー更新
                keyOyaCustGoodsCd = oyaCustGoodsCd
                keyOyaGoodsNm = oyaGoodsNm
                keyNounyuNm = nounyuNm
    ...
    Private Function SelectMPrt(ByVal ds As DataSet) As DataSet
        ...
        If count < 1 Then
            '2011/09/22 SBS)佐川 セットで印刷される送品案内書で出力対象帳票チェックを行っているため削除
            ''0件の場合
