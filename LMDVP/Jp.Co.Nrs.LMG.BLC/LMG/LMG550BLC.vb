...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMG550DAC = New LMG550DAC()
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMG550IN"
        Dim tableNmOut As String = "LMG550OUT"
        ...
        For Each dr As DataRow In ds.Tables("M_RPT").Rows
            ...
            If PRINT_KAGAMI.Equals(inTbl.Rows(0).Item("PRT_TYPE").ToString()) = True _
            AndAlso PRINT_TYPE_NOMAL.Equals(inTbl.Rows(0).Item("PRT_TYPE2").ToString()) = True Then
                busu = 4
            End If

            For i As Integer = 1 To busu
                ...
                '帳票ごとの編集があるなら行う。
                prtDs = Me.EditPrintDataSet(dr.Item("RPT_ID").ToString(), prtDs, inTbl.Rows(0).Item("PRT_TYPE").ToString(), i, tableNmOut)
                '帳票CSV出力
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function EditPrintDataSet(ByVal prtId As String, ByVal ds As DataSet, ByVal prtSyubetsu As String, ByVal rptNo As Integer, ByVal tableNmOut As String) As DataSet


        If PRINT_KAGAMI.Equals(prtSyubetsu) = False Then
            Return ds
        End If
        ...
        For j As Integer = 0 To ds.Tables(tableNmOut).Rows.Count - 1
            ds.Tables(tableNmOut).Rows(j).Item("KAGAMI_TYPE") = title
        Next
