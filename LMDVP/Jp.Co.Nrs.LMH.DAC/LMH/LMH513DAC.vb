...
'  システム名       :  LM
'  サブシステム名   :  LMH       : EDI
'  プログラムID     :  LMH513    : EDI出荷チェックリスト　横浜　古川エージェンシー
'  作  成  者       :  adachi
' ==========================================================================
Imports System.Text
...
#Region "検索処理 SQL"
    ...
    Private Const SQL_SELECT As String = " SELECT                                                       " & vbNewLine _
                                       ...
                                       & "     , EOUTM.FREE_C25 AS BUYER_ORD_NO_M                       " & vbNewLine _
...
#Region "検索処理"
    ...
    '    'DataSetのIN情報を取得
    '    Dim inTbl As DataTable = ds.Tables("LMH513IN")
    ...
    '    'SQL作成
    '    Me._StrSql.Append(LMH513DAC.SQL_MPrt_SELECT)    'SQL構築(帳票種別用SELECT句)
    '    Me._StrSql.Append(LMH513DAC.SQL_MPrt_FROM)      'SQL構築(帳票種別用FROM句)
    '    'Me._StrSql.Append(LMH513DAC.SQL_MPrt_WHERE)     'SQL構築(帳票種別用WHERE句)
    ...
    '    MyBase.Logger.WriteSQLLog("LMH513DAC", "SelectMPrt", cmd)
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        Me._StrSql.Append(LMH513DAC.SQL_SELECT)      'SQL構築(印刷データ抽出用 SELECT句)
        Call Me.SetConditionParameterSQL()           'SQL構築(印刷データ抽出項目設定)
        Me._StrSql.Append(LMH513DAC.SQL_FROM)        'SQL構築(印刷データ抽出用 FROM句)
        Call Me.SetConditionMasterSQL()              'SQL構築(印刷データ抽出条件設定)
        Me._StrSql.Append(LMH513DAC.SQL_ORDER_BY)    'SQL構築(印刷データ抽出用 ORDER BY句)
        ...
        MyBase.Logger.WriteSQLLog("LMH513DAC", "SelectPrintData", cmd)
