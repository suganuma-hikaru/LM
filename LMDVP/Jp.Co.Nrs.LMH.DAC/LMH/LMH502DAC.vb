...
#Region "検索処理 SQL"
    ...
    Private Const SQL_MPrt_FROM As String = " FROM                                                               " & vbNewLine _
                                          ...
                                          & "                  AND M_CUSTRPT1.PTN_ID      = 'BO'                 " & vbNewLine _
                                          ...
                                          & "                  AND M_CUSTRPT2.PTN_ID      = 'BO'                 " & vbNewLine _
    ...
    ''' </summary>                            
    ''' <remarks></remarks>
    Private Const SQL_SELECT_LMH502 As String = "" _
                                       & " SELECT                                                       " & vbNewLine _
                                       & "        CASE WHEN MR2.PTN_CD IS NOT NULL THEN MR2.RPT_ID      " & vbNewLine _
                                       & "             WHEN MR1.PTN_CD IS NOT NULL THEN MR1.RPT_ID      " & vbNewLine _
                                       ...
                                       & "      , CASE WHEN INKAS.NRS_BR_CD IS NOT NULL THEN M_GOODS.PKG_NB                             " & vbNewLine _
                                       & "        ELSE EINM.PKG_NB END             AS PKG_NB                                            " & vbNewLine _
                                       & "      , CASE WHEN INKAS.NRS_BR_CD IS NOT NULL THEN INKAS.KONSU * M_GOODS.PKG_NB + INKAS.HASU  " & vbNewLine _
                                       & "        ELSE EINM.NB   END               AS LABEL_NB                                          " & vbNewLine _
                                       & "      , CASE WHEN INKAS.NRS_BR_CD IS NOT NULL AND M_GOODS.PKG_NB = 1 THEN 0                   " & vbNewLine _
                                       & "             WHEN INKAS.NRS_BR_CD IS NOT NULL THEN CASE WHEN EINM.HASU % M_GOODS.PKG_NB >0 THEN INKAS.KONSU +1 ELSE  INKAS.KONSU END              " & vbNewLine _
                                       & "             ELSE CASE WHEN EINM.HASU % M_GOODS.PKG_NB >0 THEN CEILING(EINM.HASU / M_GOODS.PKG_NB)+1 ELSE CEILING(EINM.HASU / M_GOODS.PKG_NB) END END AS HAKO_NB                               " & vbNewLine _
                                     ...
                                     & "      	  (                                                         " & vbNewLine _
                                     & "      		SELECT                                                  " & vbNewLine _
                                     & "      			 SUBS.NRS_BR_CD  AS NRS_BR_CD                       " & vbNewLine _
                                     & "      			,SUBS.INKA_NO_L  AS INKA_NO_L                       " & vbNewLine _
                                     & "      			,SUBS.INKA_NO_M  AS INKA_NO_M                       " & vbNewLine _
                                     & "      			,SUBS.INKA_NO_S  AS INKA_NO_S                       " & vbNewLine _
                                     & "      			,SUBS.LOT_NO     AS LOT_NO                          " & vbNewLine _
                                     & "      			,MAX(SUBS.KONSU) AS KONSU                           " & vbNewLine _
                                     & "      			,MAX(SUBS.HASU)  AS HASU                            " & vbNewLine _
                                     & "      		FROM                                                    " & vbNewLine _
                                     & "      			LM_TRN_40..B_INKA_S SUBS                            " & vbNewLine _
                                     & "      	    WHERE                                                   " & vbNewLine _
                                     & "      				SUBS.NRS_BR_CD = @NRS_BR_CD                     " & vbNewLine _
                                     & "      	     -- AND SUBS.INKA_NO_L = @INKA_CTL_NO_L                 " & vbNewLine _
                                     & "      		GROUP BY                                                " & vbNewLine _
                                     & "      			 SUBS.NRS_BR_CD                                     " & vbNewLine _
                                     & "      			,SUBS.INKA_NO_L                                     " & vbNewLine _
                                     & "      			,SUBS.INKA_NO_M                                     " & vbNewLine _
                                     & "      			,SUBS.INKA_NO_S                                     " & vbNewLine _
                                     & "      			,SUBS.LOT_NO                                        " & vbNewLine _
                                     & "      	  ) INKAS                                                   " & vbNewLine _
                                     & "                   ON INKAS.NRS_BR_CD        = EINM.NRS_BR_CD       " & vbNewLine _
                                     & "                  AND INKAS.INKA_NO_L        = EINM.INKA_CTL_NO_L   " & vbNewLine _
                                     & "                  AND INKAS.INKA_NO_M        = EINM.INKA_CTL_NO_M	" & vbNewLine _
                                     & "      -- 荷主マスタ                                                 " & vbNewLine _
                                     & "      LEFT OUTER JOIN $LM_MST$..M_CUST M_CUST                       " & vbNewLine _
                                     & "                   ON M_CUST.NRS_BR_CD       = EINL.NRS_BR_CD       " & vbNewLine _
                                     & "                  AND M_CUST.CUST_CD_L       = EINL.CUST_CD_L       " & vbNewLine _
                                     ...
                                     & "                   ON M_GOODS.NRS_BR_CD      = EINL.NRS_BR_CD       " & vbNewLine _
                                     & "                  AND M_GOODS.GOODS_CD_NRS   = EINM.NRS_GOODS_CD    " & vbNewLine _
                                     & "      -- 区分マスタ(N001(荷姿))                                     " & vbNewLine _
                                     & "      LEFT OUTER JOIN $LM_MST$..Z_KBN KBN_N001                      " & vbNewLine _
                                     & "     	           ON KBN_N001.KBN_GROUP_CD = 'N001'                " & vbNewLine _
                                     & "     	          AND KBN_N001.KBN_CD = M_GOODS.NB_UT               " & vbNewLine _
                                     & "      -- 運送会社マスタ                                             " & vbNewLine _
    ...
    ''' </summary>                            
    ''' <remarks></remarks>
    Private Const SQL_ORDER_BY_LMH502 As String = "" _
                                         ...
                                         & "--2017.09.26 入荷EDI改善対応 Annen add start & " & vbNewLine _
                                         & "     , EINM.CUST_GOODS_CD     " & vbNewLine _
                                         & "     , EINM.LOT_NO            " & vbNewLine _
                                         & "--2017.09.26 入荷EDI改善対応 Annen add start & " & vbNewLine _
...
#Region "検索処理"
    ...
    Private Function SelectMPrintPattern(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim inTbl As DataTable = ds.Tables("LMH502IN")
        ...
        'SQL作成
        Me._StrSql.Append(LMH502DAC.SQL_MPrt_SELECT)    'SQL構築(帳票種別用SELECT句)
        Me._StrSql.Append(LMH502DAC.SQL_MPrt_FROM)      'SQL構築(帳票種別用FROM句)
        Me._StrSql.Append(LMH502DAC.SQL_MPrt_WHERE)     'SQL構築(帳票種別用WHERE句)
        ...
        MyBase.Logger.WriteSQLLog("LMH502DAC", "SelectMPrt", cmd)
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH502DAC.SQL_SELECT_LMH502)      'SQL構築(印刷データ抽出用 SELECT句)
        Call Me.SetConditionMasterSQL()                     'SQL構築(印刷データ抽出条件設定)
        Me._StrSql.Append(LMH502DAC.SQL_ORDER_BY_LMH502)    'SQL構築(印刷データ抽出用 ORDER BY句)
        ...
        MyBase.Logger.WriteSQLLog("LMH502DAC", "SelectPrintDataLMH502", cmd)
        ...
        map.Add("LABEL_NB", "LABEL_NB")
        map.Add("HAKO_NB", "HAKO_NB")
        map.Add("NB_UT_NM", "NB_UT_NM")

        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMH502OUT")
