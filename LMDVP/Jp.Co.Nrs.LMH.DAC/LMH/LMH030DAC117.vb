...
#Region "H_OUTKAEDI_HED_ANF"
    ''' <summary>
    ''' アンファEDI受信HEDのUPDATE文（H_OUTKAEDI_HED_ANF）
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_UPDATE_OUTKASAVE_EDI_RCV_HED As String = "UPDATE $LM_TRN$..H_OUTKAEDI_HED_ANF SET       " & vbNewLine _
                                              & " OUTKA_CTL_NO      = @OUTKA_CTL_NO       	        " & vbNewLine _
...
#Region "H_OUTKAEDI_DTL_ANF"
    ''' <summary>
    ''' アンファEDI受信DTLのUPDATE文（H_OUTKAEDI_DTL_ANF）
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_UPDATE_OUTKASAVE_EDI_RCV_DTL As String = "UPDATE $LM_TRN$..H_OUTKAEDI_DTL_ANF SET       " & vbNewLine _
                                              & " OUTKA_CTL_NO      = @OUTKA_CTL_NO       	        " & vbNewLine _
                                              & ",OUTKA_CTL_NO_CHU  = @OUTKA_CTL_NO_CHU       	    " & vbNewLine _
                                              & ",UPD_USER          = @UPD_USER                     " & vbNewLine _
                                              & ",UPD_DATE          = @UPD_DATE                     " & vbNewLine _
...
#Region "H_OUTKAEDI_HED_ANF"
    ''' <summary>
    ''' H_OUTKAEDI_HED_ANF(出荷取消⇒未登録)
    ''' </summary>
    ''' <remarks></remarks>
    ''' 
    Private Const SQL_UPD_TOUROKUMI_RCV_HED As String = "UPDATE                                     " & vbNewLine _
                                          & " $LM_TRN$..H_OUTKAEDI_HED_ANF                      " & vbNewLine _
                                          & " SET                                               " & vbNewLine _
                                          & " OUTKA_CTL_NO      = 'C00000000'        　　       " & vbNewLine _
...
#Region "商品マスタ"
    ...
    Private Function SelectMstGoods(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC117.SQL_SELECT_M_GOODS)

        Me._SqlPrmList = New ArrayList()

        'SQL条件設定(Where)
        Me._StrSql.Append(LMH030DAC117.SQL_WHERE1_M_GOODS)

        'ADD 2016/07/20 フィルメ対応
        If ds.Tables("LMH030_M_CUST_DETAILS").Select("SUB_KB='0L'").Count > 0 Then
            'SQL条件設定(Where + 入り目を追加)
            Me._StrSql.Append(LMH030DAC117.SQL_WHERE1_M_GOODS_FLM)

        End If

        'SQL条件設定(Group by)
        Me._StrSql.Append(LMH030DAC117.SQL_GROUP_BY_M_GOODS)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "SelectDataMgoods", cmd)
...
#Region "荷主明細マスタ（SUB_KB=87）"
    ...
    Private Function SelectMstCustD87(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC117.SELECT_M_CUST_DETL)

        Me._SqlPrmList = New ArrayList()

        'SQL条件設定(Where)
        Me._StrSql.Append(LMH030DAC117.SQL_WHERE1_M_CUST_DETAILS)
...
#Region "荷主明細マスタ（SUB_KB=0L）"
    ...
    Private Function SelectMstCustD0L(ByVal ds As DataSet) As DataSet
        ...
        'SQL条件設定(Where)
        Me._StrSql.Append(LMH030DAC117.SQL_WHERE1_M_CUST_DETAILS_FIR)
...
#Region "届先マスタ"
    ...
    Private Function SelectMstDest(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC117.SQL_SELECT_M_DEST)
        ...
        'SQL条件設定(Group by)
        Me._StrSql.Append(LMH030DAC117.SQL_GROUP_BY_M_DEST)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "SelectDataMdestOutkaToroku", cmd)
...
#Region "区分マスタ"
    ...
    Private Function SelectDataZkbn(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC117.SQL_SELECT_Z_KBN)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "SelectDataZkbn", cmd)
...
#Region "キャンセルデータ抽出"
    Private Function SelectCancelData(ByVal ds As DataSet) As DataSet
        ...
        If ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("DTL_DATACHECK_FLG").ToString().Equals(LMConst.FLG.ON) = True Then
            'SQL作成(SELECT)
            Me._StrSql.Append(LMH030DAC117.SQL_SELECT_CANCER_DATA_DTL)
            'SQL作成(WHERE)
            Me._StrSql.Append(LMH030DAC117.SQL_WHERE_CANCER_DATA_DTL)

            'オーダー番号＋オーダー番号明細チェックの場合
        ElseIf ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("DTL_DATACHECK_FLG").ToString().Equals("3") = True Then
            'SQL作成(SELECT)
            Me._StrSql.Append(LMH030DAC117.SQL_SELECT_CANCER_DATA_DTL_CNT)
            'SQL作成(WHERE)
            Me._StrSql.Append(LMH030DAC117.SQL_WHERE_CANCER_DATA_DTL_CNT)
        ...
        ElseIf ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("DTL_DATACHECK_FLG").ToString().Equals("4") = True Then
            ...
            Me._StrSql.Append(LMH030DAC117.SQL_SELECT_CANCER_DATA_DTL_CNT)
            'SQL作成(WHERE)
            Me._StrSql.Append(LMH030DAC117.SQL_WHERE_CANCER_DATA_DTL_ONLY_CNT)
        ...
        ElseIf ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("DTL_DATACHECK_FLG").ToString().Equals("5") = True Then
            ...
            Me._StrSql.Append(LMH030DAC117.SQL_SELECT_CANCER_DATA_DTL)
            'SQL作成(WHERE)
            Me._StrSql.Append(LMH030DAC117.SQL_WHERE_CANCER_DATA_DTL_M_FREE_C25)
            '2015.04.28 Ｍ・Ｒ・Ｃデュポン対応 M_FREE_C25_NOの(オーダー番号明細のみ)チェックの場合 追加END
        Else
            'SQL作成(SELECT)
            Me._StrSql.Append(LMH030DAC117.SQL_SELECT_CANCER_DATA)
            'SQL作成(WHERE)
            Me._StrSql.Append(LMH030DAC117.SQL_WHERE_CANCER_DATA)
        End If
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "SelectCancelData", cmd)
...
#If False Then
        ...
        If ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("RCV_NM_DTL").ToString().Equals("H_OUTKAEDI_DTL_ARB") = True Then
            'スキーマ設定
            sql = Me.SetSchemaNm(LMH030DAC117.SQL_SELECT_COMPARE_CHECK_ARB, ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("NRS_BR_CD").ToString())
        ...
        ElseIf ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("RCV_NM_DTL").ToString().Equals("H_OUTKAEDI_DTL_MRC") = True Then
            'スキーマ設定
            sql = Me.SetSchemaNm(LMH030DAC117.SQL_SELECT_COMPARE_CHECK_MRC, ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("NRS_BR_CD").ToString())
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "SelectCompareData", cmd)
...
#Region "荷主明細マスタ（SUB_KB検索条件に含まない）"
    ...
    Private Function SelectMstCustAll(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC117.SELECT_M_CUST_DETL_ALL)

        Me._SqlPrmList = New ArrayList()

        'SQL条件設定(Where)
        Me._StrSql.Append(LMH030DAC117.SQL_WHERE_M_CUST_DETAILS_ALL)
...
#Region "H_SENDOUTEDI_ANF SELECT句"

    Private Const SQL_SELECT_ANF_SEND_DATA As String = "SELECT                                                        " & vbNewLine _
                                          & " '0'                                          AS DEL_KB	              " & vbNewLine _
                                          & ",EDI_L.NRS_BR_CD                              AS NRS_BR_CD               " & vbNewLine _
                                          & ",EDI_L.EDI_CTL_NO                             AS EDI_CTL_NO              " & vbNewLine _
                                          & ",OUT_L.OUTKA_NO_L                             AS OUTKA_CTL_NO            " & vbNewLine _
                                          & ",OUT_L.CUST_CD_L                              AS CUST_CD_L               " & vbNewLine _
                                          & ",OUT_L.CUST_CD_M                              AS CUST_CD_M               " & vbNewLine _
                                          & ",ANF_L.CUST_OUTKA_NO                          AS OUTKA_SHIJI_NO          " & vbNewLine _
                                          & "--,ISNULL(FUL.UNSO_NO_L,'')                     AS TOIAWASE_NO             " & vbNewLine _
                                          & ",ISNULL(FUL.AUTO_DENP_NO,'')                  AS TOIAWASE_NO             " & vbNewLine _
                                          & ",OUT_L.OUTKA_PLAN_DATE                        AS HASSOUBI                " & vbNewLine _
                                          & ",'2'                                          AS JISSEKI_SHORI_FLG       " & vbNewLine

#End Region

#Region "H_SENDOUTEDI_ANF FROM句"

    Private Const SQL_FROM_ANF_SEND_DATA As String = "FROM                                                          " & vbNewLine _
                                          & "$LM_TRN$..H_OUTKAEDI_L EDI_L                                           " & vbNewLine _
                                          & "INNER JOIN $LM_TRN$..C_OUTKA_L OUT_L                                   " & vbNewLine _
                                          & "--  ON  OUT_L.SYS_DEL_FLG = '0'                                        " & vbNewLine _
                                          & "   ON OUT_L.NRS_BR_CD  = EDI_L.NRS_BR_CD                               " & vbNewLine _
                                          & "  AND OUT_L.OUTKA_NO_L = EDI_L.OUTKA_CTL_NO                            " & vbNewLine _
                                          & "INNER JOIN $LM_TRN$..H_OUTKAEDI_HED_ANF ANF_L                          " & vbNewLine _
                                          & "   ON ANF_L.SYS_DEL_FLG = '0'                                          " & vbNewLine _
                                          & "  AND ANF_L.NRS_BR_CD   = EDI_L.NRS_BR_CD                              " & vbNewLine _
                                          & "  AND ANF_L.EDI_CTL_NO  = EDI_L.EDI_CTL_NO                             " & vbNewLine _
                                          & "INNER JOIN $LM_TRN$..F_UNSO_L FUL                                      " & vbNewLine _
                                          & "   ON FUL.SYS_DEL_FLG  = '0'                                           " & vbNewLine _
                                          & "  AND FUL.NRS_BR_CD    = OUT_L.NRS_BR_CD                               " & vbNewLine _
                                          & "  AND FUL.INOUTKA_NO_L = OUT_L.OUTKA_NO_L                              " & vbNewLine _
                                          & "WHERE EDI_L.DEL_KB <> '1'                                              " & vbNewLine _
                                          & " AND  EDI_L.NRS_BR_CD  = @NRS_BR_CD                                    " & vbNewLine _
                                          & " AND  EDI_L.EDI_CTL_NO = @EDI_CTL_NO                                   " & vbNewLine _
                                          & " AND  EDI_L.JISSEKI_FLAG = '0'                                         " & vbNewLine _
                                          & "ORDER BY OUT_L.CUST_ORD_NO                                             " & vbNewLine

#End Region

#Region "実績作成処理 更新用SQL"

#Region "H_SENDOUTEDI_ANF"""

    ''' <summary>
    ''' 実績TBLのINSERT（H_SENDOUTEDI_ANF）
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_INSERT_H_SENDOUTEDI_ANF As String = "INSERT INTO       " & vbNewLine _
                                        & "$LM_TRN$..H_SENDOUTEDI_ANF       " & vbNewLine _
                                        & "(                                " & vbNewLine _
                                        & " DEL_KB                   " & vbNewLine _
                                        & ",NRS_BR_CD                " & vbNewLine _
                                        & ",EDI_CTL_NO               " & vbNewLine _
                                        ...
                                        & ",CUST_CD_L                " & vbNewLine _
                                        & ",CUST_CD_M                " & vbNewLine _
                                        & ",OUTKA_SHIJI_NO           " & vbNewLine _
                                        & ",TOIAWASE_NO              " & vbNewLine _
                                        & ",HASSOUBI                 " & vbNewLine _
                                        & ",SEIKYUBI                 " & vbNewLine _
                                        & ",SEIKYU_KIGENBI           " & vbNewLine _
...
#Region "H_OUTKAEDI_HED_ANF"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_UPDATE_JISSEKISAKUSEI_EDI_RCV_HED As String = "UPDATE $LM_TRN$..H_OUTKAEDI_HED_ANF SET    " & vbNewLine _
...
#Region "H_OUTKAEDI_DTL_ANF"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_UPDATE_JISSEKISAKUSEI_EDI_RCV_DTL As String = "UPDATE $LM_TRN$..H_OUTKAEDI_DTL_ANF SET    " & vbNewLine _
...
#Region "データ取得処理"
    ...
    ''' <param name="ds">DataSet</param>
    ''' <returns>DataSet</returns>
    ''' <remarks>アンファEDI実績データ結果取得SQLの構築・発行</remarks>
    Private Function SelectAngfaEdiSend(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC117.SQL_SELECT_ANF_SEND_DATA)      'SQL構築(データ抽出用Select句)
        Me._StrSql.Append(LMH030DAC117.SQL_FROM_ANF_SEND_DATA)        'SQL構築(データ抽出用From句)
        ...
        MyBase.Logger.WriteSQLLog("SLMH030DAC117", "SelectAngfaEdiSend", cmd)
        ...
        map.Add("CUST_CD_L", "CUST_CD_L")
        map.Add("CUST_CD_M", "CUST_CD_M")
        map.Add("OUTKA_SHIJI_NO", "OUTKA_SHIJI_NO")
        map.Add("TOIAWASE_NO", "TOIAWASE_NO")
        map.Add("HASSOUBI", "HASSOUBI")
        map.Add("JISSEKI_SHORI_FLG", "JISSEKI_SHORI_FLG")

        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMH030_H_SENDOUTEDI_ANF")

        '処理件数の設定
        MyBase.SetResultCount(ds.Tables("LMH030_H_SENDOUTEDI_ANF").Rows.Count())
        reader.Close()
    ...
    Private Function UpdateOutkaEdiLData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            Case LMH030DAC.EventShubetsu.EDITORIKESI _
                ...
                '    setSql = LMH030DAC117.SQL_UPDATE_JISSEKIZUMI_JISSEKIMI_EDI_L

                '    '実行(実績送信済⇒送信待)SQL CONST名
                'Case LMH030DAC.EventShubetsu.SOUSINZUMI_SOUSINMACHI

                '    setSql = LMH030DAC117.SQL_UPDATE_JISSEKIZUMI_SOUSINMACHI_EDI_L

                'Case LMH030DAC.EventShubetsu.TOUROKUZUMI_MITOUROKU

                '    setSql = LMH030DAC117.SQL_UPDATE_TOUROKUMI_EDI_L
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateOutkaEdiLData", cmd)
...
#Region "H_OUTKAEDI_M"
    ...
    Private Function UpdateOutkaEdiMData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            '出荷登録、紐付けSQL CONST名
            Case LMH030DAC.EventShubetsu.SAVEOUTKA, LMH030DAC.EventShubetsu.HIMODUKE
                setSql = LMH030DAC117.SQL_UPDATE_OUTKAEDI_M
                loopflg = 1

                '    '実績作成SQL CONST名
            Case LMH030DAC.EventShubetsu.CREATEJISSEKI
                setSql = LMH030DAC117.SQL_UPDATE_JISSEKISAKUSEI_EDI_M

                'EDI取消、EDI取消⇒未登録SQL CONST名
            Case LMH030DAC.EventShubetsu.EDITORIKESI _
                , LMH030DAC.EventShubetsu.TORIKESI_MITOUROKU

                setSql = LMH030DAC117.SQL_UPD_EDITORIKESI_EDI_M

                '    '実行(実績作成済⇒実績未)SQL CONST名
                'Case LMH030DAC.EventShubetsu.SAKUSEIZUMI_JISSEKIMI
                '    setSql = LMH030DAC117.SQL_UPDATE_JISSEKIZUMI_JISSEKIMI_EDI_M
                ...
                '    setSql = LMH030DAC117.SQL_UPDATE_JISSEKIZUMI_SOUSINMACHI_EDI_M

                'Case LMH030DAC.EventShubetsu.TOUROKUZUMI_MITOUROKU

                '    setSql = LMH030DAC117.SQL_UPDATE_TOUROKUMI_EDI_M
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateOutkaEdiMData", cmd)
...
#Region "H_OUTKAEDI_HED_ANF"
    ...
    Private Function UpdateEdiRcvHedData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            '出荷登録、紐付けSQL CONST名
            Case LMH030DAC.EventShubetsu.SAVEOUTKA, LMH030DAC.EventShubetsu.HIMODUKE
                setSql = LMH030DAC117.SQL_UPDATE_OUTKASAVE_EDI_RCV_HED

                '実績作成SQL CONST名
            Case LMH030DAC.EventShubetsu.CREATEJISSEKI
                setSql = LMH030DAC117.SQL_UPDATE_JISSEKISAKUSEI_EDI_RCV_HED

                '実績取消SQL CONST名
            Case LMH030DAC.EventShubetsu.TORIKESIJISSEKI
                'setSql = LMH030DAC117.SQL_UPDATE_JISSEKICANSEL_EDI_RCV_HED

                'EDI取消、EDI取消⇒未登録
            Case LMH030DAC.EventShubetsu.EDITORIKESI _
                , LMH030DAC.EventShubetsu.TORIKESI_MITOUROKU

                setSql = LMH030DAC117.SQL_UPDATE_EDITORIKESI_RCV_HED

                '実行(実績作成済⇒実績未,実績送信済⇒実績未)SQL CONST名
            Case LMH030DAC.EventShubetsu.SAKUSEIZUMI_JISSEKIMI _
                , LMH030DAC.EventShubetsu.SOUSINZUMI_JISSEKIMI
                'setSql = LMH030DAC117.SQL_UPDATE_JISSEKIZUMI_JISSEKIMI_EDI_RCV_HED

                '実行(実績送信済⇒送信待)
            Case LMH030DAC.EventShubetsu.SOUSINZUMI_SOUSINMACHI

                'setSql = LMH030DAC117.SQL_UPD_SOUSINZUMI_SOUSINMI_RCV_HED

            Case LMH030DAC.EventShubetsu.TOUROKUZUMI_MITOUROKU

                setSql = LMH030DAC117.SQL_UPD_TOUROKUMI_RCV_HED
        ...
        Call Me.SetJissekiParameterRcv(Me._Row, dtEventShubetsu)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateEdiRcvHedData", cmd)
...
#Region "H_OUTKAEDI_DTL_ANF"
    ...
    Private Function UpdateEdiRcvDtlData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            '出荷登録、紐付けSQL CONST名
            Case LMH030DAC.EventShubetsu.SAVEOUTKA, LMH030DAC.EventShubetsu.HIMODUKE
                setSql = LMH030DAC117.SQL_UPDATE_OUTKASAVE_EDI_RCV_DTL
                loopflg = 1

                '実績作成SQL CONST名
            Case LMH030DAC.EventShubetsu.CREATEJISSEKI
                setSql = LMH030DAC117.SQL_UPDATE_JISSEKISAKUSEI_EDI_RCV_DTL

                '実績取消SQL CONST名
            Case LMH030DAC.EventShubetsu.TORIKESIJISSEKI
                'setSql = LMH030DAC117.SQL_UPDATE_JISSEKICANSEL_EDI_RCV_DTL

                'EDI取消、EDI取消⇒未登録、報告用EDI取消
            Case LMH030DAC.EventShubetsu.EDITORIKESI _
                , LMH030DAC.EventShubetsu.TORIKESI_MITOUROKU

                setSql = LMH030DAC117.SQL_UPDATE_EDITORIKESI_RCV_DTL

                '実行(実績作成済⇒実績未)SQL CONST名
            Case LMH030DAC.EventShubetsu.SAKUSEIZUMI_JISSEKIMI
                'setSql = LMH030DAC117.SQL_UPDATE_JISSEKIZUMI_JISSEKIMI_EDI_RCV_DTL

                '実行(実績作成済⇒実績未)SQL CONST名
            Case LMH030DAC.EventShubetsu.SOUSINZUMI_JISSEKIMI
                'setSql = LMH030DAC117.SQL_UPDATE_JISSEKISOUSINZUMI_JISSEKIMI_EDI_RCV_DTL

                '実行(実績送信済⇒送信待)
            Case LMH030DAC.EventShubetsu.SOUSINZUMI_SOUSINMACHI

                'setSql = LMH030DAC117.SQL_UPD_SOUSINZUMI_SOUSINMI_RCV_DTL

            Case LMH030DAC.EventShubetsu.TOUROKUZUMI_MITOUROKU

                setSql = LMH030DAC117.SQL_UPD_TOUROKUMI_RCV_DTL
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateEdiRcvDtlData", cmd)
...
#Region "H_OUTKAEDI_L(セミEDI：取消更新)"
    ...
    Private Function UpdateDelOutkaEdiL(ByVal ds As DataSet) As DataSet
        ...
        '出荷EDI CONST名
        sSql = LMH030DAC117.SQL_UPDATE_DEL_OUTKAIEDI_L
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateDelOutkaEdiL", cmd)
...
#Region "H_OUTKAEDI_M(セミEDI：取消更新)"
    ...
    Private Function UpdateDelOutkaEdiM(ByVal ds As DataSet) As DataSet
        ...
        'EDI出荷SQL
        sSql = LMH030DAC117.SQL_UPDATE_DEL_OUTKAIEDI_M
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateDelOutkaEdiM", cmd)
...
#Region "H_OUTKAEDI_DTL_XXX(セミEDI：受信テーブル取消更新)"
    ...
    Private Function UpdateDelRcvDtl(ByVal ds As DataSet) As DataSet
        ...
        Dim sql As String = Me.SetSchemaNm(LMH030DAC117.SQL_UPDATE_DEL_OUTKAIEDI_DTL, ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("NRS_BR_CD").ToString())
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateDelRcvDtl", cmd)
...
#Region "H_SENDOUTEDI_AMF"
    ...
    ''' <param name="ds">DataSet</param>
    ''' <returns>DataSet</returns>
    ''' <remarks>アンファEDI実績テーブル更新SQLの構築・発行</remarks>
    Private Function InsertAngfaEdiSendData(ByVal ds As DataSet) As DataSet

        'DataSetのIN情報を取得
        Dim dowSendTbl As DataTable = ds.Tables("LMH030_H_SENDOUTEDI_ANF")

        Dim rtn As Integer = 0

        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC117.SQL_INSERT_H_SENDOUTEDI_ANF _
                                                                       , ds.Tables("LMH030_H_SENDOUTEDI_ANF").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("SLMH030DAC117", "InsertAngfaEdiSendData", cmd)
...
#End Region
    ...
    '        'SQL文のコンパイル
    '        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC117.SQL_INSERT_OUTKAEDI_L, Me._Row.Item("NRS_BR_CD").ToString()))
    ...
    '        MyBase.Logger.WriteSQLLog("LMH030DAC117", "InsertOutkaEdiL", cmd)
    ...
    '        'SQL文のコンパイル
    '        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC117.SQL_INSERT_OUTKAEDI_M _
    ...
    '        MyBase.Logger.WriteSQLLog("LMH030DAC117", "InsertOutkaEdiM", cmd)
    ...
    '        'SQL文のコンパイル
    '        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC117.SQL_UPDATE_EDIT_OUTKAEDI_L, Me._Row.Item("NRS_BR_CD").ToString()))
    ...
    '        MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateEditOutkaEdiL", cmd)
    ...
    '        'SQL文のコンパイル
    '        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC117.SQL_UPDATE_EDIT_OUTKAEDI_M, Me._Row.Item("NRS_BR_CD").ToString()))
    ...
    '        MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateEditOutkaEdiM", cmd)
...
#Region "出荷登録処理用EDIデータ 抽出処理"
    ...
    Private Function SelectEdiL(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC117.L_DEF_SQL_SELECT_DATA)      'SQL構築
        Call Me.setSQLSelectDataExists()                           '条件設定
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "SelectEdiL", cmd)
    ...
    Private Function SelectEdiM(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC117.M_DEF_SQL_SELECT_DATA)      'SQL構築(データ抽出用)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "SelectEdiM", cmd)
...
#Region "C_OUTKA_L"
    ...
    Private Function UpdateOutkaLData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            Case LMH030DAC.EventShubetsu.SAVEOUTKA
                setSql = LMH030DAC117.SQL_UPDATE_OUTKAL_MATOME

                '実績作成SQL CONST名
            Case LMH030DAC.EventShubetsu.CREATEJISSEKI
                setSql = LMH030DAC117.SQL_UPDATE_JISSEKISAKUSEI_OUTKA_L
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateOutkaLData", cmd)
    ...
    Private Function UpdateOutkaLDataWithBuyerOrdNo(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            '出荷登録(まとめ時)
            Case LMH030DAC.EventShubetsu.SAVEOUTKA
                setSql = LMH030DAC117.SQL_UPDATE_OUTKAL_MATOME_WITH_BUYER_ORD_NO
...
#Region "まとめ先EDI出荷(大)"
    ...
    Private Function UpdateMatomesakiEdiLData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            '出荷登録SQL CONST名
            Case LMH030DAC.EventShubetsu.SAVEOUTKA
                setSql = LMH030DAC117.SQL_UPDATE_MATOMESAKI_OUTKAEDI_L
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateMatomesakiEdiLData", cmd)
...
#Region "まとめ先運送(大)"
    ...
    Private Function UpdateMatomesakiUnsoLData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            '出荷登録SQL CONST名
            Case LMH030DAC.EventShubetsu.SAVEOUTKA
                setSql = LMH030DAC117.SQL_UPDATE_MATOMESAKI_UNSO_L
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateMatomesakiUnsoLData", cmd)
...
#Region "M_DEST(自動更新用)"
    ...
    Private Function UpdateMDestData(ByVal ds As DataSet) As DataSet
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateMDestData", cmd)
...
#Region "C_OUTKA_L"
    ...
    Private Function InsertOutkaLData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC117.SQL_INSERT_OUTKA_L, Me._Row.Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "InsertOutkaLData", cmd)
...
#Region "C_OUTKA_M"
    ...
    Private Function InsertOutkaMData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC117.SQL_INSERT_OUTKA_M _
                                                                       , ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC117", "UpdateOutkaMData", cmd)
...
#Region "E_SAGYO_REC"
    ...
    Private Function InsertSagyoData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC117.SQL_INSERT_SAGYO _
                                                                       , ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC117", "InsertSagyoData", cmd)
...
#Region "UNSO_L"
    ...
    Private Function InsertUnsoLData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC117.SQL_INSERT_UNSO_L _
                                                                       , ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "InsertUnsoLData", cmd)
...
#Region "UNSO_M"
    ...
    Private Function InsertUnsoMData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC117.SQL_INSERT_UNSO_M _
                                                                       , ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC117", "InsertUnsoMData", cmd)
...
#Region "UNCHIN"
    ...
    Private Function InsertUnchinData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC117.SQL_INSERT_UNCHIN _
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC117", "InsertUnchinData", cmd)
...
#Region "M_DEST(自動追加用)"
    ...
    Private Function InsertMDestData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC117.SQL_INSERT_M_DEST, Me._Row.Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "InsertMDestData", cmd)
...
#Region "EDI送信(TBL)新規登録パラメータ設定"
    ...
    Private Sub SetEdiSendCreateParameter(ByVal conditionRow As DataRow, ByVal prmList As ArrayList)
        ...
        With conditionRow
            ...
            prmList.Add(MyBase.GetSqlParameter("@OUTKA_SHIJI_NO", Me.NullConvertString(.Item("OUTKA_SHIJI_NO")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@TOIAWASE_NO", Me.NullConvertString(.Item("TOIAWASE_NO")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@HASSOUBI", Me.NullConvertString(.Item("HASSOUBI")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@SEIKYUBI", Me.NullConvertString(.Item("SEIKYUBI")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@SEIKYU_KIGENBI", "", DBDataType.NVARCHAR))
...
#If False Then
        ...
        If setDs.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("RCV_NM_DTL").ToString().Equals("H_OUTKAEDI_DTL_ARB") = True Then
            sql = Me.SetSchemaNm(LMH030DAC117.SQL_INSERT_OUTKAEDI_DTL_ARB, setDs.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("NRS_BR_CD").ToString())
        ElseIf setDs.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("RCV_NM_DTL").ToString().Equals("H_OUTKAEDI_DTL_MRC") = True Then
            sql = Me.SetSchemaNm(LMH030DAC117.SQL_INSERT_OUTKAEDI_DTL_MRC, setDs.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("NRS_BR_CD").ToString())
        End If
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC117", "InsertOutkaEdiRcvDtl", cmd)
