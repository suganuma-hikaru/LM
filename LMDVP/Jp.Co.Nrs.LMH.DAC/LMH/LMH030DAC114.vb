...
#Region "M_GOODS(セミEDI時)"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_WHERE_M_GOODS As String = _
                                       " WHERE                                              " & vbNewLine _
                                     ...
                                     & " AND                                                " & vbNewLine _
                                     & "-- SUBSTRING(M_GOODS.GOODS_CD_CUST,1,7) = @GOODS_CD_CUST             " & vbNewLine _
...
#End Region
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SELECT_M_GOODS_CUST As String _
                    = " SELECT                                                     " & vbNewLine _
                    & " COUNT(*)                       AS  MST_CNT                 " & vbNewLine _
                    & ",NRS_BR_CD                      AS  NRS_BR_CD               " & vbNewLine _
                    & ",GOODS_CD_NRS                   AS  GOODS_CD_NRS            " & vbNewLine _
...
#Region "M_GOODS(前7byte抽出)"
    ''' <summary>
    ''' 商品マスタ読込(WHERE句)
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_WHERE_M_GOODS_CUST As String = _
                                       " WHERE                                                             " & vbNewLine _
                                     & " M_GOODS.NRS_BR_CD = @NRS_BR_CD                                    " & vbNewLine _
                                     & " AND                                                               " & vbNewLine _
                                     & " M_GOODS.CUST_CD_L = @CUST_CD_L                                    " & vbNewLine _
                                     ...
                                     & " M_GOODS.CUST_CD_M = @CUST_CD_M                                    " & vbNewLine _
                                     & " AND                                                               " & vbNewLine _
                                     & "--UPD 2020/02/04 010140  SUBSTRING(M_GOODS.GOODS_CD_CUST,1,7) = SUBSTRING(@GOODS_CD_CUST,1,7)             " & vbNewLine _
                                     & " M_GOODS.GOODS_CD_CUST = @GOODS_CD_CUST                            " & vbNewLine _
                                     & " AND                                                               " & vbNewLine _
                                     & " M_GOODS.SYS_DEL_FLG = '0'                                         " & vbNewLine
...
#Region "GROUP_BY_M_GOODS(前7byte抽出)"
    ''' <summary>
    ''' 商品マスタ読込(GROUP BY句)
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_GROUP_BY_M_GOODS_CUST As String _
                    = " GROUP BY              " & vbNewLine _
                    & " NRS_BR_CD             " & vbNewLine _
                    & ",GOODS_CD_NRS          " & vbNewLine _
                    & ",CUST_CD_L             " & vbNewLine _
...
#Region "SELECT_M_DEST_TEL"

    Private Const SQL_SELECT_M_DEST_TEL As String = " SELECT                             " & vbNewLine _
                                     & " COUNT(*)           AS MST_CNT                   " & vbNewLine _
                                     & ",NRS_BR_CD			AS NRS_BR_CD	             " & vbNewLine _
                                     & ",CUST_CD_L			AS CUST_CD_L				 " & vbNewLine _
...
#Region "GROUP_BY_M_DEST"
    ...
    Private Const SQL_GROUP_BY_M_DEST_TEL As String = " GROUP BY                     " & vbNewLine _
                                     & "NRS_BR_CD                                " & vbNewLine _
                                     & ",CUST_CD_L                               " & vbNewLine _
                                     & ",DEST_CD                                 " & vbNewLine _
                                     & ",EDI_CD                                  " & vbNewLine _
...
#Region "SELECT_M_GENERIC_UNSOCO"
    ''' <summary>
    ''' 運送会社設定マスタ読込
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SELECT_M_GENERIC_UNSOCO As String = " SELECT                         " & vbNewLine _
                                     & " NRS_BR_CD                     AS NRS_BR_CD        " & vbNewLine _
                                     & ",CUST_CD_L                     AS CUST_CD_L        " & vbNewLine _
                                     & ",CUST_CD_M                     AS CUST_CD_M        " & vbNewLine _
                                     & ",TARGET_CD                     AS TARGET_CD        " & vbNewLine _
                                     & ",UNSO_CD                       AS UNSO_CD          " & vbNewLine _
                                     & ",UNSO_BR_CD                    AS UNSO_BR_CD       " & vbNewLine _
                                     & ",MOTO_DATA_KB                  AS MOTO_DATA_KB     " & vbNewLine _
                                     & ",UNSO_TEHAI_KB                 AS UNSO_TEHAI_KB    " & vbNewLine _
                                     & ",TARIFF_BUNRUI_KB              AS TARIFF_BUNRUI_KB " & vbNewLine _
                                     & ",BIN_KB                        AS BIN_KB           " & vbNewLine _
                                     & ",DEFAULT_UNSO_FLG              AS DEFAULT_UNSO_FLG " & vbNewLine _
                                     & ",SYS_DEL_FLG                   AS SYS_DEL_FLG      " & vbNewLine _
                                     & " FROM                                              " & vbNewLine _
                                     & " $LM_MST$..M_GENERIC_UNSOCO    M_G_UNSOCO          " & vbNewLine _
                                     & " WHERE                                             " & vbNewLine _
                                     & " M_G_UNSOCO.NRS_BR_CD          = @NRS_BR_CD        " & vbNewLine _
                                     & " AND                                               " & vbNewLine _
                                     & " M_G_UNSOCO.CUST_CD_L          = @CUST_CD_L        " & vbNewLine _
                                     & " AND                                               " & vbNewLine _
                                     & " M_G_UNSOCO.CUST_CD_M          = @CUST_CD_M        " & vbNewLine _
                                     & " AND                                               " & vbNewLine _
                                     & " RIGHT(M_G_UNSOCO.TARGET_CD,3) = @FREE_C02         " & vbNewLine _
                                     & " AND                                               " & vbNewLine _
                                     & " M_G_UNSOCO.DEFAULT_UNSO_FLG   = '0'               " & vbNewLine _
                                     ...
                                     & " M_G_UNSOCO.CUST_CD_M          = @CUST_CD_M        " & vbNewLine _
                                     & " AND                                               " & vbNewLine _
                                     & " M_G_UNSOCO.DEFAULT_UNSO_FLG   = '1'               " & vbNewLine

#End Region

#End Region

#Region "アクタス情報設定マスタ読込"

#Region "SELECT_M_INFOSTATE_ATS"
    ''' <summary>
    ''' アクタス情報設定マスタ読込
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SELECT_M_INFOSTATE_ATS As String = " SELECT                          " & vbNewLine _
                                     & " NRS_BR_CD                     AS NRS_BR_CD        " & vbNewLine _
                                     & ",CUST_CD_L                     AS CUST_CD_L        " & vbNewLine _
                                     & ",CUST_CD_M                     AS CUST_CD_M        " & vbNewLine _
                                     & ",DATA_SEND_CD                  AS DATA_SEND_CD     " & vbNewLine _
                                     & ",VIA_CD                        AS VIA_CD           " & vbNewLine _
                                     & ",UNSO_CD                       AS UNSO_CD          " & vbNewLine _
                                     & ",UNSO_BR_CD                    AS UNSO_BR_CD       " & vbNewLine _
                                     & ",MOTO_DATA_KB                  AS MOTO_DATA_KB     " & vbNewLine _
                                     & ",UNSO_TEHAI_KB                 AS UNSO_TEHAI_KB    " & vbNewLine _
                                     & ",TARIFF_BUNRUI_KB              AS TARIFF_BUNRUI_KB " & vbNewLine _
                                     & ",BIN_KB                        AS BIN_KB           " & vbNewLine _
                                     & ",OUTKA_ADDTION_DATE            AS OUTKA_ADDTION_DATE " & vbNewLine _
                                     & ",ARR_ADDTION_DATE              AS ARR_ADDTION_DATE " & vbNewLine _
                                     & ",DELIVERY_KBN                  AS DELIVERY_KBN     " & vbNewLine _
                                     & ",DELIVERY_CD                   AS DELIVERY_CD      " & vbNewLine _
                                     & ",LAST_DELIVERY_FLG             AS LAST_DELIVERY_FLG  " & vbNewLine _
                                     & ",DEFAULT_INFO_FLG              AS DEFAULT_INFO_FLG " & vbNewLine _
                                     & ",SYS_DEL_FLG                   AS SYS_DEL_FLG      " & vbNewLine _
                                     & " FROM                                              " & vbNewLine _
                                     & " $LM_MST$..M_INFOSTATE_ATS     M_INFO_ATS          " & vbNewLine _
                                     & " WHERE                                             " & vbNewLine _
                                     & " M_INFO_ATS.NRS_BR_CD          = @NRS_BR_CD        " & vbNewLine _
                                     & " AND                                               " & vbNewLine _
                                     & " M_INFO_ATS.CUST_CD_L          = @CUST_CD_L        " & vbNewLine _
                                     & " AND                                               " & vbNewLine _
                                     & " M_INFO_ATS.CUST_CD_M          = @CUST_CD_M        " & vbNewLine _
                                     & " AND                                               " & vbNewLine _
                                     & " M_INFO_ATS.VIA_CD             = @FREE_C02         " & vbNewLine _
                                     & " AND                                               " & vbNewLine _
                                     & " M_INFO_ATS.DATA_SEND_CD       = @WEEKDAY          " & vbNewLine _
                                     & " AND                                               " & vbNewLine _
                                     & " M_INFO_ATS.DEFAULT_INFO_FLG   = '0'               " & vbNewLine _
                                     & " UNION ALL                                         " & vbNewLine _
                                     & " SELECT                                            " & vbNewLine _
                                     & " NRS_BR_CD                     AS NRS_BR_CD        " & vbNewLine _
                                     & ",CUST_CD_L                     AS CUST_CD_L        " & vbNewLine _
                                     ...
                                     & " M_INFO_ATS.DATA_SEND_CD       = @WEEKDAY          " & vbNewLine _
                                     & " AND                                               " & vbNewLine _
                                     & " M_INFO_ATS.DEFAULT_INFO_FLG   = '1'               " & vbNewLine
...
#Region "H_OUTKAEDI_Mデータ抽出用"
    ...
    Private Const M_DEF_SQL_SELECT_DATA As String = " SELECT                                                             " & vbNewLine _
                                        ...
                                        & ",H_OUTKAEDI_M.GOODSM_FUKAKUTEI_FLG               AS  GOODSM_FUKAKUTEI_FLG  --ADD 2019/12/24 009299   " & vbNewLine _
...
#Region "商品マスタ"
    ...
    Private Function SelectMstGoods(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC114.SQL_SELECT_M_GOODS)

        Me._SqlPrmList = New ArrayList()

        'SQL条件設定(Where) 先頭7Byteで
        Me._StrSql.Append(LMH030DAC114.SQL_WHERE_M_GOODS)

        'SQL条件設定(Group by)
        Me._StrSql.Append(LMH030DAC114.SQL_GROUP_BY_M_GOODS)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "SelectDataMgoods", cmd)
...
#Region "商品マスタ(荷主商品コード前7byteで抽出)"
    ...
    ''' <returns>DataSet</returns>
    ''' <remarks></remarks>
    Private Function SelectDataMgoodsCust(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC114.SQL_SELECT_M_GOODS_CUST)
        Me._StrSql.Append(LMH030DAC114.SQL_WHERE_M_GOODS_CUST)

        Me._SqlPrmList = New ArrayList()

        'SQL条件設定(Group by)
        Me._StrSql.Append(LMH030DAC114.SQL_GROUP_BY_M_GOODS_CUST)

        'パラメータ設定
        Call Me.SetSelectMstGoodsCustParameter(dtL, dtM)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "SelectDataMgoodsCust", cmd)
        ...
        If reader.HasRows() = True Then
            ...
            Call Me.SetGoodsCustMap(reader, map)
...
#Region "商品マスタをHashTableに設定"
    ...
    Private Function SetGoodsCustMap(ByVal reader As SqlDataReader, ByVal map As Hashtable) As Hashtable
...
#Region "荷主明細マスタ（SUB_KB=87）"
    ...
    Private Function SelectMstCustD87(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC114.SELECT_M_CUST_DETL)

        Me._SqlPrmList = New ArrayList()

        'SQL条件設定(Where)
        Me._StrSql.Append(LMH030DAC114.SQL_WHERE1_M_CUST_DETAILS)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "SelectMstCustD87", cmd)
...
#Region "届先マスタ"
    ...
    Private Function SelectMstDest(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC114.SQL_SELECT_M_DEST)
        ...
        'SQL条件設定(Group by)
        Me._StrSql.Append(LMH030DAC114.SQL_GROUP_BY_M_DEST)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "SelectMstDest", cmd)
...
#Region "運送会社設定マスタ"
    ...
    ''' <returns>DataSet</returns>
    ''' <remarks></remarks>
    Private Function SelectDataMGenericUnsoco(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC114.SQL_SELECT_M_GENERIC_UNSOCO)

        Me._SqlPrmList = New ArrayList()

        'パラメータ設定
        Call Me.SetMGeniricUnsocoParameter(Me._Row)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "SelectDataMGenericUnsoco", cmd)
        ...
        If reader.HasRows() = True Then
            ...
            ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMH030_M_GENERIC_UNSOCO")
    ...
    Private Function SetUnsocoMap(ByVal reader As SqlDataReader, ByVal map As Hashtable) As Hashtable
        ...
        map.Add("BIN_KB", "BIN_KB")
        map.Add("DEFAULT_UNSO_FLG", "DEFAULT_UNSO_FLG")
        map.Add("SYS_DEL_FLG", "SYS_DEL_FLG")
...
#Region "アクタス情報設定マスタマスタ"
    ...
    ''' <returns>DataSet</returns>
    ''' <remarks></remarks>
    Private Function SelectDataMInfoStateAts(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC114.SQL_SELECT_M_INFOSTATE_ATS)

        Me._SqlPrmList = New ArrayList()

        'パラメータ設定
        Call Me.SetMInfoStateParameter(Me._Row)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "SelectDataMInfoStateAts", cmd)
        ...
        If reader.HasRows() = True Then
            ...
            Call Me.SetInfoStateAtsMap(reader, map)

            ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMH030_M_INFOSTATE_ATS")
    ...
    ''' <summary>
    ''' アクタス情報設定マスタをHashTableに設定
    ''' </summary>
    ''' <param name="reader"></param>
    ''' <param name="map"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function SetInfoStateAtsMap(ByVal reader As SqlDataReader, ByVal map As Hashtable) As Hashtable

        '取得データの格納先をマッピング
        map.Add("NRS_BR_CD", "NRS_BR_CD")
        map.Add("CUST_CD_L", "CUST_CD_L")
        map.Add("CUST_CD_M", "CUST_CD_M")
        map.Add("DATA_SEND_CD", "DATA_SEND_CD")
        map.Add("VIA_CD", "VIA_CD")
        map.Add("UNSO_CD", "UNSO_CD")
        map.Add("UNSO_BR_CD", "UNSO_BR_CD")
        map.Add("MOTO_DATA_KB", "MOTO_DATA_KB")
        map.Add("UNSO_TEHAI_KB", "UNSO_TEHAI_KB")
        map.Add("TARIFF_BUNRUI_KB", "TARIFF_BUNRUI_KB")
        map.Add("BIN_KB", "BIN_KB")
        map.Add("OUTKA_ADDTION_DATE", "OUTKA_ADDTION_DATE")
        map.Add("ARR_ADDTION_DATE", "ARR_ADDTION_DATE")
        map.Add("DELIVERY_KBN", "DELIVERY_KBN")
        map.Add("DELIVERY_CD", "DELIVERY_CD")
        map.Add("LAST_DELIVERY_FLG", "LAST_DELIVERY_FLG")
        map.Add("DEFAULT_INFO_FLG", "DEFAULT_INFO_FLG")
        map.Add("SYS_DEL_FLG", "SYS_DEL_FLG")
...
#Region "届先マスタ"
    ...
    ''' <returns>DataSet</returns>
    ''' <remarks></remarks>
    Private Function SelectDataMDestTel(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC114.SQL_SELECT_M_DEST_TEL)

        Me._StrSql.Append(vbNewLine)
        Me._StrSql.Append("AND REPLACE(REPLACE(M_DEST.TEL,'-',''),' ','') = @DEST_TEL")

        Me._SqlPrmList = New ArrayList()

        'パラメータ設定
        Call Me.SetMdestTelParameter(dt)

        'SQL条件設定(Group by)
        Me._StrSql.Append(LMH030DAC114.SQL_GROUP_BY_M_DEST_TEL)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "SelectDataMDestTel", cmd)
        ...
        If reader.HasRows() = True Then
            ...
            Call Me.SetDestTelMap(reader, map)
...
#Region "届先マスタをHashTableに設定"
    ...
    Private Function SetDestTelMap(ByVal reader As SqlDataReader, ByVal map As Hashtable) As Hashtable
    ...
    Private Function UpdateOutkaEdiLData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            Case LMH030DAC.EventShubetsu.SAVEOUTKA, LMH030DAC.EventShubetsu.HIMODUKE
                setSql = LMH030DAC114.SQL_UPDATE_OUTKASAVEEDI_L
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "UpdateOutkaEdiLData", cmd)
...
#Region "H_OUTKAEDI_M"
    ...
    Private Function UpdateOutkaEdiMData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            '出荷登録、紐付けSQL CONST名
            Case LMH030DAC.EventShubetsu.SAVEOUTKA, LMH030DAC.EventShubetsu.HIMODUKE
                setSql = LMH030DAC114.SQL_UPDATE_OUTKAEDI_M
                loopflg = 1
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC114", "UpdateOutkaEdiMData", cmd)
...
#Region "出荷登録処理用EDIデータ 抽出処理"
    ...
    Private Function SelectEdiL(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC114.L_DEF_SQL_SELECT_DATA)      'SQL構築
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "SelectEdiL", cmd)
    ...
    Private Function SelectEdiM(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC114.M_DEF_SQL_SELECT_DATA)      'SQL構築(データ抽出用)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "SelectEdiM", cmd)
        ...
        map.Add("GOODSM_FUKAKUTEI_FLG", "GOODSM_FUKAKUTEI_FLG")   'ADD 2019/12/24 009299
...
#Region "商品変換(取込時)"
    ...
    Private Function CheckGoods(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim dtDTL As DataTable = ds.Tables("LMH030_EDI_TORIKOMI_DTL")
        Dim sNRS_BR_CD As String = ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("NRS_BR_CD").ToString
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC114.SQL_SELECT_Z_KBN_M031 _
                                                                       , sNRS_BR_CD))
        'SQLパラメータ初期化
        Me._SqlPrmList = New ArrayList()
        ...
        _SqlPrmList.Add(MyBase.GetSqlParameter("@NRS_BR_CD", Me.NullConvertString(sNRS_BR_CD), DBDataType.CHAR))
        _SqlPrmList.Add(MyBase.GetSqlParameter("@CUST_CD_L", Me.NullConvertString(ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("CUST_CD_L")), DBDataType.VARCHAR))
        _SqlPrmList.Add(MyBase.GetSqlParameter("@CUST_CD_M", Me.NullConvertString(ds.Tables("LMH030_SEMIEDI_INFO").Rows(0).Item("CUST_CD_M")), DBDataType.VARCHAR))
        _SqlPrmList.Add(MyBase.GetSqlParameter("@GOODS_CD", Me.NullConvertString(_Row.Item("COLUMN_7")), DBDataType.VARCHAR))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "CheckGoods", cmd)
...
#Region "SELECT_Z_KBN(商品コード変換)"
    ...
    Private Const SQL_SELECT_Z_KBN_M031 As String _
                    = " SELECT                                                     " & vbNewLine _
                    & " KBN_NM5                         AS    HENKAN_GOODS_CD      " & vbNewLine _
                    & " FROM                                                       " & vbNewLine _
                    & " $LM_MST$..Z_KBN                                            " & vbNewLine _
                    & " WHERE                                                      " & vbNewLine _
                    & " KBN_GROUP_CD = 'M031'                                      " & vbNewLine _
                    & " AND                                                        " & vbNewLine _
    ...
    Private Function UpdateOutkaLData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            Case LMH030DAC.EventShubetsu.SAVEOUTKA
                setSql = LMH030DAC114.SQL_UPDATE_OUTKAL_MATOME
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "UpdateOutkaLData", cmd)
...
#Region "まとめ先EDI出荷(大)"
    ...
    Private Function UpdateMatomesakiEdiLData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            '出荷登録SQL CONST名
            Case LMH030DAC.EventShubetsu.SAVEOUTKA
                setSql = LMH030DAC114.SQL_UPDATE_MATOMESAKI_OUTKAEDI_L
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "UpdateMatomesakiEdiLData", cmd)
...
#Region "まとめ先運送(大)"
    ...
    Private Function UpdateMatomesakiUnsoLData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            '出荷登録SQL CONST名
            Case LMH030DAC.EventShubetsu.SAVEOUTKA
                setSql = LMH030DAC114.SQL_UPDATE_MATOMESAKI_UNSO_L
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "UpdateMatomesakiUnsoLData", cmd)
...
#Region "M_DEST(自動更新用)"
    ...
    Private Function UpdateMDestData(ByVal ds As DataSet) As DataSet
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "UpdateMDestData", cmd)
...
#Region "H_OUTKAEDI_L(セミEDI)"
    ...
    Private Function InsertOutkaEdiL(ByVal setDs As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC114.SQL_INSERT_OUTKAEDI_L, Me._Row.Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "InsertOutkaEdiL", cmd)
...
#Region "H_OUTKAEDI_M(セミEDI)"
    ...
    Private Function InsertOutkaEdiM(ByVal setDs As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC114.SQL_INSERT_OUTKAEDI_M _
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "InsertOutkaEdiM", cmd)
...
#Region "H_OUTKAEDI_L(セミEDI)同一データ整合性差異有りの場合の更新"
    ...
    Private Function UpdateEditOutkaEdiL(ByVal setDs As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC114.SQL_UPDATE_EDIT_OUTKAEDI_L, Me._Row.Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "UpdateEditOutkaEdiL", cmd)
...
#Region "H_OUTKAEDI_M(セミEDI)同一データ整合性差異有りの場合の更新"
    ...
    Private Function UpdateEditOutkaEdiM(ByVal setDs As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC114.SQL_UPDATE_EDIT_OUTKAEDI_M, Me._Row.Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "UpdateEditOutkaEdiM", cmd)
...
#Region "C_OUTKA_L"
    ...
    Private Function InsertOutkaLData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC114.SQL_INSERT_OUTKA_L, Me._Row.Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "InsertOutkaLData", cmd)
...
#Region "C_OUTKA_M"
    ...
    Private Function InsertOutkaMData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC114.SQL_INSERT_OUTKA_M _
                                                                       , ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC114", "UpdateOutkaMData", cmd)
...
#Region "E_SAGYO_REC"
    ...
    Private Function InsertSagyoData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC114.SQL_INSERT_SAGYO _
                                                                       , ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC114", "InsertSagyoData", cmd)
...
#Region "UNSO_L"
    ...
    Private Function InsertUnsoLData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC114.SQL_INSERT_UNSO_L _
                                                                       , ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "InsertUnsoLData", cmd)
...
#Region "UNSO_M"
    ...
    Private Function InsertUnsoMData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC114.SQL_INSERT_UNSO_M _
                                                                       , ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC114", "InsertUnsoMData", cmd)
...
#Region "UNCHIN"
    ...
    Private Function InsertUnchinData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC114.SQL_INSERT_UNCHIN _
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC114", "InsertUnchinData", cmd)
...
#Region "M_DEST(自動追加用)"
    ...
    Private Function InsertMDestData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC114.SQL_INSERT_M_DEST, Me._Row.Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC114", "InsertMDestData", cmd)
...
#Region "商品マスタパラメータ設定"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub SetSelectMstGoodsCustParameter(ByVal dtL As DataTable, ByVal dtM As DataTable)

        Dim drL As DataRow = dtL.Rows(0)
        Dim drM As DataRow = dtM.Rows(0)

        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@NRS_BR_CD", drM.Item("NRS_BR_CD"), DBDataType.CHAR))
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@GOODS_CD_CUST", Me.NullConvertString(drM.Item("CUST_GOODS_CD")), DBDataType.NVARCHAR))
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@CUST_CD_L", Me.NullConvertString(drL.Item("CUST_CD_L")), DBDataType.CHAR))
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@CUST_CD_M", Me.NullConvertString(drL.Item("CUST_CD_M")), DBDataType.CHAR))

    End Sub

#End Region

#Region "届先マスタパラメータ設定(TEL用)"

    ''' <summary>
    ''' 届先マスタのパラメータ設定
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub SetMdestTelParameter(ByVal dt As DataTable)
        ...
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@DEST_TEL", Replace(Replace(dt.Rows(0).Item("FREE_C26").ToString(), "-", ""), " ", ""), DBDataType.NVARCHAR))

    End Sub

#End Region

#Region "運送会社設定マスタパラメータ設定"
    ''' <summary>
    ''' 運送会社設定マスタパラメータ設定
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub SetMGeniricUnsocoParameter(ByVal row As DataRow)
...
#End Region
    ...
    '2014.07.10 追加START
#Region "アクタス情報設定マスタパラメータ設定"
    ''' <summary>
    ''' アクタス情報設定マスタパラメータ設定
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub SetMInfoStateParameter(ByVal row As DataRow)

        Dim dBussinessDate As DateTime = Convert.ToDateTime(Me.GetSlashEditDate(row.Item("OUTKA_PLAN_DATE").ToString()))

        Dim intWeekday As Integer = Weekday(dBussinessDate)

        If Weekday(dBussinessDate) <> 2 AndAlso Weekday(dBussinessDate) <> 3 Then
            intWeekday = 0
        Else
            intWeekday = Weekday(dBussinessDate)
        End If

        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@NRS_BR_CD", row.Item("NRS_BR_CD"), DBDataType.CHAR))
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@WEEKDAY", intWeekday, DBDataType.NUMERIC))
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@FREE_C02", Me.NullConvertString(row.Item("FREE_C02")), DBDataType.NVARCHAR))
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@CUST_CD_L", Me.NullConvertString(row.Item("CUST_CD_L")), DBDataType.CHAR))
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@CUST_CD_M", Me.NullConvertString(row.Item("CUST_CD_M")), DBDataType.CHAR))
...
#Region "使用していない(中には後々有用なものもあります)"
