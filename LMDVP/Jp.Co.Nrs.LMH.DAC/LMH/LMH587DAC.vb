...
'  システム名       :  LM
'  サブシステム名   :  LMH       : 出荷管理
'  プログラムID     :  LMH587    : 納品書(埼玉BP・納品送状[オートバックス用])
'  作  成  者       :  篠田
' ==========================================================================
Imports System.Text
...
#Region "検索処理 SQL"
    ...
    Private Const SQL_SELECT_DATA As String = " SELECT                                                          " & vbNewLine _
                                            ...
                                            & "      , RIGHT('000000' + CONVERT(VARCHAR,BPHED.DENPYO_NO),6)   AS DENPYO_NO  " & vbNewLine _
                                            ...
                                            & "             THEN CAST(CEILING(BPDTL.ROW_NO / Z_KBN.VALUE2) AS INT)   " & vbNewLine _
                                            & "             ELSE 0                                              " & vbNewLine _
    ...
    Private Const SQL_FROM As String = " --出荷(大)                                                                                         " & vbNewLine _
                                     ...
                                     & "--(2013.06.17) 追加END                                                                            " & vbNewLine _
                                     & "       --商品Ｍ                                                                                     " & vbNewLine _
                                     & "--(2015.12.14 変更 要望番号2471対応 START)                                                            " & vbNewLine _
                                     ...
                                     & "--(2015.12.14 変更 要望番号2471対応 END)                                                            " & vbNewLine _
                                     ...
                                     & "                    AND M_CUSTRPT1.PTN_ID      = 'BN'                                               " & vbNewLine _
                                     ...
                                     & "                    AND M_CUSTRPT2.PTN_ID      = 'BN'                                               " & vbNewLine _
                                     ...
                                     & "                    AND MR3.PTN_ID             = 'BN'                                               " & vbNewLine _
                                     & "                    AND MR3.STANDARD_FLAG      = '01'                                               " & vbNewLine _
                                     & "                    AND MR3.SYS_DEL_FLG        = '0'                                                " & vbNewLine _
                                     & "        -- 外装名 ' 要望番号2471対応 added 2015.12.14                                                                                     " & vbNewLine _
...
#Region "検索処理"
    ...
    Private Function SelectMPrt(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim inTbl As DataTable = ds.Tables("LMH587IN")
        ...
        'SQL作成
        Me._StrSql.Append(LMH587DAC.SQL_SELECT_MPrt)      'SQL構築(帳票種別用Select句)
        Me._StrSql.Append(LMH587DAC.SQL_FROM)             'SQL構築(データ抽出用From句)
        ...
        MyBase.Logger.WriteSQLLog("LMH587DAC", "SelectMPrt", cmd)
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH587DAC.SQL_SELECT_DATA)      'SQL構築(データ抽出用Select句)
        Me._StrSql.Append(LMH587DAC.SQL_FROM)             'SQL構築(データ抽出用From句)

        ''(2013.02.27) BP新システム切替対応START
        Me._StrSql.Append(LMH587DAC.SQL_WHERE)            'SQL構築(データ抽出用WHERE句)(新規)
        Me._StrSql.Append(LMH587DAC.SQL_EXISTS)           'SQL構築(データ抽出用SELECT句(AND EXISTS))(新規)
        ''(2013.02.27) BP新システム切替対応END

        '(2013.02.05)要望番号1822 -- START --
        'If Me._Row.Item("PRTFLG").ToString = "1" Then
        '    '出力済の場合
        '    'Me._StrSql.Append(LMH587DAC.SQL_WHERE)      'SQL構築(データ抽出用Where句)
        '    Call Me.SetConditionMasterSQL_OUT()
        'Else
        '    '未出力の場合
        '    Call Me.SetConditionMasterSQL()
        ...
        ''(2013.02.27) BP新システム切替対応START
        Me._StrSql.Append(LMH587DAC.SQL_EXISTS_WHERE) 'SQL構築(データ抽出用WHERE句(AND EXISTS))(新規)
        ''(2013.02.27) BP新システム切替対応END

        Me._StrSql.Append(LMH587DAC.SQL_ORDER_BY)         'SQL構築(データ抽出用ORDER BY句)
        Call Me.SetConditionPrintPatternMSQL()            '条件設定
        ...
        MyBase.Logger.WriteSQLLog("LMH587DAC", "SelectPrintData", cmd)
...
#End If
        ...
        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMH587OUT")
