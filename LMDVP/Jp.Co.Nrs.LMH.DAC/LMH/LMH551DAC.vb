...
'  システム名       :  LM
'  サブシステム名   :  LMH       : EDI
'  プログラムID     :  LMH551    : EDI出荷伝票(埼玉_大日精化用)
'  作  成  者       :  大貫和正
' ==========================================================================
Imports System.Text
...
#Region "検索処理 SQL"
    ...
    ''' </remarks>
    Private Const SQL_FROM As String = "  FROM $LM_TRN$..H_OUTKAEDI_HED_DNS HED                              " & vbNewLine _
                                     ...
                                     & "       -- EDI出荷(大)                                                " & vbNewLine _
                                     & "       LEFT OUTER JOIN $LM_TRN$..H_OUTKAEDI_L EDIL                   " & vbNewLine _
                                     & "                    ON EDIL.NRS_BR_CD  = HED.NRS_BR_CD               " & vbNewLine _
                                     & "                   AND EDIL.EDI_CTL_NO = HED.EDI_CTL_NO              " & vbNewLine _
                                     & "        --           AND EDIL.DEL_KB     = HED.DEL_KB                  " & vbNewLine _
                                     & "       -- 商品マスタ                                                 " & vbNewLine _
                                     ...
                                     & "                   AND M_CUSTRPT1.PTN_ID      = '98'                 " & vbNewLine _
                                     ...
                                     & "                   AND M_CUSTRPT2.PTN_ID      = '98'                 " & vbNewLine _
                                     ...
                                     & "                   AND MR3.PTN_ID             = '98'                 " & vbNewLine _
                                     & "                   AND MR3.STANDARD_FLAG      = '01'                 " & vbNewLine _
                                     & "                   AND MR3.SYS_DEL_FLG        = '0'                  " & vbNewLine _
                                     & "       LEFT OUTER JOIN $LM_MST$..Z_KBN Z5                           " & vbNewLine _
                                     & "                    ON EDIL.DEL_KB = Z5.KBN_CD                       " & vbNewLine _
                                     & "                  AND Z5.KBN_GROUP_CD = 'E011'                  " & vbNewLine _
                                     & "       LEFT OUTER JOIN $LM_TRN$..C_OUTKA_L OUTL                           " & vbNewLine _
                                     & "                    ON HED.NRS_BR_CD          = OUTL.NRS_BR_CD       " & vbNewLine _
                                     & "                   AND HED.OUTKA_CTL_NO       = OUTL.OUTKA_NO_L                 " & vbNewLine _
    ...
    Private Const SQL_SELECT As String = " SELECT                                                           " & vbNewLine _
                                       ...
                                       & "      , MAIN.DEL_KB           AS DEL_KB                           " & vbNewLine _
                                       ...
                                       & "      , MAIN.ZANSU1           AS ZANSU1                           " & vbNewLine _
                                       & "      , MAIN.ZANSU2           AS ZANSU2                           " & vbNewLine _
                                       & "      , MAIN.ZANSU3           AS ZANSU3                           " & vbNewLine _
                                       & "      , (MAIN.YOURYO1*MAIN.KOSU1) + (MAIN.YOURYO2*MAIN.KOSU2) + (MAIN.YOURYO3*MAIN.KOSU3) AS SUM_SURYO " & vbNewLine _
                                       ...
                                       & "               , Z5.KBN_CD           AS HORYU                   " & vbNewLine _
                                       ...
                                       & "               --【残数01】                                       " & vbNewLine _
                                       & "               , ISNULL((SELECT DTL.ZANSU                         " & vbNewLine _
                                       & "                           FROM $LM_TRN$..H_OUTKAEDI_DTL_DNS DTL  " & vbNewLine _
                                       & "                          WHERE DTL.CRT_DATE  = HED.CRT_DATE      " & vbNewLine _
                                       & "                            AND DTL.FILE_NAME = HED.FILE_NAME     " & vbNewLine _
                                       & "                            AND DTL.REC_NO    = HED.REC_NO        " & vbNewLine _
                                       & "                            AND DTL.GYO       = '01'              " & vbNewLine _
                                       & "                 ),0) AS ZANSU1                                   " & vbNewLine _
                                       & "               --【残数02】                                       " & vbNewLine _
                                       & "               , ISNULL((SELECT DTL.ZANSU                         " & vbNewLine _
                                       & "                           FROM $LM_TRN$..H_OUTKAEDI_DTL_DNS DTL  " & vbNewLine _
                                       & "                          WHERE DTL.CRT_DATE  = HED.CRT_DATE      " & vbNewLine _
                                       ...
                                       & "                            AND DTL.REC_NO    = HED.REC_NO        " & vbNewLine _
                                       & "                            AND DTL.GYO       = '03'              " & vbNewLine _
                                       & "                 ),0) AS ZANSU3                                   " & vbNewLine _
                                       & "               , EDIL.UNSO_CD    AS UNSO_CD                       " & vbNewLine _
                                       & "               , EDIL.UNSO_NM    AS UNSO_NM                       " & vbNewLine _
                                       & "               , EDIL.UNSO_BR_CD AS UNSO_BR_CD                    " & vbNewLine _
                                       & "               , EDIL.UNSO_BR_NM AS UNSO_BR_NM                    " & vbNewLine
    ...
    Private Const SQL_GROUP_BY As String = " GROUP BY                     " & vbNewLine _
                                         ...
                                         & "        , EDIL.UNSO_CD        " & vbNewLine _
                                         & "        , EDIL.UNSO_NM        " & vbNewLine _
                                         & "        , EDIL.UNSO_BR_CD     " & vbNewLine _
                                         & "        , EDIL.UNSO_BR_NM     " & vbNewLine _
                                         & "        , Z5.KBN_CD           " & vbNewLine _
                                         & " ) MAIN                       " & vbNewLine
...
#Region "検索処理"
    ...
    Private Function SelectMPrintPattern(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim inTbl As DataTable = ds.Tables("LMH551IN")
        ...
        'SQL作成
        Me._StrSql.Append(LMH551DAC.SQL_MPrt_SELECT)    'SQL構築(帳票種別用SELECT句)
        Me._StrSql.Append(LMH551DAC.SQL_FROM)      'SQL構築(帳票種別用FROM句)
        ...
        MyBase.Logger.WriteSQLLog("LMH551DAC", "SelectMPrt", cmd)
    ...
    ''' <summary>
    ''' 大日精化EDI受信データ(HEAD)・大日精化EDI受信データ(DETAIL)対象データ検索
    ''' </summary>
    ''' <param name="ds">DataSet</param>
    ''' <returns>DataSet</returns>
    ''' <remarks>大日精化EDI受信データ(HEAD)・(DETAIL)対象データ一覧検索結果取得SQLの構築・発行</remarks>
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH551DAC.SQL_SELECT)         'SQL構築(印刷データ抽出用 SELECT句)
        Me._StrSql.Append(LMH551DAC.SQL_FROM)           'SQL構築(印刷データ抽出用 FROM句)
        If Me._Row.Item("PRTFLG").ToString = "1" Then
            Call Me.SetConditionMasterSQL_OUT()         '出力済の場合
        Else
            Call Me.SetConditionMasterSQL()             '未出力・両方(出力済、未出力併せて)
        End If
        Me._StrSql.Append(LMH551DAC.SQL_GROUP_BY)       'SQL構築(印刷データ抽出用 GROUP BY句)
        Me._StrSql.Append(LMH551DAC.SQL_ORDER_BY)       'SQL構築(印刷データ抽出用 ORDER BY句)
        ...
        MyBase.Logger.WriteSQLLog("LMH551DAC", "SelectPrintData", cmd)
        ...
        map.Add("ZANSU1", "ZANSU1")
        map.Add("ZANSU2", "ZANSU2")
        map.Add("ZANSU3", "ZANSU3")
        map.Add("SUM_SURYO", "SUM_SURYO")
        map.Add("UNSO_CD", "UNSO_CD")
        map.Add("UNSO_NM", "UNSO_NM")
        map.Add("UNSO_BR_CD", "UNSO_BR_CD")
        map.Add("UNSO_BR_NM", "UNSO_BR_NM")

        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMH551OUT")
        ...
        With Me._Row
            ...
            Select Case whereStr
                ...
                Case "0"
                    ...
                    '2015/6/8 保留の場合は未出力から印刷しない
                    Me._StrSql.Append(" AND EDIL.DEL_KB = '0'  ")
                    Me._StrSql.Append(" AND (HED.DEL_KB = '0' or ( HED.DEL_KB = '1' AND EDIL.OUTKA_CTL_NO <> '000000000')) ")
                Case "1"
                    '出力済
                    Me._StrSql.Append(" AND (HEDIPRINT.PRT_COUNT >= 1 ) ")
                    '20150706 出
                    Me._StrSql.Append(" AND EDIL.DEL_KB= '0'  ")
                    Me._StrSql.Append(" AND (HED.DEL_KB = '0' or ( HED.DEL_KB = '1' AND (OUTL.SYS_DEL_FLG = '0' or OUTL.SYS_DEL_FLG IS NULL ) ))   ")
            ...
            If String.IsNullOrEmpty(whereStr) = False Then
                Me._StrSql.Append(" AND EDIL.CUST_CD_L = @CUST_CD_L ")
                Me._StrSql.Append(vbNewLine)
            End If

            whereStr = .Item("CUST_CD_M").ToString()
            If String.IsNullOrEmpty(whereStr) = False Then
                Me._StrSql.Append(" AND EDIL.CUST_CD_M = @CUST_CD_M ")
                Me._StrSql.Append(vbNewLine)
            End If
