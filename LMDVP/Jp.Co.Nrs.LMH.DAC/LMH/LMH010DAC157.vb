...
'  システム名       :  LM
'  サブシステム名   :  LMH             : EDI
'  プログラムID     :  LMH010DAC157    : EDI入荷検索
'  EDI荷主ID　　　　:  157              : シグマアルドリッチジャパン
'  作  成  者       :  
' ==========================================================================
Imports System.Text
...
#Region "EDI受信(DTL)テーブル 件数 SELECT SQL"

    ''' <summary>
    ''' EDI受信(DTL)テーブル 件数 SELECT SQL
    ''' </summary>
    ''' <remarks></remarks>

    Private Const SQL_SELECT_COUNT_INKAEDI_DTL As String = "" _
        & " SELECT                     " & vbNewLine _
...
#End Region ' "EDI受信(DTL)テーブル 件数取得 SQL"
...
#Region "H_INKAEDI_DTL_SGM(INSERT)"
    ...
    Private Const SQL_INSERT_INKAEDI_DTL As String _
                                        ...
                                        & " ,BILL_OF_LADING             " & vbNewLine _
                                        & " ,CUST_GOODS_CD              " & vbNewLine _
                                        & " ,NB                         " & vbNewLine _
                                        & " ,LOT_NO                     " & vbNewLine _
                                        & " ,UT                         " & vbNewLine _
                                        & " ,COMPLIANCE                 " & vbNewLine _
                                        & " ,JFC                        " & vbNewLine _
                                        & " ,PDC                        " & vbNewLine _
                                        & " ,ROAN_DISP                  " & vbNewLine _
                                        & " ,F_F                        " & vbNewLine _
                                        & " ,ONDO                       " & vbNewLine _
                                        & " ,OUTKA_POINT                " & vbNewLine _
...
#Region "EDI入荷データ件数および入荷データL 入荷管理番号L 等 SELECT SQL"
    ...
    Private Const SQL_SELECT_INKA_NO_L_AND_CNT As String = "" _
        ...
        & "AND H_INKAEDI_L.OUTKA_FROM_ORD_NO = @OUTKA_FROM_ORD_NO_L   " & vbNewLine _
...
#Region "H_INKAEDI_L(セミEDI)"
    ...
    Private Function InsertInkaEdiL(ByVal setDs As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH010DAC157.SQL_INSERT_INKAEDI_L, Me._Row.Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "InsertInkaEdiL", cmd)
...
#Region "H_INKAEDI_M(セミEDI)"
    ...
    Private Function InsertInkaEdiM(ByVal setDs As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH010DAC157.SQL_INSERT_INKAEDI_M _
        ...
        'パラメータ設定
        Call Me.SetInsertInkaEdiMParameter(Me._Row, Me._SqlPrmList, LMH010DAC.EventShubetsu.TORIKOMI)
        Call Me.SetDataInsertParameter()
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "InsertInkaEdiM", cmd)
...
#Region "EDI受信(DTL)テーブル 件数取得"
    ...
    ''' <returns></returns>
    Private Function GetInkaEdiCnt(ByVal setDs As DataSet) As DataSet

        ' SQL格納変数の初期化
        Me._StrSql = New StringBuilder()

        ' SQL作成
        Me._StrSql.Append(LMH010DAC157.SQL_SELECT_COUNT_INKAEDI_DTL)

        Dim dt As DataTable = setDs.Tables("LMH010_INKAEDI_DTL_SGM")

        Me._SqlPrmList = New ArrayList()

        ' パラメータ設定
        Call Me.SetSqlParamGetInkaEdiCnt(dt)
        '受信DTLテーブル名設定
        ...
        Me._StrSql.Replace("$RCV_DTL$", RcvNm)
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "GetInkaEdiCnt", cmd)
...
#End Region ' "EDI受信(DTL)テーブル 件数取得"
...
#Region "EDI受信(DTL)テーブル 物理削除"
    ...
    Private Function DeleteInkaEdi(ByVal setDs As DataSet) As DataSet
        ...
        ' SQL作成
        Me._StrSql.Append(LMH010DAC157.SQL_DELETE_INKAEDI_DTL)
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "DeleteInkaEdi", cmd)
...
#Region "H_INKAEDI_DTL_SGM(セミEDI)"
    ...
    Private Function InsertInkaEdiRcvDtl(ByVal setDs As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim dtInkaEdiRcvDtl As DataTable = setDs.Tables("LMH010_INKAEDI_DTL_SGM")

        'スキーマ名設定
        Dim sql As String = Me.SetSchemaNm(LMH010DAC157.SQL_INSERT_INKAEDI_DTL, setDs.Tables("LMH010_INKAEDI_DTL_SGM").Rows(0).Item("NRS_BR_CD").ToString())
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "InsertInkaEdiRcvDtl", cmd)
...
#Region "商品マスタ(セミEDI：受信情報からの抽出)"
    ...
    Private Function SelectMstGoods(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH010_INKAEDI_DTL_SGM")
        ...
        'SQL作成
        Me._StrSql.Append(LMH010DAC157.SQL_SELECT_M_GOODS)

        Me._SqlPrmList = New ArrayList()

        'SQL条件設定(Where)
        Me._StrSql.Append(LMH010DAC157.SQL_WHERE1_M_GOODS)

        'SQL条件設定(Group by)
        Me._StrSql.Append(LMH010DAC157.SQL_GROUP_BY_M_GOODS)
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "SelectMstGoods", cmd)
...
#Region "セミEDI入力ファイル(Excel Book) 伝票番号からの、EDI入荷件数および入荷管理番号L 等 の取得"

    ''' <summary>
    ''' セミEDI入力ファイル(Excel Book) 伝票番号からの、EDI入荷件数および入荷管理番号L 等 の取得
    ''' </summary>
    ''' <param name="ds"></param>
    ''' <returns></returns>
    Private Function GetInkaCntAndNoL(ByVal ds As DataSet) As DataSet

        ' SQL格納変数の初期化
        Me._StrSql = New StringBuilder()

        ' SQL作成
        Me._StrSql.Append(LMH010DAC157.SQL_SELECT_INKA_NO_L_AND_CNT)

        Dim dt As DataTable = ds.Tables("LMH010_INKAEDI_DTL_SGM")

        Me._SqlPrmList = New ArrayList()

        ' パラメータ設定
        Call Me.SetSqlParamGetInkaCntAndNoL(dt)
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "GetInkaCntAndNoL", cmd)
        ...
        ' 処理件数の設定
        MyBase.SetResultCount(Convert.ToInt32(reader("INKA_CNT")))
...
#End Region ' "セミEDI入力ファイル(Excel Book) 伝票番号からの、EDI入荷件数および入荷管理番号L 等 の取得"
...
#Region "商品マスタ(セミEDI：荷主商品コードより抽出)"
    ...
    Private Function CheckGoods(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH010DAC157.SQL_SELECT_M_GOODS_CNT)
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "CheckGoods", cmd)
...
#Region "EDI入荷(大)テーブル更新(論理削除)"
    ...
    Private Function UpdateDelInkaEdiL(ByVal ds As DataSet) As DataSet
        ...
        ' SQL作成
        Me._StrSql.Append(LMH010DAC157.SQL_UPDATE_DEL_INKAIEDI_L)
        ...
        ' パラメータ設定
        Call Me.SetSqlParamUpdateDelInkaEdiL(dt)
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "UpdateDelInkaEdiL", cmd)
...
#Region "EDI入荷(中)テーブル更新(論理削除)"
    ...
    Private Function UpdateDelInkaEdiM(ByVal ds As DataSet) As DataSet
        ...
        ' SQL作成
        Me._StrSql.Append(LMH010DAC157.SQL_UPDATE_DEL_INKAIEDI_M)
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "UpdateDelInkaEdiM", cmd)
...
#Region "RCV_DTL(入荷登録)"
    ...
    Private Const SQL_UPDATE_RCV_DTL As String = "UPDATE                                            " & vbNewLine _
                                              & " $LM_TRN$..H_INKAEDI_DTL_SGM                       " & vbNewLine _
                                              & " SET                                               " & vbNewLine _
                                              & " INKA_CTL_NO_L     = @INKA_CTL_NO_L    	        " & vbNewLine _
                                              & ",INKA_CTL_NO_M     = @INKA_CTL_NO_M        	    " & vbNewLine _
...
#Region "EDI(大)データ取得処理"
    ...
    Private Function SelectEdiL(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH010DAC157.SQL_SELECT_EDI_L)
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "SelectEdiL", cmd)
...
#Region "EDI(中)データ取得処理"
    ...
    Private Function SelectEdiM(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH010DAC157.SQL_SELECT_EDI_M)
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "SelectEdiM", cmd)
...
#Region "SQL パラメータ設定 EDI受信(DTL)テーブル 件数取得 用"

    ''' <summary>
    ''' SQL パラメータ設定 EDI受信(DTL)テーブル 件数取得 用
    ''' </summary>
    ''' <param name="dt"></param>
    Private Sub SetSqlParamGetInkaEdiCnt(ByVal dt As DataTable)
...
#End Region ' "SQL パラメータ設定 EDI受信(DTL)テーブル 件数取得 用"
...
#Region "SQL パラメータ設定 EDI受信(DTL)テーブル 物理削除 用"
    ...
    Private Sub SetSqlParamDeleteInkaEdi(ByVal dt As DataTable)
        ...
        ' 件数取得用パラメータ設定と兼用する。
        Call SetSqlParamGetInkaEdiCnt(dt)
...
#Region "EDI受信(DTL)新規追加パラメータ設定"
    ...
    Private Sub SetInsertInkaEdiRcvDtlParameter(ByVal conditionRow As DataRow, ByVal prmList As ArrayList)
        ...
        With conditionRow
            ...
            prmList.Add(MyBase.GetSqlParameter("@OUTKA_FROM_ORD_NO_L", Me.NullConvertString(.Item("OUTKA_FROM_ORD_NO_L")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@BILL_OF_LADING", Me.NullConvertString(.Item("BILL_OF_LADING")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@CUST_GOODS_CD", Me.NullConvertString(.Item("CUST_GOODS_CD")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@NB", Me.NullConvertString(.Item("NB")), DBDataType.NUMERIC))
            prmList.Add(MyBase.GetSqlParameter("@LOT_NO", Me.NullConvertString(.Item("LOT_NO")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@UT", Me.NullConvertString(.Item("UT")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@COMPLIANCE", Me.NullConvertString(.Item("COMPLIANCE")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@JFC", Me.NullConvertString(.Item("JFC")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@PDC", Me.NullConvertString(.Item("PDC")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@ROAN_DISP", Me.NullConvertString(.Item("ROAN_DISP")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@F_F", Me.NullConvertString(.Item("F_F")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@ONDO", Me.NullConvertString(.Item("ONDO")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@OUTKA_POINT", Me.NullConvertString(.Item("OUTKA_POINT")), DBDataType.NVARCHAR))
...
#Region "まとめ先データ取得"
    ...
    Private Function SelectMatomeTarget(ByVal ds As DataSet) As DataSet
        ...
        Select Case matomeFlg
            Case "2"
                Me._StrSql.Append(LMH010DAC157.SQL_SELECT_MATOME_TARGET)

        End Select
        Dim matomeTargets As String = Me.GetMatomeTargetInkaNoL(ds)

        'SQL作成
        Me._StrSql.Append(String.Format(LMH010DAC157.SQL_SELECT_MATOME_TARGET, matomeTargets))
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "SelectMatomeTarget", cmd)
    ...
    Private Function GetMatomeFlg(ByVal ds As DataSet) As String
        ...
        Me._StrSql.Append(LMH010DAC157.SQL_SELECT_MATOMEFLG)
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "GetMatomeFlg", cmd)
    ...
    Private Function UpdateEdiL(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(eventShubetsu, LMH010DAC.EventShubetsu)
            ...
            Case LMH010DAC.EventShubetsu.TOROKU, LMH010DAC.EventShubetsu.HIMODUKE
                ...
                setSql = LMH010DAC157.SQL_UPD_INKATOROKU_EDI_L
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "UpdateEdiL", cmd)
    ...
    Private Function UpdateEdiM(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(eventShubetsu, LMH010DAC.EventShubetsu)
            ...
            Case LMH010DAC.EventShubetsu.TOROKU, LMH010DAC.EventShubetsu.HIMODUKE
                ...
                setSql = LMH010DAC157.SQL_UPD_INKATOROKU_EDI_M
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH010DAC157", "UpdateEdiM", cmd)
...
#Region "RCV_DTL"
    ...
    Private Function UpdateRcvDtl(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(eventShubetsu, LMH010DAC.EventShubetsu)
            '入荷登録
            Case LMH010DAC.EventShubetsu.TOROKU, LMH010DAC.EventShubetsu.HIMODUKE
                setSql = LMH010DAC157.SQL_UPDATE_RCV_DTL
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH010DAC157", "UpdateRcvDtl", cmd)
...
#Region "INKA_L"
    ...
    Private Function InsertInkaL(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH010DAC157.SQL_INSERT_INKA_L, dtRow.Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "InsertInkaL", cmd)
...
#Region "INKA_M"
    ...
    Private Function InsertInkaM(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH010DAC157.SQL_INSERT_INKA_M, brCd))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH010DAC157", "InsertInkaM", cmd)
...
#Region "INKA_S"
    ...
    Private Function InsertInkaS(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH010DAC157.SQL_INSERT_INKA_S, brCd))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH010DAC157", "InsertInkaS", cmd)
...
#Region "E_SAGYO_REC"
    ...
    Private Function InsertSagyo(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH010DAC157.SQL_INSERT_SAGYO, nrsBrCd))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH010DAC157", "InsertSagyo", cmd)
...
#Region "UNSO_L"
    ...
    Private Function InsertUnsoL(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH010DAC157.SQL_INSERT_UNSO_L, nrsBrCd))
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "InsertUnsoL", cmd)
...
#Region "UNSO_M"
    ...
    Private Function InsertUnsoM(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH010DAC157.SQL_INSERT_UNSO_M, nrsBrCd))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH010DAC157", "InsertUnsoM", cmd)
...
#Region "UNCHIN"
    ...
    Private Function InsertUnchinData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH010DAC157.SQL_INSERT_UNCHIN _
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH010DAC157", "InsertUnchinData", cmd)
...
#Region "INKA_L（まとめ登録）"
    ...
    Private Function UpdateInkaLMatome(ByVal ds As DataSet) As DataSet
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "UpdateInkaLMatome", cmd)
...
#Region "UNSO_L（まとめ登録）"
    ...
    Private Function UpdateUnsoLMatome(ByVal ds As DataSet) As DataSet
        ...
        setSql = LMH010DAC157.SQL_UPDATE_MATOMESAKI_UNSO_L
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "UpdateUnsoLMatome", cmd)
...
#Region "まとめ先EDI入荷(大)"
    ...
    Private Function UpdateMatomesakiEdiLData(ByVal ds As DataSet) As DataSet
        ...
        'イベント種別の判断
        Select Case DirectCast(dtEventShubetsu, LMH010DAC.EventShubetsu)

            '出荷登録SQL CONST名
            Case LMH010DAC.EventShubetsu.TOROKU
                setSql = LMH010DAC157.SQL_UPDATE_MATOMESAKI_INKAEDI_L
        ...
        MyBase.Logger.WriteSQLLog("LMH010DAC157", "UpdateMatomesakiEdiLData", cmd)
...
#Region "商品マスタパラメータ設定(セミEDI用)"
    ...
    Private Sub SetSelectChkGoodsParameter(ByVal semiInfoDr As DataRow, ByVal dtRcvDtl As DataTable)
        ...
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@GOODS_CD_CUST", dtRcvDtl.Rows(0).Item(GetColumnName(SGMArrivalColumns.CUST_GOODS_CD)).ToString(), DBDataType.NVARCHAR))
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GetColumnName(column As SGMArrivalColumns) As String
...
#Region "SQL パラメータ設定 セミEDI入力ファイル(Excel Book) 伝票番号からの、EDI入荷件数および入荷管理番号L 等 の取得 用"

    ''' <summary>
    ''' SQL パラメータ設定 セミEDI入力ファイル(Excel Book) 伝票番号からの、EDI入荷件数および入荷管理番号L 等 の取得 用
    ''' </summary>
    ''' <param name="dt"></param>
    Private Sub SetSqlParamGetInkaCntAndNoL(ByVal dt As DataTable)
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@NRS_BR_CD", dt.Rows(0).Item("NRS_BR_CD"), DBDataType.CHAR))
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@CUST_CD_L", dt.Rows(0).Item("CUST_CD_L"), DBDataType.CHAR))
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@CUST_CD_M", dt.Rows(0).Item("CUST_CD_M"), DBDataType.CHAR))
        Me._SqlPrmList.Add(MyBase.GetSqlParameter("@OUTKA_FROM_ORD_NO_L", dt.Rows(0).Item("OUTKA_FROM_ORD_NO_L"), DBDataType.CHAR))
    End Sub

#End Region ' "SQL パラメータ設定 セミEDI入力ファイル(Excel Book) 伝票番号からの、EDI入荷件数および入荷管理番号L 等 の取得 用"

#Region "SQL パラメータ設定 EDI入荷(大・中共通)テーブル更新(論理削除) 用"

    ''' <summary>
    ''' SQL パラメータ設定 EDI入荷(大・中共通)テーブル更新(論理削除) 用
    ''' </summary>
    ''' <param name="dt"></param>
    Private Sub SetSqlParamUpdateDelInkaEdiL(ByVal dt As DataTable)
...
#End Region '  "SQL パラメータ設定 EDI入荷(大・中共通)テーブル更新(論理削除) 用"
