...
'  EDI荷主ID　　　　:  207　　　 : JC,JC大秦化工,アイカ工業(大阪)
'  作  成  者       :  umano
' ==========================================================================
Imports System.Text
...
#Region "H_OUTKAEDI_RCV_CAN 取消データ抽出用"
    ...
    Private Const SQL_SELECT_OUTKAEDI_RCV_CAN As String = _
                  " SELECT                                                  " & vbNewLine _
                & " H_OUTKAEDI_HED.CRT_DATE            AS CRT_DATE          " & vbNewLine _
                & ",H_OUTKAEDI_HED.FILE_NAME           AS FILE_NAME         " & vbNewLine _
                & ",H_OUTKAEDI_HED.REC_NO              AS REC_NO            " & vbNewLine _
                & ",H_OUTKAEDI_HED.NRS_BR_CD           AS NRS_BR_CD         " & vbNewLine _
                & ",H_OUTKAEDI_HED.EDI_CTL_NO          AS EDI_CTL_NO        " & vbNewLine _
                & ",H_OUTKAEDI_HED.OUTKA_CTL_NO        AS OUTKA_CTL_NO      " & vbNewLine _
                & ",H_OUTKAEDI_HED.CANCEL_FLG          AS CANCEL_FLG        " & vbNewLine _
                & " FROM                                                    " & vbNewLine _
                & " $LM_TRN$..$RCV_HED$                H_OUTKAEDI_HED       " & vbNewLine _
                & " WHERE H_OUTKAEDI_HED.DEL_KB        = '0'                " & vbNewLine _
                & " AND   H_OUTKAEDI_HED.NRS_BR_CD     = @NRS_BR_CD         " & vbNewLine _
                & " AND   H_OUTKAEDI_HED.CUST_CD_L     = @CUST_CD_L         " & vbNewLine _
                & " AND   H_OUTKAEDI_HED.CUST_CD_M     = @CUST_CD_M         " & vbNewLine _
                & " AND   H_OUTKAEDI_HED.DENP_NO       = @DENP_NO           " & vbNewLine _
...
#Region "H_OUTKAEDI_HED_JC"
    ''' <summary>
    ''' EDI受信HEDのUPDATE文（H_INOUTKAEDI_HED_JC）
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_UPDATE_OUTKASAVE_EDI_RCV_HED_1 As String = "UPDATE $LM_TRN$..H_OUTKAEDI_HED_JC SET       " & vbNewLine _
                                              & " OUTKA_CTL_NO      = @OUTKA_CTL_NO       	        " & vbNewLine

    Private Const SQL_UPDATE_OUTKASAVE_EDI_RCV_HED_2 As String = ",OUTKA_USER        = @UPD_USER    " & vbNewLine _
                                              & ",OUTKA_DATE        = @UPD_DATE                     " & vbNewLine _
                                              & ",OUTKA_TIME        = @UPD_TIME                     " & vbNewLine

    Private Const SQL_UPDATE_OUTKASAVE_EDI_RCV_HED_3 As String = ",UPD_USER          = @UPD_USER    " & vbNewLine _
                                              & ",UPD_DATE          = @UPD_DATE                     " & vbNewLine _
                                              & ",UPD_TIME          = @UPD_TIME                     " & vbNewLine _
                                              & ",SYS_UPD_DATE      = @SYS_UPD_DATE                 " & vbNewLine _
                                              & ",SYS_UPD_TIME      = @SYS_UPD_TIME                 " & vbNewLine _
...
#Region "H_OUTKAEDI_DTL_JC"
    ''' <summary>
    ''' EDI受信DTLのUPDATE文（H_OUTKAEDI_DTL_JC）
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_UPDATE_OUTKASAVE_EDI_RCV_DTL As String = "UPDATE $LM_TRN$..H_OUTKAEDI_DTL_JC SET       " & vbNewLine _
                                              & " OUTKA_CTL_NO      = @OUTKA_CTL_NO       	        " & vbNewLine _
                                              & ",OUTKA_CTL_NO_CHU  = @OUTKA_CTL_NO_CHU       	    " & vbNewLine _
                                              & ",UPD_USER          = @UPD_USER                     " & vbNewLine _
                                              & ",UPD_DATE          = @UPD_DATE                     " & vbNewLine _
...
#Region "M_DEST(JC,JC大秦化工,アイカ専用)INSERT"
...
#Region "M_DEST(JC,JC大秦化工,アイカ専用)UPDATE(届先項目差異有りの場合)"
    ''' <summary>
    ''' 届先マスタUPDATE文（M_DEST）
    ''' </summary>
    ''' <remarks></remarks>
...
#Region "M_DEST(JC,JC大秦化工,アイカ専用)UPDATE(荷送人コードより届先名称入替の場合)"
    ''' <summary>
    ''' 届先マスタUPDATE文（M_DEST）
    ''' </summary>
    ''' <remarks></remarks>
...
#Region "出荷登録処理"
    ...
    Private Function SelectEdiL(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC207.L_DEF_SQL_SELECT_DATA)      'SQL構築
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "SelectEdiL", cmd)
    ...
    Private Function SelectEdiM(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC207.M_DEF_SQL_SELECT_DATA)      'SQL構築(データ抽出用)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "SelectEdiM", cmd)
...
#Region "存在チェック(出荷登録用)"
    ...
    Private Function SelectDataNcgoMdest(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC207.SQL_SELECT_COUNT_M_DEST)
        ...
        Me._StrSql.Append(LMH030DAC207.SQL_GROUP_BY_M_DEST_COUNT)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "SelectDataNcgoMdest", cmd)
...
#Region "H_OUTKAEDI_L"
    ...
    Private Function UpdateOutkaEdiLData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            '出荷登録、紐付けSQL CONST名
            Case LMH030DAC.EventShubetsu.SAVEOUTKA, LMH030DAC.EventShubetsu.HIMODUKE _
               , LMH030DAC.EventShubetsu.SAVEUNSO
                '2012.03.26 大阪対応修正END

                setSql = LMH030DAC207.SQL_UPDATE_OUTKASAVEEDI_L
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateOutkaEdiLData", cmd)
    ...
    Private Function UpdateOutkaEdiMData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            Case LMH030DAC.EventShubetsu.SAVEOUTKA, LMH030DAC.EventShubetsu.HIMODUKE _
               , LMH030DAC.EventShubetsu.SAVEUNSO
                '2012.03.26 大阪対応修正END
                setSql = LMH030DAC207.SQL_UPDATE_OUTKAEDI_M
                loopflg = 1
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateOutkaEdiMData", cmd)
    ...
    Private Function UpdateEdiRcvHedData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            Case LMH030DAC.EventShubetsu.SAVEOUTKA, LMH030DAC.EventShubetsu.HIMODUKE
                '2012.03.26 大阪対応修正END
                setSql = LMH030DAC207.SQL_UPDATE_OUTKASAVE_EDI_RCV_HED_1
                setSql = String.Concat(setSql, LMH030DAC207.SQL_UPDATE_OUTKASAVE_EDI_RCV_HED_2)
                setSql = String.Concat(setSql, LMH030DAC207.SQL_UPDATE_OUTKASAVE_EDI_RCV_HED_3)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateEdiRcvHedData", cmd)
    ...
    Private Function UpdateEdiRcvDtlData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            Case LMH030DAC.EventShubetsu.SAVEOUTKA, LMH030DAC.EventShubetsu.HIMODUKE _
               , LMH030DAC.EventShubetsu.SAVEUNSO
                '2012.03.26 大阪対応修正END
                setSql = LMH030DAC207.SQL_UPDATE_OUTKASAVE_EDI_RCV_DTL
                loopflg = 1
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateEdiRcvDtlData", cmd)
    ...
    Private Function UpdateOutkaLData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            Case LMH030DAC.EventShubetsu.SAVEOUTKA
                setSql = LMH030DAC207.SQL_UPDATE_OUTKAL_MATOME
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateOutkaLData", cmd)
...
#Region "まとめ先EDI出荷(大)"
    ...
    Private Function UpdateMatomesakiEdiLData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            '出荷登録SQL CONST名
            Case LMH030DAC.EventShubetsu.SAVEOUTKA
                setSql = LMH030DAC207.SQL_UPDATE_MATOMESAKI_OUTKAEDI_L
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateMatomesakiEdiLData", cmd)
...
#Region "まとめ先運送(大)"
    ...
    Private Function UpdateMatomesakiUnsoLData(ByVal ds As DataSet) As DataSet
        ...
        Select Case DirectCast(dtEventShubetsu, LMH030DAC.EventShubetsu)
            ...
            '出荷登録SQL CONST名
            Case LMH030DAC.EventShubetsu.SAVEOUTKA
                setSql = LMH030DAC207.SQL_UPDATE_MATOMESAKI_UNSO_L
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateMatomesakiUnsoLData", cmd)
...
#Region "M_DEST"
    ...
    Private Function UpdateMDestData(ByVal ds As DataSet) As DataSet
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateMDestData", cmd)
...
#Region "C_OUTKA_L"
    ...
    Private Function InsertOutkaLData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC207.SQL_INSERT_OUTKA_L, Me._Row.Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "InsertOutkaLData", cmd)
...
#Region "C_OUTKA_M"
    ...
    Private Function InsertOutkaMData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC207.SQL_INSERT_OUTKA_M _
                                                                       , ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateOutkaMData", cmd)
...
#Region "E_SAGYO_REC"
    ...
    Private Function InsertSagyoData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC207.SQL_INSERT_SAGYO _
                                                                       , ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC207", "InsertSagyoData", cmd)
...
#Region "UNSO_L"
    ...
    Private Function InsertUnsoLData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC207.SQL_INSERT_UNSO_L _
                                                                       , ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "InsertUnsoLData", cmd)
...
#Region "UNSO_M"
    ...
    Private Function InsertUnsoMData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC207.SQL_INSERT_UNSO_M _
                                                                       , ds.Tables("LMH030_OUTKAEDI_L").Rows(0).Item("NRS_BR_CD").ToString()))
        ...
        For i As Integer = 0 To max
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC207", "InsertUnsoMData", cmd)
...
#Region "UNCHIN"
    ...
    Private Function InsertUnchinData(ByVal ds As DataSet) As DataSet
        ...
        '2012.03.28 大阪対応START
        'JC大秦化工の場合は、距離等項目を 0 更新する対応
        Dim custCdL As String = String.Empty
        Dim custCdM As String = String.Empty
        Dim saveUnsoFlg As Integer = 0
        '2012.03.28 大阪対応END

        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC207.SQL_INSERT_UNCHIN _
        ...
        For i As Integer = 0 To max
            ...
            '2012.03.28 大阪対応START
            'JC大秦化工の場合は、距離等項目を 0 更新する対応
            custCdL = Me._Row.Item("CUST_CD_L").ToString()
            custCdM = Me._Row.Item("CUST_CD_M").ToString()

            'JC大秦化工の場合は、距離等項目を 0 に更新するフラグを立てる
            If custCdL.Equals("31022") = True AndAlso custCdM.Equals("01") = True Then
                saveUnsoFlg = 1
            End If
            '2012.03.28 大阪対応END
            ...
            Call Me.SetDataInsertParameter(Me._SqlPrmList)
            '2012.03.28 大阪対応START
            Call Me.SetUnchinComParameter(Me._Row, Me._SqlPrmList, saveUnsoFlg)
            '2012.03.28 大阪対応END
            ...
            MyBase.Logger.WriteSQLLog("LMH030DAC207", "InsertUnchinData", cmd)
...
#Region "M_DEST"
    ...
    Private Function InsertMDestData(ByVal ds As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC207.SQL_INSERT_M_DEST_NCGO, Me._Row.Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "InsertMDestData", cmd)
...
#Region "EDI受信(DTL)更新パラメータ設定"
    ...
    Private Sub SetEdiRcvDtlComParameter(ByVal conditionRow As DataRow, ByVal prmList As ArrayList)
        ...
        With conditionRow
            ...
            prmList.Add(MyBase.GetSqlParameter("@EDI_USER", Me.NullConvertString(.Item("EDI_USER")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@EDI_DATE", Me.NullConvertString(.Item("EDI_DATE")), DBDataType.CHAR))
            prmList.Add(MyBase.GetSqlParameter("@EDI_TIME", Me.NullConvertString(.Item("EDI_TIME")), DBDataType.CHAR))
...
#Region "EDI出荷(大)更新パラメータ設定"
    ...
    Private Sub SetOutkaEdiLComParameter(ByVal conditionRow As DataRow, ByVal prmList As ArrayList, ByVal eventShubetsu As Integer)
        ...
        With conditionRow
            ...
            'prmList.Add(MyBase.GetSqlParameter("@JISSEKI_USER", Me.NullConvertString(.Item("JISSEKI_USER")), DBDataType.CHAR))
            'prmList.Add(MyBase.GetSqlParameter("@JISSEKI_DATE", Me.NullConvertString(.Item("JISSEKI_DATE")), DBDataType.CHAR))
            'prmList.Add(MyBase.GetSqlParameter("@JISSEKI_TIME", Me.NullConvertString(.Item("JISSEKI_TIME")), DBDataType.CHAR))
...
#Region "EDI出荷(中)更新パラメータ設定"
    ...
    Private Sub SetOutkaEdiMComParameter(ByVal conditionRow As DataRow, ByVal prmList As ArrayList)
        ...
        With conditionRow
            ...
            'prmList.Add(MyBase.GetSqlParameter("@JISSEKI_USER", .Item("JISSEKI_USER").ToString(), DBDataType.CHAR))
...
#Region "届先マスタ更新パラメータ設定(JC,JC大秦化工,アイカ専用)"
...
#Region "届先マスタ自動追加用パラメータ設定(JC,JC大秦化工,アイカ専用)"
...
#Region "運賃パラメータ設定"
    ...
    ''' <param name="prmList">パラメータ</param>
    ''' <remarks></remarks>
    Private Sub SetUnchinComParameter(ByVal conditionRow As DataRow, ByVal prmList As ArrayList _
                                      , ByVal saveUnsoFlg As Integer)
        ...
        With conditionRow
            ...
            Select Case saveUnsoFlg
                Case 1       'JC大秦化工の場合は、以下の項目を項目を 0 更新する
                    prmList.Add(MyBase.GetSqlParameter("@SEIQ_KYORI", 0, DBDataType.NUMERIC))
                    prmList.Add(MyBase.GetSqlParameter("@SEIQ_UNCHIN", 0, DBDataType.NUMERIC))
                    prmList.Add(MyBase.GetSqlParameter("@SEIQ_CITY_EXTC", 0, DBDataType.NUMERIC))
                    prmList.Add(MyBase.GetSqlParameter("@SEIQ_WINT_EXTC", 0, DBDataType.NUMERIC))
                    prmList.Add(MyBase.GetSqlParameter("@SEIQ_RELY_EXTC", 0, DBDataType.NUMERIC))
                    prmList.Add(MyBase.GetSqlParameter("@SEIQ_TOLL", 0, DBDataType.NUMERIC))
                    prmList.Add(MyBase.GetSqlParameter("@SEIQ_INSU", 0, DBDataType.NUMERIC))
                Case Else
                    prmList.Add(MyBase.GetSqlParameter("@SEIQ_KYORI", Me.FormatNumValue(.Item("SEIQ_KYORI").ToString()), DBDataType.NUMERIC))
                    prmList.Add(MyBase.GetSqlParameter("@SEIQ_UNCHIN", Me.FormatNumValue(.Item("SEIQ_UNCHIN").ToString()), DBDataType.NUMERIC))
                    prmList.Add(MyBase.GetSqlParameter("@SEIQ_CITY_EXTC", Me.FormatNumValue(.Item("SEIQ_CITY_EXTC").ToString()), DBDataType.NUMERIC))
...
#Region "届先マスタ"
    ...
    Private Function SelectDataMdestOutkaToroku(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC207.SQL_SELECT_M_DEST)
...
#Region "セミEDI(画面取込)処理"
    ...
    Private Function SelectOutkaEdiRcvCancel(ByVal ds As DataSet) As DataSet
        ...
        'JCの場合、取消データは受信ヘッダから取得する

        'DataSetのIN情報を取得
        Dim tbOutkaEdiRcvIn As DataTable = ds.Tables("LMH030_OUTKAEDI_HED_JC")
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC207.SQL_SELECT_OUTKAEDI_RCV_CAN)         'SQL構築
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "SelectOutkaEdiRcvCancel", cmd)
        ...
        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMH030_HED_JC_CANCELOUT")

        '処理件数の設定
        MyBase.SetResultCount(ds.Tables("LMH030_HED_JC_CANCELOUT").Rows.Count())
        reader.Close()
...
#Region "商品マスタ(セミEDI：受信情報からの抽出)"
    ...
    Private Function SelectMstGoods(ByVal ds As DataSet) As DataSet
        ...
        Dim dtRcvDtl As DataTable = ds.Tables("LMH030_OUTKAEDI_DTL_JC")
        Dim drSemiEdiInfo As DataRow = ds.Tables("LMH030_SEMIEDI_INFO").Rows(0)
        ...
        'SQL作成
        Me._StrSql.Append(LMH030DAC207.SQL_SELECT_M_GOODS)

        Me._SqlPrmList = New ArrayList()

        'SQL条件設定(Where)
        Me._StrSql.Append(LMH030DAC207.SQL_WHERE1_M_GOODS)

        'SQL条件設定(Group by)
        Me._StrSql.Append(LMH030DAC207.SQL_GROUP_BY_M_GOODS)
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "SelectDataMgoods", cmd)
...
#Region "H_OUTKAEDI_L(セミEDI：取消更新)"
    ...
    Private Function UpdateDelOutkaEdiL(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim tbOutkaEdiRcvIn As DataTable = ds.Tables("LMH030_HED_JC_CANCELOUT")
        ...
        '出荷EDI CONST名
        sSql = LMH030DAC207.SQL_UPDATE_DEL_OUTKAIEDI_L
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateDelOutkaEdiL", cmd)
...
#Region "H_OUTKAEDI_M(セミEDI：画面取込)"
    ...
    Private Function UpdateDelOutkaEdiM(ByVal ds As DataSet) As DataSet
        ...
        'EDI出荷SQL
        sSql = LMH030DAC207.SQL_UPDATE_DEL_OUTKAIEDI_M
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateDelOutkaEdiM", cmd)
...
#Region "H_OUTKAEDI_HED(セミEDI：取消更新)"
    ...
    Private Function UpdateDelOutkaRcvHed(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim dtOutkaEdiRcvHedCancel As DataTable = ds.Tables("LMH030_HED_JC_CANCELOUT")
        Dim drSemiEdiInfo As DataRow = ds.Tables("LMH030_SEMIEDI_INFO").Rows(0)
        ...
        sSql = LMH030DAC207.SQL_UPDATE_DEL_OUTKAEDI_HED
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateDelOutkaRcvHed", cmd)
...
#Region "H_OUTKAEDI_DTL(セミEDI：取消更新)"
    ...
    Private Function UpdateDelOutkaRcvDtl(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim canDtlTbl As DataTable = ds.Tables("LMH030_HED_JC_CANCELOUT")   'HEDの内容で処理する
        Dim drSemiEdiInfo As DataRow = ds.Tables("LMH030_SEMIEDI_INFO").Rows(0)
        ...
        'セミEDI(画面取込)SQL CONST名
        sSql = LMH030DAC207.SQL_UPDATE_DEL_OUTKAEDI_DTL
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "UpdateDelOutkaRcvDtl", cmd)
...
#Region "H_OUTKAEDI_L(セミEDI)"
    ...
    Private Function InsertOutkaEdiL(ByVal setDs As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC207.SQL_INSERT_OUTKAEDI_L, Me._Row.Item("NRS_BR_CD").ToString()))
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "InsertOutkaEdiL", cmd)
...
#Region "H_OUTKAEDI_M(セミEDI)"
    ...
    Private Function InsertOutkaEdiM(ByVal setDs As DataSet) As DataSet
        ...
        'SQL文のコンパイル
        Dim cmd As SqlCommand = MyBase.CreateSqlCommand(Me.SetSchemaNm(LMH030DAC207.SQL_INSERT_OUTKAEDI_M _
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "InsertOutkaEdiM", cmd)
...
#Region "H_OUTKAEDI_HED_XXX(セミEDI)"
    ...
    Private Function InsertOutkaEdiRcvHed(ByVal setDs As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim dtOutkaEdiRcvHed As DataTable = setDs.Tables("LMH030_OUTKAEDI_HED_JC")

        Dim rtn As Integer = 0

        'スキーマ名設定
        Dim sql As String = Me.SetSchemaNm(LMH030DAC207.SQL_INSERT_OUTKAEDI_HED, setDs.Tables("LMH030_OUTKAEDI_HED_JC").Rows(0).Item("NRS_BR_CD").ToString())
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "InsertOutkaEdiRcvHed", cmd)
...
#Region "H_OUTKAEDI_DTL_XXX(セミEDI)"
    ...
    Private Function InsertOutkaEdiRcvDtl(ByVal setDs As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim dtOutkaEdiRcvDtl As DataTable = setDs.Tables("LMH030_OUTKAEDI_DTL_JC")

        'スキーマ名設定
        Dim sql As String = Me.SetSchemaNm(LMH030DAC207.SQL_INSERT_OUTKAEDI_DTL, setDs.Tables("LMH030_OUTKAEDI_DTL_JC").Rows(0).Item("NRS_BR_CD").ToString())
        ...
        MyBase.Logger.WriteSQLLog("LMH030DAC207", "InsertOutkaEdiRcvDtl", cmd)
...
#Region "EDI受信(HED)更新パラメータ設定"
    ...
    Private Sub SetInsertOutkaEdiRcvHedParameter(ByVal conditionRow As DataRow, ByVal prmList As ArrayList)
        ...
        With conditionRow
            ...
            'EDI受信（DTL）荷主個別項目
            prmList.Add(MyBase.GetSqlParameter("@CANCEL_FLG", Me.NullConvertString(.Item("CANCEL_FLG")), DBDataType.NVARCHAR))
            ...
            prmList.Add(MyBase.GetSqlParameter("@JIGYO_CD", Me.NullConvertString(.Item("JIGYO_CD")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@JIGYO_NM", Me.NullConvertString(.Item("JIGYO_NM")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@SOUKO_CD", Me.NullConvertString(.Item("SOUKO_CD")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@SOUKO_NM", Me.NullConvertString(.Item("SOUKO_NM")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@OUTKA_BI", Me.NullConvertString(.Item("OUTKA_BI")), DBDataType.NVARCHAR))
...
#Region "EDI受信(DTL)更新パラメータ設定"
    ...
    Private Sub SetInsertOutkaEdiRcvDtlParameter(ByVal conditionRow As DataRow, ByVal prmList As ArrayList)
        ...
        With conditionRow
            ...
            'EDI受信（DTL）荷主個別項目
            prmList.Add(MyBase.GetSqlParameter("@DENP_NO", Me.NullConvertString(.Item("DENP_NO")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@GYO_NO", Me.NullConvertZero(.Item("GYO_NO")), DBDataType.NUMERIC))
            prmList.Add(MyBase.GetSqlParameter("@GOODS_CD", Me.NullConvertString(.Item("GOODS_CD")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@GOODS_NM", Me.NullConvertString(.Item("GOODS_NM")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@QT", Me.NullConvertZero(.Item("QT")), DBDataType.NUMERIC))
            prmList.Add(MyBase.GetSqlParameter("@QT_UT", Me.NullConvertString(.Item("QT_UT")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@NOUKI", Me.NullConvertString(.Item("NOUKI")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@ORDER_NO", Me.NullConvertString(.Item("ORDER_NO")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@LOT", Me.NullConvertString(.Item("LOT")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@REMARK", Me.NullConvertString(.Item("REMARK")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@DENP_NM", Me.NullConvertString(.Item("DENP_NM")), DBDataType.NVARCHAR))
            prmList.Add(MyBase.GetSqlParameter("@IRISU", Me.NullConvertZero(.Item("IRISU")), DBDataType.NUMERIC))
            prmList.Add(MyBase.GetSqlParameter("@TANKA", Me.NullConvertZero(.Item("TANKA")), DBDataType.NUMERIC))
            prmList.Add(MyBase.GetSqlParameter("@KINGAKU", Me.NullConvertZero(.Item("KINGAKU")), DBDataType.NUMERIC))
...
#Region "EDI受信(HED/DTL共通)の削除UPDATE文(セミEDI用)"
    ...
    Private Sub SetUpdateDelOutkaEdiRcvParameter(ByVal drOutkaEdiRcvIn As DataRow, ByVal prmList As ArrayList)
        ...
        prmList.Add(MyBase.GetSqlParameter("@DELETE_EDI_NO_CHU", Me.NullConvertString(drOutkaEdiRcvIn.Item("DELETE_USER")), DBDataType.NVARCHAR))
