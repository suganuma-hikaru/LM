...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMB530DAC = New LMB530DAC()
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMB530IN"
        Dim tableNmOut As String = "LMB530OUT"
        ...
        For Each dr As DataRow In ds.Tables("M_RPT").Rows
            ...
            prtDs = New DataSet
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables("LMB530OUT"), dr.Item("RPT_ID").ToString())
            '帳票ごとの編集があるなら行う。
    ...
    Private Function EditPrintDataSet(ByVal prtId As String, ByVal ds As DataSet) As DataSet
        ...
        Select Case prtId
            ...
            Case "LMB530"
                ...
                'ソート処理追加開始2012/12/03
                Dim outDt As DataTable = ds.Tables("LMB530OUT") 'データ取得
                Dim max As Integer = outDt.Rows.Count - 1       '抽出データ数取得
                Dim count As Integer = 0                        '項番(初期値0)

                'ソート条件設定
                Dim sort As String = "NRS_BR_CD  ASC,GOODS_NM_1  ASC, INKA_NO_L  ASC,INKA_NO_M   ASC, INKA_NO_S  ASC"
                'ソート実行
                ...
                For Each row As DataRow In dr
                    rtnDs.Tables("LMB530OUT").ImportRow(row)
                    'ソート処理追加終了2012/12/03
                Next
