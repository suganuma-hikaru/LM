...
#Region "SELECT句"
    ...
    Private Const SQL_SELECT_DATA As String = "SELECT                                     " & vbNewLine _
                                            ...
                                            & ", SEKY_MEI.TOKU_CD        AS TOKU_CD       " & vbNewLine _
                                            & ", NRS_BR.NRS_BR_NM        AS NRS_BR_NM     " & vbNewLine _
                                            & ", SEKY_MEI.KAGAMI_KB      AS KAGAMI_KB     " & vbNewLine _
                                            & ", SEKY_MEI.MEISAI_NO      AS MEISAI_NO     " & vbNewLine _
                                            & ", SEKY_MEI.HUTANKA        AS HUTANKA       " & vbNewLine _
                                            & ", SEKY_MEI.H_JOKEN        AS H_JOKEN       " & vbNewLine _
                                            & ", SUM(SEKY_MEI.ZAN_QT)    AS ZAN_QT        " & vbNewLine _
                                            & ", SUM(SEKY_MEI.HOKAN)     AS HOKAN         " & vbNewLine _
                                            & ", SUM(SEKY_MEI.NIYAKU)    AS NIYAKU        " & vbNewLine _
                                            & ", SUM(SEKY_MEI.HN_TTL)    AS HN_TTL        " & vbNewLine _
                                            & ", SUM(SEKY_MEI.NIZUKURI)  AS NIZUKURI      " & vbNewLine _
                                            & ", SUM(SEKY_MEI.YOKOMOCHI) AS YOKOMOCHI     " & vbNewLine _
                                            & ", SUM(SEKY_MEI.GAICHU)    AS GAICHU        " & vbNewLine _
                                            & ", SUM(SEKY_MEI.TTL)       AS TTL           " & vbNewLine _
                                            & ", SUM(SEKY_MEI.TAX_TTL)   AS TAX_TTL       " & vbNewLine _
                                            & ", SUM(SEKY_MEI.G_TTL)     AS G_TTL         " & vbNewLine _
                                            & ", SEKY_MEI.TAX            AS TAX           " & vbNewLine
...
#Region "WHERE句"
    ...
    Private Const SQL_SELECT_WHERE As String = "WHERE                                " & vbNewLine _
                                             & "    SEKY_MEI.NRS_BR_CD = @NRS_BR_CD  " & vbNewLine _
...
#Region "GROUP BY句"
    ...
    Private Const SQL_GROUP_BY As String = "GROUP BY              " & vbNewLine _
                                         ...
                                         & ", SEKY_MEI.TOKU_CD    " & vbNewLine _
                                         & ", NRS_BR.NRS_BR_NM    " & vbNewLine _
                                         & ", SEKY_MEI.KAGAMI_KB  " & vbNewLine _
                                         & ", SEKY_MEI.MEISAI_NO  " & vbNewLine _
                                         & ", SEKY_MEI.HUTANKA    " & vbNewLine _
                                         & ", SEKY_MEI.H_JOKEN    " & vbNewLine _
                                         & ", SEKY_MEI.TAX        " & vbNewLine
...
#Region "印刷処理"
    ...
    Private Function SelectMPrt(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim inTbl As DataTable = ds.Tables("LMI650IN")
        ...
        'SQL作成
        Me._StrSql.Append(LMI650DAC.SQL_SELECT_MPRT)      'SQL構築(帳票種別用Select句)
        Me._StrSql.Append(LMI650DAC.SQL_SELECT_FROM)      'SQL構築(データ抽出用From句)
        Me._StrSql.Append(LMI650DAC.SQL_SELECT_WHERE)     'SQL構築(データ抽出用Where句)
        ...
        MyBase.Logger.WriteSQLLog("LMI650DAC", "SelectMPRT", cmd)
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMI650DAC.SQL_SELECT_DATA)      'SQL構築(データ抽出用Select句)
        Me._StrSql.Append(LMI650DAC.SQL_SELECT_FROM)      'SQL構築(データ抽出用From句)
        Me._StrSql.Append(LMI650DAC.SQL_SELECT_WHERE)     'SQL構築(データ抽出用Where句)
        Me._StrSql.Append(LMI650DAC.SQL_GROUP_BY)         'SQL構築(データ抽出用GroupBy句)
        Call Me.SetSQLSelectWhere()                       'パラメータ設定
        ...
        MyBase.Logger.WriteSQLLog("LMI650DAC", "SelectPrintData", cmd)
        ...
        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMI650OUT")
