...
#Region "画面制御"
    ...
    Friend Sub SetCheckBox(ByVal ctl As Win.LMCheckBox, ByVal value As String)
        ...
        If value.Equals(LMGControlC.YN_FLG_YES) Then
            ctl.SetBinaryValue(LMConst.FLG.ON)
        ElseIf value.Equals(LMGControlC.YN_FLG_NO) Then
...
#End Region
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Friend Function GetCustNm(ByVal NrsBrCd As String, _
        ...
        Dim custDr As DataRow() = MyBase.GetLMCachedDataTable(LMConst.CacheTBL.CUST).Select(Me.SelectCustString(NrsBrCd, custcdl, custcdm, custcds, custcdss))
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function GetSeqNm(ByVal NrsBrCd As String, ByVal sekySaki As String) As ArrayList
        ...
        If String.IsNullOrEmpty(sekySaki) = False Then
            ...
            Seiqdr = MyBase.GetLMCachedDataTable(LMConst.CacheTBL.SEIQTO).Select(String.Concat("SEIQTO_CD = ", " '", sekySaki, "' ", " AND ", "NRS_BR_CD = ", " '", NrsBrCd, "' ", " AND ", " SYS_DEL_FLG = '0' "))
            ...
            If Seiqdr.Count < 1 = False Then
                ...
                '2011/08/16 菱刈 口座区分の取得 スタート
                sekysakiCdNm.Add(Seiqdr(0).Item("KOUZA_KB").ToString())
                '2011/08/16 菱刈 口座区分の取得 エンド
                '2011/08/28 須賀 口座区分名／全体値引率／値引額の取得 スタート
                ...
                '2011/08/28 須賀 口座区分名／全体値引率／値引額の取得 エンド
            End If
        End If
    ...
    Friend Function GetCellValue(ByVal aCell As Cell) As String
        ...
        ElseIf TypeOf aCell.Editor Is CellType.CheckBoxCellType Then
            ...
            If aCell.Text.Equals("True") = True Then
                getCell = LMConst.FLG.ON
            ElseIf aCell.Text.Equals("False") = True Then
                getCell = LMConst.FLG.OFF
            Else
                getCell = aCell.Text
            End If
        ...
        ElseIf TypeOf aCell.Editor Is CellType.DateTimeCellType Then
            ...
            If aCell.Value Is Nothing = False AndAlso String.IsNullOrEmpty(aCell.Value.ToString()) = False Then
                getCell = Convert.ToDateTime(aCell.Value).ToString("yyyyMMdd")
            End If

        Else

            'テキストの場合、Trimした値を返却
            getCell = aCell.Text.Trim()
