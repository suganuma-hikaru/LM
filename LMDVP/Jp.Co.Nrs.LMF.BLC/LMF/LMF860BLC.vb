...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMF860DAC = New LMF860DAC()
...
#Region "CSVデータ取得処理"
    ...
    Private Function MeitetuCsv(ByVal ds As DataSet) As DataSet
        ...
        '出力対象データが0件の場合は終了
        If rtnDs.Tables("LMF860OUT").Rows.Count = 0 Then
            '処理終了メッセージの表示
    ...
    Private Function SelectMeitetuCsv(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim dt As DataTable = ds.Tables("LMF860IN")
        Dim outDt As DataTable = ds.Tables("LMF860OUT")
        Dim max As Integer = dt.Rows.Count - 1

        '別インスタンス
        Dim setDs As DataSet = ds.Copy()
        Dim inTbl As DataTable = setDs.Tables("LMF860IN")
        Dim setDt As DataTable = setDs.Tables("LMF860OUT")
