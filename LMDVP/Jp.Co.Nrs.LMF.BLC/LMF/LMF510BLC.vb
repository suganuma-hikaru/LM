...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMF510DAC = New LMF510DAC()
...
#Region "印刷"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMF510IN"
        Dim tableNmOut As String = "LMF510OUT"
        ...
        For Each dr As DataRow In ds.Tables("M_RPT").Rows
            ...
            prtDs = New DataSet
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables("LMF510OUT"), dr.Item("RPT_ID").ToString())
            '帳票ごとの編集があるなら行う。
    ...
    Private Function EditPrintDataSet(ByVal prtId As String, ByVal ds As DataSet) As DataSet
        ...
        Select Case prtId
            ...
            Case "LMF511"
                'ソート対象データテーブル取得
                Dim outDt As DataTable = ds.Tables("LMF510OUT")
                'ソート条件設定
                Dim sort As String = "CUST_CD_L ASC, CUST_CD_M ASC, CUST_CD_S ASC, CUST_CD_SS ASC, CUST_COST_CD2 ASC, SEARCH_KEY_1 ASC, OUTKA_PLAN_DATE ASC"
                'ソート実行
                ...
                For Each row As DataRow In dr
                    rtnDs.Tables("LMF510OUT").ImportRow(row)
                Next
