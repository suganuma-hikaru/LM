...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMF581DAC = New LMF581DAC()
...
#Region "印刷"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMF581IN"
        Dim tableNmOut As String = "LMF581OUT"
        ...
        For Each dr As DataRow In ds.Tables("M_RPT").Rows
            ...
            prtDs = New DataSet
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables("LMF581OUT"), dr.Item("RPT_ID").ToString())
    ...
    Private Function EditPrintDataSet(ByVal prtId As String, ByVal ds As DataSet) As DataSet
        ...
        '改ページキーが空の場合は、運送会社をキーに入れる

        For i As Integer = 0 To ds.Tables("LMF581OUT").Rows.Count - 1
            If System.String.IsNullOrEmpty(ds.Tables("LMF581OUT").Rows(i).Item("PAGE_KBN_NM").ToString) = True Then
                ds.Tables("LMF581OUT").Rows(i).Item("PAGE_KBN_NM") = ds.Tables("LMF581OUT").Rows(i).Item("UNSOCO_NM").ToString
            End If
