...
#Region "Const"
    ...
    Private Const TABLE_NM_RESULT As String = "LMF800RESULT"
    ...
    Private Const TABLE_NM_UNSO_HOKENRYO As String = "UNSO_HOKENRYO"            'ADD 2018/10/23 依頼番号 : 002400   【LMS】運送保険_設定商品を出荷時、運送の保険料欄に保険料を自動入力させる
    ...
    Private Const ACT_GET_EXTC_UNCHIN As String = "GetExtcUnchin"
...
#Region "Field"
    ...
    Private _dac As LMF800DAC = New LMF800DAC()
...
#Region " ○1-1 Main Method"
    ...
    Public Function CreateUnchinData(ByVal ds As DataSet) As DataSet
        ...
        ' 横持ち＆明細分割有の場合（１、２行目）
        ' 届先マスタにレコードが存在する場合（３行目）
        If (Me._yokoSplitUmu.Equals(FLG_ON) And _
            ds.Tables(TABLE_NM_UNSO_L).Rows(0).Item("TARIFF_BUNRUI_KB").ToString.Equals(TARIFF_BUNRUI_YKMOTI)) Or _
            ds.Tables(TABLE_NM_OUT).Rows(0).Item("DEST_MST_SEIQTO_EXIST_FLG").ToString.Equals(FLG_ON) Then
            '運賃分割なし

        Else
            ' 特定荷主判断
            If Me.ChkSpecificCust(ds) Then

                ' 運送Ｍより請求先単位の運賃基礎情報を取得
                If Me.GetRevUnitUnsoM(ds) Then

                    '請求先毎に運賃を配分する
                    Me.DivideUnchinSpecificCust(ds)

                    '請求先毎に運賃レコードを作成する
                    Me.SetSpecificCustForm(ds)
...
#If True Then   'ADD 2018/10/22 依頼番号 : 002400   【LMS】運送保険_設定商品を出荷時、運送の保険料欄に保険料を自動入力させる

        ' 運送保険料取得
        If Me.GetUnsoHokenryo(ds) Then
...
#Region " ○1-2 Sub Method"
    ...
    Private Function SetNomal(ByVal ds As DataSet) As Boolean
        ...
        For idx As Integer = 0 To Me._detailCount - 1
            ...
            '[初期運賃レコードの削除]
            If idx = 0 Then
                ds.Tables(TABLE_NM_UNCHIN).Clear()
            End If
    ...
    Private Function SetYoko(ByVal ds As DataSet) As Boolean
        ...
        'If Me.GetYokoTariffHd(ds) = False Then
        '    Return False
        'End If
...
#Region " ○1-3 部品"
    ...
    ''' <remarks>請求先単位に運賃を配分する。</remarks>
    Private Sub DivideUnchinSpecificCust(ByVal ds As DataSet)

        ' パラメータとなるテーブル
        Dim revUnitDt As DataTable = ds.Tables(TABLE_NM_REV_UNIT)
        Dim unchinDt As DataTable = ds.Tables(TABLE_NM_UNCHIN)
        Dim unsoLDr As DataRow = ds.Tables(TABLE_NM_UNSO_L).Rows(0)

        Dim unchin_base As Double = INIT_DOU
        Dim unchin_city As Double = INIT_DOU
        Dim unchin_wint As Double = INIT_DOU
        Dim unchin_rely As Double = INIT_DOU
        Dim unchin_toll As Double = INIT_DOU
        Dim unchin_insu As Double = INIT_DOU

        Dim calc_base As Double = INIT_DOU
        Dim calc_city As Double = INIT_DOU
        Dim calc_wint As Double = INIT_DOU
        Dim calc_rely As Double = INIT_DOU
        Dim calc_toll As Double = INIT_DOU
        Dim calc_insu As Double = INIT_DOU

        Dim leave_base As Double = INIT_DOU
        Dim leave_city As Double = INIT_DOU
        Dim leave_wint As Double = INIT_DOU
        Dim leave_rely As Double = INIT_DOU
        Dim leave_toll As Double = INIT_DOU
        Dim leave_insu As Double = INIT_DOU

        Dim base_wt As Double = INIT_DOU
        Dim para_wt As Double = INIT_DOU

        Dim fitFlg As Boolean = False
        Dim sizeCount As Integer = -1

        Dim lvFlg As Integer = Convert.ToInt32(Convert.ToDecimal(ds.Tables(TABLE_NM_OUT).Rows(0).Item("REV_UNCHIN_LV_FLG").ToString))

        For idx As Integer = 0 To unchinDt.Rows.Count - 1

            With unchinDt.Rows(idx)

                '基礎重量
                base_wt = Convert.ToDouble(.Item("SEIQ_WT").ToString)
                ...
                For cnt As Integer = 0 To revUnitDt.Rows.Count - 1

                    ' サイズ区分が一致したとき運賃配分を行う
                    If revUnitDt.Rows(cnt).Item("SIZE_KB").ToString.Equals(.Item("SIZE_KB").ToString) Then

                        ' 按分重量
                        para_wt = Convert.ToDouble(revUnitDt.Rows(cnt).Item("BETU_WT").ToString)

                        ' 第一ループ
                        If fitFlg = False Then

                            '現在登録値を確保
                            unchin_base = Convert.ToDouble(.Item("SEIQ_UNCHIN").ToString)
                            unchin_city = Convert.ToDouble(.Item("SEIQ_CITY_EXTC").ToString)
                            unchin_wint = Convert.ToDouble(.Item("SEIQ_WINT_EXTC").ToString)
                            unchin_rely = Convert.ToDouble(.Item("SEIQ_RELY_EXTC").ToString)
                            unchin_toll = Convert.ToDouble(.Item("SEIQ_TOLL").ToString)
                            unchin_insu = Convert.ToDouble(.Item("SEIQ_INSU").ToString)

                        End If

                        ' 基本運賃は必ず按分する
                        calc_base = Me.ToHalfAdjust(unchin_base * para_wt / base_wt, 2)

                        ' 運賃按分有無判断（ 0 : 基本のみ　1 : 基本＆割増）
                        If lvFlg = 0 Then
                            ' 先頭行に全額
                            If fitFlg = False Then
                                calc_city = unchin_city
                                calc_wint = unchin_wint
                                calc_rely = unchin_rely
                                calc_toll = unchin_toll
                                calc_insu = unchin_insu
                            Else
                                calc_city = 0
                                calc_wint = 0
                                calc_rely = 0
                            ...
                            End If
                        Else
                            calc_city = Me.ToHalfAdjust(unchin_city * para_wt / base_wt, 2)
                            calc_wint = Me.ToHalfAdjust(unchin_wint * para_wt / base_wt, 2)
                            calc_rely = Me.ToHalfAdjust(unchin_rely * para_wt / base_wt, 2)
                            calc_toll = Me.ToHalfAdjust(unchin_toll * para_wt / base_wt, 2)
                            calc_insu = Me.ToHalfAdjust(unchin_insu * para_wt / base_wt, 2)
                        End If

                        revUnitDt.Rows(cnt).Item("UNCHIN") = calc_base
                        revUnitDt.Rows(cnt).Item("CITY_EXTC") = calc_city
                        revUnitDt.Rows(cnt).Item("WINT_EXTC") = calc_wint
                        revUnitDt.Rows(cnt).Item("RELY_EXTC") = calc_rely
                        revUnitDt.Rows(cnt).Item("TOLL") = calc_toll
                        revUnitDt.Rows(cnt).Item("INSU") = calc_insu

                        leave_base = leave_base + calc_base
                        leave_city = leave_city + calc_city
                        leave_wint = leave_wint + calc_wint
                        leave_rely = leave_rely + calc_rely
                        leave_toll = leave_toll + calc_toll
                        leave_insu = leave_insu + calc_insu

                        '行数格納
                        sizeCount = cnt

                        ' [注意] 第一ループの判断で使用しているため
                        '        "運賃按分有無判断" より情報に移動しないこと
                        fitFlg = True
                ...
                If fitFlg = True Then
                    ...
                    revUnitDt.Rows(sizeCount).Item("UNCHIN") = calc_base + unchin_base - leave_base
                    revUnitDt.Rows(sizeCount).Item("CITY_EXTC") = calc_city + unchin_city - leave_city
                    revUnitDt.Rows(sizeCount).Item("WINT_EXTC") = calc_wint + unchin_wint - leave_wint
                    revUnitDt.Rows(sizeCount).Item("RELY_EXTC") = calc_rely + unchin_rely - leave_rely
                    revUnitDt.Rows(sizeCount).Item("TOLL") = calc_toll + unchin_toll - leave_toll
                    revUnitDt.Rows(sizeCount).Item("INSU") = calc_insu + unchin_insu - leave_insu
        ...
        If ds.Tables(TABLE_NM_UNSO_L).Rows(0).Item("TARIFF_BUNRUI_KB").ToString.Equals(TARIFF_BUNRUI_YKMOTI) _
          AndAlso ds.Tables(TABLE_NM_YOKO_HD).Rows.Count <> 0 Then
            ...
            For idx As Integer = 0 To unchinDt.Rows.Count - 1
                ...
                If Convert.ToDouble(unchinDt.Rows(idx).Item("SEIQ_UNCHIN").ToString) < unchinMin Then
                    unchinDt.Rows(idx).Item("SEIQ_UNCHIN") = unchinMin
...
#If True Then           'ADD 2018/10/22 依頼番号 : 002400   【LMS】運送保険_設定商品を出荷時、運送の保険料欄に保険料を自動入力させる
    ...
    ''' <returns>True：特定荷主 ／ False：特定荷主以外</returns>
    ''' <remarks>対象運送の荷主を元に、「運賃レコード分割用荷主」であるか否かを判断する</remarks>
    Private Function GetUnsoHokenryo(ByVal ds As DataSet) As Boolean

        'unsou 
        '運送保険料取得SQL実行()
        ds = Me.DacAccess(ds, "GET_HOKENRYO")

        '結果件数の確認
        Dim count As Integer = MyBase.GetResultCount()

        If count > 0 Then
            'SEIQ_INSU   DECI_INSU HOKENRYO
            Dim unchinDt As DataTable = ds.Tables(TABLE_NM_UNCHIN)

            ' 運送保険料で設定値設定
            Dim hokenryo As Double = Convert.ToDouble(ds.Tables(TABLE_NM_UNSO_HOKENRYO).Rows(0).Item("HOKENRYO").ToString)

            If hokenryo > 0 Then

                If ("1").Equals(ds.Tables(TABLE_NM_UNSO_HOKENRYO).Rows(0).Item("SET1_NAIYO1").ToString) = True Then

                    If hokenryo > 0 Then
                        '保険料有時
                        Dim setteiAmt As Double = Convert.ToDouble(ds.Tables(TABLE_NM_UNSO_HOKENRYO).Rows(0).Item("SET1_NAIYO2").ToString)
                        Dim kasanAmt As Double = Convert.ToDouble(ds.Tables(TABLE_NM_UNSO_HOKENRYO).Rows(0).Item("SET1_NAIYO3").ToString)
                        Dim kasan As Double = Math.Ceiling(hokenryo / setteiAmt)

                        Dim hokneAmt As Double = kasanAmt * kasan

                        unchinDt.Rows(0).Item("SEIQ_INSU") = hokneAmt.ToString
                        unchinDt.Rows(0).Item("DECI_INSU") = hokneAmt.ToString
                        unchinDt.Rows(0).Item("KANRI_INSU") = hokneAmt.ToString

                    End If

                Else
                    'その他は運送料をそのまま設定
                    unchinDt.Rows(0).Item("SEIQ_INSU") = ds.Tables(TABLE_NM_UNSO_HOKENRYO).Rows(0).Item("HOKENRYO").ToString
                    unchinDt.Rows(0).Item("DECI_INSU") = ds.Tables(TABLE_NM_UNSO_HOKENRYO).Rows(0).Item("HOKENRYO").ToString
                    unchinDt.Rows(0).Item("KANRI_INSU") = ds.Tables(TABLE_NM_UNSO_HOKENRYO).Rows(0).Item("HOKENRYO").ToString
...
#End If
    ...
    Private Sub ClearParamToInput(ByVal ds As DataSet)
        ...
        'DataSet初期化
        ds.Tables("LMF800OUT").Clear()
        ds.Tables("LMF800RESULT").Clear()
        ds.Tables("GOODS_INFO_REV_UNIT").Clear()
        ds.Tables("UNCHIN_TARIFF_LATEST_REC").Clear()
        ds.Tables("UNCHIN_CALC_OUT").Clear()
        ds.Tables("F_UNCHIN_TRS").Clear()

        ds.Tables("M_EXTC_UNCHIN").Clear()
        ds.Tables("M_SYUHAI_SET").Clear()
        ds.Tables("M_UNCHIN_KYORI").Clear()
        ds.Tables("M_UNCHIN_WT").Clear()
        ds.Tables("M_YOKO_TARIFF_HD").Clear()
        ds.Tables("M_YOKO_TARIFF_HD_CALC").Clear()
        ds.Tables("M_YOKO_TARIFF_DTL_CALC").Clear()
    ...
    Private Sub ClearParamToOutput(ByVal ds As DataSet)
        ...
        'DataSet初期化
        ds.Tables("LMF800IN").Clear()
        ds.Tables("LMF800OUT").Clear()
        ds.Tables("GOODS_INFO_REV_UNIT").Clear()
        ds.Tables("UNCHIN_TARIFF_LATEST_REC").Clear()
        ds.Tables("UNCHIN_CALC_IN").Clear()
    ...
    ''' <param name="index">処理対象Mの行番号</param>
    ''' <return>計算結果商品重量</return>
    ''' <remark>商品単位の重量を算出</remark>
    Private Function GetPkgWt(ByVal ds As DataSet, ByVal index As Integer) As Double '2013.03.19 / NOTES 1911 オプションとして運送L追加
...
#Region " ○1-4 データ参照"
    ...
    Private Function CommonMethod(ByVal ds As DataSet) As Boolean
        ...
        '20120711 forKSK 
        'If ds.Tables(TABLE_NM_UNSO_L).Rows(0).Item("MOTO_DATA_KB").ToString.Equals(MOTO_UNSOU) Then
        '    '運送時は倉庫起点ではないため取得距離を初期化
        '    ds.Tables(TABLE_NM_OUT).Rows(0).Item("KYORI") = INIT_NUM
        'End If
...
#Region " ○1-5 check Method"
    ...
    ''' <returns>True：特定荷主 ／ False：特定荷主以外</returns>
    ''' <remarks>対象運送の荷主を元に、「運賃レコード分割用荷主」であるか否かを判断する</remarks>
    Private Function ChkSpecificCust(ByVal ds As DataSet) As Boolean

        '特定荷主チェック用SQL実行
        ds = Me.DacAccess(ds, ACT_GET_SPECIFIC_CUST_COUNT)
...
#Region " ○1-6 運賃レコード構成"
    ...
    Private Sub SetInitForm(ByVal ds As DataSet)
        ...
        With unchinDr
            ...
            .Item("CUST_CD_SS") = ""
            .Item("SEIQ_GROUP_NO") = ""
            .Item("SEIQ_GROUP_NO_M") = ""
            .Item("SEIQTO_CD") = ""
            .Item("UNTIN_CALCULATION_KB") = ""
            .Item("SEIQ_SYARYO_KB") = unsoLDr.Item("VCLE_KB")
            .Item("SEIQ_PKG_UT") = unsoLDr.Item("NB_UT")
            .Item("SEIQ_NG_NB") = unsoLDr.Item("UNSO_PKG_NB")
            .Item("SEIQ_DANGER_KB") = "01"
            .Item("SEIQ_TARIFF_BUNRUI_KB") = unsoLDr.Item("TARIFF_BUNRUI_KB")
            .Item("SEIQ_TARIFF_CD") = unsoLDr.Item("SEIQ_TARIFF_CD")
            .Item("SEIQ_ETARIFF_CD") = unsoLDr.Item("SEIQ_ETARIFF_CD")
            .Item("SEIQ_KYORI") = 0
            .Item("SEIQ_WT") = unsoLDr.Item("UNSO_WT")
            .Item("SEIQ_UNCHIN") = 0
            .Item("SEIQ_CITY_EXTC") = 0
            .Item("SEIQ_WINT_EXTC") = 0
            .Item("SEIQ_RELY_EXTC") = 0
            .Item("SEIQ_TOLL") = 0
            .Item("SEIQ_INSU") = 0
            .Item("SEIQ_FIXED_FLAG") = "00"
            ...
            If unsoLDr.Item("NRS_BR_CD").ToString() = "30" AndAlso _
               ...
               unsoLDr.Item("CUST_CD_M").ToString() = "00" Then
                ...
                If ds.Tables(TABLE_NM_RCV_BP).Rows.Count <> 0 Then
                    ...
                    .Item("SEIQ_WT") = bpRcvDr.Item("BP_UNSO_WT")
    ...
    Private Sub SetNomalUnchinForm(ByVal ds As DataSet, ByVal index As Integer, ByVal createLv As String)
        ...
        With unchinDr
            ...
            .Item("SEIQ_GROUP_NO") = ""
            .Item("SEIQ_GROUP_NO_M") = ""
            .Item("SEIQTO_CD") = outDr.Item("UNCHIN_SEIQTO_CD")
            ...
            .Item("SEIQ_TARIFF_CD") = unsoLDr.Item("SEIQ_TARIFF_CD")
            .Item("SEIQ_ETARIFF_CD") = unsoLDr.Item("SEIQ_ETARIFF_CD")
            .Item("SEIQ_KYORI") = outDr.Item("KYORI")
            .Item("SEIQ_WT") = unsoLDr.Item("UNSO_WT")
            .Item("SEIQ_UNCHIN") = 0
            .Item("SEIQ_CITY_EXTC") = 0
            .Item("SEIQ_WINT_EXTC") = 0
            ...
            If createLv.Equals(LVL_UNSOU) Then
                ...
                .Item("DECI_NG_NB") = unsoLDr.Item("UNSO_PKG_NB")
            Else
                '要望管理533対応 koba Start
                '.Item("SEIQ_NG_NB") = GetPkgNb(unsoMDr)
                '.Item("DECI_NG_NB") = GetPkgNb(unsoMDr)
                .Item("SEIQ_NG_NB") = unsoMDr.Item("UNSO_TTL_NB")
    ...
    Private Sub SetYokoUnchinForm(ByVal ds As DataSet, ByVal index As Integer)
        ...
        With unchinDr
            ...
            .Item("UNTIN_CALCULATION_KB") = outDr.Item("UNTIN_CALCULATION_KB")
            .Item("SEIQ_SYARYO_KB") = unsoLDr.Item("VCLE_KB")
            .Item("SEIQ_DANGER_KB") = outDr.Item("KIKEN_KB")
            .Item("SEIQ_TARIFF_BUNRUI_KB") = TARIFF_BUNRUI_YKMOTI
            .Item("SEIQ_TARIFF_CD") = unsoLDr.Item("SEIQ_TARIFF_CD")
            .Item("SEIQ_ETARIFF_CD") = unsoLDr.Item("SEIQ_ETARIFF_CD")
            .Item("SEIQ_KYORI") = outDr.Item("KYORI")
            .Item("SEIQ_UNCHIN") = 0
            ...
            If Me._yokoSplitUmu.Equals(FLG_ON) Then
                '分割有
                .Item("SEIQ_PKG_UT") = unsoMDr.Item("NB_UT")
                .Item("SEIQ_NG_NB") = GetPkgNb(unsoMDr)
                .Item("DECI_NG_NB") = GetPkgNb(unsoMDr)

                '2013.03.18 / NOTES1911 風袋重量バグ 対応 開始
                '.Item("SEIQ_WT") = GetPkgWt(unsoMDr) 'オプション引数運送L追加
                '.Item("DECI_WT") = GetPkgWt(unsoMDr) 'オプション引数運送L追加
                .Item("SEIQ_WT") = GetPkgWt(ds, index) 'オプションでDSまるごと + index
    ...
    Private Sub SetUnchinInfo(ByVal ds As DataSet, ByVal index As Integer)
        ...
        ' 結果格納用運賃テーブル
        Dim unchinDr As DataRow = ds.Tables(TABLE_NM_UNCHIN).Rows(index)

        ' 運賃設定
        With unchinDr

            .Item("SEIQ_UNCHIN") = resultDr.Item("UNCHIN").ToString
            .Item("SEIQ_CITY_EXTC") = resultDr.Item("CITY_EXTC").ToString
            .Item("SEIQ_WINT_EXTC") = resultDr.Item("WINT_EXTC").ToString
            .Item("SEIQ_RELY_EXTC") = resultDr.Item("RELY_EXTC").ToString
            .Item("SEIQ_TOLL") = resultDr.Item("TOLL").ToString
            .Item("SEIQ_INSU") = resultDr.Item("INSU").ToString
    ...
    Private Sub SetNyukaTyakuUnchinForm(ByVal ds As DataSet)
        ...
        With unchinDr
            ...
            .Item("SEIQ_KYORI") = outDr.Item("KYORI")
            .Item("SEIQ_WT") = unsoLDr.Item("UNSO_WT")
            .Item("SEIQ_UNCHIN") = inDr.Item("UNCHIN")
            .Item("SEIQ_CITY_EXTC") = 0
            .Item("SEIQ_WINT_EXTC") = 0
            .Item("SEIQ_RELY_EXTC") = 0
            .Item("SEIQ_TOLL") = 0
    ...
    ''' </summary>
    ''' <param name="ds">起動元機能からのパラメータ</param>
    ''' <remarks>「特定荷主」時における運賃レコードを構成する。</remarks>
    Private Sub SetSpecificCustForm(ByVal ds As DataSet)
        ...
        ' 結果データ格納先
        Dim newUnchinDt As DataTable = ds.Tables(TABLE_NM_UNCHIN).Clone
            ...
            With unchinDt.Rows(idx)
                ...
                For cnt As Integer = 0 To revUnitDt.Rows.Count - 1
                    ...
                    If revUnitDt.Rows(cnt).Item("SIZE_KB").ToString.Equals(.Item("SIZE_KB").ToString) Then
                        ...
                        Me.SetSpecificCustParam(unchinDt.Rows(idx), revUnitDt.Rows(cnt), newUnchinDt)
        ...
        'テーブルが消えないので、処理方法変更
        'ds.Tables(TABLE_NM_UNCHIN).Dispose()
        'ds.Tables.Add(newUnchinDt)
        ds.Tables(TABLE_NM_UNCHIN).Clear()
        For i As Integer = 0 To newUnchinDt.Rows.Count - 1
            ds.Tables(TABLE_NM_UNCHIN).ImportRow(newUnchinDt.Rows(i))
        Next

    End Sub

    ''' <summary>
    ''' [レコード構成] 特定荷主（請求先毎）パラメータセット
    ''' </summary>
    ''' <param name="unchinDr">既生成の運賃レコード</param>
    ''' <param name="revUnitDr">請求先単位の運賃情報基礎値</param>
    ''' <param name="resultDt">結果用運賃データテーブル</param>
    ''' <remarks>請求先単位の運賃レコードを構成し返却する</remarks>
    Private Sub SetSpecificCustParam(ByVal unchinDr As DataRow, ByVal revUnitDr As DataRow, ByVal resultDt As DataTable)

        ' 結果データ格納先
        Dim newUnchinDr As DataRow = resultDt.NewRow

        ' 運賃内容を転記
        resultDt.ImportRow(unchinDr)

        Dim editRow As Integer = resultDt.Rows.Count - 1

        ' 運賃設定
        With resultDt.Rows(editRow)

            'YUSO_BR_CD
            'NRS_BR_CD
            'UNSO_NO_L
            .Item("UNSO_NO_M") = revUnitDr.Item("UNSO_NO_M")
            'CUST_CD_L
            'CUST_CD_M
            .Item("CUST_CD_S") = revUnitDr.Item("CUST_CD_S")
            .Item("CUST_CD_SS") = revUnitDr.Item("CUST_CD_SS")
            'SEIQ_GROUP_NO
            'SEIQ_GROUP_NO_M
            .Item("SEIQTO_CD") = revUnitDr.Item("SEIQTO_CD")
            .Item("UNTIN_CALCULATION_KB") = revUnitDr.Item("UNTIN_CALCULATION_KB")
            'SEIQ_SYARYO_KB
            'SEIQ_PKG_UT
            .Item("SEIQ_NG_NB") = revUnitDr.Item("UNSO_TTL_NB")
            'SEIQ_DANGER_KB
            'SEIQ_TARIFF_BUNRUI_KB
            'SEIQ_TARIFF_CD
            'SEIQ_ETARIFF_CD
            'SEIQ_KYORI
            .Item("SEIQ_WT") = revUnitDr.Item("BETU_WT")
            .Item("SEIQ_UNCHIN") = revUnitDr.Item("UNCHIN")
            .Item("SEIQ_CITY_EXTC") = revUnitDr.Item("CITY_EXTC")
            .Item("SEIQ_WINT_EXTC") = revUnitDr.Item("WINT_EXTC")
            .Item("SEIQ_RELY_EXTC") = revUnitDr.Item("RELY_EXTC")
            .Item("SEIQ_TOLL") = revUnitDr.Item("TOLL")
            .Item("SEIQ_INSU") = revUnitDr.Item("INSU")
            'SEIQ_FIXED_FLAG
            .Item("DECI_NG_NB") = revUnitDr.Item("UNSO_TTL_NB")
            'DECI_KYORI
            .Item("DECI_WT") = revUnitDr.Item("BETU_WT")
            .Item("DECI_UNCHIN") = revUnitDr.Item("UNCHIN")
            .Item("DECI_CITY_EXTC") = revUnitDr.Item("CITY_EXTC")
            .Item("DECI_WINT_EXTC") = revUnitDr.Item("WINT_EXTC")
            .Item("DECI_RELY_EXTC") = revUnitDr.Item("RELY_EXTC")
            .Item("DECI_TOLL") = revUnitDr.Item("TOLL")
            .Item("DECI_INSU") = revUnitDr.Item("INSU")
            .Item("KANRI_UNCHIN") = revUnitDr.Item("UNCHIN")
            .Item("KANRI_CITY_EXTC") = revUnitDr.Item("CITY_EXTC")
            .Item("KANRI_WINT_EXTC") = revUnitDr.Item("WINT_EXTC")
            .Item("KANRI_RELY_EXTC") = revUnitDr.Item("RELY_EXTC")
            .Item("KANRI_TOLL") = revUnitDr.Item("TOLL")
            .Item("KANRI_INSU") = revUnitDr.Item("INSU")
            'REMARK
            'SIZE_KB
            'TAX_KB
            'SAGYO_KANRI
...
#Region " ○1-7 運賃取得(calcUnchin CALL)"
    ...
    Private Sub GetUnchinRec(ByVal ds As DataSet, ByVal index As Integer)
        ...
        With calcDr
            ...
            .Item("UNSO_PKG_NB") = unchinDr.Item("SEIQ_NG_NB").ToString
            .Item("NB_UT") = ""
            ...
            .Item("VCLE_KB") = unchinDr.Item("SEIQ_SYARYO_KB").ToString
            .Item("MOTO_DATA_KB") = unsoLDr.Item("MOTO_DATA_KB").ToString
            .Item("SEIQ_TARIFF_CD") = unchinDr.Item("SEIQ_TARIFF_CD").ToString
            .Item("SEIQ_ETARIFF_CD") = unchinDr.Item("SEIQ_ETARIFF_CD").ToString
    ...
    Private Sub GetYokoUnchin(ByVal ds As DataSet, ByVal idx As Integer)
        ...
        Dim unchinDr As DataRow = ds.Tables(TABLE_NM_UNCHIN).Rows(idx)
        ...
        With calcDr
            ...
            .Item("WT_LV") = ""
            .Item("KYORI") = 0
            .Item("DANGER_KB") = unchinDr.Item("SEIQ_DANGER_KB").ToString
...
#Region " ○2-1 Main Method"
    ...
    Public Function CalcUnchin(ByVal ds As DataSet) As DataSet
        ...
        ' Out用DataTable以外は初期化(LMF800内でCallする場合は除外)
    ...
    Private Function GetUnchin(ByVal ds As DataSet, ByVal count As Integer) As Boolean
        ...
        '[割増運賃有無判定]
        If inTbl.Rows(0).Item("SEIQ_ETARIFF_CD").ToString.Equals("") = False Then
...
#Region " ○2-2 check"
    ...
    Private Function ChkInParamOnCalc(ByVal dr As DataRow) As Boolean
        ...
        'タリフコード
        If dr.Item("SEIQ_TARIFF_CD").ToString.Equals("") Then
    ...
    Private Function ChkSecondTariff(ByVal ds As DataSet) As Boolean
        ...
        If ds.Tables(TABLE_NM_CALC_KYORI).Rows(0).Item("UNCHIN_TARIFF_CD2").ToString.Equals("") = False Then
            chkFlg = True
        End If
...
#Region " ○2-3 Set param"
    ...
    Private Sub SetInDataToSecondTariff(ByVal ds As DataSet)
        ...
        With inDr
            ...
            .Item("SEIQ_TARIFF_CD") = ds.Tables(TABLE_NM_CALC_KYORI).Rows(0).Item("UNCHIN_TARIFF_CD2").ToString
            .Item("DEST_JIS") = ""
            .Item("CAR_TP") = ""
            .Item("WT_LV") = ""
...
#Region " ○2-5 運賃設定"
    ...
    Private Function GetExtcShuhai(ByVal ds As DataSet) As Double
        ...
        Else
            ...
            ' 集配料 + 保険料 を計算
            'START YANAI 要望番号782
            'retUnchin = Convert.ToDouble(ds.Tables(TABLE_NM_CALC_KYORI).Rows(0).Item(String.Concat("KYORI_", shuhaiRow))) _
    ...
    Private Function GetExtcWinter(ByVal ds As DataSet) As Double
        ...
        Dim retUnchin As Double = 0     '返却変数
        Dim extcDt As DataTable = ds.Tables(TABLE_NM_CALC_E_UNCHIN)      '割増運賃DataTable
        ...
        Select Case extcDt.Rows(0).Item("WINT_EXTC_YN").ToString
            Case EXTC_WINT_A, _
                ...
                Dim arrPlanDate As String = Right(Replace(inDt.Rows(0).Item("ARR_PLAN_DATE").ToString, "/", ""), 4)   '納入予定日 (下４桁)
    ...
    Private Function GetExtcToll(ByVal ds As DataSet) As Double
        ...
        If extcDt.Rows(0).Item("FRRY_EXTC_YN").ToString.Equals(FLG_ON) Then
            ...
            'START YANAI 要望番号377
            ''10KGあたりの航送料
            'retUnchin = Convert.ToDouble(ds.Tables(TABLE_NM_CALC_IN).Rows(0).Item("UNSO_WT").ToString) _
            '            / 10 _
            '            * Convert.ToDouble(ds.Tables(TABLE_NM_CALC_WEIGHT).Rows(Me._weightLine).Item("FRRY_EXTC_10KG").ToString)

            ''1件あたりの航送料
            'retUnchin = retUnchin _
            '            + Convert.ToDouble(ds.Tables(TABLE_NM_CALC_WEIGHT).Rows(Me._weightLine).Item("FRRY_EXTC_PART").ToString)
            '10KGあたりの航送料
            'START YANAI 要望番号889
            'retUnchin = Convert.ToDouble(ds.Tables(TABLE_NM_CALC_IN).Rows(0).Item("UNSO_WT").ToString) _
            '            / 10 _
            '            * Convert.ToDouble(extcDt.Rows(0).Item("FRRY_EXTC_10KG").ToString)
...
#Region " ○2-6 データ参照"
    ...
    Private Function GetExtcUnchin(ByVal ds As DataSet) As Boolean
        ...
        '[データアクセス]
        ds = Me.DacAccess(ds, ACT_GET_EXTC_UNCHIN)

        '[データ存在チェック]
        If Me.IsDataExists(ds, TABLE_NM_CALC_E_UNCHIN, FLG_ON, RESULT_WAR_APPLI) = False Then
            Return False
        End If
