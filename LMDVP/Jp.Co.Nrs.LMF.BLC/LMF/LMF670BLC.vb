...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMF670DAC = New LMF670DAC()
...
#Region "印刷"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMF670IN"
        Dim tableNmOut As String = "LMF670OUT"
        ...
        For i As Integer = 0 To max
            ...
            '出力枚数の取得
            setDs = MyBase.CallDAC(Me._Dac, "SelectPrintCount", setDs)

            prtCnt = MyBase.GetResultCount()

            '検索結果取得
            setDs = Me.SelectPrintData(setDs)

            Dim rowCnt As Integer = setDs.Tables("LMF670OUT").Rows.Count - 1

            For j As Integer = 1 To prtCnt

                For k As Integer = 0 To rowCnt

                    setDs.Tables("LMF670OUT").ImportRow(setDs.Tables("LMF670OUT").Rows(k))
                    setDs.Tables("LMF670OUT").Rows(k).Item("PRINT_NB") = Convert.ToInt32(setDs.Tables("LMF670OUT").Rows(k).Item("PRINT_NB")) + j
        ...
        'ソート実行
        Dim outDr As DataRow() = dtOut.Select(Nothing, "NRS_BR_CD ASC, UNSO_NO_L ASC, PRINT_NB ASC")
        ...
        For Each dr As DataRow In ds.Tables("M_RPT").Rows
            ...
            prtDs = New DataSet
            '指定したレポートIDのデータを抽出する。
            'prtDs = comPrt.CallDataSet(ds.Tables("LMF670OUT"), dr.Item("RPT_ID").ToString())
