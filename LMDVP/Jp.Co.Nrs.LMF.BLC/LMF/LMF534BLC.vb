...
'  システム名       :  LM
'  サブシステム名   :  LMF       : 運送
'  プログラムID     :  LMF534BLC : 運賃ﾁｪｯｸﾘｽﾄ(運行基準)
'  作  成  者       :  黎
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMF534DAC = New LMF534DAC()
...
#Region "印刷"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMF534IN"
        Dim tableNmOut As String = "LMF534OUT"
    ...
    Private Function EditPrintDataSet(ByVal prtId As String, ByVal ds As DataSet) As DataSet
        ...
        'Dim outDt As DataTable = ds.Tables("LMF534OUT") 'データ取得
        ...
        Select Case prtId
            ...
            'レポートIDがLMF511の場合、荷主コード、荷主勘定科目コード、荷主カテゴリ、出荷日でソートする
            Case "LMF534"
                Dim outDt As DataTable = ds.Tables("LMF534OUT") 'データ取得
                Dim max As Integer = outDt.Rows.Count - 1       '抽出データ数取得
                Dim count As Integer = 0                        '項番(初期値0)
                ...
                Dim BIN_NM As String = ""
                '===ソート後整列用===
                'Dim RefCN As String = outDt.Rows(0).Item("CAR_NO").ToString        'CAR_NO比較用
                'Dim RefTN As String = outDt.Rows(0).Item("TRIP_NO").ToString()  'TRIP_NO比較用
                'Dim TN As String = ""                                           'TRPI_NO設定用
                'Dim ReFLG As Boolean = False
                '====================
                '編集処理
                For i As Integer = 0 To max
                ...
                'ソート条件設定
                ' Dim sort As String = "TRIP_DATE   ASC,CAR_NO    ASC, TRIP_NO   ASC, DEST_CD  ASC,UNSO_NO_L  ASC"
                Dim sort As String = "TRIP_DATE   ASC,CAR_NO    ASC, DEST_CD  ASC,TRIP_NO   ASC, UNSO_NO_L  ASC"
                'ソート実行
                ...
                For Each row As DataRow In dr
                    rtnDs.Tables("LMF534OUT").ImportRow(row)
                Next

                '2012/11/26ソート後にトリップ番号の入れ替え仮追加開始
                'For i As Integer = 1 To max

                '    If RefCN = outDt.Rows(i).Item("CAR_NO").ToString() AND RefTN = outDt.Rows(i).Item("TRIP_NO").ToString()  Then

                '        If ReFLG = False Then

                '            TN = outDt.Rows(i).Item("TRIP_NO").ToString()

                '            ReFLG = True

                '        End If

                '        outDt.Rows(i).Item("TRIP_NO") = TN

                '    Else

                '        RefCN = outDt.Rows(i).Item("CAR_NO").ToString()

                '        RefTN = outDt.Rows(i).Item("TRIP_NO").ToString()
                ...
                'Next
                '2012/11/26ソート後にトリップ番号の入れ替え仮追加終了
