...
#Region "初期処理"
    ...
    Public Sub Main(ByVal prm As LMFormData)
        ...
        'フォームの作成
        Dim frm As LMZ330F = New LMZ330F(Me)
        ...
        'Gamenクラスの設定
        Me._G = New LMZ330G(Me, frm)

        'Validateクラスの設定
        Me._V = New LMZ330V(Me, frm, Me._LMZConV, Me._LMZConG, Me._G)
        ...
        Dim prmdRow As DataRow = Me._S.PrmDs.Tables(LMZ330C.TABLE_NM_IN).Rows(0)
        ...
        Dim outTbl As DataTable = Me._S.OutDs.Tables(LMZ330C.TABLE_NM_OUT)
        ...
        If Me._LMZConH.LoadMsgChk(frm, prmdRow, count) = True Then
            outTbl = Me._LMZConH.SetLoadData(Me._S, frm, outTbl, LMZ330C.TABLE_NM_OUT)
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub FrmShow(ByVal frm As LMZ330F)
...
#Region "イベント定義(一覧)"
    ...
    ''' <remarks></remarks>
    Private Sub CloseForm(ByVal frm As LMZ330F)

        Call Me._LMZConH.CloseForm(Me._S, LMZ330C.TABLE_NM_OUT)
    ...
    ''' <remarks></remarks>
    Private Sub SelectListEvent(ByVal frm As LMZ330F)
        ...
        Dim outTbl As DataTable = rtnDs.Tables(LMZ330C.TABLE_NM_OUT)
        Dim count As Integer = outTbl.Rows.Count
        ...
        If Me._LMZConH.CountRows(frm, frm.sprDetail, outTbl) = True AndAlso 0 < count Then
            ...
            'セッションクラスのOUTテーブルに設定
            Call Me._LMZConH.SetOutds(Me._S, outTbl, count, LMZ330C.TABLE_NM_OUT)
            '取得データをSPREADに表示
            Call Me._G.SetSpread(outTbl)
            'メッセージエリアの設定
    ...
    ''' <remarks></remarks>
    Private Sub RowSelection(ByVal frm As LMZ330F, ByVal e As Integer)
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Private Sub RowOkSelect(ByVal frm As LMZ330F)

        'チェックの付いたSpreadのRowIndexを取得
        Dim defNo As Integer = LMZ330G.sprDetailDef.DEF.ColNo
    ...
    ''' <param name="e">イベント詳細</param>
    ''' <remarks></remarks>
    Private Sub EnterAction(ByVal frm As LMZ330F, ByVal e As System.Windows.Forms.KeyEventArgs)
...
#Region "個別メソッド"
    ...
    ''' <remarks></remarks>
    Private Function SelectList(ByVal frm As LMZ330F) As DataSet
    ...
    ''' <remarks></remarks>
    Private Sub SelectionRowToFrm(ByVal frm As LMZ330F, ByVal rowIdx As Integer)

        If 0 < rowIdx Then

            'データテーブルから選択行を抽出
            Dim rowI As Integer = Convert.ToInt32(frm.sprDetail.ActiveSheet.Cells(rowIdx, LMZ330G.sprDetailDef.ROW_INDEX.ColNo).Value.ToString())

            Call Me._LMZConH.SetRtnParam(Me._S, New LMZ330DS(), LMZ330C.TABLE_NM_OUT, rowI)
...
#Region "DataSet設定"
    ...
    ''' <remarks></remarks>
    Private Sub SetDatasetTariffInData(ByVal frm As LMZ330F, ByVal ds As DataSet)

        Dim drow As DataRow = ds.Tables(LMZ330C.TABLE_NM_IN).NewRow

        With frm.sprDetail.ActiveSheet

            '営業所コードを画面間データから取得
            drow("NRS_BR_CD") = Me._S.PrmDs.Tables(LMZ330C.TABLE_NM_IN).Rows(0).Item("NRS_BR_CD")

            drow("UN_NO") = Me._LMZConV.GetCellValue(.Cells(0, LMZ330G.sprDetailDef.UN_NO.ColNo))
            drow("PG_KBN") = Me._LMZConV.GetCellValue(.Cells(0, LMZ330G.sprDetailDef.PG_KBN.ColNo))
            drow("IMDG_CLASS") = Me._LMZConV.GetCellValue(.Cells(0, LMZ330G.sprDetailDef.IMDG_CLASS.ColNo))
            drow("IMDG_CLASS1") = Me._LMZConV.GetCellValue(.Cells(0, LMZ330G.sprDetailDef.IMDG_CLASS1.ColNo))
            drow("IMDG_CLASS2") = Me._LMZConV.GetCellValue(.Cells(0, LMZ330G.sprDetailDef.IMDG_CLASS2.ColNo))
            drow("MP_FLG") = Me._LMZConV.GetCellValue(.Cells(0, LMZ330G.sprDetailDef.MP_FLG_NM.ColNo))

        End With

        ds.Tables(LMZ330C.TABLE_NM_IN).Rows.Add(drow)
...
#Region "イベント振分け"
    ...
    ''' <remarks></remarks>
    Friend Sub FunctionKey9Press(ByRef frm As LMZ330F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey11Press(ByRef frm As LMZ330F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey12Press(ByRef frm As LMZ330F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    Friend Sub ClosingForm(ByRef frm As LMZ330F, ByVal e As FormClosingEventArgs)
    ...
    ''' <param name="e">イベント詳細</param>
    ''' <remarks></remarks>
    Friend Sub sprCellDoubleClick(ByRef frm As LMZ330F, ByVal e As Integer)
    ...
    ''' <remarks></remarks>
    Friend Sub LMZ330F_KeyDown(ByVal frm As LMZ330F, ByVal e As System.Windows.Forms.KeyEventArgs)

        MyBase.Logger.StartLog(MyBase.GetType.Name, "LMZ330F_KeyDown")

        Call Me.EnterAction(frm, e)

        MyBase.Logger.EndLog(MyBase.GetType.Name, "LMZ330F_KeyDown")
