...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _V As LMZ240V

    ''' <summary>
    ''' Gamenクラスを格納するフィールド
    ''' </summary>
    ''' <remarks></remarks>
    Private _G As LMZ240G
...
#Region "初期処理"
    ...
    Public Sub Main(ByVal prm As LMFormData)
        ...
        'フォームの作成
        Dim frm As LMZ240F = New LMZ240F(Me)
        ...
        'Gamenクラスの設定
        Me._G = New LMZ240G(Me, frm)

        'Validateクラスの設定
        Me._V = New LMZ240V(Me, frm, Me._LMZConV, Me._LMZConG, Me._G)
        ...
        Dim prmdRow As DataRow = Me._S.PrmDs.Tables(LMZ240C.TABLE_NM_IN).Rows(0)
        ...
        If LMConst.FLG.ON.Equals(prmdRow("SEARCH_CS_FLG")) = True Then
            ...
            '荷主情報
            Dim custTbl As DataTable = Me._S.OutDs.Tables(LMZ240C.TABLE_NM_CUST)
        ...
        Else
            ...
            ds.Tables(LMZ240C.TABLE_NM_IN).ImportRow(prmdRow)
            '荷主コードの有無
            If String.IsNullOrEmpty(prmdRow("CUST_CD_L").ToString()) = True Then
                Me._S.OutDs = Me.SelectExtcUnchinOutListData(frm, ds)
            Else
                Me._S.OutDs = Me.SelectUnchinTariffSetOutListData(frm, ds)
                '初期検索結果件数が1件以外
                If Me._S.OutDs.Tables(LMZ240C.TABLE_NM_OUT).Rows.Count <> 1 Then
                    '画面ヘッダー部セット
        ...
        Dim outTbl As DataTable = Me._S.OutDs.Tables(LMZ240C.TABLE_NM_OUT)
        ...
        If Me._LMZConH.LoadMsgChk(frm, prmdRow, count) = True Then
            outTbl = Me._LMZConH.SetLoadData(Me._S, frm, outTbl, LMZ240C.TABLE_NM_OUT)
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub FrmShow(ByVal frm As LMZ240F)
...
#Region "イベント定義(一覧)"
    ...
    ''' <remarks></remarks>
    Private Sub CloseForm(ByVal frm As LMZ240F)

        Call Me._LMZConH.CloseForm(Me._S, LMZ240C.TABLE_NM_OUT)
    ...
    ''' <remarks></remarks>
    Private Sub SelectListEvent(ByVal frm As LMZ240F)

        '処理開始アクション
        Call Me._LMZConH.StartAction(frm)

        'START YANAI 要望番号1193 運賃タリフセットGRPのキャッシュを削除し、POP画面はDBからデータを取得する。
        Me._S.PrmDs.Tables(LMZ240C.TABLE_NM_IN).Rows(0)("SEARCH_CS_FLG") = LMConst.FLG.ON
        'END YANAI 要望番号1193 運賃タリフセットGRPのキャッシュを削除し、POP画面はDBからデータを取得する。

        '入力チェック
        If Me._V.IsInputChk(Me._S.PrmDs.Tables(LMZ240C.TABLE_NM_IN).Rows(0)("SEARCH_CS_FLG").ToString()) = False Then
        ...
        Dim outTbl As DataTable = rtnDs.Tables(LMZ240C.TABLE_NM_OUT)
        Dim count As Integer = outTbl.Rows.Count
        ...
        If Me._LMZConH.CountRows(frm, frm.sprDetail, outTbl) = True AndAlso 0 < count Then
            ...
            'セッションクラスのOUTテーブルに設定
            Call Me._LMZConH.SetOutds(Me._S, outTbl, count, LMZ240C.TABLE_NM_OUT)
            '取得データをSPREADに表示
            Call Me._G.SetSpread(outTbl)
            'メッセージエリアの設定
    ...
    ''' <remarks></remarks>
    Private Sub RowSelection(ByVal frm As LMZ240F, ByVal e As Integer)
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Private Sub RowOkSelect(ByVal frm As LMZ240F)

        'チェックの付いたSpreadのRowIndexを取得
        Dim defNo As Integer = LMZ240G.sprDetailDef.DEF.ColNo
    ...
    ''' <param name="e">イベント詳細</param>
    ''' <remarks></remarks>
    Private Sub EnterAction(ByVal frm As LMZ240F, ByVal e As System.Windows.Forms.KeyEventArgs)
...
#Region "個別メソッド"
    ...
    ''' <remarks></remarks>
    Private Function SelectList(ByVal frm As LMZ240F) As DataSet
        ...
        If prmdRow("SEARCH_CS_FLG").Equals(LMConst.FLG.ON) = True Then
            ...
            End If
            '荷主情報
            Dim custTbl As DataTable = rtnDs.Tables(LMZ240C.TABLE_NM_CUST)
            If 0 < custTbl.Rows.Count Then
                Call Me._G.CustHeaderDataSet(custTbl.Rows(0))
            Else
        ...
        Else
            ...
            If String.IsNullOrEmpty(frm.txtCustCdL.TextValue) = True Then
                ...
                Call Me._G.CustHeaderClear()
                'キャッシュテーブルからデータ抽出
                rtnDs = Me.SelectExtcUnchinOutListData(frm, ds)
            Else
    ...
    ''' <summary>
    ''' 割増運賃タリフマスタ(キャッシュテーブル)からデータを抽出する
    ''' </summary>
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <param name="ds">LMZ240DS</param>
    ''' <returns>抽出された行列データセット</returns>
    ''' <remarks></remarks>
    Private Function SelectExtcUnchinOutListData(ByVal frm As LMZ240F, ByVal ds As DataSet) As DataSet

        'DataSetのIN情報を取得
        Dim inTbl As DataTable = ds.Tables(LMZ240C.TABLE_NM_IN)
        ...
        Return Me._LMZConH.SelectListData(ds, LMZ240C.TABLE_NM_OUT, LMConst.CacheTBL.EXTC_UNCHIN, andstr.ToString(), sort)
    ...
    ''' <returns>抽出された行列データセット</returns>
    ''' <remarks></remarks>
    Private Function SelectUnchinTariffSetOutListData(ByVal frm As LMZ240F, ByVal ds As DataSet) As DataSet
        ...
        '▼▼キャッシュテーブルへの抽出条件文作成▼▼
        'タリフ区分 = "01"固定
        andstr.Append(Me._LMZConH.SetWhereData(andstr, LMZControlC.KBNCD1, LMZControlC.ConditionPattern.equal, "TARIFF_KBN"))
        ...
        'Return Me._LMZConH.SelectListData(ds, LMZ240C.TABLE_NM_OUT, LMConst.CacheTBL.UNCHIN_TARIFF_SET_GRP, andstr.ToString(), sort)
        Return Nothing
    ...
    ''' <remarks></remarks>
    Private Sub SelectionRowToFrm(ByVal frm As LMZ240F, ByVal rowIdx As Integer)

        If 0 < rowIdx Then

            'データテーブルから選択行を抽出
            Dim rowI As Integer = Convert.ToInt32(frm.sprDetail.ActiveSheet.Cells(rowIdx, LMZ240G.sprDetailDef.ROW_INDEX.ColNo).Value.ToString())

            Call Me._LMZConH.SetRtnParam(Me._S, New LMZ240DS(), LMZ240C.TABLE_NM_OUT, rowI)
...
#Region "DataSet設定"
    ...
    ''' <remarks></remarks>
    Private Sub SetDatasetTariffInData(ByVal frm As LMZ240F, ByVal ds As DataSet)

        Dim drow As DataRow = ds.Tables(LMZ240C.TABLE_NM_IN).NewRow
        ...
        With frm.sprDetail.ActiveSheet
            ...
            drow("EXTC_TARIFF_CD") = Me._LMZConV.GetCellValue(.Cells(0, LMZ240G.sprDetailDef.EXTC_TARIFF_CD.ColNo))
            drow("EXTC_TARIFF_REM") = Me._LMZConV.GetCellValue(.Cells(0, LMZ240G.sprDetailDef.EXTC_TARIFF_REM.ColNo))

        End With

        ds.Tables(LMZ240C.TABLE_NM_IN).Rows.Add(drow)
...
#Region "イベント振分け"
    ...
    ''' <remarks></remarks>
    Friend Sub FunctionKey9Press(ByRef frm As LMZ240F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey11Press(ByRef frm As LMZ240F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey12Press(ByRef frm As LMZ240F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    Friend Sub ClosingForm(ByRef frm As LMZ240F, ByVal e As FormClosingEventArgs)
    ...
    ''' <param name="e">イベント詳細</param>
    ''' <remarks></remarks>
    Friend Sub sprCellDoubleClick(ByRef frm As LMZ240F, ByVal e As Integer)
    ...
    ''' <remarks></remarks>
    Friend Sub LMZ240F_KeyDown(ByVal frm As LMZ240F, ByVal e As System.Windows.Forms.KeyEventArgs)

        MyBase.Logger.StartLog(MyBase.GetType.Name, "LMZ240F_KeyDown")

        Call Me.EnterAction(frm, e)

        MyBase.Logger.EndLog(MyBase.GetType.Name, "LMZ240F_KeyDown")
