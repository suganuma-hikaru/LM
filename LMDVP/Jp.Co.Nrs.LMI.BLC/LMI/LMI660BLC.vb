...
'  システム名       :  LM
'  サブシステム名   :  LMI       : 特定荷主機能
'  プログラムID     :  LMI660BLC : 請求鑑(DIC鑑種別：A+鑑種別B（横持料）)
'  作  成  者       :  yamanaka
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMI660DAC = New LMI660DAC()
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        If dtOut.Rows.Count = 0 Then
            MyBase.SetMessageStore("00", "E078", New String() {"請求鑑(LMI660)"})
            Return ds
        End If
    ...
    Private Function EditPrintDataSet(ByVal rptId As String, ByVal ds As DataSet) As DataSet
        ...
        Dim outDt As DataTable = outDs.Tables("LMI660OUT")
        Dim outDr As DataRow = Nothing
        Dim dr As DataRow() = ds.Tables("LMI660OUT").Select
        ...
        For i As Integer = 0 To max
            ...
            outDr.Item("TTL") = Convert.ToDecimal(outDr.Item("TTL")) + Convert.ToDecimal(dr(i).Item("TTL"))
            outDr.Item("TAX_TTL") = Convert.ToDecimal(outDr.Item("TAX_TTL")) + Convert.ToDecimal(dr(i).Item("TAX_TTL"))
            outDr.Item("G_TTL") = Convert.ToDecimal(outDr.Item("G_TTL")) + Convert.ToDecimal(dr(i).Item("G_TTL"))
