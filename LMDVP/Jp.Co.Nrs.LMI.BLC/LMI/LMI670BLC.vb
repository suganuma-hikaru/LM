...
'  システム名       :  LM
'  サブシステム名   :  LMF       : 特殊荷主機能
'  プログラムID     :  LMI670BLC : 売上仕入伝票(DIC鑑種別：A+鑑種別B（横持料）)
'  作  成  者       :  寺川
' ==========================================================================
...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMI670DAC = New LMI670DAC()
...
#Region "印刷"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMI670IN"
        Dim tableNmOut As String = "LMI670OUT"
        ...
        If dtOut.Rows.Count = 0 Then
            MyBase.SetMessageStore("00", "E078", New String() {"売上仕入伝票(LMI670)"})
            Return ds
        End If
        ...
        For Each dr As DataRow In ds.Tables("M_RPT").Rows
            ...
            prtDs = New DataSet
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables("LMI670OUT"), dr.Item("RPT_ID").ToString())
            ''帳票ごとの編集があるなら行う。
