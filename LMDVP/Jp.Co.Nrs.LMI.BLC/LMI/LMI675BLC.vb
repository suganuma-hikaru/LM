...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMI675DAC = New LMI675DAC()
...
#Region "印刷"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMI675IN"
        Dim tableNmOut As String = "LMI675OUT"
        ...
        If dtOut.Rows.Count = 0 Then
            MyBase.SetMessageStore("00", "E078", New String() {"鑑 + 売上仕入伝票(LMI675)"})
            Return ds
        End If
        ...
        For Each dr As DataRow In ds.Tables("M_RPT").Rows
            ...
            prtDs = New DataSet
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables("LMI675OUT"), dr.Item("RPT_ID").ToString())
            ''帳票ごとの編集があるなら行う。
