...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMI650DAC = New LMI650DAC()
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMI650IN"
        Dim tableNmOut As String = "LMI650OUT"
        ...
        If dtOut.Rows.Count = 0 Then
            MyBase.SetMessageStore("00", "E078", New String() {"請求明細書(LMI650)"})
            Return ds
        End If
        ...
        For Each dr As DataRow In ds.Tables("M_RPT").Rows
            ...
            prtDs = New DataSet
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables("LMI650OUT"), dr.Item("RPT_ID").ToString())
            '帳票ごとの編集があるなら行う。
    ...
    Private Function EditPrintDataSet(ByVal rptId As String, ByVal ds As DataSet) As DataSet
        ...
        'ソート実行
        Dim dr As DataRow() = ds.Tables("LMI650OUT").Select(Nothing, "KAGAMI_KB, MEISAI_NO")

        '別テーブル作成
        Dim outDt As DataTable = ds.Tables("LMI650OUT").Clone
        ...
        'データテーブルをクリア
        ds.Tables("LMI650OUT").Clear()

        '別テーブルをマージ
        ds.Tables("LMI650OUT").Merge(outDt)
