...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMI980DAC = New LMI980DAC()
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMI980IN"
        Dim tableNmOut As String = "LMI980OUT"
        ...
        If dtOut.Rows.Count = 0 Then
            MyBase.SetMessage("E011", New String() {"運送依頼書(LMI980)"})
            Return ds
        End If
        ...
        For Each dr As DataRow In ds.Tables("M_RPT").Rows
            ...
            prtDs = New DataSet
            '指定したレポートIDのデータを抽出する。
            prtDs = comPrt.CallDataSet(ds.Tables("LMI980OUT"), dr.Item("RPT_ID").ToString())
            '帳票CSV出力
    ...
    Private Function EditPrintDataSet(ByVal rptId As String, ByVal ds As DataSet) As DataSet
        ...
        'ソート実行
        Dim dr As DataRow() = ds.Tables("LMI980OUT").Select(Nothing, "KAGAMI_KB, MEISAI_NO")

        '別テーブル作成
        Dim outDt As DataTable = ds.Tables("LMI980OUT").Clone
        ...
        'データテーブルをクリア
        ds.Tables("LMI980OUT").Clear()

        '別テーブルをマージ
        ds.Tables("LMI980OUT").Merge(outDt)
