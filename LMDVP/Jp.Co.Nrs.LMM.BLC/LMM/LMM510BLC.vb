...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMM510DAC = New LMM510DAC()
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMM510IN"
        Dim tableNmOut As String = "LMM510OUT"
        ...
        For i As Integer = 0 To max
            ...
            '検索結果取得
            setDs = Me.SelectDetailsData(setDs)

            'データセットの編集
            Call Me.EditPrintDataSetDetails(setDs)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function EditPrintDataSetDetails(ByVal ds As DataSet) As DataSet


        'DataSetのIN情報を取得
        Dim outTbl As DataTable = ds.Tables("LMM510OUT")
        'DataSetのを取得
        Dim detailTbl As DataTable = ds.Tables("LMM510_GOODS_DETAILS")
        '設定項目名作成用
        Dim setNaiyoNm As String = String.Empty
        Dim setNaiyoTaitleNm As String = String.Empty


        Dim max As Integer = detailTbl.Rows.Count - 1


        For i As Integer = 0 To max

            '設定項目名作成
            setNaiyoNm = String.Concat("SET_NAIYO", "_", (i + 1).ToString())
            setNaiyoTaitleNm = String.Concat("SET_NAIYO_TAITLE", "_", (i + 1).ToString())

            If i < 9 Then
                setNaiyoNm = String.Concat("SET_NAIYO", "_", "0", (i + 1).ToString())
                outTbl.Rows(0).Item(setNaiyoNm) = detailTbl.Rows(i).Item("SET_NAIYO").ToString()
                setNaiyoTaitleNm = String.Concat("SET_NAIYO_TAITLE", "_", "0", (i + 1).ToString())
                outTbl.Rows(0).Item(setNaiyoTaitleNm) = detailTbl.Rows(i).Item("SUB_KB").ToString()
            ElseIf max > 9 Then
                outTbl.Rows(0).Item(setNaiyoNm) = String.Concat("*", detailTbl.Rows(i).Item("SET_NAIYO").ToString())
                outTbl.Rows(0).Item(setNaiyoTaitleNm) = detailTbl.Rows(i).Item("SUB_KB").ToString()
                Exit For
            Else
                outTbl.Rows(0).Item(setNaiyoNm) = detailTbl.Rows(i).Item("SET_NAIYO").ToString()
    ...
    Private Function SelectDetailsData(ByVal ds As DataSet) As DataSet
        ...
        Return MyBase.CallDAC(Me._Dac, "SelectDetailsData", ds)
