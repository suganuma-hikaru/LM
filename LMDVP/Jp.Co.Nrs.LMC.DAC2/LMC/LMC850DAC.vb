...
#Region "検索処理 SQL"
    ...
    Private Const SQL_SELECT_MEITETU_CSV As String = " SELECT                                                          " & vbNewLine _
                                                   ...
                                                   & "      , SUBSTRING(OUTKAL.OUTKA_PLAN_DATE,1,4) +                  " & vbNewLine _
                                                   & "        '/' +                                                    " & vbNewLine _
                                                   & "        SUBSTRING(OUTKAL.OUTKA_PLAN_DATE,5,2) +                  " & vbNewLine _
                                                   & "        '/' +                                                    " & vbNewLine _
                                                   & "        SUBSTRING(OUTKAL.OUTKA_PLAN_DATE,7,2) AS SYUKKABI        " & vbNewLine _
                                                   ...
                                                   & "      --(2013.01.30)月日の前ZEROを消すために型変換処理を行う -- START --      " & vbNewLine _
                                                   ...
                                                   & "      --(2013.01.30)月日の前ZEROを消すために型変換処理を行う --  END  --      " & vbNewLine _
                                                   & "      , DEST3.DEST_NM AS SHIP_NM_L                                " & vbNewLine _
                                                   & "      , CUST.DENPYO_NM AS DENPYO_NM                               " & vbNewLine _
                                                   & " --   , OKURIJOCSV.FREE_C03 AS KIJI_1                             " & vbNewLine _
                                                   & " --   , OKURIJOCSV.FREE_C04 AS KIJI_2                             " & vbNewLine _
                                                   & " --   , OKURIJOCSV.FREE_C05 AS KIJI_3                             " & vbNewLine _
                                                   & " --   , OKURIJOCSV.FREE_C06 AS KIJI_4                             " & vbNewLine _
                                                   & " --★2013.02.26 / Notes1896 荷主コードが見当たらない場合強制'XXXXX'指定開始              " & vbNewLine _
                                                   & " ,CASE WHEN OKURIJOCSV.NRS_BR_CD IS NOT NULL                                             " & vbNewLine _
                                                   ...
                                                   & "      , OUTKAL.ARR_PLAN_DATE AS ARR_PLAN_DATE                    " & vbNewLine
    ''' <summary>
    ''' 名鉄CSV作成(埼玉)データ検索用SQL FROM・WHERE部
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SELECT_MEITETU_CSV_FROM As String = "  FROM $LM_TRN$..C_OUTKA_L OUTKAL                           " & vbNewLine _
                                                        ...
                                                        & "       --送り状CSVレイアウトＭ                              " & vbNewLine _
                                                        & " --      LEFT JOIN $LM_MST$..M_OKURIJO_CSV OKURIJOCSV       " & vbNewLine _
                                                        ...
                                                        & " --           AND OKURIJOCSV.OKURIJO_TP = '03'              " & vbNewLine _
                                                        & " --★2013.02.26 / Notes1896 荷主コードが見当たらない場合強制'XXXXX'指定開始              " & vbNewLine _
                                                        ...
                                                        & " OKURIJOCSV.OKURIJO_TP  = '03'                                                           " & vbNewLine _
                                                        ...
                                                        & " OKURIJOCSVX.OKURIJO_TP = '03'                                                           " & vbNewLine _
                                                        & " --★2013.02.26 / Notes1896 荷主コードが見当たらない場合強制'XXXXX'指定終了              " & vbNewLine _
                                                        & "       --荷主Ｍ                                             " & vbNewLine _
    ...
    Private Const SQL_SELECT_MEITETU_CSV_GROUPBY As String = " GROUP BY                        " & vbNewLine _
                                                           ...
                                                           & " --★2013.02.26 / Notes1896 追加終了 " & vbNewLine
...
#Region "検索処理"
    ...
    Private Function SelectMeitetuCsv(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim inTbl As DataTable = ds.Tables("LMC850IN")
        ...
        'SQL作成
        Me._StrSql.Append(LMC850DAC.SQL_SELECT_MEITETU_CSV)
        Me._StrSql.Append(LMC850DAC.SQL_SELECT_MEITETU_CSV_FROM)
        Me._StrSql.Append(LMC850DAC.SQL_SELECT_MEITETU_CSV_GROUPBY)
        ...
        MyBase.Logger.WriteSQLLog("LMC850DAC", "SelectMeitetuCsv", cmd)
        ...
        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMC850OUT")

        '処理件数の設定
        MyBase.SetResultCount(ds.Tables("LMC850OUT").Rows.Count())
        reader.Close()
    ...
    Private Function UpdateMeitetuCsv(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Me._Row = ds.Tables("LMC850OUT").Rows(0)
        ...
        'スキーマ名設定
        Dim sql As String = Me.SetSchemaNm(LMC850DAC.SQL_UPDATE_MEITETU_CSV, Me._Row.Item("NRS_BR_CD").ToString())
        ...
        MyBase.Logger.WriteSQLLog("LMC850DAC", "UpdateMeitetuCsv", cmd)
