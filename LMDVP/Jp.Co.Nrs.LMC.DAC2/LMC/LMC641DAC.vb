...
#End If
    ...
    Private Const SQL_SELECT_DATA As String _
        ...
        & "--,INS.LT_DATE                                          AS LT_DATE  --DEL 2023/06/20 依頼番号 : 036507           " & vbNewLine _
        & ",ZAI.LT_DATE                                          AS LT_DATE    --INS 2023/06/20 依頼番号 : 036507           " & vbNewLine _
    ...
    Private Const SQL_FROM As String _
        ...
        & "AND MCR1.PTN_ID = 'D3'                                                       " & vbNewLine _
        ...
        & "AND MCR2.PTN_ID = 'D3'                                                       " & vbNewLine _
        & "--帳票パターン取得                                                           " & vbNewLine _
    ...
    Private Const SQL_SELECT_DATA_FREE As String = "SELECT                                                                                " & vbNewLine _
                                               ...
                                               & "      WHEN CUD2.SUB_KB = '9T' AND RTRIM(ISNULL(EDIM.FREE_C04, '')) <> '' THEN EDIM.FREE_C04 --ADD 2022/09/06 029847 フィルメ対応 " & vbNewLine _
                                               & "      WHEN CUD3.SUB_KB = 'B2' AND RTRIM(ISNULL(EDIM.FREE_C02, '')) <> '' THEN EDIM.FREE_C02 -- アフトン " & vbNewLine _
    ...
    Private Const SQL_FROM_FREE As String = "--出荷L                                                                 " & vbNewLine _
                                     ...
                                     & "-- 追加終了 ---                                                                                                                " & vbNewLine _
                                     & "-- ADD 2022/09/06 フィルメ対応　029847                                                                                         " & vbNewLine _
                                     ...
                                     & "LEFT JOIN LM_MST..M_CUST_DETAILS CUD3                                                                                          " & vbNewLine _
                                     & "ON  OUTL.NRS_BR_CD = CUD3.NRS_BR_CD                                                                                            " & vbNewLine _
                                     & "AND OUTL.CUST_CD_L = CUD3.CUST_CD                                                                                              " & vbNewLine _
                                     & "AND CUD3.CUST_CLASS  = '00'                                                                                                    " & vbNewLine _
                                     & "AND CUD3.SUB_KB      = 'B2'                                                                                                    " & vbNewLine _
                                     & "AND CUD3.SET_NAIYO   = '1'                                                                                                     " & vbNewLine _
                                     & "AND CUD3.SYS_DEL_FLG = '0'                                                                                                     " & vbNewLine _
                                     & "--出荷S                                                                      " & vbNewLine _
                                     & "LEFT JOIN $LM_TRN$..C_OUTKA_S OUTS                                           " & vbNewLine _
                                     & "ON  OUTS.NRS_BR_CD = OUTL.NRS_BR_CD                                          " & vbNewLine _
                                     & "AND OUTS.OUTKA_NO_L = OUTL.OUTKA_NO_L                                        " & vbNewLine _
...
#Region "検索処理"
    ...
    Private Function SelectMPrt(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim inTbl As DataTable = ds.Tables("LMC641IN")
        ...
        'SQL作成
        Me._StrSql.Append(LMC641DAC.SQL_SELECT_MPrt)      'SQL構築(帳票種別用Select句)
        Me._StrSql.Append(LMC641DAC.SQL_FROM)       'SQL構築(データ抽出用From句)
        Me._StrSql.Append(LMC641DAC.SQL_WHERE)
        ...
        MyBase.Logger.WriteSQLLog("LMC641DAC", "SelectMPrt", cmd)
    ...
    Private Function SelectTouNo(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMC641DAC.SQL_SELECT_TouNo)      'SQL構築(帳票種別用Select句)
        Call Me.SetConditionMasterSQL()                   '条件設定
    ...
    Private Function SelectMCustDetailsData(ByVal ds As DataSet, ByVal SyoriKBN As String) As DataSet
        ...
        'INTableの条件rowの格納
        Me._Row = ds.Tables("LMC641IN").Rows(0)
        ...
        Select Case SyoriKBN
            Case "0"
                '荷姿並び順
                Me._StrSql.Append(LMC641DAC.SQL_SELECT_MCUST_DETAILS)      'SQL構築(荷主明細マスタ設定値Select句)
                Call Me.setIndataParameter(Me._Row)                        '条件設定
            Case "1"
                'FREE_C03項目使用有無
                Me._StrSql.Append(LMC641DAC.SQL_SELECT_MCUST_DETAILS_FREE) 'SQL構築(荷主明細マスタ設定値Select句)
                Call Me.setIndataParameter(Me._Row)                        '条件設定
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        Select Case rptTbl.Rows(0).Item("RPT_ID").ToString()
            ...
            Case Else
                ...
                If FreeC03_Umu = "01" Then
                    'FREE_C03使用(サクラ)
                    Me._StrSql.Append(LMC641DAC.SQL_SELECT_DATA)        'SQL構築(データ抽出用SELECT句)
                    Me._StrSql.Append(LMC641DAC.SQL_SELECT_ADD_SAGYO)
                    Me._StrSql.Append(LMC641DAC.SQL_FROM)               'SQL構築(データ抽出用FROM句)
                    Me._StrSql.Append(LMC641DAC.SQL_FROM_ADD_SAGYO)
                    Me._StrSql.Append(LMC641DAC.SQL_WHERE)

                    Call Me.SetConditionMasterSQL()                     'SQL構築(条件設定)
                    Me._StrSql.Append(LMC641DAC.SQL_GROUP_BY)           'SQL構築(データ抽出用GROUP BY句)
                Else
                    If "600".Equals(wh_cd) Then
                        'FREE_C03未使用(中部物流センター)
                        Me._StrSql.Append(LMC641DAC.SQL_SELECT_DATA_FREE_WH600) 'SQL構築(データ抽出用SELECT句)
                        Me._StrSql.Append(LMC641DAC.SQL_SELECT_ADD_SAGYO)
                        Me._StrSql.Append(LMC641DAC.SQL_FROM_FREE)              'SQL構築(データ抽出用FROM句)
                        Me._StrSql.Append(LMC641DAC.SQL_FROM_ADD_SAGYO)
                        Me._StrSql.Append(LMC641DAC.SQL_WHERE)
                        Call Me.SetConditionMasterSQL()                         'SQL構築(条件設定)
                        Me._StrSql.Append(LMC641DAC.SQL_GROUP_BY_FREE_WH600)    'SQL構築(データ抽出用GROUP BY句)
                        Me._StrSql.Append(LMC641DAC.SQL_GROUP_BY_FREE_ADD)      'SQL構築(データ抽出用GROUP BY句)
                    Else
                        'FREE_C03未使用(通常)
                        Me._StrSql.Append(LMC641DAC.SQL_SELECT_DATA_FREE)   'SQL構築(データ抽出用SELECT句)
                        Me._StrSql.Append(LMC641DAC.SQL_SELECT_ADD_SAGYO)
                        Me._StrSql.Append(LMC641DAC.SQL_FROM_FREE)          'SQL構築(データ抽出用FROM句)
                        Me._StrSql.Append(LMC641DAC.SQL_FROM_ADD_SAGYO)
                        Me._StrSql.Append(LMC641DAC.SQL_WHERE)
                        Call Me.SetConditionMasterSQL()                     'SQL構築(条件設定)
                        Me._StrSql.Append(LMC641DAC.SQL_GROUP_BY_FREE)      'SQL構築(データ抽出用GROUP BY句)
                        Me._StrSql.Append(LMC641DAC.SQL_GROUP_BY_FREE_ADD)      'SQL構築(データ抽出用GROUP BY句)
                    End If
                End If
                ...
                If String.IsNullOrEmpty(setNaiyo) = True Then
                    Me._StrSql.Append(LMC641DAC.SQL_ORDER_BY1)         'SQL構築(データ抽出用ORDER BY句)
                Else
                    Me._StrSql.Append(LMC641DAC.SQL_ORDER_BY1)         'SQL構築(データ抽出用ORDER BY句)
                ...
                If "600".Equals(wh_cd) Then
                    '中部物流センター
                    Me._StrSql.Append(LMC641DAC.SQL_ORDER_BY_WH600)    'SQL構築(データ抽出用ORDER BY句)
                Else
                    '通常
                    Me._StrSql.Append(LMC641DAC.SQL_ORDER_BY2)         'SQL構築(データ抽出用ORDER BY句)
                    Me._StrSql.Append(LMC641DAC.SQL_ORDER_BY3)         'SQL構築(データ抽出用ORDER BY句)
                End If
        ...
        MyBase.Logger.WriteSQLLog("LMC641DAC", "SelectPrintData", cmd)
        ...
        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMC641OUT")
