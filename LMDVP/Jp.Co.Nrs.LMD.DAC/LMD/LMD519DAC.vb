...
#Region "検索処理 SQL"
    ...
    ''' <summary>
    ''' 印刷データ抽出用map.Add("CHECK_NO", "CHECK_NO")
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SELECT_DATA As String = "  SELECT CASE WHEN MR2.PTN_CD IS NOT NULL THEN MR2.RPT_ID               " & vbNewLine _
                                            & "              WHEN MR1.PTN_CD IS NOT NULL THEN MR1.RPT_ID               " & vbNewLine _
                                            & "         ELSE MR3.RPT_ID                                                " & vbNewLine _
                                            & "         END                           AS RPT_ID       --帳票ID         " & vbNewLine _
                                            & "       , ZAITRS.NRS_BR_CD              AS NRS_BR_CD    --営業所コード   " & vbNewLine _
                                            & "       , ZAITRS.WH_CD                  AS WH_CD        --倉庫コード     " & vbNewLine _
                                            & "       , ZAITRS.CUST_CD_L              AS CUST_CD_L    --荷主コード(大) " & vbNewLine _
                                            & "       , ZAITRS.CUST_CD_M              AS CUST_CD_M    --荷主コード(中) " & vbNewLine _
                                            & "       , GOODS.CUST_CD_S               AS CUST_CD_S    --荷主コード(小) " & vbNewLine _
                                            & "       , CUST.CUST_NM_L                AS CUST_NM_L    --荷主名(大)     " & vbNewLine _
                                            & "       , ZAITRS.TOU_NO                 AS TOU_NO       --棟             " & vbNewLine _
                                            & "       , ZAITRS.SITU_NO                AS SITU_NO      --室             " & vbNewLine _
                                            & "       , RTRIM(ZAITRS.ZONE_CD)         AS ZONE_CD      --ZONE           " & vbNewLine _
                                            & "       , ZAITRS.LOCA                   AS LOCA         --ロケーション   " & vbNewLine _
                                            & "       , ZAITRS.SERIAL_NO              AS MASTER_NO    --MASTER №      " & vbNewLine _
                                            & "       , CARGO.HOUSE_NO                AS HOUSE_NO     --HOUSE_NO       " & vbNewLine _
                                            & "       , INKA_L.INKA_DATE              AS INKA_DATE    --入荷日         " & vbNewLine _
                                            & "       , CARGO_DTL.MARK                AS MARK         --MARK           " & vbNewLine _
                                            & "       , ZAITRS.PORA_ZAI_NB            AS PORA_ZAI_NB  --個数           " & vbNewLine _
                                            & "       , ISNULL(KBN1.KBN_NM1,'')       AS PKG_UT       --荷姿           " & vbNewLine _
                                            & "       , ISNULL(CARGO_DTL.CHECK_NO,'') AS CHECK_NO     --社内識別番号   " & vbNewLine
    ...
    Private Const SQL_FROM As String = " FROM $LM_TRN$..D_ZAI_TRS ZAITRS                           " & vbNewLine _
                                     ...
                                     & "             ON INKA_L.NRS_BR_CD = ZAITRS.NRS_BR_CD        " & vbNewLine _
                                     & "            AND INKA_L.INKA_NO_L = ZAITRS.INKA_NO_L        " & vbNewLine _
                                     & "            AND INKA_L.SYS_DEL_FLG = '0'                   " & vbNewLine _
                                     & "      --貨物詳細[NF_DB]                                                               " & vbNewLine _
                                     & "      LEFT JOIN NF_DB..D_CARGO_DETL CARGO_DTL  --★2013.02.25 / Notes1891 対応 => × [CARGODTL]" & vbNewLine _
                                     & "             ON CARGO_DTL.ORD_NO         = SUBSTRING(ZAITRS.REMARK,1,7)               " & vbNewLine _
                                     & "            AND CARGO_DTL.CARGO_SEQ      = CONVERT(int,SUBSTRING(ZAITRS.REMARK,8,3))  " & vbNewLine _
                                     & "            AND CARGO_DTL.CARGO_DETL_SEQ = CONVERT(int,SUBSTRING(ZAITRS.REMARK,11,3)) " & vbNewLine _
                                     & "            AND CARGO_DTL.SYS_DEL_FLG    = '0'                                        " & vbNewLine _
                                     & "--    --貨物詳細[NF_DB]                                                  " & vbNewLine _
                                     & "--    LEFT JOIN NF_DB..D_CARGO_DETL CARGO_DTL                            " & vbNewLine _
                                     & "--           ON CARGO_DTL.ORD_NO         = SUBSTRING(ZAITRS.REMARK,1,7)  " & vbNewLine _
                                     & "--          AND CARGO_DTL.CARGO_SEQ      = SUBSTRING(ZAITRS.REMARK,8,3)  " & vbNewLine _
                                     & "--          AND CARGO_DTL.CARGO_DETL_SEQ = SUBSTRING(ZAITRS.REMARK,11,3) " & vbNewLine _
                                     & "--          AND CARGO_DTL.SYS_DEL_FLG  = '0'                             " & vbNewLine _
                                     & "--旧  LEFT JOIN NF_DB..D_CARGO_DETL CARGO_DTL              " & vbNewLine _
                                     & "--旧         ON CARGO_DTL.IFKEY_TO_LMS = ZAITRS.REMARK     " & vbNewLine _
                                     & "--旧        AND CARGO_DTL.IFKEY_TO_LMS <> ''               " & vbNewLine _
                                     & "--旧        AND CARGO_DTL.SYS_DEL_FLG  = '0'               " & vbNewLine _
                                     & "      --貨物テーブル                                       " & vbNewLine _
                                     & "      LEFT JOIN NF_DB..D_CARGO CARGO                       " & vbNewLine _
                                     & "             ON CARGO.ORD_NO    = CARGO_DTL.ORD_NO         " & vbNewLine _
                                     & "            AND CARGO.CARGO_SEQ = CARGO_DTL.CARGO_SEQ      " & vbNewLine _
                                     & "            AND CARGO.SYS_DEL_FLG = '0'                    " & vbNewLine _
                                     & "      --商品Ｍ                                             " & vbNewLine _
                                     ...
                                     & "             ON CUST.NRS_BR_CD   = ZAITRS.NRS_BR_CD        " & vbNewLine _
                                     ...
                                     & "             ON KBN1.KBN_CD = GOODS.PKG_UT                 " & vbNewLine _
                                     ...
                                     & "            AND MCR1.PTN_ID = 'AB'                         " & vbNewLine _
                                     ...
                                     & "            AND MCR2.PTN_ID = 'AB'                         " & vbNewLine _
                                     & "      --帳票パターン取得                                   " & vbNewLine _
                                     ...
                                     & "   WHERE KBN1.KBN_GROUP_CD = 'S004') KBN2                  " & vbNewLine _
                                     & "  ON SHOBO.RUI = KBN2.KBN_CD                               " & vbNewLine
...
#Region "検索処理"
    ...
    Private Function SelectMPrt(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim inTbl As DataTable = ds.Tables("LMD519IN")
        ...
        'SQL作成
        Me._StrSql.Append(LMD519DAC.SQL_SELECT_MPrt)    'SQL構築(帳票種別用Select句)
        Me._StrSql.Append(LMD519DAC.SQL_FROM)           'SQL構築(データ抽出用From句)
        ...
        MyBase.Logger.WriteSQLLog("LMD519DAC", "SelectMPrt", cmd)
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        Me._StrSql.Append(LMD519DAC.SQL_SELECT_DATA)    'SQL構築(データ抽出用Select句)
        Me._StrSql.Append(LMD519DAC.SQL_FROM)           'SQL構築(データ抽出用From句)
        Call Me.SetConditionMasterSQL()                 'SQL構築(条件設定)
        Me._StrSql.Append(LMD519DAC.SQL_ORDER_BY)       'SQL構築(データ抽出用ORDER BY句)
        ...
        MyBase.Logger.WriteSQLLog("LMD519DAC", "SelectPrintData", cmd)
        ...
        map.Add("LOCA", "LOCA")
        map.Add("MASTER_NO", "MASTER_NO")
        map.Add("HOUSE_NO", "HOUSE_NO")
        map.Add("INKA_DATE", "INKA_DATE")
        map.Add("MARK", "MARK")
        map.Add("PORA_ZAI_NB", "PORA_ZAI_NB")
        map.Add("PKG_UT", "PKG_UT")
        map.Add("CHECK_NO", "CHECK_NO")

        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMD519OUT")
