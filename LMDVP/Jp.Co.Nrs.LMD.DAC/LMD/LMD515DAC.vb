...
#Region "検索処理 SQL"
    ...
    Private Const SQL_SELECT_DATA As String = " SELECT                                                                         " & vbNewLine _
                                            ...
                                            & " ,ZAITRS.ALLOC_CAN_NB % GOODS.PKG_NB                      AS ALLOC_CAN_HASU     " & vbNewLine _
                                            & " ,ISNULL(CUSTCOND.JOTAI_NM,'')                            AS GOODS_COND_NM_3    " & vbNewLine
    ...
    Private Const SQL_FROM As String = "FROM                                                                                   " & vbNewLine _
                                      ...
                                      & "$LM_MST$..M_CUSTCOND CUSTCOND ON                                                      " & vbNewLine _
                                      & "CUSTCOND.NRS_BR_CD = ZAITRS.NRS_BR_CD                                                 " & vbNewLine _
                                      & "AND CUSTCOND.CUST_CD_L = ZAITRS.CUST_CD_L                                             " & vbNewLine _
                                      & "AND CUSTCOND.JOTAI_CD = ZAITRS.GOODS_COND_KB_3                                        " & vbNewLine _
                                      & "LEFT OUTER JOIN                                                                       " & vbNewLine _
                                      ...
                                      & "AND MCR1.PTN_ID = 'A7'                                                                " & vbNewLine _
                                      ...
                                      & "AND MCR2.PTN_ID = 'A7'                                                                " & vbNewLine _
                                      & "--帳票パターン取得                                                                    " & vbNewLine _
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ORDER_BY511 As String = "ORDER BY                                                                        " & vbNewLine _
                                         & "    ZAITRS.WH_CD                                                                   " & vbNewLine _
                                         & "    ,ZAITRS.TOU_NO                                                                 " & vbNewLine _
                                         & "    ,ZAITRS.SITU_NO                                                                " & vbNewLine _
                                         & "    ,ZAITRS.ZONE_CD                                                                " & vbNewLine _
...
#Region "検索処理"
    ...
    Private Function SelectMPrt(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim inTbl As DataTable = ds.Tables("LMD515IN")
        ...
        'SQL作成
        Me._StrSql.Append(LMD515DAC.SQL_SELECT_MPrt)      'SQL構築(帳票種別用Select句)
        Me._StrSql.Append(LMD515DAC.SQL_FROM)             'SQL構築(データ抽出用From句)
        ...
        MyBase.Logger.WriteSQLLog("LMD515DAC", "SelectMPrt", cmd)
    ...
    Private Function SelectPrintData(ByVal ds As DataSet) As DataSet
        ...
        If ("LMD511").Equals(_rptRow.Item("RPT_ID").ToString) = False Then

            'SQL作成
            Me._StrSql.Append(LMD515DAC.SQL_SELECT_DATA)      'SQL構築(データ抽出用Select句)
            Me._StrSql.Append(LMD515DAC.SQL_FROM)             'SQL構築(データ抽出用From句)
            Call Me.SetConditionMasterSQL()                   'SQL構築(条件設定)
            Me._StrSql.Append(LMD515DAC.SQL_ORDER_BY)         'SQL構築(データ抽出用ORDER BY句)
        Else
            'SQL作成
            Me._StrSql.Append(LMD515DAC.SQL_SELECT_DATA)      'SQL構築(データ抽出用Select句)
            Me._StrSql.Append(LMD515DAC.SQL_FROM)             'SQL構築(データ抽出用From句)
            Call Me.SetConditionMasterSQL()                   'SQL構築(条件設定)
            Me._StrSql.Append(LMD515DAC.SQL_ORDER_BY511)      'SQL構築(データ抽出用ORDER BY句)
        ...
        MyBase.Logger.WriteSQLLog("LMD515DAC", "SelectPrintData", cmd)
        ...
        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMD515OUT")
