...
#Region "初期処理"
    ...
    Public Sub Main(ByVal prm As LMFormData)
        ...
        'フォームの作成
        Dim frm As LMF050F = New LMF050F(Me)
        ...
        'Gamenクラスの設定
        Me._G = New LMF050G(Me, frm, Me._LMFConG, Me._V)

        'Validateクラスの設定
        Me._V = New LMF050V(Me, frm, Me._LMFConV, Me._LMFConG, Me._G)
        ...
        'START YANAI 要望番号561
        '連続入力フラグの設定
        If ("01").Equals(prmDs.Tables(LMF050C.TABLE_NM_IN).Rows(0).Item("RENZOKU_FLG").ToString) = True Then
            Me._RenzokuFlg = True
        End If
        'END YANAI 要望番号561
        ...
        'ファンクションキーの設定
        'START YANAI 要望番号561
        'Call Me._G.SetFunctionKey(LMF050C.EventShubetsu.MAIN)
        Call Me._G.SetFunctionKey(LMF050C.EventShubetsu.MAIN, Me._RenzokuFlg)
        'END YANAI 要望番号561
        ...
        Dim ds As DataSet = Me.SetDataSetInData(frm, New LMF050DS())
        Dim blf As String = String.Concat(MyBase.GetPGID(), LMControlC.BLF)

        'START YANAI 要望番号561
        Me._RenzokuCnt = 0
        prmDs.Tables(LMF050C.TABLE_NM_IN).Rows(0).Item("RENZOKU_CNT") = Me._RenzokuCnt
        Me._DsInData = prmDs
        'END YANAI 要望番号561

        prmDs = MyBase.CallWSA(blf, LMF050C.ACTION_ID_SELECT, prmDs)
        ...
        '荷主、出荷日のdrから取得
        Dim dt As DataTable = prmDs.Tables(LMF050C.TABLE_NM_OUT)
        Dim max As Integer = dt.Rows.Count - 1

        Dim seiqGrouopNo As String = String.Empty
        Dim seiqGrouopNoM As String = String.Empty
        ...
        For i As Integer = 0 To max
            ...
            'START YANAI 要望番号583
            seiqGrouopNo = dr.Item("SEIQ_GROUP_NO").ToString
            seiqGrouopNoM = dr.Item("SEIQ_GROUP_NO_M").ToString
            unsoNoL = dr.Item("UNSO_NO_L").ToString
            unsoNoM = dr.Item("UNSO_NO_M").ToString
            'END YANAI 要望番号583

            'まとめか判定
            If String.IsNullOrEmpty(seiqGrouopNo) = False _
                AndAlso String.IsNullOrEmpty(seiqGrouopNoM) = False Then

                '荷主がまとめの時は親を表示
                If seiqGrouopNo.Equals(unsoNoL) = True _
                    AndAlso seiqGrouopNoM.Equals(unsoNoM) = True Then
                    Exit For
                End If
...
#Region "イベント定義(一覧)"
    ...
    ''' <remarks></remarks>
    Private Sub ShiftEditMode(ByVal frm As LMF050F)

        '処理開始アクション
        Call Me.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMF050C.EventShubetsu.HENSHU) = False Then
            '終了処理
            Call Me.EndAction(frm)
            'START YANAI 要望番号561
            If Me._RenzokuFlg = True Then
                Call Me._G.SetFunctionKey(LMF050C.EventShubetsu.HENSHU, Me._RenzokuFlg)
            End If
            'END YANAI 要望番号561
            Exit Sub
        End If
        ...
        '他営業所選択チェック
        If Me._V.IsUserNrsBrCdChk(frm, LMF050C.EventShubetsu.HENSHU) = False Then
            Call Me.EndAction(frm)
            'START YANAI 要望番号561
            If Me._RenzokuFlg = True Then
                Call Me._G.SetFunctionKey(LMF050C.EventShubetsu.HENSHU, Me._RenzokuFlg)
        ...
        'WSAクラス呼出
        '==========================
        Me._Ds = MyBase.CallWSA("LMF050BLF", "HaitaData", _Ds)
        ...
        'ファンクションキーの設定
        'START YANAI 要望番号561
        'Call Me._G.SetFunctionKey(LMF050C.EventShubetsu.HENSHU)
        Call Me._G.SetFunctionKey(LMF050C.EventShubetsu.HENSHU, Me._RenzokuFlg)
        'END YANAI 要望番号561
    ...
    ''' <param name="frm">フォーム</param>
    ''' <remarks></remarks>
    Private Sub OpenMasterPop(ByVal frm As LMF050F)

        'カーソル位置の設定
        Dim objNm As String = frm.FocusedControlName()

        '権限チェック
        Dim rtnResult As Boolean = Me._V.IsAuthorityChk(LMF050C.EventShubetsu.MASTEROPEN)

        'カーソル位置チェック
        rtnResult = rtnResult AndAlso Me._V.IsFocusChk(objNm, LMF050C.EventShubetsu.MASTEROPEN)
        ...
        Call Me.ShowPopupControl(frm, objNm, LMF050C.EventShubetsu.MASTEROPEN)
    ...
    ''' <remarks></remarks>
    Private Sub EnterAction(ByVal frm As LMF050F, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        '権限チェック
        rtnResult = rtnResult AndAlso Me._V.IsAuthorityChk(LMF050C.EventShubetsu.MASTEROPEN)

        'カーソル位置チェック
        rtnResult = rtnResult AndAlso Me._V.IsFocusChk(objNm, LMF050C.EventShubetsu.ENTER)
        ...
        Call Me.ShowPopupControl(frm, objNm, LMF050C.EventShubetsu.ENTER)
    ...
    ''' <remarks></remarks>
    Private Function SaveUncoItemData(ByVal frm As LMF050F, ByVal eventShubetsu As LMF050C.EventShubetsu) As Boolean
        ...
        '権限チェック
        If Me._V.IsAuthorityChk(LMF050C.EventShubetsu.SAVE) = False Then
        ...
        ''項目チェック
        If Me._V.IsInputCheckSave() = False Then
        ...
        '更新処理(保存処理)
        Me._Ds = MyBase.CallWSA("LMF050BLF", "UpdateData", Me._Ds)
        ...
        If Me._RenzokuFlg = True Then
            ...
            If Me._KakuteiFlg = True Then
                '終了処理
                Call Me.EndAction(frm)
                If Me.Kakutei(frm, LMF050C.EventShubetsu.KAKUTEI) = False Then
                    Me._KakuteiFlg = False
            ...
            'スキップ処理を行う
            Me.skipData(frm, LMF050C.EventShubetsu.SKIP)
        ...
        Else
            ...
            '更新成功の場合、再検索
            Dim blf As String = String.Concat(MyBase.GetPGID(), LMControlC.BLF)
            Me._DsInData.Tables(LMF050C.TABLE_NM_IN).Rows(0).Item("RENZOKU_CNT") = Me._RenzokuCnt
            Me._Ds = MyBase.CallWSA(blf, LMF050C.ACTION_ID_SELECT, Me._DsInData)
            'END YANAI 要望番号561
            ...
            'ファンクションキーの設定
            'START YANAI 要望番号561
            'Call Me._G.SetFunctionKey(LMF050C.EventShubetsu.SAVE)
            Call Me._G.SetFunctionKey(LMF050C.EventShubetsu.SAVE, Me._RenzokuFlg)
            'END YANAI 要望番号561
            Dim unsoL As String = String.Empty
            Dim unsoM As String = String.Empty
            Dim msg As String = String.Empty

            If String.IsNullOrEmpty(Me._LMFConG.GetCellValue(frm.sprDetail.ActiveSheet.Cells(0, LMF050G.sprDetailDef.GROUP.ColNo))) = True Then

                unsoL = Me._LMFConG.GetCellValue(frm.sprDetail.ActiveSheet.Cells(0, LMF050G.sprDetailDef.UNSO_NO.ColNo))
                unsoM = Me._LMFConG.GetCellValue(frm.sprDetail.ActiveSheet.Cells(0, LMF050G.sprDetailDef.UNSO_NO_EDA.ColNo))
            ...
            MyBase.ShowMessage(frm, "G079", New String() {frm.FunctionKey.F11ButtonName, unsoL, unsoM})
    ...
    ''' <returns>True:エラーなし,OK False:エラーあり</returns>
    ''' <remarks></remarks>
    Private Function Kakutei(ByVal frm As LMF050F, ByVal eventShubetsu As LMF050C.EventShubetsu) As Boolean

        '処理開始アクション
        Call Me.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMF050C.EventShubetsu.KAKUTEI) = False Then
            Call Me.EndAction(frm)  '終了処理
            Return False
        End If

        '他営業所選択チェック
        If Me._V.IsUserNrsBrCdChk(frm, LMF050C.EventShubetsu.KAKUTEI) = False Then
            Call Me.EndAction(frm)
            Exit Function
        End If
        ...
        Dim dt As DataTable = _Ds.Tables(LMF050C.TABLE_NM_OUT)
        ...
        '更新処理(確定処理)
        Me._Ds = MyBase.CallWSA("LMF050BLF", "UpdateDataKakutei", _Ds)
        ...
        'ファンクションキーの設定
        'START YANAI 要望番号561
        'Call Me._G.SetFunctionKey(LMF050C.EventShubetsu.KAKUTEI)
        Call Me._G.SetFunctionKey(LMF050C.EventShubetsu.KAKUTEI, Me._RenzokuFlg)
        'END YANAI 要望番号561
        Dim unsoL As String = String.Empty
        Dim unsoM As String = String.Empty
        Dim msg As String = String.Empty
        ...
        If Me._KakuteiFlg = False Then
            '2016.01.06 UMANO 英語化対応START
            'MyBase.ShowMessage(frm, "G002", New String() {"確定", msg})
            MyBase.ShowMessage(frm, "G079", New String() {frm.FunctionKey.F5ButtonName, unsoL, unsoM})
            '2016.01.06 UMANO 英語化対応END
        Else
            '2016.01.06 UMANO 英語化対応START
            'MyBase.ShowMessage(frm, "G002", New String() {"保存", msg})
    ...
    ''' <returns>True:エラーなし,OK False:エラーあり</returns>
    ''' <remarks></remarks>
    Private Function KakuteiKaijo(ByVal frm As LMF050F, ByVal eventShubetsu As LMF050C.EventShubetsu) As Boolean

        '処理開始アクション
        Call Me.StartAction(frm)
      
        '権限チェック
        If Me._V.IsAuthorityChk(LMF050C.EventShubetsu.KAKUTEIKAIJO) = False Then
            Call Me.EndAction(frm)  '終了処理
            Return False
        End If

        '他営業所選択チェック
        If Me._V.IsUserNrsBrCdChk(frm, LMF050C.EventShubetsu.KAKUTEIKAIJO) = False Then
            Call Me.EndAction(frm)
            Exit Function
        End If
        ...
        ''更新処理(確定解除)
        Me._Ds = MyBase.CallWSA("LMF050BLF", "UpdateDataKakuteikaijo", _Ds)
        ...
        MyBase.ShowMessage(frm, "G079", New String() {frm.FunctionKey.F6ButtonName, unsoL, unsoM})
        '2016.01.06 UMANO 英語化対応END
...
#Region "再計算処理"
    ...
    ''' <returns>True:エラーなし,OK False:エラーあり</returns>
    ''' <remarks></remarks>
    Private Function Silica(ByVal frm As LMF050F) As Boolean
        ...
        If (LMFControlC.TARIFF_KONSAI).Equals(tariffKbn) = True OrElse _
            ...
            If 0 < unchinTariffDr.Length Then
                If (LMFControlC.TABTP_KOKEN).Equals(unchinTariffDr(0).Item("TABLE_TP").ToString()) OrElse _
                    ...
                    'START YANAI 要望番号938
                    'deciKyori = dr.Item("DEST_JIS_CD").ToString.Substring(0, 2)
        ...
        '運送日は締め日基準で切り替える
        Dim unsoDate As String = frm.imdShukka.TextValue
        ...
        With prmDr
            ...
            ''届先コードが入力されていない場合は"0000000"を設定
            'If String.IsNullOrEmpty(dr.Item("DEST_JIS_CD").ToString()) = True Then
            '    .Item("DEST_JIS") = "0000000"
            'Else
            '    .Item("DEST_JIS") = dr.Item("DEST_JIS_CD").ToString
            'End If
            ...
            Dim outTbl As DataTable = rtnDs.Tables("UNCHIN_CALC_OUT")

            'LMF800RESULTからエラーメッセージを取得する
            Dim errDr As DataRow = rtnDs.Tables("LMF800RESULT").Rows(0)
            ...
            Select Case errDr.Item("STATUS").ToString()
                ...
                Case Else
                    ...
                    '要望番号:1023 yamanaka 2013.03.27 Start
                    'Call Me.EndAction(frm)
                    'Return False
                    '要望番号:1023 yamanaka 2013.03.27 End
        ...
        For i As Integer = 0 To max
            ...
            dr.Item("SEIQ_TARIFF_BUNRUI_KB") = tariffKbn
            dr.Item("SEIQ_TARIFF_BUNRUI_NM") = tariffNm
            dr.Item("SEIQ_TARIFF_CD") = tariffCd
            dr.Item("SEIQ_ETARIFF_CD") = extcCd
            dr.Item("SEIQ_PKG_UT_KB") = nisugataKbn
            dr.Item("SEIQ_PKG_UT_NM") = nisugataNm
            dr.Item("SEIQ_SYARYO_KB") = shashu
            dr.Item("SEIQ_SYARYO_NM") = shashuNm
            dr.Item("SEIQ_DANGER_KB") = kiken
            dr.Item("SEIQ_DANGER_NM") = kikenNm
    ...
    ''' <returns>True:エラーなし,OK False:エラーあり</returns>
    ''' <remarks></remarks>
    Private Function skipData(ByVal frm As LMF050F, ByVal eventShubetsu As LMF050C.EventShubetsu) As Boolean

        If (Me._RenzokuCnt).Equals(Me._DsInData.Tables(LMF050C.TABLE_NM_IN).Rows.Count - 1) = True Then
            '最終データの場合はフォームを閉じる
        ...
        '荷主、出荷日のdrから取得
        Dim dt As DataTable = Me._Ds.Tables(LMF050C.TABLE_NM_OUT)
        Dim max As Integer = dt.Rows.Count - 1
...
#Region "按分処理"
    ...
    Private Function SetUncinData(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            '重量の設定
            juryo = Convert.ToDecimal(Me._LMFConG.FormatNumValue(dr.Item("SEIQ_WT").ToString()))
...
#Region "タリフ分類区分値変更"
    ...
    Private Sub Bunrui(ByVal frm As LMF050F)
...
#End Region
    ...
    ''' <remarks></remarks>
    Friend Sub CloseForm(ByVal frm As LMF050F, ByVal e As FormClosingEventArgs)
        ...
        Select Case MyBase.ShowMessage(frm, "W002")
            ...
            Case MsgBoxResult.Yes  '「はい」押下時
                ...
                If Me.SaveUncoItemData(frm, LMF050C.EventShubetsu.TOJIRU) = False Then
...
#Region "Pop"
    ...
    ''' <returns>True:エラーなし,OK False:エラーあり</returns>
    ''' <remarks></remarks>
    Private Function ShowPopupControl(ByVal frm As LMF050F, ByVal objNm As String, ByVal actionType As LMF050C.EventShubetsu) As Boolean
...
#Region "起動Popタリフマスタ照会、割増タリフマスタ照会のPGが完了しているか確認"
    ...
    ''' <remarks></remarks>
    Private Function TariffPop(ByVal frm As LMF050F, ByVal actinType As LMF050C.EventShubetsu) As Boolean
    ...
    ''' <param name="frm">フォーム</param>
    ''' <remarks></remarks>
    Private Function ShowTariffPopup(ByVal frm As LMF050F, ByVal actinType As LMF050C.EventShubetsu) As LMFormData

        Dim ds As DataSet = New LMZ230DS()
        Dim dt As DataTable = ds.Tables(LMZ230C.TABLE_NM_IN)
        Dim dr As DataRow = dt.NewRow()
        '運賃締め日基準の取得
        Dim Prmdt As DataTable = _Ds.Tables(LMF050C.TABLE_NM_OUT)
        ...
        With dr
            ...
            If actinType = LMF050C.EventShubetsu.ENTER Then
    ...
    ''' <returns>True:選択有 False:選択無</returns>
    ''' <remarks></remarks>
    Private Function WarimashiPop(ByVal frm As LMF050F, ByVal actinType As LMF050C.EventShubetsu) As Boolean
    ...
    ''' <param name="frm">フォーム</param>
    ''' <remarks></remarks>
    Private Function ShowWarimashiPopup(ByVal frm As LMF050F, ByVal actinType As LMF050C.EventShubetsu) As LMFormData
    ...
    ''' <returns>True:選択有 False:選択無</returns>
    ''' <remarks></remarks>
    Private Function YokoTariffPop(ByVal frm As LMF050F, ByVal actionType As LMF050C.EventShubetsu) As Boolean
    ...
    ''' <param name="frm">フォーム</param>
    ''' <remarks></remarks>
    Private Function ShowYokoTariffPopup(ByVal frm As LMF050F, ByVal actionType As LMF050C.EventShubetsu) As LMFormData
        ...
        With dr
            ...
            .Item("NRS_BR_CD") = frm.cmbEigyo.SelectedValue.ToString()
            'START SHINOHARA 要望番号513
            If actionType = LMF050C.EventShubetsu.ENTER Then
                'END SHINOHARA 要望番号513
                '.Item("CUST_CD_L") = frm.lblCustCdL.TextValue
                '.Item("CUST_CD_M") = frm.lblCustCdM.TextValue
...
#Region "イベント振分け"
    ...
    ''' <remarks></remarks>
    Friend Sub FunctionKey2Press(ByVal frm As LMF050F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey5Press(ByVal frm As LMF050F, ByVal e As System.Windows.Forms.KeyEventArgs)

        MyBase.Logger.StartLog(MyBase.GetType.Name, System.Reflection.MethodBase.GetCurrentMethod.Name)

        Me.Kakutei(frm, LMF050C.EventShubetsu.KAKUTEI)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey6Press(ByVal frm As LMF050F, ByVal e As System.Windows.Forms.KeyEventArgs)

        MyBase.Logger.StartLog(MyBase.GetType.Name, System.Reflection.MethodBase.GetCurrentMethod.Name)

        Me.KakuteiKaijo(frm, LMF050C.EventShubetsu.KAKUTEIKAIJO)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey7Press(ByVal frm As LMF050F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey10Press(ByVal frm As LMF050F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey11Press(ByVal frm As LMF050F, ByVal e As System.Windows.Forms.KeyEventArgs)

        MyBase.Logger.StartLog(MyBase.GetType.Name, System.Reflection.MethodBase.GetCurrentMethod.Name)

        Call Me.SaveUncoItemData(frm, LMF050C.EventShubetsu.SAVE)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey12Press(ByVal frm As LMF050F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub ButtonSilica(ByVal frm As LMF050F, ByVal e As System.EventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub Bunrui(ByVal frm As LMF050F, ByVal e As System.EventArgs)
    ...
    Friend Sub ClosingForm(ByVal frm As LMF050F, ByVal e As FormClosingEventArgs)
    ...
    ''' <remarks></remarks>
    Friend Sub KeyDown(ByVal frm As LMF050F, ByVal e As System.Windows.Forms.KeyEventArgs)
...
#Region "データセット"
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Private Function SetDataSetInData(ByVal frm As LMF050F, ByVal ds As DataSet) As DataSet

        Dim dt As DataTable = ds.Tables(LMF050C.TABLE_NM_IN)
        Dim dr As DataRow = dt.NewRow()

        With frm
            'パラメータはLMF040のものを検索条件とする
    ...
    ''' <remarks></remarks>
    Private Function SetDatasetUp(ByVal frm As LMF050F, ByVal ds As DataSet) As DataSet
        ...
        With spr
            ...
            For i As Integer = 0 To max
                ...
                syayo = Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.SYARYO_KB.ColNo)).ToString()
                pkg = Convert.ToDecimal(Me._LMFConG.FormatNumValue(Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.KOSU.ColNo)).ToString()))
                danger = Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.DANGER_KB.ColNo)).ToString()
                tariff = Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.TARIFF_CD.ColNo)).ToString()
                warimasi = Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.WARIMASHI.ColNo)).ToString()
                tariffkb = Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.TARIF_BUN.ColNo)).ToString()
                wt = Convert.ToDecimal(Me._LMFConG.FormatNumValue(Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.JURYO.ColNo)).ToString()))
                kyori = Convert.ToDecimal(Me._LMFConG.FormatNumValue(Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.KYORI.ColNo)).ToString()))
                untichin = Convert.ToDecimal(Me._LMFConG.FormatNumValue(Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.UNCHIN.ColNo)).ToString()))
                toshi = Convert.ToDecimal(Me._LMFConG.FormatNumValue(Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.TOSHI.ColNo)).ToString()))
                toki = Convert.ToDecimal(Me._LMFConG.FormatNumValue(Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.TOKI.ColNo)).ToString()))
                thukei = Convert.ToDecimal(Me._LMFConG.FormatNumValue(Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.TYUKEI.ColNo)).ToString()))
                koso = Convert.ToDecimal(Me._LMFConG.FormatNumValue(Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.KOSO.ColNo)).ToString()))
                sonota = Convert.ToDecimal(Me._LMFConG.FormatNumValue(Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.SONOTA.ColNo)).ToString()))
                biko = Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.REMARK.ColNo)).ToString()
                zei = Me._LMFConG.GetCellValue(.Cells(i, LMF050G.sprDetailDef.ZEI_KB.ColNo)).ToString()
            ...
            For j As Integer = 0 To max
                If rowNo <> j Then
                    sumWt = sumWt + Convert.ToDecimal(Me._LMFConG.FormatNumValue(Me._LMFConG.GetCellValue(.Cells(j, LMF050G.sprDetailDef.JURYO.ColNo)).ToString()))
                End If
            Next
            ...
            dr.Item("SEIQ_SYARYO_KB") = syayo
            dr.Item("DECI_NG_NB") = pkg
            dr.Item("SEIQ_DANGER_KB") = danger
            dr.Item("SEIQ_TARIFF_CD") = tariff
            dr.Item("SEIQ_ETARIFF_CD") = warimasi
            dr.Item("SEIQ_TARIFF_BUNRUI_KB") = tariffkb
            dr.Item("DECI_WT") = wt
            dr.Item("DECI_KYORI") = kyori
            ...
            dr.Item("DECI_UNCHIN") = untichin
            dr.Item("REMARK") = biko
            'START YANAI 要望番号1267 運賃重量を変更しても運送重量が変わらない
            dr.Item("SEIQ_WT") = wt - sumWt
            'END YANAI 要望番号1267 運賃重量を変更しても運送重量が変わらない
    ...
    Private Function SetDataJug(ByVal spr As SheetView, ByVal rowNo As Integer) As Boolean
        ...
        With spr
            ...
            Dim groupL As String = Me._LMFConG.GetCellValue(.Cells(rowNo, LMF050G.sprDetailDef.GROUP.ColNo))
            ...
            '親レコードの場合、True
            If groupL.Equals(Me._LMFConG.GetCellValue(.Cells(rowNo, LMF050G.sprDetailDef.UNSO_NO.ColNo))) _
                AndAlso Me._LMFConG.GetCellValue(.Cells(rowNo, LMF050G.sprDetailDef.GROUP_UNSO.ColNo)).Equals(Me._LMFConG.GetCellValue(.Cells(rowNo, LMF050G.sprDetailDef.UNSO_NO_EDA.ColNo))) = True _
                Then
                Return True
            End If
...
#Region "ユーティリティ(共通クラスでメソッドが作成されたら共通クラスを使用(開始、終了アクション)"
    ...
    ''' <param name="frm">フォーム</param>
    ''' <remarks></remarks>
    Private Sub StartAction(ByVal frm As LMF050F)
    ...
    ''' <param name="frm">フォーム</param>
    ''' <remarks></remarks>
    Private Sub EndAction(ByVal frm As LMF050F)
...
#Region "メッセージ設定(共通クラスでメソッドができたら共通クラスを使用)"
    ...
    ''' <param name="frm">フォーム</param>
    ''' <remarks></remarks>
    Private Sub ShowGMessage(ByVal frm As LMF050F)
    ...
    ''' <param name="frm">フォーム</param>
    ''' <remarks></remarks>
    Private Sub SetGMessage(ByVal frm As LMF050F)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function ConfirmMsg(ByVal frm As LMF050F, ByVal msg As String) As Boolean
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function ConfirmIKakuteiMsg(ByVal frm As LMF050F, ByVal msg As String) As Boolean
