...
#Region "Method"
    ...
    Private Sub MakeCsv(ByVal prmDs As DataSet)
        ...
        Dim setOutDt As DataTable = setDs.Tables("LMF860OUT")
        setOutDt.Clear()

        '並び替えをする
        Dim outDr As DataRow() = prmDs.Tables("LMF860OUT").Select(Nothing, "CUST_CD_L,JIS,NIUKENIN_CD,SYUKKABI,HAISOBI,DENPYO_NO")
        ...
        '保存先のCSVファイルのパス
        Dim csvPath As String = String.Concat(prmDs.Tables("LMF860OUT").Rows(0).Item("FILEPATH").ToString, _
                                              prmDs.Tables("LMF860OUT").Rows(0).Item("FILENAME").ToString, _
                                              prmDs.Tables("LMF860OUT").Rows(0).Item("SYS_DATE").ToString, _
                                               "-", _
                                              Left(prmDs.Tables("LMF860OUT").Rows(0).Item("SYS_TIME").ToString, 6), _
                                              ".csv")
        ...
        'ファイルを開く
        System.IO.Directory.CreateDirectory(prmDs.Tables("LMF860OUT").Rows(0).Item("FILEPATH").ToString)
