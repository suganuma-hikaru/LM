...
#Region "SELECT句"
    ...
    Private Const SQL_SELECT_DATA As String = "SELECT                                                                                          " & vbNewLine _
                                            ...
                                            & "    ,MG.HIKIATE_ALERT_YN                                                                                        AS HIKIATE_ALERT_YN         " & vbNewLine

    ''' <summary>
    ''' CUST_Mデータ抽出用(DSP用)
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SELECT_DATA_DSP As String = "SELECT                                                                                          " & vbNewLine _
                                            & "     MG.NRS_BR_CD                                                                                               AS NRS_BR_CD                " & vbNewLine _
                                            & "    ,MG.GOODS_NM_1                                                                                              AS GOODS_NM_1               " & vbNewLine _
                                            & "    ,(SELECT KBN_NM1 FROM LM_MST..Z_KBN ZK WHERE ZK.KBN_GROUP_CD = 'I001' AND MG.STD_IRIME_UT = ZK.KBN_CD)      AS STD_IRIME_NM             " & vbNewLine _
                                            & "    ,MGD.SET_NAIYO                                                                                              AS GOODS_CD_CUST            " & vbNewLine _
                                            & "    ,IsNull(ZAI.ALLOC_CAN_NB, 0)																				   AS ALLOC_CAN_NB	           " & vbNewLine _
                                            & "    ,ZAI.LOT_NO              																				   AS LOT_NO    	           " & vbNewLine _
                                            & "    ,(SELECT KBN_NM1 FROM LM_MST..Z_KBN ZK WHERE ZK.KBN_GROUP_CD = 'K002' AND MG.NB_UT = ZK.KBN_CD)             AS NB_UT_NM                 " & vbNewLine _
                                            & "    ,MG.NB_UT                                                                                                   AS NB_UT                    " & vbNewLine _
...
#Region "FROM句"
    ...
    Private Const SQL_FROM_DATA As String = "FROM                                                                                                                                          " & vbNewLine _
                                            ...
                                            & "	    FROM [LM_TRN_40].[dbo].[D_ZAI_TRS]                                                                                                     " & vbNewLine _
                                            & "	    WHERE [SYS_DEL_FLG] = '0'                                                                                                              " & vbNewLine _
                                            & "	    AND   [ALLOC_CAN_NB] > 0                                                                                                               " & vbNewLine _
                                            & "	    GROUP BY [NRS_BR_CD],[GOODS_CD_NRS],[CUST_CD_L],[CUST_CD_M],[LOT_NO]) ZAI                                                              " & vbNewLine _
                                            & "	   ON MG.NRS_BR_CD = ZAI.NRS_BR_CD                                                                                                         " & vbNewLine _
                                            & "	   AND MG.GOODS_CD_NRS = ZAI.GOODS_CD_NRS                                                                                                  " & vbNewLine
...
#Region "検索処理"
    ...
    Private Function SelectData(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim inTbl As DataTable = ds.Tables("LMZ021IN")
        ...
        'SQL作成
        Me._StrSql.Append(LMZ021DAC.SQL_SELECT_COUNT)     'SQL構築(カウント用Select句)
        Me._StrSql.Append(LMZ021DAC.SQL_FROM_DATA)        'SQL構築(カウント用from句)
        Me._StrSql.Append(LMZ021DAC.SQL_FROM_DATA_DETAIL_DISP)
        ...
        MyBase.Logger.WriteSQLLog("LMZ021DAC", "SelectData", cmd)
    ...
    Private Function SelectListData(ByVal ds As DataSet) As DataSet
        ...
        'SQL作成
        If Me._Row.Item("DSP_FLG").ToString().Equals("1") = True Then
            Me._StrSql.Append(LMZ021DAC.SQL_SELECT_DATA_DSP)  'SQL構築(データ抽出用Select句DSP用)
        Else
            Me._StrSql.Append(LMZ021DAC.SQL_SELECT_DATA)      'SQL構築(データ抽出用Select句)
        End If
        Me._StrSql.Append(LMZ021DAC.SQL_FROM_DATA)        'SQL構築(データ抽出用from句)
        Me._StrSql.Append(LMZ021DAC.SQL_FROM_DATA_DETAIL_DISP)

        Call Me.SetConditionMasterSQL()                   '条件設定
        Me._StrSql.Append(LMZ021DAC.SQL_ORDER_BY)         'SQL構築(データ抽出用ORDER BY句)
        ...
        MyBase.Logger.WriteSQLLog("LMZ021DAC", "SelectListData", cmd)
        ...
        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMZ021OUT")
    ...
    Private Sub SetConditionMasterSQL()
        ...
        With Me._Row
            ...
            If .Item("DSP_FLG").ToString().Equals("1") = True Then
                'DSP五協の場合
...
#End If
            ...
            If String.IsNullOrEmpty(whereStr) = False Then
                Me._StrSql.Append("AND ZAI.LOT_NO = @LOT_NO")
                Me._StrSql.Append(vbNewLine)
                Me._SqlPrmList.Add(MyBase.GetSqlParameter("@LOT_NO", whereStr, DBDataType.NVARCHAR))
            End If

            whereStr = .Item("ALLOC_CAN_NB").ToString()
            If String.IsNullOrEmpty(whereStr) = False Then
                Me._StrSql.Append("AND ZAI.ALLOC_CAN_NB = @ALLOC_CAN_NB")
                Me._StrSql.Append(vbNewLine)
                Me._SqlPrmList.Add(MyBase.GetSqlParameter("@ALLOC_CAN_NB", whereStr, DBDataType.NUMERIC))
            End If
