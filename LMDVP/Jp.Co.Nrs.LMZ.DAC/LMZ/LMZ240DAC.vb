...
'  システム名       :  LM
'  サブシステム名   :  LMZ       : 共通
'  プログラムID     :  LMZ240DAC : 割増運賃タリフマスタ照会
'  作  成  者       :  平山
' ==========================================================================
Imports System.Text
...
#Region "FROM句"
    ...
    Private Const SQL_FROM_DATA_EXTC As String = "FROM                                                                                " & vbNewLine _
                                              & "      $LM_MST$..M_EXTC_UNCHIN  AS EXTC                                                 " & vbNewLine _
                                              & "      WHERE                                                                          " & vbNewLine _
...
#Region "検索処理"
    ...
    Private Function SelectData(ByVal ds As DataSet) As DataSet
        ...
        'DataSetのIN情報を取得
        Dim inTbl As DataTable = ds.Tables("LMZ240IN")
        ...
        If String.IsNullOrEmpty(Me._Row.Item("CUST_CD_L").ToString()) = True Then
            Me._StrSql.Append(LMZ240DAC.SQL_SELECT_COUNT_EXTC)     'SQL構築(カウント用Select句:横持ちヘッダ)
            Me._StrSql.Append(LMZ240DAC.SQL_FROM_DATA_EXTC)        'SQL構築(カウント用from句)
            Call Me.SetConditionMasterSQL("EXTC")               '条件設定
        Else
            Me._StrSql.Append(LMZ240DAC.SQL_SELECT_COUNT_UNCHIN)   'SQL構築(カウント用Select句:運賃タリフセット)
            Me._StrSql.Append(LMZ240DAC.SQL_SELECT_DATA_UNCHIN)      'SQL構築(データ抽出用Select句)
            Me._StrSql.Append(LMZ240DAC.SQL_FROM_DATA_UNCHIN)      'SQL構築(カウント用from句)
            Call Me.SetConditionMasterSQL("UNCHIN_SET")            '条件設定
            Me._StrSql.Append(LMZ240DAC.SQL_GROUP_UNCHINSET)      'SQL構築(GROUP BY句)
            Me._StrSql.Append(LMZ240DAC.SQL_COUNT_TBLNM)          'SQL構築(カウント用テーブル名)
        End If
        ...
        MyBase.Logger.WriteSQLLog("LMZ240DAC", "SelectData", cmd)
    ...
    Private Function SelectListData(ByVal ds As DataSet) As DataSet
        ...
        MyBase.Logger.WriteSQLLog("LMZ240DAC", "SelectListData", cmd)
        ...
        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMZ240OUT")
    ...
    Private Function CheckExistCustM(ByVal ds As DataSet) As DataSet
        ...
        MyBase.Logger.WriteSQLLog("LMZ240DAC", "CheckExistCustM", cmd)
        ...
        ds = MyBase.SetSelectResultToDataSet(map, ds, reader, "LMZ240CUST")
    ...
    Private Function SelectExtc() As String
        ...
        'SQL作成
        Me._StrSql.Append(LMZ240DAC.SQL_SELECT_DATA_EXTC)      'SQL構築(データ抽出用Select句)
        Me._StrSql.Append(LMZ240DAC.SQL_FROM_DATA_EXTC)             'SQL構築(データ抽出用from句)
        Call Me.SetConditionMasterSQL("EXTC")                   '条件設定
        Me._StrSql.Append(LMZ240DAC.SQL_ORDER_BY_EXTC)         'SQL構築(データ抽出用ORDER BY句)
    ...
    Private Function SelectUnchinTariff() As String
        ...
        'SQL作成
        Me._StrSql.Append(LMZ240DAC.SQL_SELECT_DATA_UNCHIN)      'SQL構築(データ抽出用Select句)
        Me._StrSql.Append(LMZ240DAC.SQL_FROM_DATA_UNCHIN)        'SQL構築(データ抽出用from句)
        Call Me.SetConditionMasterSQL("UNCHIN_SET")                   '条件設定
        Me._StrSql.Append(LMZ240DAC.SQL_GROUP_UNCHINSET)         'SQL構築(データ抽出用GROUP BY句)
        Me._StrSql.Append(LMZ240DAC.SQL_ORDER_BY_UNCHIN)         'SQL構築(データ抽出用ORDER BY句)
    ...
    Private Function SelecrtCustExistChk() As String
        ...
        'SQL作成
        Me._StrSql.Append(LMZ240DAC.SQL_EXIST_CUST)      'SQL構築(荷主マスタ存在チェック用Select句)
        Call Me.SetCustMasterSQL()                       '条件設定
        Me._StrSql.Append(LMZ240DAC.SQL_CUST_GROUP)      'SQL構築(荷主マスタ存在チェック用Group句)
        Return Me.SetSchemaNm(Me._StrSql.ToString(), Me._Row.Item("NRS_BR_CD").ToString())
