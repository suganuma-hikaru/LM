...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _V As LMM060V

    ''' <summary>
    ''' Gamenクラスを格納するフィールド
    ''' </summary>
    ''' <remarks></remarks>
    Private _G As LMM060G
...
#Region "初期処理"
    ...
    Public Sub Main(ByVal prm As LMFormData)
        ...
        'Handler共通クラスの設定
        Me._ControlH = New LMMControlH("LMM060", Me._ControlV, Me._ControlG)

        'Gamenクラスの設定
        Me._G = New LMM060G(Me, Me._Frm, Me._ControlG)

        'Validateクラスの設定
        Me._V = New LMM060V(Me, Me._Frm, Me._ControlV)
        ...
        '2011/08/24  共通動作(右セル移動不可) スタート
        'Enter押下イベントの設定
        'Call Me._ControlH.SetEnterEvent(Me._Frm)
        '2011/08/24  共通動作(右セル移動不可) エンド
...
#Region "イベント定義(一覧)"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub NewDataEvent(ByVal frm As LMM060F)
        ...
        '権限チェック
        If Me._V.IsAuthorityChk(LMM060C.EventShubetsu.SHINKI) = False Then
    ...
    ''' <remarks></remarks>
    Private Sub EditDataEvent(ByVal frm As LMM060F)

        '処理開始アクション
        Call Me._ControlH.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM060C.EventShubetsu.HENSHU) = False Then
            Call Me._ControlH.EndAction(frm)  '終了処理
            Exit Sub
        End If
        ...
        Dim TariffCd As String = frm.txtUnchinTariffCd.TextValue
        ...
        Select Case frm.cmbTableTp.SelectedValue.ToString()
            ...
            Case LMM060C.TABLE_TP_KBN_00, LMM060C.TABLE_TP_KBN_02, LMM060C.TABLE_TP_KBN_03, _
                ...
                sort = "NRS_BR_CD ASC,UNCHIN_TARIFF_CD ASC,DATA_TP ASC,TABLE_TP ASC,WT_LV ASC,CAR_TP ASC,UNCHIN_TARIFF_CD_EDA ASC"

            Case LMM060C.TABLE_TP_KBN_01, LMM060C.TABLE_TP_KBN_06

                sort = "NRS_BR_CD ASC,UNCHIN_TARIFF_CD ASC,DATA_TP ASC,TABLE_TP ASC,CAR_TP ASC,WT_LV ASC,UNCHIN_TARIFF_CD_EDA ASC"
        ...
        For i As Integer = 0 To cnt
            dr = drs(i)
            Me._Ds.Tables(LMM060C.TABLE_NM_KYORI).ImportRow(dr)
        Next
        ...
        'WSAクラス呼出()
        '==========================
        Dim rtnds As DataSet = MyBase.CallWSA("LMM060BLF", "HaitaData", Me._Ds)

        'データセットの内容保持
        _Ds = rtnds

        'MAX運賃タリフコードを隠し項目に保持
        Dim max As Integer = _Ds.Tables(LMM060C.TABLE_NM_UNCHIN_TARRIF_MAXEDA).Rows.Count
        If (0).Equals(max) = False Then
            frm.lblMaxEda.TextValue = _Ds.Tables(LMM060C.TABLE_NM_UNCHIN_TARRIF_MAXEDA).Rows(0).Item("UNCHIN_TARIFF_MAXEDA").ToString()
        End If
        ...
        Else
            ...
            '運賃タリフスプレッドのロック制御
            Call Me.SetLockControl(LMM060C.EventShubetsu.HENSHU)
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub CopyDataEvent(ByVal frm As LMM060F)

        '2011.08.23 検証結果一覧№8対応 START
        '保存用データセットインスタンス作成
        Me._Ds = New LMM060DS()
        '2011.08.23 検証結果一覧№8対応 END

        '処理開始アクション
        Call Me._ControlH.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM060C.EventShubetsu.HUKUSHA) = False Then
            Call Me._ControlH.EndAction(frm)  '終了処理
            Exit Sub
        End If
        ...
        '運賃タリフSpreadのロック制御
        Call Me.SetLockControl(LMM060C.EventShubetsu.HUKUSHA)
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub DeleteDataEvent(ByVal frm As LMM060F)

        '処理開始アクション
        Call Me._ControlH.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM060C.EventShubetsu.SAKUJO) = False Then
            Call Me._ControlH.EndAction(frm)  '終了処理
            Exit Sub
        End If
        ...
        'DataSet設定
        Dim ds As DataSet = New LMM060DS()
        ...
        '更新処理(排他処理)
        Dim rtnDs As DataSet = MyBase.CallWSA("LMM060BLF", "DeleteData", ds)
        ...
        'キャッシュ最新化
        MyBase.LMCacheMasterData(LMConst.CacheTBL.UNCHIN_TARIFF_R)

        'メッセージ用コード格納
        Dim UnchinCd As String = frm.txtUnchinTariffCd.TextValue
        MyBase.ShowMessage(frm, "G002", New String() {frm.FunctionKey.F4ButtonName.Replace("　", String.Empty) _
                                              , String.Concat("[", frm.lblUnchinTariffCd.Text, " = ", UnchinCd, "]")})
    ...
    Private Sub ApprovalEvent(ByVal frm As LMM060F, ByVal eventShubetu As LMM060C.EventShubetsu)

        '処理開始アクション
        Call Me._ControlH.StartAction(frm)

        '対象行を取得
        Dim arr As ArrayList = Me._ControlH.GetCheckList(frm.sprDetail.ActiveSheet, LMM060G.sprDetailDef.DEF.ColNo)
        If arr.Count = 0 Then
        ...
        Dim ds As DataSet = New LMM060DS()
        Dim dt As DataTable = ds.Tables(LMM060C.TABLE_NM_IN)

        For i As Integer = 0 To arr.Count - 1
            Dim rowIdx As Integer = Convert.ToInt32(arr(i))
            Dim approvalCd As String = Me._ControlV.GetCellValue(frm.sprDetail.ActiveSheet.Cells(rowIdx, LMM060G.sprDetailDef.APPROVAL_CD.ColNo))

            '処理対象とならない行ならばエラー
            Select Case eventShubetu
                Case LMM060C.EventShubetsu.REQUEST
                    '申請：承認状況が未でなければエラー
                    If Not "00".Equals(approvalCd) Then
                        ...
                        Return
                    End If
                Case LMM060C.EventShubetsu.APPROVAL, LMM060C.EventShubetsu.REMAND
            ...
            dr.Item("NRS_BR_CD") = Me._ControlV.GetCellValue(frm.sprDetail.ActiveSheet.Cells(rowIdx, LMM060G.sprDetailDef.NRS_BR_CD.ColNo))
            dr.Item("UNCHIN_TARIFF_CD") = Me._ControlV.GetCellValue(frm.sprDetail.ActiveSheet.Cells(rowIdx, LMM060G.sprDetailDef.UNCHIN_TARIFF_CD.ColNo))
            dr.Item("STR_DATE") = DateFormatUtility.DeleteSlash(Me._ControlV.GetCellValue(frm.sprDetail.ActiveSheet.Cells(rowIdx, LMM060G.sprDetailDef.STR_DATE.ColNo)))
            dr.Item("SYS_UPD_DATE") = Me._ControlV.GetCellValue(frm.sprDetail.ActiveSheet.Cells(rowIdx, LMM060G.sprDetailDef.SYS_UPD_DATE.ColNo))
            dr.Item("SYS_UPD_TIME") = Me._ControlV.GetCellValue(frm.sprDetail.ActiveSheet.Cells(rowIdx, LMM060G.sprDetailDef.SYS_UPD_TIME.ColNo))
        ...
        '==========================
        '承認処理
        ds = MyBase.CallWSA("LMM060BLF", "ApprovalData", ds)
    ...
    ''' <param name="frm"></param>
    ''' <remarks></remarks>
    Private Sub InputExcelEvent(ByVal frm As LMM060F)

        '処理開始アクション
        Call Me._ControlH.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM060C.EventShubetsu.EXCELINPUT) = False Then
            Call Me._ControlH.EndAction(frm) '終了処理　
            Exit Sub
        End If
        ...
        ofd.FileName = LMM060C.DEF_FILENM
        ofd.InitialDirectory = LMM060C.DEF_DRIVE
        ofd.Filter = LMM060C.FILETYPE
        ofd.FilterIndex = LMM060C.ALL_FILE
        ofd.Title = LMM060C.DLGTITLE
        ...
        If ofd.ShowDialog() = DialogResult.OK Then
            ...
            '取込データのチェック
            If Me.SetExcelData(Me._Frm, Me._Ds, LMM060C.EventShubetsu.EXCELINPUT) = 0 Then
                ...
                '取込データを画面に表示
                Call Me._G.SetExcelUnchinData(Me._Ds, LMM060C.EventShubetsu.EXCELINPUT)
                ...
                '運賃タリフスプレッドのロック制御
                Call Me.SetLockControl(LMM060C.EventShubetsu.EXCELINPUT)
    ...
    ''' <param name="ds"></param>
    ''' <remarks></remarks>    
    Private Function SetExcelFromDatasetTo(ByVal ds As DataSet, ByVal filename As String, ByVal frm As LMM060F) As Boolean

        'Dim dr As DataRow = ds.Tables(LMM060C.TABLE_NM_IN).NewRow()
        ...
        '                cn.ConnectionString = "Provider=Microsoft.ACE.OLEDB.16.0;Data Source=" & _
        '                                     filename & ";Extended properties=""Excel 16.0 Xml;HDR=YES;IMEX=1"""
        ...
        '                'データを取得する
        '                da.Fill(ds, LMM060C.TABLE_NM_KYORI)
        ...
        '    MyBase.ShowMessage(Me._Frm, "E100")
        '    Return False
        'End Try

        Dim inTbl As DataTable = ds.Tables(LMM060C.TABLE_NM_KYORI)
        ...
        Try
            ...
            For row As Integer = 2 To maxData.Y + 1
                ...
                inRow.Item("UNCHIN_TARIFF_CD") = CStr(excelCreator.Cell("B" & row).Value)
                inRow.Item("UNCHIN_TARIFF_CD_EDA") = CStr(excelCreator.Cell("C" & row).Value)
                inRow.Item("STR_DATE") = CStr(excelCreator.Cell("D" & row).Value)
                inRow.Item("UNCHIN_TARIFF_REM") = CStr(excelCreator.Cell("E" & row).Value)
                ...
                inRow.Item("UNCHIN_TARIFF_CD2") = CStr(excelCreator.Cell("CJ" & row).Value)
                inRow.Item("UNCHIN_TARIFF_REM2") = CStr(excelCreator.Cell("CK" & row).Value)
    ...
    ''' <remarks></remarks>
    Private Function SetExcelData(ByVal frm As LMM060F, ByVal ds As DataSet, ByVal eventShubetsu As LMM060C.EventShubetsu) As Integer
        ...
        For i As Integer = 0 To rowMax
            For j As Integer = 0 To colMax
                InData(i, j) = ds.Tables(LMM060C.TABLE_NM_KYORI).Rows(i).Item(j).ToString
        ...
        With Me._Frm
            ...
            For i As Integer = 0 To rowMax
                ...
                '【タリフコード】
                'バイト数チェック
                If Me.ChkByte(frm, dr("UNCHIN_TARIFF_CD").ToString, rowNo, .lblUnchinTariffCd.Text, 10) = False Then
                    ErrCnt = ErrCnt + 1
                End If
                ...
                '【備考】
                'バイト数チェック
                If Me.ChkByte(frm, dr("UNCHIN_TARIFF_REM").ToString, rowNo, .lblUnchinTariffRem.Text, 50) = False Then
                    ErrCnt = ErrCnt + 1
                End If

                '【２次タリフコード】
                'バイト数チェック
                If Me.ChkByte(frm, dr("UNCHIN_TARIFF_CD2").ToString, rowNo, .lblUnchinTariffCd2.Text, 10) = False Then
                    ErrCnt = ErrCnt + 1
                End If
                ...
                '【車種】
                '必須チェック
                If Me.ChkHissu(frm, dr("CAR_TP").ToString, rowNo, LMM060C.CAR_TP) = False Then
                    ErrCnt = ErrCnt + 1
                End If
                '禁止文字チェック
                If Me._V.IsExcelForbiddenWordsCheck(frm, dr, LMM060C.CAR_TP) = False Then
                    rtnResult = Me.SetExcelInputErrData("E260", New String() {String.Concat(LMM060C.CAR_TP, "に禁止文字が含まれているデータ")}, rowNo)
                    ErrCnt = ErrCnt + 1
                Else
                    '存在チェック(禁止文字チェックがエラーでなければチェックする)
                    If Me._V.IsExcelExistCheck(frm, dr, LMM060C.CAR_TP) = False Then
                        rtnResult = Me.SetExcelInputErrData("E079", New String() {"区分マスタ", LMM060C.CAR_TP}, rowNo)
                        ErrCnt = ErrCnt + 1
                    End If
                End If
                ...
                If String.IsNullOrEmpty(dr("WT_LV").ToString) = False Then '取込データがNULLならチェックなし
                    '禁止文字チェック
                    If Me._V.IsExcelForbiddenWordsCheck(frm, dr, LMM060C.WT_LV) = False Then
                        rtnResult = Me.SetExcelInputErrData("E260", New String() {String.Concat(LMM060C.WT_LV, "に禁止文字が含まれているデータ")}, rowNo)
                        ErrCnt = ErrCnt + 1
                    Else
                        'バイト数チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkByte(frm, dr("WT_LV").ToString, rowNo, LMM060C.WT_LV, 9) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                        '範囲チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkRange(frm, Convert.ToDecimal(dr("WT_LV")), rowNo, LMM060C.WT_LV, Convert.ToDecimal(0), Convert.ToDecimal(999999999)) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                    End If
                End If
                ...
                For t As Integer = 1 To 70
                    If String.IsNullOrEmpty(dr("KYORI_" & t).ToString) = False Then '取込データがNULLならチェックなし
                        '禁止文字チェック
                        If Me._V.IsExcelForbiddenWordsCheck(frm, dr, LMM060C.KYORI, t) = False Then
                            rtnResult = Me.SetExcelInputErrData("E260", New String() {String.Concat((LMM060C.KYORI & t), "に禁止文字が含まれているデータ")}, rowNo)
                            ErrCnt = ErrCnt + 1
                        Else
                            'バイト数チェック(禁止文字チェックがエラーでなければチェックする)
                            If Me.ChkByte(frm, dr("KYORI_" & t).ToString, rowNo, (LMM060C.KYORI & t), 12) = False Then
                                ErrCnt = ErrCnt + 1
                            End If
                            '範囲チェック(禁止文字チェックがエラーでなければチェックする)
                            If Me.ChkRange(frm, Convert.ToDecimal(dr("KYORI_" & t)), rowNo, (LMM060C.KYORI & t), Convert.ToDecimal(0.0), Convert.ToDecimal(999999999.999)) = False Then
                                ErrCnt = ErrCnt + 1
                            End If
                        End If
                    End If
                ...
                If String.IsNullOrEmpty(dr("CITY_EXTC_A").ToString) = False Then '取込データがNULLならチェックなし
                    '禁止文字チェック
                    If Me._V.IsExcelForbiddenWordsCheck(frm, dr, LMM060C.CITY_EXTC_A) = False Then
                        rtnResult = Me.SetExcelInputErrData("E260", New String() {String.Concat(LMM060C.CITY_EXTC_A, "に禁止文字が含まれているデータ")}, rowNo)
                        ErrCnt = ErrCnt + 1
                    Else
                        'バイト数チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkByte(frm, dr("CITY_EXTC_A").ToString, rowNo, LMM060C.CITY_EXTC_A, 9) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                        '範囲チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkRange(frm, Convert.ToDecimal(dr("CITY_EXTC_A")), rowNo, LMM060C.CITY_EXTC_A, Convert.ToDecimal(0), Convert.ToDecimal(999999999)) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                    End If
                End If
                ...
                If String.IsNullOrEmpty(dr("CITY_EXTC_B").ToString) = False Then '取込データがNULLならチェックなし
                    '禁止文字チェック
                    If Me._V.IsExcelForbiddenWordsCheck(frm, dr, LMM060C.CITY_EXTC_B) = False Then
                        rtnResult = Me.SetExcelInputErrData("E260", New String() {String.Concat(LMM060C.CITY_EXTC_B, "に禁止文字が含まれているデータ")}, rowNo)
                        ErrCnt = ErrCnt + 1
                    Else
                        'バイト数チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkByte(frm, dr("CITY_EXTC_B").ToString, rowNo, LMM060C.CITY_EXTC_B, 9) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                        '範囲チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkRange(frm, Convert.ToDecimal(dr("CITY_EXTC_B")), rowNo, LMM060C.CITY_EXTC_B, Convert.ToDecimal(0), Convert.ToDecimal(999999999)) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                    End If
                End If
                ...
                If String.IsNullOrEmpty(dr("WINT_EXTC_A").ToString) = False Then '取込データがNULLならチェックなし
                    '禁止文字チェック            
                    If Me._V.IsExcelForbiddenWordsCheck(frm, dr, LMM060C.WINT_EXTC_A) = False Then
                        rtnResult = Me.SetExcelInputErrData("E260", New String() {String.Concat(LMM060C.WINT_EXTC_A, "に禁止文字が含まれているデータ")}, rowNo)
                        ErrCnt = ErrCnt + 1
                    Else
                        'バイト数チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkByte(frm, dr("WINT_EXTC_A").ToString, rowNo, LMM060C.WINT_EXTC_A, 9) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                        '範囲チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkRange(frm, Convert.ToDecimal(dr("WINT_EXTC_A")), rowNo, LMM060C.WINT_EXTC_A, Convert.ToDecimal(0), Convert.ToDecimal(999999999)) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                    End If
                End If
                ...
                If String.IsNullOrEmpty(dr("WINT_EXTC_B").ToString) = False Then '取込データがNULLならチェックなし
                    '禁止文字チェック
                    If Me._V.IsExcelForbiddenWordsCheck(frm, dr, LMM060C.WINT_EXTC_B) = False Then
                        rtnResult = Me.SetExcelInputErrData("E260", New String() {String.Concat(LMM060C.WINT_EXTC_B, "に禁止文字が含まれているデータ")}, rowNo)
                        ErrCnt = ErrCnt + 1
                    Else
                        'バイト数チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkByte(frm, dr("WINT_EXTC_B").ToString, rowNo, LMM060C.WINT_EXTC_B, 9) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                        '範囲チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkRange(frm, Convert.ToDecimal(dr("WINT_EXTC_B")), rowNo, LMM060C.WINT_EXTC_B, Convert.ToDecimal(0), Convert.ToDecimal(999999999)) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                    End If
                End If
                ...
                If String.IsNullOrEmpty(dr("RELY_EXTC").ToString) = False Then '取込データがNULLならチェックなし
                    '禁止文字チェック
                    If Me._V.IsExcelForbiddenWordsCheck(frm, dr, LMM060C.RELY_EXTC) = False Then
                        rtnResult = Me.SetExcelInputErrData("E260", New String() {String.Concat(LMM060C.RELY_EXTC, "に禁止文字が含まれているデータ")}, rowNo)
                        ErrCnt = ErrCnt + 1
                    Else
                        'バイト数チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkByte(frm, dr("RELY_EXTC").ToString, rowNo, LMM060C.RELY_EXTC, 9) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                        '範囲チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkRange(frm, Convert.ToDecimal(dr("RELY_EXTC")), rowNo, LMM060C.RELY_EXTC, Convert.ToDecimal(0), Convert.ToDecimal(999999999)) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                    End If
                End If
                ...
                If String.IsNullOrEmpty(dr("INSU").ToString) = False Then '取込データがNULLならチェックなし
                    '禁止文字チェック
                    If Me._V.IsExcelForbiddenWordsCheck(frm, dr, LMM060C.INSU) = False Then
                        rtnResult = Me.SetExcelInputErrData("E260", New String() {String.Concat(LMM060C.INSU, "に禁止文字が含まれているデータ")}, rowNo)
                        ErrCnt = ErrCnt + 1
                    Else
                        'バイト数チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkByte(frm, dr("INSU").ToString, rowNo, LMM060C.INSU, 9) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                        '範囲チェック(禁止文字チェックがエラーでなければチェックする)
                        If Me.ChkRange(frm, Convert.ToDecimal(dr("INSU")), rowNo, LMM060C.INSU, Convert.ToDecimal(0), Convert.ToDecimal(999999999)) = False Then
                            ErrCnt = ErrCnt + 1
                        End If
                    End If
                End If
                ...
                'START YANAI 要望番号377
                ''【10㎏あたりの航送料】
                'If String.IsNullOrEmpty(dr("FRRY_EXTC_10KG").ToString) = False Then '取込データがNULLならチェックなし
                '    '禁止文字チェック
                '    If Me._V.IsExcelForbiddenWordsCheck(frm, dr, LMM060C.FRRY_EXTC_10KG) = False Then
                '        rtnResult = Me.SetExcelInputErrData("E260", New String() {String.Concat(LMM060C.FRRY_EXTC_10KG, "に禁止文字が含まれているデータ")}, rowNo)
                '        ErrCnt = ErrCnt + 1
                '    Else
                '        'バイト数チェック(禁止文字チェックがエラーでなければチェックする)
                '        If Me.ChkByte(frm, dr("FRRY_EXTC_10KG").ToString, rowNo, LMM060C.FRRY_EXTC_10KG, 9) = False Then
                '            ErrCnt = ErrCnt + 1
                '        End If
                '        '範囲チェック(禁止文字チェックがエラーでなければチェックする)
                '        If Me.ChkRange(frm, Convert.ToDecimal(dr("FRRY_EXTC_10KG")), rowNo, LMM060C.FRRY_EXTC_10KG, Convert.ToDecimal(0), Convert.ToDecimal(999999999)) = False Then
                '            ErrCnt = ErrCnt + 1
                '        End If
                '    End If
                'End If
                ...
                ''【1件あたりの航送料】
                'If String.IsNullOrEmpty(dr("FRRY_EXTC_PART").ToString) = False Then '取込データがNULLならチェックなし
                '    '禁止文字チェック
                '    If Me._V.IsExcelForbiddenWordsCheck(frm, dr, LMM060C.FRRY_EXTC_PART) = False Then
                '        rtnResult = Me.SetExcelInputErrData("E260", New String() {String.Concat(LMM060C.FRRY_EXTC_PART, "に禁止文字が含まれているデータ")}, rowNo)
                '        ErrCnt = ErrCnt + 1
                '    Else
                '        'バイト数チェック(禁止文字チェックがエラーでなければチェックする)
                '        If Me.ChkByte(frm, dr("FRRY_EXTC_PART").ToString, rowNo, LMM060C.FRRY_EXTC_PART, 9) = False Then
                '            ErrCnt = ErrCnt + 1
                '        End If
                '        '範囲チェック(禁止文字チェックがエラーでなければチェックする)
                '        If Me.ChkRange(frm, Convert.ToDecimal(dr("FRRY_EXTC_PART")), rowNo, LMM060C.FRRY_EXTC_PART, Convert.ToDecimal(0), Convert.ToDecimal(999999999)) = False Then
                '            ErrCnt = ErrCnt + 1
                '        End If
                '    End If
                'End If
                ...
                If String.IsNullOrEmpty(dr("WT_LV").ToString) = False Then '取込データがNULLならチェックなし
                    If LMM060C.DATA_TP_KBN_00.Equals(dr("DATA_TP").ToString) = True Then
                        If (LMM060C.DATA_TP_KBN_00.Equals(dr("CAR_TP").ToString) = True _
                ...
                '整合性チェック(テーブルタイプ・重量)
                If String.IsNullOrEmpty(dr("WT_LV").ToString) = False Then '取込データがNULLならチェックなし
                    If (LMM060C.TABLE_TP_KBN_01.Equals(dr("TABLE_TP").ToString) = True _
                     OrElse LMM060C.TABLE_TP_KBN_06.Equals(dr("TABLE_TP").ToString)) = True Then
                ...
                '整合性チェック(テーブルタイプ・車種)
                If LMM060C.TABLE_TP_KBN_00.Equals(dr("TABLE_TP").ToString) = True _
                 OrElse LMM060C.TABLE_TP_KBN_02.Equals(dr("TABLE_TP").ToString) = True _
                 OrElse LMM060C.TABLE_TP_KBN_03.Equals(dr("TABLE_TP").ToString) = True _
                 OrElse LMM060C.TABLE_TP_KBN_04.Equals(dr("TABLE_TP").ToString) = True _
                 OrElse LMM060C.TABLE_TP_KBN_05.Equals(dr("TABLE_TP").ToString) = True _
                 OrElse LMM060C.TABLE_TP_KBN_07.Equals(dr("TABLE_TP").ToString) = True Then
                    If LMM060C.TABLE_TP_KBN_00.Equals(dr("CAR_TP").ToString) = False Then
                ...
                If LMConst.FLG.OFF.Equals(ChkEndFlg2) = True Then
                    If i <> rowMax Then   '最終行の場合、次の行がないのでチェックしない
                        If dt.Rows(i)("NRS_BR_CD").ToString <> dt.Rows(i + 1)("NRS_BR_CD").ToString _
                          OrElse dt.Rows(i)("UNCHIN_TARIFF_CD").ToString <> dt.Rows(i + 1)("UNCHIN_TARIFF_CD").ToString _
                          OrElse dt.Rows(i)("STR_DATE").ToString <> dt.Rows(i + 1)("STR_DATE").ToString Then
                            rtnResult = Me.SetExcelInputErrData("E028", New String() {"営業所・運賃タリフコード・適用開始日の組合せが同一でないデータ", .FunctionKey.F7ButtonName}, rowNo)
                            ErrCnt = ErrCnt + 1
                ...
                If LMConst.FLG.OFF.Equals(ChkEndFlg3) = True Then
                    'データタイプ='00'はチェック対象外
                    If LMM060C.DATA_TP_KBN_00.Equals(dt.Rows(i)("DATA_TP").ToString) = False Then
    ...
    Private Function ExcelDataPGSet(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To lngcnt
            ...
            dr("UNCHIN_TARIFF_CD_EDA") = unchinCdEda

            '削除フラグ："0"固定
            dr("SYS_DEL_FLG") = LMM060C.FLG.OFF

            '重量～1件あたりの航送料(数値項目)：NULLの場合0固定
            If String.IsNullOrEmpty(dr("WT_LV").ToString) = True Then
                dr("WT_LV") = LMM060C.FLG.OFF
            End If
            For t As Integer = 1 To 70
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub OutputExcelEvent(ByVal frm As LMM060F)

        '処理開始アクション
        Call Me._ControlH.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM060C.EventShubetsu.EXCELOUTPUT) = False Then
            Call Me._ControlH.EndAction(frm) '終了処理　
            Exit Sub
        End If
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Private Function ExcelDataMake(ByVal frm As LMM060F) As DataSet
        ...
        Dim rtnDs As DataSet = MyBase.CallWSA("LMM060BLF", "ExcelDataMake", ds)
    ...
    ''' <param name="ds">データセット</param>
    ''' <remarks></remarks>
    Private Sub ExcelOut(ByVal ds As DataSet, ByVal frm As LMM060F)

        '★★★エクセルデータ出力時、FRRY_EXTC_PART項目を出力されないようにするための修正ポイント
        Dim copyDs As DataSet = ds.Copy()
        copyDs.Tables(LMM060C.TABLE_NM_KYORI).Columns.Remove("FRRY_EXTC_PART")

        'DataSetの値を二次元配列に格納する
        Dim rowMax As Integer = copyDs.Tables(LMM060C.TABLE_NM_KYORI).Rows.Count - 1
        Dim colMax As Integer = copyDs.Tables(LMM060C.TABLE_NM_KYORI).Columns.Count - 4
        Dim excelData(rowMax + 1, colMax + 1) As String

        'タイトル列を設定
        For i As Integer = 0 To colMax
            excelData(0, i) = copyDs.Tables(LMM060C.TABLE_NM_KYORI).Columns(i).ColumnName
        Next

        '値を設定
        For i As Integer = 0 To rowMax
            For j As Integer = 0 To colMax
                excelData(i + 1, j) = copyDs.Tables(LMM060C.TABLE_NM_KYORI).Rows(i).Item(j).ToString
            Next
        Next
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub SelectListEvent(ByVal frm As LMM060F)

        '処理開始アクション
        Call Me._ControlH.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM060C.EventShubetsu.KENSAKU) = False Then
            Call Me._ControlH.EndAction(frm) '終了処理　
            Exit Sub
        End If
    ...
    ''' <remarks></remarks>
    Private Sub SprCellLeave(ByVal frm As LMM060F, ByVal e As FarPoint.Win.Spread.LeaveCellEventArgs)
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Private Sub SprCellSelect(ByVal frm As LMM060F, ByVal e As FarPoint.Win.Spread.CellClickEventArgs)
    ...
    ''' <remarks></remarks>
    Private Sub RowSelection(ByVal frm As LMM060F, ByVal rowNo As Integer)
        ...
        NrsBrcd = frm.sprDetail.ActiveSheet.Cells(rowNo, LMM060G.sprDetailDef.NRS_BR_CD.ColNo).Text()
        TariffCd = frm.sprDetail.ActiveSheet.Cells(rowNo, LMM060G.sprDetailDef.UNCHIN_TARIFF_CD.ColNo).Text()
        DataTp = frm.sprDetail.ActiveSheet.Cells(rowNo, LMM060G.sprDetailDef.DATA_TP.ColNo).Text()
        '2016.01.22 英語化不具合対応 修正START
        'StrDate = DateFormatUtility.DeleteSlash(frm.sprDetail.ActiveSheet.Cells(rowNo, LMM060G.sprDetailDef.STR_DATE.ColNo).Text())
        StrDate = frm.sprDetail.ActiveSheet.Cells(rowNo, LMM060G.sprDetailDef.STR_DATE.ColNo).Text()
        '2016.01.22 英語化不具合対応 修正END
        TableTp = frm.sprDetail.ActiveSheet.Cells(rowNo, LMM060G.sprDetailDef.TABLE_TP.ColNo).Text()
        ...
        '権限チェック
        If Me._V.IsAuthorityChk(LMM060C.EventShubetsu.DCLICK) = False Then
            Call Me._ControlH.EndAction(frm) '終了処理
            Exit Sub
        End If
        ...
        If LMConst.FLG.OFF.Equals(Me._ControlV.GetCellValue(frm.sprDetail.ActiveSheet.Cells(rowNo, LMM060G.sprDetailDef.SYS_DEL_FLG.ColNo))) = True Then
        ...
        'SPREAD(明細)へデータを移動
        Call Me._G.SetSpread2(dt2, LMM060C.EventShubetsu.DCLICK, NrsBrcd, TariffCd, DataTp, StrDate, TableTp)
    ...
    ''' <param name="frm"></param>
    ''' <remarks></remarks>
    Private Sub MasterShowEvent(ByVal frm As LMM060F)

        'カーソル位置の設定
        Dim objNm As String = frm.ActiveControl.Name()

        '権限チェック
        Dim rtnResult As Boolean = Me._V.IsAuthorityChk(LMM060C.EventShubetsu.MASTEROPEN)
        ...
        Call Me.ShowPopupControl(frm, objNm, LMM060C.EventShubetsu.MASTEROPEN)

        '処理終了アクション
        Call Me._ControlH.EndAction(frm)

        '運賃タリフスプレッドのロック制御
        Call Me.SetLockControl(LMM060C.EventShubetsu.MASTEROPEN)
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Private Function SaveUnchinTariffItemData(ByVal frm As LMM060F, ByVal eventShubetsu As LMM060C.EventShubetsu) As Boolean

        '処理開始アクション
        Call Me._ControlH.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM060C.EventShubetsu.HOZON) = False Then
            Call Me._ControlH.EndAction(frm)  '終了処理
            Return False
        End If
        ...
        'DataSet設定
        'Dim ds As DataSet = New LMM060DS()        
        Call Me.SetDatasetUnchinTariffColItemData(frm, Me._Ds, LMM060C.EventShubetsu.HOZON)
        ...
        Select Case frm.lblSituation.RecordStatus
            Case RecordStatus.NEW_REC, RecordStatus.COPY_REC
                '新規登録処理
                rtnDs = MyBase.CallWSA("LMM060BLF", "InsertData", Me._Ds)
            Case Else
                '更新処理(排他処理)
                rtnDs = MyBase.CallWSA("LMM060BLF", "UpdateData", Me._Ds)
        End Select
    ...
    Friend Sub CloseFormEvent(ByVal e As FormClosingEventArgs)
        ...
        Select Case MyBase.ShowMessage(Me._Frm, "W002")
            ...
            Case MsgBoxResult.Yes  '「はい」押下時
                ...
                If Me.SaveUnchinTariffItemData(Me._Frm, LMM060C.EventShubetsu.TOJIRU) = False Then
    ...
    ''' <param name="e">イベント詳細</param>
    ''' <remarks></remarks>
    Private Sub EnterAction(ByVal frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        With Me._Frm
            ...
            '権限チェック
            rtnResult = rtnResult AndAlso Me._V.IsAuthorityChk(LMM060C.EventShubetsu.ENTER)
            ...
            Call Me.ShowPopupControl(frm, objNm, LMM060C.EventShubetsu.ENTER)
    ...
    ''' <param name="frm"></param>
    ''' <remarks></remarks>
    Private Sub RowAdd(ByVal frm As LMM060F)
        ...
        If rtnResult = True Then
            ...
            '運賃タリフ(距離刻み/運賃)情報表示
            Call Me._G.SetSpread2(Me._Ds.Tables(LMM060C.TABLE_NM_KYORI), LMM060C.EventShubetsu.INS_T, NrsBrcd, , , , TableTp)

            '計算種別をロック
            Call Me._ControlG.LockComb(Me._Frm.cmbTableTp, True)

            '運賃タリフスプレッドのロック制御
            Call Me.SetLockControl(LMM060C.EventShubetsu.INS_T)
        End If
    ...
    ''' <param name="frm"></param>
    ''' <remarks></remarks>
    Private Function RowAddUnchinDt(ByVal frm As LMM060F) As Boolean

        '現在の運賃Spread情報(距離刻み/運賃)をDataSetに設定
        Call Me.SetDatasetUnchinTariffColItemData(frm, Me._Ds, LMM060C.EventShubetsu.INS_T)

        Dim dt As DataTable = Me._Ds.Tables(LMM060C.TABLE_NM_KYORI)

        '処理開始アクション
        Call Me._ControlH.SetEnterEvent(frm)

        '行追加時チェック
        If Me._V.IsRowCheck(LMM060C.EventShubetsu.INS_T, frm) = False Then
            '処理終了アクション
            Call Me._ControlH.EndAction(frm)
            Return False
        End If
        ...
        '運賃タリフコード枝番の採番
        If Me._G.SetUnchinCdEdaDataSet(Me._Ds, LMM060C.EventShubetsu.INS_T) = False Then
            '処理終了アクション
            Call Me._ControlH.EndAction(frm)
            Return False
        End If
        ...
        With Me._Frm
            ...
            dr.Item("UNCHIN_TARIFF_CD") = .txtUnchinTariffCd.TextValue
            dr.Item("STR_DATE") = .imdStrDate.TextValue
            dr.Item("UNCHIN_TARIFF_REM") = .txtUnchinTariffRem.TextValue
            ...
            dr.Item("UNCHIN_TARIFF_CD2") = .txtUnchinTariffCd2.TextValue
            dr.Item("UNCHIN_TARIFF_CD_EDA") = .lblMaxEda.TextValue
    ...
    ''' <param name="frm">フォーム</param>
    ''' <remarks></remarks>
    Private Sub DeleteUnchinData(ByVal frm As LMM060F)
        ...
        '項目チェック
        If Me._V.IsRowCheck(LMM060C.EventShubetsu.DEL_T, frm) = False Then
            Call Me._ControlH.EndAction(frm)  '終了処理
            Exit Sub
        End If

        '現在の運賃Spread情報(距離刻み/運賃)をDataSetに設定
        Call Me.SetDatasetUnchinTariffColItemData(frm, Me._Ds, LMM060C.EventShubetsu.DEL_T)

        'チェックリスト取得
        Dim arr As ArrayList = Nothing
        arr = Me._ControlH.GetCheckList(frm.sprDetail2.ActiveSheet, LMM060G.sprDetailDef2.DEF.ColNo)
        ...
        Dim TariffCd As String = Me._Frm.txtUnchinTariffCd.TextValue.ToString
        ...
        sql = String.Concat("NRS_BR_CD = '", NrsBrcd, "' " _
                                      ...
                                      , "AND SYS_DEL_FLG = '", LMM060C.FLG.OFF, "' " _
        ...
        sql2 = String.Concat("NRS_BR_CD = '", NrsBrcd, "' " _
                                      , "AND UNCHIN_TARIFF_CD = '", TariffCd, "' " _
                                      , "AND STR_DATE = '", StrDate, "' " _
                                      , "AND SYS_DEL_FLG = '", LMM060C.FLG.ON, "' " _
                                      , "AND (DATA_TP = '", DataTp, "' " _
                                      , "OR  DATA_TP = '", LMM060C.TABLE_TP_KBN_00, "') " _
                                      )

        Dim setDrs As DataRow() = Me._Ds.Tables(LMM060C.TABLE_NM_KYORI).Select(sql)
        Dim setDrs2 As DataRow() = Me._Ds.Tables(LMM060C.TABLE_NM_KYORI).Select(sql2)

        Dim spr As SheetView = frm.sprDetail2.ActiveSheet
        For i As Integer = max To 0 Step -1

            '行番号を設定
            recNo = Me.GetRecNo(spr, arr(i).ToString(), LMM060G.sprDetailDef2.RECNO.ColNo)
        ...
        For i As Integer = 0 To cnt2
            dr2 = setDrs2(i)
            ds.Tables(LMM060C.TABLE_NM_KYORI).ImportRow(dr2)
        Next
        ...
        For i As Integer = 0 To cnt
            dr = setDrs(i)
            ds.Tables(LMM060C.TABLE_NM_KYORI).ImportRow(dr)
        Next

        '運賃タリフ(距離刻み/運賃)情報表示
        Call Me._G.SetSpread2(ds.Tables(LMM060C.TABLE_NM_KYORI), LMM060C.EventShubetsu.DEL_T, NrsBrcd, , , , TableTp)

        '運賃タリフスプレッドのロック制御
        Call Me.SetLockControl(LMM060C.EventShubetsu.DEL_T)
...
#Region "内部メソッド"
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Private Sub SelectData(ByVal frm As LMM060F, ByVal clearflg As Integer)
        ...
        'WSAクラス呼出
        '==========================
        Dim rtnDs As DataSet = Me._ControlH.CallWSAAction(frm, "LMM060BLF", "SelectListData", _FindDs, lc, Nothing, mc)

        'Dim rtnDs As DataSet = Me._ControlH.CallWSAAction(frm, _
        '                                         "LMM060BLF", "SelectListData", _FindDs _
    ...
    ''' <remarks></remarks>
    Private Sub SuccessSelect(ByVal frm As LMM060F, ByVal ds As DataSet)

        Dim dt As DataTable = ds.Tables(LMM060C.TABLE_NM_OUT)
        Dim dt2 As DataTable = ds.Tables(LMM060C.TABLE_NM_KYORI)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function ConfirmMsg() As Boolean
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub SetLockControl(ByVal actionType As LMM060C.EventShubetsu)
    ...
    ''' <remarks></remarks>
    Private Function ChkExistDataTp(ByVal frm As LMM060F, ByVal dr As DataRow, ByVal rowNo As Integer) As Integer
    ...
    ''' <returns>True:エラーなし,OK False:エラーあり</returns>
    ''' <remarks></remarks>
    Private Function ChkHissu(ByVal frm As LMM060F, ByVal ChkData As String, ByVal rowNo As Integer, ByVal ChkCell As String) As Boolean
    ...
    ''' <returns>True:エラーなし,OK False:エラーあり</returns>
    ''' <remarks></remarks>
    Private Function ChkByte(ByVal frm As LMM060F, ByVal ChkData As String, ByVal rowNo As Integer, ByVal ChkCell As String, ByVal ByteCnt As Integer) As Boolean
        ...
        With Me._Frm
            ...
            If ChkData.Length > ByteCnt Then
                ...
                If ChkCell.Equals(.lblUnchinTariffRem.Text) = True Then
                    ByteCnt = ByteCnt * 2
                End If
    ...
    ''' <returns>True:エラーなし,OK False:エラーあり</returns>
    ''' <remarks></remarks>
    Private Function ChkRange(ByVal frm As LMM060F, ByVal ChkData As Decimal, ByVal rowNo As Integer, ByVal ChkCell As String, ByVal ByteCntF As Decimal, ByVal ByteCntT As Decimal) As Boolean
...
#Region "PopUp"
    ...
    ''' <returns>True:エラーなし,OK False:エラーあり</returns>
    ''' <remarks></remarks>
    Private Function ShowPopupControl(ByVal frm As LMM060F, ByVal objNm As String, ByVal eventshubetsu As LMM060C.EventShubetsu) As Boolean
    ...
    ''' <remarks></remarks>
    Private Function SetReturnUnchinTariffPop(ByVal frm As LMM060F, ByVal objNm As String, ByVal eventshubetsu As LMM060C.EventShubetsu) As Boolean
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function ShowUnchinTariffPopup(ByVal frm As LMM060F, ByVal ctl As Win.InputMan.LMImTextBox, ByVal eventshubetsu As LMM060C.EventShubetsu) As LMFormData
        ...
        With dr
            ...
            If eventshubetsu = LMM060C.EventShubetsu.ENTER Then
...
#Region "DataSet設定"
    ...
    Private Sub SetDataSetInData()
        ...
        Dim dr As DataRow = Me._FindDs.Tables(LMM060C.TABLE_NM_IN).NewRow()

        With Me._Frm.sprDetail.ActiveSheet

            dr.Item("SYS_DEL_FLG") = Me._ControlV.GetCellValue(.Cells(0, LMM060G.sprDetailDef.SYS_DEL_NM.ColNo))
            dr.Item("NRS_BR_CD") = Me._ControlV.GetCellValue(.Cells(0, LMM060G.sprDetailDef.NRS_BR_NM.ColNo))
            dr.Item("UNCHIN_TARIFF_CD") = Me._ControlV.GetCellValue(.Cells(0, LMM060G.sprDetailDef.UNCHIN_TARIFF_CD.ColNo))
            dr.Item("DATA_TP") = Me._ControlV.GetCellValue(.Cells(0, LMM060G.sprDetailDef.DATA_TP_NM.ColNo))
            dr.Item("TABLE_TP") = Me._ControlV.GetCellValue(.Cells(0, LMM060G.sprDetailDef.TABLE_TP_NM.ColNo))
            dr.Item("APPROVAL_CD") = Me._ControlV.GetCellValue(.Cells(0, LMM060G.sprDetailDef.APPROVAL_NM.ColNo))

            '修正 2015.05.20 営業所またぎ処理のため画面値より営業所コード取得
            'dr.Item("USER_BR_CD") = LMUserInfoManager.GetNrsBrCd.ToString()
            dr.Item("USER_BR_CD") = Me._ControlV.GetCellValue(.Cells(0, LMM060G.sprDetailDef.NRS_BR_CD.ColNo))


            Me._FindDs.Tables(LMM060C.TABLE_NM_IN).Rows.Add(dr)
    ...
    Private Sub SetDataSetHaitaChk()
        ...
        Dim dr As DataRow = Me._Ds.Tables(LMM060C.TABLE_NM_IN).NewRow()

        With Me._Frm

            '排他処理条件を格納
            dr.Item("UNCHIN_TARIFF_CD") = .txtUnchinTariffCd.TextValue.Trim
            ...
            Me._Ds.Tables(LMM060C.TABLE_NM_IN).Rows.Add(dr)
    ...
    ''' <param name="ds">このハンドラクラスに紐づくデータセット</param>
    ''' <remarks></remarks>
    Private Sub SetDatasetUnchinTariffColItemData(ByVal frm As LMM060F, ByVal ds As DataSet, ByVal eventShubetsu As LMM060C.EventShubetsu)
        ...
        With frm
            ...
            Dim drs As DataRow() = ds.Tables(LMM060C.TABLE_NM_KYORI).Select(String.Concat("NRS_BR_CD = '", NrsBrcd, "' " _
                                                                     , "AND UNCHIN_TARIFF_CD = '", TariffCd, "' " _
                                                                     , "AND SYS_DEL_FLG = '", LMM060C.FLG.ON, "' " _
                                                                     ), sort)
            'DataSetのクリア
            ...
            For i As Integer = 0 To cnt
                dr2 = drs(i)
                Me._Ds.Tables(LMM060C.TABLE_NM_KYORI).ImportRow(dr2)
            Next
            ...
            For i As Integer = 1 To sprMax
                ...
                dr = Me._Ds.Tables(LMM060C.TABLE_NM_KYORI).NewRow()

                dr.Item("NRS_BR_CD") = .cmbNrsBrCd.SelectedValue
                dr.Item("UNCHIN_TARIFF_CD") = .txtUnchinTariffCd.TextValue.Trim
                dr.Item("STR_DATE") = .imdStrDate.TextValue.Trim
                If i = 1 Then
                    dr.Item("DATA_TP") = LMM060C.DATA_TP_KBN_00
                Else                    
                ...
                End If
                dr.Item("TABLE_TP") = .cmbTableTp.SelectedValue
                dr.Item("UNCHIN_TARIFF_REM") = .txtUnchinTariffRem.TextValue.Trim
                dr.Item("UNCHIN_TARIFF_CD2") = .txtUnchinTariffCd2.TextValue.Trim
                '修正 2015.05.20 営業所またぎ処理のため画面値より営業所コード取得
                'dr.Item("USER_BR_CD") = LMUserInfoManager.GetNrsBrCd.ToString()
                dr.Item("USER_BR_CD") = .cmbNrsBrCd.SelectedValue.ToString()
                ...
                If i = 1 Then
                    dr.Item("WT_LV") = 0
                    dr.Item("CAR_TP") = LMM060C.DATA_TP_KBN_00
                    ...
                    dr.Item("FRRY_EXTC_PART") = 0
                    dr.Item("UNCHIN_TARIFF_CD_EDA") = "000"
                    '列数の増減によって、列がズレた分を修正
                    Dim ADColCnt As Integer = 0
                    If (LMM060C.EventShubetsu.COLADD).Equals(eventShubetsu) = True Then
                        ADColCnt = ADColCnt + 1
                    End If
                    If (LMM060C.EventShubetsu.COLDEL).Equals(eventShubetsu) = True Then
                        ADColCnt = ADColCnt - 1
                    End If
                    dr.Item("UPD_FLG") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.UPD_FLG.ColNo + ADColCnt)))
                    dr.Item("SYS_DEL_FLG") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.SYS_DEL_FLG_T.ColNo + ADColCnt)))
                    dr.Item("SYS_UPD_DATE") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.SYS_UPD_DATE.ColNo + ADColCnt)))
                    dr.Item("SYS_UPD_TIME") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.SYS_UPD_TIME.ColNo + ADColCnt)))
                Else

                    If (LMM060C.TABLE_TP_KBN_00).Equals(.cmbTableTp.SelectedValue) = True _
                    OrElse (LMM060C.TABLE_TP_KBN_03).Equals(.cmbTableTp.SelectedValue) = True _
                    OrElse (LMM060C.TABLE_TP_KBN_07).Equals(.cmbTableTp.SelectedValue) = True Then
                        dr.Item("WT_LV") = Convert.ToDecimal(_ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, LMM060G.sprDetailDef2.WT_LV.ColNo)))
                    End If

                    If (LMM060C.TABLE_TP_KBN_02).Equals(.cmbTableTp.SelectedValue) = True _
                    OrElse (LMM060C.TABLE_TP_KBN_05).Equals(.cmbTableTp.SelectedValue) = True Then
                        dr.Item("WT_LV") = Convert.ToDecimal(_ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, LMM060G.sprDetailDef2.NB.ColNo)))
                    End If

                    If (LMM060C.TABLE_TP_KBN_04).Equals(.cmbTableTp.SelectedValue) = True Then
                        dr.Item("WT_LV") = Convert.ToDecimal(_ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, LMM060G.sprDetailDef2.QT.ColNo)))
                    End If

                    '画面：テーブルタイプが01,06の場合は画面の値(0)を設定
                    If (LMM060C.TABLE_TP_KBN_01).Equals(.cmbTableTp.SelectedValue) = True _
                    OrElse (LMM060C.TABLE_TP_KBN_06).Equals(.cmbTableTp.SelectedValue) = True Then
                        dr.Item("WT_LV") = Convert.ToDecimal(_ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, LMM060G.sprDetailDef2.WT_LV.ColNo)))
                    End If


                    '画面：車種がロックされている場合(or空白の場合)は、'00'固定
                    If String.IsNullOrEmpty(_ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, LMM060G.sprDetailDef2.CAR_TP.ColNo))) = True Then
                        dr.Item("CAR_TP") = LMM060C.DATA_TP_KBN_00
                        '2011.08.22 検証結果一覧№7対応 START
                        If (LMM060C.TABLE_TP_KBN_01).Equals(.cmbTableTp.SelectedValue) = True Then
                            dr.Item("CAR_TP") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, LMM060G.sprDetailDef2.CAR_TP.ColNo))
                        End If
                        If (LMM060C.TABLE_TP_KBN_06).Equals(.cmbTableTp.SelectedValue) = True Then
                            dr.Item("CAR_TP") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, LMM060G.sprDetailDef2.T_SIZE.ColNo))
                        End If
                        '2011.08.22 検証結果一覧№7対応 END
                    Else
                        If (LMM060C.TABLE_TP_KBN_01).Equals(.cmbTableTp.SelectedValue) = True Then
                            dr.Item("CAR_TP") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, LMM060G.sprDetailDef2.CAR_TP.ColNo))
                        End If
                    ...
                    dr.Item("CITY_EXTC_A") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.CITY_EXTC_A.ColNo + ADColCnt)))
                    dr.Item("CITY_EXTC_B") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.CITY_EXTC_B.ColNo + ADColCnt)))
                    dr.Item("WINT_EXTC_A") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.WINT_EXTC_A.ColNo + ADColCnt)))
                    dr.Item("WINT_EXTC_B") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.WINT_EXTC_B.ColNo + ADColCnt)))
                    dr.Item("RELY_EXTC") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.RELY_EXTC.ColNo + ADColCnt)))
                    dr.Item("INSU") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.INSU.ColNo + ADColCnt)))
                    'START YANAI 要望番号377
                    'dr.Item("FRRY_EXTC_10KG") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.FRRY_EXTC_10KG.ColNo + ADColCnt)))
                    'END YANAI 要望番号377
                    dr.Item("FRRY_EXTC_PART") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.FRRY_EXTC_PART.ColNo + ADColCnt)))
                    dr.Item("UNCHIN_TARIFF_CD_EDA") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.UNCHIN_TARIFF_CD_EDA.ColNo + ADColCnt)))
                    dr.Item("UPD_FLG") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.UPD_FLG.ColNo + ADColCnt)))
                    dr.Item("SYS_DEL_FLG") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.SYS_DEL_FLG_T.ColNo + ADColCnt)))
                    dr.Item("SYS_UPD_DATE") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.SYS_UPD_DATE.ColNo + ADColCnt)))
                    dr.Item("SYS_UPD_TIME") = _ControlG.GetCellValue(.sprDetail2.ActiveSheet.Cells(i, (LMM060G.sprDetailDef2.SYS_UPD_TIME.ColNo + ADColCnt)))
                ...
                For s As Integer = LMM060C.SprColumnIndex2.KYORI_1 To LMM060C.SprColumnIndex2.KYORI_70
                ...
                Me._Ds.Tables(LMM060C.TABLE_NM_KYORI).Rows.Add(dr)
    ...
    Private Sub SetDatasetDelData(ByVal ds As DataSet)
        ...
        With Me._Frm
            ...
            ds.Tables(LMM060C.TABLE_NM_IN).Rows.Add(dr)
...
#Region "イベント振分け"
    ...
    ''' <remarks></remarks>
    Friend Sub FunctionKey1Press(ByRef frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey2Press(ByRef frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey3Press(ByRef frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey4Press(ByRef frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey5Press(ByRef frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)

        MyBase.Logger.StartLog(MyBase.GetType.Name, "ApprovalEvent")

        '承認処理（申請）
        Me.ApprovalEvent(frm, LMM060C.EventShubetsu.REQUEST)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey6Press(ByRef frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)

        MyBase.Logger.StartLog(MyBase.GetType.Name, "ApprovalEvent")

        '承認処理（承認）
        Me.ApprovalEvent(frm, LMM060C.EventShubetsu.APPROVAL)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey7Press(ByRef frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey8Press(ByRef frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey9Press(ByRef frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey10Press(ByRef frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey11Press(ByRef frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)

        MyBase.Logger.StartLog(MyBase.GetType.Name, "SaveUnchinTariffItemData")

        Me.SaveUnchinTariffItemData(frm, LMM060C.EventShubetsu.HOZON)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey12Press(ByRef frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    Friend Sub ClosingForm(ByRef frm As LMM060F, ByVal e As FormClosingEventArgs)
    ...
    ''' <param name="e">イベント詳細</param>
    ''' <remarks></remarks>
    Friend Sub sprCellDoubleClick(ByRef frm As LMM060F, ByVal e As FarPoint.Win.Spread.CellClickEventArgs)
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Friend Sub btnROW_INS_UNCHIN_Click(ByRef frm As LMM060F)
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Friend Sub btnROW_DEL_UNCHIN_Click(ByRef frm As LMM060F)
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Friend Sub ColAddDel(ByVal frm As LMM060F, ByVal mode As LM.GUI.Win.Spread.LMSpread.ColMode)

        Dim EventShubetsu As LMM060C.EventShubetsu = LMM060C.EventShubetsu.MAIN
        ...
        If LMM060C.SprColumnIndex2.KYORI_1 <= Index And Index <= LMM060C.SprColumnIndex2.KYORI_70 Then

            If (CType(1, LMSpread.ColMode)).Equals(mode) = True Then
                _BeCnt = CType(Me._Frm.lblDefAddCnt.TextValue, Integer) + 1
                EventShubetsu = LMM060C.EventShubetsu.COLADD
            End If
            If (CType(-1, LMSpread.ColMode)).Equals(mode) = True Then
                _BeCnt = CType(Me._Frm.lblDefAddCnt.TextValue, Integer) - 1
                EventShubetsu = LMM060C.EventShubetsu.COLDEL
            End If
            ...
            '運賃タリフ(距離刻み/運賃)情報表示
            Call Me._G.SetSpread2(Me._Ds.Tables(LMM060C.TABLE_NM_KYORI), EventShubetsu, NrsBrcd, , , , TableTp)

            '運賃タリフスプレッドのロック制御
            Call Me.SetLockControl(EventShubetsu)

            '運賃タリフスプレッドのフォーカス設定
            frm.sprDetail2.ActiveSheet.SetActiveCell(0, LMM060C.SprColumnIndex2.KYORI_1)
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Friend Sub ColLockUnLock(ByVal frm As LMM060F, ByVal mode As Boolean)
    ...
    ''' <remarks></remarks>
    Friend Sub LMM060F_KeyDown(ByVal frm As LMM060F, ByVal e As System.Windows.Forms.KeyEventArgs)

        MyBase.Logger.StartLog(MyBase.GetType.Name, "LMM060F_KeyDown")

        Call Me.EnterAction(frm, e)

        MyBase.Logger.EndLog(MyBase.GetType.Name, "LMM060F_KeyDown")
    ...
    ''' <param name="e">イベント詳細情報</param>
    ''' <remarks></remarks>
    Friend Sub sprDetail_LeaveCell(ByVal frm As LMM060F, ByVal e As FarPoint.Win.Spread.LeaveCellEventArgs)
    ...
    ''' <param name="e">イベント詳細</param>
    ''' <remarks></remarks>
    Friend Sub btnRemand_Click(ByVal frm As LMM060F, ByVal e As System.EventArgs)

        MyBase.Logger.StartLog(MyBase.GetType.Name, System.Reflection.MethodBase.GetCurrentMethod.Name)

        '承認処理（差し戻し）
        Me.ApprovalEvent(frm, LMM060C.EventShubetsu.REMAND)
