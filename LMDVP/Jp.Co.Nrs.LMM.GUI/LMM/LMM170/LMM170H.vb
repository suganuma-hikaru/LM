...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _V As LMM170V

    ''' <summary>
    ''' Gamenクラスを格納するフィールド
    ''' </summary>
    ''' <remarks></remarks>
    Private _G As LMM170G
...
#Region "初期処理"
    ...
    Public Sub Main(ByVal prm As LMFormData)
        ...
        'Handler共通クラスの設定
        Me._LMMConH = New LMMControlH("LMM170", Me._LMMConV, Me._LMMConG)

        'Gamenクラスの設定
        Me._G = New LMM170G(Me, Me._Frm, Me._LMMConG)

        'Validateクラスの設定
        Me._V = New LMM170V(Me, Me._Frm, Me._LMMConV)
        ...
        'フォーカスの設定
        Call Me._G.SetFoucus(LMM170C.EventShubetsu.MAIN)
...
#Region "イベント定義(一覧)"
    ...
    ''' <remarks></remarks>
    Friend Function CloseForm(ByVal frm As LMM170F) As Boolean
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub NewDataEvent(ByVal frm As LMM170F)

        '処理開始アクション
        Call Me._LMMConH.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM170C.EventShubetsu.SHINKI) = False Then
        ...
        'フォーカスの設定
        Call Me._G.SetFoucus(LMM170C.EventShubetsu.SHINKI)
    ...
    ''' <remarks></remarks>
    Private Sub EditDataEvent(ByVal frm As LMM170F)

        '処理開始アクション
        Call Me._LMMConH.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM170C.EventShubetsu.HENSHU) = False Then
            Call Me._LMMConH.EndAction(frm)  '終了処理
            Exit Sub
        End If

        'データ存在チェック
        If Me._LMMConV.IsExistDataChk(frm, frm.txtZipNo.TextValue) = False Then
            Call Me._LMMConH.EndAction(frm)  '終了処理
            Exit Sub
        End If
        ...
        'DataSet設定(排他チェック)
        Dim ds As DataSet = New LMM170DS()
        Call Me.SetDatasetUserItemData(frm, ds)

        '==========================
        'WSAクラス呼出
        '==========================
        Dim rtnds As DataSet = MyBase.CallWSA("LMM170BLF", "HaitaData", ds)
        ...
        'フォーカスの設定
        Call Me._G.SetFoucus(LMM170C.EventShubetsu.HENSHU)
    ...
    ''' <param name="frm"></param>
    ''' <remarks></remarks>
    Private Sub DeleteDataEvent(ByVal frm As LMM170F)

        '処理開始アクション
        Call Me._LMMConH.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM170C.EventShubetsu.SAKUJO) = False Then
            Call Me._LMMConH.EndAction(frm)  '終了処理
            Exit Sub
        End If
        ...
        '更新処理(排他処理)
        Dim rtnDs As DataSet = MyBase.CallWSA("LMM170BLF", "DeleteData", ds)
        ...
        'キャッシュ最新化
        'MyBase.LMCacheMasterData(LMConst.CacheTBL.ZIP)

        'メッセージ用請求先コード格納
        Dim zipNo As String = frm.txtZipNo.TextValue
        MyBase.ShowMessage(frm, "G002", New String() {frm.FunctionKey.F4ButtonName.Replace("　", String.Empty), String.Concat("[ 郵便番号 = ", zipNo, "]")})
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Private Sub SelectListEvent(ByVal frm As LMM170F)

        '処理開始アクション
        Call Me._LMMConH.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM170C.EventShubetsu.KENSAKU) = False Then
            Call Me._LMMConH.EndAction(frm) '終了処理
            Exit Sub
        End If
        ...
        'フォーカスの設定
        Call Me._G.SetFoucus(LMM170C.EventShubetsu.KENSAKU)
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Private Sub SelectData(ByVal frm As LMM170F, ByVal clearflg As Integer)
        ...
        'WSAクラス呼出
        '==========================
        Dim rtnDs As DataSet = Me._LMMConH.CallWSAAction(frm, "LMM170BLF", "SelectListData", _FindDs, lc, Nothing, mc)

        'Dim rtnDs As DataSet = Me._LMMConH.CallWSAAction(frm, _
        '                                         "LMM170BLF", "SelectListData", Me._FindDs _
    ...
    ''' <remarks></remarks>
    Private Sub SuccessSelect(ByVal frm As LMM170F, ByVal ds As DataSet)

        Dim dt As DataTable = ds.Tables(LMM170C.TABLE_NM_OUT)
    ...
    ''' <param name="frm"></param>
    ''' <remarks></remarks>
    Private Sub MasterShowEvent(ByVal frm As LMM170F)

        'カーソル位置の設定
        Dim objNm As String = frm.ActiveControl.Name()

        '権限チェック
        Dim rtnResult As Boolean = Me._V.IsAuthorityChk(LMM170C.EventShubetsu.MASTEROPEN)

        'カーソル位置チェック
        rtnResult = rtnResult AndAlso Me._V.IsFocusChk(objNm, LMM170C.EventShubetsu.MASTEROPEN)
        ...
        Call Me.ShowPopupControl(frm, objNm, LMM170C.EventShubetsu.MASTEROPEN)
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Private Function SaveChuiItemData(ByVal frm As LMM170F, ByVal eventShubetsu As LMM170C.EventShubetsu) As Boolean

        '処理開始アクション
        Call Me._LMMConH.StartAction(frm)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM170C.EventShubetsu.HOZON) = False Then
            Call Me._LMMConH.EndAction(frm)  '終了処理
            Return False
        End If
        ...
        Select Case frm.lblSituation.RecordStatus
            Case RecordStatus.NEW_REC
                '新規登録処理
                rtnDs = MyBase.CallWSA("LMM170BLF", "InsertData", ds)
            Case Else
                '更新処理(排他処理)
                rtnDs = MyBase.CallWSA("LMM170BLF", "UpdateData", ds)
        End Select
        ...
        '処理結果メッセージ表示
        Dim zipNo As String = frm.txtZipNo.TextValue
        MyBase.ShowMessage(frm, "G002", New String() {frm.FunctionKey.F11ButtonName.Replace("　", String.Empty), String.Concat("[ 郵便番号 = ", zipNo, "]")})
    ...
    ''' <remarks></remarks>
    Private Sub EnterAction(ByVal frm As LMM170F, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        '権限チェック
        rtnResult = rtnResult AndAlso Me._V.IsAuthorityChk(LMM170C.EventShubetsu.ENTER)

        'カーソル位置チェック
        rtnResult = rtnResult AndAlso Me._V.IsFocusChk(objNm, LMM170C.EventShubetsu.ENTER)
        ...
        Call Me.ShowPopupControl(frm, objNm, LMM170C.EventShubetsu.ENTER)
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Private Sub SprCellSelect(ByVal frm As LMM170F, ByVal e As FarPoint.Win.Spread.CellClickEventArgs)
    ...
    ''' <param name="e">イベント詳細</param>
    ''' <remarks></remarks>
    Private Sub SprCellLeave(ByVal frm As LMM170F, ByVal e As FarPoint.Win.Spread.LeaveCellEventArgs)
    ...
    ''' <remarks></remarks>
    Private Sub RowSelection(ByVal frm As LMM170F, ByVal rowNo As Integer)

        '権限チェック
        If Me._V.IsAuthorityChk(LMM170C.EventShubetsu.DCLICK) = False Then
            Call Me._LMMConH.EndAction(frm) '終了処理
            Exit Sub
        End If
        ...
        If LMConst.FLG.OFF.Equals(Me._LMMConV.GetCellValue(frm.sprDetail.ActiveSheet.Cells(rowNo, LMM170G.sprDetailDef.SYS_DEL_FLG.ColNo))) = True Then
    ...
    Private Function GetKenNmData() As DataSet
        ...
        'DataSet設定
        Dim ds As DataSet = New LMM170DS()
        Dim dt As DataTable = ds.Tables(LMM170C.TABLE_NM_IN)
        Dim dr As DataRow = dt.NewRow()
        dr.Item("USER_BR_CD") = LMUserInfoManager.GetNrsBrCd()
        dt.Rows.Add(dr)
        Return MyBase.CallWSA("LMM170BLF", "ComboData", ds)
...
#Region "DataSet設定"
    ...
    ''' <param name="frm">このハンドラクラスに紐づくフォーム</param>
    ''' <remarks></remarks>
    Private Sub SetDataSetInData(ByVal frm As LMM170F)

        Dim dt As DataTable = Me._FindDs.Tables(LMM170C.TABLE_NM_IN)
        Dim dr As DataRow = dt.NewRow()

        With frm.sprDetail.ActiveSheet

            dr("SYS_DEL_FLG") = Me._LMMConV.GetCellValue(.Cells(0, LMM170G.sprDetailDef.SYS_DEL_NM.ColNo))
            dr("ZIP_NO") = Me._LMMConV.GetCellValue(.Cells(0, LMM170G.sprDetailDef.ZIP_NO.ColNo))
            dr("JIS_CD") = Me._LMMConV.GetCellValue(.Cells(0, LMM170G.sprDetailDef.JIS_CD.ColNo))
            dr("KEN_N") = Me._LMMConV.GetCellValue(.Cells(0, LMM170G.sprDetailDef.KEN_N.ColNo))
            dr("CITY_N") = Me._LMMConV.GetCellValue(.Cells(0, LMM170G.sprDetailDef.CITY_N.ColNo))
            dr("TOWN_N") = Me._LMMConV.GetCellValue(.Cells(0, LMM170G.sprDetailDef.TOWN_N.ColNo))
    ...
    ''' <remarks></remarks>
    Private Sub SetDatasetUserItemData(ByVal frm As LMM170F, ByVal ds As DataSet)
        ...
        With frm
            ...
            dr("ZIP_NO") = .txtZipNo.TextValue.Trim()
            dr("KEN_K") = .txtKenK.TextValue.Trim()
            dr("CITY_K") = .txtCityK.TextValue.Trim()
            dr("TOWN_K") = .txtTownK.TextValue.Trim()
            dr("KEN_N") = .txtKenN.TextValue.Trim()
            dr("CITY_N") = .txtCityN.TextValue.Trim()
            dr("TOWN_N") = .txtTownN.TextValue.Trim()
    ...
    ''' <param name="ds">このハンドラクラスに紐づくデータセット</param>
    ''' <remarks></remarks>
    Private Sub SetDatasetDelData(ByVal frm As LMM170F, ByVal ds As DataSet)
        Dim dr As DataRow = ds.Tables(LMM170C.TABLE_NM_IN).NewRow()
        ...
        ds.Tables(LMM170C.TABLE_NM_IN).Rows.Add(dr)
    End Sub
...
#Region "PopUp"
    ...
    ''' <remarks></remarks>
    Private Function ShowPopupControl(ByVal frm As LMM170F, ByVal objNm As String, ByVal eventshubetsu As LMM170C.EventShubetsu) As Boolean
    ...
    ''' <remarks></remarks>
    Private Function SetReturnCustPop(ByVal frm As LMM170F, ByVal eventshubetsu As LMM170C.EventShubetsu) As Boolean
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Private Function ShowCustPopup(ByVal frm As LMM170F, ByVal eventshubetsu As LMM170C.EventShubetsu) As LMFormData
...
#Region "メッセージ設定"
    ...
    ''' <param name="frm">フォーム</param>
    ''' <remarks></remarks>
    Private Sub ShowGMessage(ByVal frm As LMM170F)
    ...
    ''' <param name="frm">フォーム</param>
    ''' <remarks></remarks>
    Private Sub SetGMessage(ByVal frm As LMM170F)
...
#Region "イベント振分け"
    ...
    ''' <remarks></remarks>
    Friend Sub FunctionKey1Press(ByRef frm As LMM170F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey2Press(ByRef frm As LMM170F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey4Press(ByRef frm As LMM170F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey9Press(ByRef frm As LMM170F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey10Press(ByRef frm As LMM170F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey11Press(ByRef frm As LMM170F, ByVal e As System.Windows.Forms.KeyEventArgs)

        MyBase.Logger.StartLog(MyBase.GetType.Name, "SaveChuiItemData")

        '保存処理
        Me.SaveChuiItemData(frm, LMM170C.EventShubetsu.HOZON)
    ...
    ''' <param name="e">ファンクションキーイベント</param>
    ''' <remarks></remarks>
    Friend Sub FunctionKey12Press(ByRef frm As LMM170F, ByVal e As System.Windows.Forms.KeyEventArgs)
    ...
    ''' <param name="e">イベント詳細</param>
    ''' <remarks></remarks>
    Friend Sub sprCellDoubleClick(ByRef frm As LMM170F, ByVal e As FarPoint.Win.Spread.CellClickEventArgs)
    ...
    ''' <remarks></remarks>
    Friend Sub LMM170F_KeyDown(ByVal frm As LMM170F, ByVal e As System.Windows.Forms.KeyEventArgs)

        MyBase.Logger.StartLog(MyBase.GetType.Name, "LMM170F_KeyDown")

        Call Me.EnterAction(frm, e)

        MyBase.Logger.EndLog(MyBase.GetType.Name, "LMM170F_KeyDown")
    ...
    Friend Sub ClosingForm(ByRef frm As LMM170F, ByVal e As FormClosingEventArgs)
    ...
    ''' <param name="e">イベント詳細</param>
    ''' <remarks></remarks>
    Friend Function CloseFormEvent(ByVal frm As LMM170F, ByVal e As FormClosingEventArgs) As Boolean
        ...
        If DispMode.EDIT.Equals(frm.lblSituation.DispMode) = True Then
            ...
            Select Case MyBase.ShowMessage(frm, "W002")
                Case MsgBoxResult.Yes  '「はい」押下時
                    If Me.SaveChuiItemData(frm, LMM170C.EventShubetsu.TOJIRU) = True Then
                        Return True
                    Else '保存失敗時
                        e.Cancel = True
                        Return False
    ...
    ''' <param name="e">イベント詳細情報</param>
    ''' <remarks></remarks>
    Friend Sub sprDetail_LeaveCell(ByVal frm As LMM170F, ByVal e As FarPoint.Win.Spread.LeaveCellEventArgs)
