...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMD515DAC = New LMD515DAC()
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMD515IN"
        Dim tableNmOut As String = "LMD515OUT"
    ...
    Private Function EditPrintDataSet(ByVal prtId As String, ByVal ds As DataSet) As DataSet
        ...
        Select Case prtId
            '2011/9/13 SBS)佐川　ソート処理削除(DACで既にソートしているため)
            ''LMD515の場合は営業所コード、荷主コード(大)、倉庫コード、
            ''棟番号、室番号、ZONEコード、ロケーション、商品コード、ロット番号の順番でソートする
            'Case "LMD515"
            '    'ソート対象データテーブル取得
            '    Dim outDt As DataTable = ds.Tables("LMD515OUT")
            '    'ソート条件設定
            '    Dim sort As String = "NRS_BR_CD ASC, CUST_CD_L ASC, WH_CD ASC, TOU_NO ASC, SITU_NO ASC, ZONE_CD ASC, LOCA ASC, GOODS_CD_CUST ASC,LOT_NO ASC"
            '    'ソート実行
            ...
            '        rtnDs.Tables("LMD515OUT").ImportRow(row)
            '    Next
