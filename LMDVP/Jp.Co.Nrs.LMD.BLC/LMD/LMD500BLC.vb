...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Dac As LMD500DAC = New LMD500DAC()
...
#Region "印刷処理"
    ...
    Private Function DoPrint(ByVal ds As DataSet) As DataSet
        ...
        '元のデータ
        Dim tableNmIn As String = "LMD500IN"
        Dim tableNmOut As String = "LMD500OUT"
        ...
        For Each dr As DataRow In ds.Tables(tableNmRpt).Rows
            ...
            prtDs = New DataSet
            '指定したレポートIDのデータを抽出する
            prtDs = comPrt.CallDataSet(ds.Tables("LMD500OUT"), dr.Item("RPT_ID").ToString())
            '帳票ごとの編集があるなら行う。
    ...
    Private Function EditPrintDataSet(ByVal prtId As String, ByVal ds As DataSet) As DataSet
        ...
        Select Case prtId
            'LMD500の場合は倉庫コード、荷主コード(大)、荷主コード(中)、棟番号、室番号、ZONEコード、ロケーション
            'ダメージ区分、荷主コード、荷主カテゴリ、ロット番号、シリアル番号、入荷日の順番でソートする
            Case "LMD500"
                'ソート対象データテーブル取得
                Dim outDt As DataTable = ds.Tables("LMD500OUT")
                'ソート条件設定
                Dim sort As String = "WH_CD ASC, CUST_CD_L ASC, CUST_CD_M ASC, TOU_NO ASC, SITU_NO ASC, ZONE_CD ASC, LOCA ASC, GOODS_COND_KB ASC, GOODS_CD_CUST ASC, SEARCH_KEY ASC, LOT_NO ASC, SERIAL_NO ASC, INKO_DATE ASC"
                'ソート実行
                ...
                For Each row As DataRow In dr
                    rtnDs.Tables("LMD500OUT").ImportRow(row)
                Next
