...
#Region "Method"
    ...
    Private Sub MakeCsv(ByVal prmDs As DataSet)
        ...
        Dim setOutDt As DataTable = setDs.Tables("LMC810OUT")
        setOutDt.Clear()

        '並び替えをする
        Dim outDr As DataRow() = prmDs.Tables("LMC810OUT").Select(Nothing, "OUTKA_NO_L,OUTKA_NO_M")
        ...
        For i As Integer = 0 To max
            ...
            ElseIf String.IsNullOrEmpty(outDr(i).Item("OUTKA_NO_L").ToString) = False AndAlso _
                   (outDr(i).Item("OUTKA_NO_L").ToString).Equals(outDr(i - 1).Item("OUTKA_NO_L").ToString) = True Then
        ...
        '保存先のCSVファイルのパス
        Dim csvPath As String = String.Concat(prmDs.Tables("LMC810OUT").Rows(0).Item("FILEPATH").ToString, _
                                              prmDs.Tables("LMC810OUT").Rows(0).Item("FILENAME").ToString, _
                                              prmDs.Tables("LMC810OUT").Rows(0).Item("SYS_DATE").ToString, _
                                               "-", _
                                              Left(prmDs.Tables("LMC810OUT").Rows(0).Item("SYS_TIME").ToString, 6), _
                                              ".csv")
        ...
        'ファイルを開く
        System.IO.Directory.CreateDirectory(prmDs.Tables("LMC810OUT").Rows(0).Item("FILEPATH").ToString)
    ...
    Private Function UpdateOkakenCsv(ByVal ds As DataSet) As DataSet
        ...
        '==== WSAクラス呼出（変更処理） ====
        Dim rtnDs As DataSet = MyBase.CallWSA("LMC810BLF", "UpdateOkakenCsv", ds)
