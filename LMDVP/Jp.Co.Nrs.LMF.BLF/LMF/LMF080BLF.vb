...
#Region "Field"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private _Blc As LMF080BLC = New LMF080BLC()
...
#Region "Const"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_810IN As String = "LMF810UNCHIN_CALC_IN"

    ''' <summary>
    ''' LMF810OUTテーブル
    ''' </summary>
    ''' <remarks></remarks>
    Private Const TABLE_NM_810OUT As String = "LMF810UNCHIN_CALC_OUT"
...
#Region "検索処理"
    ...
    Private Function SelectListData(ByVal ds As DataSet) As DataSet
        ...
        If MyBase.GetForceOparation() = False Then
            ...
            'データ件数取得
            ds = Me.BlcAccess(ds, LMF080BLF.ACTION_ID_COUNT)
            'メッセージの判定
...
#Region "まとめ"
    ...
    '    Dim dt As DataTable = ds.Tables(LMF080BLF.TABLE_NM_UNCHIN)
    ...
    '    Dim selectDt As DataTable = selectDs.Tables(LMF080BLF.TABLE_NM_UNCHIN)
    ...
    '        '値取得
    '        selectDs = Me.BlcAccess(selectDs, LMF080BLF.ACTION_ID_SELECT_UNCHIN)

    '        '取得できない場合、終了
    '        If MyBase.IsMessageExist() = True Then
    '            Return ds
    '        End If
    '        dr = selectDs.Tables(LMF080BLF.TABLE_NM_UNCHIN).Rows(0)
    ...
    '            'まとめ番号の設定
    '            .Item("SHIHARAI_GROUP_NO") = unsoL
    '            .Item("SHIHARAI_GROUP_NO_M") = unsoM
    ...
    '    '親レコードの計算した運賃を設定
    '    ds = Me.SetUnchinData(ds, oyaDr, New LMF810DS(), New LMF810BLC(), oyaDr.Item("SHIHARAI_TARIFF_CD").ToString())
    ...
    Private Function SetGroupData(ByVal ds As DataSet) As DataSet
        ...
        Dim dt As DataTable = ds.Tables(LMF080BLF.TABLE_NM_UNCHIN)
        Dim drs As DataRow() = dt.Select(String.Empty, " SHIHARAI_GROUP_NO , SHIHARAI_GROUP_NO_M , UNSO_NO_L , UNSO_NO_M ")
        Dim dr As DataRow = Nothing
        Dim setDr As DataRow = Nothing
        Dim rowMax As Integer = drs.Length - 1
        Dim colMax As Integer = dt.Columns.Count - 1
        ...
        Using scope As TransactionScope = MyBase.BeginTransaction()
            ...
            For i As Integer = 0 To rowMax
                ...
                If (seiqGroupNo).Equals(drs(i).Item("SHIHARAI_GROUP_NO").ToString) = False OrElse _
                    (seiqGroupNoM).Equals(drs(i).Item("SHIHARAI_GROUP_NO_M").ToString) = False Then
                    'まとめ番号が変わった時
                    ...
                    If String.IsNullOrEmpty(seiqGroupNo) = False Then
                        ...
                        '親レコードの計算した運賃を設定
                        ds = Me.SetUnchinData(ds, drs(oyaRowNo), New LMF810DS(), New LMF810BLC(), drs(oyaRowNo).Item("SHIHARAI_TARIFF_CD").ToString())

                        '按分処理
                        ds = Me.SetUncinDataMatome(ds, drs(oyaRowNo))

                    End If

                    '各種値のクリア
                    seiqGroupNo = drs(i).Item("SHIHARAI_GROUP_NO").ToString
                    seiqGroupNoM = drs(i).Item("SHIHARAI_GROUP_NO_M").ToString
                    oyaRowNo = i
                    soJuryo = 0
                    kyori = 0
                    maxKyori = 0
                ...
                drs(i).Item("SHIHARAI_GROUP_NO") = String.Empty
                drs(i).Item("SHIHARAI_GROUP_NO_M") = String.Empty
                ...
                'まとめ番号を設定
                dr.Item("SHIHARAI_GROUP_NO") = seiqGroupNo
                dr.Item("SHIHARAI_GROUP_NO_M") = seiqGroupNoM
            ...
            If String.IsNullOrEmpty(seiqGroupNo) = False Then
                ...
                drs(oyaRowNo).Item("DECI_KYORI") = drs(oyaRowNo).Item("SHIHARAI_KYORI")
    ...
    Private Function SetGroupCancellData(ByVal ds As DataSet) As DataSet
        ...
        Call Me.UpdateCommonAction(ds, LMF080BLF.ActionType.GROUP_CANCELL, System.Reflection.MethodBase.GetCurrentMethod.Name, False)
    ...
    Private Function SetInitData(ByVal ds As DataSet) As DataSet
        ...
        Dim shiharaiDs As DataSet = Nothing
        For i As Integer = 0 To max

            dr = dt.Rows(i)

            ''支払情報を確定、管理情報に設定
            'unchin = dr.Item("SHIHARAI_UNCHIN").ToString()
            'city = dr.Item("SHIHARAI_CITY_EXTC").ToString()
            'wint = dr.Item("SHIHARAI_WINT_EXTC").ToString()
            'rely = dr.Item("SHIHARAI_RELY_EXTC").ToString()
            'toll = dr.Item("SHIHARAI_TOLL").ToString()
            'insu = dr.Item("SHIHARAI_INSU").ToString()
            'juryo = dr.Item("SHIHARAI_WT").ToString()
            'kyori = dr.Item("SHIHARAI_KYORI").ToString()
            'kosu = dr.Item("SHIHARAI_NG_NB").ToString()
            ...
            '--------------------------------------------------------------------------------------------------
            ' LMF810をCallして支払運賃を再計算する
            '--------------------------------------------------------------------------------------------------
            'データセット設定
            Dim outDr As DataRow = Me.SetShiharaiInDataSet(ds, dr)

            With dr

                .Item("SHIHARAI_UNCHIN") = outDr.Item("UNCHIN").ToString()
                .Item("SHIHARAI_CITY_EXTC") = outDr.Item("CITY_EXTC").ToString()
                .Item("SHIHARAI_WINT_EXTC") = outDr.Item("WINT_EXTC").ToString()
                .Item("SHIHARAI_RELY_EXTC") = outDr.Item("RELY_EXTC").ToString()
                .Item("SHIHARAI_TOLL") = outDr.Item("TOLL").ToString()
                .Item("SHIHARAI_INSU") = outDr.Item("INSU").ToString()
                ...
                .Item("DECI_INSU") = outDr.Item("INSU").ToString()

                .Item("KANRI_UNCHIN") = outDr.Item("UNCHIN").ToString()
                .Item("KANRI_CITY_EXTC") = outDr.Item("CITY_EXTC").ToString()
                .Item("KANRI_WINT_EXTC") = outDr.Item("WINT_EXTC").ToString()
                .Item("KANRI_RELY_EXTC") = outDr.Item("RELY_EXTC").ToString()
                .Item("KANRI_TOLL") = outDr.Item("TOLL").ToString()
                .Item("KANRI_INSU") = outDr.Item("INSU").ToString()

                .Item("DECI_NG_NB") = .Item("SHIHARAI_NG_NB").ToString()
                .Item("DECI_KYORI") = .Item("SHIHARAI_KYORI").ToString()
                .Item("DECI_WT") = .Item("SHIHARAI_WT").ToString()
    ...
    ''' <param name="ds">DataSet</param>
    ''' <remarks></remarks>
    Friend Function SetShiharaiInDataSet(ByVal ds As DataSet, ByVal dr As DataRow) As DataRow

        Dim unchinDs As DataSet = New LMF810DS()
        Dim insRows As DataRow = unchinDs.Tables(LMF080BLF.TABLE_NM_810IN).NewRow
        ...
        With insRows
            ...
            .Item("UNSO_PKG_NB") = dr.Item("SHIHARAI_NG_NB").ToString
            'END YANAI 要望番号1424 支払処理
            .Item("NB_UT") = dr.Item("SHIHARAI_PKG_UT").ToString
            'START YANAI 要望番号1424 支払処理
            '.Item("UNSO_WT") = dr.Item("DECI_WT").ToString
            .Item("UNSO_WT") = dr.Item("SHIHARAI_WT").ToString
            'END YANAI 要望番号1424 支払処理
            .Item("UNSO_ONDO_KB") = dr.Item("UNSO_ONDO_KB").ToString
            .Item("TARIFF_BUNRUI_KB") = dr.Item("SHIHARAI_TARIFF_BUNRUI_KB").ToString
            .Item("VCLE_KB") = dr.Item("VCLE_KB").ToString
            .Item("MOTO_DATA_KB") = dr.Item("MOTO_DATA_KB").ToString
            .Item("SHIHARAI_TARIFF_CD") = dr.Item("SHIHARAI_TARIFF_CD").ToString
            .Item("SHIHARAI_ETARIFF_CD") = dr.Item("SHIHARAI_ETARIFF_CD").ToString
            ...
            .Item("KYORI") = dr.Item("SHIHARAI_KYORI").ToString()
            'END YANAI 要望番号1424 支払処理
            .Item("DANGER_KB") = dr.Item("SHIHARAI_DANGER_KB").ToString()
            .Item("GOODS_CD_NRS") = String.Empty

        End With

        'データセットに追加
        unchinDs.Tables("LMF810UNCHIN_CALC_IN").Rows.Add(insRows)

        'プログラム起動
        unchinDs = MyBase.CallBLC(New LMF810BLC(), LMF080BLF.ACTION_ID_UNCHINI_CALC, unchinDs)

        '戻り値の反映
        Dim outDr As DataRow = unchinDs.Tables(LMF080BLF.TABLE_NM_810OUT).Rows(0)
...
#Region "一括変更"
    ...
    Private Function SaveUnchinItemData(ByVal ds As DataSet) As DataSet
        ...
        'タリフを更新しない場合、通常の一括変更
        Select Case ds.Tables(LMF080BLF.TABLE_NM_UNCHIN).Rows(0).Item("ITEM_DATA").ToString()

            Case LMF080BLF.SHUSEI_SHIHARAI

                '支払先の一括更新
                Return Me.SetComSaveData(ds, LMF080BLF.ACTION_ID_SAVE_SEIQTO)

            Case LMF080BLF.SHUSEI_TARIFF, LMF080BLF.SHUSEI_YOKO
            Case Else
    ...
    Private Function SaveUnchinTariffData(ByVal ds As DataSet) As DataSet
        ...
        Call Me.UpdateCommonAction(ds, LMF080BLF.ActionType.IKKATU_TARIFF, System.Reflection.MethodBase.GetCurrentMethod.Name, False, New LMF810DS(), New LMF810BLC())
    ...
    ''' <param name="blc">LMF810BLC</param>
    ''' <returns>DataSet</returns>
    ''' <remarks></remarks>
    Private Function SetIkkatuTariffUnchinData(ByVal ds As DataSet, ByVal unchinDs As DataSet, ByVal blc As LMF810BLC) As DataSet

        Dim dt As DataTable = ds.Tables(LMF080BLF.TABLE_NM_UNCHIN)
        Dim dr As DataRow = dt.Rows(0)

        'まとめレコードの場合
        If String.IsNullOrEmpty(dr.Item("SHIHARAI_GROUP_NO").ToString()) = False Then
    ...
    Private Function UpdUnchinData(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            Using scope As TransactionScope = MyBase.BeginTransaction()
                ...
                '値のクリア
                inTbl = setDs.Tables(LMF080BLF.TABLE_NM_UNCHIN)
                inTbl.Clear()
...
#Region "共通"
    ...
    Private Function SetComSaveData(ByVal ds As DataSet, ByVal actionId As String) As DataSet
        ...
        Call Me.UpdateCommonAction(ds, LMF080BLF.ActionType.NOMAL, actionId, False)
...
#Region "ユーティリティ"
    ...
    Private Function SetUnchinData(ByVal ds As DataSet _
                                   ...
                                   , ByVal blc As LMF810BLC _
                                   , ByVal tariffCd As String _
                                   ) As DataSet

        'パラメータ設定
        Dim inTbl As DataTable = unchinDs.Tables(LMF080BLF.TABLE_NM_810IN)
        ...
        With inDr
            ...
            .Item("ACTION_FLG") = LMF080BLF.ACT_FGL_GYOM
            .Item("NRS_BR_CD") = dr.Item("NRS_BR_CD").ToString()
            .Item("ARR_PLAN_DATE") = dr.Item("ARR_PLAN_DATE").ToString()
            .Item("MOTO_DATA_KB") = dr.Item("MOTO_DATA_KB").ToString()
            ...
            .Item("DEST_CD") = dr.Item("DEST_CD").ToString()
            .Item("DEST_JIS") = dr.Item("DEST_JIS").ToString()
            .Item("VCLE_KB") = dr.Item("SHIHARAI_SYARYO_KB").ToString()
            .Item("UNSO_ONDO_KB") = dr.Item("UNSO_ONDO_KB").ToString()
            .Item("SIZE_KB") = dr.Item("SIZE_KB").ToString()
            .Item("UNSO_DATE") = Me.SetUnsoDate(dr)
        ...
        'プログラム起動
        unchinDs = MyBase.CallBLC(blc, LMF080BLF.ACTION_ID_UNCHINI_CALC, unchinDs)
    ...
    Private Function GetOyaDataSql(ByVal dr As DataRow) As String
        ...
        '通常レコードの場合
        If String.IsNullOrEmpty(dr.Item("SHIHARAI_GROUP_NO").ToString()) = True Then
            Return String.Empty
        End If

        Return String.Concat(" UNSO_NO_L = '", dr.Item("SHIHARAI_GROUP_NO").ToString(), "' " _
                                          , " AND UNSO_NO_M = '", dr.Item("SHIHARAI_GROUP_NO_M").ToString(), "' ")
    ...
    Private Function SetUnsoDate(ByVal dr As DataRow) As String
        ...
        With dr
            ...
            Select Case .Item("MOTO_DATA_KB").ToString()
                ...
                Case LMF080BLF.MOTO_DATA_NYUKA
                ...
                Case Else
                    ...
                    '    Case LMF080BLF.CALC_SHUKKA

                    '        SetUnsoDate = .Item("OUTKA_PLAN_DATE").ToString()

                    '    Case LMF080BLF.CALC_NYUKA
    ...
    Private Function UpdateCommonAction(ByVal ds As DataSet _
                                            , ByVal actionType As LMF080BLF.ActionType _
                                            , ByVal actionId As String _
                                            , ByVal chkFlg As Boolean _
                                            , Optional ByVal unchinDs As DataSet = Nothing _
                                            , Optional ByVal blc As LMF810BLC = Nothing _
                                            ) As DataSet

        Dim chkDt As DataTable = ds.Tables(LMF080BLF.TABLE_NM_UNCHIN).Copy
        Dim dt As DataTable = Me.SetLoopDataSet(ds).Tables(LMF080BLF.TABLE_NM_UNCHIN)
        Dim upDs As DataSet = ds.Copy
        Dim upDt As DataTable = upDs.Tables(LMF080BLF.TABLE_NM_UNCHIN)
        ...
        While dt.Rows.Count <> 0
            ...
            For i As Integer = 0 To cnt
                ...
                If unsoL.Equals(chkUnsoL) = False Then
                    ...
                    selectDs = Me.BlcAccess(selectDs, LMF080BLF.ACTION_ID_SELECT_INIT_UNCHIN)
                    recCnt = selectDt.Rows.Count - 1
                    For j As Integer = 0 To recCnt
                        Call Me.SetUpdData(ds, dt, upDt, actionType, selectDt.Rows(j).Item("SHIHARAI_GROUP_NO").ToString(), selectDt.Rows(j).Item("SHIHARAI_GROUP_NO_M").ToString(), unchinDs, blc)
                    Next
    ...
    Private Function SetUpdData(ByVal ds As DataSet _
        ...
        For i As Integer = max To 0 Step -1
            ...
            Select Case actionType
                ...
                Case LMF080BLF.ActionType.GROUP_CANCELL

                    'まとめ解除時に再計算
                    selectDs = Me.SetInitData(selectDs)

                Case LMF080BLF.ActionType.IKKATU_TARIFF
            ...
            '取得した情報を保持
            selectDt = selectDs.Tables(LMF080BLF.TABLE_NM_UNCHIN)
    ...
    Private Function SetLoopDataSet(ByVal ds As DataSet) As DataSet
        ...
        Dim drs As DataRow() = setDt.Select(String.Empty, "SHIHARAI_GROUP_NO desc,SHIHARAI_GROUP_NO_M desc")
        Dim setDr As DataRow = Nothing
        Dim max As Integer = 0
        ...
        While setDt.Rows.Count <> 0
            ...
            '通常レコードの場合、自レコードを削除
            If String.IsNullOrEmpty(setDr.Item("SHIHARAI_GROUP_NO").ToString()) = True Then
            ...
            'まとめレコードの場合
            drs = setDt.Select(String.Concat("SHIHARAI_GROUP_NO = '", setDr.Item("SHIHARAI_GROUP_NO").ToString(), "' AND SHIHARAI_GROUP_NO_M = '", setDr.Item("SHIHARAI_GROUP_NO_M").ToString(), "' "))
...
#Region "計算"
    ...
    Private Function SetUncinData(ByVal ds As DataSet) As DataSet
        ...
        For i As Integer = 0 To max
            ...
            '重量の合計
            soJuryo += Convert.ToDecimal(dt.Rows(i).Item("SHIHARAI_WT").ToString())
        ...
        For i As Integer = 0 To max
            ...
            '重量の設定
            juryo = Convert.ToDecimal(dr.Item("SHIHARAI_WT").ToString())
    ...
    '''' <returns>DataSet</returns>
    '''' <remarks></remarks>
    'Private Function SetUncinDataMatome(ByVal ds As DataSet, _
    '                                 ByVal oyadr As DataRow) As DataSet

    '    Dim dr() As DataRow = ds.Tables(LMF080BLF.TABLE_NM_UNCHIN).Select(String.Concat("SHIHARAI_GROUP_NO = '", oyadr.Item("SHIHARAI_GROUP_NO").ToString, "' AND ", _
    '                                                                                    "SHIHARAI_GROUP_NO_M = '", oyadr.Item("SHIHARAI_GROUP_NO_M").ToString, "'"))
    ...
    '        '重量の合計
    '        soJuryo += Convert.ToDecimal(dr(i).Item("SHIHARAI_WT").ToString())
    ...
    '        '重量の設定
    '        juryo = Convert.ToDecimal(dr(i).Item("SHIHARAI_WT").ToString())
    ...
    Private Function SetUncinDataMatome(ByVal ds As DataSet, _
        ...
        For i As Integer = 0 To max
            ...
            '運賃の設定
            dr(i).Item("KANRI_UNCHIN") = dr(i).Item("DECI_UNCHIN").ToString

            '都市の設定
            dr(i).Item("KANRI_CITY_EXTC") = dr(i).Item("DECI_CITY_EXTC").ToString

            '冬期の設定
            dr(i).Item("KANRI_WINT_EXTC") = dr(i).Item("DECI_WINT_EXTC").ToString

            '中継料の設定
            dr(i).Item("KANRI_RELY_EXTC") = dr(i).Item("DECI_RELY_EXTC").ToString

            '通行料の設定
            dr(i).Item("KANRI_TOLL") = dr(i).Item("DECI_TOLL").ToString

            '保険料の設定
            dr(i).Item("KANRI_INSU") = dr(i).Item("DECI_INSU").ToString
